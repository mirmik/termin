...
namespace termin::openxr {
    ...
    termin::Vec3 xr_direction_to_scene_direction(const XrVector3f& p) {
        return termin::Vec3{static_cast<double>(p.x), static_cast<double>(-p.z), static_cast<double>(p.y)};
    }
    ...
    std::array<float, 16> multiply_matrix(const std::array<float, 16>& a, const std::array<float, 16>& b) {
