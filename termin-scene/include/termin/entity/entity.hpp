...
namespace termin {
    ...
    public:
        ...
        Entity(tc_entity_handle h)
            : _h(h) {}

        // Construct from pool handle + id
        Entity(tc_entity_pool_handle pool_handle, tc_entity_id id)
        ...
        static Entity
        create_with_uuid(tc_entity_pool_handle pool_handle, const std::string& name, const std::string& uuid);
        ...
        const char* uuid() const {
            return tc_entity_uuid(_h);
        }
        void set_uuid(const char* uuid) {
            tc_entity_set_uuid(_h, uuid);
        }
        uint64_t runtime_id() const {
            auto* p = pool_ptr();
            return p ? tc_entity_pool_runtime_id(p, _h.id) : 0;
        }
        uint32_t pick_id() const {
            auto* p = pool_ptr();
            return p ? tc_entity_pool_pick_id(p, _h.id) : 0;
        }

        // --- Name ---

        const char* name() const {
            return tc_entity_name(_h);
        }
        void set_name(const std::string& n) {
            tc_entity_set_name(_h, n.c_str());
        }

        // --- Transform ---

        // Get position/rotation/scale
        void get_local_position(double* xyz) const {
            tc_entity_get_local_position(_h, xyz);
        }
        void set_local_position(const double* xyz) {
            tc_entity_set_local_position(_h, xyz);
        }

        void get_local_rotation(double* xyzw) const {
            tc_entity_get_local_rotation(_h, xyzw);
        }
        void set_local_rotation(const double* xyzw) {
            tc_entity_set_local_rotation(_h, xyzw);
        }

        void get_local_scale(double* xyz) const {
            tc_entity_get_local_scale(_h, xyz);
        }
        void set_local_scale(const double* xyz) {
            tc_entity_set_local_scale(_h, xyz);
        }

        void get_global_position(double* xyz) const {
            auto* p = pool_ptr();
            if (p)
                tc_entity_pool_get_global_position(p, _h.id, xyz);
        }
        void get_world_matrix(double* m16) const {
            tc_entity_get_world_matrix(_h, m16);
        }

        void mark_transform_dirty() {
            tc_entity_mark_dirty(_h);
        }
        ...
        bool visible() const {
            return tc_entity_visible(_h);
        }
        void set_visible(bool v) {
            tc_entity_set_visible(_h, v);
        }

        bool enabled() const {
            return tc_entity_enabled(_h);
        }
        void set_enabled(bool v) {
            tc_entity_set_enabled(_h, v);
        }

        bool pickable() const {
            return tc_entity_pickable(_h);
        }
        void set_pickable(bool v) {
            tc_entity_set_pickable(_h, v);
        }

        bool selectable() const {
            return tc_entity_selectable(_h);
        }
        void set_selectable(bool v) {
            tc_entity_set_selectable(_h, v);
        }

        int priority() const {
            return tc_entity_priority(_h);
        }
        void set_priority(int p) {
            tc_entity_set_priority(_h, p);
        }

        uint64_t layer() const {
            return tc_entity_layer(_h);
        }
        void set_layer(uint64_t l) {
            tc_entity_set_layer(_h, l);
        }

        uint64_t flags() const {
            return tc_entity_flags(_h);
        }
        void set_flags(uint64_t f) {
            tc_entity_set_flags(_h, f);
        }
        ...
        bool replace_component_at_checked(size_t index, tc_component* expected, tc_component* replacement) {
            return valid() &&
                   tc_entity_pool_replace_component_at_checked(pool_ptr(), id(), index, expected, replacement);
        }

        size_t component_count() const {
            return tc_entity_component_count(_h);
        }
        tc_component* component_at(size_t index) const {
            return tc_entity_component_at(_h, index);
        }
        ...
        static nos::trent make_clone_payload(const nos::trent& data,
                                             const std::string& name_suffix,
                                             std::unordered_map<std::string, std::string>& uuid_remap);
        static void remap_entity_refs(nos::trent& data, const std::unordered_map<std::string, std::string>& uuid_remap);
        static Entity deserialize_hierarchy(const nos::trent& data, tc_scene_handle scene, const Entity& parent);
        ...
        void deserialize_components_trent(const nos::trent& data,
        ...
        tc_entity_pool* pool() const {
            return pool_ptr();
        }
        tc_entity_id id() const {
            return _h.id;
        }

        // Get pool handle (safe, never dangling)
        tc_entity_pool_handle pool_handle() const {
            return _h.pool;
        }
