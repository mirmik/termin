...
namespace termin {
    ...
    public:
        ComponentTypeDescriptorBuilder(const char* type_name,
                                       const char* owner,
                                       const char* parent,
        ...
        template <typename T>
        static ComponentTypeDescriptorBuilder
        native(const char* type_name, const char* owner, const char* parent = "CxxComponent");
    ...
    namespace detail {
        template <typename Base, typename Derived, typename = void> struct is_base_of_safe : std::false_type {};

        template <typename Base, typename Derived>
        struct is_base_of_safe<Base, Derived, std::enable_if_t<sizeof(Base) != 0 && std::is_base_of_v<Base, Derived>>>
            : std::true_type {};
    } // namespace detail

    template <typename T> void mark_drawable_if_base(const char* name) {
        (void)name;
    }

    // Factory data stored in static variables per template instantiation
    template <typename T> struct CxxComponentFactoryData {
    ...
    template <typename T>
    ComponentTypeDescriptorBuilder
    ComponentTypeDescriptorBuilder::native(const char* type_name, const char* owner, const char* parent) {
        ...
        return ComponentTypeDescriptorBuilder(
            ...
            owner,
            parent,
            tc_runtime_owned_factory_make(&CxxComponentFactoryData<T>::create, nullptr, nullptr),
            TC_CXX_COMPONENT,
            false);
    }
