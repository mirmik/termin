...
namespace {
    ...
    int register_clone_ref_inspect_fields() {
        ...
        auto descriptor =
            termin::ComponentTypeDescriptorBuilder::native<CloneRefComponent>("CloneRefComponent", "termin-scene-test");
        ...
        TEST_ASSERT(
            (inspect.add<CloneRefComponent, std::string>(
                "CloneRefComponent", &CloneRefComponent::plain_uuid_string, "plain_uuid", "Plain UUID", "string")),
...
int main() {
    ...
    TEST_ASSERT(register_clone_ref_inspect_fields() == 0, "CloneRefComponent descriptor should register");
    ...
    termin::Entity standalone = termin::Entity::create_with_uuid(
        termin::Entity::standalone_pool(), "Standalone", "entity-migration-stable-uuid");
    ...
    TEST_ASSERT(std::string(migrated.uuid()) == standalone_uuid, "scene migration should preserve UUID");
    ...
    termin::Entity collision_source = termin::Entity::create_with_uuid(
        termin::Entity::standalone_pool(), "CollisionSource", "entity-migration-collision-uuid");
    termin::Entity collision_destination = termin::Entity::create_with_uuid(
        migration_scene.entity_pool(), "CollisionDestination", "entity-migration-collision-uuid");
    ...
    CloneRefComponent* component = dynamic_cast<CloneRefComponent*>(termin::CxxComponent::from_tc(raw_component));
    ...
    TEST_ASSERT(source_data["components"].as_list()[0]["data"]["target"]["uuid"].as_string() ==
                    std::string(child.uuid()),
    ...
    const std::string cloned_child_uuid = clone_data["children"].as_list()[0]["uuid"].as_string();
    TEST_ASSERT(clone_data["components"].as_list()[0]["data"]["target"]["uuid"].as_string() ==
                    std::string(child.uuid()),
    ...
    const std::string component_plain_uuid = clone_data["components"].as_list()[0]["data"]["plain_uuid"].as_string();
    TEST_ASSERT(component_plain_uuid == std::string(child.uuid()),
                "clone serialization should not remap untyped uuid-looking strings");

    termin::Entity clone = termin::Entity::deserialize_hierarchy(clone_data, scene.handle(), parent);
    ...
    TEST_ASSERT(cloned_component->target == cloned_child,
                "clone component entity ref should be remapped to cloned child");
    ...
    TEST_ASSERT(ordered_children[0] == child_a && ordered_children[1] == child_c && ordered_children[2] == child_b,
    ...
    TEST_ASSERT(order_data["entities"].as_list()[1]["children"].as_list()[1]["name"].as_string() == "ChildC",
                "scene serialization should preserve child order");

    termin::TcSceneRef restored_order_scene = termin::TcSceneRef::create("sibling-order-restored-test");
    TEST_ASSERT(restored_order_scene.load_from_data(order_data) == 6, "ordered scene should deserialize every entity");
    ...
    TEST_ASSERT(std::string(restored_roots[0].name()) == "Third" && std::string(restored_roots[1].name()) == "First" &&
