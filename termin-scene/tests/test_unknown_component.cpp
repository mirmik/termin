...
namespace {
    ...
    bool register_reloadable_component() {
        ...
        (void)descriptor.inspect().add<ReloadableComponent, int>(
            &ReloadableComponent::value, tc::InspectFieldSpec{"ReloadableComponent", "value", "Value", "int"});
        return descriptor.commit();
    }

    bool register_test_component_types() {
        if (!register_reloadable_component())
            return false;
        ...
        (void)secondary.inspect().add<SecondaryComponent, int>(
            &SecondaryComponent::amount, tc::InspectFieldSpec{"SecondaryComponent", "amount", "Amount", "int"});
        if (!secondary.commit())
            return false;
    ...
    template <typename T> T* create_registered_component(const char* type_name) {
    ...
    bool register_owner_scoped_component(const char* owner) {
        ...
        (void)descriptor.inspect().add<OwnerScopedComponent, int>(
            &OwnerScopedComponent::value, tc::InspectFieldSpec{"OwnerScopedComponent", "value", "Value", "int"});
        return descriptor.commit();
    }

    bool prepare_component_unload_for_test(const char* type_name, void*, void*) {
        if (!type_name) {
            return false;
        }
        ...
        std::string error;
        if (!termin::prepare_registered_components_to_unknown({type_name}, plan, &error)) {
            return false;
        }
        return plan.commit(&error) && tc_component_registry_instance_count(type_name) == 0;
    }
    ...
    int test_module_owner_registration_cleanup() {
        ...
        TEST_ASSERT(register_owner_scoped_component(module_id), "owned component descriptor committed");

        TEST_ASSERT(components.owner_of("OwnerScopedComponent") == module_id, "component owner captured");
        TEST_ASSERT(inspect.owner_of("OwnerScopedComponent") == module_id, "inspect owner captured");
        TEST_ASSERT(components.has("OwnerScopedComponent"), "owned component registered");
        TEST_ASSERT(inspect.find_field("OwnerScopedComponent", "value") != nullptr, "owned inspect field registered");

        TEST_ASSERT(tc_runtime_type_registry_unregister_owner(module_id) == 1, "runtime owner cleanup count");
        TEST_ASSERT(!components.has("OwnerScopedComponent"), "owned component unregistered");
        TEST_ASSERT(inspect.find_field("OwnerScopedComponent", "value") == nullptr, "owned inspect field removed");
        TEST_ASSERT(tc_runtime_type_registry_unregister_owner(module_id) == 0, "runtime owner cleanup idempotent");

        constexpr const char* standalone_owner = "standalone_owner_cleanup_test";
        auto standalone = termin::ComponentTypeDescriptorBuilder::native<OwnerScopedComponent>("OwnerScopedComponent",
                                                                                               standalone_owner);
        ...
        TEST_ASSERT(components.unregister_owner(module_id) == 0, "unrelated owner cleanup does not remove component");
        TEST_ASSERT(components.has("OwnerScopedComponent"), "explicitly owned component survives unrelated cleanup");
        TEST_ASSERT(components.unregister_owner(standalone_owner) == 1, "standalone owner cleanup removes component");
    ...
    int test_module_owner_cleanup_prepares_live_components() {
        ...
        auto* owner_scoped = dynamic_cast<OwnerScopedComponent*>(termin::CxxComponent::from_tc(component));
        ...
        tc_component_registry_set_prepare_unload_callback(prepare_component_unload_for_test, nullptr);
        const size_t removed = tc_runtime_type_registry_unregister_owner_with_context(module_id, nullptr);
        ...
        TEST_ASSERT(!components.has("OwnerScopedComponent"), "owned component registration removed after prepare");
        ...
        TEST_ASSERT(unknown->original_type == "OwnerScopedComponent", "UnknownComponent preserves original type");
    ...
    int test_registered_component_degradation_uses_runtime_instance_lists() {
        ...
        auto* first_component = create_registered_component<ReloadableComponent>("ReloadableComponent");
        auto* second_component = create_registered_component<ReloadableComponent>("ReloadableComponent");
        TEST_ASSERT(first_component && second_component, "registered-list components created");
        ...
        TEST_ASSERT(tc_component_registry_instance_count("ReloadableComponent") == 2, "both runtime instances linked");

        termin::UnknownComponentDegradationPlan plan;
        std::string error;
        TEST_ASSERT(termin::prepare_registered_components_to_unknown({"ReloadableComponent"}, plan, &error),
                    "registered-list component batch prepared");
        TEST_ASSERT(plan.size() == 2, "plan includes instances from both scenes without scene inventory");
        ...
        TEST_ASSERT(upgraded.upgraded == 2 && upgraded.failed == 0, "registered-list upgrade restores both scenes");
        auto* restored_first = static_cast<ReloadableComponent*>(
            termin::CxxComponent::from_tc(first_entity.get_component_by_type_name("ReloadableComponent")));
        auto* restored_second = static_cast<ReloadableComponent*>(
            termin::CxxComponent::from_tc(second_entity.get_component_by_type_name("ReloadableComponent")));
    ...
    int test_cpp_inspect_registry_roundtrip() {
        ...
        TEST_ASSERT(reg.find_field("ReloadableComponent", "value") != nullptr,
                    "ReloadableComponent.value field registered");
        TEST_ASSERT(reg.find_field("ReloadableComponent", "update_priority") != nullptr,
                    "ReloadableComponent inherits update priority");
        TEST_ASSERT(reg.find_field("ReloadableComponent", "fixed_update_priority") != nullptr,
                    "ReloadableComponent inherits fixed-update priority");
    ...
    int test_degrade_upgrade_roundtrip() {
        ...
        termin::UnknownComponentStats degraded = termin::degrade_components_to_unknown(scene, {"ReloadableComponent"});
    ...
    int test_degrade_filtering() {
        ...
        termin::UnknownComponentStats upgraded = termin::upgrade_unknown_components(scene, {"ReloadableComponent"});
        ...
        TEST_ASSERT(entity.get_component_by_type_name("UnknownComponent") == nullptr,
                    "unknown component removed after upgrade");
    ...
    int test_upgrade_requires_registered_type() {
        ...
        TEST_ASSERT(entity.get_component_by_type_name("UnknownComponent") != nullptr,
                    "unknown component remains after blocked upgrade");
    ...
    int test_auto_add_required_component() {
        ...
        TEST_ASSERT(entity.get_component_by_type_name("NeedsBaseComponent") != nullptr, "dependent component added");
    ...
    int test_requirement_satisfied_by_derived_component() {
        ...
        for (size_t i = 0; i < entity.component_count(); ++i) {
            ...
            if (!tc)
                continue;
            if (termin::ComponentRegistry::instance().is_a(tc_component_type_name(tc), "RequiredBaseComponent")) {
                base_like_count++;
            }
        }

        TEST_ASSERT(base_like_count == 1, "derived component satisfies requirement without duplicate");
    ...
    int test_serialized_requirements_load_before_dependents() {
        ...
        TEST_ASSERT(entity.component_count() == 2, "serialized required component is not duplicated");
        TEST_ASSERT(std::string(tc_component_type_name(entity.component_at(0))) == "RequiredBaseComponent",
                    "serialized requirement loads before dependent");
        TEST_ASSERT(std::string(tc_component_type_name(entity.component_at(1))) == "NeedsBaseComponent",
    ...
    int test_requirement_cycle_blocks_add() {
        ...
        TEST_ASSERT(entity.get_component_by_type_name("CycleAComponent") == nullptr, "cycle root component not added");
    ...
    int test_unknown_only_deserialization() {
        ...
        scene.from_json_string(json, termin::ComponentDeserializationMode::UnknownOnly);
        ...
        auto* unknown = static_cast<termin::UnknownComponent*>(termin::CxxComponent::from_tc(unknown_tc));
        TEST_ASSERT(unknown != nullptr, "UnknownComponent cast succeeds");
        TEST_ASSERT(unknown->original_type == "ReloadableComponent", "original type preserved in UnknownOnly");
        TEST_ASSERT(unknown->original_data.type == TC_VALUE_DICT, "original data stored as dict");
    ...
    int test_unknown_only_auto_upgrade_keeps_default_enabled() {
        ...
        scene.from_json_string(
            json, termin::ComponentDeserializationMode::UnknownOnly, termin::UnknownUpgradeStrategy{}, true);

        const auto entities = scene.get_all_entities();
        TEST_ASSERT(entities.size() == 1, "single entity created");
        termin::Entity entity = entities.front();
        TEST_ASSERT(entity.valid(), "entity created");
        TEST_ASSERT(entity.get_component_by_type_name("UnknownComponent") == nullptr, "UnknownComponent upgraded away");
    ...
    int test_unknown_upgrade_is_lossless_on_schema_errors() {
        ...
        for (size_t case_index = 0; case_index < 2; ++case_index) {
            ...
            std::string error;
            TEST_ASSERT(!termin::upgrade_unknown_component_ref(entity, unknown_tc, &error), "invalid payload rejected");
    ...
    int test_custom_upgrade_strategy() {
        ...
        termin::UnknownUpgradeStrategy strategy = [](const termin::UnknownComponent& unknown,
                                                     const termin::Entity&,
            ...
            tc_value* value = tc_value_dict_get(const_cast<tc_value*>(&unknown.original_data), "value");
            ...
            auto decision = termin::UnknownUpgradeDecision::custom("SecondaryComponent", &target_data);
            tc_value_free(&target_data);
            return decision;
        };

        scene.from_json_string(json, termin::ComponentDeserializationMode::UnknownOnly, strategy, true);
        ...
        auto* upgraded = dynamic_cast<SecondaryComponent*>(entity.get_component<SecondaryComponent>());
    ...
    int test_component_degradation_prepare_is_all_or_zero() {
        ...
        for (size_t failure_index = 0; failure_index < 3; ++failure_index) {
            ...
            auto* first_component = create_registered_component<ReloadableComponent>("ReloadableComponent");
            auto* middle_component = create_registered_component<ReloadableComponent>("ReloadableComponent");
            auto* last_component = create_registered_component<ReloadableComponent>("ReloadableComponent");
            TEST_ASSERT(first_component && middle_component && last_component, "prepare test components created");
            first_entity.add_component(first_component);
            ...
            std::string error;
            TEST_ASSERT(
                !termin::prepare_components_to_unknown({first, second}, {"ReloadableComponent"}, plan, &error, hooks),
        ...
        for (size_t failure_index = 0; failure_index < 3; ++failure_index) {
            ...
            termin::UnknownComponentDegradationPlan plan;
            std::string error;
            TEST_ASSERT(!termin::prepare_components_to_unknown({scene}, {"ReloadableComponent"}, plan, &error, hooks),
                        "replacement allocation failure rejects complete batch");
            TEST_ASSERT(entity.get_component_by_type_name("UnknownComponent") == nullptr,
                        "allocation failure leaves entity unchanged");
            TEST_ASSERT(entity.component_count() == 3, "allocation failure preserves component count");
            scene.destroy();
        }
    ...
    int test_component_degradation_rejects_stale_plan() {
        ...
        termin::UnknownComponentDegradationPlan plan;
        std::string error;
        TEST_ASSERT(termin::prepare_components_to_unknown({scene}, {"ReloadableComponent"}, plan, &error),
        ...
        TEST_ASSERT(entity.component_at(0) == first->c_component(), "stale commit performs zero prepared replacements");
        TEST_ASSERT(entity.get_component_by_type_name("UnknownComponent") == nullptr,
    ...
    int test_component_is_a_after_rejected_parent_cycle() {
        ...
        auto root = termin::ComponentTypeDescriptorBuilder::abstract_native(root_name, "termin-scene-cycle-test");
        TEST_ASSERT(root.commit(), "cycle guard root registered");
        auto child =
            termin::ComponentTypeDescriptorBuilder::abstract_native(child_name, "termin-scene-cycle-test", root_name);
        TEST_ASSERT(child.commit(), "cycle guard child registered");
        termin::ComponentTypeDescriptorBuilder rejected(
            root_name, "termin-scene-cycle-test", child_name, tc_runtime_owned_factory{}, TC_CXX_COMPONENT, true, true);
    ...
    int test_component_descriptor_failure_is_atomic() {
        ...
        (void)descriptor.inspect().add<ReloadableComponent, int>(
            &ReloadableComponent::value, tc::InspectFieldSpec{type_name, "value", "Value", "int"});
