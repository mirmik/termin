...
static int basis_near(tc_basis3d actual, tc_basis3d expected) {
    return vec_near(actual.x, expected.x)
        && vec_near(actual.y, expected.y)
        && vec_near(actual.z, expected.z);
}
...
static int test_classification_and_exact_shear(void) {
    ...
    CHECK(tc_entity_pool_get_world_transform_kind(pool, parent)
          == TC_TRANSFORM_RIGID);
    ...
    CHECK(tc_entity_pool_get_world_transform_kind(pool, child)
          == TC_TRANSFORM_AFFINE);
...
static int test_promotion_demotion_and_affine_descendant(void) {
    ...
    set_pose(
        pool, child,
        (tc_vec3){0.0, 0.0, 0.0},
        (tc_quat){0.0, half_sqrt2, 0.0, half_sqrt2},
        (tc_vec3){2.0, 2.0, 2.0});
    CHECK(tc_entity_pool_get_world_transform_kind(pool, child)
          == TC_TRANSFORM_SIMILARITY);
    ...
    CHECK(tc_entity_pool_get_world_transform_kind(pool, child)
          == TC_TRANSFORM_AFFINE);
    CHECK(tc_entity_pool_get_world_transform_kind(pool, leaf)
          == TC_TRANSFORM_AFFINE);

    // Recompute from current inputs: uniformizing the ancestor demotes the branch.
    tc_entity_pool_set_local_scale(pool, root, uniform_scale);
    CHECK(tc_entity_pool_get_world_transform_kind(pool, root)
          == TC_TRANSFORM_SIMILARITY);
    CHECK(tc_entity_pool_get_world_transform_kind(pool, child)
          == TC_TRANSFORM_SIMILARITY);
    CHECK(tc_entity_pool_get_world_transform_kind(pool, leaf)
...
static int test_zero_negative_growth_and_reuse(void) {
    ...
    for (int i = 0; i < 12; ++i) {
        ...
        CHECK(tc_entity_pool_get_world_transform_kind(pool, entities[i])
              == TC_TRANSFORM_RIGID);
    }

    const double zero_scale[3] = {0.0, 0.0, 0.0};
    tc_entity_pool_set_local_scale(pool, entities[3], zero_scale);
    CHECK(tc_entity_pool_get_world_transform_kind(pool, entities[3])
          == TC_TRANSFORM_AXIS_SCALED);
