...
namespace termin {
    ...
    nos::trent TcSceneRef::get_metadata_at_path(const std::string& path) const {
        ...
        while (!remaining.empty() && current != nullptr) {
            ...
            std::string key = (dot_pos == std::string::npos) ? remaining : remaining.substr(0, dot_pos);
        ...
        if (update_settings) {
            if (data.contains("fixed_timestep")) {
                ...
                if (std::isfinite(dt) && dt > 0.0) {
                    ...
                    tc::Log::error("[TcSceneRef] invalid serialized fixed_timestep=%g", dt);
                }
            }
    ...
    int TcSceneRef::load_from_data_unknown_only(const nos::trent& data,
                                                bool update_settings,
                                                const UnknownUpgradeStrategy& strategy,
                                                bool auto_upgrade_unknown) {
        return load_from_data(
            data, update_settings, ComponentDeserializationMode::UnknownOnly, strategy, auto_upgrade_unknown);
    }
