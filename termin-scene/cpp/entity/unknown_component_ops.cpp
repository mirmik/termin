...
bool upgrade_unknown_component_ref(const Entity& entity, tc_component* component, std::string* error) {
    ...
    if (!ComponentRegistry::instance().has(unknown_obj->original_type)) {
        if (error) {
            *error = "Original type is not registered: " + unknown_obj->original_type;
        }
        return false;
    }

    tc_component* upgraded_tc = tc_component_registry_create(unknown_obj->original_type.c_str());
    if (upgraded_tc == nullptr) {
        if (error) {
            *error = "Failed to create component: " + unknown_obj->original_type;
        }
        return false;
    }
    ...
    tc_inspect_deserialize(
        upgraded_obj,
        unknown_obj->original_type.c_str(),
