...
namespace termin {
    ...
    static nb::dict scene_event_to_python(const tc_event* event) {
        ...
        if (event && event->type && std::string(event->type) == TC_EVENT_SCENE_STRUCTURE_CHANGED && event->payload &&
    ...
    void bind_tc_scene_core(nb::module_& m) {
        ...
        nb::class_<TcSceneRef>(m, "TcScene")
            ...
            .def(nb::init<tc_scene_handle>(), nb::arg("handle"), "Create from existing handle")
            .def_static("create",
                        &TcSceneRef::create,
                        nb::arg("name") = "",
                        nb::arg("uuid") = "",
            ...
            .def("destroy", &TcSceneRef::destroy, "Explicitly destroy scene and release all resources")
            ...
            .def("request_render", &TcSceneRef::request_render, "Request a render frame for this scene")
            .def("consume_render_request",
                 &TcSceneRef::consume_render_request,
            ...
            .def(
                "entity_pool_ptr",
                [](TcSceneRef& self) { return reinterpret_cast<uintptr_t>(self.entity_pool()); },
                "Get scene's entity pool as uintptr_t")

            // Scene handle access
            .def(
                "scene_handle", [](TcSceneRef& self) -> tc_scene_handle { return self._h; }, "Get scene handle")
            .def(
                "equal",
                [](TcSceneRef& self, const TcSceneRef& other) -> bool { return tc_scene_handle_eq(self._h, other._h); },
                nb::arg("other"),
                "True if both scenes reference the same scene")

            // Entity creation in pool
            .def("create_entity",
                 &TcSceneRef::create_entity,
                 nb::arg("name") = "",
                 "Create a new entity directly in scene's pool.")

            // Get all entities
            .def("get_all_entities", &TcSceneRef::get_all_entities, "Get all entities in scene's pool.")
            .def("get_root_entities", &TcSceneRef::get_root_entities, "Get root entities in persistent sibling order.")

            // Entity migration
            .def("migrate_entity",
                 &TcSceneRef::migrate_entity,
                 nb::arg("entity"),
            ...
            .def(
                "get_entity",
                [](TcSceneRef& self, const std::string& uuid) -> nb::object {
            ...
            .def(
                "get_entity_by_pick_id",
                [](TcSceneRef& self, uint32_t pick_id) -> nb::object {
                ...
                nb::arg("pick_id"),
                "Find entity by pick_id.")

            .def(
                "find_entity_by_name",
                [](TcSceneRef& self, const std::string& name) -> nb::object {
            ...
            .def(
                "set_metadata",
                [](TcSceneRef& self, nb::handle data) {
            ...
            .def(
                "get_metadata_value",
                [](TcSceneRef& self, const std::string& path) -> nb::object {
            ...
            .def(
                "set_metadata_value",
                [](TcSceneRef& self, const std::string& path, nb::handle value) {
            ...
            .def(
                ...
                [](TcSceneRef& self, const std::string& path) { self.set_metadata_at_path(path, nos::trent()); },
                nb::arg("path"))
            .def("metadata_to_json", &TcSceneRef::metadata_to_json)
            .def("metadata_from_json", &TcSceneRef::metadata_from_json, nb::arg("json_str"))

            // Scene mode
            .def("get_mode", [](TcSceneRef& self) { return tc_scene_get_mode(self._h); })
            .def(
                "set_mode",
                [](TcSceneRef& self, tc_scene_mode mode) { tc_scene_set_mode(self._h, mode); },
                nb::arg("mode"))

            // Component type iteration
            .def(
                "count_components_of_type",
                [](TcSceneRef& self, const std::string& type_name) {
                    return tc_scene_count_components_of_type(self._h, type_name.c_str());
                },
                nb::arg("type_name"))

            .def(
                "foreach_component_of_type",
                [](TcSceneRef& self, const std::string& type_name, nb::object callback) {
            ...
            .def(
                ...
                [](TcSceneRef& self, const std::string& class_name) -> nb::object {
            ...
            .def("notify_editor_start", [](TcSceneRef& self) { tc_scene_notify_editor_start(self._h); })
            .def("notify_scene_inactive", [](TcSceneRef& self) { tc_scene_notify_scene_inactive(self._h); })
            .def("notify_scene_active", [](TcSceneRef& self) { tc_scene_notify_scene_active(self._h); })

            .def(
                "subscribe_event",
                [](TcSceneRef& self, const std::string& event_type, nb::callable callback) {
                    ...
                    subscription->subscription =
                        tc_event_bus_subscribe(bus, event_type.c_str(), scene_event_callback, subscription);
            ...
            // Serialization
            .def("serialize", [](TcSceneRef& self) -> nb::object { return trent_to_python(self.serialize()); })
            .def(
                "load_from_data",
                [](TcSceneRef& self,
                   nb::handle data,
            ...
            .def(
                "from_json_string",
                [](TcSceneRef& self, const std::string& json, int mode, bool auto_upgrade_unknown) {
            ...
            // Convenience
            .def_prop_ro("is_destroyed", [](TcSceneRef& self) { return !self.is_alive(); })
            .def_prop_ro("entities", [](TcSceneRef& self) { return self.get_all_entities(); })
            .def_prop_ro("root_entities", [](TcSceneRef& self) { return self.get_root_entities(); })

            // Entity management with callbacks
            .def(
                "add",
                [](TcSceneRef& self, Entity& entity) -> Entity {
        ...
        // dispatch_input moved to termin-input bindings

        m.def("degrade_components_to_unknown",
