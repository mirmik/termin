...
namespace termin {
    ...
    public:
        ...
        explicit TcComponentRef(tc_component* c)
            : _c(c) {}
        ...
        void set_source_id(const std::string& value) {
            if (_c)
                tc_component_set_source_id(_c, value.c_str());
        }
        ...
        nb::object to_python() const {
            ...
            try {
                ...
                tc::Log::debug("[TcComponentRef] no typed Python projection for %s: %s", type_name(), e.what());
                return nb::none();
            }
        }
        ...
        nb::object field_value_or_throw(const std::string& field_name) const {
            if (!_c) {
                throw std::runtime_error("Cannot read field '" + field_name + "' from an invalid component reference");
            }
            ...
            if (!obj_ptr) {
                throw std::runtime_error("Component '" + std::string(type_name()) + "' has no inspectable object");
            }

            return tc::InspectRegistry_get(
                tc::InspectRegistry::instance(), obj_ptr, tc_component_type_name(_c), field_name);
        }
        ...
        nb::object require_field(const std::string& field_name) const {
            try {
                ...
                tc::Log::error(
                    "[TcComponentRef] require_field(%s) on %s failed: %s", field_name.c_str(), type_name(), e.what());
                throw;
            } catch (...) {
                tc::Log::error("[TcComponentRef] require_field(%s) on %s failed with unknown exception",
                               field_name.c_str(),
                               type_name());
                throw std::runtime_error("Failed to read required field '" + field_name + "' from component '" +
        ...
        void set_field(const std::string& field_name, nb::object value, TcSceneRef scene = TcSceneRef()) {
            ...
            void* obj_ptr = (_c->kind == TC_CXX_COMPONENT) ? static_cast<void*>(CxxComponent::from_tc(_c)) : _c->body;
            if (!obj_ptr)
                return;
        ...
        void action_field(const std::string& field_name) {
            ...
            tc::InspectRegistry::instance().action_field(obj_ptr, tc_component_type_name(_c), field_name, inspect_ctx);
        }

        bool operator==(const TcComponentRef& other) const {
            return _c == other._c;
        }
        bool operator!=(const TcComponentRef& other) const {
            return _c != other._c;
    ...
    void bind_tc_component_ref(nb::module_& m) {
        ...
        nb::class_<TcComponentRef>(m, "TcComponentRef")
            ...
            .def(
                "__init__",
                [](TcComponentRef& self, uintptr_t ptr) {
            ...
            .def("__repr__",
                 [](const TcComponentRef& self) {
                     if (!self.valid())
                         return std::string("<TcComponentRef: invalid>");
            ...
            // is_drawable moved to termin-render bindings
            .def_prop_ro("tc_component_ptr",
                         [](TcComponentRef& self) -> uintptr_t { return reinterpret_cast<uintptr_t>(self._c); })
            ...
            .def("to_python",
                 &TcComponentRef::to_python,
                 "Try to get typed Python component object. Returns None if no bindings available.")
            .def("serialize", &TcComponentRef::serialize, "Serialize component to dict with 'type' and 'data' keys.")
            .def("serialize_data", &TcComponentRef::serialize_data, "Serialize component data (fields only) to dict.")
            .def(
                "deserialize_data",
                [](TcComponentRef& self, nb::object data, std::optional<TcSceneRef> scene) {
                    self.deserialize_data(data, scene.value_or(TcSceneRef{}));
                },
                nb::arg("data"),
                nb::arg("scene").none() = nb::none(),
                "Deserialize data dict into component fields. Pass scene for resolution.")
            .def("get_field",
                 &TcComponentRef::get_field,
                 nb::arg("field_name"),
                 "Get field value by name. Returns None if field not found.")
            .def("require_field",
                 &TcComponentRef::require_field,
                 nb::arg("field_name"),
