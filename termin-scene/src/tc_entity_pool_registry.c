...
static EntityPoolRegistry* g_entity_pool_registry = NULL;
...
void tc_entity_pool_registry_init(void) {
    ...
    g_entity_pool_registry = (EntityPoolRegistry*)calloc(1, sizeof(EntityPoolRegistry));
    if (!g_entity_pool_registry) {
    ...
    g_entity_pool_registry->generations = (uint32_t*)calloc(cap, sizeof(uint32_t));
    g_entity_pool_registry->alive = (bool*)calloc(cap, sizeof(bool));
    g_entity_pool_registry->pools = (tc_entity_pool**)calloc(cap, sizeof(tc_entity_pool*));
    g_entity_pool_registry->free_stack = (uint32_t*)malloc(cap * sizeof(uint32_t));

    // Initialize free stack
    for (size_t i = 0; i < cap; i++) {
        g_entity_pool_registry->free_stack[i] = (uint32_t)(cap - 1 - i);
    }
    g_entity_pool_registry->free_count = cap;
    g_entity_pool_registry->capacity = cap;
    g_entity_pool_registry->count = 0;
}
...
void tc_entity_pool_registry_shutdown(void) {
    ...
    for (size_t i = 0; i < g_entity_pool_registry->capacity; i++) {
        if (g_entity_pool_registry->alive[i] && g_entity_pool_registry->pools[i]) {
            tc_entity_pool_destroy(g_entity_pool_registry->pools[i]);
        }
    }

    free(g_entity_pool_registry->generations);
    free(g_entity_pool_registry->alive);
    free(g_entity_pool_registry->pools);
    free(g_entity_pool_registry->free_stack);
    free(g_entity_pool_registry);
    g_entity_pool_registry = NULL;
...
static void registry_grow(void) {
    ...
    g_entity_pool_registry->generations = realloc(g_entity_pool_registry->generations, new_cap * sizeof(uint32_t));
    g_entity_pool_registry->alive = realloc(g_entity_pool_registry->alive, new_cap * sizeof(bool));
    g_entity_pool_registry->pools = realloc(g_entity_pool_registry->pools, new_cap * sizeof(tc_entity_pool*));
    g_entity_pool_registry->free_stack = realloc(g_entity_pool_registry->free_stack, new_cap * sizeof(uint32_t));

    // Initialize new slots
    memset(g_entity_pool_registry->generations + old_cap, 0, (new_cap - old_cap) * sizeof(uint32_t));
    memset(g_entity_pool_registry->alive + old_cap, 0, (new_cap - old_cap) * sizeof(bool));
    memset(g_entity_pool_registry->pools + old_cap, 0, (new_cap - old_cap) * sizeof(tc_entity_pool*));
...
tc_entity_pool_handle tc_entity_pool_registry_create(size_t initial_capacity) {
    ...
    if (g_entity_pool_registry->free_count == 0) {
        registry_grow();
        if (g_entity_pool_registry->free_count == 0) {
    ...
    uint32_t idx = g_entity_pool_registry->free_stack[--g_entity_pool_registry->free_count];
    uint32_t gen = g_entity_pool_registry->generations[idx];

    // Create the actual pool
    tc_entity_pool* pool = tc_entity_pool_create(initial_capacity);
    if (!pool) {
        // Return slot to free stack
        g_entity_pool_registry->free_stack[g_entity_pool_registry->free_count++] = idx;
        return TC_ENTITY_POOL_HANDLE_INVALID;
    }

    g_entity_pool_registry->alive[idx] = true;
...
void tc_entity_pool_registry_destroy(tc_entity_pool_handle h) {
    ...
    // Destroy the pool
    if (g_entity_pool_registry->pools[idx]) {
        tc_entity_pool_destroy(g_entity_pool_registry->pools[idx]);
        g_entity_pool_registry->pools[idx] = NULL;
    }

    // Mark as dead and bump generation
    g_entity_pool_registry->alive[idx] = false;
    g_entity_pool_registry->generations[idx]++;
    g_entity_pool_registry->free_stack[g_entity_pool_registry->free_count++] = idx;
    g_entity_pool_registry->count--;
}
