...
tc_component_cap_id tc_component_capability_register_with_destructor(
    ...
    if (g_component_capability_registry.count >= TC_COMPONENT_MAX_CAPABILITIES) {
...
const char* tc_component_capability_name(tc_component_cap_id id) {
    uint32_t slot = 0;
    return capability_slot_from_id(id, &slot) ? g_component_capability_registry.names[slot] : NULL;
}
...
size_t tc_component_capability_count(void) {
    return g_component_capability_registry.count;
}
