...
static tc_archetype* pool_get_or_create_archetype(tc_entity_pool* pool, uint64_t mask) {
    ...
    tc_archetype* archetype = tc_archetype_create(mask, type_ids, type_count, tc_soa_global_registry());
    if (!archetype)
        return NULL;

    if (pool->archetype_count >= pool->archetype_capacity) {
        size_t new_capacity = pool->archetype_capacity == 0 ? 8 : pool->archetype_capacity * 2;
        tc_archetype** archetypes = realloc(pool->archetypes, new_capacity * sizeof(tc_archetype*));
        if (!archetypes) {
            tc_log_error("[tc_entity_pool] failed to grow archetype storage to %zu entries", new_capacity);
...
static void pool_move_entity_archetype(tc_entity_pool* pool,
                                       uint32_t entity_index,
                                       tc_archetype* old_archetype,
                                       uint32_t old_row,
                                       tc_archetype* new_archetype) {
    tc_entity_id entity = {entity_index, pool->generations[entity_index]};
    uint32_t new_row = tc_archetype_alloc_row(new_archetype, entity, tc_soa_global_registry());
    ...
    if (old_archetype) {
        for (size_t i = 0; i < new_archetype->type_count; ++i) {
            ...
            const tc_soa_type_desc* desc = tc_soa_get_type(tc_soa_global_registry(), type_id);
            void* destination = (char*)new_archetype->data[i] + new_row * desc->element_size;
            void* source_element = NULL;
            for (size_t j = 0; j < old_archetype->type_count; ++j) {
                if (old_archetype->type_ids[j] == type_id) {
                    source_element = (char*)old_archetype->data[j] + old_row * desc->element_size;
                    break;
                }
            }
            if (source_element) {
        ...
        tc_entity_id swapped = tc_archetype_detach_row(old_archetype, old_row, tc_soa_global_registry());
    ...
    tc_u64_map_get(pool->archetype_by_mask, new_archetype->type_mask, &new_archetype_index);
...
void tc_entity_pool_add_soa(tc_entity_pool* pool, tc_entity_id id, tc_soa_type_id type) {
    ...
    pool_move_entity_archetype(pool, index, old_archetype, old_row, new_archetype);
...
void tc_entity_pool_remove_soa(tc_entity_pool* pool, tc_entity_id id, tc_soa_type_id type) {
    ...
    tc_archetype* old_archetype = pool->archetypes[pool->soa_archetype_ids[index]];
    uint32_t old_row = pool->soa_archetype_rows[index];

    if (new_mask == 0) {
        tc_entity_id swapped = tc_archetype_free_row(old_archetype, old_row, tc_soa_global_registry());
        if (tc_entity_id_valid(swapped)) {
            pool->soa_archetype_rows[swapped.index] = old_row;
        }
...
void* tc_entity_pool_get_soa(const tc_entity_pool* pool, tc_entity_id id, tc_soa_type_id type) {
    ...
    if (pool->soa_archetype_ids[index] == UINT32_MAX)
        return NULL;
    if (!(pool->soa_type_masks[index] & (1ULL << type)))
        return NULL;

    tc_archetype* archetype = pool->archetypes[pool->soa_archetype_ids[index]];
    uint32_t row = pool->soa_archetype_rows[index];
    return tc_archetype_get_element(archetype, row, type, tc_soa_global_registry());
}
