...
static bool
project_world_surface(tc_component* component, const tc_world_pointer_ray* ray, tc_world_pointer_hit* out_hit) {
    const tc_world_pointer_surface_capability* capability = tc_world_pointer_surface_capability_get(component);
    world_surface_probe* probe = capability ? (world_surface_probe*)capability->userdata : NULL;
    if (!probe || !ray || !out_hit)
        return false;
...
static bool dispatch_world_pointer(tc_component* component, const tc_world_pointer_event* event) {
    const tc_world_pointer_surface_capability* capability = tc_world_pointer_surface_capability_get(component);
    world_surface_probe* probe = capability ? (world_surface_probe*)capability->userdata : NULL;
    if (!probe || !event)
        return false;
...
GUARD_C_TEST(test_input_source_mask_defaults_and_updates) {
    ...
    GUARD_C_REQUIRE(tc_component_set_input_source_mask(&component, TC_INPUT_SOURCE_RUNTIME | TC_INPUT_SOURCE_EDITOR));
...
GUARD_C_TEST(test_world_pointer_surface_capability_routes_projection_and_events) {
    ...
    GUARD_C_REQUIRE(tc_world_pointer_surface_capability_attach(&component, &g_world_surface_vtable, &probe));
    ...
    GUARD_C_REQUIRE(tc_world_pointer_surface_project_ray(&component, &ray, &hit));
    ...
    GUARD_C_REQUIRE(tc_world_pointer_surface_dispatch_pointer(&component, &event));
