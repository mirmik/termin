...
NB_MODULE(_input_native, m) {
    ...
    nb::class_<termin::xr::XrRigInputState>(m, "XrRigInputState")
        .def_prop_ro("id", [](const termin::xr::XrRigInputState& state) { return state.id; })
        .def_prop_ro("frame_index", [](const termin::xr::XrRigInputState& state) { return state.frame_index; })
        .def_prop_ro("head_axes_active",
                     [](const termin::xr::XrRigInputState& state) { return state.head_axes_active; })
        .def(
            "thumbstick",
            [](const termin::xr::XrRigInputState& state, termin::xr::XrHand hand) {
                const termin::xr::XrAxis2State& axis = state.hand(hand).thumbstick;
                return nb::make_tuple(axis.value.x, axis.value.y, axis.active);
            },
            nb::arg("hand"))
        .def("head_forward_in_origin",
             [](const termin::xr::XrRigInputState& state) {
                 return nb::make_tuple(
                     state.head_forward_in_origin.x, state.head_forward_in_origin.y, state.head_forward_in_origin.z);
             })
        .def("head_right_in_origin", [](const termin::xr::XrRigInputState& state) {
            return nb::make_tuple(
                state.head_right_in_origin.x, state.head_right_in_origin.y, state.head_right_in_origin.z);
        });

    nb::class_<termin::xr::XrInput>(m, "XrInput")
        .def_static(
            "get_state",
            [](const std::string& id) -> nb::object {
    ...
    m.def("input_capability_id", []() { return tc_input_capability_id(); }, "Get the input capability ID");
    ...
    m.def(
        "set_input_priority",
        [](uintptr_t c_ptr, int priority) -> bool {
            auto* c = reinterpret_cast<tc_component*>(c_ptr);
    ...
    m.def(
        ...
        [](uintptr_t c_ptr, uint32_t source_mask) -> bool {
            auto* c = reinterpret_cast<tc_component*>(c_ptr);
    ...
    m.def(
        ...
        [](uintptr_t c_ptr, uint32_t source) -> bool {
            auto* c = reinterpret_cast<tc_component*>(c_ptr);
