...
static inline bool tc_component_is_overlay_input_handler(const tc_component* c) {
    return c && tc_overlay_input_capability_get(c) != NULL;
}
...
static inline const tc_input_vtable* tc_component_get_overlay_input_vtable(const tc_component* c) {
    if (!c) return NULL;
    return tc_overlay_input_capability_get(c);
}

static inline const tc_input_vtable* tc_component_get_any_input_vtable(const tc_component* c) {
    const tc_input_vtable* vt = tc_component_get_input_vtable(c);
    return vt ? vt : tc_component_get_overlay_input_vtable(c);
}

static inline void tc_component_on_mouse_button(tc_component* c, tc_mouse_button_event* event) {
    const tc_input_vtable* vt = tc_component_get_any_input_vtable(c);
