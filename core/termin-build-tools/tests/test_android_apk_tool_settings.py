...
pytestmark = pytest.mark.skipif(
    os.name == "nt", reason="exercises the POSIX scripts/build/android-apk.sh wrapper"
)
...
def test_android_apk_wrapper_uses_shared_toolchain_settings(tmp_path: Path) -> None:
    ...
    (termin_root / "platform/termin-android/assets").mkdir(parents=True)
    shutil.copy2(
        REPO_ROOT / "scripts/build/android-apk.sh",
        termin_root / "scripts/build/android-apk.sh",
    )
    shutil.copy2(
    ...
    result = subprocess.run(
        [str(termin_root / "scripts/build/android-apk.sh")],
        cwd=termin_root,
        env=env,
        text=True,
        capture_output=True,
