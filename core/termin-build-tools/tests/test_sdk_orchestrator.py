...
def test_installed_core_environment_is_ignored_for_repository_owned_profile(
    ...
    monkeypatch.setenv(sdk.CORE_SDK_ENV, str(external_core))
    monkeypatch.setenv(sdk.CORE_BUILD_ID_ENV, "stale-build-id")

    def unexpected_load(*args, **kwargs):
        pytest.fail("repository-owned profiles must not load an installed Core SDK")

    monkeypatch.setattr(sdk, "load_installed_sdk_input", unexpected_load)

    result = sdk._installed_core_input(
        tmp_path,
