...
namespace termin {
    ...
    // their finite unit-quaternion precondition; use try_rotate,
    // try_inverse_rotate or try_to_matrix when the input is not already known
    // to be a valid rotation. Checked normalization is full-range scale-by-max.
    using Quat = ::tc_quat;
