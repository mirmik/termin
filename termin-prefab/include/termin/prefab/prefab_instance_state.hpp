...
namespace termin::prefab {
    ...
    public:
        ...
        const std::string& prefab_asset_uuid() const {
            return _prefab_asset_uuid;
        }
        void set_prefab_asset_uuid(std::string value) {
            _prefab_asset_uuid = std::move(value);
        }

        const std::string& source_revision() const {
            return _source_revision;
        }
        void set_source_revision(std::string value) {
            _source_revision = std::move(value);
        }

        void set_entity_mapping(std::vector<std::string> source_ids, std::vector<Entity> runtime_entities);
        void set_component_mapping(std::vector<std::string> source_ids, std::vector<Entity> runtime_owners);
        ...
        std::string source_for_component(const Entity& runtime_owner,
                                         const std::string& runtime_component_source_id) const;
        bool rebind_entity_mapping(const std::string& source_id, const Entity& runtime_entity, std::string& error);
        bool rebind_component_mapping(const std::string& source_id, const Entity& runtime_owner, std::string& error);
        ...
        PrefabOverrideRestoreResult clear_property_override(const PrefabDocument& source,
                                                            const std::string& source_entity_id,
                                                            const std::string& source_component_id,
                                                            const std::string& field_path);
        PrefabOverrideRestoreResult clear_all_property_overrides(const PrefabDocument& source);
        PrefabReconcileResult reconcile_properties(const PrefabDocument& source,
                                                   const PrefabOverrideResourceResolver* resource_resolver = nullptr);
        PrefabReconcileResult reconcile(const PrefabDocument& source,
                                        const PrefabOverrideResourceResolver* resource_resolver = nullptr);
        bool set_structural_override(PrefabStructuralOverride structural_override, std::string& error);
        bool discard_structural_override(PrefabStructuralOverrideKind kind, const std::string& source_id);
        const std::vector<PrefabStructuralOverride>& structural_overrides() const {
            return _structural_overrides;
        }
        size_t structural_override_count() const {
            return _structural_overrides.size();
        }
        bool structural_overrides_valid() const {
            return _structural_overrides_valid;
        }
        ...
        bool discard_property_override(const std::string& source_entity_id,
                                       const std::string& source_component_id,
                                       const std::string& field_path);
        void discard_all_property_overrides();
        const PrefabPropertyOverride* property_override(const std::string& source_entity_id,
                                                        const std::string& source_component_id,
                                                        const std::string& field_path) const;
        const PrefabStructuralOverride* structural_override(PrefabStructuralOverrideKind kind,
                                                            const std::string& source_id) const;
        const std::vector<PrefabPropertyOverride>& property_overrides() const {
            return _property_overrides;
        }
        size_t property_override_count() const {
            return _property_overrides.size();
        }
        bool overrides_valid() const {
            return _overrides_valid;
        }
    ...
    private:
        ...
        void reconcile_structure(const PrefabDocument& source, PrefabReconcileResult& result);
    ...
    TERMIN_PREFAB_API std::vector<Entity> find_live_prefab_instances(const std::string& prefab_asset_uuid);
    TERMIN_PREFAB_API std::vector<Entity> find_live_prefab_instances(tc_scene_handle scene,
                                                                     const std::string& prefab_asset_uuid);
    TERMIN_PREFAB_API size_t count_live_prefab_instances(const std::string& prefab_asset_uuid);
