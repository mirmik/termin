...
namespace {
    ...
    public:
        ...
        explicit TestResourceHandle(std::string uuid)
            : _uuid(std::move(uuid)) {}
    ...
    public:
        PrefabRefProbe()
            : termin::CxxComponent("PrefabRefProbe") {}
    ...
    public:
        PrefabRefProbeV2()
            : termin::CxxComponent("PrefabRefProbeV2") {}

        double gain = 0.0;
    };

    class TestOverrideResourceResolver final : public termin::prefab::PrefabOverrideResourceResolver {
    public:
        bool resolve(std::string_view resource_type,
                     std::string_view target_kind,
                     std::string_view uuid,
                     std::string_view display_name,
                     tc::trent& result,
                     std::string& error) const override {
            if (resource_type != "test" || target_kind != "test_resource" || uuid != "resource-local") {
    ...
    int register_inspect() {
        ...
        auto probe =
            termin::ComponentTypeDescriptorBuilder::native<PrefabRefProbe>("PrefabRefProbe", "termin-prefab-test");
        ...
        auto probe_v2 =
            termin::ComponentTypeDescriptorBuilder::native<PrefabRefProbeV2>("PrefabRefProbeV2", "termin-prefab-test");
...
int main() {
    ...
    termin::prefab::PrefabDocumentResult editable = parsed_document.document.materialize_source(edit_scene.handle());
    ...
    termin::prefab::PrefabDocumentResult captured =
        termin::prefab::PrefabDocument::capture(parsed_document.document.asset_uuid(), editable.root);
    ...
    termin::prefab::PrefabInstantiateResult document_instance =
        termin::prefab::PrefabInstantiator::instantiate(parsed_document.document, scene.handle(), parent);
    ...
    TEST_ASSERT(std::string(first.root.uuid()) != std::string(second.root.uuid()), "instance root UUIDs should differ");
    ...
    TEST_ASSERT(document_state != nullptr, "canonical document instance should publish native instance state");
    ...
    TEST_ASSERT(document_state->entity_mapping_count() == 3, "instance state should map every source entity");
    ...
    TEST_ASSERT(first_probe != nullptr && second_probe != nullptr, "instance components should deserialize");
    ...
    TEST_ASSERT(document_state->entity_for_source("prefab-source-child") == document_instance.root.find_child("Child"),
                "source entity mapping should resolve the matching runtime child");
    TEST_ASSERT(document_state->component_owner_for_source("prefab-source-probe-component") == document_instance.root,
                "component source mapping should resolve its runtime owner");

    auto record_override = [](termin::prefab::PrefabInstanceState& state,
        ...
        auto value = termin::prefab::PrefabOverrideValue::parse_json(
            R"({"schema":"termin.prefab.override-value","version":1,"value":{"tag":"none"}})", error);
        if (!value)
            return false;
    ...
    PrefabRefProbe* document_probe = document_instance.root.get_component<PrefabRefProbe>();
    ...
    TEST_ASSERT(
        record_override(*document_state, "prefab-source-root", "", "name", "string") &&
            record_override(*document_state, "prefab-source-root", "", "transform.position", "vec3") &&
            record_override(
                *document_state, "prefab-source-root", "prefab-source-probe-component", "labels", "list[string]") &&
            record_override(
                *document_state, "prefab-source-root", "prefab-source-probe-component", "target", "entity") &&
            record_override(
                *document_state, "prefab-source-root", "prefab-source-probe-component", "resource", "test_resource") &&
            record_override(
                *document_state, "prefab-source-root", "prefab-source-probe-component", "missing", "double"),
    ...
    TEST_ASSERT(batch_restore.failures.size() == 1 &&
                    batch_restore.failures[0].error == termin::prefab::PrefabOverrideRestoreError::FieldNotFound,
    ...
    TEST_ASSERT(restored_position[0] == 1.0 && restored_position[1] == 2.0 && restored_position[2] == 3.0,
    ...
    TEST_ASSERT(
        record_override(*document_state, "prefab-source-root", "prefab-source-probe-component", "weight", "double"),
        "scalar restore fixture should be accepted");
    termin::prefab::PrefabOverrideRestoreResult scalar_restore = document_state->clear_property_override(
        newer_document.document, "prefab-source-root", "prefab-source-probe-component", "weight");
    TEST_ASSERT(scalar_restore.ok() && scalar_restore.restored_count == 1 && document_probe->weight == 7.0,
    ...
    auto* reconcile_state = reconcile_instance.root.get_component<termin::prefab::PrefabInstanceState>();
    ...
    TEST_ASSERT(reconcile_override_value.has_value(), "reconcile typed override fixture should parse");
    ...
    TEST_ASSERT(reconcile_state->set_property_override(std::move(reconcile_override), reconcile_override_error),
    ...
    auto reconcile_resource_value = termin::prefab::PrefabOverrideValue::parse_json(
        ...
        reconcile_override_error);
    TEST_ASSERT(reconcile_resource_value.has_value(), "reconcile resource override fixture should parse");
    ...
    TEST_ASSERT(
        reconcile_state->set_property_override(std::move(reconcile_resource_override), reconcile_override_error),
    ...
    TEST_ASSERT(!unresolved_override_reconcile.ok() && !unresolved_override_reconcile.revision_updated &&
    ...
    termin::prefab::PrefabReconcileResult reconcile_result =
        reconcile_state->reconcile_properties(refreshed_document.document, &reconcile_resource_resolver);
    ...
    TEST_ASSERT(reconcile_result.override_count == 3 && reconcile_result.overrides_applied == 3 &&
                    reconcile_state->property_override_count() == 3,
    ...
    TEST_ASSERT(reconcile_state->source_revision() == refreshed_document.document.source_revision(),
    ...
    termin::prefab::PrefabReconcileResult broken_reconcile =
        reconcile_state->reconcile_properties(broken_refresh_document.document, &reconcile_resource_resolver);
    ...
    TEST_ASSERT(broken_reconcile.failures.size() == 1 &&
                    broken_reconcile.failures[0].phase == termin::prefab::PrefabReconcilePhase::SourceValue &&
                    broken_reconcile.failures[0].field_path == "unknown_field",
                "field drift should produce one deterministic source diagnostic");
    TEST_ASSERT(reconcile_state->property_override_count() == 3 && reconcile_probe->weight == 42.5,
                "partial reconcile should retain and reapply stored overrides");

    nos::trent structural_refresh_data = refreshed_data;
    nos::trent added_source_child = structural_refresh_data["root"]["children"].as_list()[0];
    ...
    structural_refresh_data["root"]["children"].push_back(std::move(added_source_child));
    termin::prefab::PrefabDocumentResult structural_refresh_document =
        termin::prefab::PrefabDocument::parse_json(nos::json::dump(structural_refresh_data));
    ...
    termin::prefab::PrefabReconcileResult structural_reconcile =
        reconcile_state->reconcile(structural_refresh_document.document, &reconcile_resource_resolver);
    TEST_ASSERT(structural_reconcile.ok() && structural_reconcile.revision_updated &&
                    ...
                    structural_reconcile.structure_operations_applied == structural_reconcile.structure_operation_count,
    ...
    termin::prefab::PrefabReconcileResult reparent_result =
        reconcile_state->reconcile(reparent_document.document, &reconcile_resource_resolver);
    termin::Entity reparented_grandchild = reconcile_state->entity_for_source("prefab-source-grandchild");
    termin::Entity reordered_child = reconcile_state->entity_for_source("prefab-source-child");
    TEST_ASSERT(reparent_result.ok() && reparented_grandchild.parent() == reconcile_instance.root &&
                    reparented_grandchild.sibling_index() == 0 && reconcile_local.sibling_index() == 1 &&
                    reordered_child.sibling_index() == 2,
                "source reparent and reorder should converge while local sibling keeps its slot");
    TEST_ASSERT(reconcile_state->reconcile(refreshed_document.document, &reconcile_resource_resolver).ok() &&
    ...
    TEST_ASSERT(local_component->entity() == reconcile_instance.root, "local component fixture should attach");
    ...
    component_add_data["root"]["components"].as_list().insert(
        component_add_data["root"]["components"].as_list().begin(), std::move(added_component));
    ...
    termin::prefab::PrefabReconcileResult component_add_result =
        reconcile_state->reconcile(component_add_document.document, &reconcile_resource_resolver);
    ...
    for (size_t index = 0; index < reconcile_instance.root.component_count(); ++index) {
        ...
        if (candidate != nullptr &&
            std::string(tc_component_get_source_id(candidate)) == "prefab-source-added-component") {
            added_runtime_component = dynamic_cast<PrefabRefProbeV2*>(termin::CxxComponent::from_tc(candidate));
        }
    }
    TEST_ASSERT(component_add_result.ok() && added_runtime_component != nullptr &&
                    added_runtime_component->gain == 9.5 && local_component->entity() == reconcile_instance.root,
    ...
    incompatible_type_data["root"]["components"].as_list()[0]["type"] = "PrefabRefProbeV2";
    ...
    termin::prefab::PrefabReconcileResult incompatible_type_result =
        reconcile_state->reconcile(incompatible_type_document.document, &reconcile_resource_resolver);
    ...
    termin::prefab::PrefabInstantiateResult structural_instance =
        termin::prefab::PrefabInstantiator::instantiate(refreshed_document.document, scene.handle(), parent);
    TEST_ASSERT(structural_instance.ok(), "structural policy instance should instantiate");
    auto* structural_state = structural_instance.root.get_component<termin::prefab::PrefabInstanceState>();
    TEST_ASSERT(structural_state != nullptr, "structural policy state should exist");
    TEST_ASSERT(structural_state->reconcile(incompatible_type_document.document, &reconcile_resource_resolver).ok() &&
    ...
    TEST_ASSERT(structural_state->reconcile(refreshed_document.document, &reconcile_resource_resolver).ok() &&
    ...
    unavailable_component_data["root"]["components"].push_back(std::move(unavailable_component));
    termin::prefab::PrefabDocumentResult unavailable_component_document =
        termin::prefab::PrefabDocument::parse_json(nos::json::dump(unavailable_component_data));
    TEST_ASSERT(unavailable_component_document.ok(), "unavailable component factory fixture should parse");
    const std::string structural_revision_before_failure = structural_state->source_revision();
    termin::prefab::PrefabReconcileResult unavailable_component_result =
        structural_state->reconcile(unavailable_component_document.document, &reconcile_resource_resolver);
    TEST_ASSERT(!unavailable_component_result.ok() && !unavailable_component_result.revision_updated &&
                    structural_state->source_revision() == structural_revision_before_failure,
    ...
    termin::Entity structural_child = structural_state->entity_for_source("prefab-source-child");
    ...
    auto dormant_name_value = termin::prefab::PrefabOverrideValue::parse_json(
        ...
        reconcile_override_error);
    TEST_ASSERT(dormant_name_value.has_value(), "dormant property override fixture should parse");
    ...
    TEST_ASSERT(structural_state->set_property_override(std::move(dormant_name_override), reconcile_override_error),
    ...
    TEST_ASSERT(structural_state->set_structural_override(std::move(suppress_child), reconcile_override_error),
                "entity tombstone should be accepted");
    termin::prefab::PrefabReconcileResult suppress_result =
        structural_state->reconcile(refreshed_document.document, &reconcile_resource_resolver);
    ...
    TEST_ASSERT(
        structural_state->discard_structural_override(termin::prefab::PrefabStructuralOverrideKind::SuppressEntity,
                                                      "prefab-source-child") &&
            structural_state->reconcile(refreshed_document.document, &reconcile_resource_resolver).ok() &&
            structural_state->entity_for_source("prefab-source-child").valid() &&
            structural_state->entity_for_source("prefab-source-grandchild").valid() &&
            std::string(structural_state->entity_for_source("prefab-source-child").name()) == "LocalChildName" &&
    ...
    TEST_ASSERT(structural_state->set_structural_override(std::move(place_grandchild), reconcile_override_error) &&
                    structural_state->reconcile(refreshed_document.document, &reconcile_resource_resolver).ok(),
                "explicit entity placement should reconcile");
    termin::Entity placed_grandchild = structural_state->entity_for_source("prefab-source-grandchild");
    ...
    auto* cloned_structural_state = structural_clone.get_component<termin::prefab::PrefabInstanceState>();
    TEST_ASSERT(cloned_structural_state != nullptr && cloned_structural_state->structural_override_count() == 1,
                "clone serialization should retain structural override metadata");
    TEST_ASSERT(structural_state->discard_structural_override(termin::prefab::PrefabStructuralOverrideKind::PlaceEntity,
                                                              "prefab-source-grandchild") &&
                    structural_state->reconcile(refreshed_document.document, &reconcile_resource_resolver).ok() &&
                    placed_grandchild.parent() == structural_state->entity_for_source("prefab-source-child"),
    ...
    TEST_ASSERT(structural_state->set_structural_override(std::move(cyclic_placement), reconcile_override_error),
                "cyclic placement fixture should be stored for reconcile validation");
    termin::Entity canonical_child = structural_state->entity_for_source("prefab-source-child");
    termin::Entity canonical_grandchild = structural_state->entity_for_source("prefab-source-grandchild");
    termin::prefab::PrefabReconcileResult cyclic_placement_result =
        structural_state->reconcile(refreshed_document.document, &reconcile_resource_resolver);
    TEST_ASSERT(!cyclic_placement_result.ok() && canonical_child.parent() == structural_instance.root &&
    ...
    termin::prefab::PrefabReconcileResult duplicate_runtime_component_result =
        structural_state->reconcile(refreshed_document.document, &reconcile_resource_resolver);
    TEST_ASSERT(!duplicate_runtime_component_result.ok() && !duplicate_runtime_component_result.revision_updated,
    ...
    termin::prefab::PrefabInstantiateResult customized_instance = termin::prefab::PrefabInstantiator::instantiate(
    ...
    auto* customized_state = customized_instance.root.get_component<termin::prefab::PrefabInstanceState>();
    TEST_ASSERT(customized_state != nullptr && customized_state->property_override_count() == 2,
    ...
    TEST_ASSERT(customized_reconcile.ok() && customized_reconcile.revision_updated &&
                    ...
                    customized_position[0] == 10.0 && customized_position[1] == 11.0 && customized_position[2] == 12.0,
    ...
    missing_resource_data["root"]["components"].as_list()[0]["data"]["resource"]["uuid"] = "resource-missing";
    ...
    termin::prefab::PrefabOverrideRestoreResult missing_resource_restore = document_state->clear_property_override(
        missing_resource_document.document, "prefab-source-root", "prefab-source-probe-component", "resource");
    ...
    termin::prefab::PrefabOverrideRestoreResult mismatch_restore = document_state->clear_property_override(
        foreign_document.document, "prefab-source-root", "prefab-source-probe-component", "weight");
    TEST_ASSERT(!mismatch_restore.ok() &&
                    ...
                    document_probe->weight == 101.0 && document_state->property_override_count() == 1,
    ...
    termin::prefab::PrefabOverrideRestoreResult absent_restore =
        document_state->clear_property_override(parsed_document.document, "prefab-source-root", "", "name");
    TEST_ASSERT(!absent_restore.ok() &&
                    absent_restore.failures[0].error == termin::prefab::PrefabOverrideRestoreError::OverrideNotFound,
    ...
    TEST_ASSERT(!malformed_transform_document.ok() &&
                    malformed_transform_document.message.find("root.pose.position") != std::string::npos,
    ...
    TEST_ASSERT(scene.entity_count() == stable_count, "malformed hierarchy should not modify the target scene");
    ...
    TEST_ASSERT(scene.entity_count() == stable_count, "duplicate source UUID should not modify the target scene");
    ...
    termin::prefab::PrefabInstantiateResult duplicate_component_result =
        termin::prefab::PrefabInstantiator::instantiate(duplicate_component, scene.handle(), parent);
    ...
    TEST_ASSERT(scene.entity_count() == stable_count, "cross-pool parent should not modify the target scene");

    termin::TcSceneRef tracking_scene = termin::TcSceneRef::create("prefab-instance-tracking-test");
    termin::prefab::PrefabInstantiateResult tracked_first =
        termin::prefab::PrefabInstantiator::instantiate(parsed_document.document, tracking_scene.handle());
    termin::prefab::PrefabInstantiateResult tracked_second =
        termin::prefab::PrefabInstantiator::instantiate(parsed_document.document, tracking_scene.handle());
    TEST_ASSERT(tracked_first.ok() && tracked_second.ok(), "tracking fixtures should instantiate");
    TEST_ASSERT(termin::prefab::find_live_prefab_instances(tracking_scene.handle(), "prefab-document-asset").size() ==
                    2,
    ...
    termin::prefab::PrefabInstantiateResult tracked_other =
        termin::prefab::PrefabInstantiator::instantiate(other_document.document, tracking_scene.handle());
    ...
    auto* tracked_first_state = tracked_first.root.get_component<termin::prefab::PrefabInstanceState>();
    ...
    TEST_ASSERT(tracked_first_state->set_property_override(std::move(property_override), override_error),
    ...
    auto* cloned_state = cloned_instance.get_component<termin::prefab::PrefabInstanceState>();
    TEST_ASSERT(cloned_state != nullptr, "generic clone should retain native prefab state");
    TEST_ASSERT(cloned_state->entity_for_source("prefab-source-child") == cloned_instance.find_child("Child"),
                "generic clone should remap serialized state entity references");
    TEST_ASSERT(cloned_state->component_owner_for_source("prefab-source-probe-component") == cloned_instance,
                "generic clone should remap component owner references");
    TEST_ASSERT(cloned_state->property_override_count() == 1, "generic clone should preserve typed property overrides");
    const auto* cloned_override =
        cloned_state->property_override("prefab-source-root", "prefab-source-probe-component", "weight");
    ...
    termin::TcSceneRef restored_tracking_scene = termin::TcSceneRef::create("prefab-instance-tracking-restored");
    ...
    std::vector<termin::Entity> restored_instances =
        termin::prefab::find_live_prefab_instances(restored_tracking_scene.handle(), "prefab-document-asset");
    TEST_ASSERT(restored_instances.size() == 3, "scene load should restore all matching native instance states");
    size_t restored_override_count = 0;
    for (termin::Entity& restored_root : restored_instances) {
        auto* restored_state = restored_root.get_component<termin::prefab::PrefabInstanceState>();
        TEST_ASSERT(restored_state != nullptr, "restored instance root should contain native state");
        TEST_ASSERT(restored_state->entity_for_source("prefab-source-child") == restored_root.find_child("Child"),
    ...
    for (nos::trent& root_data : malformed_override_data["entities"].as_list()) {
        for (nos::trent& component_data : root_data["components"].as_list()) {
            if (component_data["type"].as_string_default("") != termin::prefab::PrefabInstanceState::TypeName) {
                continue;
            }
            if (!component_data["data"]["property_overrides"].as_list().empty()) {
                component_data["data"]["property_overrides"].as_list()[0]["value"]["value"]["tag"] = "unknown-test-tag";
                corrupted_override = true;
                break;
            }
        }
        if (corrupted_override)
            break;
    }
    TEST_ASSERT(corrupted_override, "fixture should contain a serialized typed override");
    TEST_ASSERT(nos::json::dump(malformed_override_data).find("unknown-test-tag") != std::string::npos,
                "fixture mutation should replace the serialized override tag");
    termin::TcSceneRef malformed_override_scene = termin::TcSceneRef::create("prefab-instance-override-malformed");
    ...
    TEST_ASSERT(
        termin::prefab::find_live_prefab_instances(malformed_override_scene.handle(), "prefab-document-asset").size() ==
            2,
        "instance with malformed typed overrides should be excluded from live queries");
    const std::string reserialized_malformed = nos::json::dump(malformed_override_scene.serialize());
    ...
    for (nos::trent& root_data : malformed_structural_data["entities"].as_list()) {
        for (nos::trent& component_data : root_data["components"].as_list()) {
            ...
            component_data["data"]["structural_overrides"].push_back(std::move(malformed));
            corrupted_structural_override = true;
            break;
        }
        if (corrupted_structural_override)
            break;
    }
    TEST_ASSERT(corrupted_structural_override, "fixture should contain serialized structural metadata");
    termin::TcSceneRef malformed_structural_scene = termin::TcSceneRef::create("prefab-instance-structural-malformed");
    TEST_ASSERT(malformed_structural_scene.load_from_data(malformed_structural_data) == 12,
                "malformed structural metadata should not corrupt hierarchy loading");
    TEST_ASSERT(termin::prefab::find_live_prefab_instances(malformed_structural_scene.handle(), "prefab-document-asset")
                        .size() == 2,
                "instance with malformed structural intent should be excluded from live queries");
    TEST_ASSERT(nos::json::dump(malformed_structural_scene.serialize()).find("unknown-structural-kind") !=
                    std::string::npos,
    ...
    termin::TcSceneRef malformed_tracking_scene = termin::TcSceneRef::create("prefab-instance-tracking-malformed");
    ...
    TEST_ASSERT(
        termin::prefab::find_live_prefab_instances(malformed_tracking_scene.handle(), "prefab-document-asset").size() ==
            2,
        "malformed state should be diagnosed and excluded from live queries");

    std::vector<termin::Entity> mutation_snapshot =
        termin::prefab::find_live_prefab_instances(tracking_scene.handle(), "prefab-document-asset");
    TEST_ASSERT(mutation_snapshot.size() == 3, "query should return a complete pre-mutation snapshot");
    mutation_snapshot[0].destroy_children();
    tc_entity_free(mutation_snapshot[0].handle());
    TEST_ASSERT(!mutation_snapshot[0].valid(), "snapshot handles should invalidate after entity destruction");
    TEST_ASSERT(termin::prefab::find_live_prefab_instances(tracking_scene.handle(), "prefab-document-asset").size() ==
                    2,
                "entity destruction should immediately remove native instance state from queries");

    termin::TcSceneRef manual_scene = termin::TcSceneRef::create("prefab-instance-manual-lifecycle");
    termin::Entity manual_root = manual_scene.create_entity("ManualRoot");
    for (int iteration = 0; iteration < 3; ++iteration) {
        auto* manual_state = new termin::prefab::PrefabInstanceState("manual-prefab-asset");
        manual_root.add_component(manual_state);
        TEST_ASSERT(termin::prefab::count_live_prefab_instances("manual-prefab-asset") == 1,
                    "repeated state attachment should register exactly one live instance");
        manual_root.remove_component(manual_state);
        TEST_ASSERT(termin::prefab::count_live_prefab_instances("manual-prefab-asset") == 0,
    ...
    for (const termin::Entity& stale : scene_teardown_snapshot) {
        TEST_ASSERT(!stale.valid(), "scene teardown should invalidate every previously returned handle");
    }
    TEST_ASSERT(termin::prefab::count_live_prefab_instances("prefab-document-asset") == 8,
