...
int main() {
    ...
    for (const char* field : {"priority", "layer", "flags"}) {
        ...
        TEST_ASSERT(rejects(malformed, std::string("root.") + field), "entity integer fields must reject fractions");
    }

    nos::trent priority_overflow = valid;
    priority_overflow["root"]["priority"] = static_cast<double>(std::numeric_limits<int>::max()) + 1.0;
    TEST_ASSERT(rejects(priority_overflow, "root.priority"), "priority must fit native int");

    for (const char* field : {"layer", "flags"}) {
        nos::trent negative = valid;
        negative["root"][field] = int64_t{-1};
        TEST_ASSERT(rejects(negative, std::string("root.") + field), "bit fields must be non-negative");
    }
    ...
    for (const VectorCase& item : vectors) {
        nos::trent malformed = valid;
        nos::trent& value =
            std::string(item.path) == "scale" ? malformed["root"]["scale"] : malformed["root"]["pose"][item.path];
        value.as_list().pop_back();
        const std::string expected_path =
            std::string("root.") + (std::string(item.path) == "scale" ? "scale" : std::string("pose.") + item.path);
        TEST_ASSERT(rejects(malformed, expected_path), "transform vectors must have their canonical arity");

        malformed = valid;
        nos::trent& non_numeric =
            std::string(item.path) == "scale" ? malformed["root"]["scale"] : malformed["root"]["pose"][item.path];
        non_numeric[0] = "not-a-number";
        TEST_ASSERT(rejects(malformed, expected_path + "[0]"), "transform vectors must contain finite numbers");
    }
    ...
    non_finite_json.replace(first_position_value, first_position_end - first_position_value, "1e9999");
    const auto non_finite = termin::prefab::PrefabDocument::parse_json(non_finite_json);
    TEST_ASSERT(!non_finite.ok() && non_finite.message.find("root.pose.position[0]") != std::string::npos,
