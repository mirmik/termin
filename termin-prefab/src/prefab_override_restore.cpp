...
namespace termin::prefab {
    namespace {
        ...
        Failure failure(const PrefabPropertyOverride& item, Error error, std::string message) {
            tc::Log::error("[PrefabPropertyApply] %s/%s/%s: %s",
        ...
        const nos::trent* find_source_entity(const nos::trent& entity, const std::string& source_id) {
            if (!entity.is_dict())
                return nullptr;
            if (entity["uuid"].as_string_default("") == source_id)
                return &entity;
        ...
        const nos::trent* find_source_component(const nos::trent& entity, const std::string& source_id) {
            if (!entity.contains("components") || !entity["components"].is_list())
                return nullptr;
            for (const nos::trent& component : entity["components"].as_list()) {
                if (component.is_dict() && component["source_id"].as_string_default("") == source_id) {
                    return &component;
                }
            }
            return nullptr;
        ...
        tc_component* find_runtime_component(const Entity& owner, const std::string& source_id, bool& duplicate) {
            duplicate = false;
        ...
        bool source_entity_is_suppressed(const nos::trent& source,
            ...
            const bool suppressed = ancestor_suppressed || state.source_entity_suppressed(source_id);
            if (source_id == target_id)
                return suppressed;
            if (!source.contains("children") || !source["children"].is_list())
                return false;
            for (const nos::trent& child : source["children"].as_list()) {
                if (source_entity_is_suppressed(child, target_id, state, suppressed))
                    return true;
            }
            return false;
        }
        ...
        bool finite_vector(const nos::trent& value, size_t size, double* result) {
            if (!value.is_list() || value.as_list().size() != size)
                return false;
            for (size_t index = 0; index < size; ++index) {
                if (!value.as_list()[index].is_numer())
                    return false;
        ...
        const nos::trent* entity_source_value(const nos::trent& source, const std::string& field_path) {
            ...
            if (field_path == "transform.rotation") {
                if (!source.contains("pose") || !source["pose"].is_dict() || !source["pose"].contains("rotation"))
                    return nullptr;
        ...
        bool require_kind(const PrefabPropertyOverride& item, const char* expected, Failure& result) {
            if (item.target_kind == expected)
                return true;
            result =
                failure(item,
                        Error::KindMismatch,
                        "target kind '" + item.target_kind + "' does not match source field kind '" + expected + "'");
            return false;
        }

        std::optional<Failure>
        apply_entity_value(const PrefabPropertyOverride& item, const nos::trent& source, Entity runtime) {
            ...
            if (item.field_path == "name") {
                if (!require_kind(item, "string", kind_failure))
                    return kind_failure;
                if (!value->is_string()) {
            ...
            if (item.field_path == "visible" || item.field_path == "enabled" || item.field_path == "pickable" ||
                item.field_path == "selectable") {
                if (!require_kind(item, "bool", kind_failure))
                    return kind_failure;
                if (!value->is_bool()) {
                    return failure(item, Error::InvalidSourceValue, "source flag must be a boolean");
                }
                const bool flag = value->as_bool();
                if (item.field_path == "visible")
                    runtime.set_visible(flag);
                else if (item.field_path == "enabled")
                    runtime.set_enabled(flag);
                else if (item.field_path == "pickable")
                    runtime.set_pickable(flag);
                else
                    runtime.set_selectable(flag);
                return std::nullopt;
            }
            if (item.field_path == "priority") {
                if (!require_kind(item, "int", kind_failure))
                    return kind_failure;
                if (!value->is_numer() || !std::isfinite(static_cast<double>(value->as_numer())) ||
                    ...
                    return failure(item, Error::InvalidSourceValue, "source priority must fit a signed native int");
                }
            ...
            if (!require_kind(item, rotation ? "quat" : "vec3", kind_failure))
                return kind_failure;
            double data[4] = {};
            ...
            if (item.field_path == "transform.position")
                runtime.set_local_position(data);
            else if (rotation)
                runtime.set_local_rotation(data);
            else
                runtime.set_local_scale(data);
            return std::nullopt;
        }

        std::optional<Failure> apply_component_value(const PrefabPropertyOverride& item,
                                                     const nos::trent& source_entity,
                                                     Entity runtime_entity,
                                                     const PrefabInstanceState& state,
                                                     const nos::trent& source_root) {
            const nos::trent* source_component = find_source_component(source_entity, item.source_component_id);
            if (source_component == nullptr) {
                return failure(
                    item, Error::SourceComponentNotFound, "source component ID does not belong to the source entity");
            }

            Entity owner = state.component_owner_for_source(item.source_component_id);
            if (!owner.valid()) {
                return failure(
                    item, Error::RuntimeComponentNotFound, "runtime component owner mapping does not resolve");
            }
            if (owner != runtime_entity) {
                return failure(item,
                               Error::ComponentOwnerMismatch,
                               "runtime component owner does not match the mapped source entity");
            }

            bool duplicate = false;
            tc_component* runtime_component = find_runtime_component(owner, item.source_component_id, duplicate);
            ...
            if (!source_component->contains("data") || !(*source_component)["data"].is_dict() ||
                ...
                return failure(
                    item, Error::FieldNotFound, "current source component has no serialized value for the field");
            }
            ...
            if (item.field_path == "display_name") {
                if (item.target_kind != "string" || !component_data[item.field_path].is_string()) {
                    return failure(item, Error::InvalidSourceValue, "component display_name must be a string");
                }
                tc_component_set_display_name(runtime_component, component_data[item.field_path].as_string().c_str());
                return std::nullopt;
            }
            if (item.field_path == "enabled" || item.field_path == "active_in_editor") {
                if (item.target_kind != "bool" || !component_data[item.field_path].is_bool()) {
                    return failure(item, Error::InvalidSourceValue, "component lifecycle field must be a boolean");
                }
                if (item.field_path == "enabled") {
                    tc_component_set_enabled(runtime_component, component_data[item.field_path].as_bool());
                } else {
                    tc_component_set_active_in_editor(runtime_component, component_data[item.field_path].as_bool());
                }
                return std::nullopt;
            }
            ...
            if (!field.is_serializable) {
                return failure(
                    item, Error::FieldNotSerializable, "inspect field is not part of serialized prefab source data");
            }
            if (field.kind == nullptr || item.target_kind != field.kind) {
                return failure(
                    item, Error::KindMismatch, "stored target kind does not match current inspect field kind");
            }
            ...
            auto remap_entity_reference = [&state, &source_root](nos::trent& reference) -> bool {
                if (!reference.is_dict() || !reference.contains("uuid") || !reference["uuid"].is_string())
                    return false;
                Entity mapped = state.entity_for_source(reference["uuid"].as_string());
                if (!mapped.valid()) {
                    if (source_entity_is_suppressed(source_root, reference["uuid"].as_string(), state)) {
                        reference["uuid"] = "";
                        return true;
                    }
                    return false;
            ...
            if (item.target_kind == "entity") {
                ...
                if (!source_value.is_list()) {
                    return failure(item, Error::InvalidSourceValue, "source entity reference list must be an array");
                }
            ...
            const tc_inspect_apply_result applied =
                tc_component_inspect_deserialize_checked(runtime_component, &one_field, &context);
            tc_value_free(&one_field);
            if (applied.status != TC_INSPECT_APPLY_OK || applied.applied_fields != 1) {
                const Error code = applied.status == TC_INSPECT_APPLY_KIND_CONVERSION_FAILED ? Error::InvalidSourceValue
                                                                                             : Error::SetterFailed;
                return failure(item, code, "inspect rejected the current serialized source value");
            }
            return std::nullopt;
        }

        std::optional<Failure> apply_source_value(const PrefabPropertyOverride& item,
                                                  const PrefabDocument& document,
                                                  const PrefabInstanceState& state) {
            const nos::trent* source_entity = find_source_entity(document.source_hierarchy(), item.source_entity_id);
            if (source_entity == nullptr) {
                return failure(
                    item, Error::SourceEntityNotFound, "source entity ID is absent from the current prefab document");
            }
            Entity runtime_entity = state.entity_for_source(item.source_entity_id);
            if (!runtime_entity.valid()) {
                return failure(item, Error::RuntimeEntityNotFound, "runtime entity mapping does not resolve");
            }
            if (item.source_component_id.empty()) {
                return apply_entity_value(item, *source_entity, runtime_entity);
            }
            return apply_component_value(item, *source_entity, runtime_entity, state, document.source_hierarchy());
        }

        std::optional<Failure> validate_operation(const PrefabInstanceState& state,
                                                  const PrefabDocument& document,
                                                  const PrefabPropertyOverride& identity) {
            const PrefabDocumentResult validation = document.validate();
            if (!validation.ok()) {
                return failure(
                    identity, Error::InvalidDocument, "current prefab document is invalid: " + validation.message);
            }
            if (document.asset_uuid() != state.prefab_asset_uuid()) {
                return failure(
                    identity, Error::DocumentMismatch, "prefab document asset UUID does not match the instance state");
            }
            ...
            if (!state.mapping_valid()) {
                return failure(identity, Error::InvalidState, "prefab source-to-runtime mapping is invalid");
            }
            return std::nullopt;
        }

        PrefabReconcileFailure reconcile_failure(PrefabReconcilePhase phase, const Failure& source) {
            return {
                phase,
                source.error,
        ...
        void index_source_entity(const nos::trent& entity, const std::string& parent_id, SourceIndex& index) {
            if (!entity.is_dict())
                return;
        ...
        std::vector<PrefabPropertyOverride> source_properties(const SourceIndex& index) {
            ...
            const auto add = [&result](const std::string& entity_id,
            ...
            for (const auto& [component_id, component] : index.components) {
                ...
                for (const auto& [path, value] : (*component)["data"].as_dict()) {
                    ...
                    if (path == "display_name")
                        kind = "string";
                    else if (path == "enabled" || path == "active_in_editor")
                        kind = "bool";
                    else {
                        tc_field_info field{};
        ...
        std::optional<Failure> apply_override_value(const PrefabPropertyOverride& item,
                                                    const PrefabDocument& document,
                                                    const PrefabInstanceState& state,
            ...
            if (source_entity == nullptr) {
                return failure(item,
                               Error::SourceEntityNotFound,
                               "override source entity is absent from the current prefab document");
            }
            Entity runtime_entity = state.entity_for_source(item.source_entity_id);
            if (!runtime_entity.valid()) {
                return failure(item, Error::RuntimeEntityNotFound, "override runtime entity mapping does not resolve");
            }

            std::string materialize_error;
            std::optional<tc::trent> materialized =
                resource_resolver
                    ? item.value.materialize_for_inspect(item.target_kind, *resource_resolver, materialize_error)
                    : item.value.materialize_for_inspect(item.target_kind, materialize_error);
            if (!materialized) {
                const Error code =
                    item.value.tag() == "resource" ? Error::ResourceResolutionFailed : Error::InvalidSourceValue;
            ...
            if (item.source_component_id.empty()) {
                if (item.field_path == "layer" || item.field_path == "flags") {
                    if (item.target_kind != "uint64") {
                        return failure(item, Error::KindMismatch, "entity bit-field override must target uint64");
                    }
                    uint64_t bits = 0;
                    if (materialized->is_integer()) {
                        ...
                        if (signed_bits < 0) {
                            return failure(
                                item, Error::InvalidSourceValue, "entity bit-field override must be non-negative");
                        }
                        ...
                        if (parsed.ec != std::errc() || parsed.ptr != end || std::to_string(bits) != encoded) {
                            return failure(
                                item, Error::InvalidSourceValue, "entity bit-field override is not canonical uint64");
                        }
                    } else {
                        return failure(item,
                                       Error::InvalidSourceValue,
                    ...
                    if (item.field_path == "layer")
                        runtime_entity.set_layer(bits);
                    else
                        runtime_entity.set_flags(bits);
                    return std::nullopt;
                }
                nos::trent synthetic;
            ...
            if (source_component == nullptr) {
                return failure(
                    item, Error::SourceComponentNotFound, "override source component is absent from its source entity");
            }
            Entity owner = state.component_owner_for_source(item.source_component_id);
            if (!owner.valid() || owner != runtime_entity) {
                return failure(item,
                               owner.valid() ? Error::ComponentOwnerMismatch : Error::RuntimeComponentNotFound,
            ...
            const std::string source_type = (*source_component)["type"].as_string_default("");
            const std::string runtime_type = tc_component_type_name(component) ? tc_component_type_name(component) : "";
            if (source_type.empty() || source_type != runtime_type) {
                return failure(item,
                               Error::ComponentTypeMismatch,
            ...
            if (item.field_path == "display_name") {
                if (item.target_kind != "string" || !materialized->is_string()) {
                    return failure(item, Error::KindMismatch, "display_name override must materialize as string");
                }
            ...
            if (item.field_path == "enabled" || item.field_path == "active_in_editor") {
                if (item.target_kind != "bool" || !materialized->is_bool()) {
                    return failure(item, Error::KindMismatch, "component lifecycle override must materialize as bool");
                }
                if (item.field_path == "enabled") {
            ...
            if (!field.is_serializable) {
                return failure(item, Error::FieldNotSerializable, "override target field is not serializable");
            }
            if (field.kind == nullptr || item.target_kind != field.kind) {
                return failure(
                    item, Error::KindMismatch, "stored override kind does not match current inspect field kind");
            }
            tc_scene_handle scene = tc_entity_pool_get_scene(runtime_entity.pool_ptr());
            tc_scene_inspect_context context = tc_scene_inspect_context_make(scene);
            tc_value one_field = tc_value_dict_new();
            tc_value_dict_set(&one_field, item.field_path.c_str(), tc_value_copy(materialized->raw()));
            const tc_inspect_apply_result applied =
                tc_component_inspect_deserialize_checked(component, &one_field, &context);
            tc_value_free(&one_field);
            if (applied.status != TC_INSPECT_APPLY_OK || applied.applied_fields != 1) {
                const Error code = applied.status == TC_INSPECT_APPLY_KIND_CONVERSION_FAILED ? Error::InvalidSourceValue
                                                                                             : Error::SetterFailed;
    ...
    PrefabOverrideRestoreResult PrefabInstanceState::clear_property_override(const PrefabDocument& source,
        ...
        const PrefabPropertyOverride* stored = property_override(source_entity_id, source_component_id, field_path);
        if (stored == nullptr) {
            result.failures.push_back(failure(identity, Error::OverrideNotFound, "property override does not exist"));
            return result;
        }
        ...
        if (!discard_property_override(source_entity_id, source_component_id, field_path)) {
            result.failures.push_back(
                failure(snapshot, Error::InvalidState, "override disappeared after its source value was restored"));
            return result;
        }
    ...
    PrefabOverrideRestoreResult PrefabInstanceState::clear_all_property_overrides(const PrefabDocument& source) {
        PrefabOverrideRestoreResult result;
        ...
        for (const PrefabPropertyOverride& item : snapshot) {
            ...
            if (!discard_property_override(item.source_entity_id, item.source_component_id, item.field_path)) {
                result.failures.push_back(
                    failure(item, Error::InvalidState, "override disappeared after its source value was restored"));
                continue;
            }
    ...
    PrefabInstanceState::reconcile_properties(const PrefabDocument& source,
                                              const PrefabOverrideResourceResolver* resource_resolver) {
        return reconcile(source, resource_resolver);
    }

    PrefabReconcileResult PrefabInstanceState::reconcile(const PrefabDocument& source,
                                                         const PrefabOverrideResourceResolver* resource_resolver) {
        ...
        PrefabPropertyOverride identity;
        if (auto invalid = validate_operation(*this, source, identity)) {
            result.failures.push_back(reconcile_failure(PrefabReconcilePhase::Validation, *invalid));
            return result;
        }
        ...
        for (const PrefabPropertyOverride& item : _property_overrides) {
            if (suppressed_entities.contains(item.source_entity_id) ||
                (!item.source_component_id.empty() && suppressed_components.contains(item.source_component_id))) {
        ...
        for (const PrefabPropertyOverride& item : properties) {
            ...
            if (!item.source_component_id.empty()) {
                if (!has_component_mapping(item.source_component_id))
                    continue;
                Entity owner = component_owner_for_source(item.source_component_id);
                bool duplicate = false;
                tc_component* runtime_component =
                    owner.valid() ? find_runtime_component(owner, item.source_component_id, duplicate) : nullptr;
                const nos::trent* source_component = index.components.contains(item.source_component_id)
                                                         ? index.components.at(item.source_component_id)
                                                         : nullptr;
                const std::string source_type =
                    source_component != nullptr ? (*source_component)["type"].as_string_default("") : "";
                const char* runtime_type =
                    runtime_component != nullptr ? tc_component_type_name(runtime_component) : nullptr;
                if (owner != runtime_entity || duplicate || runtime_component == nullptr || runtime_type == nullptr ||
                    source_type != runtime_type) {
                    continue;
                }
            }
            if (overridden.contains({item.source_entity_id, item.source_component_id, item.field_path})) {
                continue;
            }
            ++result.source_field_count;
            if (auto failed = apply_source_value(item, source, *this)) {
                result.failures.push_back(reconcile_failure(PrefabReconcilePhase::SourceValue, *failed));
            } else {
                ++result.source_fields_applied;
            }
        }
        for (const PrefabPropertyOverride& item : overrides) {
            if (auto failed = apply_override_value(item, source, *this, resource_resolver)) {
                result.failures.push_back(reconcile_failure(PrefabReconcilePhase::OverrideValue, *failed));
            } else {
        ...
        std::sort(result.failures.begin(), result.failures.end(), [](const auto& left, const auto& right) {
            return std::tie(left.phase, left.source_entity_id, left.source_component_id, left.field_path, left.error) <
                   std::tie(
                       right.phase, right.source_entity_id, right.source_component_id, right.field_path, right.error);
        });
        if (result.failures.empty() && result.source_fields_applied == result.source_field_count &&
