...
namespace termin::prefab {
    namespace {
        ...
        bool has_only_keys(const tc_value* value, std::initializer_list<std::string_view> allowed, std::string& error) {
            if (value == nullptr || value->type != TC_VALUE_DICT) {
        ...
        bool require_string(const tc_value* object,
                            const char* key,
                            std::string& result,
                            std::string& error,
        ...
        bool parse_int64_string(const std::string& text, int64_t& result) {
            ...
            return parsed.ec == std::errc() && parsed.ptr == end && std::to_string(result) == text;
        }
        ...
        bool validate_node(const tc_value* node, size_t depth, ValidationBudget& budget, std::string& error);

        bool validate_items(const tc_value* items, size_t depth, ValidationBudget& budget, std::string& error) {
        ...
        bool validate_node(const tc_value* node, size_t depth, ValidationBudget& budget, std::string& error) {
            ...
            if (tag == "bool") {
                if (!has_only_keys(node, {"tag", "value"}, error))
                    return false;
            ...
            if (tag == "float64") {
                ...
                std::string text;
                double parsed = 0.0;
                if (!require_string(node, "value", text, error) || !parse_finite_double_string(text, parsed)) {
            ...
            if (tag == "list" || tag == "tuple") {
                if (!has_only_keys(node, {"tag", "items"}, error))
                    return false;
                return validate_items(dict_value(node, "items"), depth, budget, error);
            }
            if (tag == "dict") {
                if (!has_only_keys(node, {"tag", "entries"}, error))
                    return false;
                ...
                for (size_t index = 0; index < tc_value_list_size(entries); ++index) {
                    ...
                    std::string key;
                    if (!require_string(entry, "key", key, error, true))
                        return false;
            ...
            if (tag == "array") {
                if (!has_only_keys(node, {"tag", "dtype", "shape", "items"}, error))
                    return false;
                std::string dtype;
                if (!require_string(node, "dtype", dtype, error))
                    return false;
                static const std::unordered_set<std::string> supported = {"bool",
                                                                          "int8",
                                                                          "int16",
                                                                          "int32",
                                                                          "int64",
                ...
                if (shape == nullptr || shape->type != TC_VALUE_LIST || items == nullptr ||
                    items->type != TC_VALUE_LIST) {
                ...
                if (!validate_items(items, depth, budget, error))
                    return false;
                ...
                for (size_t index = 0; index < tc_value_list_size(items); ++index) {
                    const tc_value* item = tc_value_list_get(const_cast<tc_value*>(items), index);
                    std::string item_tag;
                    if (!require_string(item, "tag", item_tag, error))
                        return false;
                    ...
                    if (is_bool) {
                        ...
                        compatible = item_tag == "float64" || item_tag == "int64" || item_tag == "uint64";
                    }
                    if (!compatible) {
                        error = "array item " + std::to_string(index) + " is incompatible with dtype '" + dtype + "'";
                        return false;
                    }
                }
                return true;
            ...
            if (tag == "kind") {
                if (!has_only_keys(node, {"tag", "kind", "payload"}, error))
                    return false;
                std::string kind;
                if (!require_string(node, "kind", kind, error))
                    return false;
            ...
            if (tag == "resource") {
                ...
                if (!require_string(node, "resource_type", ignored, error) ||
                    !require_string(node, "kind", ignored, error) || !require_string(node, "uuid", ignored, error)) {
                    return false;
                }
        ...
        std::optional<tc::trent> materialize_node(const tc_value* node, std::string& error) {
            ...
            if (tag == "none")
                return tc::trent::nil();
            if (tag == "bool")
                return tc::trent::boolean(dict_value(node, "value")->data.b);
            if (tag == "int64") {
                int64_t value = 0;
            ...
            if (tag == "string")
                return tc::trent::string(dict_value(node, "value")->data.s);
            if (tag == "list" || tag == "tuple" || tag == "array") {
                tc::trent result = tc::trent::list();
                const tc_value* items = dict_value(node, "items");
                for (size_t index = 0; index < tc_value_list_size(items); ++index) {
                    auto child = materialize_node(tc_value_list_get(const_cast<tc_value*>(items), index), error);
                    if (!child)
                        return std::nullopt;
            ...
            if (tag == "kind")
                return materialize_node(dict_value(node, "payload"), error);
            if (tag == "resource") {
                tc::trent result = tc::trent::dict();
                result.set("uuid", dict_value(node, "uuid")->data.s);
                const tc_value* name = dict_value(node, "name");
                if (name != nullptr)
                    result.set("name", name->data.s);
                return result;
            }
        ...
        std::optional<tc::trent> encode_inspect_node(const tc_value* value, size_t depth, std::string& error) {
            ...
            case TC_VALUE_DOUBLE: {
                const double number =
                    value->type == TC_VALUE_FLOAT ? static_cast<double>(value->data.f) : value->data.d;
                if (!std::isfinite(number)) {
                    error = "inspect override floats must be finite";
                    return std::nullopt;
                }
                char buffer[64];
                auto formatted = std::to_chars(buffer,
            ...
            case TC_VALUE_DICT: {
                ...
                for (size_t index = 0; index < value->data.dict.count; ++index) {
                    const char* key = nullptr;
                    tc_value* child = tc_value_dict_get_at(const_cast<tc_value*>(value), index, &key);
    ...
    std::optional<PrefabOverrideValue> PrefabOverrideValue::parse(const tc_value* encoded, std::string& error) {
        error.clear();
    ...
    std::optional<PrefabOverrideValue> PrefabOverrideValue::parse_json(const std::string& json, std::string& error) {
        try {
    ...
    std::optional<PrefabOverrideValue>
    PrefabOverrideValue::from_inspect_value(const tc_value* value, std::string_view target_kind, std::string& error) {
        error.clear();
    ...
    std::optional<tc::trent> PrefabOverrideValue::materialize_for_inspect(std::string_view target_kind,
    ...
    std::optional<tc::trent>
    PrefabOverrideValue::materialize_for_inspect(std::string_view target_kind,
        ...
        const tc_value* name = dict_value(node, "name");
        tc::trent result;
        if (!resource_resolver.resolve(dict_value(node, "resource_type")->data.s,
                                       stored_kind,
                                       ...
                                       error)) {
            if (error.empty())
                error = "resource resolver rejected prefab override resource";
            return std::nullopt;
        }
        return result;
    }
