...
namespace termin::prefab {
    namespace {
        ...
        bool
        require_string(const nos::trent& entity, const char* field, const std::string& path, std::string& message) {
        ...
        bool require_bool(const nos::trent& entity, const char* field, const std::string& path, std::string& message) {
        ...
        bool require_integer(const nos::trent& entity,
                             const char* field,
                             long double minimum,
                             long double maximum,
        ...
        bool require_finite_vector(const nos::trent& value,
                                   ...
                                   std::string& message) {
            if (!value.is_list()) {
                message = path + " must be a list of " + std::to_string(expected_size) + " finite numbers";
                return false;
            }
            const auto& items = value.as_list();
            if (items.size() != expected_size) {
                message = path + " must contain exactly " + std::to_string(expected_size) + " finite numbers";
                return false;
            }
        ...
        bool validate_entity_base(const nos::trent& entity, const std::string& path, std::string& message) {
            if (!require_string(entity, "name", path, message))
                return false;
            for (const char* field : {"visible", "enabled", "pickable", "selectable"}) {
                if (!require_bool(entity, field, path, message))
                    return false;
            }
            if (!require_integer(entity,
                                 ...
                                 "must be an integer in the native int range",
                                 path,
                                 message)) {
                return false;
            }
            ...
            if (!entity.contains("scale") || !require_finite_vector(entity["scale"], 3, path + ".scale", message)) {
        ...
        bool validate_entity(const nos::trent& entity,
            ...
            for (const nos::trent& component : entity["components"].as_list()) {
                const std::string component_path = path + ".components[" + std::to_string(component_index) + "]";
                ...
                if (!component_ids.insert(component_id).second) {
                    message = component_path + ".source_id duplicates component source identity '" + component_id + "'";
                    return false;
                }
                ++component_index;
            }
    ...
    PrefabDocumentResult PrefabDocument::parse_json(const std::string& json) {
        ...
        try {
            ...
            return failure(PrefabDocumentError::InvalidJson, std::string("invalid JSON: ") + error.what());
        }
        ...
        if (version != CurrentVersion) {
            return failure(PrefabDocumentError::UnsupportedVersion,
    ...
    PrefabDocumentResult PrefabDocument::capture(const std::string& asset_uuid, const Entity& editable_root) {
    ...
    PrefabDocumentResult PrefabDocument::empty(const std::string& asset_uuid,
    ...
    PrefabDocumentResult PrefabDocument::materialize_source(tc_scene_handle target_scene, const Entity& parent) const {
        PrefabDocumentResult validation = validate();
        if (!validation.ok()) {
            return validation;
        }
        ...
        if (!root.valid()) {
            return failure(PrefabDocumentError::SourceMaterializationFailed,
