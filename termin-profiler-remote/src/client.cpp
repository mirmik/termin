...
bool transfer_exact(Bytes bytes, const std::atomic<bool> &running,
                    Transfer transfer) {
  std::size_t offset = 0;
...
bool send_bytes(Socket socket, std::span<const std::uint8_t> bytes,
  ...
  return transfer_exact(
      bytes, running, [socket](std::span<const std::uint8_t> remaining) {
...
bool receive_bytes(Socket socket, std::span<std::uint8_t> bytes,
                   const std::atomic<bool> &running) {
  return transfer_exact(
      bytes, running, [socket](std::span<std::uint8_t> remaining) {
...
CodecResult<DecodedMessage> receive_message(Socket socket,
  ...
  if (!receive_bytes(socket, bytes, running))
    return {.value = std::nullopt, .error = CodecError::truncated,
...
public:
  Impl(ClientConfig target_config, MessageHandler target_handler,
       DisconnectHandler target_disconnect_handler)
      : config(std::move(target_config)), handler(std::move(target_handler)),
    ...
    if (config.address != "127.0.0.1" || config.port == 0 ||
        config.authentication_token.empty() || !handler)
  ...
  bool send_message(Socket socket, const Message &message,
                    std::uint64_t &sequence, std::uint64_t session_id) {
    auto encoded = encode_message(message, sequence++, session_id);
    if (!encoded) {
      tc_log_error("remote profiler client: encode failed: %s",
    ...
    while (running.load(std::memory_order_acquire)) {
      ...
      if (socket == invalid_socket) {
        if (!reported_unavailable) {
          ...
          notify_disconnect("remote target is unavailable at 127.0.0.1:" +
      ...
      if (!response ||
          !std::holds_alternative<TargetHello>(response.value->message)) {
        std::string detail = response.detail;
        if (response &&
            std::holds_alternative<ErrorEvent>(response.value->message))
          detail = std::get<ErrorEvent>(response.value->message).detail;
        if (detail.empty())
          detail = "unexpected handshake response";
        tc_log_error("remote profiler client: target handshake failed: %s",
                     detail.c_str());
        release(socket);
      ...
      tc_log_info("remote profiler client: connected to 127.0.0.1:%u",
                  static_cast<unsigned>(config.port));
  ...
  void serve(Socket socket, std::uint64_t sequence, std::uint64_t session_id) {
    while (running.load(std::memory_order_acquire)) {
      ...
      if (!message) {
        if (running.load(std::memory_order_acquire))
          tc_log_error("remote profiler client: malformed/closed stream: %s",
  ...
  void discard_pending_commands() {
    ...
    while (commands.try_pop(discarded))
      ++count;
    if (count != 0)
      tc_log_error("remote profiler client: discarded %zu stale commands",
                   count);
  }
...
bool RemoteProfilerClient::start() { return impl_->start(); }
void RemoteProfilerClient::stop() { impl_->stop(); }
