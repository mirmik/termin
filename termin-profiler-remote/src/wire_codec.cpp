...
public:
  ...
  bool string(std::string &value, std::uint32_t limit, const char *field) {
    ...
    if (length > limit) {
      ...
      detail_ = std::string(field) + " length " + std::to_string(length) +
  ...
  bool count(std::uint32_t &value, std::uint32_t limit, const char *field) {
    ...
    if (value > limit) {
      error_ = CodecError::limit_exceeded;
      detail_ = std::string(field) + " count " + std::to_string(value) +
                " exceeds limit " + std::to_string(limit);
      return false;
    }
    return true;
...
private:
  bool available(std::size_t count) {
    ...
    detail_ =
        "message payload is truncated at byte " + std::to_string(position_);
    return false;
  }
      ...
      [&](const auto &value) -> std::optional<CodecResult<Bytes>> {
        ...
        if constexpr (std::is_same_v<T, ClientHello>) {
          ...
          if (value.max_payload_bytes > WireLimits::max_payload_bytes ||
            ...
            return limit_error(
                "ClientHello advertises a limit above the local hard limit");
          if (value.authentication_token.size() > WireLimits::max_token_bytes)
            return limit_error(
                "ClientHello authentication token exceeds hard limit");
          ...
          if (!version_is_compatible(value.negotiated_major,
            ...
            return invalid_value(
                "TargetHello negotiated an incompatible version");
          if (value.max_payload_bytes > WireLimits::max_payload_bytes ||
              value.max_frames_per_batch > WireLimits::max_frames_per_batch ||
              value.max_sections_per_frame > WireLimits::max_sections_per_frame)
            return limit_error(
                "TargetHello advertises a limit above the local hard limit");
          for (const auto *field : {&value.platform, &value.abi,
                                    &value.build_type, &value.build_id})
            if (field->size() > WireLimits::max_identity_bytes)
              return limit_error(
                  "TargetHello identity string exceeds hard limit");
          ...
          for (const auto &entry : value.entries) {
            if (entry.id == 0 || !ids.insert(entry.id).second)
              return invalid_value(
                  "DictionaryAdd IDs must be non-zero and unique");
            if (entry.name.empty() ||
                entry.name.size() > WireLimits::max_name_bytes)
              return limit_error(
                  "DictionaryAdd name length is outside hard limits");
            out.u32(entry.id);
          ...
          for (const auto &frame : value.frames) {
            if (!valid_finite_nonnegative(frame.start_time_ms) ||
              ...
              return invalid_value(
                  "FrameBatch contains a non-finite or negative timing");
            ...
            if (!frame.sections_profiled && !frame.sections.empty())
              return invalid_value(
                  "FrameBatch cadence-only frame contains sections");
            ...
            for (const auto &section : frame.sections) {
              if (section.name_id == 0 ||
                  !valid_finite_nonnegative(section.cpu_ms) ||
                  !valid_finite_nonnegative(section.children_ms) ||
                  section.children_ms > section.cpu_ms ||
                  section.parent_index < -1 || section.parent_index >= count ||
                  section.first_child < -1 || section.first_child >= count ||
                  section.next_sibling < -1 || section.next_sibling >= count)
                return invalid_value(
          ...
          if (!valid_gap(value.kind) ||
              value.last_missing_frame < value.first_missing_frame)
          ...
          if (!valid_drop(value.kind) || value.dropped_batches == 0 ||
              value.dropped_frames == 0)
          ...
          if (value.detail.empty() ||
            ...
            return limit_error(
                "ErrorEvent detail length is outside hard limits");
          out.u32(value.code);
...
CodecResult<Message> decode_payload(MessageType type,
  ...
  case MessageType::client_hello: {
    ClientHello v;
    ok = in.u16(v.minimum_major) && in.u16(v.minimum_minor) &&
         in.u64(v.capabilities) && in.u32(v.max_payload_bytes) &&
         in.u32(v.max_frames_per_batch) && in.u32(v.max_sections_per_frame) &&
         in.string(v.authentication_token, WireLimits::max_token_bytes,
                   "authentication token");
    if (ok && (v.minimum_major == 0 ||
               v.max_payload_bytes > WireLimits::max_payload_bytes ||
  ...
  case MessageType::target_hello: {
    TargetHello v;
    ok =
        in.u16(v.negotiated_major) && in.u16(v.negotiated_minor) &&
        in.u64(v.capabilities) && in.u64(v.clock_frequency_hz) &&
        in.u32(v.process_id) && in.u32(v.max_payload_bytes) &&
        in.u32(v.max_frames_per_batch) && in.u32(v.max_sections_per_frame) &&
        in.boolean(v.capturing) && in.boolean(v.profiling_sections) &&
  ...
  case MessageType::control: {
    Control v;
    ok = in.u64(v.request_id) && read_enum_u8(in, v.kind) &&
         in.boolean(v.enabled) && in.u64(v.client_time_ns);
    if (ok && !valid_control(v.kind))
      return failure<Message>(CodecError::invalid_value,
                              "Control command kind is unknown");
    result = v;
    break;
  }
  case MessageType::status: {
    Status v;
    ok = in.u64(v.request_id) && in.boolean(v.capturing) &&
         in.boolean(v.profiling_sections) && in.u64(v.completed_frames) &&
         in.u64(v.dropped_frames) && in.u32(v.queue_depth) &&
         in.u64(v.target_time_ns) &&
         in.string(v.detail, WireLimits::max_detail_bytes, "status detail");
    result = std::move(v);
    break;
  }
  case MessageType::dictionary_add: {
    ...
    ok =
        in.count(count, WireLimits::max_dictionary_entries, "dictionary entry");
    if (ok) {
      std::unordered_set<std::uint32_t> ids;
      ids.reserve(count);
      ...
      for (std::uint32_t i = 0; i < count && ok; ++i) {
        ...
        ok = in.u32(entry.id) &&
             in.string(entry.name, WireLimits::max_name_bytes, "section name");
        if (ok && (entry.id == 0 || entry.name.empty() ||
                   !ids.insert(entry.id).second))
          return failure<Message>(
              CodecError::invalid_value,
  ...
  case MessageType::frame_batch: {
    ...
    for (std::uint32_t frame_index = 0; frame_index < frame_count && ok;
         ++frame_index) {
      WireFrame frame;
      std::uint32_t section_count = 0;
      ok = in.i64(frame.frame_number) && in.f64(frame.start_time_ms) &&
           in.f64(frame.interval_ms) && in.f64(frame.active_ms) &&
           in.f64(frame.target_interval_ms) &&
           in.f64(frame.deadline_lateness_ms) &&
           in.u32(frame.missed_intervals) &&
           in.boolean(frame.sections_profiled) &&
           in.count(section_count, WireLimits::max_sections_per_frame,
                    "frame section");
      if (ok && (!valid_finite_nonnegative(frame.start_time_ms) ||
                 !valid_finite_nonnegative(frame.interval_ms) ||
                 !valid_finite_nonnegative(frame.active_ms) ||
                 !valid_finite_nonnegative(frame.target_interval_ms) ||
                 !valid_finite_nonnegative(frame.deadline_lateness_ms) ||
      ...
      for (std::uint32_t section_index = 0; section_index < section_count && ok;
           ++section_index) {
        WireSection section;
        ok = in.u32(section.name_id) && in.f64(section.cpu_ms) &&
             in.f64(section.children_ms) && in.u32(section.call_count) &&
             in.i32(section.parent_index) && in.i32(section.first_child) &&
             in.i32(section.next_sibling);
        const auto count = static_cast<std::int32_t>(section_count);
        if (ok && (section.name_id == 0 ||
                   !valid_finite_nonnegative(section.cpu_ms) ||
                   !valid_finite_nonnegative(section.children_ms) ||
                   section.children_ms > section.cpu_ms ||
                   section.parent_index < -1 || section.parent_index >= count ||
  ...
  case MessageType::gap: {
    GapEvent v;
    ok = read_enum_u8(in, v.kind) && in.i64(v.first_missing_frame) &&
         in.i64(v.last_missing_frame) && in.u64(v.after_sequence);
    if (ok &&
        (!valid_gap(v.kind) || v.last_missing_frame < v.first_missing_frame))
      return failure<Message>(CodecError::invalid_value,
                              "GapEvent kind or range is invalid");
    result = v;
    break;
  }
  case MessageType::drop: {
    DropEvent v;
    ok = read_enum_u8(in, v.kind) && in.u64(v.dropped_batches) &&
         in.u64(v.dropped_frames) && in.u64(v.after_sequence);
    if (ok && (!valid_drop(v.kind) || v.dropped_batches == 0 ||
               v.dropped_frames == 0))
      return failure<Message>(CodecError::invalid_value,
                              "DropEvent kind or counts are invalid");
    result = v;
    break;
  }
  ...
  if (!ok) {
    const auto error =
        in.error() == CodecError::none ? CodecError::truncated : in.error();
    const auto detail =
        in.detail().empty() ? "message payload is truncated" : in.detail();
...
CodecResult<Envelope> decode_envelope(std::span<const std::uint8_t> bytes) {
  if (bytes.size() < envelope_size)
    return failure<Envelope>(CodecError::truncated,
                             "envelope is truncated: got " +
                                 std::to_string(bytes.size()) + " of 32 bytes");

  Reader in(bytes.first(envelope_size));
  std::uint32_t magic = 0;
  std::uint16_t raw_type = 0;
  Envelope envelope;
  if (!in.u32(magic) || !in.u16(envelope.major) || !in.u16(envelope.minor) ||
      !in.u16(raw_type) || !in.u16(envelope.flags) ||
      !in.u32(envelope.payload_length) || !in.u64(envelope.sequence) ||
  ...
  if (envelope.payload_length > WireLimits::max_payload_bytes)
    return failure<Envelope>(CodecError::limit_exceeded,
                             "payload length " +
                                 std::to_string(envelope.payload_length) +
                                 " exceeds hard limit");
...
decode_message(std::span<const std::uint8_t> bytes) {
  ...
  if (bytes.size() != expected)
    return failure<DecodedMessage>(
        CodecError::invalid_length,
        "envelope declares " + std::to_string(expected) +
            " total bytes, received " + std::to_string(bytes.size()));
