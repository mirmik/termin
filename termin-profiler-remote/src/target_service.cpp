...
enum class WaitResult { ready, timeout, failed };

WaitResult wait_socket(Socket socket, bool write,
  ...
  return interrupted_error(socket_error()) ? WaitResult::timeout
                                           : WaitResult::failed;
}

template <typename Bytes, typename Transfer>
bool transfer_exact(Socket socket, Bytes bytes, bool write,
                    const std::atomic<bool> &running,
                    std::chrono::milliseconds timeout, Transfer transfer) {
  std::size_t offset = 0;
  ...
  while (offset < bytes.size() && running.load(std::memory_order_acquire)) {
    ...
    if (wait == WaitResult::failed) {
      tc_log_error("remote profiler target: socket wait failed (error=%d)",
                   socket_error());
      return false;
    }
    ...
    tc_log_error("remote profiler target: socket %s failed (error=%d)",
                 write ? "send" : "receive", error);
    return false;
  }
  return offset == bytes.size();
}

bool send_bytes(Socket socket, std::span<const std::uint8_t> bytes,
                const std::atomic<bool> &running) {
  return transfer_exact(
      socket, bytes, true, running, std::chrono::seconds(2),
      [socket](std::span<const std::uint8_t> remaining) {
        return send(socket, reinterpret_cast<const char *>(remaining.data()),
                    static_cast<int>(remaining.size()), 0);
      });
}

bool receive_bytes(Socket socket, std::span<std::uint8_t> bytes,
                   const std::atomic<bool> &running) {
  return transfer_exact(
      socket, bytes, false, running, std::chrono::seconds(2),
      [socket](std::span<std::uint8_t> remaining) {
        return recv(socket, reinterpret_cast<char *>(remaining.data()),
                    static_cast<int>(remaining.size()), 0);
      });
}

CodecResult<DecodedMessage> receive_message(Socket socket,
                                            const std::atomic<bool> &running) {
...
public:
  explicit Impl(TargetServiceConfig target_config)
    ...
    validate_config();
    capture =
        tc_profiler_capture_create(static_cast<int>(config.capture_capacity));
    if (!capture) {
      throw std::runtime_error(
          "remote profiler target failed to create capture");
    }
  ...
  bool start() {
    ...
    if (config.bind_address != "127.0.0.1") {
      tc_log_error(
          "remote profiler target: non-loopback bind '%s' is forbidden",
          config.bind_address.c_str());
      return false;
    }
    if (config.authentication_token.empty()) {
      tc_log_error(
          "remote profiler target: authentication token must not be empty");
      return false;
    }
#if defined(_WIN32)
    WSADATA data{};
    ...
    if (listener == invalid_socket) {
      tc_log_error("remote profiler target: socket creation failed (error=%d)",
                   socket_error());
    ...
    if (bind(listener, reinterpret_cast<const sockaddr *>(&address),
      ...
      tc_log_error("remote profiler target: loopback bind/listen failed "
                   "(port=%u error=%d)",
                   static_cast<unsigned>(config.port), socket_error());
    ...
    if (getsockname(listener, reinterpret_cast<sockaddr *>(&actual),
                    &actual_size) != 0) {
      tc_log_error("remote profiler target: getsockname failed (error=%d)",
                   socket_error());
      close_socket(listener);
      cleanup_socket_runtime();
      return false;
  ...
  void pump_frame_thread() {
    ...
    while (offset < static_cast<std::size_t>(range.count)) {
      const std::size_t count =
          std::min(config.frames_per_batch,
                   static_cast<std::size_t>(range.count) - offset);
      OutboundPacket packet;
      FrameBatch batch;
      ...
      for (std::size_t index = 0; index < count; ++index) {
        const tc_frame_profile *source = tc_profiler_capture_at(
            capture, range.first_index + static_cast<int>(offset + index));
        if (!source) {
        ...
        for (int section_index = 0; section_index < source->section_count;
             ++section_index) {
          ...
          if (packet_names.insert(name_id).second) {
            packet.required_names.push_back(DictionaryEntry{
                name_id, names_by_id[static_cast<std::size_t>(name_id - 1)]});
          }
        }
...
private:
  void validate_config() {
    if (config.capture_capacity == 0 || config.capture_capacity > INT_MAX) {
      throw std::invalid_argument(
          "remote profiler target capture capacity is invalid");
    }
    if (config.frames_per_batch == 0 ||
        config.frames_per_batch > WireLimits::max_frames_per_batch) {
      throw std::invalid_argument(
          "remote profiler target frames-per-batch is invalid");
    }
    if (config.authentication_token.size() > WireLimits::max_token_bytes) {
      throw std::invalid_argument(
          "remote profiler target token exceeds wire limit");
    }
    for (const auto *field : {&config.platform, &config.abi, &config.build_type,
                              &config.build_id}) {
      if (field->size() > WireLimits::max_identity_bytes) {
        throw std::invalid_argument(
            "remote profiler target identity exceeds wire limit");
      }
    }
  }
  ...
  Status current_status(std::uint64_t request_id,
  ...
  void apply_command(const Control &control) {
    ...
    case ControlKind::set_sections:
      ...
      detail = control.enabled ? "section profiling enabled"
                               : "section profiling disabled";
      break;
  ...
  void enqueue_frame_packet(OutboundPacket packet) {
    ...
    if (pending_batches != 0) {
      packet.drop = DropEvent{DropKind::producer_queue, pending_batches,
                              pending_frames, 0};
    }
  ...
  bool send_wire(Socket socket, const Message &message, std::uint64_t &sequence,
                 std::uint64_t session_id) {
    auto encoded = encode_message(message, sequence++, session_id);
    if (!encoded) {
      tc_log_error(
          "remote profiler target: failed to encode outgoing message: %s",
  ...
  bool handshake(Socket socket, std::uint64_t &sequence,
    ...
    if (!version_ok || !constant_time_equal(hello.authentication_token,
                                            config.authentication_token)) {
      const std::string detail = !version_ok
                                     ? "client version range is incompatible"
                                     : "client authentication token is invalid";
      send_wire(socket,
                ErrorEvent{401, decoded.value->envelope.sequence, detail},
                sequence, session_id);
      tc_log_error("remote profiler target: rejected client: %s",
                   detail.c_str());
      return false;
    }
    TargetHello target;
    target.capabilities =
        static_cast<std::uint64_t>(Capability::cadence_capture) |
  ...
  bool send_packet(Socket socket, OutboundPacket &packet,
                   std::unordered_set<std::uint32_t> &sent_names,
                   std::uint64_t &sequence, std::uint64_t session_id) {
    ...
    if (!additions.entries.empty() &&
        !send_wire(socket, additions, sequence, session_id)) {
      return false;
    }
    ...
    if (packet.status &&
        !send_wire(socket, *packet.status, sequence, session_id)) {
      return false;
    }
    if (packet.frames &&
        !send_wire(socket, *packet.frames, sequence, session_id)) {
      return false;
    }
    return true;
  }
  ...
  void serve_client(Socket socket, std::uint64_t sequence,
                    std::uint64_t session_id) {
    std::unordered_set<std::uint32_t> sent_names;
    while (running.load(std::memory_order_acquire)) {
      OutboundPacket packet;
      ...
      if (!std::holds_alternative<Control>(decoded.value->message)) {
        send_wire(
            socket,
            ErrorEvent{400, decoded.value->envelope.sequence,
      ...
      if (control.kind == ControlKind::disconnect) {
        send_wire(socket, current_status(control.request_id, "disconnecting"),
                  sequence, session_id);
        return;
      }
      if (!command_queue.try_push(FrameCommand{control})) {
        tc_log_error(
            "remote profiler target: command queue full; command rejected");
        if (!send_wire(socket,
                       ErrorEvent{429, decoded.value->envelope.sequence,
  ...
  void release_client(Socket socket) {
    ...
    if (client_socket.compare_exchange_strong(expected, invalid_socket,
...
bool RemoteProfilerTarget::start() { return impl_->start(); }

void RemoteProfilerTarget::stop() { impl_->stop(); }

void RemoteProfilerTarget::pump_frame_thread() { impl_->pump_frame_thread(); }
