...
bool receive_all(TestSocket socket, std::span<std::uint8_t> bytes) {
  ...
  while (offset < bytes.size()) {
    const int received =
        recv(socket, reinterpret_cast<char *>(bytes.data() + offset),
             static_cast<int>(bytes.size() - offset), 0);
    if (received <= 0) {
      return false;
    }
...
bool send_message(TestSocket socket, const Message &message,
...
TestSocket handshake(RemoteProfilerTarget &target, const std::string &token,
...
TEST_CASE(
    "Bounded SPSC queue preserves order and rejects complete newest values") {
...
TEST_CASE(
    "Target refuses non-loopback listeners and has idempotent lifecycle") {
...
TEST_CASE("Target stop interrupts a stalled partial handshake") {
  ...
  REQUIRE(send_all(
      client,
      std::span<const std::uint8_t>(*encoded.value).first(envelope_size)));
...
TEST_CASE("Target streams real profiler frames and acknowledges frame-thread "
  ...
  REQUIRE(client != invalid_test_socket);
  REQUIRE(
      send_message(client, Control{10, ControlKind::set_sections, true, 0}, 2));
  REQUIRE(send_message(client,
                       Control{11, ControlKind::start_capture, false, 0}, 3));
  REQUIRE(wait_until([&] {
  ...
  for (int index = 0; index < 8 && !saw_frame; ++index) {
    ...
    if (const auto *status = std::get_if<Status>(&message->message)) {
      saw_start_ack |= status->request_id == 11 && status->capturing;
                   ...
                   std::get_if<DictionaryAdd>(&message->message)) {
...
TEST_CASE(
    "Disconnected overflow drops complete batches and reports the next gap") {
  ProfilerCleanup cleanup;
  TargetServiceConfig config;
  ...
  REQUIRE(client != invalid_test_socket);
  REQUIRE(send_message(client,
                       Control{20, ControlKind::start_capture, false, 0}, 2));
  REQUIRE(wait_until([&] {
    target.pump_frame_thread();
    return target.status().capturing;
  }));
