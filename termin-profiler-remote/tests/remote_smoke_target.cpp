...
int main(int argc, char **argv) {
  ...
  while (std::chrono::steady_clock::now() < deadline) {
    ...
    if (capture_sequence_complete && !status.client_connected) {
      std::cout << "COMPLETE frames=" << status.completed_frames
  ...
  const TargetServiceStatus status = target.status();
  std::cerr << "Remote profiler target timed out: connected="
            << status.client_connected << " capturing=" << status.capturing
            << " detailed=" << status.profiling_sections
