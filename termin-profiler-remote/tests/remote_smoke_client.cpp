...
int main(int argc, char **argv) {
  ...
  RemoteProfilerClient client(
      ...
      [&](const DecodedMessage &decoded) {
        ...
        if (const auto *hello = std::get_if<TargetHello>(&decoded.message)) {
          ...
          observed.start_cadence_acknowledged |=
              status->request_id == 2 && status->capturing &&
              !status->profiling_sections;
          observed.pause_cadence_acknowledged |=
              status->request_id == 3 && !status->capturing;
          observed.start_detailed_acknowledged |=
              status->request_id == 5 && status->capturing &&
              status->profiling_sections;
          observed.pause_detailed_acknowledged |=
              status->request_id == 6 && !status->capturing;
        } else if (const auto *batch =
                       std::get_if<FrameBatch>(&decoded.message)) {
          for (const WireFrame &frame : batch->frames) {
            if (observed.start_cadence_acknowledged &&
                !observed.pause_cadence_acknowledged &&
                frame.frame_number > 0 && !frame.sections_profiled &&
                !observed.cadence_frame)
              observed.cadence_frame = frame;
            if (observed.start_detailed_acknowledged &&
                !observed.pause_detailed_acknowledged &&
                frame.sections_profiled && !frame.sections.empty() &&
                !observed.detailed_frame)
              observed.detailed_frame = frame;
          }
        } else if (const auto *error =
                       std::get_if<ErrorEvent>(&decoded.message)) {
      ...
      [&](std::string detail) {
        ...
        if (observed.hello && !observed.pause_detailed_acknowledged &&
  ...
  if (!client.send_control(Control{1, ControlKind::set_sections, false}) ||
      ...
      !wait_for(observed, [&] {
        return (observed.start_cadence_acknowledged &&
                observed.cadence_frame.has_value()) ||
               !observed.error.empty();
      }) ||
      !client.send_control(Control{3, ControlKind::pause_capture}) ||
      !wait_for(observed, [&] {
        return observed.pause_cadence_acknowledged || !observed.error.empty();
      }) ||
      !client.send_control(Control{4, ControlKind::set_sections, true}) ||
      !client.send_control(Control{5, ControlKind::start_capture}) ||
      !wait_for(observed, [&] {
        return (observed.start_detailed_acknowledged &&
                observed.detailed_frame.has_value()) ||
               !observed.error.empty();
      }) ||
      !client.send_control(Control{6, ControlKind::pause_capture}) ||
      !wait_for(observed, [&] {
        return observed.pause_detailed_acknowledged || !observed.error.empty();
      })) {
  ...
  std::cout << "target=" << hello.platform << '/' << hello.abi
            ...
            << "cadence_frame=" << cadence.frame_number
            << " interval_ms=" << cadence.interval_ms
            << " active_ms=" << cadence.active_ms << '\n'
            << "detailed_frame=" << detailed.frame_number
            << " sections=" << detailed.sections.size()
            << " interval_ms=" << detailed.interval_ms
            << " active_ms=" << detailed.active_ms << '\n';
  return 0;
}
