...
namespace termin {
    namespace collision {
        ...
        public:
             ...
             */
            template <typename Callback> void query_aabb(const AABB& aabb, Callback&& callback) const {
             ...
             */
            template <typename Callback> void query_ray(const Ray3& ray, Callback&& callback) const {
                if (root_ == BVH_NULL_NODE)
                    return;
                ...
                while (!stack.empty()) {
                    ...
                    double t_min, t_max;
                    if (!ray_intersects_aabb(ray, inv_dir, dir, node.bounds, t_min, t_max))
                        continue;
             ...
             */
            template <typename Callback> void query_all_pairs(Callback&& callback) const {
                if (root_ == BVH_NULL_NODE)
                    return;
        ...
        private:
            ...
            void remove_leaf(int32_t leaf_index) {
                ...
                int32_t sibling = (nodes_[parent].left == leaf_index) ? nodes_[parent].right : nodes_[parent].left;
            ...
            bool contains(const AABB& outer, const AABB& inner) const {
                return outer.min_point.x <= inner.min_point.x && outer.min_point.y <= inner.min_point.y &&
                       outer.min_point.z <= inner.min_point.z && outer.max_point.x >= inner.max_point.x &&
                       outer.max_point.y >= inner.max_point.y && outer.max_point.z >= inner.max_point.z;
            }

            bool ray_intersects_aabb(const Ray3& ray,
                                     const Vec3& inv_dir,
                                     const Vec3& dir,
                                     const AABB& aabb,
                                     double& t_min,
            ...
            bool validate_structure(int32_t node_index) const {
                ...
                if (nodes_[node.left].parent != node_index)
                    return false;
                if (nodes_[node.right].parent != node_index)
                    return false;

                // Check bounds contain children
                AABB combined = nodes_[node.left].bounds.merge(nodes_[node.right].bounds);
                if (!contains(node.bounds, combined))
                    return false;
