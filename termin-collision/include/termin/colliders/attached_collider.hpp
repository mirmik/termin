...
namespace termin {
    namespace colliders {
        ...
        public:
            ...
            AttachedCollider(ColliderPrimitive* collider,
            ...
            ColliderPrimitive* collider() const {
                return collider_;
            }
            GeneralTransform3* transform() const {
                return transform_;
            }
            tc_entity_id owner_entity_id() const {
            ...
            GeneralPose3 world_transform() const {
                ...
                if (!affine.is_finite() || !std::isfinite(determinant) || std::abs(determinant) <= 1.0e-12) {
                    tc_log_error("[AttachedCollider] entity '%s' has a singular or non-finite "
                                 "world transform; lossy collider projection is undefined",
                                 transform_->name());
                    throw std::runtime_error("Attached collider cannot project a singular world transform");
                }
                if (determinant < 0.0) {
                    tc_log_error("[AttachedCollider] entity '%s' has a reflected world "
                                 "transform; lossy collider projection is forbidden",
                                 transform_->name());
                    throw std::runtime_error("Attached collider does not support reflected world transforms");
                }
                return transform_->lossy_global_pose().compose_trs_projected(collider_->transform);
            }
