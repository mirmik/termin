...
namespace termin::collision {
    ...
    std::array<CollisionWorld::ClipPlane, 4> CollisionWorld::build_clip_planes(const std::array<Vec3, 4>& face_verts,
    ...
    std::vector<Vec3> CollisionWorld::sutherland_hodgman_clip(const std::array<Vec3, 4>& subject,
    ...
    void
    CollisionWorld::generate_box_box_contacts(Collider* a, Collider* b, const ColliderHit& hit, ContactPatch& patch) {
        ...
        for (std::size_t clipped_index = 0; clipped_index < clipped.size(); ++clipped_index) {
            ...
            if (depth < 0) {
                ...
                candidate.features.feature_a = static_cast<uint32_t>(ref_face_idx);
                candidate.features.feature_b =
                    (static_cast<uint32_t>(inc_face_idx) << 16U) | static_cast<uint32_t>(clipped_index);
