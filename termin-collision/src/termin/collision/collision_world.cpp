...
namespace termin::collision
{
    ...
    {
        return reinterpret_cast<CollisionWorld*>(
            tc_collision_world_get_scene(scene));
    }

    void CollisionWorld::set_scene(tc_scene_handle scene)
    {
        scene_ = scene;
    }
    ...
    {
        return std::find(colliders_.begin(), colliders_.end(), collider) !=
               colliders_.end();
    }

    size_t CollisionWorld::size() const
    {
    ...
    BroadPhaseMode CollisionWorld::broad_phase_mode() const
    {
    ...
    void CollisionWorld::test_contact_pair(Collider* a,
                                           Collider* b,
                                           std::vector<ContactPatch>& patches)
    {
        ...
        patch.collider_a = a;
        patch.collider_b = b;
        patch.normal_world = hit.normal;
        if (a->type() == colliders::ColliderType::Box &&
            b->type() == colliders::ColliderType::Box)
        {
            generate_box_box_contacts(a, b, hit, patch);
        }
        else
        {
            ContactCandidate point;
            point.point_on_a_world = hit.point_on_a;
            point.point_on_b_world = hit.point_on_b;
    ...
    std::array<Vec3, 4>
    CollisionWorld::get_face_vertices(const Vec3& center,
    ...
    Vec3 CollisionWorld::get_face_normal(const std::array<Vec3, 3>& axes,
                                         int face_index)
    {
        switch (face_index)
        {
    ...
    int CollisionWorld::find_reference_face(const std::array<Vec3, 3>& axes,
    ...
    int CollisionWorld::find_incident_face(const std::array<Vec3, 3>& axes,
                                           const Vec3& direction)
    {
        int best_face = 0;
        double best_dot = std::numeric_limits<double>::infinity();
    ...
    CollisionWorld::build_clip_planes(const std::array<Vec3, 4>& face_verts,
    ...
    Vec3 CollisionWorld::intersect_edge_plane(const Vec3& a,
                                              const Vec3& b,
    ...
    std::vector<Vec3> CollisionWorld::sutherland_hodgman_clip(
    ...
    void CollisionWorld::generate_box_box_contacts(Collider* a,
                                                   Collider* b,
                                                   ...
                                                   ContactPatch& patch)
    {
        ...
        {
            box_a = dynamic_cast<const colliders::BoxCollider*>(
                attached_a->collider());
        ...
        {
            box_b = dynamic_cast<const colliders::BoxCollider*>(
                attached_b->collider());
        ...
        auto axes_a = world_box_a.get_axes_world(),
             axes_b = world_box_b.get_axes_world();
        Vec3 half_a = world_box_a.effective_half_size(),
             half_b = world_box_b.effective_half_size();
        ...
        auto ref_verts =
            get_face_vertices(center_a, axes_a, half_a, ref_face_idx);
        auto inc_verts =
            get_face_vertices(center_b, axes_b, half_b, inc_face_idx);
        ...
        for (const auto& p : clipped)
        {
            ...
            if (depth < 0)
            {
                ...
                candidate.point_on_b_world = p;
                candidate.point_on_a_world = p - hit.normal * depth;
                candidate.signed_gap = depth;
    ...
    std::vector<RayHit> CollisionWorld::raycast(const Ray3& ray,
                                                const RaycastQuery& query) const
    {
        std::vector<RayHit> hits;
        bvh_.query_ray(ray,
                       [&](Collider* collider, double, double)
                       {
                           if (!accepts_raycast_collider(collider, query))
                               return;
                           colliders::RayHit collider_hit =
                               collider->closest_to_ray(ray);
                           if (!collider_hit.hit())
                               return;
                           RayHit hit;
    ...
    RayHit CollisionWorld::raycast_closest(const Ray3& ray,
                                           const RaycastQuery& query) const
    {
    ...
    const BVH& CollisionWorld::bvh() const
    {
        return bvh_;
    }
    ...
    bool
    CollisionWorld::accepts_raycast_collider(Collider* collider,
                                             const RaycastQuery& query) const
    {
