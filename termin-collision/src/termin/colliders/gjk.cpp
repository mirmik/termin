...
namespace termin {
    namespace colliders {
        MinkowskiPoint
        minkowski_support(const ColliderPrimitive& a, const ColliderPrimitive& b, const Vec3& direction) {
        ...
        namespace detail {
            ...
            EpaFace make_epa_face_no_flip(const std::vector<MinkowskiPoint>& polytope, int a, int b, int c) {
                ...
                if (face.distance < 0)
                    face.distance = 0; // numerical guard
                return face;
            }
            bool build_epa_tetrahedron(const ColliderPrimitive& a,
        ...
        GjkResult gjk(const ColliderPrimitive& a, const ColliderPrimitive& b) {
            GjkResult result;
            ...
            for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {
                ...
                if (result.simplex_size == 2) {
                    ...
                    for (int i = 0; i < 3; ++i) {
                        if (weights[i] > 1e-10)
                            reduced[count++] = result.simplex[i];
                    }
                    if (count > 0 && count < 3) {
                        for (int i = 0; i < count; ++i)
                            result.simplex[i] = reduced[i];
                    ...
                    MinkowskiPoint face_pts[3] = {
                        result.simplex[best_idx[0]], result.simplex[best_idx[1]], result.simplex[best_idx[2]]};
                    double weights[3] = {best_bary.u, best_bary.v, best_bary.w};
                    int count = 0;
                    for (int i = 0; i < 3; ++i) {
                        if (weights[i] > 1e-10)
                            result.simplex[count++] = face_pts[i];
                    }
                    result.simplex_size = count;
                }
            }
            ...
            if (result.simplex_size == 1) {
                ...
                auto bary = detail::closest_on_triangle(
                    result.simplex[0].point, result.simplex[1].point, result.simplex[2].point);
                result.closest_on_a = result.simplex[0].support_a * bary.u + result.simplex[1].support_a * bary.v +
                                      result.simplex[2].support_a * bary.w;
                result.closest_on_b = result.simplex[0].support_b * bary.u + result.simplex[1].support_b * bary.v +
                                      result.simplex[2].support_b * bary.w;
                result.distance = bary.closest.norm();
            }

            return result;
        }
        EpaResult epa(const ColliderPrimitive& a, const ColliderPrimitive& b) {
            EpaResult result;
            constexpr int MAX_ITERATIONS = 64;
            ...
            for (int i = 0; i < 4; ++i) {
                faces.push_back(
                    detail::make_epa_face_no_flip(polytope, face_idx[i][0], face_idx[i][1], face_idx[i][2]));
            }
            ...
            for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {
                ...
                if (new_dist - min_dist < EPA_TOLERANCE) {
                    ...
                    if (std::abs(denom) > 1e-14) {
                        ...
                        double bu = 1.0 - bv - bw;
                        result.point_on_a =
                            polytope[f.a].support_a * bu + polytope[f.b].support_a * bv + polytope[f.c].support_a * bw;
                        result.point_on_b =
                            polytope[f.a].support_b * bu + polytope[f.b].support_b * bv + polytope[f.c].support_b * bw;
                    } else {
                ...
                for (int i = 0; i < (int)faces.size(); ++i) {
                    if (!visible[i])
                        continue;
                    int edges[3][2] = {{faces[i].a, faces[i].b}, {faces[i].b, faces[i].c}, {faces[i].c, faces[i].a}};
                    for (int e = 0; e < 3; ++e) {
                        ...
                        for (int j = 0; j < (int)faces.size(); ++j) {
                            if (j == i || visible[j])
                                continue;
                            if ((faces[j].a == eb && faces[j].b == ea) || (faces[j].b == eb && faces[j].c == ea) ||
        ...
        ColliderHit gjk_collide(const ColliderPrimitive& a, const ColliderPrimitive& b) {
            ColliderHit result;
