...
namespace
{
    ...
    std::vector<uint32_t>
    feature_ids(const std::vector<ContactCandidate>& candidates)
    {
    ...
    void check_vec_near(const Vec3& actual,
                        const Vec3& expected,
...
TEST_CASE("Contact patch reducer handles a near-degenerate patch")
{
    ...
    const auto forward =
        reduce_contact_candidates(points, Vec3::unit_z(), config);
    std::reverse(points.begin(), points.end());
    const auto reverse =
        reduce_contact_candidates(points, Vec3::unit_z(), config);
...
TEST_CASE("Contact patch reducer covers a planar patch")
{
    ...
    {
        corner_count +=
            std::find(ids.begin(), ids.end(), id) != ids.end() ? 1 : 0;
    }
...
{
    const Quat rotation =
        Quat::from_axis_angle(Vec3{1.0, 2.0, 3.0}.normalized(), 0.73);
    ...
    {
        point.point_on_a_world =
            rotation.rotate(point.point_on_a_world) + translation;
        point.point_on_b_world =
            rotation.rotate(point.point_on_b_world) + translation;
    }

    const auto reduced_original =
        reduce_contact_candidates(original, Vec3::unit_z());
    const auto reduced_transformed =
        reduce_contact_candidates(transformed, rotation.rotate(Vec3::unit_z()));
    ...
    for (std::size_t i = 0; i < reduced_original.size(); ++i)
    {
        check_vec_near(reduced_transformed[i].point_on_a_world,
                       rotation.rotate(reduced_original[i].point_on_a_world) +
                           translation);
        check_vec_near(reduced_transformed[i].point_on_b_world,
                       rotation.rotate(reduced_original[i].point_on_b_world) +
                           translation);
...
TEST_CASE("CollisionWorld box-box contact uses a reduced geometric patch")
{
    ...
    BoxCollider box_b(Vec3{2.0, 2.0, 1.0},
                      GeneralPose3(Quat::from_axis_angle(Vec3::unit_z(), 0.3),
                                   Vec3{0.2, 0.1, 1.5}));
    world.add(&box_a);
    world.add(&box_b);
    ...
    for (const ContactCandidate& point : patches[0].points)
    {
        const double geometric_gap =
            (point.point_on_b_world - point.point_on_a_world)
                .dot(patches[0].normal_world);
