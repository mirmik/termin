...
MultiviewResolvePass::MultiviewResolvePass(
    const std::string& input,
    const std::string& output)
...
std::set<const char*> MultiviewResolvePass::compute_reads() const {
    std::set<const char*> reads;
    if (!input_res.empty()) reads.insert(input_res.c_str());
    if (!output_res_target.empty()) reads.insert(output_res_target.c_str());
    return reads;
}
...
MultiviewResolvePass::get_inplace_aliases() const {
...
void MultiviewResolvePass::execute(ExecuteContext& ctx) {
    ...
    if (input == ctx.tex2_reads.end() || !input->second ||
        output == ctx.tex2_writes.end() || !output->second) {
    ...
    if (in_desc.array_layers != 2 || out_desc.array_layers != 2 ||
        in_desc.sample_count <= 1 || out_desc.sample_count != 1) {
        tc::Log::error(
            "[MultiviewResolvePass] expected 2-layer MSAA input and 2-layer single-sample output");
        return;
    }
...
void MultiviewResolvePass::register_type() {
    auto descriptor = FramePassTypeDescriptorBuilder::native<MultiviewResolvePass>(
        "MultiviewResolvePass", "termin-render-passes");
    auto& inspect = descriptor.inspect();
    _register_inspect_input_res(inspect);
    _register_inspect_output_res(inspect);
    _register_inspect_output_res_target(inspect);
