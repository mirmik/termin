...
bool quad_outside_frustum(
    const tc_render_item_world_quad_payload& quad,
    const Mat44f& model_view_projection)
{
...
bool compatible_batch(
    const tc_render_item& lhs,
    const tc_render_item& rhs)
{
    return lhs.payload.world_quad.texture_handle.index ==
               rhs.payload.world_quad.texture_handle.index &&
...
void World2DPass::execute(ExecuteContext& ctx) {
    ...
    if (!shader || !tc_shader_ensure_tgfx2(
            shader, &ctx.ctx2->device(), &vertex_shader, &fragment_shader)) {
    ...
    ctx.ctx2->set_viewport(
        0, 0, static_cast<int>(output_desc.width), static_cast<int>(output_desc.height));
    ctx.ctx2->set_depth_test(true);
    ctx.ctx2->set_depth_write(false);
    ctx.ctx2->set_blend(true);
    ctx.ctx2->set_blend_func(
        tgfx::BlendFactor::SrcAlpha,
    ...
    ctx.ctx2->bind_uniform_data(
        "world2d_frame", view_projection.data, sizeof(view_projection.data));
    ...
    while (begin < order.size()) {
        ...
        while (end < order.size() &&
               compatible_batch(
                   first,
                   *submissions[order[end].submission_index])) {
            ++end;
        }
        ...
        for (size_t index = begin; index < end; ++index) {
            append_quad_vertices(
                vertices,
                *submissions[order[index].submission_index]);
        }
        tgfx::TextureHandle texture =
            ctx.ctx2->device().ensure_tc_texture(first.payload.world_quad.texture);
        const tgfx::SamplerHandle sampler =
            first.payload.world_quad.sampling == TC_WORLD_QUAD_SAMPLING_NEAREST
                ? nearest_sampler_
                : linear_sampler_;
        if (!texture) {
            ...
            ctx.ctx2->draw_transient_arrays(
                vertices.data(),
...
void World2DPass::register_type() {
    auto descriptor = FramePassTypeDescriptorBuilder::native<World2DPass>(
        "World2DPass", "termin-render-passes");
