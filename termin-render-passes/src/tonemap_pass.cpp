...
namespace termin {
    ...
    constexpr const char* MULTIVIEW_TONEMAP_ENGINE_SHADER_UUID = "termin-engine-multiview-tonemap";
    ...
    static_assert(sizeof(TonemapParamsStd140) == 16, "TonemapParamsStd140 must be 16 bytes for std140 compliance");

    TonemapPass::TonemapPass(const std::string& input, const std::string& output, float exposure_val, int method_val)
        : input_res(input),
          output_res(output),
    ...
    void TonemapPass::execute(ExecuteContext& ctx) {
        ...
        if (out_it == ctx.tex2_writes.end() || !out_it->second) {
            tc::Log::error("[TonemapPass/tgfx2] Missing tgfx2 output texture handle for '%s'", output_res.c_str());
            return;
        }
        ...
        if (in_it == ctx.tex2_reads.end() || !in_it->second) {
            tc::Log::error("[TonemapPass/tgfx2] Missing tgfx2 input texture handle for '%s'", input_res.c_str());
            return;
        }
        ...
        if (tc_shader_handle_is_invalid(shader_handle_)) {
            shader_handle_ = tgfx::register_builtin_shader_from_catalog(
                multiview_mode_ ? MULTIVIEW_TONEMAP_ENGINE_SHADER_UUID : TONEMAP_ENGINE_SHADER_UUID);
            if (tc_shader_handle_is_invalid(shader_handle_))
                return;
        }
        if (!params_ubo_) {
        ...
        if (uploaded_exposure_ != exposure || uploaded_method_ != method) {
            ...
            device2_->upload_buffer(
                params_ubo_, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&params), sizeof(params)));
        ...
        if (multiview_mode_) {
            ...
            if (!ctx.ctx2->begin_multiview_pass(pass))
                return;
        } else {
    ...
    void TonemapPass::register_type() {
        auto descriptor = FramePassTypeDescriptorBuilder::native<TonemapPass>("TonemapPass", "termin-render-passes");
    ...
    MultiviewTonemapPass::MultiviewTonemapPass(const std::string& input,
