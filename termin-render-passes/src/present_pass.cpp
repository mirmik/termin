...
namespace termin {
    ...
    PresentToScreenPass::PresentToScreenPass(const std::string& input, const std::string& output)
        : input_res(input),
          output_res(output) {
    ...
    void PresentToScreenPass::execute(ExecuteContext& ctx) {
        ...
        if (in_it == ctx.tex2_reads.end() || !in_it->second) {
            tc::Log::warn("[PresentToScreenPass] missing tgfx2 input '%s'", input_res.c_str());
            return;
        }
        auto out_it = ctx.tex2_writes.find(output_res);
        if (out_it == ctx.tex2_writes.end() || !out_it->second) {
            tc::Log::warn("[PresentToScreenPass] missing tgfx2 output '%s'", output_res.c_str());
            return;
        }
    ...
    void PresentToScreenPass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<PresentToScreenPass>("PresentToScreenPass", "termin-render-passes");
    ...
    BlitPass::BlitPass(const std::string& input, const std::string& output, const std::string& pass_name)
        : input_res(input),
          output_res(output) {
    ...
    void BlitPass::register_type() {
        auto descriptor = FramePassTypeDescriptorBuilder::native<BlitPass>("BlitPass", "termin-render-passes");
        auto& inspect = descriptor.inspect();
        _register_inspect_input_res(inspect);
        _register_inspect_output_res(inspect);
