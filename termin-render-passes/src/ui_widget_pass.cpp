...
std::uint64_t stable_component_identity(const tc_component* component) {
    ...
    const std::string_view entity_uuid =
        tc_entity_handle_valid(owner)
            ? std::string_view(tc_entity_uuid(owner))
            : std::string_view{};
    const std::string_view source_id =
        std::string_view(tc_component_get_source_id(component));
    ...
    value = hash_bytes(
        value, &owner.pool.generation, sizeof(owner.pool.generation));
    value = hash_bytes(value, &owner.id.index, sizeof(owner.id.index));
    value = hash_bytes(
        value, &owner.id.generation, sizeof(owner.id.generation));
    const std::size_t component_index =
        tc_entity_handle_valid(owner)
            ? tc_entity_component_index(owner, component)
            : SIZE_MAX;
...
struct SubmissionCollector {
    ...
    void append(tc_component* component) {
        ...
        if (!tc_scene_ui_document_snapshot_get(component, &snapshot)) {
            tc::Log::error(
                "[UIWidgetPass] scene_ui_document capability failed to "
        ...
        if (!document.valid()) {
            tc::Log::error(
                "[UIWidgetPass] scene_ui_document capability returned a stale "
...
void collect_internal_hierarchy(
    ...
    const bool hierarchy_enabled =
        ancestors_enabled && tc_entity_enabled(entity);
    if (hierarchy_enabled) {
        const std::size_t component_count =
            tc_entity_component_count(entity);
        for (std::size_t index = 0; index < component_count; ++index) {
            tc_component* component =
                tc_entity_component_at(entity, index);
    ...
    for (std::size_t index = 0; index < child_count; ++index) {
        collect_internal_hierarchy(
            tc_entity_child_at(entity, index),
            hierarchy_enabled,
            collector);
    }
}
...
UIWidgetPass::get_inplace_aliases() const {
...
void UIWidgetPass::execute(ExecuteContext& ctx) {
    ...
    const int width = ctx.render_rect.width > 0
        ? ctx.render_rect.width
        : static_cast<int>(output_desc.width);
    const int height = ctx.render_rect.height > 0
        ? ctx.render_rect.height
    ...
    try {
        painter_->paint_documents(
            *ctx.ctx2, width, height, submissions);
    } catch (const std::exception& error) {
        ctx.ctx2->end_pass();
        tc::Log::error(
            "[UIWidgetPass] failed to paint %zu native document(s): %s",
            submissions.size(), error.what());
        return;
    } catch (...) {
        ctx.ctx2->end_pass();
        tc::Log::error(
            "[UIWidgetPass] failed to paint %zu native document(s) with "
...
void UIWidgetPass::destroy() {
    ...
    try {
        ...
        tc::Log::error(
            "[UIWidgetPass] GPU resource release failed: %s",
            error.what());
    } catch (...) {
        tc::Log::error(
            "[UIWidgetPass] GPU resource release failed with an unknown "
            "exception");
    }
}
