...
namespace termin {
    ...
    namespace {
        ...
        static_assert(sizeof(IdPerFrameStd140) == 128, "IdPerFrameStd140 must be 2 * mat4");

        struct IdModelStd140 {
            float u_model[16];
        };
        static_assert(sizeof(IdModelStd140) == 64, "IdModelStd140 must be mat4");

        struct IdPickStd140 {
            float u_pickColor[4]; // vec3 + pad
        };
        static_assert(sizeof(IdPickStd140) == 16, "IdPickStd140 must be vec4");
        ...
        VertexOutputAdapter id_vertex_output_adapter() {
            ...
            adapter.source_module = {"termin_id_vertex_output_adapter",
        ...
        MaterialPipelinePassContract id_material_pass_contract() {
            ...
            contract.static_vertex_transform = material_pipeline_make_static_mesh_vertex_transform_provider(
                "static_id", MeshVertexTransformProfile::Position, "id_model.model");
            contract.static_vertex_transform->resources.push_back(
                material_pipeline_draw_resource_decl("id_model", TC_SHADER_STAGE_VERTEX, 64u));
            contract.skinned_vertex_transform = material_pipeline_make_skinned_mesh_vertex_transform_provider(
                "skinned_id", MeshVertexTransformProfile::Position, "id_model.model");
            ...
            MaterialPipelineResourceDecl id_pick =
                material_pipeline_draw_resource_decl("id_pick", TC_SHADER_STAGE_FRAGMENT, 16u);
    ...
    void IdPass::execute_with_data_tgfx2(ExecuteContext& ctx,
                                         const Rect2i& rect,
        ...
        if (color_it == ctx.tex2_writes.end() || !color_it->second) {
            tc::Log::error("IdPass/tgfx2: missing tgfx2 color texture for '%s'", output_res.c_str());
            return;
        }
        ...
        if (!ensure_material_pipeline_shader(*ctx.ctx2, device, id_shader_handle_, "IdPass", id_shader)) {
            return;
        }
        ...
        auto draw_item = [&](const DrawCall& dc) {
            ...
            const bool submitted = submit_render_item_draw(*ctx.ctx2, item, encode_request);
            restore_id_raster_state();
            if (!submitted) {
                return;
            }
    ...
    void IdPass::execute(ExecuteContext& ctx) {
        const SceneRenderServices* services = require_scene_render_services(ctx, "IdPass");
        if (!services) {
            return;
        }
        ...
        execute_with_data_tgfx2(
            ctx, rect, scene, view, projection, camera->get_position(), camera_layer_mask, camera_render_category_mask);
    }

    void IdPass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<IdPass>("IdPass", "termin-render-passes", "GeometryPassBase");
