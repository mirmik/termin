...
root:
  ...
  background_color: [0.2, 0.3, 0.4, 1]
    ...
    termin::UIComponent* make_component(termin::Entity entity,
...
TEST_CASE("UIWidgetPass collects ordered enabled layer-filtered documents") {
    ...
    const TcUiDocumentAsset asset = TcUiDocumentAsset::declare_source(
        "ui-widget-pass-contract", "UIWidgetPass Contract", "UI/widget-pass.uiscript", kSource);
    REQUIRE(asset.valid());
    ...
    UIComponent* back = make_component(back_entity, asset, 5, "ui-back");
    REQUIRE(back->document().set_presentation_metrics(tc_ui_presentation_metrics{
        2.0f,
        1.25f,
    ...
    UIComponent* front = make_component(front_entity, asset, 20, "ui-front");

    Entity disabled_component_entity = scene.create_entity("DisabledComponent");
    UIComponent* disabled_component = make_component(disabled_component_entity, asset, 1, "ui-disabled-component");
    tc_component_set_enabled(disabled_component->tc_component_ptr(), false);
...
TEST_CASE("UIWidgetPass includes internal hierarchy without duplicates") {
    ...
    TcUiDocumentAsset::clear_registry_for_tests();
    const TcUiDocumentAsset asset = TcUiDocumentAsset::declare_source(
        "ui-widget-pass-internal", "UIWidgetPass Internal", "UI/widget-pass-internal.uiscript", kSource);
    REQUIRE(asset.valid());

    TcSceneRef scene = TcSceneRef::create("ui-widget-pass-internal");
    Entity scene_entity = scene.create_entity("Scene UI");
    UIComponent* scene_component = make_component(scene_entity, asset, 10, "scene-ui");

    Entity internal_root = Entity::create(Entity::standalone_pool_handle(), "Internal Root");
    Entity internal_child = internal_root.create_child("Internal Child");
    UIComponent* internal_component = make_component(internal_child, asset, 3, "internal-ui");
