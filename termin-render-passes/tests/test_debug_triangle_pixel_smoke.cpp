...
namespace {
    ...
    bool matches_clear_color(const float pixel[4]) {
        return pixel[0] > 0.05f && pixel[0] < 0.11f && pixel[1] > 0.06f && pixel[1] < 0.12f && pixel[2] > 0.08f &&
               pixel[2] < 0.14f && pixel[3] > 0.95f;
    }

    bool differs_from_clear_color(const float pixel[4]) {
        return pixel[0] > 0.13f || pixel[1] > 0.15f || pixel[2] > 0.18f;
    }
    ...
    int run_smoke(const char* argv0) {
        ...
        const bool pass_ok =
            read_ok && differs_from_clear_color(center) && matches_clear_color(corner) && pipeline_cache_size == 1;
