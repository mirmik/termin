...
FragmentOutput fs_main(FragmentInput input) {
    ...
    bool matches_clear_color(const float pixel[4]) {
        return pixel[0] >= 0.00f && pixel[0] < 0.08f && pixel[1] >= 0.01f && pixel[1] < 0.09f && pixel[2] >= 0.02f &&
               pixel[2] < 0.10f && pixel[3] > 0.95f;
    }

    bool matches_material_color(const float pixel[4]) {
        return pixel[0] > 0.08f && pixel[0] < 0.18f && pixel[1] > 0.66f && pixel[1] < 0.80f && pixel[2] > 0.18f &&
               pixel[2] < 0.32f && pixel[3] > 0.95f;
    }
    ...
    termin::TcMaterial create_color_material(const termin::TcShader& shader) {
        termin::TcMaterial material =
            termin::TcMaterial::create("ColorPassPixelSmokeMaterial", "termin-color-pass-pixel-smoke-material");
    ...
    int run_smoke(const char* argv0) {
        ...
        float corner[4] = {};
        const bool read_ok =
            device->read_pixel_rgba8(target, static_cast<int>(kWidth / 2), static_cast<int>(kHeight / 2), center) &&
