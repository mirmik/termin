...
struct ScopedArtifactConfiguration {
    ...
    ScopedArtifactConfiguration()
    {
        ...
        root = std::filesystem::temp_directory_path() /
            ("termin-standard-surface-forward-" + std::to_string(unique));
        ...
        termin::tgfx2_set_shader_compiler_path(
            TERMIN_STANDARD_SURFACE_FORWARD_TEST_SHADERC);
#endif
...
termin::TcShader make_standard_surface_producer()
{
    ...
    for (uint32_t index = 0; index < 6u; ++index) {
        std::snprintf(
            inputs[index].semantic,
            sizeof(inputs[index].semantic),
            "%s",
            semantics[index]);
...
TEST_CASE("standard forward consumer composes static skinned and foliage variants")
{
    ...
    const termin::MaterialPipelinePassContract pass =
        termin::color_material_pass_contract();
    REQUIRE(
    ...
    for (termin::VertexTransformKind transform : transforms) {
        ...
        CHECK(
            std::strstr(
                ...
                "evaluate_standard_surface") != nullptr);
        CHECK(
            std::strstr(
                variant.fragment_source(),
                "termin_standard_pbr_forward") != nullptr);
