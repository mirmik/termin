...
std::vector<std::filesystem::path> shaderc_candidates(const char* argv0) {
    ...
    if (argv0 && argv0[0] != '\0') {
        ...
        if (!ec && !exe_dir.empty()) candidates.push_back(exe_dir / "termin_shaderc");
    }
...
bool mip_count_preserves_energy(
    tgfx::IRenderDevice& device,
    tgfx::RenderContext2& render_ctx,
    tgfx::TextureHandle source
) {
    ...
    device.upload_texture(
        ...
        std::span<const uint8_t>(
            reinterpret_cast<const uint8_t*>(constant.data()),
    ...
    if (three_mips.empty() || six_mips.empty()) return false;
...
bool impulse_has_smooth_falloff(
    tgfx::IRenderDevice& device,
    tgfx::RenderContext2& render_ctx,
    tgfx::TextureHandle source
) {
