...
void tc_voxel_grid_init(void) {
    if (g_initialized)
        return;
    if (!tc_pool_init_rebootstrap(&g_voxel_grid_pool, sizeof(tc_voxel_grid), 32, &g_voxel_grid_generation_epoch)) {
...
tc_voxel_grid_handle tc_voxel_grid_declare(const char* uuid, const char* name) {
    ...
    if (!tc_voxel_grid_handle_is_invalid(existing))
        return existing;
...
tc_voxel_grid* tc_voxel_grid_get(tc_voxel_grid_handle h) {
    if (!g_initialized)
        return nullptr;
    return static_cast<tc_voxel_grid*>(tc_pool_get_checked(&g_voxel_grid_pool, h, "tc_voxel_grid"));
}
...
tc_voxel_grid_info* tc_voxel_grid_get_all_info(size_t* count) {
    ...
    *count = 0;
    ...
    auto* infos = static_cast<tc_voxel_grid_info*>(std::calloc(grid_count, sizeof(tc_voxel_grid_info)));
    if (!infos) {
    ...
    for (uint32_t index = 0; index < g_voxel_grid_pool.capacity && output_index < grid_count; ++index) {
        if (g_voxel_grid_pool.states[index] != TC_SLOT_OCCUPIED)
            continue;
        auto* grid = static_cast<tc_voxel_grid*>(tc_pool_get_unchecked(&g_voxel_grid_pool, index));
...
bool tc_voxel_grid_set_metadata(tc_voxel_grid* grid, const char* name, const char* source_path) {
    ...
    if (source_path && source_path[0] != '\0')
        grid->source_path = tc_intern_string(source_path);
    voxel_grid_bump_version(grid);
    return true;
}
