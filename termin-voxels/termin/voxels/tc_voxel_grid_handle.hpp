...
namespace termin {
    namespace voxels {
        ...
        public:
            ...
            explicit TcVoxelGrid(tc_voxel_grid_handle h)
                : handle(h) {
            ...
            TcVoxelGrid(const TcVoxelGrid& other)
                : handle(other.handle) {
                if (tc_voxel_grid* grid = tc_voxel_grid_get(handle)) {
                    tc_voxel_grid_add_ref(grid);
                }
            ...
            TcVoxelGrid(TcVoxelGrid&& other) noexcept
                : handle(other.handle) {
            ...
            tc_voxel_grid* get() const {
                return tc_voxel_grid_get(handle);
            }
            bool is_valid() const {
                return tc_voxel_grid_is_valid(handle);
            }
            bool is_loaded() const {
                return tc_voxel_grid_is_loaded(handle);
            }
            bool ensure_loaded() const {
                return tc_voxel_grid_ensure_loaded(handle);
            }
            ...
            void deserialize_from(const tc_value* data, void* = nullptr) {
                ...
                if (data->type == TC_VALUE_STRING && data->data.s && data->data.s[0]) {
                    ...
                    if (std::strcmp(grid_name, "(None)") == 0)
                        return;
