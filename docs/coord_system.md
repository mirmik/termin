...
ей; любой backend (OpenGL / Vulkan / Direct3D 11) **обязан** выдать этому API такое же
...
Текущая политика для scene и 2D pixel render paths описана в
...
Оставшиеся fullscreen/postprocess paths мигрируются отдельно и не должны
расширять CPU-side projection adaptation.
...
- **Direct3D 11** — native `D3D11_VIEWPORT.TopLeftY` уже top-left, поэтому
  `set_viewport` передаётся напрямую. Y-согласование для 3D/scene paths
  делается не viewport-ом, а shader-boundary conversion из §2.
...
- D3D11 не должен менять camera/view math. D3D11-specific flip применяется
  только в shader helper-е `termin_to_native_clip(...)` на границе final
  vertex output. CPU-side projection/view-projection adapters для 3D/scene
  paths считаются legacy и не должны возвращаться.
