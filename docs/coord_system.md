...
- **Direct3D 11** — native viewport mapping противоположен Vulkan по Y
  ...
  всё равно остаются Vulkan-native, а перед загрузкой в GPU uniforms/push data
...
- **Direct3D 11** — native `D3D11_VIEWPORT.TopLeftY` уже top-left, поэтому
  `set_viewport` передаётся напрямую. Y-согласование делается не viewport-ом,
  а projection adapter-ом из §2.
...
- **Direct3D 11** — получает projection уже адаптированным под native
  D3D11 viewport mapping. Этот adapter переворачивает Y на границе загрузки
  GPU uniforms, поэтому native winding, который видит rasterizer, получается
  обратным относительно API-level `FrontFace`. Поэтому `FrontFace::CW`
...
что backend-и применяют разные нативные Y conventions и разные места
для clip-space адаптации.
...
- D3D11 не должен менять camera/view math. D3D11-specific flip применяется
  только на границе загрузки projection в GPU state (`frame_uniforms`,
  render passes, `ImmediateRenderer`, `SolidPrimitiveRenderer`).
