...
1. #678 — сузить оставшийся C ABI surface и удалить window/OpenGL/presentation
   delegation, legacy SDL surface и presenter no-op calls.
2. #638 — завершить RAII/lifetime ownership offscreen surface и rename
   `FBOSurface`; device-domain validation уже входит в #678.
3. #679 — перевести C#/WPF scene hosting на обязательную D3D11 tgfx texture и
   display input endpoint без `BackendWindow` и OpenGL.
4. На каждом этапе проверить штатную SDK build/test gate; для #679 дополнительно
