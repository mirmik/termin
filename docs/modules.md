...
Отвечает за render framework поверх canonical resources и `termin-graphics`: render engine, frame graph, presenter/debugger, scene render mount data и render-state integration helpers. В переходной архитектуре здесь совместно живут scene-neutral execution contracts и `tc_scene` adapter. В #1359–#1360 neutral snapshot, view и execution context уже отделены от scene capabilities внутри модуля; следующий шаг #1358 — отдельный generic execution entry point, после чего граница может быть закреплена отдельным target.
