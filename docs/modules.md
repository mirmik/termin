...
Отвечает за render framework поверх canonical resources и `termin-graphics`: render engine, frame graph, presenter/debugger, scene render mount data и render-state integration helpers. В переходной архитектуре здесь совместно живут scene-neutral execution contracts и `tc_scene` adapter. В #1359–#1361 neutral snapshot, view, execution context и единый `RenderEngine::execute_pipeline()` уже отделены от scene capabilities внутри модуля. Scene path заранее публикует snapshots/services и вызывает общий executor; следующий шаг #1358 — явный item-source contract и последующее закрепление границы отдельным target.
