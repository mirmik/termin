...
| Архитектурные границы | [docs/modules.md](modules.md), [docs/library-dependencies.md](library-dependencies.md) | ownership модулей, допустимые зависимости, правила перемещения кода |
| Документация модуля | `<module>/docs/` | API, lifecycle, ownership, gotchas, примеры модуля |
| Планы миграций | [docs/plans/](plans/) | временные планы, решения, чеклисты до завершения миграции |
...
- Что не должно попадать в модуль.
...
Документ надо держать в корне `docs/`, если он описывает границу между несколькими модулями. Документ надо держать в `<module>/docs/`, если он описывает внутренний API, lifecycle или практику использования одного модуля.
