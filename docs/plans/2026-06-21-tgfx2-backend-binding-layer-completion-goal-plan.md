...
- Vulkan resource sets всё ещё строятся из `ResourceBinding::set/binding/kind`.
- `BoundResourceSetDesc` пока flat list, без backend-native scope groups.
...
Зачем: `BoundResourceSetDesc` is still flat. That is acceptable for the current
boundary, but not the end-state. Backend-и должны видеть frame/pass/material/draw
groups so they can bind native slots efficiently and without descriptor-set
shaped assumptions.
...
- Decide whether to extend `BoundResourceSetDesc` in-place or add
  `BoundResourceBundle` / `BackendResourceBindings`.
- Keep compatibility with current flat list during migration.
- Make `RenderContext2` preserve scope buckets until backend boundary.
- Backend execution:
  - OpenGL: bind groups by binding points / texture units.
  - D3D11: bind groups by stage/register class.
  - Vulkan: still update descriptor set for now, but grouping prepares multi-set
    descriptor layouts.

Acceptance:

- Dirty tracking can identify which scopes changed.
- No backend needs to infer scope from resource name.
- Flat list remains only as compatibility representation.
...
Optional manual SDK build after public API changes:
...
7. Decide whether to start scope grouping now or split into next goal.
8. Update live docs and final status.

Recommended stopping point for one goal: finish checkpoints 1-6. Scope grouping
is valuable but likely deserves its own goal if it starts touching renderer
ownership code.

## Completion Checklist

- [ ] D3D11 smoke validated on Windows.
- [ ] Vulkan stores `BoundResourceSetDesc`.
- [ ] Vulkan descriptor writes use `placement.vulkan`.
- [ ] Vulkan bound runtime test added.
- [ ] OpenGL bound runtime test remains passing.
- [ ] D3D11 bound runtime test remains passing.
- [ ] `RenderContext2` migrated path is bound-first.
- [ ] Binding plan validation covers wrong/missing placement.
- [ ] Legacy numeric API documented as compatibility path.
- [ ] Architecture docs updated.
- [ ] Kanboard #21 updated with final status.
