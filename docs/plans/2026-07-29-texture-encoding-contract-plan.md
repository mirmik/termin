...
(`#1049`) и shader property encoding contract (`#1050`) реализованы и
проверены под Linux. Карточки остаются в `On Test`, но не блокируют следующие
этапы.
...
`#1048`, `#1049`, `#1050` и `#1051` реализованы и находятся в `On Test`; их
blocker-связи с последующими этапами сняты. `#1052` и `#1053` разблокированы
и находятся в `Ready`; `#1054` также разблокирована и переведена в `Ready`.
`#1055` сохраняет типизированные зависимости от `#1052`, `#1053` и `#1054`.
