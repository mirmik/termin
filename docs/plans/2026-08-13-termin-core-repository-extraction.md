...
Implementation status (2026-08-13): Linux `graphics` and `full`, Android and
Web SDK composition are implemented and verified against standalone
`termin-core` artifacts. Desktop consumers use `termin-core/sdk`; Android and
Web use native-only platform trees whose manifests additionally bind target
system, architecture, API/toolchain version and artifact hashes. Every consumer
requires an absolute SDK path; its exact native build identity is read from the
verified artifact manifest. An explicit ID remains an optional expectation
check:

```bash
./build-sdk.sh --profile graphics \
...
The output starts as a staged copy of the immutable Core SDK. Domain install
then rejects every file collision, combines Core and domain wheels without
rebuilding Core distributions, and records input ownership in
`sdk-inputs.json`. Core-owned native artifacts in the composed manifest resolve
back to the input manifest, never to same-named leftovers in the consumer build
tree.

The repository-owned source-hiding gate is:

```bash
scripts/smoke-installed-core-source-hidden \
  --profile graphics \
  --core-sdk /absolute/path/to/termin-core-sdk
```

It builds in a temporary hard-linked checkout after physically removing the
old Core native/MCP source directories. Aurora CI additionally bootstraps one
installed Core SDK per platform from pinned Core revision
`aca907c6f766d54ab4722af6b14db40792e4b2d3`, propagates its exact
`native_build_id` to every native consumer, and runs dedicated source-hidden
graphics builds on Linux and Windows plus Android and Web cross-builds. This
source bootstrap is temporary:
publication and download of versioned Core release artifacts remains required
before Stage 5 is closed.
...
to installed package discovery in every consumer build path.
...
Implementation status (2026-08-13): complete. The last duplicated revision is
available in Termin git history. Core sources, its product profile, runtime lock
and Core-owned tests were removed from Termin; normal `full` and `graphics`
builds now require an installed Core artifact. The source-hidden CI jobs remain
as assertions that the removed directories do not return.
...
### Coordinated Core API changes

During active development, a change spanning Core and Termin is delivered in
this order:

1. Change and verify the API in `termin-core`, including installed consumers.
2. Publish a Core artifact and record its exact `native_build_id`.
3. Update Termin against that artifact; do not expose the Core source checkout
   through `CMAKE_PREFIX_PATH`, `PYTHONPATH` or a sibling-directory fallback.
4. Land the Termin consumer change only after its source-absent gates pass.
5. Advance the pinned Core revision/artifact used by CI in the same Termin
   change, then retire the previous pin after the compatibility window.

An incompatible transition may temporarily publish old and new Core artifacts,
but it must not restore duplicated sources or make the consumer guess which
Core checkout to use.
...
cutover, rollback means pinning the previous verified Core artifact and
reverting the corresponding Termin consumer commit, not copying sources back
into the consumer repository. If a bad Core artifact has already been
published, leave its build identity immutable and publish a corrected artifact
