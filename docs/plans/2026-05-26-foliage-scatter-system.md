...
Обновление 2026-07-07: описанный ниже direct `draw_tgfx2()` путь был
заменен в живом коде на `FOLIAGE_BATCH` RenderItem + registered encoder.
Исторические упоминания `draw_tgfx2()` в этом документе оставлены как контекст
первой реализации, а не как текущая целевая архитектура.
