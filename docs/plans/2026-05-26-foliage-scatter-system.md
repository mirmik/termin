...
MVP может не поддерживать тени от травы. В текущем `ShadowPass` drawable без `tc_mesh*` пропускаются. Позже надо добавить shadow direct draw hook или отдельный foliage shadow path.
