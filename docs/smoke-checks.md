...
| Graphics Python publication preflight | `task publish:graphics:python` | Performs a network-free, exact-manifest check of the complete 0.5.0 candidate: one distribution, two ABI wheels, manylinux tags, public dependency/license metadata and SHA-256. Add `-- --check` for Twine metadata checks or `-- --remote-status` for read-only filename/digest reconciliation with PyPI. Actual upload additionally requires `--upload --confirm-version 0.5.0` and safely resumes a matching partial release with 429 backoff. |
