...
| Standalone Graphics Python product (initial Linux contract) | `task package:graphics:python` | Independently rebuilds the graphics graph with `termin-window`, the GUI window adapter and pinned bundled SDL2, emits CPython 3.14t wheels under `dist/graphics-python`, centralizes native libraries plus precompiled Vulkan/OpenGL shaders and fonts, and proves both offscreen and SDL-offscreen windowed showcases work with an empty `PATH` and no packaged Slang/`termin_shaderc`, without consuming `sdk-graphics`. |
