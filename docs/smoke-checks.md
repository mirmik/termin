...
| Installed graphics-profile showcase | `task build -- --profile graphics --no-sdl && scripts/smoke-graphics-showcase` | The installed editor-free SDK imports its complete unconditional Python closure in isolated bundled Python, contains no `termin.display`/engine runtime, and renders native UI, visual-scene/nodegraph, Plot2D, Plot3D and their composition to a non-empty PNG. On Windows use `task build -- --profile graphics --no-sdl --no-vulkan --no-opengl` and the documented headless showcase command for the D3D11 variant. |
