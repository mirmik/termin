...
- Python build слои: `termin.project_builder`, `termin.project_build`;
...
центральный артефакт `runtime package`, и desktop build уже частично переведен
на переносимый bundle. Но миграция не завершена. Сейчас одновременно существуют
старый broad-copy build, новый runtime package build, два runtime loader-а и
несколько platform wrapper-ов с разными правилами доставки assets.

Главное направление cleanup: сделать `termin.project_build` канонической build
оркестрацией, формализовать `runtime package` как версионированный контракт,
а `termin.project_builder` либо переименовать/ограничить как legacy dev export,
либо превратить в compatibility shim поверх нового pipeline.

## Текущая карта pipeline

### Build Profile CLI

`termin_builder` читает `project_settings/build_profiles.json`, но фактически
поддерживает только `target: "desktop"` и запускает:

```text
python3 -m termin.project_builder.profile_build desktop ...
```

Код: `termin-app/cpp/app/termin_builder.cpp`.

При этом `profile_build.py` уже не вызывает старый `build_project`, а делегирует
в новый `termin.project_build.build_desktop_project(...)`. Это переходный слой:
имя модуля старое, backend новый.

### Старый Project Build

`termin.project_builder.build_project(...)`:

- сканирует проект широким обходом;
- копирует ресурсы в `assets/`;
- пишет `build.json` и `assets/manifest.json`;
- может компилировать shader usages в `.build/shaders/source` и `assets/shaders/...`.

Этот путь все еще используется editor action `Build` / `Run Build` из
`ProjectBuildController`. Он полезен для dev/play-mode, но не должен оставаться
основным packaged build contract.
...
### 2. `project_builder` и `project_build` смешаны и вводят в заблуждение

Новый backend живет в `termin.project_build`, но entrypoint для профилей называется
`termin.project_builder.profile_build`. Editor generic build до сих пор вызывает
`termin.project_builder.build_project(...)`, тогда как Android/Quest используют
`termin.project_build`.

Риск: любое изменение "build system" легко попасть не в тот слой. Тесты тоже
разделены: `test_project_builder.py` проверяет старый broad-copy contract, а
`test_runtime_package_exporter.py` проверяет новый runtime package/bundle.

Направление:

- переименовать старый путь в `termin.project_builder.legacy_project_export`
  или `termin.project_build.dev_project_export`;
- перенести `profile_build.py` в `termin.project_build`;
- оставить `termin.project_builder` как compatibility re-export только на время
  миграции;
- editor action `Build` должен явно выбирать: `Build Dev Copy` или
...
- старый `build.json/assets` путь еще поддерживается `termin_runner` как legacy.
...
- удалить legacy `build.json` launcher path после прохождения relocated smoke.
...
### Этап 2. Развести legacy/dev build и packaged build

1. Перенести `profile_build.py` из `termin.project_builder` в `termin.project_build`.
2. Сделать `termin.project_build` единственной публичной точкой packaged builds.
3. Переименовать старый broad-copy exporter и editor action так, чтобы было
   очевидно: это dev artifact, не standalone build.
4. В `termin_runner` оставить legacy path только под явным flag/mode.
