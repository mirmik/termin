...
Старый переходный entrypoint `termin.project_builder.profile_build` удален:
profile dispatch теперь живет только в `termin.project_build.profile_build`.
...
Packaged build backend живет в `termin.project_build`. Editor generic build до
сих пор вызывает `termin.project_builder.build_project(...)`, тогда как
profile/Android/Quest используют `termin.project_build`.
...
- держать profile/backend код в `termin.project_build`;
- не возвращать `termin.project_builder.profile_build` compatibility shim;
