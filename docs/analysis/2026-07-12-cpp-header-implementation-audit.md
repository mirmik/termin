...
- `termin-openxr/src/openxr_android_scene_runtime.hpp` — единственный
  ...
  `src/openxr_android_runtime.cpp`; implementation header удалён.
...
- `termin-engine/include/termin/render/viewport_render_state.hpp` — RAII/move и
  allocation/release texture ресурсов перенесены в `src/viewport_render_state.cpp`;
  public type получил export visibility и focused lifetime tests.
- `termin-graphics/include/tgfx2/backend_binding_plan.hpp` — нетемплейтная
  backend placement policy вынесена в установленную shared library
  `termin_graphics_binding_policy`, общую для `termin_graphics2` и
  `termin_shaderc`.
- `termin-gui-native/include/termin/gui_native/document.hpp` и
...
- `termin-lighting/include/termin/lighting/light.hpp` — runtime sampling и
  ...
  shared library.
- `termin-render/include/termin/render/render_task.hpp` — allocative
  `RenderTask::set_resources` находится в `src/render_task.cpp`.
- `termin-gui-native/python/bindings/gui_native_bindings_shared.hpp` — shared
  header сокращён с 1064 до 314 строк declarations, wrapper layouts и
  действительно header-visible templates. RAII wrappers, Python/C value
  conversion, document registry, Python widget/surface callbacks, factory
  serialization, `Document` и snapshot helpers перенесены в один
  `gui_native_bindings_runtime.cpp`. `gui_native_module.cpp` использует отдельный
  узкий module header и больше не разбирает весь binding runtime.

## Оставшиеся кандидаты

В зафиксированных очередях аудита незавершённых кандидатов нет. Новый scope
следует добавлять только по конкретному измеримому compile-time/coupling
случаю, а не продолжать механически удалять любой небольшой inline body.
