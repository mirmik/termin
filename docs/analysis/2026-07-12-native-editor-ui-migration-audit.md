...
| Console | Готово | Отдельные накопительный editor/runtime log и Python Console | Editor Console забирает bounded `tc_log` queue через pull/drain; native, Python logging и editor/build сообщения проходят через общий C logging path. |
...
| Dialogs | В основном готово | Settings, Project Settings, About, scene properties, layers/flags, shadows, navigation settings, SpaceMouse, Scene Manager, Undo History, Audio, Quest/OpenXR; startup module load снова показывает non-dismissible native progress dialog с live build log; Framegraph Debugger восстановлен как отдельное OS-окно через общий native multi-window host | Layers & Flags заменил 64 индексированных поля двумя multiline areas; функционально возможно, но хуже видна связь строки с индексом. Build Profiles всё ещё не портирован, но это общий незавершённый feature, а не потеря уже доступного пункта основного старого меню. |
