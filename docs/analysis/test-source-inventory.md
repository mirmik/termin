...
| needs-work | cpp | `termin-graphics/tests/test_per_pipeline_layout.cpp` | Useful Vulkan descriptor-layout smoke, but it is not registered in current CMake/CTest and overlaps later Vulkan smoke coverage; either wire it intentionally or delete/consolidate it. |
...
| needs-work | cpp | `termin-graphics/tests/test_tgfx2_sdf.cpp` | Exercises bitmap/SDF atlas paths but mostly prints metrics and glyph data; should assert texture handles, positive measurements, and expected glyph presence instead of being manual smoke. Verified current behavior by graphics CTest batch. |
...
| needs-work | python | `termin-gui/python/tests/test_hstack_layout.py` | Removed duplicated full-height child check and unused import; symmetric layout cases could still be table-driven. |
...
| needs-work | python | `termin-gui/python/tests/test_splitter.py` | Removed private initial-state check; drag and constraints remain useful but could be table-driven. |
...
| needs-work | python | `termin-gui/python/tests/test_vstack_layout.py` | Removed duplicated full-width child check and unused import; symmetric layout cases could still be table-driven. |
...
| needs-work | cpp | `termin-inspect/tests/test_python_inspect.cpp` | Replaced release-disabled `assert` checks with runtime failures, but `termin_inspect_python_test` is not built in current `build/Release-tests` (`cmake --build ... --target termin_inspect_python_test` has no rule); wire it into the active test config or remove it. |
