...
| --- | --- | --- | --- |
| pruned | c | `termin-app/core_c/tests/test_main.c` | Stale legacy runner path; no current repository source exists at this location. Current C coverage is tracked by module-owned C tests such as `termin-inspect/tests/test_main.c`. |
...
| reviewed | cpp | `termin-app/cpp/tests/tests_shader_parser_std140.cpp` | Large but cohesive shader/std140 regression coverage; helper duplication is acceptable while the file protects parser, resource-layout, rewrite, and contract behavior. Verified through `ctest --test-dir build/Release-tests -R '^termin_tests$' --output-on-failure`. |
...
| reviewed | python | `termin-default-assets/tests/asset_plugin_test.py` | Default asset/plugin/resource cases overlap intentionally with the canonical registry matrix while exercising runtime preload/register behavior through `DefaultResourceManager`; no extra consolidation needed for #43. |
...
| reviewed | python | `termin-base/tests/python/pose_test.py` | Strengthened `looking_at` to assert forward/up/right axes directly; remaining Pose3 tests use concrete transforms, inverses, matrices, and helper contracts. Verified by `./run-tests-python.sh termin-base/tests/python/pose_test.py termin-base/tests/python/test_general_pose3.py termin-app/tests/test_shader_tool_resolution.py termin-qopt/tests/active_set_test.py`. |
...
| reviewed | python | `termin-navmesh/tests/test_edge_flipping.py` | Useful triangulation/refinement coverage remains; deeper replacement of count-only checks with geometric invariants is tracked separately in #165 rather than blocking #43. |
| reviewed | python | `termin-app/tests/test_editor_mcp_server.py` | Useful MCP server coverage for tool listing, Python execution, framegraph capture/inspect, and settings path behavior; repeated loop/session helpers are acceptable test-local scaffolding. |
...
| migrated | c++ | `termin-app/cpp/tests/tests_rendering_manager.cpp` | The deleted Python debugger-model tests were replaced by native target polling, pipeline replacement, suspension, and exact generation-bearing target identity coverage. |
| migrated | c++ | `termin-render` frame-local capture integration | The deleted Python internal-point tests described an injected-pass implementation. Internal capture now travels through the current `ExecuteContext`; resource capture runs after the last scheduled writer without mutating the pipeline. |
| pruned | python | `termin-render/tests/test_framegraph_presenter_bindings.py` | Deleted brittle source-scanning test; framegraph/debugger behavior is covered by render/app tests and literal C++ string matching would block harmless refactors. |
| reviewed | python | `termin-navmesh/tests/test_funnel_algorithm.py` | Funnel coverage remains useful; deeper no-cut/corridor geometry oracle work is tracked separately in #165 rather than blocking #43. |
...
| reviewed | python | `termin-base/tests/python/test_general_pose3.py` | Copy, matrix roundtrip, and Pose3 conversion checks now cover fuller translation/rotation/scale behavior; direction/vector scale contract is covered too. Verified by `./run-tests-python.sh termin-base/tests/python/pose_test.py termin-base/tests/python/test_general_pose3.py termin-app/tests/test_shader_tool_resolution.py termin-qopt/tests/active_set_test.py`. |
| reviewed | python | `termin-components/termin-components-kinematic/tests/test_general_transform3.py` | Direction helper coverage now separates scale-aware `transform_vector` from scale-free `transform_direction`; forward/right/up assert orientation axes and distance scaling independent of transform scale. Verified by `./run-tests-python.sh termin-components/termin-components-kinematic/tests` after rebuilding SDK and refreshing `.venv`. |
...
| reviewed | python | `termin-app/tests/test_shader_tool_resolution.py` | Replaced dynamic source loading with package import of `termin.editor_tcgui.shader_runtime`; remaining Linux skip is explicit because the test targets Windows `LOCALAPPDATA` behavior. Verified by `./run-tests-python.sh termin-base/tests/python/pose_test.py termin-base/tests/python/test_general_pose3.py termin-app/tests/test_shader_tool_resolution.py termin-qopt/tests/active_set_test.py`. |
...
| reviewed | python | `termin-voxels/tests/test_voxel_shader.py` | Added timeout to subprocess import-order smoke; remaining checks cover shader catalog/material layout contract. |
...
| pruned | python | `termin-app/tests/voxels_test.py` | Stale inventory entry for a moved/deleted file; current voxel tests live under `termin-voxels/tests`. |
...
| reviewed | python | `termin-csg/tests/test_csg_python.py` | Broad CAD/CSG regression file intentionally covers document, edit, controller, and standalone CadApp integration boundaries; size/duplication alone is not a #43 blocker. Verified by targeted CSG Python test run. |
...
| reviewed | python | `termin-graphics/tests/python/test_termin_shaderc_cli.py` | Large but valuable shaderc CLI regression coverage with concrete sidecar/resource-layout assertions; fake slangc/reflection builders are appropriate test fixtures for CLI behavior. Verified by targeted shaderc CLI test run. |
...
| reviewed | cpp | `termin-graphics/tests/test_tgfx2_sdf.cpp` | Test assertions are now concrete enough; remaining issue is environment availability for GL-backed runtime verification, tracked separately in #166. |
...
| reviewed | python | `termin-gui/python/tests/test_clip_stack.py` | Keeps production `UIRenderer` clip-stack checks plus focused nested mock renderer regressions; mock scaffolding is narrow and acceptable. Verified by GUI layout/clip test run. |
| reviewed | python | `termin-gui/python/tests/test_combined_layout.py` | Editor-like combined layout coverage is useful as integration guard even where it overlaps hstack/vstack unit tests. Verified by GUI layout/clip test run. |
| reviewed | python | `termin-gui/python/tests/test_editor_style_layout.py` | Large class groups cover editor-style layout regressions with concrete rect/viewport invariants; further parameterization is optional maintainability work. Verified by GUI layout/clip test run. |
...
| reviewed | python | `termin-pga/tests/algeom_test.py` | Generated ellipsoid/hyperboloid/paraboloid fixtures have deterministic inputs and concrete numeric assertions; helperization would be cosmetic. Verified by targeted PGA test run. |
...
| reviewed | python | `termin-qopt/tests/active_set_test.py` | Replaced exact iteration-count assertions with bounded convergence checks while preserving KKT, solution, multiplier, and active-set behavior. Verified by `./run-tests-python.sh termin-base/tests/python/pose_test.py termin-base/tests/python/test_general_pose3.py termin-app/tests/test_shader_tool_resolution.py termin-qopt/tests/active_set_test.py`. |
| reviewed | python | `termin-qopt/tests/fem/doll2d_test.py` | Small construction/API/kinematics cases are concrete and cheap; consolidation is not required for #43. Verified by qopt/FEM Python test run. |
...
| reviewed | python | `termin-qopt/tests/fem/fem_test.py` | Assembler, conditioning, and variable-solution blocks provide useful FEM coverage; repeated setup is acceptable test scaffolding. Verified by qopt/FEM Python test run. |
...
| reviewed | python | `termin-qopt/tests/fem/mbody2d_test.py` | Rigid-body one-step cases are concrete and cheap; optional parameterization is not a #43 blocker. Verified by qopt/FEM Python test run. |
| reviewed | python | `termin-qopt/tests/fem/mbody3d_test.py` | Large integration scenarios are still meaningful mbody3d coverage; splitting would be maintainability cleanup, not required for #43. Verified by qopt/FEM Python test run. |
| reviewed | python | `termin-qopt/tests/fem/mechanic_test.py` | Analytical FEM triangle shear/body-force checks are useful and concrete enough; consolidation is optional. Verified by qopt/FEM Python test run. |
