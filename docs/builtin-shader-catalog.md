...
- SDK builds may opt into offline WebGPU artifacts with
  `TERMIN_BUILD_BUILTIN_SHADER_ARTIFACTS=ON` and
  `TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS=webgpu`. This requires the pinned
  Slang and Naga executables at configure/build time and installs the validated
  WGSL plus layout sidecars into `share/termin/shaders/webgpu/`.
...
shaders/webgpu/<uuid>.vert.wgsl
shaders/webgpu/<uuid>.frag.wgsl
...
WebGPU artifacts use sidecar contract version 3. Every reflected resource keeps
