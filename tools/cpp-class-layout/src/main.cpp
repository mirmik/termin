...
namespace {
    ...
    llvm::cl::list<std::string> extra_excludes("exclude-path",
    ...
    bool is_path_prefix(std::string_view prefix, std::string_view value) {
        ...
        return value.size() > prefix.size() && value.starts_with(prefix) && value[prefix.size()] == '/';
    }
    ...
    public:
        ...
        std::optional<std::string> owned_relative_path(llvm::StringRef filename, bool apply_path_filters = true) const {
            if (filename.empty()) {
                return std::nullopt;
            }
            ...
            if (apply_path_filters && !filters_.empty()) {
                bool matches = std::ranges::any_of(
                    filters_, [&](const std::string& filter) { return is_path_prefix(filter, relative); });
                if (!matches) {
                    return std::nullopt;
                }
            }
        ...
        void sort() {
            std::ranges::sort(violations_, [](const Violation& left, const Violation& right) {
                return std::tie(left.file, left.class_point.line, left.class_point.column, left.class_name) <
                       std::tie(right.file, right.class_point.line, right.class_point.column, right.class_name);
            });
        }
    ...
    SourcePoint source_point(const clang::SourceManager& source_manager, clang::SourceLocation location) {
    ...
    std::string declaration_name(const clang::NamedDecl& declaration, std::string_view fallback) {
    ...
    const clang::CXXMethodDecl* method_info(const clang::Decl& declaration) {
        ...
        if (const auto* function_template = llvm::dyn_cast<clang::FunctionTemplateDecl>(&declaration)) {
            return llvm::dyn_cast<clang::CXXMethodDecl>(function_template->getTemplatedDecl());
        }
    ...
    public:
        ...
        bool VisitCXXRecordDecl(clang::CXXRecordDecl* record) {
            ...
            clang::SourceLocation record_location = source_manager_.getExpansionLoc(record->getBeginLoc());
            auto relative_file = report_.owned_relative_path(source_manager_.getFilename(record_location));
            ...
            std::string definition_key = *relative_file + ":" + std::to_string(class_point.line) + ":" +
            ...
            for (const clang::Decl* member : record->decls()) {
                if (member == nullptr || member->isImplicit() || member->getLexicalDeclContext() != record) {
                    continue;
                }
    ...
    public:
        explicit ClassLayoutAction(Report& report)
            : report_(report) {}
    ...
    public:
        explicit ClassLayoutActionFactory(Report& report)
            : report_(report) {}
    ...
    void print_text_report(const std::vector<Violation>& violations) {
        for (const Violation& violation : violations) {
            ...
            llvm::outs() << violation.file << ':' << first_field.point.line << ':' << first_field.point.column
                         << ": error: class '" << violation.class_name
                         << "' declares data members after member function '" << violation.first_method
    ...
    int analyze_sources(const clang::tooling::CompilationDatabase& database,
        ...
        for (size_t remaining_workers = worker_count; remaining_workers > 0; --remaining_workers) {
            workers.emplace_back([&] {
                while (true) {
                    ...
                    clang::tooling::ClangTool tool(database, {source_files[source_index]});
...
int main(int argc, const char** argv) {
    ...
    llvm::cl::ParseCommandLineOptions(
        argc, argv, "Check that C++ data members are declared before member functions.\n");
    ...
    std::unique_ptr<clang::tooling::CompilationDatabase> database =
        clang::tooling::CompilationDatabase::autoDetectFromDirectory(build_path, database_error);
    if (database == nullptr) {
        llvm::errs() << "error: failed to load compilation database from '" << build_path << "': " << database_error
                     << '\n';
        return 2;
    }
    ...
    llvm::errs() << "termin-cpp-class-layout: analyzing " << source_files.size() << " C++ translation unit(s) with "
    ...
    llvm::errs() << "termin-cpp-class-layout: " << report.violations().size() << " class violation(s)\n";
