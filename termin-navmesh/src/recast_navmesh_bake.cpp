...
namespace termin {
    ...
    RecastBuildResult build_recast_navmesh(const float* verts,
                                           int nverts,
                                           const int* tris,
                                           int ntris,
        ...
        tc_log_info("[NavMesh] Config: cs=%.3f ch=%.3f grid=%dx%d", cfg.cs, cfg.ch, cfg.width, cfg.height);
        ...
        std::vector<unsigned char> tri_areas(ntris, navmesh_detour_area_to_recast_area(build_config.default_area_id));
        ...
        for (int i = 0; i < ntris; ++i) {
            ...
            const int area_id =
                triangle_area_ids ? static_cast<int>(triangle_area_ids[i]) : build_config.default_area_id;
        ...
        tc_log_info("[NavMesh] PolyMesh built: %d verts, %d polys (nvp=%d)", pmesh->nverts, pmesh->npolys, pmesh->nvp);

        if (pmesh->nverts <= 0 || pmesh->npolys <= 0) {
            result.error = "Recast produced an empty polygon mesh; check agent radius, cell size, region area, and "
                           "source mesh scale";
    ...
    RecastBuildResult build_recast_navmesh(const float* verts,
                                           ...
                                           const RecastNavMeshBuildConfig& build_config,
                                           const RecastNavMeshBuildDebugHooks* debug_hooks) {
        return build_recast_navmesh(verts, nverts, tris, ntris, nullptr, build_config, debug_hooks);
    }
