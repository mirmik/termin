...
namespace termin {
    ...
    namespace {
        ...
        void append_bake_geometry(const NavMeshBakeInput& input,
    ...
    RecastBuildResult RecastNavMeshBuilderComponent::build_from_entity_geometry() {
        ...
        tc_log_info(
            "[NavMesh] Base world matrix col0: (%.2f, %.2f, %.2f, %.2f)", b_data[0], b_data[1], b_data[2], b_data[3]);
        tc_log_info(
            "[NavMesh] Base world matrix col1: (%.2f, %.2f, %.2f, %.2f)", b_data[4], b_data[5], b_data[6], b_data[7]);
        tc_log_info(
            "[NavMesh] Base world matrix col2: (%.2f, %.2f, %.2f, %.2f)", b_data[8], b_data[9], b_data[10], b_data[11]);
        ...
        RecastBuildResult result = build_internal(
            verts.data(), nverts, tris.data(), ntris, area_ids.empty() ? nullptr : area_ids.data(), false);
