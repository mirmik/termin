...
namespace termin {
    ...
    namespace {
        ...
        tc_shader_contract_vertex_input contract_vertex_input(const char* semantic, uint32_t type) {
    ...
    tc_material_phase* add_builtin_slang_debug_phase(TcMaterial& material,
        ...
        const std::string vertex_source = tgfx::load_builtin_shader_stage_source_from_catalog(shader_uuid, "vertex");
    ...
    std::string base64_encode(const unsigned char* data, int size) {
        ...
        for (int i = 0; i < size; i += 3) {
            ...
            if (i + 2 < size)
                v |= static_cast<unsigned int>(data[i + 2]);
    ...
    std::string stable_uuid(const std::string& seed) {
        ...
        std::ostringstream out;
        out << std::hex << std::setfill('0') << std::setw(8) << static_cast<uint32_t>(a >> 32) << "-" << std::setw(4)
            << static_cast<uint16_t>(a >> 16) << "-" << std::setw(4) << static_cast<uint16_t>(a) << "-" << std::setw(4)
            << static_cast<uint16_t>(b >> 48) << "-" << std::setw(12) << (b & 0x0000ffffffffffffull);
        return out.str();
    }
    ...
    std::filesystem::path resolve_navmesh_output_path(const Entity& entity, const std::string& agent_type_name) {
        ...
        return scene_file_path.parent_path() / "navmeshes" / sanitize_filename(scene_stem) / file_name;
    }
        ...
        while (!ec && it != end) {
            ...
            if (it->is_regular_file(ec) && path.extension() == ".meta") {
                try {
                    ...
                    if (meta.is_dict() && meta.contains("uuid") && meta["uuid"].is_string() &&
                        meta["uuid"].as_string() == uuid) {
                    ...
                    tc_log_warn(
                        "[NavMeshKeeperComponent] failed to read meta '%s': %s", path.string().c_str(), e.what());
                }
            }
            it.increment(ec);
        }
    ...
    TcMaterial get_or_create_navmesh_debug_material(TcMaterial& material) {
        ...
        tc_material_phase* phase = add_builtin_slang_debug_phase(material,
    ...
    TcMesh build_detour_debug_mesh(const std::vector<std::vector<unsigned char>>& blobs) {
        ...
        tc_mesh_compute_uuid(
            vertices.data(), vertices.size() * sizeof(NavMeshDebugVertex), indices.data(), indices.size(), uuid);
        ...
        tc_log_info(
            "[NavMeshKeeperComponent] Detour debug mesh: %zu verts, %zu tris", vertices.size(), indices.size() / 3);
        return TcMesh(h);
    }
    ...
    bool load_detour_tile_blobs(const std::filesystem::path& asset_path,
        ...
        if (!data.is_dict() || !data.contains("format") || !data["format"].is_string() ||
            data["format"].as_string() != "termin.detour_navmesh" || !data.contains("tiles") ||
            !data["tiles"].is_list()) {
            return false;
        }

        for (const nos::trent& tile_data : data["tiles"].as_list()) {
            if (!tile_data.is_dict() || !tile_data.contains("data_encoding") ||
                !tile_data["data_encoding"].is_string() || tile_data["data_encoding"].as_string() != "base64" ||
                !tile_data.contains("data") || !tile_data["data"].is_string()) {
                continue;
            }
    ...
    bool load_detour_tile_blobs_from_navmesh(const TcNavMesh& navmesh, std::vector<std::vector<unsigned char>>& blobs) {
        blobs.clear();
        if (!navmesh.is_valid()) {
            return false;
        }
