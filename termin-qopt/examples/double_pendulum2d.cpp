...
int main() {
  ...
  const auto upper = system.add_body(
      {1.0, 0.1, {}},
      {{0.0, {std::sin(0.35), -std::cos(0.35)}}, {}, 0.0},
      "upper"
  );
  const auto lower = system.add_body(
      {1.0, 0.1, {}},
      {
          {
              0.0,
  ...
  if (!system.add_fixed_point_joint(
          upper.handle,
          {-std::sin(0.35), std::cos(0.35)},
          {},
          "world"
      ).ok()
  ...
  const double drift =
      std::abs(system.total_energy(gravity) - initial_energy)
      / std::max(1.0, std::abs(initial_energy));
  std::printf(
      "constraint=%g velocity=%g relative_energy_drift=%g\n",
