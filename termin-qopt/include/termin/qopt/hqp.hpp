...
namespace termin::qopt {
    ...
    [[nodiscard]] TERMIN_QOPT_API const char* hqp_diagnostic_name(HqpDiagnostic diagnostic) noexcept;
    ...
    struct HqpLevelHandle {
        ...
        [[nodiscard]] constexpr bool valid() const noexcept {
            return solver_id != 0 && index != std::numeric_limits<std::size_t>::max();
        }
    };
    ...
    struct HqpSolveResult {
        ...
        double priority_violation_linf = std::numeric_limits<double>::infinity();
    };
    ...
    public:
        ...
        [[nodiscard]] HqpLevelRegistrationResult add_level(int priority) noexcept;
        [[nodiscard]] HqpDiagnostic add_task(HqpLevelHandle level, QuadraticTaskView task) noexcept;
        [[nodiscard]] HqpDiagnostic add_equality(HqpLevelHandle level, EqualityConstraintView constraint) noexcept;
        [[nodiscard]] HqpDiagnostic add_inequality(HqpLevelHandle level, InequalityConstraintView constraint) noexcept;

        [[nodiscard]] HqpSolveResult solve(HqpSolutionView solution,
