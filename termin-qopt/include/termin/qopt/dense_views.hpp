...
struct ConstDenseVectorView {
    ...
    constexpr ConstDenseVectorView(
        const double* data_value,
        std::size_t size_value,
        DenseIndex stride_value = 1
    ) noexcept
        : data(data_value), size(size_value), stride(stride_value) {}

    constexpr ConstDenseVectorView(DenseVectorView view) noexcept
        : data(view.data), size(view.size), stride(view.stride) {}
...
struct DenseMatrixView {
    ...
    [[nodiscard]] static constexpr DenseMatrixView row_major(
        double* data,
        std::size_t rows,
        std::size_t columns
    ) noexcept {
    ...
    [[nodiscard]] static constexpr DenseMatrixView column_major(
        double* data,
        std::size_t rows,
        std::size_t columns
    ) noexcept {
    ...
    [[nodiscard]] constexpr double& operator()(
        ...
        return data[
            static_cast<DenseIndex>(row) * row_stride
...
struct ConstDenseMatrixView {
    ...
    [[nodiscard]] static constexpr ConstDenseMatrixView row_major(
        const double* data,
        std::size_t rows,
        std::size_t columns
    ) noexcept {
    ...
    [[nodiscard]] static constexpr ConstDenseMatrixView column_major(
        const double* data,
        std::size_t rows,
        std::size_t columns
    ) noexcept {
    ...
    [[nodiscard]] constexpr const double& operator()(
        std::size_t row,
        std::size_t column
    ) const noexcept {
        return data[
