...
assembly_diagnostic_name(AssemblyDiagnostic diagnostic) noexcept;
...
struct DenseBlockHandle {
  ...
  [[nodiscard]] constexpr bool valid() const noexcept {
    return topology_id != 0
        && index != std::numeric_limits<std::size_t>::max();
  }

  friend constexpr bool operator==(
      DenseBlockHandle, DenseBlockHandle
  ) noexcept = default;
};
...
public:
  ...
  [[nodiscard]] DenseBlockRegistrationResult register_block(
...
public:
  ...
  [[nodiscard]] DenseMatrixBlockResult block(
      DenseBlockHandle row, DenseBlockHandle column
  ) const noexcept;
  [[nodiscard]] AssemblyDiagnostic add(
