...
namespace
{
    ...
    void test_body_static_assembly()
    {
        ...
        TERMIN_QOPT_CHECK(contacts.set_contacts({ground_contact(17, body, 0.25)}) ==
        ...
        TERMIN_QOPT_CHECK(contacts.register_unilateral_constraints(
                              unilateral_topology, 0.1) == AssemblyDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(
            contacts.assemble(assembly, DynamicsAssemblyPhase::PositionProjection) ==
            AssemblyDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(assembly.clear() == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(
            contacts.assemble(assembly, DynamicsAssemblyPhase::VelocityProjection) ==
            AssemblyDiagnostic::None);
    ...
    void test_two_dynamic_endpoints_accumulate()
    {
        ...
        DynamicsUnilateralTopology unilateral_topology;
        TERMIN_QOPT_CHECK(contacts.register_unilateral_constraints(
                              unilateral_topology, 0.01) == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(unilateral_topology.finalize() == AssemblyDiagnostic::None);
    ...
    void test_split_position_correction()
    {
        ...
        auto* body = add(
            ...
            std::make_unique<RigidBody3DContribution>(
                unit_inertia(),
                RigidBody3DState{Pose3::translation(0.0, -0.1, 0.0), Screw3::zero()}));
        auto contacts = std::make_unique<ContactSet3DContribution>("ground");
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(1, *body, -0.1)}) ==
                          Contact3DDiagnostic::None);
    ...
    void test_impact_and_removal()
    {
        ...
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(5, *body, 0.0)}) ==
                          Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
    ...
    void test_resting_support_and_separation()
    {
        ...
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(9, *body, 0.0)}) ==
                          Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
    ...
    void test_articulation_endpoint()
    {
        ...
        Articulation3DContribution articulation(std::move(links), {{0.2}, {-0.3}});
        DynamicsTopology topology;
    ...
    void test_articulation_contact_rows()
    {
        ...
        Articulation3DContribution articulation(std::move(links), {{0.0}, {0.0}});
    ...
    void test_invalid_contact_step_is_transactional()
    {
        DynamicsSystem system;
        auto* body =
            add(system, std::make_unique<RigidBody3DContribution>(unit_inertia()));
