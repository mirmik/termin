...
int main()
{
    ...
    TERMIN_QOPT_CHECK(anchored.max_velocity_constraint_error() <
                      anchored_oracle.constraint_linf);
    TERMIN_QOPT_CHECK(std::abs(quaternion_norm(pendulum->state().pose.ang) - 1.0) <
    ...
    auto* hinge_body = add(
        hinge,
        std::make_unique<RigidBody3DContribution>(inertia(hinge_oracle.body.inertia),
    ...
    const auto first_hinge =
        hinge.step(options(hinge_oracle.time_step, hinge_oracle.constraint_linf));
