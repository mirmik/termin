...
[[nodiscard]] ConstDenseMatrixView view(
    const OracleDenseMatrix& matrix
) {
  return ConstDenseMatrixView::row_major(
      matrix.values.data(), matrix.rows, matrix.columns
  );
}

[[nodiscard]] ConstDenseVectorView view(
    const std::vector<double>& vector
) {
  return {vector.data(), vector.size(), 1};
}

[[nodiscard]] double linf_difference(
...
int main() {
  for (const OracleNullspaceCase& oracle : nullspace_oracle_cases()) {
    for (const NullspaceMethod method : {
      ...
      TERMIN_QOPT_CHECK(
          result.orthogonality_linf <= oracle.orthogonality_linf
      );
      for (std::size_t column = result.nullity;
           column < oracle.matrix.columns;
           ++column) {
        for (std::size_t row = 0; row < oracle.matrix.columns; ++row) {
          TERMIN_QOPT_CHECK(
              basis[row * oracle.matrix.columns + column] == 0.0
          );
        }
      }
    }
  ...
  const NullspaceResult projector_result = write_nullspace_projector(
      ConstDenseMatrixView::row_major(projector_matrix.data(), 2, 3),
  ...
  for (const OracleHqpCase& oracle : hqp_oracle_cases()) {
    ...
    for (const OracleHqpLevel& encoded_level : oracle.levels) {
      const HqpLevelRegistrationResult level =
          solver.add_level(encoded_level.priority);
      ...
      for (const OracleHqpConstraint& equality :
           encoded_level.equalities) {
        TERMIN_QOPT_CHECK(
            solver.add_equality(
                level.handle,
                {view(equality.matrix), view(equality.target)}
            ) == HqpDiagnostic::None
        );
      }
      for (const OracleHqpConstraint& inequality :
           encoded_level.inequalities) {
        TERMIN_QOPT_CHECK(
            solver.add_inequality(
                level.handle,
                {view(inequality.matrix), view(inequality.target)}
            ) == HqpDiagnostic::None
        );
      }
    }
    ...
    TERMIN_QOPT_CHECK(
        result.priority_violation_linf <= oracle.level_residual_tolerance
    );
    TERMIN_QOPT_CHECK(
        linf_difference(primal, oracle.expected_primal)
        <= oracle.primal_linf
    );
    TERMIN_QOPT_CHECK(
        linf_difference(residuals, oracle.expected_level_residuals)
  ...
  TERMIN_QOPT_CHECK(
      overconstrained.add_level(1).diagnostic
      == HqpDiagnostic::DuplicatePriority
  );
  const double one[] = {1.0};
  const double zero[] = {0.0};
  TERMIN_QOPT_CHECK(
      overconstrained.add_task(
          high.handle,
          {
  ...
  TERMIN_QOPT_CHECK(
      semidefinite_weight.add_task(
          weighted_high.handle,
          {
