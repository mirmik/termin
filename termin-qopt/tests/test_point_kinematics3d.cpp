...
namespace
{
    ...
    void test_rigid_body_point()
    {
        ...
        {
            const Vec3 positive = (state.pose * se3_exp(basis[column] * epsilon))
                                      .transform_point(point_local);
            const Vec3 negative = (state.pose * se3_exp(basis[column] * -epsilon))
                                      .transform_point(point_local);
            const Vec3 derivative = (positive - negative) / (2.0 * epsilon);
            check_near(derivative,
    ...
    void test_articulation_point()
    {
        ...
        Articulation3DContribution articulation(
            branching_links(), state, Vec3::zero(), "branching-point-model");
        bind(articulation);
        ...
        TERMIN_QOPT_CHECK(result.value.dofs.valid());
        check_near(
            result.value.position_world,
            articulation.link_poses_world()[link_index].transform_point(point_local));
        check_near(multiply(result.value.linear_jacobian_world(), state.velocities),
                   result.value.velocity_world);
    ...
    void test_diagnostics()
    {
        ...
        Articulation3DContribution articulation(branching_links(),
