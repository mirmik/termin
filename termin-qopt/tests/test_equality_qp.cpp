...
void test_shared_oracle() {
  for (const OracleEqualityQpCase &test_case : equality_qp_oracle_cases()) {
    ...
    const EqualityQpProblemView problem{
        ...
        row_major(test_case.equalities, test_case.constraints,
                  test_case.variables),
        const_vector(test_case.equality_targets),
    };
    const QpSolveResult result =
        solve_equality_qp(problem, {vector(primal), vector(dual)});
    ...
    if (result.status == QpStatus::Optimal) {
      ...
      TERMIN_QOPT_CHECK(std::ranges::all_of(
          primal, [](double value) { return value == 123.0; }));
      TERMIN_QOPT_CHECK(std::ranges::all_of(
...
void test_invalid_inputs_are_diagnostic_and_do_not_write_outputs() {
  ...
  TERMIN_QOPT_CHECK(
      std::ranges::all_of(primal, [](double value) { return value == 123.0; }));
}
