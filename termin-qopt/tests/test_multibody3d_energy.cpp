...
namespace
{
    ...
    EnergyEnvelope simulate_single_pendulum(double dt, double duration)
    {
        ...
        auto* body = add(system,
                         std::make_unique<RigidBody3DContribution>(
                             ...
                             "pendulum"));
        add(system,
            std::make_unique<FixedRevoluteJoint3DContribution>(*body,
                                                               Vec3{-0.5, 0.0, 0.0},
                                                               Vec3::unit_y(),
                                                               Vec3::zero(),
                                                               Vec3::unit_y(),
    ...
    FreeBodyInvariantError simulate_free_body(double dt, double duration)
    {
        ...
        for (std::size_t step = 0; step < steps; ++step)
        {
            ...
            result.maximum_angular_momentum_error =
                std::max(result.maximum_angular_momentum_error,
                           ...
                           double fine_bound)
    {
        const bool converges =
            std::isfinite(coarse.maximum_relative_drift) &&
...
int main()
{
    ...
    check_convergence(
        "single pendulum", coarse_dt, single_coarse, fine_dt, single_fine, 1.1e-4);
    ...
    check_convergence(
        "double pendulum", coarse_dt, double_coarse, fine_dt, double_fine, 3.5e-3);
