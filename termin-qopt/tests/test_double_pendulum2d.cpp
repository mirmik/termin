...
bool finite_state(RigidBody2DState body, double limit) {
  return std::isfinite(body.pose.lin.x)
      && std::isfinite(body.pose.lin.y)
      && std::isfinite(body.pose.ang)
      && std::isfinite(body.linear_velocity_world.x)
      && std::isfinite(body.linear_velocity_world.y)
      && std::isfinite(body.angular_velocity)
      && std::abs(body.pose.lin.x) < limit
      && std::abs(body.pose.lin.y) < limit
      && std::abs(body.pose.ang) < limit;
}
...
int main() {
  ...
  TERMIN_QOPT_CHECK(
      system.add_fixed_point_joint(
          first.handle,
          vec(oracle.body_a_fixed_anchor),
          vec(oracle.world_anchor),
          "world_anchor"
      ).ok()
  );
  TERMIN_QOPT_CHECK(
      system.add_revolute_joint(
          first.handle,
  ...
  for (std::size_t step = 0; step <= oracle.steps; ++step) {
    if (sample_index < std::size(oracle.sample_steps)
        && step == oracle.sample_steps[sample_index]) {
      max_constraint = std::max(
          max_constraint, system.max_position_constraint_error()
      );
      TERMIN_QOPT_CHECK(
          finite_state(system.body_state(first.handle), oracle.finite_state_limit)
      );
      TERMIN_QOPT_CHECK(
          finite_state(
              system.body_state(second.handle), oracle.finite_state_limit
          )
      );
      ++sample_index;
    }
    ...
    const Multibody2DStepResult result = system.step(
        gravity,
        {
  ...
  TERMIN_QOPT_CHECK(
      system.max_velocity_constraint_error() <= oracle.constraint_linf
  );
  TERMIN_QOPT_CHECK(
      relative_energy_drift <= oracle.relative_energy_drift
  );
  return 0;
}
