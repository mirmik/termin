...
[[nodiscard]] bool finite(RigidBody2DState state) noexcept {
  return finite(state.pose.ang)
      && finite(state.pose.lin)
...
[[nodiscard]] double cross(
    termin::Vec2 first, termin::Vec2 second
) noexcept {
...
[[nodiscard]] Multibody2DStepResult failure(
    QpStatus status,
...
struct Multibody2DSystem::Impl {
  ...
  [[nodiscard]] std::array<double, 9> body_mass(
      const Body& body
  ) const noexcept {
    const termin::Vec2 center =
        body.state.pose.rotate_vector(body.inertia.center_of_mass_local);
    ...
    return {
        ...
        mass_value * tangent.x,
        mass_value * tangent.y,
        body.inertia.moment_at_center
            + mass_value * center.dot(center),
    };
  }

  [[nodiscard]] std::array<double, 3> body_load(
      const Body& body, termin::Vec2 gravity
  ) const noexcept {
    const termin::Vec2 center =
        body.state.pose.rotate_vector(body.inertia.center_of_mass_local);
  ...
  [[nodiscard]] std::array<double, 6> point_jacobian(
      const Body& body, termin::Vec2 anchor_local
  ) const noexcept {
  ...
  [[nodiscard]] AssemblyDiagnostic assemble(
      DynamicsAssembly& assembly, termin::Vec2 gravity
  ) noexcept {
    ...
    for (const Joint& joint : joints) {
      if (const auto* fixed = std::get_if<FixedPointJoint>(&joint.model)) {
        ...
        const termin::Vec2 radius_a = body_a.state.pose.rotate_vector(
            revolute.body_a_anchor_local
        );
        const termin::Vec2 radius_b = body_b.state.pose.rotate_vector(
  ...
  [[nodiscard]] termin::Vec2 position_error(
      const Joint& joint
  ) const noexcept {
    if (const auto* fixed = std::get_if<FixedPointJoint>(&joint.model)) {
      const Body& body = bodies[fixed->body];
      return body.state.pose.transform_point(fixed->body_anchor_local)
          - fixed->world_anchor;
    }
    const auto& revolute = std::get<RevoluteJoint>(joint.model);
  ...
  [[nodiscard]] termin::Vec2 velocity_error(
      const Joint& joint
  ) const noexcept {
    ...
    const auto& revolute = std::get<RevoluteJoint>(joint.model);
    return point_velocity(
               bodies[revolute.body_a], revolute.body_a_anchor_local
           )
        - point_velocity(
            bodies[revolute.body_b], revolute.body_b_anchor_local
        );
  }
  ...
  void apply_generalized_delta(
      ConstDenseVectorView delta, bool velocity
  ) noexcept {
...
std::string_view multibody2d_diagnostic_name(
    Multibody2DDiagnostic diagnostic
) noexcept {
  switch (diagnostic) {
...
Multibody2DSystem& Multibody2DSystem::operator=(
    Multibody2DSystem&&
) noexcept = default;

Multibody2DRegistrationResult<RigidBody2DHandle>
Multibody2DSystem::add_body(
    SpatialInertia2D inertia,
  ...
  try {
    ...
    std::fprintf(
        stderr, "[termin-qopt] add 2D body failed with an unknown exception\n"
    );
  }
  return {{}, Multibody2DDiagnostic::InternalFailure};
}
...
Multibody2DSystem::add_fixed_point_joint(
  ...
  try {
    ...
    std::fprintf(
        stderr, "[termin-qopt] add fixed 2D joint failed: %s\n", error.what()
    );
  } catch (...) {
    std::fprintf(
        stderr,
        "[termin-qopt] add fixed 2D joint failed with an unknown exception\n"
    );
  }
  return {{}, Multibody2DDiagnostic::InternalFailure};
}
...
Multibody2DSystem::add_revolute_joint(
  ...
  try {
    ...
    std::fprintf(
        stderr,
        "[termin-qopt] add revolute 2D joint failed with an unknown exception\n"
    );
  }
  return {{}, Multibody2DDiagnostic::InternalFailure};
}
...
Multibody2DDiagnostic Multibody2DSystem::finalize() noexcept {
  ...
  try {
    ...
    std::fprintf(
        stderr, "[termin-qopt] finalize 2D model failed: %s\n", error.what()
    );
  } catch (...) {
    std::fprintf(
        stderr,
        "[termin-qopt] finalize 2D model failed with an unknown exception\n"
    );
  }
  return Multibody2DDiagnostic::InternalFailure;
}
...
Multibody2DDiagnostic Multibody2DSystem::set_body_state(
...
Multibody2DDiagnostic Multibody2DSystem::set_body_wrench(
...
RigidBody2DState Multibody2DSystem::body_state(
    RigidBody2DHandle body
) const noexcept {
  if (impl_ == nullptr || !impl_->valid(body)) {
    return {};
...
RigidBody2DAcceleration Multibody2DSystem::body_acceleration(
    RigidBody2DHandle body
) const noexcept {
  if (impl_ == nullptr || !impl_->valid(body)) {
    return {};
...
termin::Vec2 Multibody2DSystem::joint_reaction(
    Joint2DHandle joint
) const noexcept {
...
Multibody2DStepResult Multibody2DSystem::step(
    termin::Vec2 gravity_world, Multibody2DStepOptions options
) noexcept {
  if (impl_ == nullptr || !impl_->finalized) {
    return failure(
        QpStatus::InvalidInput, Multibody2DDiagnostic::ModelNotFinalized
    );
  }
  if (!finite(gravity_world)) {
    return failure(
        QpStatus::InvalidInput, Multibody2DDiagnostic::NonFiniteInput
    );
  }
  if (!finite(options.time_step) || options.time_step <= 0.0) {
    return failure(
        QpStatus::InvalidInput, Multibody2DDiagnostic::InvalidTimeStep
    );
  }
  ...
  try {
    ...
    if (!assembly.valid()
      ...
      return failure(
          QpStatus::InvalidInput, Multibody2DDiagnostic::AssemblyFailure
      );
    }
    ...
    restore();
    std::fprintf(
        stderr, "[termin-qopt] 2D multibody step failed: %s\n", error.what()
    );
  } catch (...) {
    restore();
    std::fprintf(
        stderr,
        "[termin-qopt] 2D multibody step failed with an unknown exception\n"
    );
  }
  return failure(
      QpStatus::NumericalFailure, Multibody2DDiagnostic::InternalFailure
  );
}
...
double Multibody2DSystem::total_energy(
  ...
  for (const Impl::Body& body : impl_->bodies) {
    ...
    energy += 0.5 * body.inertia.mass
        * center_velocity.dot(center_velocity);
