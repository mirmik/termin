...
namespace termin::qopt
{
    namespace
    {
        ...
        {
            return view.empty() || (view.data != nullptr && view.row_stride > 0 &&
                                         ...
                                         ContactFrictionOptions options) noexcept
    {
        ...
        if (!valid(problem.mass) || !valid(problem.bilateral_jacobian) ||
            ...
            !valid(problem.tangent_jacobian) || !valid(problem.normal_impulse) ||
            !valid(problem.friction_coefficient) || !valid(solution.velocity) ||
            !valid(solution.tangent_impulse) || !valid(solution.normal_impulse) ||
            !valid(solution.friction_work) || !valid(solution.bilateral_impulse))
        {
        ...
        try
        {
            ...
            for (std::size_t row = 0; row < variables; ++row)
            {
                ...
                {
                    const double symmetric = 0.5 * (hessian[row * variables + column] +
            ...
            for (std::size_t row = 0; row < normal_constraints; ++row)
            {
                ...
                limits[row] = normal_velocity - problem.minimum_normal_velocity[row];
                for (std::size_t variable = 0; variable < variables; ++variable)
                {
                    for (std::size_t dof = 0; dof < dofs; ++dof)
                    {
            ...
            for (std::size_t contact = 0; contact < contacts; ++contact)
            {
                ...
                if (problem.normal_impulse[contact] > options.qp.active_tolerance)
                {
                    for (std::size_t variable = 0; variable < variables; ++variable)
                    {
                        for (std::size_t dof = 0; dof < dofs; ++dof)
                        {
                            inequalities[active_normal_row * variables + variable] +=
                                problem.contact_normal_jacobian(contact, dof) *
                                response[dof * variables + variable];
                        }
                    }
                ...
                for (std::size_t facet = 0; facet < options.cone_facets; ++facet)
                {
                    const std::size_t row = normal_constraints + contacts * 2 +
                                            contact * options.cone_facets + facet;
                    const double angle = (static_cast<double>(facet) + 0.5) * 2.0 *
                                         std::numbers::pi /
                                         static_cast<double>(options.cone_facets);
            ...
            for (std::size_t dof = 0; dof < dofs; ++dof)
            {
                ...
                for (std::size_t variable = 0; variable < variables; ++variable)
                {
                    output_velocity[dof] += response[dof * variables + variable] *
            ...
            for (std::size_t contact = 0; contact < contacts; ++contact)
            {
                for (std::size_t axis = 0; axis < 2; ++axis)
                {
                    ...
                    contact_work[contact] +=
                        0.5 *
                        (gradient[contact * 3 + axis + 1] + final_tangent_velocity) *
            ...
            if (!solution.bilateral_impulse.empty())
            {
                for (std::size_t row = 0; row < bilateral; ++row)
                {
                    ...
                    for (std::size_t variable = 0; variable < variables; ++variable)
                    {
                        impulse += bilateral_response[row * variables + variable] *
                                   variable_impulse[variable];
                    }
