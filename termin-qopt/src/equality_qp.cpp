...
[[nodiscard]] bool same_tolerance(QpTolerance left,
                                  QpTolerance right) noexcept {
  return left.absolute == right.absolute && left.relative == right.relative &&
         left.symmetry == right.symmetry;
}

[[nodiscard]] bool same_matrix(const Matrix &left,
...
[[nodiscard]] bool compatible(const RankRevealingFactorization &factor,
  ...
  return factor.valid && same_tolerance(factor.tolerance, tolerance) &&
         same_matrix(factor.input_hessian, hessian) &&
         same_matrix(factor.input_equalities, equalities);
}
...
[[nodiscard]] QpSolveResult solve_impl(EqualityQpProblemView problem,
  ...
  if (compatible(factor, input_hessian, input_equalities, tolerance)) {
    ...
    if (constraint_count == 0) {
      ...
      Eigen::JacobiSVD<Matrix> svd(
          factor.equalities, Eigen::ComputeFullU | Eigen::ComputeFullV);
      if (svd.info() != Eigen::Success) {
        std::fprintf(stderr,
                     "[termin-qopt] equality QP constraint SVD failed: "
                     "variables=%zu constraints=%zu scale=%g\n",
                     variables, constraints, matrix_linf(factor.equalities));
      ...
      const double rank_tolerance =
          scaled_tolerance(tolerance, singular_scale);
      ...
      const Eigen::Index rank =
          static_cast<Eigen::Index>(factor.constraint_rank);
  ...
  const Eigen::Index rank =
      static_cast<Eigen::Index>(factor.constraint_rank);
  if (rank > 0) {
    feasible = factor.right_range *
               factor.inverse_singular_values.asDiagonal() *
  ...
  if (!factor.reduced_valid) {
    ...
    if (reduced_variables > 0) {
      const Matrix raw_reduced_hessian =
          factor.nullspace.transpose() * factor.hessian * factor.nullspace;
      ...
      if (eigensolver.info() != Eigen::Success) {
        std::fprintf(stderr,
                     ...
                     variables, constraints, factor.constraint_rank,
      ...
      factor.curvature_tolerance =
          scaled_tolerance(tolerance, linf(factor.eigenvalues));
  ...
  if (reduced_variables > 0) {
    const Vector reduced_gradient =
        factor.nullspace.transpose() *
        (factor.hessian * feasible + gradient);
    const Vector spectral_gradient =
        factor.eigenvectors.transpose() * reduced_gradient;
  ...
  if (factor.constraint_rank > 0) {
    equality_dual =
        -factor.left_range * factor.inverse_singular_values.asDiagonal() *
...
EqualityQpFactorizationCache::EqualityQpFactorizationCache(
    EqualityQpFactorizationCache &&) noexcept = default;

EqualityQpFactorizationCache &EqualityQpFactorizationCache::operator=(
    EqualityQpFactorizationCache &&) noexcept = default;

QpSolveResult EqualityQpFactorizationCache::solve(
  ...
  try {
    ...
    return solve_impl(problem, solution, tolerance, false, &impl_->factor,
                      &impl_->counters);
...
EqualityQpFactorizationCounters
EqualityQpFactorizationCache::counters() const noexcept {
  return impl_ == nullptr ? EqualityQpFactorizationCounters{}
                          : impl_->counters;
}
