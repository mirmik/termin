...
namespace termin::qopt::detail {
    ...
    [[nodiscard]] QpSolveResult solve_equality_qp_spd_first(EqualityQpProblemView problem,
                                                            EqualityQpSolutionView solution,
                                                            QpTolerance tolerance) noexcept;

    [[nodiscard]] inline QpSolveResult
    failure(QpStatus status, QpDiagnostic diagnostic, std::size_t iterations = 0) noexcept {
    ...
    [[nodiscard]] inline double scaled_tolerance(QpTolerance tolerance, double scale) noexcept {
    ...
    [[nodiscard]] inline bool valid_tolerance(QpTolerance tolerance) noexcept {
        return std::isfinite(tolerance.absolute) && std::isfinite(tolerance.relative) &&
               std::isfinite(tolerance.symmetry) && tolerance.absolute >= 0.0 && tolerance.relative >= 0.0 &&
               tolerance.symmetry >= 0.0;
    }

    [[nodiscard]] inline QpDiagnostic validate_vector(ConstDenseVectorView view) noexcept {
        if (view.size == 0) {
    ...
    [[nodiscard]] inline QpDiagnostic validate_vector(DenseVectorView view) noexcept {
        return validate_vector(ConstDenseVectorView{view});
    }

    [[nodiscard]] inline QpDiagnostic validate_matrix(ConstDenseMatrixView view) noexcept {
        ...
        return view.row_stride == 0 || view.column_stride == 0 ? QpDiagnostic::InvalidStride : QpDiagnostic::None;
    }
    ...
    [[nodiscard]] inline Matrix copy_matrix(ConstDenseMatrixView view) {
        Matrix result(static_cast<Eigen::Index>(view.rows), static_cast<Eigen::Index>(view.columns));
        for (std::size_t row = 0; row < view.rows; ++row) {
            for (std::size_t column = 0; column < view.columns; ++column) {
                result(static_cast<Eigen::Index>(row), static_cast<Eigen::Index>(column)) = view(row, column);
            }
        }
        return result;
    }

    inline void copy_to_view(const Vector& source, DenseVectorView destination) noexcept {
    ...
    [[nodiscard]] inline ConstDenseMatrixView view(const Matrix& value) noexcept {
        return ConstDenseMatrixView::column_major(
            value.data(), static_cast<std::size_t>(value.rows()), static_cast<std::size_t>(value.cols()));
    }
    ...
    [[nodiscard]] inline AddressInterval address_interval(DenseVectorView value) noexcept {
        if (value.size == 0 || value.data == nullptr) {
            return {};
        }

        constexpr std::uintptr_t max_address = std::numeric_limits<std::uintptr_t>::max();
        const std::uintptr_t base = reinterpret_cast<std::uintptr_t>(value.data);
        const std::uintptr_t stride_magnitude = value.stride >= 0
                                                    ? static_cast<std::uintptr_t>(value.stride)
    ...
    [[nodiscard]] inline bool overlaps(DenseVectorView left, DenseVectorView right) noexcept {
        const AddressInterval left_interval = address_interval(left);
        const AddressInterval right_interval = address_interval(right);
        return !left_interval.empty && !right_interval.empty && left_interval.first <= right_interval.last &&
