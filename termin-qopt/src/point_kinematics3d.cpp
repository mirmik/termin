...
namespace termin::qopt
{
    ...
    std::string_view
    point_kinematics3d_diagnostic_name(PointKinematics3DDiagnostic diagnostic) noexcept
    {
        switch (diagnostic)
        {
    ...
    PointKinematics3DDiagnostic PointKinematics3D::map_force_to_generalized_effort(
        termin::Vec3 force_world, DenseVectorView generalized_effort) const noexcept
    {
        ...
        if (linear_jacobian_world_storage.size() != 3 * columns ||
            ...
            (columns != 0 &&
             (generalized_effort.data == nullptr || generalized_effort.stride == 0)))
        {
            std::fprintf(stderr,
                         "[termin-qopt] invalid point-force transpose output\n");
        ...
        for (std::size_t column = 0; column < columns; ++column)
        {
            ...
            for (std::size_t row = 0; row < 3; ++row)
            {
                generalized_effort[column] +=
                    linear_jacobian_world_storage[row * columns + column] * force[row];
            }
        }
