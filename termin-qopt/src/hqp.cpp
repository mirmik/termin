...
[[nodiscard]] HqpSolveResult failure(
    QpStatus status,
...
[[nodiscard]] Matrix stack_matrices(
...
[[nodiscard]] Vector stack_targets(
    const std::vector<Constraint>& constraints
) {
  Eigen::Index size = 0;
  for (const Constraint& constraint : constraints) {
...
[[nodiscard]] Matrix nullspace(
  ...
  result = write_nullspace_basis(
      ...
      DenseMatrixView::column_major(
          storage.data(), variables, variables
      ),
      options
  );
...
[[nodiscard]] double task_residual_l2(
    const Level& level, const Vector& primal
) {
  double squared = 0.0;
...
[[nodiscard]] HqpDiagnostic validate_task(
    QuadraticTaskView task, std::size_t variables
) noexcept {
  const QpDiagnostic jacobian_diagnostic =
      validate_matrix(task.jacobian);
  ...
  if (
      jacobian_diagnostic != QpDiagnostic::None
      || target_diagnostic != QpDiagnostic::None
      || weight_diagnostic != QpDiagnostic::None
  ) {
    return HqpDiagnostic::DimensionMismatch;
  }
  if (
      task.jacobian.columns != variables
      || task.target.size != task.jacobian.rows
      || (
          !task.weight.empty()
          && (
              task.weight.rows != task.jacobian.rows
              || task.weight.columns != task.jacobian.rows
          )
      )
  ) {
    return HqpDiagnostic::DimensionMismatch;
...
[[nodiscard]] HqpDiagnostic validate_constraint(
    ConstDenseMatrixView matrix,
    ConstDenseVectorView target,
    std::size_t variables
) noexcept {
  if (
      validate_matrix(matrix) != QpDiagnostic::None
      || validate_vector(target) != QpDiagnostic::None
      || matrix.columns != variables
      || target.size != matrix.rows
  ) {
    return HqpDiagnostic::DimensionMismatch;
  }
...
struct HierarchicalQpSolver::Impl {
  std::uint64_t id =
      next_hqp_solver_id.fetch_add(1, std::memory_order_relaxed);
...
HierarchicalQpSolver::HierarchicalQpSolver(
    HierarchicalQpSolver&&
) noexcept = default;
HierarchicalQpSolver& HierarchicalQpSolver::operator=(
    HierarchicalQpSolver&&
) noexcept = default;
...
HqpLevelRegistrationResult HierarchicalQpSolver::add_level(
    int priority
) noexcept {
  ...
  try {
    ...
    std::fprintf(
        stderr, "[termin-qopt] add HQP level failed: %s\n", error.what()
    );
  } catch (...) {
    std::fprintf(
        stderr, "[termin-qopt] add HQP level failed with unknown exception\n"
    );
  }
  return {{}, HqpDiagnostic::InternalFailure};
}

HqpDiagnostic HierarchicalQpSolver::add_task(
    HqpLevelHandle level, QuadraticTaskView task
) noexcept {
  if (impl_ == nullptr || !impl_->valid(level)) {
    return HqpDiagnostic::InvalidLevel;
  }
  const HqpDiagnostic diagnostic =
      validate_task(task, impl_->variables);
  if (diagnostic != HqpDiagnostic::None) {
    return diagnostic;
  }
  try {
    Matrix weight = task.weight.empty()
        ? Matrix::Identity(
    ...
    const double symmetry_error =
        matrix_linf(weight - weight.transpose());
    ...
    const double spectral_scale = eigenvalues.size() == 0
        ? 1.0
        : std::max(1.0, eigenvalues.cwiseAbs().maxCoeff());
    const double rank_tolerance = 1e-12 * spectral_scale;
    const Eigen::Index protected_rows =
        (eigenvalues.array() > rank_tolerance).count();
    ...
    for (Eigen::Index index = 0; index < eigenvalues.size(); ++index) {
      ...
      weight_root.row(protected_row) =
          ...
          * eigensolver.eigenvectors().col(index).transpose();
      ++protected_row;
    }
    ...
    std::fprintf(
        stderr, "[termin-qopt] add HQP task failed: %s\n", error.what()
    );
  } catch (...) {
    std::fprintf(
        stderr, "[termin-qopt] add HQP task failed with unknown exception\n"
    );
  }
  return HqpDiagnostic::InternalFailure;
}

HqpDiagnostic HierarchicalQpSolver::add_equality(
    HqpLevelHandle level, EqualityConstraintView constraint
) noexcept {
  if (impl_ == nullptr || !impl_->valid(level)) {
    return HqpDiagnostic::InvalidLevel;
  }
  const HqpDiagnostic diagnostic = validate_constraint(
      constraint.matrix, constraint.target, impl_->variables
  );
  if (diagnostic != HqpDiagnostic::None) {
    return diagnostic;
  }
  try {
    ...
    std::fprintf(
        stderr, "[termin-qopt] add HQP equality failed: %s\n", error.what()
    );
  } catch (...) {
    std::fprintf(
        stderr,
        "[termin-qopt] add HQP equality failed with unknown exception\n"
    );
  }
  return HqpDiagnostic::InternalFailure;
}

HqpDiagnostic HierarchicalQpSolver::add_inequality(
  ...
  const HqpDiagnostic diagnostic = validate_constraint(
      constraint.matrix, constraint.limit, impl_->variables
  );
  if (diagnostic != HqpDiagnostic::None) {
    return diagnostic;
  }
  try {
    ...
    std::fprintf(
        stderr,
        "[termin-qopt] add HQP inequality failed: %s\n",
        error.what()
    );
  } catch (...) {
    std::fprintf(
        stderr,
        "[termin-qopt] add HQP inequality failed with unknown exception\n"
    );
  }
  return HqpDiagnostic::InternalFailure;
}

HqpSolveResult HierarchicalQpSolver::solve(
  ...
  if (impl_ == nullptr || impl_->variables == 0) {
    return failure(
        QpStatus::InvalidInput, HqpDiagnostic::InvalidVariableCount
    );
  }
  if (
      validate_vector(solution.primal) != QpDiagnostic::None
      || validate_vector(solution.level_task_residual_l2)
          != QpDiagnostic::None
      || solution.primal.size != impl_->variables
      || solution.level_task_residual_l2.size != impl_->levels.size()
      || (
          !initial_primal.empty()
          && initial_primal.size != impl_->variables
      )
  ) {
    return failure(
        QpStatus::InvalidInput, HqpDiagnostic::DimensionMismatch
    );
  }
  if (
      validate_vector(initial_primal) != QpDiagnostic::None
      || !finite(initial_primal)
  ) {
    return failure(
        QpStatus::InvalidInput, HqpDiagnostic::NonFiniteInput
    );
  }
  if (
      !valid_tolerance(options.qp.tolerance)
      || !valid_tolerance(options.nullspace.tolerance)
      || !std::isfinite(options.priority_tolerance)
      || options.priority_tolerance < 0.0
  ) {
    return failure(
        QpStatus::InvalidInput, HqpDiagnostic::InvalidOptions
    );
  }

  try {
    const Eigen::Index variables =
        static_cast<Eigen::Index>(impl_->variables);
    Vector primal = initial_primal.empty()
        ? Vector::Zero(variables)
        : copy_vector(initial_primal);
    ...
    std::sort(
        ...
        [&](std::size_t left, std::size_t right) {
          return impl_->levels[left].priority
              < impl_->levels[right].priority;
        }
    );
    ...
    for (std::size_t ordered_index = 0;
         ordered_index < order.size();
         ++ordered_index) {
      const Level& level = impl_->levels[order[ordered_index]];
      equalities.insert(
          equalities.end(),
          level.equalities.begin(),
          level.equalities.end()
      );
      inequalities.insert(
          inequalities.end(),
          level.inequalities.begin(),
          level.inequalities.end()
      );
      ...
      for (const Task& task : level.tasks) {
        hessian +=
            task.jacobian.transpose() * task.weight * task.jacobian;
        gradient -=
            task.jacobian.transpose() * task.weight * task.target;
      }

      const Matrix equality_matrix =
          stack_matrices(equalities, variables);
      const Vector equality_target = stack_targets(equalities);
      const Matrix inequality_matrix =
          stack_matrices(inequalities, variables);
      const Vector inequality_limit = stack_targets(inequalities);

      if (directions.cols() > 0) {
        const Matrix reduced_hessian =
            directions.transpose() * hessian * directions;
        const Vector reduced_gradient =
            directions.transpose() * (hessian * primal + gradient);
        const Matrix reduced_equalities =
            equality_matrix * directions;
        const Vector reduced_equality_targets =
            equality_target - equality_matrix * primal;
        const Matrix reduced_inequalities =
            inequality_matrix * directions;
        const Vector reduced_inequality_limits =
            inequality_limit - inequality_matrix * primal;
        ...
        Vector inequality_dual =
            Vector::Zero(reduced_inequalities.rows());
        Vector empty;
        ...
        if (level_result.status != QpStatus::Optimal) {
          HqpSolveResult failed = failure(
              level_result.status,
              HqpDiagnostic::LevelSolveFailure,
              ordered_index,
              level_result
          );
          failed.levels_solved = ordered_index;
          failed.remaining_dofs =
              static_cast<std::size_t>(directions.cols());
          return failed;
        }
        primal += directions * increment;
      } else {
        const double equality_error =
            linf(equality_matrix * primal - equality_target);
        const Vector inequality_error =
            inequality_matrix * primal - inequality_limit;
        const double inequality_violation =
            inequality_error.size() == 0
            ? 0.0
            : std::max(0.0, inequality_error.maxCoeff());
        const double tolerance = scaled_tolerance(
            options.qp.tolerance,
            std::max(
                matrix_linf(equality_matrix),
                matrix_linf(inequality_matrix)
            )
        );
        if (
            equality_error > tolerance
            || inequality_violation > tolerance
        ) {
          QpSolveResult level_result;
          level_result.status = QpStatus::Infeasible;
          level_result.diagnostic =
      ...
      for (const auto& [jacobian, frozen_value] : frozen_tasks) {
        result.priority_violation_linf = std::max(
            result.priority_violation_linf,
            linf(jacobian * primal - frozen_value)
        );
      }
      ...
      for (const Task& task : level.tasks) {
        ...
        frozen_tasks.emplace_back(
            task.protected_jacobian,
            task.protected_jacobian * primal
        );
      }
      if (!preserve.empty() && directions.cols() > 0) {
        const Matrix reduced_preserve =
            stack_matrices(preserve, variables) * directions;
        NullspaceResult nullspace_result;
        const Matrix reduced_nullspace = nullspace(
            reduced_preserve, options.nullspace, nullspace_result
        );
        if (nullspace_result.status != QpStatus::Optimal) {
          return failure(
              nullspace_result.status,
              HqpDiagnostic::LevelSolveFailure,
              ordered_index
          );
        }
    ...
    std::fprintf(
        stderr, "[termin-qopt] HQP solve failed: %s\n", error.what()
    );
  } catch (...) {
    std::fprintf(
        stderr, "[termin-qopt] HQP solve failed with unknown exception\n"
    );
  }
  return failure(
      QpStatus::NumericalFailure, HqpDiagnostic::InternalFailure
  );
}
