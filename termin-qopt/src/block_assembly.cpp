...
[[nodiscard]] AssemblyDiagnostic validate_matrix_view(
    ConstDenseMatrixView view,
...
[[nodiscard]] AssemblyDiagnostic validate_vector_view(
    ConstDenseVectorView view, std::size_t expected_size
) noexcept {
...
void log_internal_failure(const char* operation, const char* message) noexcept {
  std::fprintf(
      stderr,
      "[termin-qopt] dense block assembly %s failed: %s\n",
      operation,
      message
  );
}
...
std::string_view assembly_diagnostic_name(
...
DenseBlockTopology& DenseBlockTopology::operator=(
    DenseBlockTopology&&
) noexcept = default;

DenseBlockRegistrationResult DenseBlockTopology::register_block(
  ...
  if (!diagnostic_name.empty()) {
    const auto duplicate = std::find_if(
        impl_->blocks.begin(),
        impl_->blocks.end(),
        [diagnostic_name](const Impl::Block& block) {
...
DenseBlockInfo DenseBlockTopology::block_info(
    DenseBlockHandle handle
) const noexcept {
  if (impl_ == nullptr
      || handle.topology_id != impl_->id
      || handle.index >= impl_->blocks.size()) {
...
DenseBlockMatrixAssembly::DenseBlockMatrixAssembly(
    const DenseBlockTopology& row_topology,
    const DenseBlockTopology& column_topology,
    DenseMatrixView target
) noexcept
    : rows_(&row_topology),
  ...
  diagnostic_ = validate_matrix_view(
      ConstDenseMatrixView(target_),
      rows_->total_size(),
      columns_->total_size()
  );
}
...
DenseMatrixBlockResult DenseBlockMatrixAssembly::block(
    DenseBlockHandle row, DenseBlockHandle column
) const noexcept {
  if (!valid()) {
    return {{}, diagnostic_};
  }
...
AssemblyDiagnostic DenseBlockMatrixAssembly::add(
    DenseBlockHandle row,
  ...
  const AssemblyDiagnostic source_diagnostic = validate_matrix_view(
      contribution, destination.view.rows, destination.view.columns
  );
...
DenseBlockVectorAssembly::DenseBlockVectorAssembly(
    const DenseBlockTopology& topology, DenseVectorView target
) noexcept
  ...
  diagnostic_ = validate_vector_view(
      ConstDenseVectorView(target_), topology_->total_size()
  );
}
...
DenseVectorBlockResult DenseBlockVectorAssembly::block(
    DenseBlockHandle handle
) const noexcept {
  if (!valid()) {
    return {{}, diagnostic_};
...
AssemblyDiagnostic DenseBlockVectorAssembly::add(
    DenseBlockHandle handle, ConstDenseVectorView contribution
) noexcept {
  const DenseVectorBlockResult destination = block(handle);
  if (!destination.ok()) {
    return destination.diagnostic;
  }
  const AssemblyDiagnostic source_diagnostic = validate_vector_view(
      contribution, destination.view.size
  );
  if (source_diagnostic != AssemblyDiagnostic::None) {
    return source_diagnostic;
  }
