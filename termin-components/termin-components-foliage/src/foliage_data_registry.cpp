...
namespace termin {
    ...
    bool TcFoliageData::ensure_loaded() const {
        ...
        if (!ptr->load_from_file(ptr->source_path)) {
            tc_log_error("[FoliageData] failed to load '%s' uuid=%s", ptr->source_path.c_str(), ptr->uuid.c_str());
            return false;
        }
        return true;
    }
    ...
    bool TcFoliageData::reload() const {
        ...
        if (!ptr->load_from_file(ptr->source_path)) {
            tc_log_error("[FoliageData] failed to reload '%s' uuid=%s", ptr->source_path.c_str(), ptr->uuid.c_str());
            return false;
        }
        return true;
    }
    ...
    TcFoliageData
    TcFoliageData::declare(const std::string& uuid, const std::string& name, const std::string& source_path) {
        if (uuid.empty()) {
