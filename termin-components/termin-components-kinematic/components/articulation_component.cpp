...
namespace termin
{
    namespace
    {
        ...
        bool local_transform_is_rigid(Entity entity)
        {
            ...
            return unit_scale(pose.scale) && pose.lin.is_finite() &&
                   pose.ang.is_finite() &&
        ...
        {
            return Pose3{
                Quat{unit.origin_rotation.x,
                     unit.origin_rotation.y,
                     unit.origin_rotation.z,
                     unit.origin_rotation.w}
                    .normalized(),
                Vec3{unit.origin_position.x,
                     unit.origin_position.y,
                     unit.origin_position.z},
            };
        }
        ...
        std::vector<KinematicUnitComponent*> enabled_units(Entity entity)
        {
                 ...
                 ++index)
            {
                ...
                auto* unit = dynamic_cast<KinematicUnitComponent*>(
    ...
    std::string_view articulation_component_diagnostic_name(
        ArticulationComponentDiagnostic diagnostic) noexcept
    {
        switch (diagnostic)
        {
    ...
    {
        auto descriptor =
            ComponentTypeDescriptorBuilder::native<ArticulationComponent>(
        ...
        (void)descriptor.inspect().add_button(
            "rebuild",
            "Rebuild Articulation",
            [](void* object, const tc::InspectContext&)
            {
    ...
    bool ArticulationComponent::rebuild()
    {
        ...
        const auto fail = [&](ArticulationComponentDiagnostic diagnostic,
                              Entity candidate)
        {
        ...
        compile_children = [&](Entity parent_entity, std::size_t parent_unit)
        {
            ...
            for (Entity child : parent_entity.children())
            {
                ...
                {
                    fail(ArticulationComponentDiagnostic::NestedArticulation,
                         child);
                    return;
                }

                const std::vector<KinematicUnitComponent*> child_units =
                    enabled_units(child);
                if (child_units.empty())
                {
                    if (subtree_has_enabled_unit(child))
                    {
                        fail(ArticulationComponentDiagnostic::IndirectUnit,
                             child);
                        return;
                    }
                    continue;
                ...
                {
                    fail(ArticulationComponentDiagnostic::MultipleUnitsOnEntity,
                         child);
                    return;
                }
                ...
                {
                    fail(ArticulationComponentDiagnostic::UnsupportedUnit,
                         child);
                    return;
                }
                if (!local_transform_is_rigid(child))
                {
                    fail(ArticulationComponentDiagnostic::NonRigidTransform,
                         child);
                    return;
                }
                ...
                {
                    fail(ArticulationComponentDiagnostic::InvalidCoordinateScale,
                         child);
                    return;
                }
                if (!std::isfinite(unit->min_coordinate) ||
                    !std::isfinite(unit->max_coordinate) ||
                    unit->min_coordinate > unit->max_coordinate)
                {
                    fail(ArticulationComponentDiagnostic::InvalidCoordinateLimits,
                         child);
                    return;
                }
                ...
                if (!inertia.is_valid())
                {
                    fail(ArticulationComponentDiagnostic::InvalidInertia,
                         child);
                    return;
                }
                ...
                {
                    zero_pose = (root.transform().global_pose() * zero_pose)
                                    .normalized();
                }
        ...
        {
            tc::Log::error(
                "[ArticulationComponent] compilation failed at '%s': %s",
        ...
        {
            tc::Log::error(
                "[ArticulationComponent] reduced model is invalid: %s",
                robotics::articulation3d_diagnostic_name(
                    articulation_->diagnostic())
                    .data());
            articulation_.reset();
            bindings_.clear();
            coordinate_scales_.clear();
    ...
    bool ArticulationComponent::synchronize()
    {
        if (articulation_ == nullptr ||
            bindings_.size() != articulation_->unit_count() ||
            coordinate_scales_.size() != bindings_.size() ||
            articulation_->state().coordinates.size() != bindings_.size())
        {
            diagnostic_ =
                ArticulationComponentDiagnostic::SynchronizationFailure;
            tc::Log::error(
                "[ArticulationComponent] cannot synchronize an incomplete "
                "runtime binding");
            return false;
        }
        for (std::size_t index = 0; index < bindings_.size(); ++index)
        {
            ...
            if (unit == nullptr || !std::isfinite(scale) || scale <= 0.0)
            {
                diagnostic_ =
                    ArticulationComponentDiagnostic::SynchronizationFailure;
                tc::Log::error(
                    "[ArticulationComponent] invalid unit binding at index %zu",
                    index);
                return false;
            }
            unit->set_coordinate(
                articulation_->state().coordinates[index] / scale);
        }
        diagnostic_ = ArticulationComponentDiagnostic::None;
        diagnostic_entity_.clear();
        return true;
    ...
    bool ArticulationComponent::integrate_velocity(
        std::span<const double> generalized_velocity,
        double time_step)
    {
        if (articulation_ == nullptr)
        {
            diagnostic_ = ArticulationComponentDiagnostic::InvalidModel;
            tc::Log::error(
                "[ArticulationComponent] cannot integrate before rebuild");
            return false;
        }
        const robotics::VelocityIntegrationResult3D result =
            robotics::integrate_articulation_velocity(
                *articulation_,
                {generalized_velocity.data(), generalized_velocity.size()},
                time_step);
        if (!result.ok())
        {
            diagnostic_ = ArticulationComponentDiagnostic::IntegrationFailure;
            tc::Log::error(
                "[ArticulationComponent] velocity integration failed: %s",
                robotics::velocity_integration_diagnostic_name(
                    result.diagnostic)
                    .data());
            return false;
        }
