    ...
    tc_debug_geometry_primitive_kind kind)
{
    ...
    for (std::size_t index = 0;
         index < tc_scene_debug_geometry_primitive_count(scene);
         ++index) {
        const tc_debug_geometry_primitive* primitive =
            tc_scene_debug_geometry_primitive_at(scene, index);
        REQUIRE(primitive != nullptr);
        if (primitive->kind == kind) ++result;
    }
    return result;
}
...
TEST_CASE("colliders publish registry-controlled canonical debug primitives") {
    ...
    const tc_debug_geometry_type_id type_id =
        tc_debug_geometry_type_find("physics.colliders");
    ...
    const std::size_t all_count =
        tc_scene_debug_geometry_primitive_count(scene.handle());
