...
NB_MODULE(_components_physics_native, m) {
    ...
    nb::class_<PhysicsWorldComponent, CxxComponent>(m, "PhysicsWorldComponent")
        .def(
            "__init__",
            [](nb::handle self, const Vec3& gravity, int iterations, double restitution, double friction) {
        ...
        .def_prop_rw(
            "gravity",
            [](const PhysicsWorldComponent& c) { return Vec3{c.gravity.x, c.gravity.y, c.gravity.z}; },
            [](PhysicsWorldComponent& c, const Vec3& value) { c.gravity = value; })
        ...
        .def_prop_ro(
            "physics_world",
            [](PhysicsWorldComponent& c) -> physics::PhysicsWorld& { return c.physics_world(); },
            nb::rv_policy::reference_internal);

    nb::class_<RigidBodyComponent, CxxComponent>(m, "RigidBodyComponent")
        .def(
            "__init__",
            [](nb::handle self, double mass, bool is_static, double restitution, double friction) {
        ...
        .def_prop_ro(
            "rigid_body", [](RigidBodyComponent& c) { return c.rigid_body(); }, nb::rv_policy::reference_internal)
