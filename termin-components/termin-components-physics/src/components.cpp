...
bool poses_near(const Pose3& lhs, const Pose3& rhs) {
    ...
    const double dot = lhs.ang.x * rhs.ang.x + lhs.ang.y * rhs.ang.y +
                       lhs.ang.z * rhs.ang.z + lhs.ang.w * rhs.ang.w;
    return (lhs.lin - rhs.lin).norm() <= translation_tolerance &&
           1.0 - std::abs(dot) <= orientation_tolerance;
}
...
void stage_double(tc::InspectFacetBuilder& inspect,
    ...
    tc::stage_inspect_field(
        inspect, member, type_name, path, label, "double", min, max, step);
}
...
void RigidBodyComponent::register_type() {
    auto descriptor = ComponentTypeDescriptorBuilder::native<RigidBodyComponent>(
        "RigidBodyComponent", module_owner, "Component");
...
void RigidBodyComponent::start() {
    ...
    tc::Log::error("[RigidBodyComponent] no enabled PhysicsWorldComponent in scene for '%s'",
                   entity().name());
}
...
bool RigidBodyComponent::physics_pose_and_scale(
    Pose3& pose, Vec3& scale, bool report_error) const {
    ...
    if (!maybe_scale.has_value()) {
        if (report_error) {
            tc::Log::error(
                "[RigidBodyComponent] '%s' rejects an affine world transform; rigid physics requires a decomposed world basis",
                owner.name());
        }
        return false;
    }
    if (!valid_scale(*maybe_scale)) {
        if (report_error) {
            tc::Log::error(
                "[RigidBodyComponent] '%s' rejects non-positive or non-finite world scale (%g, %g, %g)",
                owner.name(), maybe_scale->x, maybe_scale->y, maybe_scale->z);
        }
        return false;
    }
...
bool RigidBodyComponent::make_body(physics::RigidBody& body) {
    ...
    if (!std::isfinite(mass) || mass <= 0.0) {
        tc::Log::error("[RigidBodyComponent] '%s' rejects invalid mass=%g",
                       entity().name(), mass);
        return false;
    }
    ...
    if (collider_component_ == nullptr) {
        body = physics::RigidBody::create_box(
            scale.x, scale.y, scale.z, mass, pose, is_static);
        return true;
    }
    ...
    if (!physics::try_compute_mass_properties(
            *primitive, scale, mass, properties, diagnostic)) {
        tc::Log::error("[RigidBodyComponent] '%s' rejected %s mass properties: %s",
                       entity().name(), collider_component_->collider_type.c_str(),
                       diagnostic.c_str());
        return false;
    }
    body = physics::RigidBody::create_with_mass_properties(
        properties, pose, is_static);
    return true;
}
...
bool RigidBodyComponent::ensure_collider_registered() {
    ...
    colliders::Collider* attached =
        current != nullptr ? current->attached_collider() : nullptr;
    const std::uint64_t revision =
        current != nullptr ? current->collider_revision() : 0;
...
void RigidBodyComponent::reconcile_external_transform() {
    ...
    if (has_last_synced_entity_pose_ &&
        !poses_near(entity_pose, last_synced_entity_pose_)) {
...
void RigidBodyComponent::apply_impulse_at_point(
    const Vec3& impulse, const Vec3& point) {
    if (physics::RigidBody* body = rigid_body(); body != nullptr) {
...
void PhysicsWorldComponent::register_type() {
    ...
    stage_double(inspect,
                 ...
                 0.05);
    stage_double(inspect,
                 &PhysicsWorldComponent::friction,
                 "PhysicsWorldComponent",
                 "friction",
                 "Friction",
                 0.0,
...
void PhysicsWorldComponent::fixed_update(float dt) {
    ...
    if (!std::isfinite(dt) || dt <= 0.0F) {
        tc::Log::error("[PhysicsWorldComponent] received invalid fixed dt=%g",
...
bool PhysicsWorldComponent::rebuild() {
    ...
    collision::CollisionWorld* collision_world =
        collision::CollisionWorld::from_scene(owner.scene().handle());
...
bool PhysicsWorldComponent::add_rigid_body_component(
...
void PhysicsWorldComponent::remove_rigid_body_component(
    RigidBodyComponent& component) {
