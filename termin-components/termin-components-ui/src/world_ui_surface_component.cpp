...
WorldUiSurfaceComponent* resolve_surface(tc_component* component) {
    ...
    WorldUiSurfaceComponent* surface =
        dynamic_cast<WorldUiSurfaceComponent*>(base);
    if (!surface) {
        tc_log_error(
            "[WorldUiSurfaceComponent] world-pointer capability attached "
            "to the wrong component type");
    }
    return surface;
}

bool project_surface_ray(
    tc_component* component,
    const tc_world_pointer_ray* ray,
    tc_world_pointer_hit* out_hit
) {
    WorldUiSurfaceComponent* surface = resolve_surface(component);
    return surface && ray && out_hit &&
        surface->project_ray(*ray, *out_hit);
}

bool dispatch_surface_pointer(
    tc_component* component,
...
WorldUiSurfaceComponent::WorldUiSurfaceComponent()
    ...
    if (!tc_world_pointer_surface_capability_attach(
            &_c, &kWorldUiSurfaceVtable, this)) {
        tc_log_error(
            "[WorldUiSurfaceComponent] failed to attach world-pointer "
            "surface capability");
    }
}
...
void WorldUiSurfaceComponent::register_type() {
    auto descriptor =
        ComponentTypeDescriptorBuilder::native<WorldUiSurfaceComponent>(
    ...
    inspect.add<WorldUiSurfaceComponent, std::string>(
        "WorldUiSurfaceComponent",
    ...
    inspect.add<WorldUiSurfaceComponent, double>(
        "WorldUiSurfaceComponent",
        &WorldUiSurfaceComponent::local_width,
        "local_width",
        "Local Width",
        "double");
    inspect.add<WorldUiSurfaceComponent, double>(
    ...
    inspect.add<WorldUiSurfaceComponent, bool>(
        "WorldUiSurfaceComponent",
        &WorldUiSurfaceComponent::two_sided,
        "two_sided",
        "Two Sided",
        "bool");
...
bool WorldUiSurfaceComponent::project_ray(
    const tc_world_pointer_ray& ray,
    tc_world_pointer_hit& out_hit
) {
    if (!entity().valid() || local_width <= 0.0 || local_height <= 0.0 ||
    ...
    try {
        ...
        const Vec3 local_origin = transform.transform_point_inverse(
            {ray.origin_x, ray.origin_y, ray.origin_z});
        const Vec3 local_direction = transform.transform_vector_inverse(
            {ray.direction_x, ray.direction_y, ray.direction_z});
        constexpr double parallel_epsilon = 1.0e-10;
        if (std::abs(local_direction.z) <= parallel_epsilon) return false;
        if (!two_sided && local_direction.z >= -parallel_epsilon) return false;

        const double distance = -local_origin.z / local_direction.z;
        if (distance < 0.0 || distance > ray.max_distance) return false;
        ...
        out_hit.inside =
            out_hit.u >= 0.0 && out_hit.u <= 1.0 &&
            out_hit.v >= 0.0 && out_hit.v <= 1.0;
        logged_invalid_transform_ = false;
        return true;
    } catch (const std::exception& error) {
        if (!logged_invalid_transform_) {
            tc_log_error(
                "[WorldUiSurfaceComponent] cannot project ray onto '%s': %s",
                entity().name(), error.what());
...
UIComponent* WorldUiSurfaceComponent::resolve_ui_component() {
    ...
    Entity target = ui_entity_uuid.empty()
        ? entity()
        : entity().scene().get_entity(ui_entity_uuid);
    UIComponent* ui = target.valid()
        ? target.get_component<UIComponent>()
        : nullptr;
    if (!ui) {
        if (!logged_missing_ui_) {
            tc_log_error(
                "[WorldUiSurfaceComponent] surface '%s' cannot resolve "
...
bool WorldUiSurfaceComponent::dispatch_pointer(
    const tc_world_pointer_event& event
) {
...
void WorldUiSurfaceComponent::on_removed() { cancel_last_pointer(); }
void WorldUiSurfaceComponent::on_destroy() { cancel_last_pointer(); }
void WorldUiSurfaceComponent::on_scene_inactive() { cancel_last_pointer(); }
