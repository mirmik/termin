...
namespace termin {
    ...
    public:
        ...
        std::string ui_layout_uuid() const {
            return ui_layout_.uuid();
        }
        bool set_ui_layout_uuid(const std::string& uuid);
        gui_native::TcUiDocumentAsset ui_layout() const {
            return ui_layout_;
        }
    ...
    private:
        ...
        void update_platform_services(const tc_input_platform_services* services);
        void sync_platform_state();
        void cancel_interaction(tc_ui_pointer_cancel_reason reason, bool clear_focus);
        bool physical_to_logical(float physical_x, float physical_y, tc_ui_point& logical);
        ...
        static bool clipboard_set_text(void* user_data, const char* text, size_t byte_length);
