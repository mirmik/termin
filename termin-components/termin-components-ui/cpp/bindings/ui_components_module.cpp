...
NB_MODULE(_ui_components_native, module) {
    ...
    nb::class_<termin::UIComponent, termin::CxxComponent>(
        module, "UIComponent")
        .def("__init__", [](nb::handle self, int priority) {
            termin::cxx_component_init<termin::UIComponent>(self, priority);
        }, nb::arg("priority") = 1000)
        .def_prop_rw(
            "priority",
            &termin::UIComponent::priority,
            &termin::UIComponent::set_priority)
        .def_prop_rw(
            "input_source_mask",
            ...
            [](termin::UIComponent& self, const std::string& uuid) {
                if (!self.set_ui_layout_uuid(uuid)) {
                    throw std::runtime_error(
                        "failed to assign native UI document asset '" +
                        uuid + "'");
                }
            })
        .def_prop_rw(
    ...
    module.def(
        ...
        [](std::uintptr_t component_ptr) {
            const auto* component =
                reinterpret_cast<const tc_component*>(component_ptr);
