...
namespace termin {
    ...
    public:
        ...
        INSPECT_FIELD_CHOICES(DepthPass,
                              depth_encoding,
                              "Depth Encoding",
                              "string",
                              {"linear", "Linear"},
        ...
        DepthPass(const std::string& input_res = "empty_depth",
        ...
        void execute_with_data_tgfx2(ExecuteContext& ctx, const DepthPassExecuteData& data);
    ...
    protected:
        ...
        bool uses_material_phase_shader_override() const override {
            return true;
        }
        MaterialPipelinePassContract shader_pass_contract() const override;
        tc_shader_handle shader_usage_base_shader() const override;
        std::optional<std::string> fbo_format() const override {
            return "r16f";
        }
    };

    class ENTITY_API DepthOnlyPass : public CxxFramePass, public SceneShaderUsageProvider {
    public:
        struct DrawCall {
        public:
            Entity entity;
    ...
    public:
        ...
        INSPECT_TYPE_METADATA(DepthOnlyPass,
                              graph,
                              make_pass_graph_metadata({{"output_res_target", "depth_texture"}},
                                                       {{"output_res", "depth_texture"}},
                                                       {{"output_res_target", "output_res"}}))

        DepthOnlyPass(const std::string& output_res = "depth_texture", const std::string& pass_name = "DepthOnly")
            : output_res(output_res) {
            set_pass_name(pass_name);
        }
        static void register_type();

        ~DepthOnlyPass() override {
            release_tgfx2_resources();
        }
        ...
        std::set<const char*> compute_reads() const override {
            return output_res_target.empty() ? std::set<const char*>{}
    ...
    public:
        ...
        INSPECT_TYPE_METADATA(DepthToColorPass,
                              graph,
                              make_pass_graph_metadata({{"input_res", "depth_texture"},
                                                        {"output_res_target", "color_texture"}},
                                                       {{"output_res", "color_texture"}},
                                                       {{"output_res_target", "output_res"}}))

        DepthToColorPass(const std::string& input_res = "depth_texture",
    ...
    public:
        ...
        INSPECT_TYPE_METADATA(ColorToDepthPass,
                              graph,
                              make_pass_graph_metadata({{"input_res", "color_texture"},
                                                        {"output_res_target", "depth_texture"}},
                                                       {{"output_res", "depth_texture"}},
                                                       {{"output_res_target", "output_res"}}))

        ColorToDepthPass(const std::string& input_res = "color_texture",
