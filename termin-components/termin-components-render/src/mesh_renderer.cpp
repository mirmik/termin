...
namespace termin {
    ...
    namespace {
        ...
        bool is_builtin_symbolic_texture(const char* uuid, const char* name) {
            ...
            for (const char* value : values) {
                if ((uuid && std::strcmp(uuid, value) == 0) || (name && std::strcmp(name, value) == 0)) {
                    return true;
                }
            }
            return false;
        ...
        tc_value make_mesh_renderer_inspector_layout_field(const char* path,
        ...
        tc_value make_mesh_renderer_inspector_metadata() {
            ...
            tc_value_list_push(&layout,
                               make_mesh_renderer_inspector_layout_field(
                                   "_overridden_material", "inline_material", "_override_material"));
        ...
        void register_mesh_renderer_inspect_fields(tc::InspectFacetBuilder& inspect) {
            if (!inspect.find_field("MeshRenderer", "material")) {
                tc::stage_inspect_field(
                    inspect, &MeshRenderer::material, "MeshRenderer", "material", "Material", "tc_material");
            }
            ...
            if (!inspect.find_field("MeshRenderer", "materials")) {
                inspect.add_serializable<MeshRenderer>(
                    "materials",
                    [](MeshRenderer* self) -> tc_value { return serialize_mesh_renderer_material_slots(self); },
            ...
            if (!inspect.find_field("MeshRenderer", "_overridden_material")) {
                inspect.add_with_accessors<MeshRenderer, TcMaterial>(
                    ...
                    "Overridden Material",
                    "tc_material",
                    [](MeshRenderer* self) -> TcMaterial { return self ? self->_overridden_material : TcMaterial(); },
            ...
            if (!inspect.find_field("MeshRenderer", "_overridden_material_data")) {
                inspect.add_serializable<MeshRenderer>(
                    ...
                    [](MeshRenderer* self) -> tc_value { return self ? self->get_override_data() : tc_value_nil(); },
                    [](MeshRenderer* self, const tc_value* val) {
                        if (self) {
    ...
    TcMaterial MeshRenderer::get_material_for_slot(size_t slot) const {
        ...
        tc::Log::warn("[MeshRenderer] Missing material slot %zu for renderer on entity '%s'; using slot 0",
                      slot,
    ...
    TcMaterial MeshRenderer::get_material_for_submesh(size_t submesh_index) const {
        ...
        if (!submesh) {
            tc::Log::warn("[MeshRenderer] Missing submesh %zu for mesh '%s'; using slot 0 material",
                          submesh_index,
    ...
    void MeshRenderer::apply_pending_override_data() {
        if (_pending_override_data.type == TC_VALUE_NIL || !_overridden_material.is_valid())
            return;
        ...
        if (phases_uniforms && phases_uniforms->type == TC_VALUE_LIST) {
            ...
            for (size_t i = 0; i < phase_count; ++i) {
                ...
                if (!phase_uniforms || phase_uniforms->type != TC_VALUE_DICT)
                    continue;
        ...
        if (phases_textures && phases_textures->type == TC_VALUE_LIST) {
            ...
            for (size_t i = 0; i < phase_count; ++i) {
                ...
                if (!phase_textures || phase_textures->type != TC_VALUE_DICT)
                    continue;
                ...
                for (size_t j = 0; j < tex_count; ++j) {
                    ...
                    if (!key || !val || val->type != TC_VALUE_DICT)
                        continue;
            ...
            if (mat) {
                ...
                for (size_t i = 0; i < mat->phase_count; ++i) {
                    ...
                    for (size_t j = 0; j < phase.texture_count; ++j) {
                        ...
                        if (!tex)
                            continue;
                        if (is_builtin_symbolic_texture(tex->header.uuid, tex->header.name)) {
                            continue;
                        }
