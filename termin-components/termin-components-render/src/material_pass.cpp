...
namespace termin {
    ...
    static void tc_value_to_string_map(const tc_value* value, std::unordered_map<std::string, std::string>& out) {
        out.clear();
    ...
    static bool shader_has_material_field(const tc_shader* shader, const std::string& field_name) {
        ...
        if (!material_rb || material_rb->kind != TC_SHADER_RESOURCE_CONSTANT_BUFFER || !material_rb->fields) {
            return false;
        }
    ...
    bool MaterialPass::set_graph_resource_input(const std::string& socket_name, const std::string& resource_name) {
    ...
    void MaterialPass::execute(ExecuteContext& ctx) {
        ...
        if (!mat || mat->phase_count == 0) {
            tc::Log::error("[MaterialPass] '%s': material not set or has no phases", get_pass_name().c_str());
            return;
        }

        tc_material_phase* phase = &mat->phases[0];

        ctx2->begin_pass(color_tex2,
                         /*depth=*/{},
                         /*clear_color=*/nullptr,
                         /*clear_depth=*/1.0f,
        ...
        if (!ensure_material_pipeline_shader(*ctx2, device, phase->shader, "MaterialPass", shader_binding)) {
            ctx2->end_pass();
            return;
        }
        ...
        auto bind_graph_texture = [&](const std::string& res_name, const std::string& uniform_name) {
            ...
            if (!rb || rb->kind != TC_SHADER_RESOURCE_TEXTURE) {
                tc::Log::error("[MaterialPass] '%s': texture uniform '%s' is not declared as a shader "
            ...
            if (shader_has_material_field(shader_binding.shader, texel_size_uniform)) {
                ...
                if (desc.width > 0 && desc.height > 0) {
                    ...
                    if (!tc_material_phase_set_uniform(
                            phase, texel_size_uniform.c_str(), TC_UNIFORM_VEC2, texel_size)) {
                        tc::Log::error("[MaterialPass] '%s': failed to update texture texel-size "
                                       "material property '%s'",
                                       get_pass_name().c_str(),
    ...
    void MaterialPass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<MaterialPass>("MaterialPass", "termin-components-render");
