...
namespace termin {
    ...
    namespace {
        ...
        bool collect_nearest_world_pointer_surface(tc_component* component, void* userdata) {
            auto* nearest = static_cast<NearestWorldPointerSurface*>(userdata);
            if (!nearest || !nearest->ray)
                return false;
            tc_world_pointer_hit hit{};
            if (!tc_world_pointer_surface_project_ray(component, nearest->ray, &hit) || !hit.inside) {
                return true;
            }
        ...
        tc_component* resolve_world_pointer_surface(tc_entity_handle entity, const std::string& source_id) {
            if (!tc_entity_handle_valid(entity) || source_id.empty())
                return nullptr;
            ...
            for (std::size_t index = 0; index < count; ++index) {
                ...
                const char* candidate_source_id = component ? tc_component_get_source_id(component) : nullptr;
                if (candidate_source_id && source_id == candidate_source_id && tc_component_get_enabled(component) &&
                    tc_component_has_capability(component, tc_world_pointer_surface_capability_id())) {
                    return component;
                }
            }
            return nullptr;
        ...
        bool
        same_world_pointer_surface(tc_component* component, tc_entity_handle entity, const std::string& source_id) {
            if (!component || !tc_entity_handle_valid(entity))
                return false;
            const char* component_source_id = tc_component_get_source_id(component);
            return tc_entity_handle_eq(component->owner, entity) && component_source_id &&
                   source_id == component_source_id;
        }

        bool dispatch_world_pointer(tc_component* surface,
    ...
    void XrTrackedPoseComponent::register_type() {
        ...
        if (!inspect.find_field("XrTrackedPoseComponent", "input_device_id")) {
            inspect.add<XrTrackedPoseComponent, std::string>("XrTrackedPoseComponent",
                                                             &XrTrackedPoseComponent::input_device_id,
                                                             "input_device_id",
                                                             "Input Device",
                                                             "string");
        }
        if (!inspect.find_field("XrTrackedPoseComponent", "hand"))
            register_hand_field(inspect, "XrTrackedPoseComponent");
        if (!inspect.find_field("XrTrackedPoseComponent", "pose_kind"))
            register_pose_kind_field(inspect);
    ...
    void XrTrackedPoseComponent::update(float) {
        ...
        if (!origin) {
            if (!logged_missing_origin_) {
                tc_log_error("[XrTrackedPoseComponent] entity '%s' is not under an XrOriginComponent", entity().name());
    ...
    std::string XrTrackedPoseComponent::get_hand_str() const {
        return xr::xr_hand_to_string(hand);
    }
    void XrTrackedPoseComponent::set_hand_str(const std::string& value) {
        hand = xr::xr_hand_from_string(value);
    }
    std::string XrTrackedPoseComponent::get_pose_kind_str() const {
        return xr_tracked_pose_kind_to_string(pose_kind);
    }
    ...
    void XrGrabInteractableComponent::register_type() {
        ...
        if (!inspect.find_field("XrGrabInteractableComponent", "grab_radius")) {
            inspect.add<XrGrabInteractableComponent, double>("XrGrabInteractableComponent",
                                                             &XrGrabInteractableComponent::grab_radius,
                                                             "grab_radius",
                                                             "Grab Radius",
                                                             "double");
        }
        if (!inspect.find_field("XrGrabInteractableComponent", "grabbable")) {
            inspect.add<XrGrabInteractableComponent, bool>("XrGrabInteractableComponent",
                                                           &XrGrabInteractableComponent::grabbable,
                                                           "grabbable",
                                                           "Grabbable",
                                                           "bool");
        }
    ...
    void XrDirectGrabInteractorComponent::register_type() {
        ...
        if (!inspect.find_field("XrDirectGrabInteractorComponent", "reach")) {
            inspect.add<XrDirectGrabInteractorComponent, double>(
                "XrDirectGrabInteractorComponent", &XrDirectGrabInteractorComponent::reach, "reach", "Reach", "double");
        }
        if (!inspect.find_field("XrDirectGrabInteractorComponent", "select_threshold")) {
            inspect.add<XrDirectGrabInteractorComponent, double>("XrDirectGrabInteractorComponent",
                                                                 &XrDirectGrabInteractorComponent::select_threshold,
                                                                 "select_threshold",
                                                                 "Select Threshold",
                                                                 "double");
        }
    ...
    void XrDirectGrabInteractorComponent::update(float) {
        ...
        const bool pressed = input->hand(tracker->hand).select.pressed(std::clamp(select_threshold, 0.0, 1.0));
        const Pose3 hand_world_pose = entity().transform().global_pose();

        if (pressed && !was_pressed_ && !holding_object())
            acquire_nearest(hand_world_pose);
        if (!pressed && holding_object())
            release_grabbed();

        if (pressed && holding_object()) {
            XrGrabInteractableComponent* interactable = grabbed_entity_.get_component<XrGrabInteractableComponent>();
    ...
    void XrDirectGrabInteractorComponent::acquire_nearest(const Pose3& hand_world_pose) {
        ...
        for (Entity candidate : entity().scene().get_all_entities()) {
            if (!candidate.valid() || candidate == entity())
                continue;
            XrGrabInteractableComponent* interactable = candidate.get_component<XrGrabInteractableComponent>();
            if (!interactable || !interactable->enabled() || !interactable->grabbable || interactable->owner_)
                continue;
        ...
        tc_log_info("[XrDirectGrabInteractorComponent] '%s' grabbed '%s'", entity().name(), nearest.name());
    }

    void XrDirectGrabInteractorComponent::release_grabbed(XrGrabInteractableComponent* expected) {
        if (!grabbed_entity_.valid())
            return;
        XrGrabInteractableComponent* interactable = grabbed_entity_.get_component<XrGrabInteractableComponent>();
        if (expected && interactable != expected)
            return;
        if (interactable && interactable->owner_ == this)
            interactable->owner_ = nullptr;
        tc_log_info("[XrDirectGrabInteractorComponent] '%s' released '%s'",
                    entity().valid() ? entity().name() : "<detached>",
                    grabbed_entity_.name());
        grabbed_entity_ = {};
    }

    void XrDirectGrabInteractorComponent::on_removed() {
        release_grabbed();
    }
    void XrDirectGrabInteractorComponent::on_destroy() {
        release_grabbed();
    }
    ...
    void XrRayInteractorComponent::register_type() {
        auto descriptor = ComponentTypeDescriptorBuilder::native<XrRayInteractorComponent>(
            "XrRayInteractorComponent", "termin-components-render", "CxxComponent");
        descriptor.category("Input").require("XrTrackedPoseComponent").require("LineRenderer");
        auto& inspect = descriptor.inspect();
        inspect.add<XrRayInteractorComponent, double>("XrRayInteractorComponent",
    ...
    void XrRayInteractorComponent::update(float) {
        ...
        if (!tracker) {
            if (!logged_missing_tracker_) {
                tc_log_error("[XrRayInteractorComponent] entity '%s' has no "
        ...
        if (!line) {
            if (!logged_missing_line_renderer_) {
                tc_log_error("[XrRayInteractorComponent] entity '%s' has no LineRenderer", entity().name());
        ...
        const bool pressed = select.pressed(std::clamp(select_threshold, 0.0, 1.0));
        const std::uint64_t pointer_id = entity().runtime_id();
        pointing_ = true;
        visible_ray_length_ = ray_limit;

        tc_component* captured = resolve_world_pointer_surface(captured_surface_.entity, captured_surface_.source_id);
        ...
        if (captured) {
            ...
            if (!tc_world_pointer_surface_project_ray(captured, &ray, &hit)) {
                dispatch_world_pointer(captured, pointer_id, TC_WORLD_POINTER_CANCEL, nullptr, 0.0f);
                captured_surface_.clear();
                hovered_surface_.clear();
            } else {
                visible_ray_length_ = hit.distance;
                dispatch_world_pointer(
                    captured, pointer_id, TC_WORLD_POINTER_MOVE, &hit, static_cast<float>(select.value));
                if (!pressed) {
                    dispatch_world_pointer(captured, pointer_id, TC_WORLD_POINTER_UP, &hit, 0.0f);
                    captured_surface_.clear();
                    if (!hit.inside)
                        hovered_surface_.clear();
                }
            ...
            nearest.ray = &ray;
            tc_scene_foreach_with_capability(entity().scene().handle(),
            ...
            tc_component* hovered = resolve_world_pointer_surface(hovered_surface_.entity, hovered_surface_.source_id);
            const bool same_hover =
                nearest.component && hovered &&
                same_world_pointer_surface(nearest.component, hovered_surface_.entity, hovered_surface_.source_id);
            if (hovered && !same_hover) {
                dispatch_world_pointer(hovered, pointer_id, TC_WORLD_POINTER_LEAVE, nullptr, 0.0f);
                hovered_surface_.clear();
            }

            if (nearest.component) {
                visible_ray_length_ = nearest.hit.distance;
                const char* source_id = tc_component_ensure_source_id(nearest.component);
                if (!source_id || source_id[0] == '\0') {
                    tc_log_error("[XrRayInteractorComponent] world-pointer surface on '%s' "
                    ...
                    dispatch_world_pointer(nearest.component,
                                           pointer_id,
        ...
        line->set_segment(tc_vec3{0.0, 0.0, 0.0}, tc_vec3{0.0, visible_ray_length_, 0.0});
        was_pressed_ = pressed;
    }

    void XrRayInteractorComponent::reset_interaction(bool cancel_capture, bool clear_runtime_visual) {
        const std::uint64_t pointer_id = entity().valid() ? entity().runtime_id() : 0;
        tc_component* captured = resolve_world_pointer_surface(captured_surface_.entity, captured_surface_.source_id);
        tc_component* hovered = resolve_world_pointer_surface(hovered_surface_.entity, hovered_surface_.source_id);
