...
namespace termin {
    ...
    namespace {
        ...
        static_assert(sizeof(DepthPerFrameStd140) == 144, "DepthPerFrameStd140 must be 144 bytes");
        ...
        static_assert(sizeof(DepthDrawStd140) == 64, "DepthDrawStd140 must be exactly one mat4");

        float depth_encoding_mode(const std::string& encoding) {
            if (encoding == "linear")
                return 0.0f;
            if (encoding == "linear_inverse")
                return 1.0f;
            if (encoding == "perspective")
                return 2.0f;
            if (encoding == "perspective_inverse")
                return 3.0f;
            if (encoding == "logarithmic")
                return 4.0f;
            if (encoding == "logarithmic_inverse")
                return 5.0f;
            tc::Log::warn("DepthPass: unknown depth_encoding '%s', using 'linear'", encoding.c_str());
            return 0.0f;
        }

        bool depth_encoding_is_inverse(const std::string& encoding) {
            return encoding == "linear_inverse" || encoding == "perspective_inverse" ||
        ...
        VertexOutputAdapter depth_vertex_output_adapter() {
            ...
            adapter.source_module = {"termin_depth_vertex_output_adapter",
        ...
        RenderItemTaskPlanningContract depth_only_task_planning_contract(
            tc_phase_mask phase, const MaterialPipelinePassContract& shader_contract, const char* debug_pass_name) {
        ...
        bool plan_depth_only_item_shader(const tc_render_item& item,
    ...
    tc_shader_handle DepthPass::shader_usage_base_shader() const {
        if (tc_shader_handle_is_invalid(depth_shader_handle_)) {
            depth_shader_handle_ = tgfx::register_builtin_shader_from_catalog(DEPTH_ENGINE_SHADER_UUID);
        }
    ...
    // ----------------------------------------------------------------------------
    void DepthPass::execute_with_data_tgfx2(ExecuteContext& ctx, const DepthPassExecuteData& data) {
        if (!ctx.ctx2) {
        ...
        tgfx::TextureHandle color_tex2 = (color_it != ctx.tex2_writes.end()) ? color_it->second : tgfx::TextureHandle{};
        if (color_tex2 && device.texture_desc(color_tex2).format == tgfx::PixelFormat::D32F) {
            color_tex2 = {};
        }
        if (!color_tex2 && !depth_tex2) {
            tc::Log::error("DepthPass/tgfx2: missing tgfx2 output texture for '%s'", output_res.c_str());
            return;
        }
        ...
        if (!ensure_material_pipeline_shader(*ctx.ctx2, device, depth_shader_handle_, "DepthPass", depth_shader)) {
            return;
        }
        ...
        for (const auto& dc : cached_draw_calls_) {
            ...
            tc_material_phase* material_phase = tc_shader_handle_eq(dc.final_shader.handle, depth_shader_handle_)
                                                    ? nullptr
    ...
    void DepthPass::execute(ExecuteContext& ctx) {
        const SceneRenderServices* services = require_scene_render_services(ctx, "DepthPass");
        if (!services) {
            return;
        }
    ...
    void DepthOnlyPass::ensure_tgfx2_resources(tgfx::IRenderDevice& device) {
        device2_ = &device;
        if (tc_shader_handle_is_invalid(depth_shader_handle_)) {
            depth_shader_handle_ = tgfx::register_builtin_shader_from_catalog(DEPTH_ONLY_ENGINE_SHADER_UUID);
        }
    }
    ...
    void DepthOnlyPass::collect_draw_calls(tc_scene_handle scene,
        ...
        for (size_t group_begin = 0; group_begin < items.size();) {
            ...
            while (group_end < items.size() &&
                   items[group_end].source.adapter_data == representative.source.adapter_data &&
            ...
            if (plan_depth_only_item_shader(item,
                                            selected_phase,
                                            original_shader,
                                            requested_phase,
                                            pass_contract,
    ...
    void DepthOnlyPass::collect_scene_shader_usages(tc_scene_handle scene,
        ...
        auto callback = [](tc_component* c, void* user_data) -> bool {
            ...
            for (const tc_render_item& item : items.items) {
                ...
                tc_shader_handle original_shader = collect_ctx->pass->depth_shader_handle_;
        ...
        UsageContext collect_ctx{this, &emit, depth_material_pass_contract("depth_only")};
    ...
    void DepthOnlyPass::execute(ExecuteContext& ctx) {
        ...
        const SceneRenderServices* services = require_scene_render_services(ctx, "DepthOnlyPass");
        if (!services) {
            return;
        }
        tc_scene_handle scene = services->scene.handle();
        ...
        if (!depth_tex2) {
            ...
            depth_tex2 = (texture_it != ctx.tex2_writes.end()) ? texture_it->second : tgfx::TextureHandle{};
        }
        if (!depth_tex2) {
            tc::Log::error("DepthOnlyPass/tgfx2: missing tgfx2 depth texture for '%s'", output_res.c_str());
            return;
        }
        ...
        auto& device = ctx.ctx2->device();
        ensure_tgfx2_resources(device);
        const RenderItemSnapshot* snapshot = require_render_item_snapshot(ctx, "DepthOnlyPass");
        if (!snapshot) {
            return;
        }
        ...
        MaterialPipelineShaderBinding depth_shader{};
        if (!ensure_material_pipeline_shader(*ctx.ctx2, device, depth_shader_handle_, "DepthOnlyPass", depth_shader)) {
            return;
        }
        ...
        auto capture_debug_symbol = [&](const char* entity_name) {
            ...
            ctx.ctx2->end_pass();
            ctx.capture_internal(entity_name, depth_tex2, rect.width, rect.height);
    ...
    void DepthToColorPass::ensure_tgfx2_resources(tgfx::IRenderDevice& device) {
        ...
        if (tc_shader_handle_is_invalid(shader_handle_)) {
            shader_handle_ = tgfx::register_builtin_shader_from_catalog(DEPTH_TO_COLOR_ENGINE_SHADER_UUID);
        }
    }
    ...
    void DepthToColorPass::execute(ExecuteContext& ctx) {
        ...
        auto color_it = ctx.tex2_writes.find(output_res);
        tgfx::TextureHandle color_tex = (color_it != ctx.tex2_writes.end()) ? color_it->second : tgfx::TextureHandle{};
        if (!color_tex) {
    ...
    void ColorToDepthPass::ensure_tgfx2_resources(tgfx::IRenderDevice& device) {
        device2_ = &device;
        if (tc_shader_handle_is_invalid(shader_handle_)) {
            shader_handle_ = tgfx::register_builtin_shader_from_catalog(COLOR_TO_DEPTH_ENGINE_SHADER_UUID);
        }
    }
    ...
    void ColorToDepthPass::execute(ExecuteContext& ctx) {
        ...
        tgfx::TextureHandle color_tex = (color_it != ctx.tex2_reads.end()) ? color_it->second : tgfx::TextureHandle{};
        if (!color_tex) {
    ...
    void DepthOnlyPass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<DepthOnlyPass>("DepthOnlyPass", "termin-components-render");
        auto& inspect = descriptor.inspect();
    ...
    void DepthToColorPass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<DepthToColorPass>("DepthToColorPass", "termin-components-render");
        auto& inspect = descriptor.inspect();
    ...
    void ColorToDepthPass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<ColorToDepthPass>("ColorToDepthPass", "termin-components-render");
        auto& inspect = descriptor.inspect();
        _register_inspect_input_res(inspect);
        _register_inspect_output_res(inspect);
        _register_inspect_output_res_target(inspect);
