    ...
    void*)
{
    ...
    const float vertices[] = {
        quad.min_x, 0.0f, quad.min_z,
        quad.min_x, 0.0f, quad.max_z,
        quad.max_x, 0.0f, quad.max_z,
        quad.min_x, 0.0f, quad.min_z,
        quad.max_x, 0.0f, quad.max_z,
        quad.max_x, 0.0f, quad.min_z,
    };
...
void SpriteRenderer2D::register_type() {
    ...
    inspect.add_with_callbacks<SpriteRenderer2D, std::string>(
        "SpriteRenderer2D", "sprite", "Sprite", "sprite_asset",
    ...
    inspect.add_with_callbacks<SpriteRenderer2D, Vec4>(
        "SpriteRenderer2D", "tint", "Tint", "color",
...
void SpriteRenderer2D::set_tint(const Vec4& value) { tint = value; }
void SpriteRenderer2D::set_flip_x(bool value) { flip_x = value; }
void SpriteRenderer2D::set_flip_y(bool value) { flip_y = value; }
void SpriteRenderer2D::set_sorting_layer(int value) { sorting_layer = value; }
void SpriteRenderer2D::set_order_in_layer(int value) { order_in_layer = value; }
void SpriteRenderer2D::set_visible(bool value) { visible = value; }
...
bool SpriteRenderer2D::resolved_quad(
    TcSpriteAsset& out_asset,
    ...
    float& out_v1) const
{
    ...
    if (!asset || !asset->loaded) {
        if (last_missing_asset_version_ != 0) {
            tc::Log::error(
                "[SpriteRenderer2D] sprite asset is missing or unloaded: uuid='%s'",
                sprite_uuid.c_str());
...
tc_phase_mask SpriteRenderer2D::get_phase_mask() const {
    return visible && !sprite_uuid.empty()
        ? (TC_PHASE_TRANSPARENT | TC_PHASE_ID)
        : TC_PHASE_NONE;
}

bool SpriteRenderer2D::collect_render_items(
    ...
    tc_render_item_sink& sink)
{
    ...
    if (!texture.is_valid()) {
        tc::Log::error(
            "[SpriteRenderer2D] texture is not declared: sprite='%s' texture='%s'",
            sprite_uuid.c_str(), asset->texture_uuid.c_str());
        return true;
    }
    ...
    quad.sampling = asset->sampling == SpriteSampling::Nearest
        ? TC_WORLD_QUAD_SAMPLING_NEAREST
        : TC_WORLD_QUAD_SAMPLING_LINEAR;
...
bool SpriteRenderer2D::ray_intersects(
    ...
    double* out_distance) const
{
    ...
    return ray_intersects_world2d_quad(
        ...
        ray_direction,
        rect,
        to_double_matrix(get_model_matrix(entity())),
        out_distance);
}
