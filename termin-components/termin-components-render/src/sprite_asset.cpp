...
namespace termin {
    ...
    bool TcSpriteAsset::update(const std::string& texture_uuid,
                               SpriteRegion region,
        ...
        if (!valid_region(region)) {
            tc_log_error("[SpriteAsset] invalid region (%d, %d, %d, %d) for sprite '%s'",
                         region.x,
                         region.y,
                         region.width,
                         region.height,
        ...
        if (source_width <= 0 || source_height <= 0 || region.x + region.width > source_width ||
            region.y + region.height > source_height) {
            tc_log_error("[SpriteAsset] region exceeds source size %dx%d for sprite '%s'",
                         source_width,
                         source_height,
                         ptr->uuid.c_str());
            return false;
        }
        if (!std::isfinite(pivot_x) || !std::isfinite(pivot_y) || pivot_x < 0.0f || pivot_x > 1.0f || pivot_y < 0.0f ||
            pivot_y > 1.0f) {
        ...
        if (!std::isfinite(pixels_per_unit) || pixels_per_unit <= 0.0f) {
            tc_log_error("[SpriteAsset] pixels_per_unit must be positive for sprite '%s'", ptr->uuid.c_str());
            return false;
        }
    ...
    TcSpriteAsset
    TcSpriteAsset::declare(const std::string& uuid, const std::string& name, const std::string& source_path) {
        if (uuid.empty()) {
