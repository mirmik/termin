...
namespace termin {
    namespace {
        ...
        tc_widget* find_named_widget(tc_widget* widget, std::string_view name) {
            ...
            for (std::size_t index = 0; index < child_count; ++index) {
                if (tc_widget* found = find_named_widget(tc_widget_child_at(widget, index), name)) {
                    return found;
                }
            }
            return nullptr;
        ...
        tc_widget* find_named_widget(gui_native::TcDocument document, std::string_view name) {
            const std::size_t root_count = tc_ui_document_root_count(document.handle());
            for (std::size_t index = 0; index < root_count; ++index) {
                tc_widget* root =
                    tc_ui_document_resolve_widget(document.handle(), tc_ui_document_root_at(document.handle(), index));
        ...
        gui_native::Label* find_label(gui_native::TcDocument document, std::string_view name) {
            tc_widget* widget = find_named_widget(document, name);
            if (widget == nullptr || widget->native_language != TC_LANGUAGE_CXX || widget->body == nullptr) {
                return nullptr;
            }
            return dynamic_cast<gui_native::Label*>(static_cast<gui_native::Widget*>(widget->body));
        }

        gui_native::Checkbox* find_checkbox(gui_native::TcDocument document, std::string_view name) {
            tc_widget* widget = find_named_widget(document, name);
            if (widget == nullptr || widget->native_language != TC_LANGUAGE_CXX || widget->body == nullptr) {
                return nullptr;
            }
            return dynamic_cast<gui_native::Checkbox*>(static_cast<gui_native::Widget*>(widget->body));
        }

        std::size_t set_descendant_servos_enabled(Entity entity, bool enabled) {
            std::size_t count = 0;
            if (FEMJointServoComponent* servo = entity.get_component<FEMJointServoComponent>()) {
...
#ifdef TERMIN_PHYSICS_FEM_UI_HAS_TCPLOT
        tcplot::gui_native::Plot2D* find_plot(gui_native::TcDocument document, std::string_view name) {
            tc_widget* widget = find_named_widget(document, name);
            if (widget == nullptr || widget->native_language != TC_LANGUAGE_CXX || widget->body == nullptr) {
                return nullptr;
            }
            return dynamic_cast<tcplot::gui_native::Plot2D*>(static_cast<gui_native::Widget*>(widget->body));
        }
#endif
        ...
        std::string format_energy_change(double current, double initial) {
            ...
            const double relative = 100.0 * change / std::max(1.0, std::abs(initial));
            std::ostringstream stream;
            stream << std::showpos << std::fixed << std::setprecision(3) << change << " J  (" << std::setprecision(4)
                   << relative << " %)";
            return stream.str();
        }
        ...
        std::string format_simulation(const FEMPhysicsTelemetry& telemetry) {
            std::ostringstream stream;
            stream << std::fixed << std::setprecision(2) << telemetry.simulated_time << " s   ·   "
        ...
        std::string format_topology(const FEMPhysicsTelemetry& telemetry) {
            std::ostringstream stream;
            stream << "QP multibody   ·   " << telemetry.body_count << " bodies   ·   " << telemetry.joint_count
                   << " constraints   ·   " << telemetry.articulation_count << " articulations / "
                   << telemetry.reduced_dof_count << " reduced DOF";
            return stream.str();
        }

        std::string format_motors(const FEMPhysicsTelemetry& telemetry) {
            std::ostringstream stream;
            stream << telemetry.motor_count << " motors   ·   " << std::fixed << std::setprecision(2)
                   << telemetry.motor_effort_linf << " N·m/N max   ·   " << telemetry.motor_power << " W   ·   "
                   << telemetry.motor_work << " J";
            return stream.str();
        }

        std::string format_contacts(const FEMPhysicsTelemetry& telemetry) {
            std::ostringstream stream;
            stream << telemetry.contact_count << " contacts   ·   " << telemetry.active_contact_count
                   << " active   ·   " << telemetry.cached_contact_count << " cached / "
                   << telemetry.warm_started_contact_count << " warm   ·   gap " << std::fixed << std::setprecision(3)
                   << telemetry.minimum_contact_gap * 1000.0 << " mm   ·   impulse " << telemetry.normal_impulse_sum
                   << " N·s   ·   reaction " << telemetry.normal_reaction_sum << " N";
            return stream.str();
        }

        std::string format_friction(const FEMPhysicsTelemetry& telemetry) {
            std::ostringstream stream;
            stream << telemetry.sliding_contact_count << " sliding   ·   " << std::fixed << std::setprecision(4)
                   << telemetry.tangent_speed_linf << " m/s tangent   ·   " << telemetry.tangent_impulse_sum
                   << " N·s   ·   capacity " << telemetry.friction_capacity_sum << " N·s   ·   work "
                   << telemetry.friction_work << " J";
            return stream.str();
        }

        std::string format_body(Entity body_entity, const FEMRigidBodyComponent& body) {
            if (!body.initialized()) {
            ...
            std::ostringstream stream;
            stream << std::fixed << std::setprecision(3) << "base xyz  " << position.x << "  " << position.y << "  "
                   << position.z << " m   ·   |v| " << velocity.lin.norm() << " m/s   ·   |ω| " << velocity.ang.norm()
                   << " rad/s";
            return stream.str();
        }

        std::string format_servo(const FEMJointServoComponent& servo, const FEMArticulationMotorComponent& motor) {
            ...
            std::ostringstream stream;
            stream << std::fixed << std::setprecision(2) << "q " << coordinate << " / " << servo.target_coordinate
                   << "   ·   error " << std::showpos << error << std::noshowpos << "   ·   I "
                   << servo.integral_effort() << "   ·   effort " << motor.applied_effort() << " / "
                   << motor.maximum_effort << "   ·   " << motor.power() << " W   ·   "
                   << (motor.saturated() ? "SATURATED" : "tracking");
            return stream.str();
        }

        bool set_label(gui_native::TcDocument document, std::string_view name, std::string text) {
    ...
    void FEMPhysicsHudComponent::register_type() {
        auto descriptor = ComponentTypeDescriptorBuilder::native<FEMPhysicsHudComponent>(
    ...
    void FEMPhysicsHudComponent::update(float dt) {
        ...
        if (!std::isfinite(dt) || dt < 0.0f) {
            tc::Log::error("[FEMPhysicsHudComponent] invalid frame dt=%g", static_cast<double>(dt));
            return;
        }
    ...
    void FEMPhysicsHudComponent::refresh() {
        ...
        UIComponent* ui = owner.valid() ? owner.get_component<UIComponent>() : nullptr;
        Entity world_entity = scene.valid() ? scene.find_entity_by_name(world_entity_name) : Entity{};
        FEMPhysicsWorldComponent* world =
            world_entity.valid() ? world_entity.get_component<FEMPhysicsWorldComponent>() : nullptr;
        const gui_native::TcDocument document = ui != nullptr ? ui->document() : gui_native::TcDocument{};

        if (world == nullptr || !document.valid()) {
            if (!binding_error_reported_) {
                tc::Log::error("[FEMPhysicsHudComponent] requires a UIComponent on its "
        ...
        if (!servo_group_root_entity_name.empty()) {
            Entity group_root = scene.find_entity_by_name(servo_group_root_entity_name);
            gui_native::Checkbox* checkbox = find_checkbox(document, servo_group_checkbox_name);
            if (!group_root.valid() || checkbox == nullptr) {
                if (!binding_error_reported_) {
                    tc::Log::error("[FEMPhysicsHudComponent] servo group requires entity "
                                   "'%s' and Checkbox '%s'",
            ...
            if (!servo_group_state_known_ || enabled != servo_group_enabled_) {
                const std::size_t changed = set_descendant_servos_enabled(group_root, enabled);
                if (changed == 0U) {
                    tc::Log::error("[FEMPhysicsHudComponent] servo group entity '%s' "
        ...
        const bool complete =
            set_label(document,
                      "energy_value",
                      telemetry.initialized ? format_energy(telemetry.total_energy) : "waiting for solver") &&
            set_label(document,
                      "energy_change",
                      telemetry.initialized
                          ? format_energy_change(telemetry.total_energy, telemetry.initial_total_energy)
                          : "Δ from start") &&
            set_label(document, "simulation_value", format_simulation(telemetry)) &&
        ...
        if (find_label(document, "contact_value") != nullptr) {
            (void)set_label(document, "contact_value", format_contacts(telemetry));
        }
        if (find_label(document, "friction_value") != nullptr) {
            (void)set_label(document, "friction_value", format_friction(telemetry));
        }
        if (!tracked_body_entity_name.empty()) {
            Entity body_entity = scene.find_entity_by_name(tracked_body_entity_name);
            FEMRigidBodyComponent* body =
                body_entity.valid() ? body_entity.get_component<FEMRigidBodyComponent>() : nullptr;
            if (body == nullptr) {
                if (!binding_error_reported_) {
                    tc::Log::error("[FEMPhysicsHudComponent] entity '%s' has no "
            ...
            if (find_label(document, "body_value") != nullptr) {
                (void)set_label(document, "body_value", format_body(body_entity, *body));
            }
        }
#ifdef TERMIN_PHYSICS_FEM_UI_HAS_TCPLOT
        tcplot::gui_native::Plot2D* contact_gap_plot = find_plot(document, contact_gap_plot_widget_name);
        tcplot::gui_native::Plot2D* contact_reaction_plot = find_plot(document, contact_reaction_plot_widget_name);
        if ((contact_gap_plot != nullptr || contact_reaction_plot != nullptr) && telemetry.initialized) {
            if (!contact_plot_time_.empty() && telemetry.simulated_time < contact_plot_time_.back()) {
                contact_plot_time_.clear();
                contact_plot_gap_.clear();
                contact_plot_reaction_.clear();
            }
            ...
            const double first_time = telemetry.simulated_time - std::max(plot_history, 1.0);
            const auto first = std::lower_bound(contact_plot_time_.begin(), contact_plot_time_.end(), first_time);
            const std::size_t erase_count = static_cast<std::size_t>(first - contact_plot_time_.begin());
            if (erase_count > 0) {
                contact_plot_time_.erase(contact_plot_time_.begin(), first);
                contact_plot_gap_.erase(contact_plot_gap_.begin(), contact_plot_gap_.begin() + erase_count);
                contact_plot_reaction_.erase(contact_plot_reaction_.begin(),
                                             contact_plot_reaction_.begin() + erase_count);
            }
            if (contact_gap_plot != nullptr) {
                ...
                (void)contact_gap_plot->set_line_data(0, contact_plot_time_, contact_plot_gap_);
            }
            if (contact_reaction_plot != nullptr) {
                ...
                (void)contact_reaction_plot->set_line_data(0, contact_plot_time_, contact_plot_reaction_);
            }
        }
#endif
        if (!servo_entity_name.empty()) {
            ...
            FEMJointServoComponent* servo =
                servo_entity.valid() ? servo_entity.get_component<FEMJointServoComponent>() : nullptr;
            FEMArticulationMotorComponent* motor =
                servo_entity.valid() ? servo_entity.get_component<FEMArticulationMotorComponent>() : nullptr;
            ...
            if (find_label(document, "servo_value") != nullptr) {
                (void)set_label(document, "servo_value", format_servo(*servo, *motor));
            }
#ifdef TERMIN_PHYSICS_FEM_UI_HAS_TCPLOT
            tcplot::gui_native::Plot2D* coordinate_plot = find_plot(document, plot_widget_name);
            tcplot::gui_native::Plot2D* effort_plot = find_plot(document, effort_plot_widget_name);
            if ((coordinate_plot != nullptr || effort_plot != nullptr) && telemetry.initialized &&
                servo->initialized()) {
                if (!plot_time_.empty() && telemetry.simulated_time < plot_time_.back()) {
                    plot_time_.clear();
                    plot_coordinate_.clear();
                    plot_target_.clear();
                    plot_position_effort_.clear();
                ...
                const double coordinate = servo->target_coordinate - servo->position_error();
                ...
                const double first_time = telemetry.simulated_time - std::max(plot_history, 1.0);
                const auto first = std::lower_bound(plot_time_.begin(), plot_time_.end(), first_time);
                const std::size_t erase_count = static_cast<std::size_t>(first - plot_time_.begin());
                if (erase_count > 0) {
                    plot_time_.erase(plot_time_.begin(), first);
                    plot_coordinate_.erase(plot_coordinate_.begin(), plot_coordinate_.begin() + erase_count);
                    plot_target_.erase(plot_target_.begin(), plot_target_.begin() + erase_count);
                    ...
                    plot_commanded_effort_.erase(plot_commanded_effort_.begin(),
                ...
                if (coordinate_plot != nullptr) {
                    ...
                    (void)coordinate_plot->set_line_data(0, plot_time_, plot_coordinate_);
                    (void)coordinate_plot->set_line_data(1, plot_time_, plot_target_);
                }
                if (effort_plot != nullptr) {
                    ...
                    (void)effort_plot->set_line_data(0, plot_time_, plot_position_effort_);
                    (void)effort_plot->set_line_data(1, plot_time_, plot_integral_effort_);
                    (void)effort_plot->set_line_data(2, plot_time_, plot_velocity_effort_);
                    (void)effort_plot->set_line_data(3, plot_time_, plot_commanded_effort_);
                    (void)effort_plot->set_line_data(4, plot_time_, plot_applied_effort_);
                }
            }
#endif
        }
        if (!complete && !binding_error_reported_) {
            tc::Log::error("[FEMPhysicsHudComponent] HUD layout does not contain all "
