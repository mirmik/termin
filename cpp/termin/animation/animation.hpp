...
struct Keyframe {
    ...
    Keyframe(double t, geom::Vec3 tr) : time(t), translation(tr) {}
    Keyframe(double t, geom::Quat rot) : time(t), rotation(rot) {}
    Keyframe(double t, double sc) : time(t), scale(sc) {}
};
...
public:
    ...
    Sample sample(double t_ticks) const {
        Sample result;
...
private:
    geom::Vec3 sample_translation(double t) const {
        return sample_keys_linear<geom::Vec3>(
            translation_keys, t,
            [](const Keyframe& k) { return *k.translation; },
    ...
    geom::Quat sample_rotation(double t) const {
        return sample_keys_linear<geom::Quat>(
            rotation_keys, t,
            [](const Keyframe& k) { return *k.rotation; },
    ...
    double sample_scale(double t) const {
        return sample_keys_linear<double>(
            scale_keys, t,
            [](const Keyframe& k) { return *k.scale; },
    ...
    T sample_keys_linear(const std::vector<Keyframe>& keys,
                         double t,
                         GetValue get_value,
                         Interpolate interpolate) const
    {
        ...
        const Keyframe& k1 = keys[lo];
        const Keyframe& k2 = keys[hi];
...
public:
     ...
     * Returns map: channel_name -> Sample
     */
    std::unordered_map<std::string, Channel::Sample> sample(double t_seconds) const {
        ...
        std::unordered_map<std::string, Channel::Sample> result;
