...
CameraComponent2::CameraComponent2() {
    set_type_name("CameraComponent2");
}

std::string CameraComponent2::get_projection_type_str() const {
    return projection_type == CameraProjection::Perspective ? "perspective" : "orthographic";
}

void CameraComponent2::set_projection_type_str(const std::string& type) {
...
double CameraComponent2::get_fov_degrees() const {
    return fov_y * 180.0 / M_PI;
}

void CameraComponent2::set_fov_degrees(double deg) {
    fov_y = deg * M_PI / 180.0;
}

void CameraComponent2::set_aspect(double a) {
    aspect = a;
}

Mat44 CameraComponent2::get_view_matrix() const {
    if (!entity.valid()) {
...
Mat44 CameraComponent2::get_projection_matrix() const {
...
Vec3 CameraComponent2::get_position() const {
    if (!entity.valid()) {
        return Vec3::zero();
    }
...
void CameraComponent2::add_viewport(const TcViewport& vp) {
...
void CameraComponent2::remove_viewport(const TcViewport& vp) {
    viewports_.erase(
...
bool CameraComponent2::has_viewport(const TcViewport& vp) const {
...
size_t CameraComponent2::viewport_count() const {
    return viewports_.size();
}

TcViewport CameraComponent2::viewport_at(size_t index) const {
    return index < viewports_.size() ? viewports_[index] : TcViewport();
}

void CameraComponent2::clear_viewports() {
    viewports_.clear();
}

void CameraComponent2::on_scene_inactive() {
    clear_viewports();
}

std::pair<Vec3, Vec3> CameraComponent2::screen_point_to_ray(double x, double y, int vp_x, int vp_y, int vp_w, int vp_h) const {
...
REGISTER_COMPONENT(CameraComponent2, CxxComponent);
