...
namespace termin::runtime::detail {
    ...
    RuntimeMaterialUboLayout compute_runtime_std140_layout(const std::vector<RuntimeMaterialUboEntry>& declarations) {
    ...
    void set_shader_material_ubo_layout_from_glsl(tc_shader* shader, const std::string& fragment_source) {
        ...
        for (const RuntimeMaterialUboEntry& src : layout.entries) {
            ...
            std::snprintf(dst.property_type, sizeof(dst.property_type), "%s", src.property_type.c_str());
        ...
        tc_log_info("RuntimePackageLoader: inferred material UBO layout for shader '%s' entries=%zu size=%u",
    ...
    void set_shader_features_from_glsl(tc_shader* shader, const std::string& fragment_source) {
        ...
        if (fragment_source.find("uniform LightingBlock") != std::string::npos) {
            ...
            tc_log_info("RuntimePackageLoader: inferred lighting UBO feature for shader '%s'",
                        shader->name ? shader->name : shader->uuid);
        }
    }
} // namespace termin::runtime::detail
