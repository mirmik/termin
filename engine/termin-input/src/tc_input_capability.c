...
bool tc_input_capability_attach(tc_component* c, const tc_input_vtable* vtable) {
    ...
    tc_input_capability_payload* existing = (tc_input_capability_payload*)tc_component_get_capability(c, cap_id);
    if (existing) {
        existing->vtable = vtable;
        return true;
    }

    tc_input_capability_payload* payload = (tc_input_capability_payload*)calloc(1, sizeof(tc_input_capability_payload));
    if (!payload) {
