        ...
        nb::class_<TcViewport>(m, "Viewport")
            ...
            .def(
                ...
                [](TcViewport* self,
                   ...
                   std::tuple<int, int, int, int> pixel_rect) {
                    ...
                    // Set rect
                    tc_viewport_set_rect(
                        vh, std::get<0>(rect), std::get<1>(rect), std::get<2>(rect), std::get<3>(rect));
                    tc_viewport_set_pixel_rect(vh,
                                               std::get<0>(pixel_rect),
                                               std::get<1>(pixel_rect),
                                               std::get<2>(pixel_rect),
                                               std::get<3>(pixel_rect));
            ...
            .def_prop_rw(
                ...
                [](TcViewport& self, const std::string& n) {
                    if (self.is_valid())
                        tc_viewport_set_name(self.handle_, n.c_str());
                })
            ...
            .def_prop_rw(
                ...
                [](TcViewport& self, std::tuple<float, float, float, float> r) {
                    self.set_rect(std::get<0>(r), std::get<1>(r), std::get<2>(r), std::get<3>(r));
                })
            ...
            .def_prop_rw(
                ...
                [](TcViewport& self, std::tuple<int, int, int, int> r) {
                    self.set_pixel_rect(std::get<0>(r), std::get<1>(r), std::get<2>(r), std::get<3>(r));
                })

            .def("update_pixel_rect",
                 &TcViewport::update_pixel_rect,
                 nb::arg("display_width"),
                 nb::arg("display_height"))
            ...
            .def_prop_rw(
                ...
                [](TcViewport& self, bool value) {
                    if (self.is_valid())
                        tc_viewport_set_override_resolution(self.handle_, value);
                })
            ...
            .def_prop_rw(
                ...
                [](TcViewport& self, const std::string& m) {
                    if (self.is_valid())
                        tc_viewport_set_input_mode(self.handle_, m.c_str());
                })

            // Block input in editor
            .def_prop_rw(
                "block_input_in_editor",
                [](TcViewport& self) {
                    return self.is_valid() ? tc_viewport_get_block_input_in_editor(self.handle_) : false;
                },
                [](TcViewport& self, bool b) {
                    if (self.is_valid())
                        tc_viewport_set_block_input_in_editor(self.handle_, b);
                })
            ...
            .def_prop_rw(
                ...
                [](TcViewport& self) -> nb::object {
                    if (!self.is_valid())
                        return nb::none();
                    if (!tc_viewport_has_internal_entities(self.handle_))
                        return nb::none();
            ...
            .def_prop_ro("effective_layer_mask",
                         [](TcViewport& self) -> uint64_t {
                             tc_render_target_handle rt = tc_viewport_get_render_target(self.handle_);
            ...
            .def(
                "screen_point_to_ray",
                [](TcViewport& self, float x, float y) -> nb::object {
                    tc_render_target_handle rt = tc_viewport_get_render_target(self.handle_);
                    tc_component* cam = tc_render_target_get_camera(rt);
                    nb::object camera_obj = camera_component_from_tc(cam);
                    if (camera_obj.is_none()) {
