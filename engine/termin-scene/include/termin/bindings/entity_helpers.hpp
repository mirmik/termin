...
namespace termin {
    ...
    inline void py_component_ref_retain(tc_component* c) {
        if (c && c->body)
            Py_INCREF(reinterpret_cast<PyObject*>(c->body));
    }

    inline void py_component_ref_release(tc_component* c) {
        if (c && c->body)
            Py_DECREF(reinterpret_cast<PyObject*>(c->body));
    }
    ...
    template <typename T, typename... Args> void cxx_component_init(nb::handle self, Args&&... args) {
    ...
    inline nb::object component_to_python(CxxComponent* cxx) {
        ...
        if (type_name && result_type.find(type_name) == std::string::npos) {
            throw std::runtime_error(std::string("C++ component '") + type_name +
