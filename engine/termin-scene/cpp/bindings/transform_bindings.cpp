...
namespace termin {
    ...
    void bind_transform(nb::module_& m) {
        ...
        nb::class_<GeneralTransform3>(m, "GeneralTransform3")
            ...
            .def(
                "__init__",
                [](GeneralTransform3* self, const GeneralPose3& pose) {
                    tc_entity_pool_handle pool_handle = get_standalone_pool_handle();
                    tc_entity_pool* pool = tc_entity_pool_registry_get(pool_handle);
                    tc_entity_id id = tc_entity_pool_alloc(pool, "transform");
                    new (self) GeneralTransform3(pool_handle, id);
            ...
            .def_prop_ro("name",
                         [](const GeneralTransform3& self) -> nb::object {
            ...
            .def_prop_ro("children",
                         [](const GeneralTransform3& self) -> nb::list {
                             nb::list result;
            ...
            .def("set_local_pose", [](GeneralTransform3& self, const GeneralPose3& pose) { self.set_local_pose(pose); })
            ...
            .def(
                "set_local_position",
                [](GeneralTransform3& self, const Vec3& pos) { self.set_local_position(pos); },
                nb::arg("position"))
            .def(
                "set_local_rotation",
                [](GeneralTransform3& self, const Quat& rot) { self.set_local_rotation(rot); },
                nb::arg("rotation"))
            .def(
                "set_local_scale",
                [](GeneralTransform3& self, const Vec3& scale) { self.set_local_scale(scale); },
                nb::arg("scale"))
            .def(
                "set_local_scale",
                [](GeneralTransform3& self, float x, float y, float z) { self.set_local_scale(Vec3(x, y, z)); },
                nb::arg("x"),
                nb::arg("y"),
                nb::arg("z"))
            .def("set_global_position", &GeneralTransform3::set_global_position, nb::arg("position"))
            .def("set_global_orientation", &GeneralTransform3::set_global_orientation, nb::arg("orientation"))
            .def("set_global_pose", &GeneralTransform3::set_global_pose, nb::arg("pose"))
            ...
            .def("relocate", [](GeneralTransform3& self, const Pose3& pose) { self.relocate(pose); })
            .def("relocate", [](GeneralTransform3& self, const GeneralPose3& pose) { self.relocate(pose); })
            .def("add_child", [](GeneralTransform3& self, GeneralTransform3 child) { child.set_parent(self); })
            .def(
                "set_parent",
                [](GeneralTransform3& self, nb::object parent) {
                    if (parent.is_none()) {
                        self.unparent();
                    } else {
            ...
            .def(
                ...
                [](GeneralTransform3& self, nb::object parent, double tolerance) {
                    const GeneralTransform3 parent_transform =
                        parent.is_none() ? GeneralTransform3{} : nb::cast<GeneralTransform3>(parent);
                    return self.try_reparent_preserve_world(parent_transform, tolerance);
                },
                nb::arg("parent").none(),
            ...
            .def(
                ...
                [](GeneralTransform3& self, nb::object parent, double tolerance) {
                    const GeneralTransform3 parent_transform =
                        parent.is_none() ? GeneralTransform3{} : nb::cast<GeneralTransform3>(parent);
                    if (!self.try_reparent_preserve_world(parent_transform, tolerance)) {
                        throw std::runtime_error("Cannot preserve the exact world transform while reparenting");
                    }
                },
                nb::arg("parent").none(),
                nb::arg("tolerance") = 1.0e-10)
            ...
            .def("link", [](GeneralTransform3& self, GeneralTransform3 child) { child.set_parent(self); })
            .def("transform_point",
                 [](const GeneralTransform3& self, const Vec3& point) { return self.transform_point(point); })
            .def("transform_point_inverse",
                 [](const GeneralTransform3& self, const Vec3& point) { return self.transform_point_inverse(point); })
            .def("transform_vector",
                 [](const GeneralTransform3& self, const Vec3& vec) { return self.transform_vector(vec); })
            .def("transform_vector_inverse",
                 [](const GeneralTransform3& self, const Vec3& vec) { return self.transform_vector_inverse(vec); })
            .def("transform_normal",
                 [](const GeneralTransform3& self, const Vec3& normal) { return self.transform_normal(normal); })
            .def("transform_direction",
                 [](const GeneralTransform3& self, const Vec3& vec) { return self.transform_direction(vec); })
            .def("transform_direction_inverse",
                 [](const GeneralTransform3& self, const Vec3& vec) { return self.transform_direction_inverse(vec); })
            .def(
                "forward",
                [](const GeneralTransform3& self, double distance) { return self.forward(distance); },
                nb::arg("distance") = 1.0)
            .def(
                "backward",
                [](const GeneralTransform3& self, double distance) { return self.backward(distance); },
                nb::arg("distance") = 1.0)
            .def(
                "up",
                [](const GeneralTransform3& self, double distance) { return self.up(distance); },
                nb::arg("distance") = 1.0)
            .def(
                "down",
                [](const GeneralTransform3& self, double distance) { return self.down(distance); },
                nb::arg("distance") = 1.0)
            .def(
                "right",
                [](const GeneralTransform3& self, double distance) { return self.right(distance); },
                nb::arg("distance") = 1.0)
            .def(
                "left",
                [](const GeneralTransform3& self, double distance) { return self.left(distance); },
                nb::arg("distance") = 1.0)
            .def("world_matrix",
                 [](const GeneralTransform3& self) {
                     double data[16];
                     double m44[16];
            ...
            .def(
                ...
                [](const GeneralTransform3& self, double epsilon) {
