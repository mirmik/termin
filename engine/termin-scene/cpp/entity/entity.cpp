...
namespace termin {
    ...
    namespace {
        ...
        std::vector<size_t> component_deserialization_order(const nos::trent& components_data) {
            ...
            std::function<void(size_t)> visit = [&](size_t index) {
                ...
                const size_t requirement_count =
                    type_name.empty() ? 0 : tc_component_registry_requirement_count(type_name.c_str());

                for (size_t requirement_index = 0; requirement_index < requirement_count; ++requirement_index) {
                    const char* required_type =
                        tc_component_registry_requirement_at(type_name.c_str(), requirement_index);
                    if (!required_type) {
                        continue;
                    }

                    for (size_t candidate_index = 0; candidate_index < components.size(); ++candidate_index) {
                        if (candidate_index == index) {
                            continue;
                        }
                        const std::string candidate_type = component_type(candidate_index);
                        if (!candidate_type.empty() &&
                            tc_component_registry_is_a(candidate_type.c_str(), required_type)) {
        ...
        nos::trent serialize_entity_base(const Entity& entity, const std::string& uuid, const std::string& name) {
            nos::trent data;
            data["uuid"] = uuid;
            data["name"] = name;
        ...
        nos::trent make_clone_payload_recursive(const nos::trent& source_data,
            ...
            if (clone_data.contains("children") && clone_data["children"].is_list()) {
                for (const nos::trent& child : clone_data["children"].as_list()) {
                    nos::trent cloned_child = make_clone_payload_recursive(child, root_name_suffix, false, uuid_remap);
        ...
        bool remap_entity_ref_value(nos::trent& value, const std::unordered_map<std::string, std::string>& uuid_remap) {
            if (value.is_dict()) {
        ...
        bool remap_entity_ref_list_value(nos::trent& value,
        ...
        void remap_component_entity_refs(nos::trent& data,
                                         const std::unordered_map<std::string, std::string>& uuid_remap) {
            if (!data.is_dict()) {
                return;
            }
            ...
            if (data.contains("components") && data["components"].is_list()) {
                for (nos::trent& component_data : data["components"].as_list()) {
                    if (!component_data.is_dict() || !component_data.contains("type") ||
                        !component_data.contains("data") || !component_data["data"].is_dict()) {
                        continue;
                    }
        ...
        bool deserialize_entity_hierarchy(const nos::trent& data,
            ...
            if (data.contains("children") && data["children"].is_list()) {
                for (const nos::trent& child_data : data["children"].as_list()) {
                    Entity child;
                    if (!deserialize_entity_hierarchy(child_data, scene, &entity, entity_data_pairs, child)) {
                        return false;
                    }
                }
            }
        ...
        void rollback_deserialized_entities(std::vector<std::pair<Entity, nos::trent>>& entity_data_pairs) {
    ...
    Entity
    Entity::create_with_uuid(tc_entity_pool_handle pool_handle, const std::string& name, const std::string& uuid) {
        tc_entity_pool* pool = tc_entity_pool_registry_get(pool_handle);
        if (!pool)
            return Entity();
    ...
    size_t Entity::sibling_index() const {
        return valid() ? tc_entity_sibling_index(_h) : SIZE_MAX;
    }
    ...
    nos::trent Entity::make_clone_payload(const nos::trent& data,
    ...
    void Entity::remap_entity_refs(nos::trent& data, const std::unordered_map<std::string, std::string>& uuid_remap) {
        remap_component_entity_refs(data, uuid_remap);
    }

    Entity Entity::deserialize_hierarchy(const nos::trent& data, tc_scene_handle scene, const Entity& parent) {
        if (!data.is_dict()) {
            tc::Log::error("[Entity] hierarchy deserialization failed: data must be a dict");
            return Entity();
        }
        tc_entity_pool* target_pool =
            tc_scene_handle_valid(scene) ? tc_scene_entity_pool(scene) : Entity::standalone_pool();
        ...
        if (!deserialize_entity_hierarchy(
                data, scene, parent_copy.valid() ? &parent_copy : nullptr, entity_data_pairs, cloned_root)) {
    ...
    bool Entity::validate_components() const {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (tc->kind != TC_CXX_COMPONENT && tc->kind != TC_PYTHON_COMPONENT) {
                fprintf(stderr,
                        "[validate_components] Entity '%s' component %zu has invalid kind: %d\n",
                        name(),
                        i,
                        (int)tc->kind);
                return false;
            ...
            if (tname[0] < 32 || tname[0] > 126) {
                fprintf(stderr,
                        "[validate_components] Entity '%s' component %zu type_name starts with non-printable: 0x%02x\n",
                        name(),
                        i,
                        (unsigned char)tname[0]);
                return false;
            }
        }
    ...
    static std::string tc_value_dict_string(const tc_value* dict, const char* key, const char* def = "") {
        ...
        if (v && v->type == TC_VALUE_STRING && v->data.s)
            return v->data.s;
        return def;
    }
    ...
    Entity Entity::deserialize(tc_entity_pool_handle pool_handle, const tc_value* data) {
        ...
        if (pose_v && pose_v->type == TC_VALUE_DICT) {
            ...
            if (pos && pos->type == TC_VALUE_LIST && tc_value_list_size(pos) >= 3) {
                double xyz[3] = {tc_value_as_double(tc_value_list_get(pos, 0)),
            ...
            if (rot && rot->type == TC_VALUE_LIST && tc_value_list_size(rot) >= 4) {
                double xyzw[4] = {tc_value_as_double(tc_value_list_get(rot, 0)),
        ...
        if (scl && scl->type == TC_VALUE_LIST && tc_value_list_size(scl) >= 3) {
            double xyz[3] = {tc_value_as_double(tc_value_list_get(scl, 0)),
    ...
    Entity Entity::deserialize_base_trent(const nos::trent& data, tc_scene_handle scene) {
        ...
        if (data.contains("pose") && data["pose"].is_dict()) {
            ...
            if (pose.contains("position") && pose["position"].is_list()) {
                ...
                if (p.size() >= 3) {
                    double xyz[3] = {static_cast<double>(p[0].as_numer_default(0)),
            ...
            if (pose.contains("rotation") && pose["rotation"].is_list()) {
                ...
                if (r.size() >= 4) {
                    double xyzw[4] = {static_cast<double>(r[0].as_numer_default(0)),
        ...
        if (data.contains("scale") && data["scale"].is_list()) {
            ...
            if (s.size() >= 3) {
                double xyz[3] = {static_cast<double>(s[0].as_numer_default(1)),
    ...
    void Entity::deserialize_components_trent(const nos::trent& data,
                                              tc_scene_handle scene,
        ...
        const auto& components = components_data.as_list();
        const std::vector<size_t> component_order = mode == ComponentDeserializationMode::Direct
        ...
        for (size_t ordered_index = 0; ordered_index < components.size(); ++ordered_index) {
            const size_t component_index =
                mode == ComponentDeserializationMode::Direct ? component_order[ordered_index] : ordered_index;
            const auto& comp_data = components[component_index];
            if (!comp_data.is_dict())
                continue;
            ...
            if (mode == ComponentDeserializationMode::UnknownOnly) {
                ...
                if (unk == nullptr) {
                    tc::Log::warn("[Entity] Failed to create UnknownComponent for %s", type_name.c_str());
                    continue;
                }

                tc_component_set_source_id(unk, source_id.c_str());
                add_component_ptr(unk);
                auto* unknown_obj = (unk->kind == TC_CXX_COMPONENT)
                ...
                if (unknown_obj != nullptr) {
                    ...
                    unknown_obj->original_data =
                        comp_data.contains("data") ? tc::trent_to_tc_value(comp_data["data"]) : tc_value_dict_new();
                }
