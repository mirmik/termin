...
static tc_scene_ext_type_entry* find_type(tc_scene_ext_type_id type_id) {
    ...
    if (type_id >= g_registry->type_capacity)
        return NULL;
    if (!g_registry->types[type_id].registered)
        return NULL;
...
void tc_scene_ext_registry_shutdown(void) {
    ...
    if (tc_scene_pool_count() != 0) {
        tc_log_error("[tc_scene_ext_registry_shutdown] refused with %llu live scene(s)",
...
bool tc_scene_ext_register(tc_scene_ext_type_id type_id,
                           const char* debug_name,
                           const char* persistence_key,
...
size_t tc_scene_ext_get_attached_types(tc_scene_handle scene, tc_scene_ext_type_id* out_ids, size_t max_count) {
    if (!tc_scene_alive(scene) || !out_ids || max_count == 0)
        return 0;
    if (!g_registry)
        return 0;
    ...
    for (size_t i = 0; i < g_registry->type_capacity && count < max_count; i++) {
        if (!g_registry->types[i].registered)
            continue;
        if (!tc_scene_ext_slot_get(scene, (tc_scene_ext_type_id)i))
            continue;
...
tc_value tc_scene_ext_serialize_scene(tc_scene_handle scene) {
    ...
    for (size_t i = 0; i < g_registry->type_capacity; i++) {
        ...
        if (!type_entry->registered)
            continue;
        if (!type_entry->persistence_key || !type_entry->persistence_key[0])
            continue;
        ...
        if (type_entry->vtable.serialize) {
            if (!type_entry->vtable.serialize(instance, &payload, type_entry->type_userdata)) {
                tc_log_warn("[tc_scene_ext_serialize_scene] serialize failed: type_id=%llu", (unsigned long long)i);
            }
        }
...
void tc_scene_ext_deserialize_scene(tc_scene_handle scene, const tc_value* extensions_dict) {
    ...
    if (!g_registry)
        return;
    if (!extensions_dict || extensions_dict->type != TC_VALUE_DICT)
        return;
    ...
    for (size_t i = 0; i < g_registry->type_capacity; i++) {
        ...
        if (!payload)
            continue;
        if (payload->type != TC_VALUE_DICT)
            continue;

        if (!tc_scene_ext_has(scene, (tc_scene_ext_type_id)i)) {
            if (!tc_scene_ext_attach(scene, (tc_scene_ext_type_id)i)) {
                tc_log_warn("[tc_scene_ext_deserialize_scene] attach failed: type_id=%llu", (unsigned long long)i);
                continue;
            }
        }
        ...
        if (!instance)
            continue;
        if (!type_entry->vtable.deserialize)
            continue;

        if (!type_entry->vtable.deserialize(instance, payload, type_entry->type_userdata)) {
            tc_log_warn("[tc_scene_ext_deserialize_scene] deserialize failed: type_id=%llu", (unsigned long long)i);
        }
    }
}
...
void tc_scene_ext_on_scene_update(tc_scene_handle scene, double dt) {
    ...
    for (size_t i = 0; i < g_registry->type_capacity; i++) {
        ...
        if (!instance)
            continue;
        if (!type_entry->vtable.on_scene_update)
            continue;
...
void tc_scene_ext_on_component_registered(tc_scene_handle scene, tc_component* component) {
    if (!tc_scene_alive(scene) || !component || !g_registry)
        return;
    ...
    for (size_t i = 0; i < g_registry->type_capacity; i++) {
        ...
        void* instance = tc_scene_ext_slot_get(scene, (tc_scene_ext_type_id)i);
        if (!instance || !type_entry->vtable.on_component_registered)
            continue;

        type_entry->vtable.on_component_registered(instance, component, type_entry->type_userdata);
    }
}

void tc_scene_ext_on_component_unregistering(tc_scene_handle scene, tc_component* component) {
    if (!tc_scene_alive(scene) || !component || !g_registry)
        return;
    ...
    for (size_t i = 0; i < g_registry->type_capacity; i++) {
        ...
        void* instance = tc_scene_ext_slot_get(scene, (tc_scene_ext_type_id)i);
        if (!instance || !type_entry->vtable.on_component_unregistering)
            continue;

        type_entry->vtable.on_component_unregistering(instance, component, type_entry->type_userdata);
    }
}
