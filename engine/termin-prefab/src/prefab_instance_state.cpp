...
namespace termin::prefab {
    namespace {
        ...
        template <typename T>
        void register_hidden_serialized_field(tc::InspectFacetBuilder& builder,
                                              T PrefabInstanceState::*member,
                                              const char* path,
                                              const char* kind) {
            tc::InspectFieldSpec spec =
                tc::inspect_accessor_field_spec(PrefabInstanceState::TypeName, path, path, kind, true, false);
        ...
        bool collect_scene_instance(tc_component* component, void* user_data) {
            ...
            if (state == nullptr || !state->mapping_valid() || state->prefab_asset_uuid() != *context->asset_uuid) {
                return true;
            }
        ...
        bool collect_scene(tc_scene_handle scene, void* user_data) {
            ...
            tc_scene_foreach_component_of_type(
                scene, PrefabInstanceState::TypeName, collect_scene_instance, &scene_context);
            return true;
        }
        ...
        bool override_string_field(
            const tc_value* value, const char* key, std::string& result, bool allow_empty, std::string& error) {
        ...
        std::string override_identity(const PrefabPropertyOverride& property_override) {
            return property_override.source_entity_id + "\x1f" + property_override.source_component_id + "\x1f" +
                   property_override.field_path;
        }

        bool validate_property_override(const PrefabPropertyOverride& property_override, std::string& error) {
        ...
        bool parse_structural_kind(const std::string& value, PrefabStructuralOverrideKind& result) {
            if (value == "suppress_entity")
                result = PrefabStructuralOverrideKind::SuppressEntity;
            else if (value == "suppress_component")
                result = PrefabStructuralOverrideKind::SuppressComponent;
            else if (value == "place_entity")
                result = PrefabStructuralOverrideKind::PlaceEntity;
            else if (value == "place_component")
                result = PrefabStructuralOverrideKind::PlaceComponent;
            else
                return false;
            return true;
        }
        ...
        const char* reference_kind_name(PrefabStructureReferenceKind kind) {
            switch (kind) {
            case PrefabStructureReferenceKind::End:
                return "end";
            case PrefabStructureReferenceKind::Source:
                return "source";
            case PrefabStructureReferenceKind::Local:
                return "local";
            }
            return nullptr;
        }
        ...
        bool parse_reference_kind(const std::string& value, PrefabStructureReferenceKind& result) {
            if (value == "end")
                result = PrefabStructureReferenceKind::End;
            else if (value == "source")
                result = PrefabStructureReferenceKind::Source;
            else if (value == "local")
                result = PrefabStructureReferenceKind::Local;
            else
                return false;
            return true;
        }
        ...
        std::string structural_identity(const PrefabStructuralOverride& item) {
            const std::string& source_id = item.kind == PrefabStructuralOverrideKind::SuppressComponent ||
        ...
        bool
        validate_reference(const PrefabStructureReference& reference, bool component_reference, std::string& error) {
            if (reference.kind == PrefabStructureReferenceKind::End)
                return true;
        ...
        bool validate_structural_override(const PrefabStructuralOverride& item, std::string& error) {
            const bool component = item.kind == PrefabStructuralOverrideKind::SuppressComponent ||
                                   item.kind == PrefabStructuralOverrideKind::PlaceComponent;
            ...
            if (item.kind == PrefabStructuralOverrideKind::PlaceEntity) {
                ...
                return validate_reference(item.parent, false, error) && validate_reference(item.before, false, error);
            }
        ...
        tc_value serialize_reference(const PrefabStructureReference& reference) {
            ...
            if (reference.kind == PrefabStructureReferenceKind::Source) {
                ...
                if (!reference.local_component_source_id.empty()) {
                    tc_value_dict_set(
                        &result, "component_source_id", tc_value_string(reference.local_component_source_id.c_str()));
                }
            }
            return result;
        }

        bool deserialize_reference(const tc_value* data,
            ...
            std::string kind;
            if (!override_string_field(data, "kind", kind, false, error) || !parse_reference_kind(kind, result.kind)) {
                if (error.empty())
                    error = "unknown structural reference kind";
                return false;
            }
    ...
    void PrefabInstanceState::register_type() {
        auto descriptor =
            ComponentTypeDescriptorBuilder::native<PrefabInstanceState>(TypeName, "termin-prefab", "CxxComponent");
        descriptor.category("Prefab");
        auto& inspect = descriptor.inspect();
        if (inspect.find_field(TypeName, "prefab_asset_uuid") == nullptr) {
            register_hidden_serialized_field(
                inspect, &PrefabInstanceState::_prefab_asset_uuid, "prefab_asset_uuid", "string");
            register_hidden_serialized_field(
                inspect, &PrefabInstanceState::_source_revision, "source_revision", "string");
            register_hidden_serialized_field(
                inspect, &PrefabInstanceState::_source_entity_ids, "source_entity_ids", "list[string]");
            register_hidden_serialized_field(
                inspect, &PrefabInstanceState::_runtime_entities, "runtime_entities", "list[entity]");
            register_hidden_serialized_field(
                inspect, &PrefabInstanceState::_source_component_ids, "source_component_ids", "list[string]");
            register_hidden_serialized_field(
                inspect, &PrefabInstanceState::_component_owners, "component_owners", "list[entity]");
        }
        (void)descriptor.commit();
    }

    void PrefabInstanceState::set_entity_mapping(std::vector<std::string> source_ids,
                                                 std::vector<Entity> runtime_entities) {
        if (source_ids.size() != runtime_entities.size()) {
            tc::Log::error("[PrefabInstanceState] entity mapping size mismatch: source=%zu runtime=%zu",
                           source_ids.size(),
    ...
    void PrefabInstanceState::set_component_mapping(std::vector<std::string> source_ids,
                                                    std::vector<Entity> runtime_owners) {
        if (source_ids.size() != runtime_owners.size()) {
            tc::Log::error("[PrefabInstanceState] component mapping size mismatch: source=%zu owners=%zu",
                           source_ids.size(),
                           runtime_owners.size());
    ...
    std::string PrefabInstanceState::source_for_entity(const Entity& runtime_entity) const {
        ...
        const size_t count = std::min(_source_entity_ids.size(), _runtime_entities.size());
        for (size_t index = 0; index < count; ++index) {
            if (_runtime_entities[index] == runtime_entity)
                return _source_entity_ids[index];
        }
        return {};
    }

    std::string PrefabInstanceState::source_for_component(const Entity& runtime_owner,
                                                          const std::string& runtime_component_source_id) const {
        if (!runtime_owner.valid() || runtime_component_source_id.empty())
            return {};
        const size_t count = std::min(_source_component_ids.size(), _component_owners.size());
        for (size_t index = 0; index < count; ++index) {
    ...
    bool PrefabInstanceState::rebind_entity_mapping(const std::string& source_id,
        ...
        const size_t index =
            insert ? _source_entity_ids.size() : static_cast<size_t>(found - _source_entity_ids.begin());
        ...
        for (size_t other = 0; other < _runtime_entities.size(); ++other) {
            if (other != index && _runtime_entities[other] == runtime_entity) {
                error = "runtime entity is already mapped to source id '" + _source_entity_ids[other] + "'";
                tc::Log::error("[PrefabInstanceState] %s", error.c_str());
                return false;
            }
        }
        ...
        if (insert) {
            ...
            const size_t component_count = std::min(_source_component_ids.size(), _component_owners.size());
    ...
    bool PrefabInstanceState::rebind_component_mapping(const std::string& source_id,
        ...
        if (insert) {
            ...
            if (index >= _component_owners.size()) {
                error = "prefab component mapping storage is inconsistent for source id '" + source_id + "'";
                tc::Log::error("[PrefabInstanceState] %s", error.c_str());
                return false;
            }
    ...
    bool PrefabInstanceState::set_property_override(PrefabPropertyOverride property_override_value,
                                                    std::string& error) {
        error.clear();
        if (!_overrides_valid) {
    ...
    bool PrefabInstanceState::set_structural_override(PrefabStructuralOverride structural_override,
                                                      std::string& error) {
        ...
        if (!validate_structural_override(structural_override, error)) {
            tc::Log::error("[PrefabInstanceState] rejected structural override: %s", error.c_str());
            return false;
        }
    ...
    bool PrefabInstanceState::discard_structural_override(PrefabStructuralOverrideKind kind,
        ...
        const auto found = std::find_if(
            ...
            [&key](const PrefabStructuralOverride& candidate) { return structural_identity(candidate) == key; });
        if (found == _structural_overrides.end())
            return false;
    ...
    bool PrefabInstanceState::source_entity_suppressed(const std::string& source_id) const {
        return std::any_of(_structural_overrides.begin(),
                           _structural_overrides.end(),
    ...
    const PrefabStructuralOverride* PrefabInstanceState::structural_override(PrefabStructuralOverrideKind kind,
    ...
    bool PrefabInstanceState::discard_property_override(const std::string& source_entity_id,
                                                        const std::string& source_component_id,
                                                        const std::string& field_path) {
        const std::string identity = source_entity_id + "\x1f" + source_component_id + "\x1f" + field_path;
        auto found = std::find_if(
            _property_overrides.begin(),
            _property_overrides.end(),
            [&identity](const PrefabPropertyOverride& candidate) { return override_identity(candidate) == identity; });
        if (found == _property_overrides.end())
            return false;
    ...
    const PrefabPropertyOverride* PrefabInstanceState::property_override(const std::string& source_entity_id,
                                                                         const std::string& source_component_id,
    ...
    tc_value PrefabInstanceState::serialize_data() const {
        ...
        if (!_overrides_valid && !_invalid_serialized_overrides.is_nil()) {
            ...
            for (const PrefabPropertyOverride& property_override_value : ordered) {
                ...
                tc_value_dict_set(
                    &item, "source_entity_id", tc_value_string(property_override_value.source_entity_id.c_str()));
                tc_value_dict_set(
                    &item, "source_component_id", tc_value_string(property_override_value.source_component_id.c_str()));
                tc_value_dict_set(&item, "field_path", tc_value_string(property_override_value.field_path.c_str()));
                tc_value_dict_set(&item, "target_kind", tc_value_string(property_override_value.target_kind.c_str()));
        ...
        if (!_structural_overrides_valid && !_invalid_serialized_structural_overrides.is_nil()) {
            ...
            for (const PrefabStructuralOverride& item : ordered) {
                ...
                tc_value_dict_set(&encoded, "kind", tc_value_string(structural_kind_name(item.kind)));
                tc_value_dict_set(&encoded, "source_entity_id", tc_value_string(item.source_entity_id.c_str()));
                tc_value_dict_set(&encoded, "source_component_id", tc_value_string(item.source_component_id.c_str()));
                if (item.kind == PrefabStructuralOverrideKind::PlaceEntity) {
    ...
    void PrefabInstanceState::deserialize_data(const tc_value* data, tc_scene_handle scene) {
            ...
            for (size_t index = 0; index < tc_value_list_size(serialized); ++index) {
                ...
                if (item == nullptr || item->type != TC_VALUE_DICT ||
                    !override_string_field(
                        item, "source_entity_id", property_override_value.source_entity_id, false, error) ||
                    !override_string_field(
                        item, "source_component_id", property_override_value.source_component_id, true, error) ||
                ...
                if (!_overrides_valid) {
                    ...
                    _invalid_serialized_overrides = tc::trent::copy_of(serialized);
                    tc::Log::error(
                        "[PrefabInstanceState] rejected property override at index %zu: %s", index, error.c_str());
                    return;
                }
            }
        }
        ...
        if (version == nullptr || version->type != TC_VALUE_INT || version->data.i != 1 ||
            ...
            tc::Log::error("[PrefabInstanceState] structural override metadata has an unsupported schema");
            return;
        }
        ...
        for (size_t index = 0; index < tc_value_list_size(structural); ++index) {
            ...
            if (item == nullptr || item->type != TC_VALUE_DICT ||
                ...
                !override_string_field(item, "source_entity_id", decoded.source_entity_id, true, error) ||
                !override_string_field(item, "source_component_id", decoded.source_component_id, true, error)) {
                _structural_overrides_valid = false;
                if (error.empty()) {
                    error = "unknown structural override kind '" + kind + "'";
                }
            } else if (decoded.kind == PrefabStructuralOverrideKind::PlaceEntity &&
                       (!deserialize_reference(override_field(item, "parent"), scene, decoded.parent, error) ||
                        !deserialize_reference(override_field(item, "before"), scene, decoded.before, error))) {
                _structural_overrides_valid = false;
            ...
            if (!_structural_overrides_valid) {
                ...
                _invalid_serialized_structural_overrides = tc::trent::copy_of(structural);
                tc::Log::error(
                    "[PrefabInstanceState] rejected structural override at index %zu: %s", index, error.c_str());
                return;
            }
        }
    }

    bool PrefabInstanceState::validate_mapping(bool require_live_references, std::string& message) const {
    ...
    void PrefabInstanceState::on_added() {
        ...
        if (!_mapping_valid) {
            tc::Log::error("[PrefabInstanceState] rejected invalid state on entity '%s': %s",
                           entity().valid() ? entity().name() : "<invalid>",
                           message.c_str());
        }
        if (!_overrides_valid) {
            tc::Log::error("[PrefabInstanceState] entity '%s' contains invalid property overrides",
                           entity().valid() ? entity().name() : "<invalid>");
        }
        if (!_structural_overrides_valid) {
            tc::Log::error("[PrefabInstanceState] entity '%s' contains invalid structural overrides",
                           entity().valid() ? entity().name() : "<invalid>");
        }
    }
    ...
    std::vector<Entity> find_live_prefab_instances(const std::string& prefab_asset_uuid) {
    ...
    std::vector<Entity> find_live_prefab_instances(tc_scene_handle scene, const std::string& prefab_asset_uuid) {
        std::vector<Entity> result;
        if (!tc_scene_alive(scene) || prefab_asset_uuid.empty()) {
            return result;
        }
        InstanceSnapshotContext context{&prefab_asset_uuid, &result};
        tc_scene_foreach_component_of_type(scene, PrefabInstanceState::TypeName, collect_scene_instance, &context);
        return result;
    }
