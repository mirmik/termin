...
namespace termin::prefab {
    namespace {
        ...
        bool validate_entity_node(const nos::trent& node,
            ...
            if (node.contains("components")) {
                ...
                for (const nos::trent& component : node["components"].as_list()) {
                    ...
                    if (!source_id.empty() && !component_source_ids.insert(source_id).second) {
                        message = component_path + ".source_id duplicates component source id '" + source_id + "'";
                        return false;
                    }
                    ++index;
                }
        ...
        bool collect_instance_mapping(const nos::trent& source,
            ...
            const tc_entity_id runtime_id = tc_entity_pool_find_by_uuid(target_pool, remap_it->second.c_str());
            ...
            if (source.contains("children")) {
                for (const nos::trent& child : source["children"].as_list()) {
                    if (!collect_instance_mapping(child,
                                                  uuid_remap,
                                                  target_pool,
                                                  target_pool_handle,
                                                  source_entity_ids,
        ...
        PrefabInstantiateResult instantiate_hierarchy(const nos::trent& source_hierarchy,
    ...
    PrefabInstantiateResult PrefabInstantiator::instantiate(const PrefabDocument& document,
                                                            tc_scene_handle target_scene,
                                                            const Entity& parent,
                                                            const PrefabInstantiateOptions& options) {
        PrefabDocumentResult validation = document.validate();
        if (!validation.ok()) {
            return failure(PrefabInstantiateError::InvalidDocument, validation.message);
        }
        return instantiate_hierarchy(document.source_hierarchy(), target_scene, parent, options, &document);
    }
    ...
    namespace {
        ...
        PrefabInstantiateResult instantiate_hierarchy(const nos::trent& source_hierarchy,
            ...
            if (!tc_scene_handle_valid(target_scene)) {
                return failure(PrefabInstantiateError::InvalidTargetScene, "target scene is invalid");
            }

            tc_entity_pool* target_pool = tc_scene_entity_pool(target_scene);
            if (target_pool == nullptr) {
                return failure(PrefabInstantiateError::InvalidTargetScene, "target scene has no entity pool");
            }
            if (parent.valid() && parent.pool() != target_pool) {
                return failure(PrefabInstantiateError::InvalidParent, "parent belongs to a different entity pool");
            }
            ...
            if (!validate_entity_node(
                    source_hierarchy, source_uuids, component_source_ids, validation_message, "root")) {
                const PrefabInstantiateError error =
                    validation_message.find("duplicates source uuid") != std::string::npos
            ...
            if (document != nullptr) {
                ...
                if (!ComponentRegistry::instance().has(PrefabInstanceState::TypeName)) {
                    return failure(PrefabInstantiateError::InstanceStatePublicationFailed,
            ...
            if (!instance_data.is_dict()) {
                return failure(PrefabInstantiateError::HierarchyCreationFailed,
            ...
            if (document != nullptr) {
                ...
                if (!collect_instance_mapping(source_hierarchy,
                                              uuid_remap,
                                              target_pool,
                                              root.pool_handle(),
                                              source_entity_ids,
                    ...
                    return failure(PrefabInstantiateError::InstanceStatePublicationFailed, std::move(mapping_message));
                }
                ...
                try {
                    ...
                    state->set_entity_mapping(std::move(source_entity_ids), std::move(runtime_entities));
                    state->set_component_mapping(std::move(source_component_ids), std::move(component_owners));
                    ...
                    const std::string root_source_id = source_hierarchy["uuid"].as_string_default("");
                    const auto store_option_override = [state, &root_source_id](const char* field_path,
                                                                                const char* target_kind,
                                                                                const tc_value& value) {
                        std::string error;
                        auto encoded = PrefabOverrideValue::from_inspect_value(&value, target_kind, error);
                        if (!encoded)
                            return error;
                    ...
                    if (!options.root_name.empty()) {
                        ...
                        tc_value_free(&value);
                        if (!error.empty()) {
                            throw std::runtime_error("failed to record root_name prefab override: " + error);
                        }
                    }
                    if (options.has_position) {
                        ...
                        const std::string error = store_option_override("transform.position", "vec3", value);
                        tc_value_free(&value);
                        if (!error.empty()) {
                            throw std::runtime_error("failed to record position prefab override: " + error);
                        }
                    }
    ...
    PrefabInstantiateResult PrefabInstantiator::instantiate(const nos::trent& source_hierarchy,
                                                            tc_scene_handle target_scene,
                                                            const Entity& parent,
                                                            const PrefabInstantiateOptions& options) {
