...
namespace termin::prefab {
    namespace {
        ...
        void index_entity(const nos::trent& entity, const std::string& parent_id, OrderedSourceIndex& index) {
            ...
            if (entity.contains("children") && entity["children"].is_list()) {
                for (const nos::trent& child : entity["children"].as_list()) {
                    ...
                    if (!child_id.empty())
                        child_order.push_back(child_id);
                    index_entity(child, entity_id, index);
                }
            }
        }

        void add_failure(PrefabReconcileResult& result,
                         Error error,
        ...
        std::vector<tc_component*> components_for_source(Entity owner, const std::string& source_id) {
        ...
        bool has_assignable_component(Entity owner, const char* required_type) {
            for (size_t index = 0; index < owner.component_count(); ++index) {
                ...
                if (type != nullptr && tc_component_registry_is_a(type, required_type))
                    return true;
            }
            return false;
        }
        ...
        std::string mapped_source_for_entity(const std::vector<std::string>& ids,
        ...
        bool property_targets_component(const std::vector<PrefabPropertyOverride>& overrides,
                                        const std::string& component_id) {
        ...
        bool property_targets_entity(const std::vector<PrefabPropertyOverride>& overrides,
                                     const std::string& entity_id) {
            return std::any_of(overrides.begin(), overrides.end(), [&](const auto& item) {
        ...
        bool component_override_compatible(const nos::trent& source_component,
            ...
            for (const PrefabPropertyOverride& item : overrides) {
                ...
                if (item.field_path == "display_name") {
                    if (item.target_kind == "string")
                        continue;
                } else if (item.field_path == "enabled" || item.field_path == "active_in_editor") {
                    if (item.target_kind == "bool")
                        continue;
                } else {
                    tc_field_info field{};
                    if (tc_inspect_find_field_info(type.c_str(), item.field_path.c_str(), &field) &&
                        field.is_serializable && field.kind != nullptr && item.target_kind == field.kind) {
                        continue;
                    }
                }
        ...
        std::vector<std::string> dependency_ordered_components(const std::vector<std::string>& source_order,
                                                               const OrderedSourceIndex& index) {
            std::vector<std::string> result;
            std::unordered_set<std::string> visiting;
            std::unordered_set<std::string> visited;
            std::function<void(const std::string&)> visit = [&](const std::string& component_id) {
                if (visited.contains(component_id) || !visiting.insert(component_id).second)
                    return;
                const std::string type = (*index.components.at(component_id))["type"].as_string_default("");
                for (size_t requirement_index = 0;
                     requirement_index < tc_component_registry_requirement_count(type.c_str());
                     ++requirement_index) {
                    const char* required = tc_component_registry_requirement_at(type.c_str(), requirement_index);
                    if (required == nullptr)
                        continue;
            ...
            for (const std::string& component_id : source_order)
                visit(component_id);
            return result;
        }

        tc_component* create_component_checked(Entity owner,
            ...
            for (size_t index = 0; index < tc_component_registry_requirement_count(type.c_str()); ++index) {
                const char* required = tc_component_registry_requirement_at(type.c_str(), index);
                if (required == nullptr || has_assignable_component(owner, required))
                    continue;
            ...
            if (data.is_dict()) {
                if (data.contains("display_name") && data["display_name"].is_string()) {
                    tc_component_set_display_name(component, data["display_name"].as_string().c_str());
                ...
                if (data.contains("active_in_editor") && data["active_in_editor"].is_bool()) {
                    tc_component_set_active_in_editor(component, data["active_in_editor"].as_bool());
                    data.as_dict().erase("active_in_editor");
                }
            }
            tc_value serialized = tc::trent_to_tc_value(data);
            tc_scene_inspect_context context =
                tc_scene_inspect_context_make(tc_entity_pool_get_scene(owner.pool_ptr()));
            const tc_inspect_apply_result applied =
                tc_component_inspect_deserialize_checked(component, &serialized, &context);
        ...
        template <typename T> void erase_parallel(std::vector<std::string>& ids, std::vector<T>& values, size_t index) {
    ...
    void PrefabInstanceState::reconcile_structure(const PrefabDocument& source, PrefabReconcileResult& result) {
        ...
        if (suppressed_entities.contains(root_id)) {
            add_failure(
                result, Error::StructuralOverrideInvalid, root_id, "", "the prefab instance root cannot be suppressed");
        ...
        for (const auto& [source_id, placement] : entity_placements) {
            ...
            if (suppressed_entities.contains(source_id)) {
                add_failure(result,
                            Error::StructuralOverrideInvalid,
                            source_id,
                            "",
        ...
        std::unordered_map<std::string, std::string> placement_parents = index.entity_parents;
        for (const std::string& source_id : placed_entity_ids) {
            ...
            if (placement.parent.kind == PrefabStructureReferenceKind::Source) {
                ...
                const std::string local_source =
                    mapped_source_for_entity(_source_entity_ids, _runtime_entities, placement.parent.local_entity);
                if (!local_source.empty())
                    placement_parents[source_id] = local_source;
                else
                    placement_parents.erase(source_id);
            }
        }
        ...
        std::function<bool(const std::string&)> visit_parent = [&](const std::string& source_id) {
            ...
            if (found != placement_parents.end() && !found->second.empty() && !visit_parent(found->second))
                return false;
        ...
        const auto anchor_cycle = [](const std::vector<std::string>& ids, const auto& placements) -> std::string {
            ...
            std::function<bool(const std::string&)> visit = [&](const std::string& source_id) {
                ...
                if (found != placements.end() && found->second->before.kind == PrefabStructureReferenceKind::Source &&
                    placements.contains(found->second->before.source_id) && !visit(found->second->before.source_id)) {
                    return false;
                }
                colors[source_id] = 2;
                return true;
        ...
        if (const std::string cycle = anchor_cycle(placed_entity_ids, entity_placements); !cycle.empty()) {
            add_failure(result, Error::OrderFailed, cycle, "", "entity placement anchors contain an ordering cycle");
            return;
        }
        if (const std::string cycle = anchor_cycle(placed_component_ids, component_placements); !cycle.empty()) {
            add_failure(result, Error::OrderFailed, "", cycle, "component placement anchors contain an ordering cycle");
            return;
        }
        ...
        const auto erase_entity_mapping = [&](const std::string& source_id) {
            ...
            if (found == _source_entity_ids.end())
                return;
            erase_parallel(
                _source_entity_ids, _runtime_entities, static_cast<size_t>(found - _source_entity_ids.begin()));
        };
        const auto erase_component_mapping = [&](const std::string& source_id) {
            const auto found = std::find(_source_component_ids.begin(), _source_component_ids.end(), source_id);
            if (found == _source_component_ids.end())
                return;
            erase_parallel(
                _source_component_ids, _component_owners, static_cast<size_t>(found - _source_component_ids.begin()));
        };
        ...
        for (size_t position = _source_component_ids.size(); position-- > 0;) {
            ...
            if (!owner.valid() || components_for_source(owner, _source_component_ids[position]).empty()) {
        ...
        for (const std::string& source_id : index.entity_preorder) {
            ...
            Entity created = Entity::create(entity().pool_handle(), source_entity["name"].as_string_default("entity"));
            if (!created.valid()) {
                add_failure(result,
        ...
        for (const std::string& source_id : index.entity_preorder) {
            ...
            if (!runtime.valid() || !parent.valid() || runtime.parent() == parent)
                continue;
            ++result.structure_operation_count;
        ...
        for (const std::string& entity_id : index.entity_preorder) {
            if (suppressed_entities.contains(entity_id))
                continue;
            Entity owner = entity_for_source(entity_id);
            if (!owner.valid())
                continue;
            const std::vector<std::string> creation_order =
                dependency_ordered_components(index.entity_components.at(entity_id), index);
            for (const std::string& component_id : creation_order) {
                if (suppressed_components.contains(component_id))
                    continue;
                ...
                tc_component* current = mapped_owner.valid() ? find_component(mapped_owner, component_id) : nullptr;
                if (mapped_owner.valid() && components_for_source(mapped_owner, component_id).size() > 1) {
                    add_failure(result,
                ...
                const std::string desired_type = source_component["type"].as_string_default("");
                const std::string current_type =
                    current && tc_component_type_name(current) ? tc_component_type_name(current) : "";
                ...
                if (current != nullptr && current_type != desired_type) {
                    ...
                    if (!component_override_compatible(source_component, _property_overrides, compatibility_error)) {
                        add_failure(result,
                ...
                ++result.structure_operation_count;
                std::string message;
                tc_component* replacement = create_component_checked(owner, source_component, uuid_remap, message);
                if (replacement == nullptr) {
                    add_failure(result, Error::FactoryUnavailable, entity_id, component_id, std::move(message));
                    continue;
                }
                ...
                const size_t old_index = current != nullptr ? mapped_owner.component_index(current) : SIZE_MAX;
                if (current != nullptr)
                    mapped_owner.remove_component_ptr(current);
                if (old_index != SIZE_MAX)
                    owner.set_component_index(replacement, old_index);
        ...
        for (size_t position = _source_component_ids.size(); position-- > 0;) {
            ...
            if (!absent && !suppressed)
                continue;
            Entity owner = _component_owners[position];
            const std::string entity_id = mapped_source_for_entity(_source_entity_ids, _runtime_entities, owner);
            if (absent && !suppressed &&
            ...
            ++result.structure_operation_count;
            if (owner.valid()) {
                const std::vector<tc_component*> matches = components_for_source(owner, component_id);
        ...
        for (const std::string& source_id : mapped_entities) {
            ...
            if ((!absent && !suppressed) || source_id == root_id)
                continue;
            if (absent && !suppressed &&
                (property_targets_entity(_property_overrides, source_id) || entity_placements.contains(source_id))) {
                add_failure(result,
                            Error::RemovalConflict,
                            source_id,
                            "",
            ...
            for (Entity child : runtime.children()) {
                if (!mapped_source_for_entity(_source_entity_ids, _runtime_entities, child).empty())
                    continue;
                const GeneralTransform3 parent_transform =
                    old_parent.valid() ? old_parent.transform() : GeneralTransform3{};
                if (!child.transform().try_reparent_preserve_world(parent_transform)) {
                    splice_failed = true;
                    add_failure(result,
                                Error::RemovalConflict,
                                source_id,
            ...
            for (size_t component_index = 0; component_index < runtime.component_count(); ++component_index) {
                tc_component* component = runtime.component_at(component_index);
                if (component == this->tc_component_ptr())
                    continue;
                const std::string component_id = component ? tc_component_get_source_id(component) : "";
                if (std::find(_source_component_ids.begin(), _source_component_ids.end(), component_id) ==
        ...
        for (const std::string& parent_id : index.entity_preorder) {
            ...
            for (size_t slot = 0; slot < current.size(); ++slot) {
                ...
                if (!child_id.empty() && index.entity_parents.contains(child_id) &&
                    index.entity_parents.at(child_id) == parent_id && !entity_placements.contains(child_id) &&
                    !suppressed_entities.contains(child_id)) {
                    slots.push_back(slot);
                }
            }
            for (const std::string& child_id : index.children.at(parent_id)) {
                if (entity_placements.contains(child_id) || suppressed_entities.contains(child_id))
                    continue;
                Entity child = entity_for_source(child_id);
                if (child.valid() && child.parent() == parent)
            ...
            for (size_t target_index = 0; target_index < target.size(); ++target_index) {
                ...
                if (live[target_index] == target[target_index])
                    continue;
                ++result.structure_operation_count;
                if (!target[target_index].set_sibling_index(target_index)) {
                    add_failure(result,
                                Error::OrderFailed,
                                source_entity_for(target[target_index]),
                                "",
        ...
        for (const std::string& entity_id : index.entity_preorder) {
            ...
            for (size_t component_index = 0; component_index < owner.component_count(); ++component_index) {
                current.push_back(owner.component_at(component_index));
            }
            std::vector<size_t> slots;
            std::vector<tc_component*> desired;
            for (size_t slot = 0; slot < current.size(); ++slot) {
                const std::string id = current[slot] ? tc_component_get_source_id(current[slot]) : "";
                if (index.component_owners.contains(id) && index.component_owners.at(id) == entity_id &&
                    !component_placements.contains(id) && !suppressed_components.contains(id)) {
                    slots.push_back(slot);
                }
            }
            for (const std::string& component_id : index.entity_components.at(entity_id)) {
                if (component_placements.contains(component_id) || suppressed_components.contains(component_id))
                    continue;
            ...
            for (size_t target_index = 0; target_index < target.size(); ++target_index) {
                if (owner.component_at(target_index) == target[target_index])
                    continue;
                ++result.structure_operation_count;
        ...
        for (const std::string& source_id : placed_entity_ids) {
            ...
            if (!parent.valid() || !is_descendant_or_self(parent, entity()) || is_descendant_or_self(parent, runtime)) {
                add_failure(result,
                            Error::StructuralOverrideInvalid,
                            source_id,
                            "",
            ...
            if (runtime.parent() != parent) {
                ++result.structure_operation_count;
                if (!runtime.set_parent_checked(parent)) {
                    add_failure(result, Error::ParentCycle, source_id, "", "checked entity placement reparent failed");
                    continue;
                }
                ++result.structure_operations_applied;
            }
            ...
            if (runtime.sibling_index() != target_index) {
                ...
                if (!runtime.set_sibling_index(target_index)) {
                    add_failure(result, Error::OrderFailed, source_id, "", "explicit entity placement order failed");
                } else {
                    ++result.structure_operations_applied;
                }
            }
        ...
        for (const std::string& component_id : placed_component_ids) {
            ...
            if (placement->before.kind == PrefabStructureReferenceKind::Source) {
                ...
                tc_component* before = find_component(owner, placement->before.local_component_source_id);
                if (before == nullptr) {
                    add_failure(result,
                                Error::StructuralOverrideInvalid,
                                "",
            ...
            if (owner.component_index(component) != target_index) {
                ...
                if (!owner.set_component_index(component, target_index)) {
                    add_failure(
                        result, Error::OrderFailed, "", component_id, "explicit component placement order failed");
                } else {
                    ++result.structure_operations_applied;
                }
            }
        ...
        const auto canonicalize_mapping =
            [](const std::vector<std::string>& preferred, std::vector<std::string>& ids, std::vector<Entity>& values) {
        ...
        canonicalize_mapping(index.entity_preorder, _source_entity_ids, _runtime_entities);
        std::vector<std::string> component_preorder;
        for (const std::string& entity_id : index.entity_preorder) {
            const auto& components = index.entity_components.at(entity_id);
            component_preorder.insert(component_preorder.end(), components.begin(), components.end());
        }
        canonicalize_mapping(component_preorder, _source_component_ids, _component_owners);
