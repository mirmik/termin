...
namespace termin::audio {
    ...
    public:
        ...
        explicit TcAudioClip(tc_audio_clip_handle value)
            : handle(value) {
            retain();
        }

        TcAudioClip(const TcAudioClip& other)
            : handle(other.handle) {
            retain();
        }

        TcAudioClip(TcAudioClip&& other) noexcept
            : handle(other.handle) {
        ...
        bool is_valid() const {
            return tc_audio_clip_is_valid(handle);
        }
        ...
        const tc_audio_clip* get() const {
            return tc_audio_clip_get(handle);
        }
        tc_audio_clip* get() {
            return tc_audio_clip_get(handle);
        }
        ...
        double duration() const {
            return tc_audio_clip_duration_seconds(handle);
        }
        uint64_t duration_ms() const {
            return tc_audio_clip_duration_ms(handle);
        }
        ...
        bool ensure_loaded() {
            return tc_audio_clip_ensure_loaded(handle);
        }
        bool unload() {
            return tc_audio_clip_unload(handle);
        }
        ...
        static TcAudioClip
        declare(const std::string& uuid, const std::string& name = "", const std::string& source_path = "") {
