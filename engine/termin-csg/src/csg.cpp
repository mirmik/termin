...
namespace termin::csg {
    ...
    struct Solid::Impl {
        ...
        Impl() = default;
        explicit Impl(Manifold value)
            : manifold(std::move(value)) {}
    };

    Solid::Solid()
        : impl_(new Impl()) {}

    Solid::Solid(Impl impl)
    ...
    Mesh3 to_mesh3(const Solid& solid, const std::string& name, const std::string& uuid, bool flat_shading) {
        Mesh3 mesh;
        mesh.name = name;
        mesh.uuid = uuid;
        ...
        for (size_t i = 0; i < vertex_count; ++i) {
            ...
            mesh.vertices.emplace_back(
                gl_mesh.vertProperties[base + 0], gl_mesh.vertProperties[base + 1], gl_mesh.vertProperties[base + 2]);
        }
    ...
    TcMesh to_tc_mesh(const Solid& solid, const std::string& name, const std::string& uuid, bool flat_shading) {
