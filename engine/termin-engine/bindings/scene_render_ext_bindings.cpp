...
namespace termin {
    ...
    void bind_scene_render_ext(nb::module_& m) {
        ...
        m.def("create_scene_with_render",
        ...
        m.def(
            "create_scene",
            [](const std::string& name, const std::string& uuid, nb::object extensions_obj) {
                if (extensions_obj.is_none()) {
                    return create_scene_with_render(name, uuid);
                }
                return create_scene_with_extensions(
                    name, uuid, nb::cast<std::vector<tc_scene_ext_type_id>>(extensions_obj));
            },
            nb::arg("name") = "",
            nb::arg("uuid") = "",
        ...
        m.def(
            ...
            [](nb::handle data, const std::string& name) -> TcSceneRef {
        ...
        m.def(
            ...
            [](const TcSceneRef& scene) -> std::vector<std::string> {
            ...
            nb::arg("scene"),
            "Get debug names of all attached scene extensions.");
    }
