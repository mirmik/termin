...
namespace termin {
    ...
    struct FrameGraphDebuggerPassInfo {
        ...
        bool has_internal_symbols() const {
            return !internal_symbols.empty();
        }
    ...
    public:
        ...
        const std::vector<RenderExecutionTargetInfo>& targets() const {
            return targets_;
        }
        const RenderExecutionTargetInfo* selected_target() const;
        bool has_selection() const {
            return has_desired_target_;
        }
        FrameGraphDebuggerState state() const {
            return state_;
        }
        FrameGraphDebuggerSuspendReason suspend_reason() const {
            return suspend_reason_;
        }
        uint64_t revision() const {
            return revision_;
        }
        uint64_t request_generation() const {
            return request_generation_;
        }
        tc_pipeline_handle resolved_pipeline() const {
            return resolved_pipeline_;
        }

        FrameGraphCapture& capture() {
            return capture_;
        }
        const FrameGraphCapture& capture() const {
            return capture_;
        }
        FrameGraphCapture& depth_capture() {
            return depth_capture_;
        }
        const FrameGraphCapture& depth_capture() const {
            return depth_capture_;
        }
        FrameGraphPresenter& presenter() {
            return presenter_;
        }
        const FrameGraphPresenter& presenter() const {
            return presenter_;
        }
        tgfx::TextureHandle capture_tex() const {
            return capture_.capture_tex();
        }
        tgfx::TextureHandle depth_capture_tex() const {
            return depth_capture_.capture_tex();
        }
        const std::string& requested_resource() const {
            return capture_request_.resource;
        }
        bool paused() const {
            return capture_request_.paused;
        }
        ...
        FrameGraphDebuggerMode mode() const {
            return mode_;
        }
        void set_mode(FrameGraphDebuggerMode mode);
        std::optional<size_t> selected_pass_index() const {
            return selected_pass_index_;
        }
        const std::string& selected_pass_name() const {
            return selected_pass_name_;
        }
        const std::string& selected_symbol() const {
            return selected_symbol_;
        }
        const std::string& selected_resource() const {
        ...
        bool highlight_hdr() const {
            return highlight_hdr_;
        }
        void set_highlight_hdr(bool enabled) {
            highlight_hdr_ = enabled;
            ++revision_;
        }
    ...
    private:
        void collect_render_demands(std::vector<RenderExecutionTargetId>& demands) const override;
        FrameGraphCaptureRequest* prepare_render_execution(const RenderExecutionInfo& execution) override;
        void finish_render_execution(const RenderExecutionInfo& execution, FrameGraphCaptureRequest* request) override;
        ...
        void set_connection_state(FrameGraphDebuggerState state,
