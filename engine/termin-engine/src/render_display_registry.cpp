...
namespace termin::rendering_manager_detail {
    ...
    void RenderDisplayRegistry::remove_display(tc_display_handle display,
    ...
    void RenderDisplayRegistry::remove_editor_display(tc_display_handle display,
                                                      const ViewportCleanupCallback& cleanup_viewport) {
        if (!tc_display_alive(display))
            return;
    ...
    bool RenderDisplayRegistry::try_auto_remove_display(tc_display_handle display,
                                                        const ViewportCleanupCallback& cleanup_viewport,
                                                        const DisplayRemovedCallback& removed_callback) {
        if (!tc_display_alive(display))
            return false;
        if (!tc_display_get_auto_remove_when_empty(display))
            return false;
        if (tc_display_get_viewport_count(display) > 0)
            return false;
    ...
    tc_display_handle RenderDisplayRegistry::get_or_create_display(const std::string& name,
    ...
    void RenderDisplayRegistry::cleanup_viewport_states(tc_display_handle display,
                                                        const ViewportCleanupCallback& cleanup_viewport) {
        if (!cleanup_viewport)
            return;
