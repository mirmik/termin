...
namespace termin {
    ...
    namespace {
        ...
        std::vector<std::string> collect_pass_strings(tc_pass* pass, PassStringGetter getter) {
            const size_t count = getter(pass, nullptr, 0);
            std::vector<const char*> raw(count, nullptr);
            if (count)
                getter(pass, raw.data(), raw.size());
        ...
        FrameGraphDebuggerPassInfo make_pass_info(const RenderPipeline& pipeline, tc_pass* pass) {
            ...
            if (pass) {
                ...
                if (result.type != "ShadowPass" && result.name != "ShadowPass") {
                    result.internal_symbols = collect_pass_strings(pass, tc_pass_get_internal_symbols);
                }
            }
            return result;
        }
    ...
    FrameGraphDebugger::~FrameGraphDebugger() {
        if (manager_)
            manager_->remove_render_execution_observer(*this);
    }
    ...
    bool FrameGraphDebugger::refresh() {
        ...
        if (!has_desired_target_) {
            ...
            if (!target) {
                ...
                if (binding_changed) {
                    invalidate_request();
                    set_connection_state(FrameGraphDebuggerState::Suspended,
                ...
                const bool pipeline_changed = !tc_pipeline_handle_eq(resolved_pipeline_, target->pipeline);
                const bool binding_changed = pipeline_changed || state_ != FrameGraphDebuggerState::Suspended ||
                ...
                const FrameGraphDebuggerState next_state =
                    request_active_ ? FrameGraphDebuggerState::WaitingFrame : FrameGraphDebuggerState::Bound;
                if (pipeline_changed || state_ == FrameGraphDebuggerState::Suspended ||
                    state_ == FrameGraphDebuggerState::Unbound ||
                    suspend_reason_ != FrameGraphDebuggerSuspendReason::None) {
                    set_connection_state(next_state, FrameGraphDebuggerSuspendReason::None, target->pipeline);
                    changed = true;
                }
            }
        }
    ...
    void FrameGraphDebugger::finish_frame() {
        refresh();
        if (!request_active_ || state_ == FrameGraphDebuggerState::Suspended)
            return;
        ...
        if (!capture_request_.resource.empty())
            return;
        const FrameGraphDebuggerState next =
            capture_.has_capture() ? FrameGraphDebuggerState::Captured : FrameGraphDebuggerState::WaitingFrame;
        if (state_ != next) {
            state_ = next;
            ++revision_;
        }
    ...
    void FrameGraphDebugger::clear_selection() {
        if (!has_desired_target_ && state_ == FrameGraphDebuggerState::Unbound)
            return;
        has_desired_target_ = false;
        request_active_ = false;
        invalidate_request();
        set_connection_state(
            FrameGraphDebuggerState::Unbound, FrameGraphDebuggerSuspendReason::None, TC_PIPELINE_HANDLE_INVALID);
        ++revision_;
    }
    ...
    void FrameGraphDebugger::begin_request() {
        ...
        invalidate_request();
        refresh();
        if (state_ == FrameGraphDebuggerState::Bound || state_ == FrameGraphDebuggerState::Captured) {
    ...
    bool FrameGraphDebugger::request_internal(size_t pass_index, const std::string& symbol) {
        RenderPipeline pipeline(resolved_pipeline_);
        if (!pipeline.is_valid() || pass_index >= pipeline.pass_count() || symbol.empty()) {
            return false;
        }
        const FrameGraphDebuggerPassInfo info = make_pass_info(pipeline, pipeline.get_pass_at(pass_index));
        if (!contains(info.internal_symbols, symbol))
            return false;
    ...
    void FrameGraphDebugger::set_capture_max_long_edge(std::uint32_t max_long_edge) {
        capture_request_.max_long_edge = max_long_edge;
    }

    void FrameGraphDebugger::set_paused(bool paused) {
        if (capture_request_.paused == paused)
            return;
    ...
    void FrameGraphDebugger::cancel_request() {
        ...
        invalidate_request();
        refresh();
        if (state_ == FrameGraphDebuggerState::WaitingFrame || state_ == FrameGraphDebuggerState::Captured) {
    ...
    FrameGraphCaptureRequest* FrameGraphDebugger::prepare_render_execution(const RenderExecutionInfo& execution) {
        refresh();
        const bool request_configured =
            (capture_request_.kind == FrameGraphCaptureRequestKind::Resource && !capture_request_.resource.empty()) ||
            (capture_request_.kind == FrameGraphCaptureRequestKind::InternalSymbol &&
             capture_request_.pass_index != static_cast<size_t>(-1) && !capture_request_.internal_symbol.empty());
        if (!request_active_ || !request_configured || capture_request_.paused ||
            state_ == FrameGraphDebuggerState::Suspended || !execution_matches_target(execution)) {
            return nullptr;
        }
    ...
    void FrameGraphDebugger::collect_render_demands(std::vector<RenderExecutionTargetId>& demands) const {
        if ((!connected_ && !request_active_) || capture_request_.paused || !has_desired_target_) {
            return;
        }
    ...
    std::optional<size_t> FrameGraphDebugger::selected_target_index() const {
        ...
        for (size_t index = 0; index < targets_.size(); ++index) {
            if (targets_[index].id == desired_target_)
                return index;
        }
        return std::nullopt;
    }
    ...
    void FrameGraphDebugger::set_selected_pass(std::optional<size_t> pass_index) {
        ...
        if (pass_index && pipeline.is_valid() && *pass_index < pipeline.pass_count()) {
            ...
            if (!available.empty())
                selected_symbol_ = available.back();
        }
        reconnect_request();
        ++revision_;
    }
    ...
    void FrameGraphDebugger::set_selected_resource(const std::string& resource) {
        if (selected_resource_ == resource)
            return;
        selected_resource_ = resource;
        reconnect_request();
        ++revision_;
    ...
    void FrameGraphDebugger::reconnect_request() {
        ...
        if (mode_ == FrameGraphDebuggerMode::BetweenPasses) {
            if (!selected_resource_.empty())
                request_resource(selected_resource_);
            return;
        }
    ...
    std::vector<FrameGraphDebuggerPassInfo> FrameGraphDebugger::schedule() const {
        ...
        if (!graph || tc_frame_graph_get_error(graph) != TC_FG_OK)
            return result;
    ...
    std::map<std::string, std::vector<std::string>> FrameGraphDebugger::alias_groups() const {
        ...
        if (!graph || tc_frame_graph_get_error(graph) != TC_FG_OK)
            return result;
        const size_t canonical_count = tc_frame_graph_get_canonical_resources(graph, nullptr, 0);
        std::vector<const char*> canonical(canonical_count, nullptr);
        if (canonical_count) {
            tc_frame_graph_get_canonical_resources(graph, canonical.data(), canonical.size());
        }
        for (const char* name : canonical) {
            ...
            std::vector<const char*> raw(count, nullptr);
            if (count)
                tc_frame_graph_get_alias_group(graph, name, raw.data(), raw.size());
            std::vector<std::string>& group = result[name];
            for (const char* alias : raw) {
                if (alias)
                    group.emplace_back(alias);
            }
    ...
    std::vector<std::string> FrameGraphDebugger::symbols() const {
        tc_pass* pass = selected_pass();
        return pass ? collect_pass_strings(pass, tc_pass_get_internal_symbols) : std::vector<std::string>{};
    }
    ...
    std::string FrameGraphDebugger::format_capture_info() const {
        ...
        out << "<b>" << selected_resource_
            << "</b> | Type: " << (capture_.is_depth() ? "depth_texture" : "color_texture")
            << " | Size: " << capture_.width() << "x" << capture_.height()
            << " | fmt=" << tgfx::pixel_format_name(capture_.format()) << " | MSAA=off";
        return out.str();
    }
    ...
    std::string FrameGraphDebugger::format_writer_pass() const {
        ...
        for (const auto& pass : schedule()) {
            if (contains(pass.writes, selected_resource_))
                return "<- " + pass.name;
        }
        return "(read-only)";
    }
    ...
    std::string FrameGraphDebugger::format_pipeline_info() const {
        ...
        for (const auto& pass : ordered) {
            if (!first)
                out << "<br>";
            first = false;
            const bool writer = !selected_resource_.empty() && contains(pass.writes, selected_resource_);
            if (writer)
                out << "<span style='color: #50fa7b; font-weight: bold;'>* ";
            out << pass.name << ": {";
    ...
    std::string FrameGraphDebugger::format_pass_json() const {
        tc_pass* pass = selected_pass();
        if (!pass)
            return selected_pass_index_ ? "<no pipeline>" : std::string{};
    ...
    std::string FrameGraphDebugger::format_timing() const {
        tc_pass* pass = selected_pass();
        if (!pass || selected_symbol_.empty() || pass->kind != TC_NATIVE_PASS)
            return {};
        ...
        for (const InternalSymbolTiming& timing : native->get_internal_symbols_with_timing()) {
            if (timing.name != selected_symbol_)
                continue;
            std::ostringstream out;
            out << std::fixed << std::setprecision(3) << "CPU: " << timing.cpu_time_ms << "ms | GPU: ";
            if (timing.gpu_time_ms >= 0.0)
                out << timing.gpu_time_ms << "ms";
            else
                out << "pending...";
            return out.str();
        }
    ...
    std::string FrameGraphDebugger::format_render_stats() const {
        ...
        std::ostringstream out;
        out << "Scenes: " << manager_->attached_scenes().size() << " | Pipelines: " << pipeline_count
            << " | Unmanaged: " << unmanaged << " | PipelineCache: hit=" << cache.hit_count
            << " miss=" << cache.miss_count << " create=" << cache.create_pipeline_count
            << " cached=" << cache.cached_pipeline_count << " layouts=" << cache.unique_vertex_layout_signature_count;
        return out.str();
    }
    ...
    std::string FrameGraphDebugger::analyze_hdr() {
        ...
        if (capture_.is_depth())
            return "HDR stats unavailable for depth texture";
        RenderEngine* engine = manager_->render_engine();
        if (!engine || !engine->tgfx2_device())
            return "No tgfx2 device";
        HDRStats stats = presenter_.compute_hdr_stats(engine->tgfx2_device(), capture_.capture_tex());
        std::ostringstream out;
        out << std::fixed << std::setprecision(3) << "<b>R:</b> " << stats.min_r << " - " << stats.max_r
            << " (avg: " << stats.avg_r << ")<br>" << "<b>G:</b> " << stats.min_g << " - " << stats.max_g
            << " (avg: " << stats.avg_g << ")<br>" << "<b>B:</b> " << stats.min_b << " - " << stats.max_b
            << " (avg: " << stats.avg_b << ")<br>" << "<b>Max:</b> " << stats.max_value << "<br>"
            << "<b>HDR pixels:</b> " << stats.hdr_pixel_count << " (" << std::setprecision(2) << stats.hdr_percent
            << "%)";
        return out.str();
    }

    void FrameGraphDebugger::finish_render_execution(const RenderExecutionInfo& execution,
        ...
        const FrameGraphDebuggerState next = request->status == FrameGraphCaptureRequestStatus::Captured
    ...
    bool FrameGraphDebugger::execution_matches_target(const RenderExecutionInfo& execution) const {
        if (!has_desired_target_ || !tc_pipeline_handle_eq(execution.pipeline, resolved_pipeline_)) {
            return false;
        }
        for (const RenderExecutionTargetId& target : execution.targets) {
            if (target == desired_target_)
                return true;
        }
        return false;
    }
    ...
    const RenderExecutionTargetInfo* FrameGraphDebugger::find_target(const RenderExecutionTargetId& id) const {
    ...
    void FrameGraphDebugger::set_connection_state(FrameGraphDebuggerState state,
