...
namespace termin {
    ...
    bool SceneManager::tick(double dt) {
        ...
        for (const auto& [name, scene] : _scenes) {
            ...
            if (mode == TC_SCENE_MODE_INACTIVE) {
                ...
                if (profile)
                    tc_profiler_begin_section(("Scene Editor Update: " + name).c_str());
                ...
                if (profile)
                    tc_profiler_begin_section(("Scene Update: " + name).c_str());
    ...
    void SceneManager::write_json_file(const std::string& path, const std::string& json) {
        ...
        if (ec) {
            tc_log(TC_LOG_ERROR,
                   "[SceneManager] write_json_file: failed to rename temp file to '%s': %s",
                   path.c_str(),
                   ec.message().c_str());
            std::filesystem::remove(temp_path);
        }
