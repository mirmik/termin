...
namespace termin {
    ...
    RenderAttachmentContext::RenderAttachmentContext(RenderTopology& topology, tc_scene_handle scene)
    ...
    RenderAttachmentContext& RenderAttachmentContext::operator=(const RenderAttachmentContext& other) {
        if (this == &other)
            return *this;
        if (invalidates_on_destruction_)
            invalidate();
        state_ = other.state_;
        invalidates_on_destruction_ = false;
        return *this;
    ...
    bool RenderAttachmentContext::valid() const {
        return state_ && state_->active && state_->topology != nullptr && tc_scene_handle_valid(state_->scene);
    }
    ...
    tc_render_target_handle RenderAttachmentContext::find_render_target(const std::string& name) const {
        const State& state = require_active();
        return state.topology->find_render_target(state.scene, name);
    }

    tc_render_target_handle RenderAttachmentContext::find_camera_target(const tc_component* camera) const {
        if (camera == nullptr)
            return TC_RENDER_TARGET_HANDLE_INVALID;
        for (tc_render_target_handle target : render_targets()) {
            if (tc_render_target_handle_valid(target) && tc_render_target_get_camera(target) == camera) {
                return target;
            }
        }
        return TC_RENDER_TARGET_HANDLE_INVALID;
...
namespace {
    ...
    const termin::RenderAttachmentContext* context_from_c(const tc_render_attachment_context* context) {
...
size_t tc_render_attachment_context_render_target_count(const tc_render_attachment_context* context) {
    const auto* typed = context_from_c(context);
    return typed && typed->valid() ? typed->render_targets().size() : 0;
}

tc_render_target_handle tc_render_attachment_context_render_target_at(const tc_render_attachment_context* context,
                                                                      size_t index) {
    const auto* typed = context_from_c(context);
    if (!typed || !typed->valid())
        return TC_RENDER_TARGET_HANDLE_INVALID;
    const auto targets = typed->render_targets();
    return index < targets.size() ? targets[index] : TC_RENDER_TARGET_HANDLE_INVALID;
}

tc_render_target_handle tc_render_attachment_context_find_render_target(const tc_render_attachment_context* context,
                                                                        const char* name) {
    const auto* typed = context_from_c(context);
    if (!typed || !typed->valid() || !name)
        return TC_RENDER_TARGET_HANDLE_INVALID;
    return typed->find_render_target(name);
}

tc_render_target_handle tc_render_attachment_context_find_camera_target(const tc_render_attachment_context* context,
...
tc_pipeline_handle tc_render_attachment_context_get_pipeline(const tc_render_attachment_context* context,
                                                             const char* name) {
    const auto* typed = context_from_c(context);
    if (!typed || !typed->valid() || !name)
