...
namespace termin {
    ...
    void ViewportRenderState::ensure_output_textures(tgfx::IRenderDevice& device, int width, int height) {
        ensure_output_textures(device, width, height, tgfx::PixelFormat::RGBA8_UNorm, tgfx::PixelFormat::D24_UNorm);
    }

    void ViewportRenderState::ensure_output_textures(tgfx::IRenderDevice& device,
                                                     int width,
                                                     int height,
                                                     tgfx::PixelFormat color_format,
                                                     tgfx::PixelFormat depth_format) {
        if (output_color_tex && output_width == width && output_height == height &&
            output_color_format == color_format && output_depth_format == depth_format && device_ == &device) {
            return;
        }
        release_textures();
