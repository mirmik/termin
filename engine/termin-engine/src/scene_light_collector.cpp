...
namespace termin::rendering_manager_detail {
    ...
    static bool collect_lights_cap_cb(tc_component* c, void* user_data) {
        ...
        if (!cap || !cap->vtable || !cap->vtable->get_light_data)
            return true;

        tc_light_data ld;
        if (!cap->vtable->get_light_data(c, &ld))
            return true;
    ...
    std::vector<Light> collect_lights(tc_scene_handle scene) {
        ...
        tc_scene_foreach_with_capability(
            scene, light_cap, collect_lights_cap_cb, &lights, TC_SCENE_FILTER_ENABLED | TC_SCENE_FILTER_ENTITY_ENABLED);
