...
## Копирование и путь исходного файла

Python `copy_scene(source_key, destination_key)` создаёт независимый экземпляр с новым ключом и пустым file path: обычный дубликат ещё не сохранён и не должен перезаписать оригинал. Для Editor Play используется `preserve_source_path=True`. Этот режим переносит путь исходного файла в registry и `scene.source_path` до десериализации компонентов, чтобы runtime-компоненты могли разрешать связанные файловые ресурсы. Ключи, handles и жизненные циклы исходной сцены и копии остаются независимыми; runtime world context не копируется. У несохранённой исходной сцены путь остаётся пустым в обоих режимах.
