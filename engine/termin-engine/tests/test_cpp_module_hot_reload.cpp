...
namespace {
    ...
    public:
        ...
        TempDir() {
            ...
            path = std::filesystem::temp_directory_path() / ("termin-engine-cpp-hot-reload-" + std::to_string(unique));
    ...
    std::filesystem::path copy_module_artifact(const std::filesystem::path& source, const std::filesystem::path& dir) {
#ifdef _WIN32
...
#endif
        std::filesystem::copy_file(source, artifact, std::filesystem::copy_options::overwrite_existing);
        return artifact;
    }

    void write_descriptor(const std::filesystem::path& descriptor, const std::filesystem::path& artifact) {
        write_text_file(descriptor,
                        "name: native_probe\n"
                        "build:\n"
                        "  output: " +
                            yaml_quote(artifact.string()) + "\n");
    }
    ...
    int component_int_field(tc_component* component, const char* path) {
        ...
        const int result = value && value->type == TC_VALUE_INT ? static_cast<int>(value->data.i) : -1;
        tc_value_free(&data);
        return result;
    }
    ...
    void register_engine_owned_inspect_probe() {
        ...
        if (!inspect.add<EngineOwnedProbe, int>(
                kEngineOwnedProbeType, &EngineOwnedProbe::value, "value", "Engine Value", "int")) {
            throw std::runtime_error(inspect.error());
        }
        auto* descriptor = tc_runtime_type_descriptor_create(kEngineOwnedProbeType, "termin-engine-test", nullptr);
        if (!descriptor) {
    ...
    bool engine_owned_probe_intact(const char* step, std::string& error) {
        ...
        if (tc::InspectRegistry::instance().owner_of(kEngineOwnedProbeType) != "termin-engine-test") {
    ...
    int run_cpp_module_hot_reload_smoke() {
        ...
        TEST_ASSERT(tc::InspectRegistry::instance().owner_of(kComponentType) == kModuleId, "inspect owner captured");
        TEST_ASSERT(std::string(tc_runtime_type_registry_get_owner(kComponentType)
                                    ? tc_runtime_type_registry_get_owner(kComponentType)
                                    : "") == kModuleId,
                    "runtime type owner captured");
        TEST_ASSERT(std::string(tc_runtime_type_registry_get_parent(kComponentType)
                                    ? tc_runtime_type_registry_get_parent(kComponentType)
                                    : "") == "CxxComponent",
        ...
        TEST_ASSERT(tc_runtime_type_registry_has_facet(kComponentType, tc::TC_RUNTIME_TYPE_FACET_INSPECT_FIELDS),
        ...
        TEST_ASSERT(engine_owned_probe_intact("module load", engine_probe_error), engine_probe_error.c_str());
        ...
        TEST_ASSERT(engine_owned_probe_intact("module reload", engine_probe_error), engine_probe_error.c_str());
        ...
        TEST_ASSERT(component_int_field(upgraded, "value") == 77, "component inspect data survives reload roundtrip");
        ...
        TEST_ASSERT(pass_int_field(upgraded_pass, "exposure") == 23, "pass inspect data survives reload roundtrip");
        TEST_ASSERT(std::string(upgraded_pass->pass_name) == "module-pass",
                    "pass slot identity survives reload roundtrip");

        termin::TcSceneRef unmanaged_scene = termin::TcSceneRef::create("cpp-hot-reload-unmanaged");
        ...
        TEST_ASSERT(unmanaged_component != nullptr, "native component instance created outside SceneManager");
        ...
        TEST_ASSERT(!tc_runtime_type_registry_has_type(kComponentType), "runtime type record removed on module unload");
        TEST_ASSERT(engine_owned_probe_intact("module unload", engine_probe_error), engine_probe_error.c_str());
        ...
        TEST_ASSERT(!tc_runtime_type_registry_has_type(kPassType), "pass runtime type record removed on module unload");
        ...
        TEST_ASSERT(std::string(tc_pass_type_name(unknown_pass)) == "UnknownPass", "live pass degraded during unload");
        auto* unknown_pass_object = dynamic_cast<termin::UnknownPass*>(termin::CxxFramePass::from_tc(unknown_pass));
        TEST_ASSERT(unknown_pass_object != nullptr, "UnknownPass object is accessible");
        TEST_ASSERT(unknown_pass_object->original_type == kPassType, "UnknownPass preserves original type");
        ...
        TEST_ASSERT(engine_owned_probe_intact("failed module load", engine_probe_error), engine_probe_error.c_str());
        ...
        tc_component* restored_unmanaged = unmanaged_entity.get_component_by_type_name(kComponentType);
        TEST_ASSERT(restored_unmanaged != nullptr, "runtime-list load restores component outside SceneManager");
        TEST_ASSERT(unmanaged_entity.get_component_by_type_name("UnknownComponent") == nullptr,
        ...
        TEST_ASSERT(engine_owned_probe_intact("artifact restore", engine_probe_error), engine_probe_error.c_str());
