...
namespace termin {
    ...
    namespace {
        ...
        static_assert(sizeof(ShadowPerFrameStd140) == 128, "ShadowPerFrameStd140 must be 2 * mat4");
        ...
        static_assert(sizeof(ShadowPushStd140) == 64, "ShadowPushStd140 must be mat4");

        struct ShadowTaskExtension final : RenderTaskExtension {
            ShadowPushStd140 draw_data{};
        };

        RenderItemTaskPlanningContract
        shadow_task_planning_contract(const MaterialPipelinePassContract& shader_contract,
        ...
        bool plan_shadow_item_shader(const tc_render_item& item,
                                     tc_material_phase* phase,
                                     tc_shader_handle base_shader,
                                     const MaterialPipelinePassContract& shader_contract,
                                     const char* debug_pass_name,
                                     RenderTaskList& tasks) {
            RenderItemTaskPlanningContract contract = shadow_task_planning_contract(shader_contract, debug_pass_name);
        ...
        MaterialFragmentInterface shadow_world_position_interface() {
            ...
            interface.semantics.push_back({"world_pos", MaterialPipelineValueType::Float3});
            return interface;
        }
        ...
        VertexOutputAdapter shadow_vertex_output_adapter() {
            ...
            adapter.source_module = {SHADOW_OUTPUT_ADAPTER_MODULE,
            ...
            adapter.produced_output_semantics.semantics.push_back({"clip_position", MaterialPipelineValueType::Float4});
        ...
        void configure_shadow_foliage_provider(VertexTransformProvider& provider) {
            ...
            provider.source_module = {SHADOW_FOLIAGE_TRANSFORM_MODULE,
...
struct VertexInput {
    ...
    uint instance_id : SV_InstanceID;
        ...
        MaterialPipelinePassContract shadow_material_pass_contract() {
            ...
            contract.static_vertex_transform = material_pipeline_make_static_mesh_vertex_transform_provider(
                "static_shadow", MeshVertexTransformProfile::Position, "shadow_draw.u_model");
            contract.static_vertex_transform->resources.push_back(
                material_pipeline_draw_resource_decl("shadow_draw", TC_SHADER_STAGE_VERTEX, 64u));
            contract.skinned_vertex_transform = material_pipeline_make_skinned_mesh_vertex_transform_provider(
                "skinned_shadow", MeshVertexTransformProfile::Position, "shadow_draw.u_model");
            ...
            contract.foliage_vertex_transform = material_pipeline_make_foliage_vertex_transform_provider(
                "foliage_shadow", MeshVertexTransformProfile::Position);
        ...
        tc_shader_handle register_modular_shadow_shader() {
            const std::string shadow_fragment_source =
                tgfx::load_builtin_shader_stage_source_from_catalog(SHADOW_ENGINE_SHADER_UUID, "fragment");
            if (shadow_fragment_source.empty()) {
                tc::Log::error("[ShadowPass] failed to load shadow fragment source '%s'", SHADOW_ENGINE_SHADER_UUID);
            ...
            fragment_create_info.sources.source_path = "builtin_shaders/termin-engine-shadow.slang";
            ...
            request.material = material_pipeline_material_contract_from_shader(
                base_shader, pass_contract.required_material_fragment_input);
            ...
            request.shader_uuid = material_pipeline_shader_intent_fingerprint(
                base_shader, TC_SHADER_VARIANT_NONE, request.vertex_transform, request.pass);
            request.language = base_shader.language();
            request.artifact_policy = base_shader.artifact_policy();

            MaterialPipelineShaderAssemblyResult result = material_pipeline_assemble_shader(request);
            if (!result.ok()) {
                for (const MaterialPipelineDiagnostic& diagnostic : result.diagnostics) {
                    tc::Log::error("[ShadowPass] modular shadow shader assembly failed: %s: %s",
    ...
    ShadowPass::ShadowPass(const std::string& output_res, const std::string& pass_name, float caster_offset)
    ...
    void ShadowPass::destroy() {
        if (depth_pool_device_) {
            for (auto& [_, slot] : depth_pool_) {
                if (slot.tex)
                    depth_pool_device_->destroy(slot.tex);
            }
        }
        depth_pool_.clear();
    ...
    std::vector<ResourceSpec> ShadowPass::get_resource_specs() const {
        return {ResourceSpec{output_res, "shadow_map_array", std::nullopt, std::nullopt, std::nullopt}};
    }

    tgfx::TextureHandle ShadowPass::get_or_create_depth_tex2(tgfx::IRenderDevice& device, int resolution, int index) {
        ...
        if (!tex) {
            tc::Log::error("ShadowPass: failed to create depth texture (res=%d)", resolution);
            return {};
        }
        ...
        bool collect_shadow_drawable_shader_usages(tc_component* tc, void* user_data) {
            ...
            for (const tc_render_item& item : items.items) {
                ...
                if (!plan_shadow_item_shader(
                        item, phase, data->base_shader, data->pass_contract, "ShadowPass/ShaderUsage", tasks)) {
                    continue;
                }
                // Report every executable pass variant, not evaluator-only planner
                // dependencies retained in shader_usages for ownership and
                // invalidation.
    ...
    void ShadowPass::collect_shadow_casters(tc_scene_handle scene,
        ...
        const std::span<const size_t> routed_items = snapshot.phase_item_indices(TC_PHASE_SHADOW);
        ...
        for (size_t item_index : routed_items) {
            ...
            if (!tc) {
                tc::Log::error("[ShadowPass] collect_shadow_casters: collected item %zu has null component",
                               item_index);
                continue;
            }
            ...
            if (!ent.valid()) {
                ...
                tc::Log::error("[ShadowPass] collect_shadow_casters: drawable component '%s' has invalid owner",
    ...
    void ShadowPass::collect_scene_shader_usages(tc_scene_handle scene,
    ...
    std::vector<ShadowMapResult> ShadowPass::execute_shadow_pass_tgfx2(ExecuteContext& ctx,
        ...
        const RenderItemSnapshot* scene_items = require_render_item_snapshot(ctx, "ShadowPass");
        if (!scene_items) {
            return results;
        }
        ...
        for (const auto& dc : cached_draw_calls_) {
            ...
            if (!item) {
                tc::Log::error("[ShadowPass/tgfx2] skip planning: invalid item index %zu", dc.item_index);
                continue;
            }
    ...
    void ShadowPass::execute(ExecuteContext& ctx) {
        ...
        if (profile)
            tc_profiler_begin_section("ShadowPass");

        ShadowMapArrayResource* shadow_array = ctx.get_frame_graph_resource_as<ShadowMapArrayResource>(output_res);
        if (!shadow_array) {
            tc::Log::error("[ShadowPass] output resource '%s' is missing or is not a shadow_map_array",
                           output_res.c_str());
            if (profile)
        ...
        const SceneRenderServices* services = require_scene_render_services(ctx, "ShadowPass");
        if (!services) {
            if (profile)
                tc_profiler_end_section();
            return;
    ...
    void ShadowPass::register_type() {
        ...
        auto descriptor = FramePassTypeDescriptorBuilder::native<ShadowPass>("ShadowPass", "termin-render-passes");
        auto& inspect = descriptor.inspect();
        _register_inspect_output_res(inspect);
        _register_inspect_caster_offset(inspect);
