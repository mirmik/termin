...
namespace termin {
    ...
    DebugTrianglePass::DebugTrianglePass(const std::string& output, const std::string& pass_name)
        : output_res(output) {
    ...
    void DebugTrianglePass::execute(ExecuteContext& ctx) {
        ...
        if (out_it == ctx.tex2_writes.end() || !out_it->second) {
            tc::Log::error("[DebugTrianglePass] missing tgfx2 output '%s'", output_res.c_str());
            return;
        }
        ...
        static const float vertices[] = {
            0.0f,  -0.7f, 0.0f, 1.0f, 0.15f, 0.10f, 1.0f,  -0.7f, 0.6f, 0.0f, 0.10f,
    ...
    void DebugTrianglePass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<DebugTrianglePass>("DebugTrianglePass", "termin-render-passes");
