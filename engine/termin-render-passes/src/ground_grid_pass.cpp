...
namespace termin {
    ...
    static_assert(sizeof(GridParamsStd140) == 208, "GridParamsStd140 must be 208 bytes for std140 compliance");

    GroundGridPass::GroundGridPass(const std::string& input_res,
    ...
    void GroundGridPass::_ensure_resources(tgfx::IRenderDevice* device) {
        ...
        if (tc_shader_handle_is_invalid(_shader_handle)) {
            _shader_handle = tgfx::register_builtin_shader_from_catalog(GROUND_GRID_ENGINE_SHADER_UUID);
        }
    }
    ...
    void GroundGridPass::register_type() {
        auto descriptor =
            FramePassTypeDescriptorBuilder::native<GroundGridPass>("GroundGridPass", "termin-render-passes");
