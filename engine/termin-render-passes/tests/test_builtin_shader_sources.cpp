...
namespace {
    ...
    void
    check_contract_resource(const tc_shader_contract_view& contract, const char* name, uint32_t kind, uint32_t scope) {
        const tc_shader_resource_requirement* resource = contract_resource(contract, name);
...
TEST_CASE("shadow matrix array uses driver-safe explicit row ABI") {
    ...
    CHECK(source.find("dot(shadow_block.u_light_space_matrix[row + 0], world)") != std::string::npos);
}
...
TEST_CASE("built-in fragment shader registration reads source file from resource root") {
    ...
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() / ("termin-render-passes-builtin-shader-test-" + std::to_string(unique));
    ...
    tc_shader_handle handle =
        tgfx::register_builtin_fragment_shader(kFilename, "TestBuiltinShaderFS", "test-builtin-shader-source-uuid");
...
TEST_CASE("built-in vertex-fragment shader registration reads both source files") {
    const auto unique = std::chrono::steady_clock::now().time_since_epoch().count();
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() / ("termin-render-passes-builtin-vsfs-test-" + std::to_string(unique));
    std::filesystem::remove_all(root);
    ...
    write_text(root / kProgramFilename,
               ...
               "[shader(\"vertex\")] Out vs_main(In input) { Out output; output.position = float4(input.position, "
               "1.0); return output; }\n"
    ...
    tc_shader_handle handle = tgfx::register_builtin_vertex_fragment_shader(
        kProgramFilename, kProgramFilename, "TestBuiltinShaderVSFS", "test-builtin-vsfs-source-uuid");
    REQUIRE(!tc_shader_handle_is_invalid(handle));
...
TEST_CASE("built-in shader catalog registration resolves fragment-only entry by uuid") {
    ...
    write_text(root / "engine-shader-catalog.json",
               R"({
  "version": 1,
  "shaders": [
    {
    ...
    write_text(root / "catalog-fragment-source.frag.glsl",
               "#version 450 core\n"
    ...
    tc_shader_handle handle = tgfx::register_builtin_shader_from_catalog("test-catalog-fragment");
    REQUIRE(!tc_shader_handle_is_invalid(handle));
...
TEST_CASE("built-in shader catalog registration resolves vertex-fragment entry by uuid") {
    const auto unique = std::chrono::steady_clock::now().time_since_epoch().count();
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() / ("termin-render-passes-catalog-vsfs-test-" + std::to_string(unique));
    std::filesystem::remove_all(root);
  ...
  "shaders": [
    ...
    write_text(root / "test-catalog.vert.glsl",
               "#version 450 core\n"
               "// TEST_CATALOG_VERTEX_MARKER\n"
               "layout(location = 0) in vec3 a_position;\n"
               "void main() { gl_Position = vec4(a_position, 1.0); }\n");
    write_text(root / "test-catalog.frag.glsl",
               "#version 450 core\n"
               "// TEST_CATALOG_FRAGMENT_MARKER\n"
               "void main() {}\n");
...
TEST_CASE("shader layout sidecar attaches reflected shader contract") {
  ...
  "shaders": [
    ...
    write_text(root / "sidecar-fragment-source.frag.slang",
    ...
    const std::filesystem::path artifact = artifact_root / "shaders" / "opengl" / "test-sidecar-fragment.frag.glsl";
    write_text(artifact,
               "#version 450 core\n"
               "// TEST_SIDECAR_ARTIFACT_MARKER\n");
    write_text(std::filesystem::path(artifact.string() + ".layout.json"),
               R"({
  "version": 1,
  "language": "slang",
  "target": "opengl",
  ...
  "resources": [
    ...
    tc_shader_handle handle = tgfx::register_builtin_shader_from_catalog("test-sidecar-fragment");
    REQUIRE(!tc_shader_handle_is_invalid(handle));
    tc_shader* shader = tc_shader_get(handle);
    REQUIRE(shader != nullptr);
    ...
    REQUIRE(termin::tgfx2_load_or_compile_shader_artifact_for_backend(
        shader, tgfx::BackendType::OpenGL, tgfx::ShaderStage::Fragment, artifact_bytes));
    ...
    check_contract_resource(
        contract, "u_params", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_UNSCOPED);
    const tc_shader_resource_requirement* params = contract_resource(contract, "u_params");
    REQUIRE(params != nullptr);
    CHECK(params->size == 16);
    REQUIRE(params->field_count == 1);
    CHECK(std::strcmp(params->fields[0].name, "threshold") == 0);
    check_contract_resource(contract, "u_texture", TC_SHADER_RESOURCE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_TRANSIENT);
...
TEST_CASE("real built-in shader artifact sidecar exposes reflected shader contract") {
    ...
    const std::filesystem::path artifact = artifact_root / "shaders" / "opengl" / "termin-engine-tonemap.frag.glsl";
    write_text(artifact,
               "#version 450 core\n"
  ...
  "resources": [
    ...
    tc_shader_handle handle = tgfx::register_builtin_shader_from_catalog("termin-engine-tonemap");
    REQUIRE(!tc_shader_handle_is_invalid(handle));
    tc_shader* shader = tc_shader_get(handle);
    REQUIRE(shader != nullptr);
    ...
    CHECK(contract.source_kind == TC_SHADER_CONTRACT_SOURCE_REFLECTION);
    CHECK(contract.vertex_input_count == 0);
    check_contract_resource(contract, "u_params", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_PASS);
    check_contract_resource(contract, "u_scene_color", TC_SHADER_RESOURCE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_TRANSIENT);
...
TEST_CASE("built-in shader catalog resolves shader program source by uuid") {
  ...
  "shaders": [
    ...
    write_text(root / "catalog-program-source.shader",
               "@program TestCatalogProgram\n"
               "// TEST_CATALOG_PROGRAM_MARKER\n");

    set_builtin_root(root);

    tgfx::BuiltinShaderProgramSource program = tgfx::load_builtin_shader_program_from_catalog("test-catalog-program");
...
TEST_CASE("built-in shader lookup rejects canonical files without catalog manifest") {
    ...
    write_text(root / "test-convention-vsfs.slang",
    ...
    write_text(root / "test-convention-stage.frag.slang",
               "// TEST_CONVENTION_FRAGMENT_STAGE_MARKER\n"
               "[shader(\"fragment\")] float4 fs_main() : SV_Target0 { return float4(1.0); }\n");
    write_text(root / "test-convention-program.shader",
    ...
    tc_shader_handle handle = tgfx::register_builtin_shader_from_catalog("test-convention-vsfs");
    CHECK(tc_shader_handle_is_invalid(handle));

    std::string fragment_stage =
        tgfx::load_builtin_shader_stage_source_from_catalog("test-convention-stage", "fragment");
...
TEST_CASE("engine shader descriptors resolve through the catalog manifest") {
    ...
    write_text(root / "termin-engine-fsq.vert.slang",
               "// TYPED_FSQ_MARKER\n"
               "struct In { float3 position : POSITION; float2 uv : TEXCOORD0; };\n"
               "struct Out { float4 position : SV_Position; float2 uv : TEXCOORD0; };\n"
               "[shader(\"vertex\")] Out vs_main(In input) { Out output; output.position = float4(input.position, "
               "1.0); output.uv = input.uv; return output; }\n");
    write_text(root / "termin-engine-shadow.slang",
               "// TYPED_SHADOW_MARKER\n"
               "struct In { float3 position : POSITION; };\n"
               "struct Out { float4 position : SV_Position; };\n"
               "[shader(\"vertex\")] Out vs_main(In input) { Out output; output.position = float4(input.position, "
               "1.0); return output; }\n"
               "[shader(\"fragment\")] void fs_main() {}\n");
    write_text(root / "termin-engine-tonemap.frag.slang",
    ...
    std::string fsq_vertex = tgfx::load_builtin_shader_stage_source_from_catalog("termin-engine-fsq", "vertex");
    CHECK(fsq_vertex.find("TYPED_FSQ_MARKER") != std::string::npos);

    tc_shader_handle shadow_handle = tgfx::register_builtin_shader_from_catalog("termin-engine-shadow");
    ...
    tc_shader_handle tonemap_handle = tgfx::register_builtin_shader_from_catalog("termin-engine-tonemap");
...
TEST_CASE("built-in shader catalog resolves migrated live engine shaders from canonical sources") {
    ...
    constexpr std::array<ExpectedShader, 45> kExpectedShaders{{
        ...
        {"termin-engine-bloom-blur-vertical", "BloomBlurVerticalFS", false, true},
    ...
    tgfx::BuiltinShaderProgramSource skybox = tgfx::load_builtin_shader_program_from_catalog("termin-engine-skybox");
...
TEST_CASE("shared modular skinned transform owns the bone resource") {
    ...
    CHECK(source.find("ConstantBuffer<TerminBoneBlock> bone_block;") != std::string::npos);
...
TEST_CASE("modular foliage material transform owns instance placement resources") {
    ...
    CHECK(source.find("ConstantBuffer<FoliagePushData> foliage_draw;") != std::string::npos);
    CHECK(source.find("StructuredBuffer<FoliageInstance> foliage_instances;") != std::string::npos);
    CHECK(source.find("termin_foliage_material_world_vertex") != std::string::npos);
    CHECK(source.find("per_frame") == std::string::npos);
}
