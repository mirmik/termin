...
namespace termin::voxels {
    ...
    bool triangle_aabb_intersect(Vec3 v0, Vec3 v1, Vec3 v2, const Vec3& box_center, const Vec3& box_half_size) {
        v0 = v0 - box_center;
        v1 = v1 - box_center;
        v2 = v2 - box_center;
    ...
    VoxelGrid::VoxelGrid(double cell_size, Vec3 origin, const std::string& name, const std::string& source_path)
        : cell_size_(cell_size),
          origin_(origin),
          name_(name),
          source_path_(source_path) {}
    const std::string& VoxelGrid::name() const {
        return name_;
    }
    void VoxelGrid::set_name(const std::string& name) {
        name_ = name;
    }
    const std::string& VoxelGrid::source_path() const {
        return source_path_;
    }
    void VoxelGrid::set_source_path(const std::string& path) {
        source_path_ = path;
    }
    std::tuple<int, int, int> VoxelGrid::world_to_voxel(const Vec3& p) const {
        Vec3 local = (p - origin_) / cell_size_;
        return {static_cast<int>(std::floor(local.x)),
    ...
    std::pair<ChunkKey, std::tuple<int, int, int>> VoxelGrid::voxel_to_chunk(int x, int y, int z) const {
        auto d = [](int a, int b) {
    ...
    uint8_t VoxelGrid::get(int x, int y, int z) const {
        ...
        return i == chunks_.end() ? VOXEL_EMPTY : i->second.get(std::get<0>(l), std::get<1>(l), std::get<2>(l));
    }
    ...
    double VoxelGrid::cell_size() const {
        return cell_size_;
    }
    const Vec3& VoxelGrid::origin() const {
        return origin_;
    }
    size_t VoxelGrid::chunk_count() const {
        return chunks_.size();
    }
    ...
    std::vector<std::tuple<int, int, int, uint8_t>> VoxelGrid::iter_non_empty() const {
        ...
        for (const auto& [k, c] : chunks_) {
            int bx = std::get<0>(k) * CHUNK_SIZE, by = std::get<1>(k) * CHUNK_SIZE, bz = std::get<2>(k) * CHUNK_SIZE;
    ...
    int VoxelGrid::fill_interior(uint8_t value) {
        ...
        for (const auto& [k, c] : chunks_) {
            int x = std::get<0>(k) * CHUNK_SIZE, y = std::get<1>(k) * CHUNK_SIZE, z = std::get<2>(k) * CHUNK_SIZE;
            minx = std::min(minx, x);
            miny = std::min(miny, y);
            minz = std::min(minz, z);
        ...
        for (const auto& t : triangles) {
            ...
            for (int x = x0; x <= x1; x++)
                for (int y = y0; y <= y1; y++)
                    for (int z = z0; z <= z1; z++) {
                        VoxelKey k = {x, y, z};
                        if (s.count(k) && triangle_aabb_intersect(a, b, c, voxel_to_world(x, y, z), hs)) {
    ...
    const std::unordered_map<VoxelKey, std::vector<Vec3>, ChunkKeyHash>& VoxelGrid::surface_normals() const {
    ...
    std::optional<std::pair<VoxelKey, VoxelKey>> VoxelGrid::bounds() const {
        ...
        return std::make_pair(
            VoxelKey{minx * CHUNK_SIZE, miny * CHUNK_SIZE, minz * CHUNK_SIZE},
            VoxelKey{(maxx + 1) * CHUNK_SIZE - 1, (maxy + 1) * CHUNK_SIZE - 1, (maxz + 1) * CHUNK_SIZE - 1});
    }
