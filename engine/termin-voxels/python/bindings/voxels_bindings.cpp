...
NB_MODULE(_voxels_native, m) {
    ...
    nb::class_<VoxelGrid>(m, "VoxelGrid")
        .def(
            "__init__",
            [](VoxelGrid* self, double cell_size, nb::object origin, std::string name, std::string source_path) {
                Vec3 o = Vec3::zero();
                if (!origin.is_none()) {
        ...
        .def_prop_ro(
            "surface_normals",
            [](const VoxelGrid& grid) { return surface_normals_to_dict(grid.surface_normals()); },
            nb::rv_policy::move)
        ...
        .def("get_at_world",
             [](const VoxelGrid& grid, nb::ndarray<double, nb::c_contig, nb::device::cpu> pos) {
                 double* ptr = pos.data();
                 return grid.get_at_world(Vec3(ptr[0], ptr[1], ptr[2]));
             })
        .def("set_at_world",
             [](VoxelGrid& grid, nb::ndarray<double, nb::c_contig, nb::device::cpu> pos, uint8_t value) {
                 double* ptr = pos.data();
        ...
        .def("voxel_to_world",
             [](const VoxelGrid& grid, int vx, int vy, int vz) {
        ...
        // Chunk access
        .def("get_chunk",
             [](const VoxelGrid& grid, int cx, int cy, int cz) -> nb::object {
        ...
        // Bounds
        .def("bounds",
             [](const VoxelGrid& grid) -> nb::object {
                 auto b = grid.bounds();
                 if (!b)
                     return nb::none();
                 auto [min_v, max_v] = *b;
                 return nb::make_tuple(nb::make_tuple(std::get<0>(min_v), std::get<1>(min_v), std::get<2>(min_v)),
        ...
        .def(
            ...
            nb::arg("vertices"),
            nb::arg("triangles"),
            nb::arg("voxel_type") = VOXEL_SOLID)

        .def("fill_interior", &VoxelGrid::fill_interior, nb::arg("fill_value") = VOXEL_SOLID)

        .def("mark_surface", &VoxelGrid::mark_surface, nb::arg("surface_value") = VOXEL_SURFACE)

        .def("clear_by_type", &VoxelGrid::clear_by_type, nb::arg("type_to_clear") = VOXEL_SOLID)

        .def("extract_surface", &VoxelGrid::extract_surface, nb::arg("surface_value") = VOXEL_SOLID)
        ...
        .def("get_surface_normal",
             [](const VoxelGrid& grid, int vx, int vy, int vz) -> nb::object {
        ...
        .def("add_surface_normal",
             [](VoxelGrid& grid, int vx, int vy, int vz, nb::ndarray<double, nb::c_contig, nb::device::cpu> normal) {
                 double* ptr = normal.data();
                 grid.add_surface_normal(vx, vy, vz, Vec3(ptr[0], ptr[1], ptr[2]));
             })

        .def("set_surface_normals",
             [](VoxelGrid& grid, int vx, int vy, int vz, nb::list normals) {
        ...
        .def_static("deserialize", [](const nb::dict& data) { return voxel_grid_from_dict(data); })
        .def_static("direct_deserialize", [](const nb::dict& data) { return voxel_grid_from_dict(data); });
    ...
    nb::class_<TcVoxelGrid>(m, "TcVoxelGrid")
        ...
        .def_prop_ro("grid",
                     [](const TcVoxelGrid& self) -> nb::object {
    ...
    m.def(
        ...
        [](const std::string& uuid, const std::string& name, const std::string& source_path) {
            ...
            return tc_voxel_grid_set_metadata(
                raw, name.empty() ? nullptr : name.c_str(), source_path.empty() ? nullptr : source_path.c_str());
        },
        nb::arg("uuid"),
        nb::arg("name") = "",
        nb::arg("source_path") = "");
    ...
    m.def(
        ...
        [](const std::string& uuid, const std::string& name, const std::string& source_path, const VoxelGrid& payload) {
            TcVoxelGrid grid = TcVoxelGrid::declare(uuid, name);
            tc_voxel_grid* raw = grid.get();
            if (!raw) {
                return false;
    ...
    m.def("register_voxel_grid_kind_handlers",
    ...
    m.def("triangle_aabb_intersect",
          [](nb::ndarray<double, nb::c_contig, nb::device::cpu> v0,
             ...
             nb::ndarray<double, nb::c_contig, nb::device::cpu> half_size) {
              ...
              return triangle_aabb_intersect(Vec3(b0[0], b0[1], b0[2]),
                                             Vec3(b1[0], b1[1], b1[2]),
                                             Vec3(b2[0], b2[1], b2[2]),
                                             Vec3(bc[0], bc[1], bc[2]),
                                             Vec3(bh[0], bh[1], bh[2]));
