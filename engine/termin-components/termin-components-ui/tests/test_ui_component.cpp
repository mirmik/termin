...
root:
  ...
  children:
    ...
    termin::gui_native::TcUiDocumentAsset declare_asset(const char* uuid = "ui-component-test") {
...
TEST_CASE("UIComponent publishes independent native document snapshots") {
    ...
    CHECK_EQ(tc_component_get_input_priority(first.tc_component_ptr()), 7);
    CHECK_EQ(tc_component_get_input_source_mask(first.tc_component_ptr()),
             static_cast<uint32_t>(TC_INPUT_SOURCE_RUNTIME));

    tc_scene_ui_document_snapshot snapshot{};
    REQUIRE(tc_scene_ui_document_snapshot_get(first.tc_component_ptr(), &snapshot));
    CHECK(tc_ui_document_handle_eq(snapshot.document, first.document().handle()));
    ...
    first.set_input_source_mask_value(TC_INPUT_SOURCE_RUNTIME | TC_INPUT_SOURCE_EDITOR);
    REQUIRE(tc_scene_ui_document_snapshot_get(first.tc_component_ptr(), &snapshot));
    CHECK_EQ(snapshot.priority, 19);
    CHECK_EQ(snapshot.input_source_mask, static_cast<uint32_t>(TC_INPUT_SOURCE_RUNTIME | TC_INPUT_SOURCE_EDITOR));
    CHECK_EQ(tc_component_get_capability_priority(first.tc_component_ptr(), tc_scene_ui_document_capability_id()), 19);
    first.set_input_source_mask_value(1u << 12u);
    CHECK_EQ(first.input_source_mask(), static_cast<uint32_t>(TC_INPUT_SOURCE_RUNTIME | TC_INPUT_SOURCE_EDITOR));
...
TEST_CASE("UIComponent asset replacement and failed reload are transactional") {
    ...
    const tc_widget_handle original_root = tc_ui_document_root_at(original.handle(), 0);

    CHECK_FALSE(asset.reload_source("uiscript: 2\nroot: {type: termin.gui.Missing}\n"));
    ...
    const std::string replacement = std::string(kSource).replace(
        std::string(kSource).find("name: action"), std::string("name: action").size(), "name: replacement");
    REQUIRE(asset.reload_source(replacement));
    REQUIRE(component.reload_document());
    CHECK(component.document() == original);
    const tc_widget_handle replacement_root = tc_ui_document_root_at(original.handle(), 0);
...
TEST_CASE("UIComponent native inspect serialization uses a typed asset ref") {
    ...
    CHECK(tc_component_registry_has_capability("UIComponent", tc_scene_ui_document_capability_id()));
    CHECK(tc_component_registry_has_capability("UIComponent", tc_input_capability_id()));

    tc_component* raw = tc_component_registry_create("UIComponent");
    REQUIRE(raw != nullptr);
    auto* component = dynamic_cast<UIComponent*>(CxxComponent::from_tc(raw));
    ...
    tc_value* input_source_mask = tc_value_dict_get(&data, "input_source_mask");
    REQUIRE(priority != nullptr);
    REQUIRE(input_source_mask != nullptr);
    CHECK_EQ(priority->data.i, static_cast<int64_t>(23));
    CHECK_EQ(input_source_mask->data.i, static_cast<int64_t>(TC_INPUT_SOURCE_EDITOR));
    ...
    CHECK_EQ(restored.input_source_mask(), static_cast<uint32_t>(TC_INPUT_SOURCE_EDITOR));
...
TEST_CASE("UIComponent handles viewport-local pointer streams and focus teardown") {
    ...
    const TcUiDocumentAsset asset = TcUiDocumentAsset::declare_source(
        "ui-component-input", "Component Input UI", "UI/component-input.uiscript", kInputSource);
    UIComponent component;
    REQUIRE(component.set_ui_layout_uuid(asset.uuid()));
    ...
    tc_widget* root = tc_ui_document_resolve_widget(document.handle(), tc_ui_document_root_at(document.handle(), 0));
    ...
    tc_input_focus_event_init_source(&focus_lost, TC_VIEWPORT_HANDLE_INVALID, TC_INPUT_SOURCE_RUNTIME);
...
TEST_CASE("World UI surface projects a scene ray and routes the native pointer stream") {
    ...
    TcUiDocumentAsset::clear_registry_for_tests();
    const TcUiDocumentAsset asset = TcUiDocumentAsset::declare_source(
        "world-ui-surface-input", "World UI Surface Input", "UI/world-surface-input.uiscript", kInputSource);
    REQUIRE(asset.valid());

    TcSceneRef scene = TcSceneRef::create("world-ui-surface-test");
    Entity ui_entity = Entity::create_with_uuid(scene.entity_pool_handle(), "Panel UI", "world-ui-target");
    ...
    REQUIRE(tc_component_has_capability(surface->tc_component_ptr(), tc_world_pointer_surface_capability_id()));
    ...
    REQUIRE(tc_world_pointer_surface_project_ray(surface->tc_component_ptr(), &ray, &hit));
    CHECK(hit.inside);
    ...
    tc_world_pointer_surface_dispatch_pointer(surface->tc_component_ptr(), &pointer);
    pointer.phase = TC_WORLD_POINTER_DOWN;
    pointer.pressure = 1.0f;
    REQUIRE(tc_world_pointer_surface_dispatch_pointer(surface->tc_component_ptr(), &pointer));
    CHECK_FALSE(tc_widget_handle_is_invalid(document.pointer_capture()));
