...
namespace termin {
    ...
    void bind_orbit_camera_controller(nb::module_& m) {
        nb::class_<OrbitCameraController, CxxComponent>(m, "OrbitCameraController")
            .def(
                "__init__",
                [](nb::handle self, double radius, double min_radius, double max_radius, bool prevent_moving) {
            ...
            .def("fly_rotate",
                 &OrbitCameraController::fly_rotate,
                 nb::arg("yaw"),
                 nb::arg("pitch"),
                 nb::arg("roll") = 0.0,
            ...
            .def_prop_rw(
                "_prevent_moving", // Python uses _prevent_moving
                [](OrbitCameraController& c) { return c.prevent_moving(); },
                [](OrbitCameraController& c, bool v) { c.set_prevent_moving(v); })
            ...
            .def_prop_ro("camera_component",
                         [](OrbitCameraController& c) -> nb::object {
            ...
            .def(
                "on_mouse_button",
                [](OrbitCameraController& c, MouseButtonEvent& e) { c.on_mouse_button(&e); },
                nb::arg("event"))
            .def(
                "on_mouse_move",
                [](OrbitCameraController& c, MouseMoveEvent& e) { c.on_mouse_move(&e); },
                nb::arg("event"))
            .def(
                "on_scroll", [](OrbitCameraController& c, ScrollEvent& e) { c.on_scroll(&e); }, nb::arg("event"))

            // c_component_ptr for compatibility
            .def("c_component_ptr",
                 [](OrbitCameraController& c) -> uintptr_t { return reinterpret_cast<uintptr_t>(c.c_component()); });
    }
