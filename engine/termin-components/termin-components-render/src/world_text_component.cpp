...
namespace termin {
    namespace {
        ...
        tc_render_state make_text_render_state(const WorldTextComponent& component) {
            tc_render_state state = component.blend ? tc_render_state_transparent() : tc_render_state_opaque();
        ...
        bool world_text_render_item_draw_encoder(tgfx::RenderContext2& ctx,
            ...
            if (item.kind != TC_RENDER_ITEM_KIND_TEXT_BATCH) {
                tc::Log::error("[WorldTextComponent] text encoder received unsupported item kind %u", item.kind);
                return false;
            }
            ...
            if (!renderer) {
                ...
                tc::Log::error("[WorldTextComponent] text RenderItem component is not WorldTextComponent: type='%s'",
    ...
    void WorldTextComponent::register_type() {
        ...
        inspect.add_with_accessors<WorldTextComponent, tc_vec3>(
            ...
            [](WorldTextComponent* self, tc_vec3 value) { self->set_local_offset(Vec3{value.x, value.y, value.z}); });
        inspect.add_with_accessors<WorldTextComponent, tc_vec3>(
            "WorldTextComponent",
            "plane_normal",
            "Plane Normal",
            ...
            [](WorldTextComponent* self, tc_vec3 value) { self->set_plane_normal(Vec3{value.x, value.y, value.z}); });
        inspect.add_with_accessors<WorldTextComponent, tc_vec3>(
            "WorldTextComponent",
            "text_up",
            "Text Up",
        ...
        inspect.add_with_accessor_choices<WorldTextComponent, int>(
            ...
            [](WorldTextComponent* self, int value) {
                self->set_orientation(static_cast<WorldTextOrientation>(value));
            },
            {
                {"0", "Billboard"},
                {"1", "Fixed"},
    ...
    void WorldTextComponent::set_orientation(WorldTextOrientation value) {
        ...
        case WorldTextOrientation::Billboard:
            ...
            break;
        default:
            tc::Log::error("[WorldTextComponent] invalid orientation %d; using Billboard", static_cast<int>(value));
    ...
    tc_material_phase* WorldTextComponent::sync_material_phase() const {
        ...
        if (!material_.is_valid()) {
            ...
            tc_shader_handle shader_handle = tgfx::register_builtin_shader_from_catalog(TEXT3D_SHADER_UUID);
        ...
        if (resolved_phase == TC_PHASE_NONE) {
            tc::Log::error("[WorldTextComponent] phase '%s' is not present in the project registry", mark.c_str());
            return nullptr;
        }
    ...
    tgfx::FontAtlas* WorldTextComponent::ensure_font(const char* captured_font_path) const {
        const std::string captured_path =
            captured_font_path && captured_font_path[0] != '\0' ? std::string(captured_font_path) : std::string();
    ...
    bool WorldTextComponent::encode_render_item_tgfx2(tgfx::RenderContext2& ctx2,
                                                      const tc_render_item& item,
                                                      const RenderItemDrawSubmitRequest& request) {
        if (item.kind != TC_RENDER_ITEM_KIND_TEXT_BATCH) {
            tc::Log::error("[WorldTextComponent] cannot encode item kind %u as text batch", item.kind);
            return false;
        }
        ...
        if (!decode_text_orientation(item.payload.text_batch.orientation, decoded_orientation)) {
            tc::Log::error("[WorldTextComponent] cannot encode text batch with invalid orientation %u",
        ...
        if (context.has_override_color) {
            payload_color = tc_render_item_vec4{
                context.override_color.x, context.override_color.y, context.override_color.z, context.override_color.w};
        }

        WorldTextAnchor decoded_anchor = WorldTextAnchor::Center;
        if (!decode_text_anchor(item.payload.text_batch.anchor, decoded_anchor)) {
            tc::Log::error("[WorldTextComponent] cannot encode text batch with invalid anchor %u",
