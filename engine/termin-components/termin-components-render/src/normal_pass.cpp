...
namespace termin {
    ...
    namespace {
        ...
        static_assert(sizeof(NormalPerFrameStd140) == 128, "NormalPerFrameStd140 must be 2 * mat4");
        ...
        static_assert(sizeof(NormalDrawStd140) == 64, "NormalDrawStd140 must be exactly one mat4");
        ...
        VertexOutputAdapter normal_vertex_output_adapter() {
            ...
            adapter.source_module = {"termin_normal_vertex_output_adapter",
    ...
    tc_shader_handle NormalPass::shader_usage_base_shader() const {
        if (tc_shader_handle_is_invalid(normal_shader_handle_)) {
            normal_shader_handle_ = tgfx::register_builtin_shader_from_catalog(NORMAL_ENGINE_SHADER_UUID);
        }
    ...
    // ----------------------------------------------------------------------------
    void NormalPass::execute_with_data_tgfx2(ExecuteContext& ctx,
                                             const Rect2i& rect,
        ...
        if (color_it == ctx.tex2_writes.end() || !color_it->second) {
            tc::Log::error("NormalPass/tgfx2: missing tgfx2 color texture for '%s'", output_res.c_str());
            return;
        }
        ...
        if (!ensure_material_pipeline_shader(*ctx.ctx2, device, normal_shader_handle_, "NormalPass", normal_shader)) {
            return;
        }
        ...
        for (const auto& dc : cached_draw_calls_) {
            ...
            tc_material_phase* material_phase = tc_shader_handle_eq(dc.final_shader.handle, normal_shader_handle_)
                                                    ? nullptr
    ...
    void NormalPass::execute(ExecuteContext& ctx) {
        const SceneRenderServices* services = require_scene_render_services(ctx, "NormalPass");
        if (!services) {
            return;
        }
        ...
        if (!camera_name.empty()) {
            if (!resolve_named_render_camera_for_pass(
                    scene, camera_name.c_str(), 0.0, "NormalPass", named_camera_snapshot)) {
                return;
            }
        ...
        execute_with_data_tgfx2(ctx, rect, scene, view, projection, camera_layer_mask, camera_render_category_mask);
    }
