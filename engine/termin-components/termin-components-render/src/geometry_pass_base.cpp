...
namespace termin {
    ...
    namespace {
        ...
        RenderItemTaskPlanningContract geometry_task_planning_contract(
            ...
            if (phase == TC_PHASE_ID) {
                contract.provided_input_mask |= render_item_task_input_bit(RenderItemTaskInput::OverrideColor);
            }
        ...
        bool plan_geometry_item_shader(const tc_render_item& item,
    ...
    GeometryPassBase::GeometryPassBase(const std::string& name, const std::string& input, const std::string& output)
        : input_res(input),
          output_res(output) {
        set_pass_name(name);
    }
    ...
    void GeometryPassBase::collect_scene_shader_usages(tc_scene_handle scene,
                                                       const std::function<void(TcShader)>& emit) const {
        if (!emit) {
            return;
        }
        if (!tc_scene_handle_valid(scene)) {
            tc::Log::error("[GeometryPassBase] pass '%s' cannot collect shader usages for invalid scene",
                           get_pass_name().c_str());
            return;
        }
        ...
        if (collect_phase == TC_PHASE_NONE) {
            tc::Log::error("[GeometryPassBase] pass '%s' requests unregistered phase '%s'",
                           get_pass_name().c_str(),
                           collect_phase_mark);
            return;
        }
        ...
        if (tc_shader_handle_is_invalid(base_shader)) {
            tc::Log::error("[GeometryPassBase] pass '%s' cannot collect shader usages without a valid base shader",
                           get_pass_name().c_str());
            return;
        }
        ...
        auto callback = [](tc_component* c, void* user_data) -> bool {
            ...
            for (const tc_render_item& item : items.items) {
                ...
                RenderTaskList tasks;
                if (!plan_geometry_item_shader(item,
