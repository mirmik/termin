        ...
        void register_light_shadow_fields(tc::InspectFacetBuilder& builder) {
...

            {
                ...
                info.setter = [](void* obj, tc_value value, void*) -> bool {
                    ...
                    if (value.type == TC_VALUE_DOUBLE)
                        c->shadows.bias = value.data.d;
                    if (value.type == TC_VALUE_FLOAT)
                        c->shadows.bias = static_cast<double>(value.data.f);
                    if (value.type == TC_VALUE_INT)
                        c->shadows.bias = static_cast<double>(value.data.i);
...

            {
                ...
                info.setter = [](void* obj, tc_value value, void*) -> bool {
                    auto* c = static_cast<LightComponent*>(obj);
                    if (value.type == TC_VALUE_DOUBLE)
                        c->shadows.normal_bias = value.data.d;
                    if (value.type == TC_VALUE_FLOAT)
                        c->shadows.normal_bias = static_cast<double>(value.data.f);
                    if (value.type == TC_VALUE_INT)
                        c->shadows.normal_bias = static_cast<double>(value.data.i);
                    return value.type == TC_VALUE_DOUBLE || value.type == TC_VALUE_FLOAT || value.type == TC_VALUE_INT;
                };
                (void)builder.add_field(std::move(info));
            }
