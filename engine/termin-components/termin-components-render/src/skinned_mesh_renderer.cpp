...
namespace termin {
    ...
    void SkinnedMeshRenderer::update_bone_matrices() {
        resolve_skeleton_controller();
        SkeletonController* controller = _skeleton_controller.get();
        SkeletonInstance* si = controller ? controller->skeleton_instance() : nullptr;
        if (si == nullptr) {
            _bone_count = 0;
        ...
        if (!entity().valid() || !controller->update_skeleton_instance(entity())) {
            tc::Log::error("[SkinnedMeshRenderer::update_bone_matrices] failed to update skeleton instance");
            _bone_count = 0;
            _bone_matrices_flat.clear();
            return;
        }
