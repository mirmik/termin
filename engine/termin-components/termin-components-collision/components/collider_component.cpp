...
namespace termin {
    ...
    namespace {
        ...
        bool entity_world_matrix(const tc_component& component, Mat44f& result) {
            if (!tc_entity_handle_valid(component.owner)) {
                tc::Log::error("ColliderComponent::prepare_render: component owner is invalid");
                return false;
            }
            tc_entity_pool* pool = tc_entity_pool_registry_get(component.owner.pool);
            if (!pool) {
                tc::Log::error("ColliderComponent::prepare_render: entity pool is unavailable");
                return false;
            }
            double matrix[16];
        ...
        Vec3 matrix_column(const Mat44f& matrix, int column) {
            ...
            return Vec3(matrix.data[offset], matrix.data[offset + 1], matrix.data[offset + 2]);
        }
    ...
    void ColliderComponent::prepare_render(const RenderPrepareContext& context) {
        DebugGeometryDrawer drawer = context.debug_geometry(collider_debug_geometry_type().type_id());
        if (!drawer || !_collider)
            return;

        Mat44f entity_world;
        if (!entity_world_matrix(_c, entity_world))
            return;
        const GeneralPose3& local_pose = _collider->transform;
        const Mat44f local = Mat44f::compose(local_pose.lin, local_pose.ang, local_pose.scale);
        ...
        case colliders::ColliderType::Sphere: {
            const auto* sphere = static_cast<const colliders::SphereCollider*>(_collider.get());
            const double scale = std::min({axis_x.norm(), axis_y.norm(), axis_z.norm()});
            drawer.wire_sphere(center, sphere->radius * scale, collider_debug_color, 32, false);
            break;
        }
        case colliders::ColliderType::Capsule: {
            const auto* capsule = static_cast<const colliders::CapsuleCollider*>(_collider.get());
            const double z_scale = axis_z.norm();
            if (z_scale <= 1.0e-12)
                return;
            const Vec3 half_axis = axis_z * capsule->half_height;
            const double radius = capsule->radius * std::min(axis_x.norm(), axis_y.norm());
            drawer.wire_capsule(center - half_axis, center + half_axis, radius, collider_debug_color, 32, false);
            break;
        }
    ...
    void ColliderComponent::_rebuild_collider(bool report_failure) {
        ...
        if (!_collider) {
            _build_state = source_pending ? BuildState::PendingSource : BuildState::InvalidSource;
            if (report_failure) {
        ...
        if (_transform.valid()) {
            ...
            _attached = std::make_unique<colliders::AttachedCollider>(_collider.get(), &_transform, entity_id);
    ...
    std::unique_ptr<colliders::ColliderPrimitive> ColliderComponent::_create_collider(std::string& failure_reason,
        ...
        if (collider_type == "Box") {
            ...
            if (convex_hull_mesh_source == "MeshComponent") {
                ...
                if (!mesh_component) {
                    failure_reason = "ConvexHull MeshComponent source requires MeshComponent on the same entity";
                    source_pending = true;
                    return nullptr;
                }
            ...
            return std::make_unique<colliders::ConvexHullCollider>(colliders::ConvexHullCollider::from_points(points));
        } else {
    ...
    void ColliderComponent::_subscribe_to_scene_events() {
        ...
        _mesh_changed_subscription =
            tc_event_bus_subscribe(bus, TC_EVENT_MESH_COMPONENT_CHANGED, &_mesh_component_changed_callback, this);
        ...
        _structure_changed_subscription =
            tc_event_bus_subscribe(bus, TC_EVENT_SCENE_STRUCTURE_CHANGED, &_scene_structure_changed_callback, this);
    ...
    void ColliderComponent::_mesh_component_changed_callback(const tc_event* event, void* user_data) {
    ...
    void ColliderComponent::_scene_structure_changed_callback(const tc_event* event, void* user_data) {
        auto* self = static_cast<ColliderComponent*>(user_data);
        if (self) {
    ...
    void ColliderComponent::_handle_scene_structure_changed(const tc_event* event) {
        ...
        if (!payload || payload->entity.index != _c.owner.id.index ||
            payload->entity.generation != _c.owner.id.generation || !payload->component_type ||
    ...
    void ColliderComponent::_add_to_collision_world() {
        ...
        if (!cw && tc_scene_ext_is_registered(TC_SCENE_EXT_TYPE_COLLISION_WORLD)) {
            if (!tc_scene_ext_attach(_scene_handle, TC_SCENE_EXT_TYPE_COLLISION_WORLD)) {
                tc::Log::error("ColliderComponent: failed to attach CollisionWorld to scene");
                return;
            }
        ...
        if (cw) {
            ...
            tc::Log::error("ColliderComponent: scene has no registered CollisionWorld");
        }
    }
