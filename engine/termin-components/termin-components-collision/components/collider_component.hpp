...
namespace termin {
    ...
    public:
        ...
        // Accessors
        colliders::ColliderPrimitive* collider() const {
            return _collider.get();
        }
        colliders::AttachedCollider* attached_collider() const {
            return _attached.get();
        }
        uint64_t collider_revision() const {
            return _collider_revision;
        }
    ...
    private:
        ...
        std::unique_ptr<colliders::ColliderPrimitive> _create_collider(std::string& failure_reason,
