...
namespace termin::bootstrap {
    namespace {
        ...
        template <typename H> nb::object serialize_uuid_handle(nb::object obj, const char* kind_name) {
        ...
        template <typename H> nb::object deserialize_uuid_handle(nb::object data) {
        ...
        template <typename H> void register_python_uuid_handle_kind(const char* kind_name, nb::handle type_obj) {
            tc::KindRegistry::instance().register_type(type_obj, kind_name);
            tc::KindRegistry::instance().register_python(
                kind_name,
                nb::cpp_function(
                    [kind_name](nb::object obj) -> nb::object { return serialize_uuid_handle<H>(obj, kind_name); }),
                nb::cpp_function([](nb::object data) -> nb::object { return deserialize_uuid_handle<H>(data); }));
        }
        ...
        void py_input_cb_on_pointer(void* py_self, tc_pointer_event* event) {
            PyGILState_STATE gstate = PyGILState_Ensure();
            try {
                nb::handle self((PyObject*)py_self);
                PointerEvent cpp_event(*event);
                nb::object py_event = nb::cast(cpp_event);
                self.attr("on_pointer")(py_event);
                event->handled = nb::cast<bool>(py_event.attr("handled"));
            } catch (const std::exception& e) {
                tc::Log::error(e, "InputHandler::on_pointer");
                PyErr_Print();
            }
            PyGILState_Release(gstate);
        }
    ...
    void init_python_kind_handlers(const RuntimeKindOptions& options) {
        ...
        if (options.animation && !g_animation_python_kind_initialized) {
            ...
            register_python_uuid_handle_kind<animation::TcAnimationClip>("tc_animation_clip",
        ...
        if (options.voxel_grid && !g_voxel_grid_python_kind_initialized) {
            ...
            register_python_uuid_handle_kind<voxels::TcVoxelGrid>("voxel_grid_handle",
                                                                  voxels_module.attr("TcVoxelGrid"));
    ...
    void init_python_component_callbacks() {
        ...
        tc_python_input_callbacks input_callbacks = {
            .on_pointer = py_input_cb_on_pointer,
