...
namespace termin::runtime {
    namespace {
        ...
        constexpr std::array<std::uint8_t, 8> blob_magic{'T', 'R', 'P', 'K', 'G', '0', '1', '\n'};
        ...
        std::string validate_path(std::string_view path) {
            ...
            if (path.front() == '/' || path.find('\\') != std::string_view::npos ||
                ...
                throw std::runtime_error("runtime package path must be portable and relative: " + std::string(path));
            }
            std::size_t start = 0;
            while (start <= path.size()) {
                ...
                if (component.empty() || component == "." || component == "..") {
                    throw std::runtime_error("runtime package path must not contain empty or dot segments: " +
                                             std::string(path));
                }
        ...
        std::string sha256(std::span<const std::uint8_t> input) {
            ...
            for (int index = 0; index < 8; ++index) {
                tail[tail_size - 1 - index] = static_cast<std::uint8_t>(bits >> (index * 8));
            }
            ...
            for (std::size_t index = 0; index < state.size(); ++index) {
                for (int nibble = 0; nibble < 8; ++nibble) {
                    result[index * 8 + nibble] = hex[(state[index] >> ((7 - nibble) * 4)) & 0xf];
                }
            }
            return result;
        }
        ...
        public:
            ...
            RuntimePackageBytes read(std::string_view path) const override {
                ...
                if (end < 0)
                    throw std::runtime_error("failed to determine file size: " + resolved.string());
                ...
                if (!input)
                    throw std::runtime_error("failed to read file: " + resolved.string());
            ...
            std::string materialized_path(std::string_view path) const override {
                return resolve(path).string();
            }
        ...
        private:
            std::filesystem::path resolve(std::string_view path) const {
                ...
                std::error_code error;
                const std::filesystem::path result = std::filesystem::weakly_canonical(root_ / normalized, error);
        ...
        public:
            BlobReader(std::shared_ptr<const std::vector<std::uint8_t>> blob, std::string label)
                ...
                const std::string header(reinterpret_cast<const char*>(blob_->data() + header_offset), header_size);
                ...
                if (!version || !version->is_numer() || version->as_numer() != 1 || !entries || !entries->is_list()) {
                ...
                for (const nos::trent& value : entries->as_list()) {
                    ...
                    if (digest.size() != 64 || offset != expected_offset || offset > payload_size ||
                    ...
                    const auto bytes = std::span<const std::uint8_t>(blob_->data() + payload_offset_ + offset, size);
            ...
            bool contains(std::string_view path) const override {
                try {
                    return entries_.contains(validate_path(path));
                } catch (...) {
                    return false;
                }
            }
            ...
            std::string materialized_path(std::string_view) const override {
                return {};
            }
    ...
    std::shared_ptr<RuntimePackageReader> open_runtime_package_directory(const std::string& root_path) {
        return std::make_shared<DirectoryReader>(root_path);
    }

    std::shared_ptr<RuntimePackageReader>
    open_runtime_package_blob(std::shared_ptr<const std::vector<std::uint8_t>> blob, std::string label) {
