...
namespace {
    ...
    int validate_library(const char* path, const std::string& expected_module_id, bool inspect) {
#ifdef _WIN32
        ...
        if (executable_path_size == 0 || executable_path_size >= std::size(executable_path)) {
            std::cerr << "Failed to resolve native validator directory, error code " << GetLastError() << "\n";
            return 2;
        }
        const std::filesystem::path validator_directory = std::filesystem::path(executable_path).parent_path();
        if (!SetDllDirectoryW(validator_directory.c_str())) {
            std::cerr << "Failed to add native validator directory to DLL search path: '"
                      << validator_directory.string() << "', error code " << GetLastError() << "\n";
            return 2;
        }
        std::filesystem::path load_path = std::filesystem::absolute(path);
        load_path.make_preferred();
        HMODULE handle = LoadLibraryExW(load_path.c_str(), nullptr, LOAD_WITH_ALTERED_SEARCH_PATH);
        if (!handle) {
            std::cerr << "LoadLibrary failed for '" << load_path.string() << "' with error code " << GetLastError()
                      << "\n";
            return 2;
        }
        ...
        if (!handle) {
            ...
            std::cerr << "dlopen failed for '" << path << "': " << (error ? error : "unknown error") << "\n";
            return 2;
        }
        const auto* descriptor = reinterpret_cast<const termin_native_module_descriptor_v1_data*>(
        ...
        const auto validation = termin_modules::validate_native_module_descriptor_v1(descriptor, expected_module_id);
        ...
        if (inspect) {
            std::cout << "{\"module_id\":\"" << json_escape(descriptor->module_id) << "\",\"module_version\":\""
                      << json_escape(descriptor->module_version) << "\",\"build_id\":\""
                      << json_escape(descriptor->build_id)
