...
NB_MODULE(_termin_modules_native, m) {
    nb::enum_<ModuleKind>(m, "ModuleKind").value("Cpp", ModuleKind::Cpp).value("Python", ModuleKind::Python);
    ...
    nb::class_<ModuleEvent>(m, "ModuleEvent")
        ...
        .def_prop_ro("message", [](const ModuleEvent& self) { return sanitize_external_text(self.message); });
    ...
    nb::class_<ModuleRecord>(m, "ModuleRecord")
        ...
        .def_prop_ro("error_message",
                     [](const ModuleRecord& self) { return sanitize_external_text(self.error_message); })
        .def_prop_ro("diagnostics", [](const ModuleRecord& self) { return sanitize_external_text(self.diagnostics); })
    ...
    nb::class_<ModuleRuntime>(m, "ModuleRuntime")
        ...
        .def("register_cpp_backend",
             [](ModuleRuntime& self, const CppModuleBackend&) {
                 self.register_backend(std::make_shared<CppModuleBackend>());
             })
        .def("register_python_backend",
             [](ModuleRuntime& self, const PythonModuleBackend&) {
        ...
        .def(
            "find",
            [](const ModuleRuntime& self, const std::string& module_id) -> const ModuleRecord* {
        ...
        .def(
            "resolve_closure",
            [](ModuleRuntime& self, const std::vector<std::string>& roots) {
        ...
        .def_prop_ro("last_error", [](const ModuleRuntime& self) { return sanitize_external_text(self.last_error()); })
        .def(
            "set_event_callback",
            [](ModuleRuntime& self, nb::callable callback) {
