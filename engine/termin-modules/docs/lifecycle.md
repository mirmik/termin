...
- component и WorldController классы во время импорта создают только
...
- после успешного prepare `module_context` проходит упорядоченный список
  ...
  WorldController declarations, Python kinds, runtime types и package claims
...
автоматически запускают editor через канонический
`scripts/termin-editor-virtual-display`, если нет активного display.
Они также входят в центральный `task test --` после C++ и Python тестов.
Для окружений без editor SDK/editor MCP можно временно пропустить эту стадию
через `task test -- --no-editor-smoke`.
