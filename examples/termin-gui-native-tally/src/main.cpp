...
std::string resolve_font_path() {
  ...
  throw std::runtime_error(
      "cannot find UI font; set TERMIN_UI_FONT or TERMIN_SDK");
}
...
public:
  TallyApplication()
      : session_(termin::create_native_windowed_graphics()),
        ...
        handle_(windows_->create_window(
            termin::WindowConfig{std::string(kApplicationName), 420, 260})),
        document_handle_(tc_ui_document_create()), document_(document_handle_),
        adapter_(std::make_unique<ui::GuiWindowAdapter>(
            session_->graphics(), document_,
            ui::DocumentRendererConfig{resolve_font_path(), 16,
                                       {0.025f, 0.03f, 0.04f, 1.0f}, true},
  ...
  int run(std::optional<size_t> frame_limit) {
    while (!windows_->window(handle_).should_close() &&
           (!frame_limit.has_value() ||
            adapter_->renderer().rendered_frame_count() < *frame_limit)) {
...
private:
  ...
  void refresh_count() {
    ...
    windows_->window(handle_).set_title(
