...
Цель — применить ту же модель к tcgui: общее C-ядро `tc_widget` встраивается в
C++/Python/другую языковую реализацию, а `tc_ui_document` владеет принятым
виджетом и его деревом через единый deleter. Внешние и межвиджетные ссылки
используют `WidgetHandle`; обёртка уже существующего native-виджета держит
document + handle, не создавая второй объект состояния.
