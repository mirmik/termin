...
namespace {
    ...
    void delete_native_with_state(tc_render_surface* surface) {
        AdapterState* adapter = state(surface);
        auto* native =
            reinterpret_cast<NativeSurface*>(reinterpret_cast<char*>(adapter) - offsetof(NativeSurface, state));
        (*native->delete_count)++;
        delete native;
    }
...
int main() {
    ...
    tc_render_surface* external =
        tc_render_surface_new_external(&adapter, &valid_vtable, sizeof(valid_vtable), TC_RENDER_SURFACE_ABI_VERSION);
    ...
    assert(tc_render_surface_new_external(&adapter, &malformed, sizeof(malformed), TC_RENDER_SURFACE_ABI_VERSION) ==
           nullptr);
