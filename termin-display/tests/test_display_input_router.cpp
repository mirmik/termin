...
namespace {
    ...
    uint32_t surface_get_color_texture_id(tc_render_surface*) {
        return 1;
    }
    uintptr_t surface_get_graphics_domain_key(tc_render_surface*) {
        return 1;
    }
    ...
    void count_pointer(tc_input_manager* self, uint64_t pointer_id, int, int phase, double x, double y, float) {
        auto* input = reinterpret_cast<CountingInput*>(self->userdata);
...
int main() {
    ...
    tc_render_surface_init(&fixed_surface->surface, &fixed_surface_vtable, surface_delete);
    ...
    tc_display_dispatch_pointer_button(display, 25.0, 50.0, TC_MOUSE_BUTTON_LEFT, TC_INPUT_PRESS, 0, 2);
    ...
    tc_display_dispatch_pointer_button(display, 75.0, 50.0, TC_MOUSE_BUTTON_LEFT, TC_INPUT_RELEASE, 0, 2);

    if (left_input.presses != 1 || left_input.releases != 1 || right_input.presses != 0 || right_input.releases != 0 ||
    ...
    if (right_input.last_x != 25.0 || right_input.last_y != 50.0) {
        std::fprintf(stderr, "window bridge did not route framebuffer coordinates in viewport-local space\n");
        return 1;
    }
