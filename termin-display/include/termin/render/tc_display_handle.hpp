...
namespace termin {
    ...
    public:
        ...
        TcDisplay() = default;
        explicit TcDisplay(tc_display_handle handle)
            : handle_(handle) {}
        ...
        bool is_valid() const {
            return tc_display_alive(handle_);
        }
        tc_display_handle handle() const {
            return handle_;
        }

        bool destroy() {
            if (!tc_display_handle_valid(handle_))
                return false;
            bool destroyed = tc_display_free(handle_);
            if (destroyed)
                handle_ = TC_DISPLAY_HANDLE_INVALID;
            return destroyed;
        }
        ...
        void set_name(const std::string& value) {
            tc_display_set_name(handle_, value.c_str());
        }
        ...
        void set_uuid(const std::string& value) {
            tc_display_set_uuid(handle_, value.c_str());
        }

        bool editor_only() const {
            return tc_display_get_editor_only(handle_);
        }
        void set_editor_only(bool value) {
            tc_display_set_editor_only(handle_, value);
        }

        bool enabled() const {
            return tc_display_get_enabled(handle_);
        }
        void set_enabled(bool value) {
            tc_display_set_enabled(handle_, value);
        }
        ...
        tc_render_surface* surface() const {
            return tc_display_get_surface(handle_);
        }
        ...
        bool resize(int width, int height) {
            return tc_display_resize(handle_, width, height);
        }
        ...
        bool dispatch_pointer_button(double x, double y, int button, int action, int mods, uint32_t click_count) {
            return tc_display_dispatch_pointer_button(handle_, x, y, button, action, mods, click_count);
        }
        ...
        size_t viewport_count() const {
            return tc_display_get_viewport_count(handle_);
        }
        ...
        void update_all_pixel_rects() {
            tc_display_update_all_pixel_rects(handle_);
        }

        static TcDisplay create(tc_render_surface* surface, const std::string& name = "Display") {
            return TcDisplay(surface, name);
        }
        static TcDisplay from_handle(tc_display_handle handle) {
            return TcDisplay(handle);
        }
    };
