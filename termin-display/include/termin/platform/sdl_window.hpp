...
namespace termin {
     ...
     */
    class TERMIN_DISPLAY_API SDLWindow {
    public:
        // Callback types
    ...
    public:
        ...
        bool should_close() const {
            return should_close_ || window_ == nullptr;
        }
        ...
        // Callbacks
        void set_framebuffer_size_callback(FramebufferSizeCallback cb) {
            framebuffer_size_callback_ = std::move(cb);
        }
        void set_cursor_pos_callback(CursorPosCallback cb) {
            cursor_pos_callback_ = std::move(cb);
        }
        ...
        static int translate_sdl_mods(Uint16 sdl_mods) {
            ...
            int result = 0;
            if (sdl_mods & (KMOD_LSHIFT | KMOD_RSHIFT))
                result |= 0x0001;
            if (sdl_mods & (KMOD_LCTRL | KMOD_RCTRL))
                result |= 0x0002;
            if (sdl_mods & (KMOD_LALT | KMOD_RALT))
                result |= 0x0004;
            return result;
        }
    };
    ...
    public:
        ...
        create_window(int width, int height, const std::string& title, SDLWindow* share = nullptr) {
