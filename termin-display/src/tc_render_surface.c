...
bool tc_render_surface_validate_output(tc_render_surface* surface,
    ...
    if (texture_id == 0) {
        tc_log(TC_LOG_ERROR, "[tc_render_surface_validate_output] surface has no color texture");
        return false;
    }
    uintptr_t actual_domain = tc_render_surface_get_graphics_domain_key(surface);
    if (expected_graphics_domain_key == 0 || actual_domain == 0 || actual_domain != expected_graphics_domain_key) {
        tc_log(TC_LOG_ERROR,
               "[tc_render_surface_validate_output] graphics domain mismatch: surface=%p expected=%p",
               (void*)actual_domain,
               (void*)expected_graphics_domain_key);
        return false;
    }
    if (color_texture_id)
...
tc_render_surface* tc_render_surface_new_external(void* body,
    ...
    if (vtable_size != sizeof(tc_render_surface_vtable)) {
        tc_log(TC_LOG_ERROR,
               ...
               vtable_size,
               sizeof(tc_render_surface_vtable));
        return NULL;
    }
    if (!vtable->get_size || !vtable->resize || !vtable->get_color_texture_id || !vtable->get_graphics_domain_key ||
        !vtable->destroy) {
        tc_log(TC_LOG_ERROR, "[tc_render_surface_new_external] vtable has missing mandatory callbacks");
        return NULL;
    }

    tc_external_render_surface* adapter = (tc_external_render_surface*)calloc(1, sizeof(tc_external_render_surface));
    if (!adapter) {
        tc_log(TC_LOG_ERROR, "[tc_render_surface_new_external] allocation failed");
        return NULL;
    }
    adapter->vtable = *vtable;
    tc_render_surface_init(&adapter->surface, &adapter->vtable, tc_external_render_surface_delete);
...
bool tc_render_surface_delete_unowned(tc_render_surface* surface) {
    ...
    if (tc_display_handle_valid(surface->attached_display)) {
        tc_log(TC_LOG_ERROR, "[tc_render_surface_delete_unowned] surface is owned by a display");
        return false;
    }
    tc_render_surface_deleter deleter = surface->deleter;
    if (!deleter) {
        tc_log(TC_LOG_ERROR, "[tc_render_surface_delete_unowned] surface has no storage deleter");
        return false;
    }
    ...
    if (surface->vtable && surface->vtable->destroy) {
        ...
        tc_log(TC_LOG_ERROR, "[tc_render_surface_delete_unowned] surface has no semantic destroy callback");
    }
    deleter(surface);
    return true;
}
...
bool tc_render_surface_attach(tc_render_surface* surface, tc_display_handle display) {
    ...
    if (tc_display_handle_valid(surface->attached_display) &&
        ...
        tc_log(TC_LOG_ERROR, "[tc_render_surface_attach] surface is already attached to another display");
        return false;
    }
...
bool tc_render_surface_detach(tc_render_surface* surface, tc_display_handle display) {
    ...
    if (!tc_display_handle_eq(surface->attached_display, display)) {
        tc_log(TC_LOG_ERROR, "[tc_render_surface_detach] display does not own this surface attachment");
        return false;
    }
