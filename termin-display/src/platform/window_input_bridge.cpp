...
namespace termin {
    ...
    namespace {
        ...
        size_t platform_clipboard_text(void* userdata, char* buffer, size_t capacity) {
        ...
        bool platform_set_clipboard_text(void* userdata, const char* text_utf8, size_t byte_length) {
            BackendWindow* window = platform_window(userdata);
            if (!window || (!text_utf8 && byte_length != 0))
                return false;
            return window->set_clipboard_text(std::string(text_utf8 ? text_utf8 : "", byte_length));
        }

        WindowCursor platform_cursor(tc_input_cursor cursor) {
            switch (cursor) {
            case TC_INPUT_CURSOR_TEXT:
                return WindowCursor::Text;
            case TC_INPUT_CURSOR_HAND:
                return WindowCursor::Hand;
            case TC_INPUT_CURSOR_CROSSHAIR:
                return WindowCursor::Crosshair;
            case TC_INPUT_CURSOR_MOVE:
            ...
            case TC_INPUT_CURSOR_RESIZE_NWSE:
                return WindowCursor::ResizeNWSE;
            case TC_INPUT_CURSOR_RESIZE_NESW:
        ...
        void platform_set_cursor(void* userdata, tc_input_cursor cursor) {
            BackendWindow* window = platform_window(userdata);
            if (window)
                window->set_cursor(platform_cursor(cursor));
        }

        void platform_set_text_input_enabled(void* userdata, bool enabled) {
            BackendWindow* window = platform_window(userdata);
            if (window)
                window->set_text_input_enabled(enabled);
        }
    ...
    void dispatch_window_input_event(tc_display_handle display, const WindowEvent& event) {
        ...
        case WindowEventType::PointerWheel:
            tc_display_dispatch_wheel(display,
                                      event.pointer.framebuffer_position.x,
                                      event.pointer.framebuffer_position.y,
        ...
        case WindowEventType::KeyReleased:
            tc_display_dispatch_key(display,
        ...
        case WindowEventType::TextInput: {
            size_t length = 0;
            while (length < event.text.utf8.size() && event.text.utf8[length] != '\0')
                ++length;
    ...
    void attach_window_input_display(BackendWindow& window, tc_display_handle display) {
        ...
        if (!tc_display_set_input_platform_services(display, &services)) {
            tc_log_error("[window_input_bridge] failed to install input platform services");
        }
        window.set_event_handler([display](const WindowEvent& event) { dispatch_window_input_event(display, event); });
    }
