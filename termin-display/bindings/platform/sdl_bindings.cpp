...
namespace termin {
    ...
    namespace {
        ...
        SDL_SystemCursor to_sdl_cursor(SystemCursorShape shape) {
            ...
            case SystemCursorShape::Hand:
                return SDL_SYSTEM_CURSOR_HAND;
            case SystemCursorShape::Crosshair:
                return SDL_SYSTEM_CURSOR_CROSSHAIR;
            case SystemCursorShape::Move:
                return SDL_SYSTEM_CURSOR_SIZEALL;
            case SystemCursorShape::ResizeHorizontal:
                return SDL_SYSTEM_CURSOR_SIZEWE;
            case SystemCursorShape::ResizeVertical:
                return SDL_SYSTEM_CURSOR_SIZENS;
            case SystemCursorShape::ResizeNwse:
                return SDL_SYSTEM_CURSOR_SIZENWSE;
            case SystemCursorShape::ResizeNesw:
                return SDL_SYSTEM_CURSOR_SIZENESW;
            case SystemCursorShape::Count:
                break;
            }
            ...
            if (!cursor) {
                ...
                if (!cursor) {
                    ...
                    if (cursor) {
                        ...
                        tc_log_warn("[termin-display] system cursor %d is unavailable: %s",
        ...
        nb::object translate_sdl_event(const SDL_Event& event) {
            ...
            case SDL_MOUSEBUTTONDOWN:
            case SDL_MOUSEBUTTONUP: {
                nb::dict result = make_base_event(event.type == SDL_MOUSEBUTTONDOWN ? "mouse_down" : "mouse_up", event);
            ...
            case SDL_KEYDOWN:
            case SDL_KEYUP: {
                nb::dict result = make_base_event(event.type == SDL_KEYDOWN ? "key_down" : "key_up", event);
    ...
    void bind_sdl(nb::module_& m) {
        ...
        m.def(
            "set_clipboard_text", [](const std::string& text) { SDL_SetClipboardText(text.c_str()); }, nb::arg("text"));
        m.def("start_text_input", []() { SDL_StartTextInput(); });
        m.def("stop_text_input", []() { SDL_StopTextInput(); });
        m.def("quit_sdl", []() {
            release_system_cursors();
            SDL_Quit();
        });
        m.def("poll_sdl_events",
              &poll_sdl_events,
              "Drain SDL events and return Termin event dictionaries without exposing SDL_Event.");
        m.def("wait_sdl_events_timeout",
        ...
        nb::class_<SDLWindow>(m, "SDLWindow")
            ...
            .def("set_framebuffer_size_callback",
                 [](SDLWindow& self, nb::object callback) {
                     if (callback.is_none()) {
                         self.set_framebuffer_size_callback(nullptr);
                     } else {
                         self.set_framebuffer_size_callback([callback](SDLWindow* win, int w, int h) {
            ...
            .def("set_cursor_pos_callback",
                 [](SDLWindow& self, nb::object callback) {
                     if (callback.is_none()) {
                         ...
                         self.set_cursor_pos_callback([callback](SDLWindow* win, double x, double y) {
                             nb::gil_scoped_acquire gil;
                             callback(win, x, y);
                         });
                     }
            ...
            .def("set_scroll_callback",
                 [](SDLWindow& self, nb::object callback) {
                     if (callback.is_none()) {
                         ...
                         self.set_scroll_callback([callback](SDLWindow* win, double x, double y, int mods) {
                             nb::gil_scoped_acquire gil;
                 ...
                 [](SDLWindow& self, nb::object callback) {
                     if (callback.is_none()) {
                         ...
                         self.set_mouse_button_callback(
                             [callback](SDLWindow* win, int button, int action, int mods, uint32_t click_count) {
                                 nb::gil_scoped_acquire gil;
            ...
            .def("set_key_callback",
                 [](SDLWindow& self, nb::object callback) {
                     if (callback.is_none()) {
                         ...
                         self.set_key_callback([callback](SDLWindow* win, int key, int scancode, int action, int mods) {
                             nb::gil_scoped_acquire gil;
        ...
        nb::class_<SDLWindowBackend>(m, "SDLWindowBackend")
            .def(nb::init<>())
            .def("create_window",
                 &SDLWindowBackend::create_window,
                 nb::arg("width"),
                 nb::arg("height"),
                 nb::arg("title"),
                 nb::arg("share") = nullptr)
        ...
        nb::class_<SDLBackendWindow, BackendWindow>(m, "SDLBackendWindow")
            .def_prop_ro(
                "backend",
                [](const SDLBackendWindow& self) { return std::string(tgfx::backend_name(self.backend_type())); },
                "Name of the backend actually created for this window.")
            .def_prop_ro("presentation_mode",
                         &SDLBackendWindow::presentation_mode,
                         "Presentation mode actually applied to this window group.")
            .def_prop_ro("requested_presentation_mode",
                         &SDLBackendWindow::requested_presentation_mode,
            ...
            .def(
                "set_input_display",
                [](SDLBackendWindow& self, uint32_t index, uint32_t generation) {
            ...
            .def("set_title", &SDLBackendWindow::set_title, nb::arg("title"), "Set the OS window title.")
            .def("maximize", &SDLBackendWindow::maximize, "Maximize the OS window via SDL_MaximizeWindow.")
            .def("set_icon_bmp",
                 &SDLBackendWindow::set_icon_bmp,
                 nb::arg("path"),
                 "Load a BMP file and use it as the OS window icon. Raises on load failure.")
            .def("set_fullscreen",
                 &SDLBackendWindow::set_fullscreen,
                 nb::arg("enabled"),
                 "Toggle borderless desktop fullscreen via SDL_SetWindowFullscreen.")
            .def("set_always_on_top",
                 &SDLBackendWindow::set_always_on_top,
                 nb::arg("enabled"),
                 "Set whether the OS window should stay above normal windows.")
            .def("close",
                 &SDLBackendWindow::close,
            ...
            .def(
                ...
                [](WindowedGraphicsSession& self,
                   ...
                   tgfx::PresentationMode presentation_mode) {
                    BackendWindowPtr window = self.create_window(WindowConfig{title, width, height, presentation_mode});
        ...
        nb::class_<WindowManager>(m, "WindowManager")
            .def(nb::init<WindowedGraphicsSession&>(),
                 nb::arg("graphics_session"),
                 nb::keep_alive<1, 2>(),
