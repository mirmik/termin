...
namespace termin {
    ...
    static TcDisplay configured_display(tc_display_handle handle, bool editor_only, const std::string& uuid) {
    ...
    void bind_tc_display(nb::module_& m) {
        ...
        nb::class_<TcDisplay>(m,
            ...
            .def("destroy", &TcDisplay::destroy, "Explicitly destroy the native display. Python GC does not call this.")
            ...
            .def_prop_ro(
                ...
                [](TcDisplay& self) -> uintptr_t { return reinterpret_cast<uintptr_t>(self.input_manager()); },
                "Stable display-owned tc_input_manager endpoint")

            .def(
                "dispatch_pointer_move",
                [](TcDisplay& self, double x, double y) { return self.dispatch_pointer_move(x, y); },
                nb::arg("x"),
                nb::arg("y"))
            .def(
                "dispatch_pointer_button",
                [](TcDisplay& self, double x, double y, int button, int action, int modifiers, uint32_t click_count) {
                    return self.dispatch_pointer_button(x, y, button, action, modifiers, click_count);
                },
                nb::arg("x"),
                nb::arg("y"),
                nb::arg("button"),
            ...
            .def(
                "dispatch_wheel",
                [](TcDisplay& self, double x, double y, double wheel_x, double wheel_y, int modifiers) {
            ...
            .def(
                "dispatch_key",
                [](TcDisplay& self, int key, int scancode, int action, int modifiers) {
            ...
            .def(
                "dispatch_text",
                [](TcDisplay& self, uint32_t codepoint) { return self.dispatch_text(codepoint); },
                nb::arg("codepoint"))

            // Properties
            .def_prop_rw("name", &TcDisplay::name, &TcDisplay::set_name, "Display name")

            .def_prop_rw("uuid", &TcDisplay::uuid, &TcDisplay::set_uuid, "Unique identifier (for serialization)")

            .def_prop_ro(
                "runtime_id",
                [](TcDisplay& self) -> uint64_t {
            ...
            .def(
                "get_size",
                [](TcDisplay& self) { return self.get_size(); },
            ...
            .def_prop_ro("color_tex", [](TcDisplay& self) { return tgfx::TextureHandle{self.color_texture_id()}; })
            .def("graphics_domain_key", &TcDisplay::graphics_domain_key)

            // Viewport management - returns TcViewport objects
            .def_prop_ro(
                "viewports",
                [](TcDisplay& self) -> std::vector<TcViewport> {
            ...
            .def(
                "remove_viewport",
                [](TcDisplay& self, TcViewport& vp) { self.remove_viewport(vp.handle()); },
                nb::arg("viewport"),
            ...
            .def(
                "viewport_at",
                [](TcDisplay& self, float x, float y) -> nb::object {
            ...
            .def(
                "viewport_at_pixels",
                [](TcDisplay& self, float px, float py) -> nb::object {
            ...
            .def("update_all_pixel_rects",
                 &TcDisplay::update_all_pixel_rects,
                ...
                [](TcDisplay& self,
                   ...
                   const std::string& name) -> TcViewport {
                    ...
                    if (!camera_obj.is_none()) {
                        camera_ptr =
                            reinterpret_cast<tc_component*>(nb::cast<uintptr_t>(camera_obj.attr("c_component_ptr")()));
                    }

                    // Create viewport (render target assigned separately)
                    TcViewport vp = TcViewport::create(name, scene_h, camera_ptr);
                    vp.set_rect(std::get<0>(rect), std::get<1>(rect), std::get<2>(rect), std::get<3>(rect));
                ...
                [](tgfx::IRenderDevice& device,
                   ...
                   const std::string& uuid) {
                    return configured_display(
                        create_offscreen_display(&device, width, height, name.c_str()), editor_only, uuid);
                },
                nb::arg("device"),
                nb::arg("width"),
                nb::arg("height"),
                nb::arg("name") = "Display",
                nb::arg("editor_only") = false,
                nb::arg("uuid") = "",
                "Create a display that exclusively owns an offscreen texture surface")

            .def_static(
                "from_surface",
                [](nb::object surface_object, const std::string& name, bool editor_only, const std::string& uuid) {
                    tc_render_surface* surface = create_python_render_surface(surface_object);
                    if (!surface)
                        throw std::runtime_error("failed to create Python surface adapter");
