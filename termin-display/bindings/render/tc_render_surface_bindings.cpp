...
namespace termin {
    namespace {
        ...
        void python_surface_get_size(tc_render_surface* surface, int* width, int* height) {
            ...
            try {
                auto [w, h] = nb::cast<std::tuple<int, int>>(surface_object(surface).attr("framebuffer_size")());
                if (width)
                    *width = w;
                if (height)
                    *height = h;
        ...
        uint32_t python_surface_get_color_texture_id(tc_render_surface* surface) {
            ...
            nb::gil_scoped_acquire gil;
            try {
                return nb::cast<uint32_t>(surface_object(surface).attr("get_tgfx_color_tex_id")());
            } catch (const std::exception& error) {
        ...
        uintptr_t python_surface_get_graphics_domain_key(tc_render_surface* surface) {
            ...
            nb::gil_scoped_acquire gil;
            try {
                return nb::cast<uintptr_t>(surface_object(surface).attr("graphics_domain_key")());
            } catch (const std::exception& error) {
    ...
    tc_render_surface* create_python_render_surface(nb::object python_surface) {
        ...
        tc_render_surface* surface = tc_render_surface_new_external(
            body, &python_surface_vtable, sizeof(python_surface_vtable), TC_RENDER_SURFACE_ABI_VERSION);
        if (!surface)
            Py_DECREF(body);
        return surface;
    }
