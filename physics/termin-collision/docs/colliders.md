...
- `normal` — наружная единичная нормаль поверхности в мировых координатах при пересечении; `CollisionWorld::raycast` передаёт её без пересчёта от центра. При промахе — нулевой вектор.

При начале луча внутри Box, Sphere и ConvexHull результат указывает на выходную поверхность с наружной нормалью. Capsule сохраняет семантику overlap-at-origin: точка внутри возвращает distance=0 и нулевую normal, поскольку это не пересечение поверхности; при начале на поверхности возвращается её наружная нормаль. AttachedCollider и UnionCollider передают нормаль выбранного примитива. На ребре Box выбирается первая ограничивающая грань в порядке осей X/Y/Z; у ConvexHull — первая пересечённая грань при равных параметрах луча.
