...
namespace termin {
    namespace colliders {
        ...
        public:
            ...
            ConvexHullCollider(std::vector<Vec3> verts,
                               std::vector<ConvexFace> faces_,
        ...
        namespace quickhull {
            ...
            inline std::vector<ConvexFace> build(const std::vector<Vec3>& points) {
                ...
                for (int i = 1; i < (int)verts.size(); ++i) {
                    if (verts[i].x < verts[extremes[0]].x)
                        extremes[0] = i;
                    if (verts[i].x > verts[extremes[1]].x)
                        extremes[1] = i;
                    if (verts[i].y < verts[extremes[2]].y)
                        extremes[2] = i;
                    if (verts[i].y > verts[extremes[3]].y)
                        extremes[3] = i;
                    if (verts[i].z < verts[extremes[4]].z)
                        extremes[4] = i;
                    if (verts[i].z > verts[extremes[5]].z)
                        extremes[5] = i;
                }
                ...
                if (vol > 0)
                    std::swap(p1, p2); // flip to get outward normals from standard winding
                ...
                for (int iter = 0; iter < MAX_QH_ITERATIONS; ++iter) {
                    ...
                    for (int f = 0; f < (int)faces.size(); ++f) {
                        if (!faces[f].alive || faces[f].outside_set.empty())
                            continue;
                    ...
                    for (int f = 0; f < (int)faces.size(); ++f) {
                        ...
                        for (int e = 0; e < 3; ++e) {
                            ...
                            for (int j = 0; j < (int)faces.size(); ++j) {
                                if (j == f || !faces[j].alive || visible[j])
                                    continue;
        ...
        inline ConvexHullCollider ConvexHullCollider::from_points(const std::vector<Vec3>& points,
                                                                  const GeneralPose3& t) {
