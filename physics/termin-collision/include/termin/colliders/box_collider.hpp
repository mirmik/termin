    ...
    namespace colliders {
        ...
        public:
            ...
            Vec3 support(const Vec3& direction) const override {
                ...
                Vec3 local_point(
                    local_dir.x >= 0 ? hs.x : -hs.x, local_dir.y >= 0 ? hs.y : -hs.y, local_dir.z >= 0 ? hs.z : -hs.z);
            ...
            std::array<Vec3, 3> get_axes_world() const {
                Pose3 p = pose();
                return {p.transform_vector(Vec3(1, 0, 0)),
        ...
        inline ColliderHit BoxCollider::closest_to_box_impl(const BoxCollider& other) const {
            ...
            auto project_extent = [](const std::array<Vec3, 3>& axes, const Vec3& half, const Vec3& axis) {
                return std::abs(axes[0].dot(axis)) * half.x + std::abs(axes[1].dot(axis)) * half.y +
            ...
            for (const auto& corner : corners_b) {
                ...
                bool inside = proj_x <= half_a.x + 1e-6 && proj_y <= half_a.y + 1e-6 && proj_z <= half_a.z + 1e-6;
            ...
            if (found_corner) {
                ...
                for (const auto& corner : corners_a) {
                    ...
                    bool inside = proj_x <= half_b.x + 1e-6 && proj_y <= half_b.y + 1e-6 && proj_z <= half_b.z + 1e-6;
