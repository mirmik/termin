...
namespace termin {
    ...
    void RecastNavMeshBuilderComponent::register_type() {
        auto descriptor = ComponentTypeDescriptorBuilder::native<RecastNavMeshBuilderComponent>(
            "RecastNavMeshBuilderComponent", "termin-navmesh", "Component");
        descriptor.category("Navigation").require("NavMeshKeeperComponent");
        ...
        tc::stage_inspect_field(descriptor.inspect(),
                                ...
                                "Show Detail Mesh (6)",
                                "bool");
        tc::stage_inspect_button_method<RecastNavMeshBuilderComponent>(
            descriptor.inspect(), "build_btn", "Build NavMesh", &RecastNavMeshBuilderComponent::build_from_entity);
    ...
    RecastBuildResult RecastNavMeshBuilderComponent::build(const float* verts, int nverts, const int* tris, int ntris) {
    ...
    RecastBuildResult RecastNavMeshBuilderComponent::build_with_areas(
        const float* verts, int nverts, const int* tris, int ntris, const unsigned char* triangle_area_ids) {
        return build_internal(verts, nverts, tris, ntris, triangle_area_ids, true);
    }

    RecastBuildResult RecastNavMeshBuilderComponent::build_internal(const float* verts,
                                                                    int nverts,
                                                                    const int* tris,
                                                                    int ntris,
        ...
        if (capture_heightfield) {
            debug_hooks.capture_heightfield = [this](rcHeightfield* hf) {
                capture_heightfield_data(hf);
            };
        }
        if (capture_compact) {
            debug_hooks.capture_compact = [this](rcCompactHeightfield* chf) {
                capture_compact_data(chf);
            };
        }
        if (capture_contours) {
            debug_hooks.capture_contours = [this](rcContourSet* cset) {
                capture_contour_data(cset);
            };
        }
        if (capture_poly_mesh) {
            debug_hooks.capture_poly_mesh = [this](rcPolyMesh* pmesh) {
                capture_poly_mesh_data(pmesh);
            };
        }
        if (capture_detail_mesh) {
            debug_hooks.capture_detail_mesh = [this](rcPolyMeshDetail* dmesh) {
                capture_detail_mesh_data(dmesh);
            };
        }

        RecastBuildResult result =
            build_recast_navmesh(verts, nverts, tris, ntris, triangle_area_ids, config, &debug_hooks);
        last_result = result;
    ...
    RecastNavMeshBuilderComponent::build_detour_tile_data(const RecastBuildResult& result) {
        ...
        return build_detour_navmesh_tile_data(result,
                                              detour_config,
    ...
    bool RecastNavMeshBuilderComponent::save_detour_asset(const RecastBuildResult& result) {
        ...
        DetourNavMeshTileBuildResult tile =
            build_detour_navmesh_tile_data(result,
                                           detour_config,
                                           off_mesh_links.count() > 0 ? &off_mesh_links : nullptr,
                                           linear_paths.count() > 0 ? &linear_paths : nullptr);
        if (!tile.success) {
            tc_log_error("RecastNavMeshBuilderComponent: failed to build Detour navmesh tile: %s", tile.error.c_str());
            return false;
        }
        ...
        std::string uuid =
            keeper && !keeper->navmesh_uuid.empty()
                ? keeper->navmesh_uuid
        ...
        if (!out) {
            tc_log_error("RecastNavMeshBuilderComponent: failed while writing '%s'", output_path.string().c_str());
            return false;
        }
        ...
        if (meta) {
            ...
            tc_log_error("RecastNavMeshBuilderComponent: failed to write meta '%s'", meta_path.string().c_str());
        }
    ...
    tc_phase_mask RecastNavMeshBuilderComponent::get_phase_mask() const {
        ...
        if (show_input_mesh || show_heightfield || show_regions || show_distance_field || show_contours ||
            show_poly_mesh || show_detail_mesh) {
    ...
    void RecastNavMeshBuilderComponent::build_input_mesh(const float* verts, int nverts, const int* tris, int ntris) {
        if (!verts || nverts == 0 || !tris || ntris == 0)
            return;
        ...
        for (int i = 0; i < nverts; i++) {
            ...
            vertices.push_back({{tm_x, tm_y, tm_z}, {input_color[0], input_color[1], input_color[2], input_color[3]}});
        }
        ...
        if (m->vertex_count == 0) {
            tc_mesh_set_data(
                m, vertices.data(), vertices.size(), &layout, indices.data(), indices.size(), "navmesh_debug_input");
        }
    ...
    void RecastNavMeshBuilderComponent::build_heightfield_mesh() {
        ...
        tc_log_info(
            "[NavMesh] HF debug: Recast bmin=(%.2f, %.2f, %.2f) bmax not stored", hf.bmin.x, hf.bmin.y, hf.bmin.z);
        tc_log_info("[NavMesh] HF debug: grid %dx%d, cs=%.3f ch=%.3f", hf.width, hf.height, hf.cs, hf.ch);
        ...
        tc_log_info("[NavMesh] Heightfield mesh: %zu verts, %zu tris", vertices.size(), indices.size() / 3);
    }
    ...
    void RecastNavMeshBuilderComponent::build_regions_mesh() {
        ...
        if (chf.width == 0 || chf.height == 0 || chf.span_count == 0)
            return;

        tc_log_info("[NavMesh] Regions debug: grid %dx%d, %d spans", chf.width, chf.height, chf.span_count);
        ...
        for (int rz = 0; rz < chf.height; rz++) {
            for (int rx = 0; rx < chf.width; rx++) {
                ...
                for (uint8_t s = 0; s < span_count; s++) {
                    ...
                    if (span_idx >= static_cast<uint32_t>(chf.span_count))
                        continue;
        ...
        if (m->vertex_count == 0) {
            tc_mesh_set_data(
                m, vertices.data(), vertices.size(), &layout, indices.data(), indices.size(), "navmesh_debug_regions");
        }
    ...
    void RecastNavMeshBuilderComponent::build_distance_field_mesh() {
        ...
        tc_log_info("[NavMesh] Distance field mesh: %zu verts, %zu tris", vertices.size(), indices.size() / 3);
    }
    ...
    void RecastNavMeshBuilderComponent::build_contours_mesh() {
        ...
        if (m->vertex_count == 0) {
            tc_mesh_set_data(
                m, vertices.data(), vertices.size(), &layout, indices.data(), indices.size(), "navmesh_debug_contours");
            m->draw_mode = TC_DRAW_LINES;
        }

        _contours_mesh = TcMesh(h);

        tc_log_info("[NavMesh] Contours mesh: %zu verts, %zu lines", vertices.size(), indices.size() / 2);
    }
    ...
    void RecastNavMeshBuilderComponent::build_poly_mesh_debug() {
        ...
        tc_log_info("[NavMesh] PolyMesh debug: %d verts, %d polys (nvp=%d)", pmesh.nverts, pmesh.npolys, pmesh.nvp);
        ...
        tc_log_info("[NavMesh] PolyMesh debug mesh: %zu verts, %zu tris", vertices.size(), indices.size() / 3);
    }
    ...
    void RecastNavMeshBuilderComponent::build_detail_mesh_debug() {
        ...
        if (dmesh.nmeshes == 0 || dmesh.nverts == 0 || dmesh.ntris == 0)
            return;

        tc_log_info(
            "[NavMesh] DetailMesh debug: %d meshes, %d verts, %d tris", dmesh.nmeshes, dmesh.nverts, dmesh.ntris);
        ...
        tc_log_info("[NavMesh] DetailMesh debug mesh: %zu verts, %zu tris", vertices.size(), indices.size() / 3);
    }
    ...
    TcMaterial RecastNavMeshBuilderComponent::get_debug_material() {
        if (!_debug_material.is_valid()) {
            ...
            tc_material_phase* phase = add_builtin_slang_debug_phase(_debug_material,
