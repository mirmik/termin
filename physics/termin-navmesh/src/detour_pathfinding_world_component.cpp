...
namespace termin {
    ...
    DetourClosestPointResult DetourPathfindingWorldComponent::closest_point(const Vec3f& point) {
    ...
    DetourClosestPointResult DetourPathfindingWorldComponent::closest_point_world(const Pose3& bake_frame,
                                                                                  const Vec3f& point) {
        if (!ensure_query_loaded()) {
            return {};
        }
    ...
    std::vector<Vec3f> DetourPathfindingWorldComponent::find_path(const Vec3f& start, const Vec3f& end) {
        if (!ensure_query_loaded()) {
            return {};
        }
        sync_query_settings();
    ...
    std::vector<Vec3f>
    DetourPathfindingWorldComponent::find_path_world(const Pose3& bake_frame, const Vec3f& start, const Vec3f& end) {
        if (!ensure_query_loaded()) {
            return {};
        }
        sync_query_settings();
    ...
    DetourPathResult DetourPathfindingWorldComponent::find_detailed_path(const Vec3f& start, const Vec3f& end) {
        if (!ensure_query_loaded()) {
    ...
    DetourPathResult DetourPathfindingWorldComponent::find_detailed_path_world(const Pose3& bake_frame,
                                                                               const Vec3f& start,
                                                                               const Vec3f& end) {
        if (!ensure_query_loaded()) {
    ...
    DetourRaycastResult DetourPathfindingWorldComponent::raycast(const Vec3f& start, const Vec3f& end) {
        if (!ensure_query_loaded()) {
            return {};
        }
        sync_query_settings();
    ...
    DetourRaycastResult
    DetourPathfindingWorldComponent::raycast_world(const Pose3& bake_frame, const Vec3f& start, const Vec3f& end) {
        if (!ensure_query_loaded()) {
            return {};
        }
        sync_query_settings();
    ...
    namespace {
        ...
        void register_detour_pathfinding_world_inspect_fields(tc::InspectFacetBuilder& inspect) {
            inspect.add_with_accessors<DetourPathfindingWorldComponent, TcNavMesh>(
                ...
                "NavMesh",
                "navmesh_handle",
                [](DetourPathfindingWorldComponent* self) { return TcNavMesh::from_uuid(self->navmesh_uuid); },
