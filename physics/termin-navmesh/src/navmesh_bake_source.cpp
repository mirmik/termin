...
namespace termin {
    ...
    namespace {
        ...
        bool extract_mesh_positions(const TcMesh& mesh,
        ...
        bool extract_mesh_component(Entity entity,
            ...
            return extract_mesh_positions(
                mesh_component.mesh, mesh_to_bake, context.default_area_id, entity, entity_debug_name(entity), input);
        }
        ...
        bool extract_mesh_positions(const TcMesh& mesh,
            ...
            batch.triangle_area_ids.assign(triangle_count, static_cast<unsigned char>(std::clamp(area_id, 0, 63)));

            input.add_geometry_batch(std::move(batch));
            return true;
        }

        void collect_mesh_component(Entity entity,
        ...
        void collect_off_mesh_link_component(Entity entity,
                                             CxxComponent* component,
                                             const NavMeshBakeContext& context,
                                             NavMeshBakeInput& input) {
            ...
            record.start = Vec3f{static_cast<float>(start_recast.x),
                                 static_cast<float>(start_recast.y),
                                 static_cast<float>(start_recast.z)};
            record.end = Vec3f{
                static_cast<float>(end_recast.x), static_cast<float>(end_recast.y), static_cast<float>(end_recast.z)};
            ...
            record.flags = 1;
            record.user_id = link->stable_user_id != 0 ? link->stable_user_id
    ...
    void NavMeshBakeInput::add_geometry_batch(NavMeshGeometryBatch batch) {
        ...
        if (batch.triangle_area_ids.empty()) {
            batch.triangle_area_ids.assign(triangle_count,
                                           static_cast<unsigned char>(std::clamp(batch.area_id, 0, 63)));
        }
        if (batch.triangle_area_ids.size() != triangle_count) {
            tc_log_error(
                "[NavMesh] rejecting geometry batch '%s': triangle area count %zu does not match triangle count %zu",
                batch.debug_name.c_str(),
    ...
    void NavMeshBakeInput::add_off_mesh_link(NavMeshOffMeshLinkRecord link) {
        if (link.user_id == 0) {
            tc_log_error("[NavMesh] rejecting off-mesh link '%s': stable user id is zero", link.debug_name.c_str());
            return;
        }
    ...
    int NavMeshBakeInput::add_linear_segment(NavMeshLinearPathSegmentRecord segment) {
        ...
        if (segment.user_id == 0) {
            tc_log_error("[NavMesh] rejecting linear segment '%s': stable user id is zero", segment.debug_name.c_str());
            return -1;
        }
    ...
    void NavMeshBakeInput::add_linear_link(NavMeshLinearPathLinkRecord link) {
        ...
        if (link.from_segment < 0 || link.to_segment < 0 || link.from_segment >= segment_count ||
            link.to_segment >= segment_count) {
    ...
    bool NavMeshBakeVisitorRegistry::register_geometry_visitor(const std::string& component_type,
                                                               NavMeshBakeVisitor visitor) {
        return register_visitor(
            _geometry_visitors, "geometry", component_type, std::move(visitor), _current_registration_owner);
    }

    bool NavMeshBakeVisitorRegistry::register_link_visitor(const std::string& component_type,
                                                           NavMeshBakeVisitor visitor) {
        return register_visitor(
            _link_visitors, "off-mesh link", component_type, std::move(visitor), _current_registration_owner);
    }

    bool NavMeshBakeVisitorRegistry::register_linear_visitor(const std::string& component_type,
                                                             NavMeshBakeVisitor visitor) {
        return register_visitor(
            _linear_visitors, "linear path", component_type, std::move(visitor), _current_registration_owner);
    }

    bool NavMeshBakeVisitorRegistry::unregister_geometry_visitor(const std::string& component_type) {
        return unregister_visitor(_geometry_visitors, component_type);
    }

    bool NavMeshBakeVisitorRegistry::unregister_link_visitor(const std::string& component_type) {
        return unregister_visitor(_link_visitors, component_type);
    }

    bool NavMeshBakeVisitorRegistry::unregister_linear_visitor(const std::string& component_type) {
    ...
    std::string NavMeshBakeVisitorRegistry::geometry_visitor_owner(const std::string& component_type) const {
        return visitor_owner(_geometry_visitors, component_type);
    }

    std::string NavMeshBakeVisitorRegistry::link_visitor_owner(const std::string& component_type) const {
        return visitor_owner(_link_visitors, component_type);
    }

    std::string NavMeshBakeVisitorRegistry::linear_visitor_owner(const std::string& component_type) const {
    ...
    const NavMeshBakeVisitor* NavMeshBakeVisitorRegistry::geometry_visitor(const std::string& component_type) const {
        auto it = _geometry_visitors.find(component_type);
        return it == _geometry_visitors.end() ? nullptr : &it->second.visitor;
    }

    const NavMeshBakeVisitor* NavMeshBakeVisitorRegistry::link_visitor(const std::string& component_type) const {
        auto it = _link_visitors.find(component_type);
        return it == _link_visitors.end() ? nullptr : &it->second.visitor;
    }

    const NavMeshBakeVisitor* NavMeshBakeVisitorRegistry::linear_visitor(const std::string& component_type) const {
        auto it = _linear_visitors.find(component_type);
        return it == _linear_visitors.end() ? nullptr : &it->second.visitor;
    }

    bool NavMeshBakeVisitorRegistry::register_visitor(VisitorMap& visitors,
    ...
    bool NavMeshBakeVisitorRegistry::unregister_visitor(VisitorMap& visitors, const std::string& component_type) {
        return visitors.erase(component_type) > 0;
    }

    std::string NavMeshBakeVisitorRegistry::visitor_owner(const VisitorMap& visitors,
                                                          const std::string& component_type) {
        auto it = visitors.find(component_type);
        return it == visitors.end() ? std::string() : it->second.owner;
    }

    int NavMeshBakeVisitorRegistry::visit_component(Entity entity,
                                                    CxxComponent* component,
    ...
    NavMeshBakeInput collect_navmesh_bake_input(const NavMeshBakeContext& context,
