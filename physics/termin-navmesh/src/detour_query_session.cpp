...
namespace termin {
    ...
    namespace {
        ...
        std::string straight_path_flags_to_string(unsigned char flags) {
            ...
            const unsigned char known = DT_STRAIGHTPATH_START | DT_STRAIGHTPATH_END |
                                        DT_STRAIGHTPATH_OFFMESH_CONNECTION | DT_STRAIGHTPATH_LINEAR;
        ...
        void log_detour_poly_info(dtNavMesh* navmesh, const char* prefix, int index, dtPolyRef ref) {
            if (!navmesh || ref == 0) {
                tc_log_info("%s[%d]: ref=%llu <none>", prefix, index, static_cast<unsigned long long>(ref));
                return;
            }
    ...
    bool
    DetourQuerySession::load_single_tile_data(const unsigned char* data, int data_size, const std::string& asset_name) {
        clear();
        ...
        if (!_navmesh || !dtStatusSucceed(_navmesh->init(_tile_blob.data(), static_cast<int>(_tile_blob.size()), 0))) {
    ...
    bool DetourQuerySession::find_nearest_poly(const Vec3f& point,
        ...
        dtStatus status =
            over_poly
                ? _query->findNearestPoly(vec3_ptr(rc_point), extents, &filter, &ref, vec3_ptr(nearest), over_poly)
                : _query->findNearestPoly(vec3_ptr(rc_point), extents, &filter, &ref, vec3_ptr(nearest));
    ...
    DetourClosestPointResult DetourQuerySession::closest_point_world(const Pose3& bake_frame, const Vec3f& point) {
    ...
    std::vector<Vec3f> DetourQuerySession::find_path(const Vec3f& start, const Vec3f& end) {
    ...
    std::vector<Vec3f>
    DetourQuerySession::find_path_world(const Pose3& bake_frame, const Vec3f& start, const Vec3f& end) {
        std::vector<Vec3f> result;
    ...
    DetourPathResult DetourQuerySession::find_detailed_path(const Vec3f& start, const Vec3f& end) {
        ...
        tc_log_info("[DetourQuerySession] nearest start ref=%llu over=%d "
                    ...
                    start_ref_raw,
                    start_over_poly ? 1 : 0,
                    nearest_start_term[0],
                    nearest_start_term[1],
                    nearest_start_term[2],
                    point_distance(start, nearest_start_term));
        log_detour_poly_info(
            _navmesh, "[DetourQuerySession] nearest start poly", 0, static_cast<dtPolyRef>(start_ref_raw));
        ...
        log_detour_poly_info(_navmesh, "[DetourQuerySession] nearest end poly", 0, static_cast<dtPolyRef>(end_ref_raw));
        ...
        const dtStatus find_path_status = _query->findPath(static_cast<dtPolyRef>(start_ref_raw),
        ...
        tc_log_info("[DetourQuerySession] findPath status=%s path_count=%d", find_path_status_text.c_str(), path_count);
        ...
        for (int i = 0; i < path_count; ++i) {
            log_detour_poly_info(_navmesh, "[DetourQuerySession] corridor", i, path[static_cast<size_t>(i)]);
        }
        ...
        const dtStatus straight_status = _query->findStraightPath(vec3_ptr(nearest_start),
                                                                  vec3_ptr(nearest_end),
                                                                  path.data(),
                                                                  path_count,
                                                                  straight.data(),
    ...
    DetourPathResult
    DetourQuerySession::find_detailed_path_world(const Pose3& bake_frame, const Vec3f& start, const Vec3f& end) {
        DetourPathResult result = find_detailed_path(navmesh_world_to_bake_point(bake_frame, start),
    ...
    DetourRaycastResult DetourQuerySession::raycast(const Vec3f& start, const Vec3f& end) {
        ...
        if (!dtStatusSucceed(_query->raycast(static_cast<dtPolyRef>(start_ref_raw),
                                             vec3_ptr(nearest_start),
                                             vec3_ptr(rc_end),
                                             &filter,
                                             &t,
    ...
    DetourRaycastResult
    DetourQuerySession::raycast_world(const Pose3& bake_frame, const Vec3f& start, const Vec3f& end) {
        DetourRaycastResult result =
            raycast(navmesh_world_to_bake_point(bake_frame, start), navmesh_world_to_bake_point(bake_frame, end));
        if (result.success) {
