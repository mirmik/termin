...
namespace termin {
    ...
    public:
        ...
        explicit TcNavMesh(tc_navmesh_handle h)
            : handle(h) {
        ...
        TcNavMesh(const TcNavMesh& other)
            : handle(other.handle) {
            if (tc_navmesh* navmesh = tc_navmesh_get(handle)) {
                tc_navmesh_add_ref(navmesh);
            }
        ...
        TcNavMesh(TcNavMesh&& other) noexcept
            : handle(other.handle) {
        ...
        tc_navmesh* get() const {
            return tc_navmesh_get(handle);
        }
        bool is_valid() const {
            return tc_navmesh_is_valid(handle);
        }
        bool is_loaded() const {
            return tc_navmesh_is_loaded(handle);
        }
        bool ensure_loaded() const {
            return tc_navmesh_ensure_loaded(handle);
        }
        ...
        void deserialize_from(const tc_value* data, void* = nullptr) {
            ...
            if (data->type == TC_VALUE_STRING && data->data.s && data->data.s[0]) {
                ...
                if (std::strcmp(navmesh_name, "(None)") == 0)
                    return;
