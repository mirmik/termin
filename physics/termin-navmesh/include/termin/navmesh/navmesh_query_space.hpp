...
namespace termin {
    ...
    inline Vec3f navmesh_world_to_bake_point(const Pose3& bake_frame, const Vec3f& world_point) {
        const Vec3 bake = navmesh_world_to_bake_point(bake_frame, Vec3{world_point[0], world_point[1], world_point[2]});
        return {
    ...
    inline Vec3f navmesh_bake_to_world_point(const Pose3& bake_frame, const Vec3f& bake_point) {
        const Vec3 world = navmesh_bake_to_world_point(bake_frame, Vec3{bake_point[0], bake_point[1], bake_point[2]});
        return {
    ...
    inline Vec3f navmesh_bake_to_world_vector(const Pose3& bake_frame, const Vec3f& bake_vector) {
        const Vec3 world =
            navmesh_bake_to_world_vector(bake_frame, Vec3{bake_vector[0], bake_vector[1], bake_vector[2]});
        return {
            static_cast<float>(world.x),
            static_cast<float>(world.y),
            static_cast<float>(world.z),
