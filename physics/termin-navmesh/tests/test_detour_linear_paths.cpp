...
namespace {
    ...
    void require_no_duplicate_linear_straight_points(
        const float* points, const unsigned char* flags, const dtPolyRef* refs, int count, const char* message) {
        ...
        for (int i = 1; i < count; ++i) {
            ...
            if (previousLinear && currentLinear && refs[i - 1] == refs[i] &&
                ...
                std::fprintf(stderr,
                             "%s: duplicate indices %d and %d ref=%llu\n",
                             message,
                             i - 1,
                             i,
...
int main() {
    ...
    require(dtStatusSucceed(nav->getTileAndPolyByRef(endRef, &endTile, &endPoly)), "getTileAndPolyByRef end failed");
    ...
    dtPolyRef path[16];
    int pathCount = 0;
    require(dtStatusSucceed(query->findPath(startRef, endRef, nearestStart, nearestEnd, &filter, path, &pathCount, 16)),
            "findPath failed");
    ...
    int straightCount = 0;
    require(
        dtStatusSucceed(query->findStraightPath(
            nearestStart, nearestEnd, path, pathCount, straight, straightFlags, straightRefs, &straightCount, 16, 0)),
        "findStraightPath failed");
    require(straightCount >= 4, "straight path is too short");
    require_no_duplicate_linear_straight_points(
        straight, straightFlags, straightRefs, straightCount, "straight path contains duplicate linear points");
    ...
    require(straightRefs[straightCount - 1] == endRef, "straight path final linear point does not keep end linear ref");
    ...
    require(dtStatusSucceed(
                query->findNearestPoly(sameLinearStart, extents, &filter, &sameLinearStartRef, nearestSameLinearStart)),
            "same-linear findNearestPoly start failed");
    require(dtStatusSucceed(
                query->findNearestPoly(sameLinearEnd, extents, &filter, &sameLinearEndRef, nearestSameLinearEnd)),
            "same-linear findNearestPoly end failed");
    require(sameLinearStartRef == sameLinearEndRef, "same-linear points did not resolve to the same linear poly");
    ...
    require(dtStatusSucceed(query->findPath(sameLinearStartRef,
                                            sameLinearEndRef,
                                            nearestSameLinearStart,
                                            nearestSameLinearEnd,
                                            &filter,
                                            sameLinearPath,
                                            &sameLinearPathCount,
                                            4)),
    ...
    require(dtStatusSucceed(query->findStraightPath(nearestSameLinearStart,
                                                    nearestSameLinearEnd,
                                                    sameLinearPath,
                                                    sameLinearPathCount,
                                                    sameLinearStraight,
                                                    sameLinearStraightFlags,
                                                    sameLinearStraightRefs,
                                                    &sameLinearStraightCount,
                                                    4,
                                                    0)),
    ...
    require((sameLinearStraightFlags[0] & DT_STRAIGHTPATH_START) != 0, "same-linear start point is not marked start");
    require((sameLinearStraightFlags[0] & DT_STRAIGHTPATH_LINEAR) != 0, "same-linear start point is not marked linear");
    require(sameLinearStraightRefs[0] == sameLinearStartRef, "same-linear start point does not keep linear ref");
    require((sameLinearStraightFlags[1] & DT_STRAIGHTPATH_END) != 0, "same-linear end point is not marked end");
    require((sameLinearStraightFlags[1] & DT_STRAIGHTPATH_LINEAR) != 0, "same-linear end point is not marked linear");
    require(sameLinearStraightRefs[1] == sameLinearEndRef, "same-linear end point does not keep linear ref");

    dtPolyRef reversePath[16];
    int reversePathCount = 0;
    require(dtStatusSucceed(query->findPath(
                endRef, startRef, nearestEnd, nearestStart, &filter, reversePath, &reversePathCount, 16)),
    ...
    require(dtStatusSucceed(query->findStraightPath(nearestEnd,
                                                    nearestStart,
                                                    reversePath,
                                                    reversePathCount,
                                                    reverseStraight,
    ...
    require_no_duplicate_linear_straight_points(reverseStraight,
                                                reverseStraightFlags,
                                                reverseStraightRefs,
                                                reverseStraightCount,
