...
namespace {
    TaskSettings3D task_settings(int priority, std::vector<double> diagonal_weight, std::string diagnostic_name) {
        return {
            .priority = priority,
    ...
    Vec3 tuple_vec3(nb::tuple value) {
        ...
        return {nb::cast<double>(value[0]), nb::cast<double>(value[1]), nb::cast<double>(value[2])};
    }
    ...
    public:
        ...
        VelocityControlResult3D solve(nb::list tasks, double time_step) {
            const std::vector<const ArticulationTask3D*> pointers = task_pointers(std::move(tasks));
            return controller_.solve(pointers, {.time_step = time_step});
        }

        VelocityControlResult3D solve_point_velocity(std::size_t unit_index,
                                                     nb::tuple point_local,
                                                     nb::tuple target_velocity_world,
                                                     double time_step,
                                                     double maximum_joint_velocity) {
            if (!std::isfinite(maximum_joint_velocity) || maximum_joint_velocity <= 0.0) {
                throw nb::value_error("maximum_joint_velocity must be finite and positive");
            }
            PointVelocityTask3D point_task(unit_index,
            ...
            JointVelocityLimitConstraint3D velocity_limit(
                {},
                std::vector<double>(articulation_->unit_count(), -maximum_joint_velocity),
                std::vector<double>(articulation_->unit_count(), maximum_joint_velocity),
    ...
    std::vector<InverseDynamicsActuator3D> make_actuators(const std::vector<std::size_t>& dof_indices,
                                                          const std::vector<double>& effort_limits) {
        if (dof_indices.size() != effort_limits.size()) {
            throw nb::value_error("dof_indices and effort_limits must have equal length");
        }
        ...
        for (std::size_t index = 0; index < dof_indices.size(); ++index) {
            ...
            if (!std::isfinite(limit) || limit < 0.0) {
                throw nb::value_error("effort limits must be finite and non-negative");
            }
            result.push_back({
    ...
    public:
        PythonInverseDynamicsHqpController3D(Articulation3D& articulation,
                                             const std::vector<std::size_t>& dof_indices,
                                             const std::vector<double>& effort_limits,
                                             nb::tuple gravity_world)
            : articulation_(&articulation),
              controller_(articulation, make_actuators(dof_indices, effort_limits), tuple_vec3(gravity_world)) {}
        ...
        InverseDynamicsControlResult3D solve_point_acceleration(std::size_t unit_index,
                                                                nb::tuple point_local,
            ...
            PointAccelerationTask3D point_task(unit_index,
                                               tuple_vec3(point_local),
                                               ...
                                               velocity_gain,
                                               {.priority = 1, .diagnostic_name = "point tracking"});
            JointLimitConstraint3D position_limit({.priority = 0, .diagnostic_name = "joint position limit"});
            JointVelocityLimitConstraint3D velocity_limit(
                {},
                std::vector<double>(articulation_->unit_count(), -maximum_joint_velocity),
                std::vector<double>(articulation_->unit_count(), maximum_joint_velocity),
                {.priority = 0, .diagnostic_name = "joint velocity limit"});
            const std::array<const ArticulationTask3D*, 3> tasks{&position_limit, &velocity_limit, &point_task};
            return controller_.solve(tasks, {.time_step = time_step});
        }
...
NB_MODULE(_robotics_native, m) {
    ...
    nb::class_<Articulation3D>(m, "Articulation3D")
        ...
        .def_prop_ro("coordinates", [](const Articulation3D& articulation) { return articulation.state().coordinates; })
        .def_prop_ro("velocities", [](const Articulation3D& articulation) { return articulation.state().velocities; })
        .def(
            "point_position",
            [](const Articulation3D& articulation, std::size_t unit_index, nb::tuple point_local) {
                const ArticulationPointKinematics3DResult result =
                    articulation.point_kinematics(unit_index, tuple_vec3(point_local));
                if (!result.ok()) {
                    throw std::runtime_error(std::string("point kinematics failed: ") +
                                             std::string(articulation3d_diagnostic_name(result.diagnostic)));
                }
    ...
    nb::class_<PointVelocityTask3D, ArticulationTask3D>(m, "PointVelocityTask3D")
        .def(
            "__init__",
            [](PointVelocityTask3D* self,
               ...
               std::string diagnostic_name) {
                new (self) PointVelocityTask3D(
                    ...
                    tuple_vec3(point_local),
                    tuple_vec3(target_velocity_world),
                    task_settings(priority, std::move(diagonal_weight), std::move(diagnostic_name)));
            },
            nb::arg("unit_index"),
            nb::arg("point_local"),
    ...
    nb::class_<PointAccelerationTask3D, ArticulationTask3D>(m, "PointAccelerationTask3D")
        .def(
            "__init__",
            ...
            nb::arg("feedforward_acceleration_world") = nb::make_tuple(0.0, 0.0, 0.0),
    ...
    nb::class_<JointLimitConstraint3D, ArticulationTask3D>(m, "JointLimitConstraint3D")
        .def(
            "__init__",
            [](JointLimitConstraint3D* self, int priority, std::string diagnostic_name) {
                new (self) JointLimitConstraint3D(task_settings(priority, {}, std::move(diagnostic_name)));
            },
            nb::kw_only(),
            nb::arg("priority") = 0,
            nb::arg("diagnostic_name") = "joint position limit");

    nb::class_<JointVelocityLimitConstraint3D, ArticulationTask3D>(m, "JointVelocityLimitConstraint3D")
        .def(
            "__init__",
            [](JointVelocityLimitConstraint3D* self,
               ...
               int priority,
               std::string diagnostic_name) {
                new (self) JointVelocityLimitConstraint3D(std::move(joint_indices),
    ...
    nb::class_<VelocityControlResult3D>(m, "VelocityControlResult3D")
        ...
        .def_ro("generalized_velocity", &VelocityControlResult3D::generalized_velocity)
        .def_ro("level_priorities", &VelocityControlResult3D::level_priorities)
        .def_ro("level_task_residual_l2", &VelocityControlResult3D::level_task_residual_l2)
        .def_prop_ro("diagnostic",
                     [](const VelocityControlResult3D& result) {
                         return std::string(velocity_control_diagnostic_name(result.diagnostic));
                     })
    ...
    nb::class_<PythonVelocityHqpController3D>(m, "VelocityHqpController3D")
        ...
        .def("solve", &PythonVelocityHqpController3D::solve, nb::arg("tasks"), nb::arg("time_step"))
        ...
        .def("reset_primal_warm_start", &PythonVelocityHqpController3D::reset_primal_warm_start);

    nb::class_<InverseDynamicsControlResult3D>(m, "InverseDynamicsControlResult3D")
        .def_prop_ro("ok", &InverseDynamicsControlResult3D::ok)
        .def_ro("generalized_acceleration", &InverseDynamicsControlResult3D::generalized_acceleration)
        .def_ro("required_generalized_effort", &InverseDynamicsControlResult3D::required_generalized_effort)
        .def_ro("actuator_dof_indices", &InverseDynamicsControlResult3D::actuator_dofs)
        .def_ro("actuator_effort", &InverseDynamicsControlResult3D::actuator_effort)
        .def_ro("level_priorities", &InverseDynamicsControlResult3D::level_priorities)
        .def_ro("level_task_residual_l2", &InverseDynamicsControlResult3D::level_task_residual_l2)
        .def_prop_ro("diagnostic",
                     [](const InverseDynamicsControlResult3D& result) {
                         return std::string(inverse_dynamics_control_diagnostic_name(result.diagnostic));
                     })
        .def_ro("failed_task_name", &InverseDynamicsControlResult3D::failed_task_name);

    nb::class_<PythonInverseDynamicsHqpController3D>(m, "InverseDynamicsHqpController3D")
        .def(nb::init<Articulation3D&, const std::vector<std::size_t>&, const std::vector<double>&, nb::tuple>(),
             nb::arg("articulation"),
             nb::arg("dof_indices"),
             nb::arg("effort_limits"),
             nb::arg("gravity_world") = nb::make_tuple(0.0, 0.0, 0.0),
             nb::keep_alive<1, 2>())
        .def("solve", &PythonInverseDynamicsHqpController3D::solve, nb::arg("tasks"), nb::arg("time_step"))
        ...
        .def("reset_primal_warm_start", &PythonInverseDynamicsHqpController3D::reset_primal_warm_start);
}
