...
int main() {
    ...
    TERMIN_QOPT_CHECK(rows.register_block(1, "body").diagnostic == AssemblyDiagnostic::DuplicateBlockName);
    TERMIN_QOPT_CHECK(rows.block_count() == 2);
    TERMIN_QOPT_CHECK(rows.total_size() == 3);

    std::array<double, 9> premature_storage{};
    DenseBlockMatrixAssembly premature(rows, rows, DenseMatrixView::row_major(premature_storage.data(), 3, 3));
    TERMIN_QOPT_CHECK(premature.diagnostic() == AssemblyDiagnostic::TopologyNotFinalized);

    TERMIN_QOPT_CHECK(rows.finalize() == AssemblyDiagnostic::None);
    TERMIN_QOPT_CHECK(rows.register_block(1, "late").diagnostic == AssemblyDiagnostic::TopologyFinalized);
    ...
    const DenseBlockRegistrationResult state = columns.register_block(3, "state");
    ...
    DenseBlockMatrixAssembly matrix(moved, columns, DenseMatrixView::row_major(matrix_storage.data(), 3, 3));
    ...
    TERMIN_QOPT_CHECK(
        matrix.add(body.handle, state.handle, ConstDenseMatrixView::row_major(body_contribution.data(), 2, 3)) ==
        AssemblyDiagnostic::None);
    const std::array<double, 3> joint_contribution{7.0, 8.0, 9.0};
    TERMIN_QOPT_CHECK(
        matrix.add(joint.handle, state.handle, ConstDenseMatrixView::row_major(joint_contribution.data(), 1, 3)) ==
        AssemblyDiagnostic::None);
    ...
    TERMIN_QOPT_CHECK(
        matrix.add(body.handle, state.handle, ConstDenseMatrixView::row_major(wrong_shape.data(), 1, 1)) ==
        AssemblyDiagnostic::DimensionMismatch);
    DenseBlockTopology foreign;
    const DenseBlockRegistrationResult foreign_block = foreign.register_block(2, "foreign");
    TERMIN_QOPT_CHECK(foreign.finalize() == AssemblyDiagnostic::None);
    TERMIN_QOPT_CHECK(matrix.block(foreign_block.handle, state.handle).diagnostic == AssemblyDiagnostic::InvalidBlock);

    std::array<double, 3> vector_storage{4.0, 4.0, 4.0};
    DenseBlockVectorAssembly vector(moved, DenseVectorView{vector_storage.data(), vector_storage.size(), 1});
    ...
    TERMIN_QOPT_CHECK(vector.add(body.handle, {body_vector.data(), body_vector.size(), 1}) == AssemblyDiagnostic::None);
    TERMIN_QOPT_CHECK(vector.add(joint.handle, {joint_vector.data(), joint_vector.size(), 1}) ==
                      AssemblyDiagnostic::None);
    ...
    TERMIN_QOPT_CHECK(vector.add(joint.handle, {&nan, 1, 1}) == AssemblyDiagnostic::NonFiniteContribution);
    TERMIN_QOPT_CHECK(vector_storage[2] == 5.0);
