...
namespace termin::qopt {
    namespace {
        ...
        [[nodiscard]] NullspaceResult nullspace_failure(QpStatus status, QpDiagnostic diagnostic) noexcept {
            NullspaceResult result;
        ...
        [[nodiscard]] ComputedNullspace compute(ConstDenseMatrixView source, NullspaceOptions options) {
            ...
            if (matrix.rows() == 0) {
                ...
                qr.setThreshold(scaled_tolerance(options.tolerance, matrix_linf(matrix)));
                const Eigen::Index rank = qr.rank();
                const Matrix orthogonal = qr.householderQ() * Matrix::Identity(variables, variables);
                output.basis = orthogonal.rightCols(variables - rank);
                output.result.status = QpStatus::Optimal;
                output.result.rank = static_cast<std::size_t>(rank);
                output.result.nullity = static_cast<std::size_t>(variables - rank);
            } else {
                Eigen::JacobiSVD<Matrix> svd(matrix, Eigen::ComputeFullU | Eigen::ComputeFullV);
                if (svd.info() != Eigen::Success) {
                    output.result = nullspace_failure(QpStatus::NumericalFailure, QpDiagnostic::DecompositionFailure);
                    return output;
                }
                const Vector singular_values = svd.singularValues();
                const double threshold = scaled_tolerance(options.tolerance, linf(singular_values));
                Eigen::Index rank = 0;
            ...
            const Matrix orthogonality =
                output.basis.transpose() * output.basis - Matrix::Identity(output.basis.cols(), output.basis.cols());
        ...
        validate(ConstDenseMatrixView matrix, DenseMatrixView destination, NullspaceOptions options) noexcept {
            ...
            const QpDiagnostic destination_diagnostic = validate_matrix(ConstDenseMatrixView{destination});
            if (destination_diagnostic != QpDiagnostic::None) {
                return destination_diagnostic;
            }
            if (destination.rows != matrix.columns || destination.columns != matrix.columns) {
    ...
    NullspaceResult
    write_nullspace_basis(ConstDenseMatrixView matrix, DenseMatrixView destination, NullspaceOptions options) noexcept {
                ...
                for (std::size_t column = 0; column < destination.columns; ++column) {
                    destination(row, column) =
                        column < computed.result.nullity
                            ? computed.basis(static_cast<Eigen::Index>(row), static_cast<Eigen::Index>(column))
                            : 0.0;
                }
            }
            return computed.result;
        } catch (const std::exception& error) {
            std::fprintf(stderr, "[termin-qopt] nullspace basis failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr, "[termin-qopt] nullspace basis failed with unknown exception\n");
        }
        return nullspace_failure(QpStatus::NumericalFailure, QpDiagnostic::DecompositionFailure);
    }

    NullspaceResult write_nullspace_projector(ConstDenseMatrixView matrix,
        ...
        try {
            ...
            const Matrix projector = computed.basis * computed.basis.transpose();
            for (std::size_t row = 0; row < destination.rows; ++row) {
                for (std::size_t column = 0; column < destination.columns; ++column) {
                    destination(row, column) =
                        projector(static_cast<Eigen::Index>(row), static_cast<Eigen::Index>(column));
                }
            }
            return computed.result;
        } catch (const std::exception& error) {
            std::fprintf(stderr, "[termin-qopt] nullspace projector failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr, "[termin-qopt] nullspace projector failed with unknown exception\n");
        }
        return nullspace_failure(QpStatus::NumericalFailure, QpDiagnostic::DecompositionFailure);
    }
