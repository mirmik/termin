...
namespace termin::qopt {
    namespace {
        ...
        [[nodiscard]] bool any_outputs_overlap(ActiveSetQpSolutionView solution) noexcept {
            const DenseVectorView outputs[] = {
                solution.primal,
                solution.equality_dual,
                solution.inequality_dual,
                solution.lower_bound_dual,
        ...
        [[nodiscard]] EqualityResult solve_equalities(const Matrix& hessian,
            ...
            const EqualityQpProblemView problem = {view(hessian), view(gradient), view(equalities), view(targets)};
            const EqualityQpSolutionView solution = {view(solved.primal), view(solved.dual)};
            solved.result = prefer_spd ? solve_equality_qp_spd_first(problem, solution, tolerance)
                                       : solve_equality_qp(problem, solution, tolerance);
            return solved;
        }

        [[nodiscard]] Matrix nullspace_basis(const Matrix& matrix, QpTolerance tolerance, bool& success) {
            ...
            Eigen::JacobiSVD<Matrix> svd(normalized, Eigen::ComputeFullU | Eigen::ComputeFullV);
        ...
        check_convexity(const Matrix& hessian, const Matrix& equalities, QpTolerance tolerance) {
            bool success = false;
            ...
            const double curvature_tolerance = scaled_tolerance(tolerance, linf(eigenvalues));
        ...
        [[nodiscard]] DirectionResult recession_direction(const Matrix& hessian,
            ...
            const Vector spectral_gradient = eigenvectors.transpose() * nullspace.transpose() * objective_gradient;
        ...
        [[nodiscard]] bool contains(const std::vector<std::size_t>& indices, std::size_t index) {
        ...
        [[nodiscard]] std::optional<Vector> independent_row_direction(const Vector& input, const RowBasis& basis) {
        ...
        [[nodiscard]] bool append_independent_row(const Vector& input, RowBasis& basis) {
        ...
        [[nodiscard]] RowBasis row_basis(const Matrix& rows) {
            ...
            for (Eigen::Index row = 0; row < rows.rows() && basis.size() < static_cast<std::size_t>(rows.cols());
                 ++row) {
        ...
        [[nodiscard]] std::vector<std::size_t> independent_active_set(const Matrix& equalities,
                                                                      const Matrix& inequalities,
            ...
            for (const std::size_t candidate : candidates) {
                ...
                if (append_independent_row(inequalities.row(static_cast<Eigen::Index>(candidate)).transpose(), basis)) {
        ...
        [[nodiscard]] CoreResult solve_feasible_core(const Matrix& hessian,
                                                     const Vector& gradient,
                                                     const Matrix& equalities,
                                                     const Vector& targets,
                                                     const Matrix& inequalities,
                                                     ...
                                                     Vector primal,
                                                     std::vector<std::size_t> active,
                                                     const ActiveSetQpOptions& options,
            ...
            std::vector<std::uint8_t> zero_step_readded(static_cast<std::size_t>(inequalities.rows()), 0);
            std::vector<std::size_t> zero_step_readd_counts(static_cast<std::size_t>(inequalities.rows()), 0);
            ...
            while (iterations < options.max_iterations) {
                ...
                Matrix working(equalities.rows() + static_cast<Eigen::Index>(active.size()), hessian.rows());
                Vector working_targets(targets.size() + static_cast<Eigen::Index>(active.size()));
                ...
                const EqualityResult subproblem =
                    solve_equalities(hessian, gradient, working, working_targets, options.tolerance, !active.empty());
                Vector direction;
                ...
                if (subproblem.result.status == QpStatus::Optimal) {
                    ...
                    DirectionResult recession =
                        recession_direction(hessian, gradient, working, primal, options.tolerance);
                    if (recession.status != QpStatus::Unbounded) {
                        CoreResult result;
                        result.result = failure(recession.status, recession.diagnostic, iterations);
                ...
                for (Eigen::Index row = 0; row < inequalities.rows(); ++row) {
                    ...
                    if (!independent_row_direction(inequalities.row(row).transpose(), working_basis).has_value()) {
                        continue;
                    }
                    ...
                    const double tie_tolerance = scaled_tolerance(options.tolerance, std::max(step, candidate));
                    if (candidate < step - tie_tolerance ||
                        (!blocker.has_value() && !finite_target && std::isfinite(candidate)) ||
                        (std::abs(candidate - step) <= tie_tolerance && blocker.has_value() && index < *blocker)) {
                        step = candidate;
                        blocker = index;
                    }
                }
                const double step_tolerance = scaled_tolerance(options.tolerance, std::max(1.0, std::abs(step)));
                ...
                if (blocked_before_target) {
                    ...
                    const bool repeats_last_drop = last_dropped_constraint == blocker && step <= step_tolerance;
                ...
                if (!finite_target) {
                    CoreResult result;
                    result.result = failure(QpStatus::Unbounded, QpDiagnostic::LinearDescentInNullspace, iterations);
                    result.result.active_set_size = active.size();
                    return result;
                }
                ...
                for (std::size_t offset = 0; offset < active.size(); ++offset) {
                    const double multiplier = subproblem.dual[equality_count + static_cast<Eigen::Index>(offset)];
                ...
                if (removal_offset.has_value()) {
                    ...
                    record_pivot({
                        false,
                        removed_constraint,
                        subproblem.dual[equality_count + static_cast<Eigen::Index>(*removal_offset)],
                    });
                    if (zero_step_readded[removed_constraint] != 0) {
                        std::size_t& repeat_count = zero_step_readd_counts[removed_constraint];
                        ++repeat_count;
                        if (repeat_count >= 2) {
                    ...
                    active.erase(active.begin() + static_cast<std::ptrdiff_t>(*removal_offset));
                    continue;
                }
            ...
            if (pivot_event_count != 0) {
                ...
                const std::size_t trace_begin =
                    pivot_event_count > pivot_trace.size() ? pivot_event_count - pivot_trace.size() : 0;
                for (std::size_t index = trace_begin; index < pivot_event_count; ++index) {
                    const PivotEvent& event = pivot_trace[index % pivot_trace.size()];
                    std::fprintf(stderr,
                                 "  pivot[%zu]=%s constraint=%zu value=%.17g\n",
                                 index,
                                 event.added ? "add" : "drop",
                                 event.constraint,
                                 event.value);
                }
            }
            result.result = failure(QpStatus::NumericalFailure, QpDiagnostic::IterationLimit, iterations);
            result.result.active_set_size = active.size();
            return result;
        }

        [[nodiscard]] NormalizedInequalities normalize_inequalities(const Matrix& inequalities,
                                                                    const Vector& limits,
                                                                    const Vector& lower,
                                                                    const Vector& upper) {
            ...
            const Eigen::Index rows =
                inequalities.rows() + static_cast<Eigen::Index>(finite_lower) + static_cast<Eigen::Index>(finite_upper);
            ...
            for (Eigen::Index index = 0; index < inequalities.rows(); ++index, ++row) {
                ...
                const double coefficient_scale = normalized.matrix.row(row).cwiseAbs().maxCoeff();
                ...
                normalized.origins.push_back({ConstraintFamily::Inequality, static_cast<std::size_t>(index)});
            }
            for (Eigen::Index index = 0; index < lower.size(); ++index) {
                ...
                normalized.origins.push_back({ConstraintFamily::LowerBound, static_cast<std::size_t>(index)});
                ++row;
            }
            for (Eigen::Index index = 0; index < upper.size(); ++index) {
                ...
                normalized.origins.push_back({ConstraintFamily::UpperBound, static_cast<std::size_t>(index)});
                ++row;
            }
            return normalized;
        }
        ...
        [[nodiscard]] ConstraintResidual constraint_residual(const Matrix& inequalities,
            ...
            return {
                value,
                value - limits[row],
                scaled_tolerance(tolerance, std::max(std::abs(value), std::abs(limits[row]))),
            };
        }

        [[nodiscard]] std::vector<std::size_t> tight_constraints(const Matrix& inequalities,
                                                                 const Vector& limits,
                                                                 const Vector& primal,
                                                                 QpTolerance tolerance) {
            std::vector<std::size_t> active;
            for (Eigen::Index row = 0; row < inequalities.rows(); ++row) {
                const ConstraintResidual checked = constraint_residual(inequalities, limits, primal, row, tolerance);
        ...
        [[nodiscard]] CoreResult find_feasible_point(const Matrix& equalities,
                                                     const Vector& targets,
                                                     const Matrix& inequalities,
                                                     const Vector& limits,
                                                     const Vector& seed,
            ...
            Matrix phase_inequalities = Matrix::Zero(inequalities.rows() + 1, variables + 1);
            Vector phase_limits(limits.size() + 1);
            if (inequalities.rows() > 0) {
                phase_inequalities.topLeftCorner(inequalities.rows(), variables) = inequalities;
            ...
            if (inequalities.rows() > 0) {
                maximum_violation = std::max(0.0, (inequalities * seed - limits).maxCoeff());
            }
            const double margin = 10.0 * scaled_tolerance(options.tolerance, maximum_violation);
            Vector phase_start(variables + 1);
            phase_start.head(variables) = seed;
            phase_start[variables] = maximum_violation + margin;

            std::vector<std::size_t> active =
                tight_constraints(phase_inequalities, phase_limits, phase_start, options.tolerance);
            return solve_feasible_core(phase_hessian,
                                       phase_gradient,
                                       phase_equalities,
                                       targets,
                                       phase_inequalities,
                                       phase_limits,
                                       std::move(phase_start),
                                       std::move(active),
                                       options,
                                       iterations);
        }
        ...
        [[nodiscard]] QpSolveResult solve_impl(ActiveSetQpProblemView problem,
            ...
            if (!std::isfinite(options.active_tolerance) || options.active_tolerance < 0.0 ||
            ...
            if (variables == 0 || problem.hessian.columns != variables || problem.gradient.size != variables ||
                problem.equalities.columns != variables || problem.equality_targets.size != equality_count ||
                problem.inequalities.columns != variables || problem.inequality_limits.size != inequality_count ||
                (has_lower_bounds && problem.lower_bounds.size != variables) ||
                (has_upper_bounds && problem.upper_bounds.size != variables) || solution.primal.size != variables ||
                solution.equality_dual.size != equality_count || solution.inequality_dual.size != inequality_count ||
                ...
                (!warm_start.active_inequalities.empty() && warm_start.active_inequalities.size != inequality_count) ||
            ...
            if (!finite(problem.hessian) || !finite(problem.gradient) || !finite(problem.equalities) ||
                !finite(problem.equality_targets) || !finite(problem.inequalities) ||
                !finite(problem.inequality_limits) || (!warm_start.primal.empty() && !finite(warm_start.primal)) ||
                !finite(warm_start.active_inequalities) || !finite(warm_start.active_lower_bounds) ||
                !finite(warm_start.active_upper_bounds)) {
                return failure(QpStatus::InvalidInput, QpDiagnostic::NonFiniteInput);
            }
            if (!valid_mask(warm_start.active_inequalities) || !valid_mask(warm_start.active_lower_bounds) ||
            ...
            for (std::size_t index = 0; index < variables; ++index) {
                const double lower =
                    has_lower_bounds ? problem.lower_bounds[index] : -std::numeric_limits<double>::infinity();
                const double upper =
                    has_upper_bounds ? problem.upper_bounds[index] : std::numeric_limits<double>::infinity();
                if (std::isnan(lower) || std::isnan(upper) || lower == std::numeric_limits<double>::infinity() ||
            ...
            if (!warm_start.active_lower_bounds.empty()) {
                for (std::size_t index = 0; index < variables; ++index) {
                    if (warm_start.active_lower_bounds[index] == 1.0 && !std::isfinite(problem.lower_bounds[index])) {
                        return failure(QpStatus::InvalidInput, QpDiagnostic::InvalidWarmStart);
                    }
                }
            }
            if (!warm_start.active_upper_bounds.empty()) {
                for (std::size_t index = 0; index < variables; ++index) {
                    if (warm_start.active_upper_bounds[index] == 1.0 && !std::isfinite(problem.upper_bounds[index])) {
                        return failure(QpStatus::InvalidInput, QpDiagnostic::InvalidWarmStart);
                    }
                }
            }
            ...
            if (symmetry_residual > options.tolerance.symmetry * std::max(1.0, hessian_scale)) {
            ...
            Matrix feasibility_hessian =
                Matrix::Identity(static_cast<Eigen::Index>(variables), static_cast<Eigen::Index>(variables));
            const Vector feasibility_gradient = Vector::Zero(static_cast<Eigen::Index>(variables));
            const EqualityResult equality_feasible = solve_equalities(
                feasibility_hessian, feasibility_gradient, equalities, equality_targets, options.tolerance);
            ...
            const QpSolveResult convexity = check_convexity(hessian, equalities, options.tolerance);
            ...
            if (!warm_start.primal.empty()) {
                ...
                if (equality_error > options.active_tolerance ||
                    !constraints_feasible(
                        normalized.matrix, normalized.limits, primal, options.active_tolerance, options.tolerance)) {
                    return failure(QpStatus::InvalidInput, QpDiagnostic::InvalidWarmStart);
                }
                ...
                if (has_warm_masks) {
                    for (std::size_t row = 0; row < normalized.origins.size(); ++row) {
                        ...
                        const ConstraintResidual checked = constraint_residual(normalized.matrix,
                                                                               normalized.limits,
                                                                               primal,
                                                                               static_cast<Eigen::Index>(row),
                                                                               options.tolerance);
                        if (std::abs(checked.residual) > std::max(options.active_tolerance, checked.tolerance)) {
                            return failure(QpStatus::InvalidInput, QpDiagnostic::InvalidWarmStart);
                        }
                        if (std::abs(checked.residual) <= checked.tolerance) {
                    ...
                    initial_active = tight_constraints(normalized.matrix, normalized.limits, primal, options.tolerance);
                }
            } else {
                ...
                const EqualityResult objective_seed =
                    solve_equalities(hessian, gradient, equalities, equality_targets, options.tolerance);
                primal = objective_seed.result.status == QpStatus::Optimal ? objective_seed.primal
                                                                           : equality_feasible.primal;
                if (!constraints_feasible(normalized.matrix, normalized.limits, primal, 0.0, options.tolerance)) {
                    CoreResult phase = find_feasible_point(equalities,
                                                           equality_targets,
                                                           normalized.matrix,
                                                           normalized.limits,
                                                           primal,
                    ...
                    const double phase_slack = phase.primal[static_cast<Eigen::Index>(variables)];
                    const double feasibility_tolerance =
                        scaled_tolerance(options.tolerance, std::max(1.0, std::abs(phase_slack)));
                    if (phase_slack > feasibility_tolerance) {
                        QpSolveResult result =
                            failure(QpStatus::Infeasible, QpDiagnostic::InconsistentInequalities, iterations);
            ...
            CoreResult solved = solve_feasible_core(hessian,
                                                    gradient,
                                                    equalities,
                                                    equality_targets,
                                                    normalized.matrix,
            ...
            for (std::size_t offset = 0; offset < solved.active.size(); ++offset) {
                const double multiplier = solved.active_dual[static_cast<Eigen::Index>(offset)];
                normalized_dual[static_cast<Eigen::Index>(solved.active[offset])] = multiplier;
            }
            ...
            const Vector normalized_inequality_slack = normalized.matrix * solved.primal - normalized.limits;
            const Vector complementarity = normalized_dual.cwiseProduct(normalized_inequality_slack);

            double original_inequality_violation = 0.0;
            if (inequalities.rows() > 0) {
                original_inequality_violation =
                    std::max(0.0, (inequalities * solved.primal - inequality_limits).maxCoeff());
            }
            for (Eigen::Index index = 0; index < lower.size(); ++index) {
                if (std::isfinite(lower[index])) {
                    original_inequality_violation =
                        std::max(original_inequality_violation, lower[index] - solved.primal[index]);
                }
            }
            for (Eigen::Index index = 0; index < upper.size(); ++index) {
                if (std::isfinite(upper[index])) {
                    original_inequality_violation =
                        std::max(original_inequality_violation, solved.primal[index] - upper[index]);
                }
            }
            ...
            solved.result.dual_linf = std::max({
                inequality_dual.size() == 0 ? 0.0 : std::max(0.0, -inequality_dual.minCoeff()),
            ...
            const double residual_tolerance = scaled_tolerance(options.tolerance, residual_scale);
            const double feasibility_tolerance = std::max(options.active_tolerance, residual_tolerance);
            const double normalized_dual_violation =
                normalized_dual.size() == 0 ? 0.0 : std::max(0.0, -normalized_dual.minCoeff());
            if (solved.result.stationarity_linf > residual_tolerance ||
                solved.result.equality_linf > feasibility_tolerance ||
                !constraints_feasible(
                    normalized.matrix, normalized.limits, solved.primal, options.active_tolerance, options.tolerance) ||
    ...
    QpSolveResult solve_active_set_qp(ActiveSetQpProblemView problem,
        ...
        try {
            ...
            std::fprintf(stderr, "[termin-qopt] active-set QP failed with an exception: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr, "[termin-qopt] active-set QP failed with an unknown exception\n");
        }
        return failure(QpStatus::NumericalFailure, QpDiagnostic::DecompositionFailure);
    }
