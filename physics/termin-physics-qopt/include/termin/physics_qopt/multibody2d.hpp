...
namespace termin::physics_qopt {
    ...
    struct RigidBody2DHandle {
        ...
        [[nodiscard]] constexpr bool valid() const noexcept {
            return system_id != 0 && index != std::numeric_limits<std::size_t>::max();
        }
    };
    ...
    template <typename Handle> struct Multibody2DRegistrationResult {
        Handle handle;
    ...
    struct Multibody2DStepResult {
        ...
        double position_constraint_linf = std::numeric_limits<double>::infinity();
        double velocity_constraint_linf = std::numeric_limits<double>::infinity();
        std::size_t position_iterations = 0;
    };

    // A small dense maximal-coordinate model. Model state, topology, numerical
    // assembly and QP solving remain separate internally; this class
    // orchestrates their lifetime for runtime consumers.
    class TERMIN_PHYSICS_QOPT_API Multibody2DSystem {
    public:
        Multibody2DSystem();
        ~Multibody2DSystem();
        ...
        [[nodiscard]] Multibody2DRegistrationResult<RigidBody2DHandle>
        add_body(SpatialInertia2D inertia,
        ...
        add_fixed_point_joint(RigidBody2DHandle body,
                              termin::Vec2 body_anchor_local,
                              termin::Vec2 world_anchor,
                              std::string_view diagnostic_name = {}) noexcept;

        [[nodiscard]] Multibody2DRegistrationResult<Joint2DHandle>
        add_revolute_joint(RigidBody2DHandle body_a,
        ...
        [[nodiscard]] Multibody2DDiagnostic set_body_state(RigidBody2DHandle body, RigidBody2DState state) noexcept;
        [[nodiscard]] Multibody2DDiagnostic set_body_wrench(RigidBody2DHandle body, RigidBody2DWrench wrench) noexcept;
        [[nodiscard]] RigidBody2DState body_state(RigidBody2DHandle body) const noexcept;
        [[nodiscard]] RigidBody2DAcceleration body_acceleration(RigidBody2DHandle body) const noexcept;
        [[nodiscard]] termin::Vec2 joint_reaction(Joint2DHandle joint) const noexcept;

        [[nodiscard]] Multibody2DStepResult step(termin::Vec2 gravity_world,
