...
namespace
{
    ...
    RigidBody2DState state(const OracleBody2D& body)
    {
        return {
            {body.initial_pose[2],
             {body.initial_pose[0], body.initial_pose[1]}},
    ...
    bool finite_state(RigidBody2DState body, double limit)
    {
        return std::isfinite(body.pose.lin.x) &&
               std::isfinite(body.pose.lin.y) && std::isfinite(body.pose.ang) &&
...
int main()
{
    ...
    TERMIN_QOPT_CHECK(
        system
            .add_fixed_point_joint(first.handle,
    ...
    for (std::size_t step = 0; step <= oracle.steps; ++step)
    {
        ...
        {
            max_constraint = std::max(max_constraint,
                                      system.max_position_constraint_error());
            TERMIN_QOPT_CHECK(finite_state(system.body_state(first.handle),
                                           oracle.finite_state_limit));
            TERMIN_QOPT_CHECK(finite_state(system.body_state(second.handle),
                                           oracle.finite_state_limit));
            ++sample_index;
        }
        ...
        const Multibody2DStepResult result =
            system.step(gravity,
                        {
    ...
    TERMIN_QOPT_CHECK(system.max_velocity_constraint_error() <=
                      oracle.constraint_linf);
