...
namespace
{
    ...
    void sample_energy(EnergyEnvelope& envelope, double energy)
    {
        ...
        envelope.maximum_relative_drift =
            std::max(envelope.maximum_relative_drift,
                     std::abs(energy - envelope.initial) /
                         std::max(1.0, std::abs(envelope.initial)));
    }
    ...
    EnergyEnvelope simulate_single_pendulum(double dt, double duration)
    {
        ...
        const std::size_t steps =
            static_cast<std::size_t>(std::llround(duration / dt));
        ...
        for (std::size_t step = 0; step < steps; ++step)
        {
            ...
            result.maximum_relative_energy_drift =
                std::max(result.maximum_relative_energy_drift,
                         std::abs(body->total_energy() - initial_energy) /
                             std::max(1.0, std::abs(initial_energy)));
            result.maximum_angular_momentum_error = std::max(
                           ...
                           double fine_bound)
    {
        const bool converges = std::isfinite(coarse.maximum_relative_drift) &&
                               ...
                               fine.maximum_relative_drift <
                                   0.35 * coarse.maximum_relative_drift &&
...
int main()
{
    ...
    const EnergyEnvelope single_coarse =
        simulate_single_pendulum(coarse_dt, duration);
    const EnergyEnvelope single_fine =
        simulate_single_pendulum(fine_dt, duration);
    check_convergence("single pendulum",
                      coarse_dt,
                      single_coarse,
                      fine_dt,
                      single_fine,
    ...
    const EnergyEnvelope double_coarse =
        simulate_double_pendulum(coarse_dt, duration);
    const EnergyEnvelope double_fine =
        simulate_double_pendulum(fine_dt, duration);
    check_convergence("double pendulum",
                      coarse_dt,
                      double_coarse,
                      fine_dt,
                      double_fine,
    ...
    const EnergyEnvelope single_reference =
        simulate_single_pendulum(0.0005, duration);
    const EnergyEnvelope double_reference =
        simulate_double_pendulum(0.0005, duration);
    ...
    const EnergyEnvelope double_long_fine =
        simulate_double_pendulum(0.00125, 50.0);
    ...
    TERMIN_QOPT_CHECK(double_long_fine.maximum_relative_drift <
                      0.3 * double_long.maximum_relative_drift);
    TERMIN_QOPT_CHECK(free_fine.maximum_relative_energy_drift <
                      0.35 * free_coarse.maximum_relative_energy_drift);
    TERMIN_QOPT_CHECK(free_fine.maximum_angular_momentum_error <
                      0.35 * free_coarse.maximum_angular_momentum_error);
