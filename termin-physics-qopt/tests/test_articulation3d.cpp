...
namespace {
        ...
        return {
            {
                ...
                .parent_to_unit_zero = Pose3::translation(kHalfLength, 0.0, 0.0),
                .motion_twist_at_unit =
                    Screw3{Vec3::unit_y(), Vec3::zero()}.adjoint_inv(Pose3::translation(kHalfLength, 0.0, 0.0)),
                .inertia = rod_inertia(),
    ...
    AssembledSystem assemble(DynamicsContribution& contribution) {
        ...
        const AssemblyDiagnostic diagnostic = contribution.assemble(assembly, DynamicsAssemblyPhase::Acceleration);
    ...
    void test_model_validation() {
        ...
        Articulation3D invalid_parent(std::move(units), {{0.0, 0.0}, {0.0, 0.0}});
        TERMIN_QOPT_CHECK(invalid_parent.diagnostic() == Articulation3DDiagnostic::InvalidParent);

        units = double_pendulum_units();
        units[0].limits = {.minimum = 1.0, .maximum = -1.0};
        Articulation3D invalid_limits(std::move(units), {{0.0, 0.0}, {0.0, 0.0}});
        TERMIN_QOPT_CHECK(invalid_limits.diagnostic() == Articulation3DDiagnostic::InvalidUnitLimits);

        units = double_pendulum_units();
        units[0].limits.minimum = std::numeric_limits<double>::quiet_NaN();
        Articulation3D non_finite_limits(std::move(units), {{0.0, 0.0}, {0.0, 0.0}});
        TERMIN_QOPT_CHECK(non_finite_limits.diagnostic() == Articulation3DDiagnostic::InvalidUnitLimits);
    }
    ...
    void test_floating_base_matches_rigid_body() {
        ...
        Articulation3D floating_model(
            ArticulationFloatingBase3D{inertia, pose, velocity, "base"}, {}, {{}, {}}, "floating-only");
        Articulation3DDynamicsContribution floating(floating_model, gravity, "floating-only-dynamics");
        RigidBody3DContribution rigid(inertia, RigidBody3DState{pose, velocity}, gravity, "rigid-oracle");
        ...
        const PointKinematics3DResult floating_point = floating.floating_base_point_kinematics(point_local);
        ...
        check_near((floating_point.value.position_world - rigid_point.value.position_world).norm(), 0.0, 1e-12);
        check_near((floating_point.value.velocity_world - rigid_point.value.velocity_world).norm(), 0.0, 1e-12);
        for (std::size_t index = 0; index < floating_point.value.linear_jacobian_world_storage.size(); ++index) {
            check_near(floating_point.value.linear_jacobian_world_storage[index],
                       rigid_point.value.linear_jacobian_world_storage[index],
                       1e-12);
        }
        const ContactEndpoint3D base_endpoint = ContactEndpoint3D::articulation_base(floating, point_local);
        ...
        check_near((endpoint_point.value.velocity_world - floating_point.value.velocity_world).norm(), 0.0, 1e-12);

        DynamicsTopology rollback_topology;
        TERMIN_QOPT_CHECK(floating.register_topology(rollback_topology) == AssemblyDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(floating.set_trial_configuration(rollback_topology,
                                                           {trial_velocity.data(), trial_velocity.size(), 1},
                                                           0.01) == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK((floating.floating_base()->pose_world.lin - pose.lin).norm() > 1e-5);
        floating.rollback_step();
        check_near((floating.floating_base()->pose_world.lin - pose.lin).norm(), 0.0, 1e-12);
        check_near(rotation_error(floating.floating_base()->pose_world.ang, pose.ang), 0.0, 1e-12);

        Multibody3DSystem floating_dynamics;
        Articulation3D floating_step_model(
            ArticulationFloatingBase3D{inertia, pose, velocity, "base"}, {}, {{}, {}}, "floating-step");
        auto* floating_state =
            add(floating_dynamics,
                std::make_unique<Articulation3DDynamicsContribution>(floating_step_model, gravity, "floating-step"));
        ...
        TERMIN_QOPT_CHECK(floating_dynamics.finalize() == DynamicsSystemDiagnostic::None);
        ...
        check_near((result.pose_world.lin - rigid_state->state().pose.lin).norm(), 0.0, 1e-11);
        check_near(rotation_error(result.pose_world.ang, rigid_state->state().pose.ang), 0.0, 1e-11);
        check_near((result.velocity_local.ang - rigid_state->state().velocity_local.ang).norm(), 0.0, 1e-11);
        check_near((result.velocity_local.lin - rigid_state->state().velocity_local.lin).norm(), 0.0, 1e-11);
    }
    ...
    void test_floating_base_joint_coupling_and_point_jacobian() {
            ...
            {
                {
                    ...
                    .motion_twist_at_unit =
                        Screw3{Vec3::unit_y(), Vec3::zero()}.adjoint_inv(Pose3::translation(0.5, 0.0, 0.0)),
                    .inertia = rod_inertia(),
                    .diagnostic_name = "arm",
                },
            },
        ...
        const PointKinematics3DResult point = articulation.point_kinematics(0, {0.2, -0.1, 0.3});
        ...
        for (std::size_t column = 0; column < 7; ++column) {
            jacobian_velocity.x += point.value.linear_jacobian_world_storage[column] * generalized_velocity[column];
            jacobian_velocity.y += point.value.linear_jacobian_world_storage[7 + column] * generalized_velocity[column];
            jacobian_velocity.z +=
                point.value.linear_jacobian_world_storage[14 + column] * generalized_velocity[column];
        }
    ...
    void test_double_pendulum_equations() {
        ...
        Articulation3D model(
            double_pendulum_units(), {{upper_angle, relative_angle}, {0.0, 0.0}}, "reduced-double-pendulum");
        Articulation3DDynamicsContribution contribution(model, {0.0, 0.0, -kGravity});
        ...
        const double expected_load_0 =
            kGravity * (1.5 * std::cos(upper_angle) + 0.5 * std::cos(upper_angle + relative_angle));
        const double expected_load_1 = kGravity * 0.5 * std::cos(upper_angle + relative_angle);
    ...
    void test_velocity_bias() {
        ...
        Articulation3D model(double_pendulum_units(),
                             {{upper_angle, relative_angle}, {upper_velocity, relative_velocity}},
        ...
        const double coriolis_0 =
            -coupling * (2.0 * upper_velocity * relative_velocity + relative_velocity * relative_velocity);
        const double coriolis_1 = coupling * upper_velocity * upper_velocity;
        const double gravity_load_0 =
            kGravity * (1.5 * std::cos(upper_angle) + 0.5 * std::cos(upper_angle + relative_angle));
        const double gravity_load_1 = kGravity * 0.5 * std::cos(upper_angle + relative_angle);
    ...
    void test_mass_matrix_cache_tracks_configuration() {
        Articulation3D model(double_pendulum_units(), {{0.4, -0.3}, {1.2, -0.7}}, "mass-cache");
        Articulation3DDynamicsContribution contribution(model, {0.0, 0.0, -kGravity});
        ...
        ArticulationDynamicsAssemblyCounters counters = contribution.assembly_counters();
        ...
        TERMIN_QOPT_CHECK(contribution.set_state(std::move(changed_state)) == Articulation3DDiagnostic::None);
    ...
    void test_endpoint_iterations_reuse_mass_matrix() {
        Articulation3D model(double_pendulum_units(), {{0.4, -0.3}, {1.2, -0.7}}, "endpoint-mass-cache");
        Multibody3DSystem system;
        auto contribution = std::make_unique<Articulation3DDynamicsContribution>(
        ...
        const ArticulationDynamicsAssemblyCounters counters = state->assembly_counters();
        TERMIN_QOPT_CHECK(counters.mass_matrix_evaluations == 2);
        TERMIN_QOPT_CHECK(counters.bias_evaluations > counters.mass_matrix_evaluations);
    }
        ...
        return {
            {
                ...
                .parent_to_unit_zero = Pose3::identity(),
                .motion_twist_at_unit =
                    prismatic ? Screw3{Vec3::zero(), Vec3::unit_z()} : Screw3{Vec3::unit_y(), Vec3::zero()},
                .inertia = rod_inertia(),
    ...
    void test_joint_limits() {
        Articulation3D model(limited_unit(false), {{0.0}, {-2.0}}, "limited-revolute");
        Multibody3DSystem system;
        auto contribution =
            std::make_unique<Articulation3DDynamicsContribution>(model, Vec3::zero(), "limited-revolute");
        Articulation3DDynamicsContribution* state = contribution.get();
        TERMIN_QOPT_CHECK(system.add_contribution(std::move(contribution)) == DynamicsSystemDiagnostic::None);
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(std::abs(state->unit_limit_states()[0].maximum_reaction) < 1e-12);
        TERMIN_QOPT_CHECK(state->unit_limit_states()[0].signed_effort() > 0.0);
        TERMIN_QOPT_CHECK(state->total_energy() <= energy_before + 1e-12);

        TERMIN_QOPT_CHECK(state->set_state({{0.0}, {1.0}}) == Articulation3DDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(std::abs(state->unit_limit_states()[0].minimum_reaction) < 1e-12);
        TERMIN_QOPT_CHECK(!state->unit_limit_states()[0].minimum_active);

        TERMIN_QOPT_CHECK(state->set_state({{1.0}, {2.0}}) == Articulation3DDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(state->set_state({{0.5}, {0.1}}) == Articulation3DDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(state->set_state({{0.05}, {-10.0}}) == Articulation3DDiagnostic::None);
        ...
        Articulation3D prismatic_model(limited_unit(true), {{1.0}, {1.5}}, "limited-prismatic");
        Multibody3DSystem prismatic_system;
        auto prismatic =
            std::make_unique<Articulation3DDynamicsContribution>(prismatic_model, Vec3::zero(), "limited-prismatic");
        Articulation3DDynamicsContribution* prismatic_state = prismatic.get();
        TERMIN_QOPT_CHECK(prismatic_system.add_contribution(std::move(prismatic)) == DynamicsSystemDiagnostic::None);
        TERMIN_QOPT_CHECK(prismatic_system.finalize() == DynamicsSystemDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(prismatic_state->unit_limit_states()[0].maximum_reaction > 0.0);

        auto no_limits = limited_unit(false);
        no_limits[0].limits = {};
        Articulation3D unlimited_model(std::move(no_limits), {{0.0}, {-2.0}}, "unlimited");
        Multibody3DSystem unlimited_system;
        auto unlimited =
            std::make_unique<Articulation3DDynamicsContribution>(unlimited_model, Vec3::zero(), "unlimited");
        TERMIN_QOPT_CHECK(unlimited_system.add_contribution(std::move(unlimited)) == DynamicsSystemDiagnostic::None);
        TERMIN_QOPT_CHECK(unlimited_system.finalize() == DynamicsSystemDiagnostic::None);
        const DynamicsSystemStepResult unlimited_step = unlimited_system.step(options(0.01));
    ...
    void test_floating_base_free_motion_conserves_energy_and_momentum() {
        ...
        Articulation3D model(ArticulationFloatingBase3D{inertia, Pose3::identity(), velocity, "free-base"},
                             {},
                             {{}, {}},
                             "free-floating");
        Multibody3DSystem dynamics;
        auto* state =
            add(dynamics, std::make_unique<Articulation3DDynamicsContribution>(model, Vec3::zero(), "free-floating"));
        ...
        const Vec3 final_linear_momentum = result.pose_world.ang.rotate(result.velocity_local.lin) * mass;
    ...
    void test_branching_forward_kinematics() {
        ...
        units.push_back({
            .parent_unit = 0,
            .parent_to_unit_zero = Pose3::translation(kHalfLength, kHalfLength, 0.0),
            .motion_twist_at_unit =
                Screw3{Vec3::unit_z(), Vec3::zero()}.adjoint_inv(Pose3::translation(0.0, kHalfLength, 0.0)),
            .inertia = rod_inertia(),
            .diagnostic_name = "branch",
        });
        Articulation3D model(std::move(units), {{0.2, -0.1, 0.3}, {0.4, -0.2, 0.1}}, "branched-articulation");
        Articulation3DDynamicsContribution contribution(model, {0.0, 0.0, -kGravity});
        TERMIN_QOPT_CHECK(contribution.diagnostic() == Articulation3DDiagnostic::None);
        ...
        check_near((poses[1].transform_point({-kHalfLength, 0.0, 0.0}) - common_joint).norm(), 0.0, 1e-12);
        check_near((poses[2].transform_point({0.0, -kHalfLength, 0.0}) - common_joint).norm(), 0.0, 1e-12);

        const AssembledSystem system = assemble(contribution);
        for (std::size_t row = 0; row < 3; ++row) {
            TERMIN_QOPT_CHECK(system.mass[row * 3 + row] > 0.0);
            for (std::size_t column = 0; column < 3; ++column) {
                check_near(system.mass[row * 3 + column], system.mass[column * 3 + row], 1e-12);
            }
        }
    }
    ...
    void test_reduced_matches_maximal_double_pendulum() {
        ...
        Articulation3D reduced_model(double_pendulum_units(), initial_state, "reduced-double-pendulum");
        Multibody3DSystem reduced_system;
        auto reduced = std::make_unique<Articulation3DDynamicsContribution>(
            reduced_model, Vec3{0.0, 0.0, -kGravity}, "reduced-double-pendulum");
        Articulation3DDynamicsContribution* reduced_state = reduced.get();
        TERMIN_QOPT_CHECK(reduced_system.add_contribution(std::move(reduced)) == DynamicsSystemDiagnostic::None);
        TERMIN_QOPT_CHECK(reduced_system.finalize() == DynamicsSystemDiagnostic::None);

        const std::vector<Pose3> initial_poses = reduced_state->unit_poses_world();
        Multibody3DSystem maximal_system;
        auto* upper = add(maximal_system,
                          std::make_unique<RigidBody3DContribution>(rod_inertia(),
        ...
        auto* lower = add(maximal_system,
                          std::make_unique<RigidBody3DContribution>(rod_inertia(),
                                                                    ...
                                                                    "maximal-lower"));
        add(maximal_system,
            std::make_unique<FixedRevoluteJoint3DContribution>(*upper,
        ...
        const double maximal_initial_energy = upper->total_energy() + lower->total_energy();
        ...
        const std::vector<Pose3>& reduced_poses = reduced_state->unit_poses_world();
        ...
        check_near(rotation_error(reduced_poses[0].ang, upper->state().pose.ang), 0.0, 3e-5);
        check_near(rotation_error(reduced_poses[1].ang, lower->state().pose.ang), 0.0, 4e-5);

        const double reduced_drift = std::abs(reduced_state->total_energy() - reduced_initial_energy) /
                                     std::max(1.0, std::abs(reduced_initial_energy));
        const double maximal_drift = std::abs(upper->total_energy() + lower->total_energy() - maximal_initial_energy) /
