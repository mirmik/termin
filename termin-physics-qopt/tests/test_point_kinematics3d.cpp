...
namespace
{
    ...
    Vec3 multiply(ConstDenseMatrixView matrix,
    ...
    void test_static_point()
    {
        ...
        const PointKinematics3DResult result =
            static_point_kinematics(position);
        ...
        const ConstDenseMatrixView jacobian =
            result.value.linear_jacobian_world();
        TERMIN_QOPT_CHECK(jacobian.rows == 3);
        TERMIN_QOPT_CHECK(jacobian.columns == 0);
        TERMIN_QOPT_CHECK(
            result.value.map_force_to_generalized_effort({1.0, 2.0, 3.0}, {}) ==
    ...
    void test_rigid_body_point()
    {
        const RigidBody3DState state{
            Pose3::translation(0.4, -0.3, 1.2) *
                Pose3::rotation(Vec3{1.0, 2.0, -1.0}.normalized(), 0.7),
            {{0.3, -0.5, 0.8}, {1.2, -0.4, 0.6}},
        };
        RigidBody3DContribution body(
            inertia(), state, Vec3::zero(), "point-body");
        bind(body);

        const Vec3 point_local{0.2, -0.7, 0.5};
        const PointKinematics3DResult result =
            body.point_kinematics(point_local);
        TERMIN_QOPT_CHECK(result.ok());
        TERMIN_QOPT_CHECK(result.value.dof_count() == 6);
        TERMIN_QOPT_CHECK(!result.value.is_static());
        TERMIN_QOPT_CHECK(result.value.dofs.valid());
        check_near(result.value.position_world,
                   state.pose.transform_point(point_local));
        ...
        check_near(multiply(result.value.linear_jacobian_world(), velocity),
                   result.value.velocity_world);
        ...
        {
            const Vec3 positive =
                (state.pose * se3_exp(basis[column] * epsilon))
                    .transform_point(point_local);
            const Vec3 negative =
                (state.pose * se3_exp(basis[column] * -epsilon))
                    .transform_point(point_local);
            ...
            check_near(
                derivative,
                {jacobian(0, column), jacobian(1, column), jacobian(2, column)},
                2e-10);
        }
        ...
        TERMIN_QOPT_CHECK(result.value.map_force_to_generalized_effort(
                              force_world, {effort.data(), effort.size(), 1}) ==
                          PointKinematics3DDiagnostic::None);
        check_near(dot(velocity, effort),
                   result.value.velocity_world.dot(force_world));
    }
    ...
    void test_articulation_point()
    {
        ...
        check_near(multiply(model_result.value.linear_jacobian_world(),
                            state.velocities),
        ...
        constexpr double epsilon = 1e-6;
        for (std::size_t column = 0; column < state.coordinates.size();
             ++column)
        {
            ...
            const Vec3 derivative =
                (positive_position - negative_position) / (2.0 * epsilon);
            check_near(
                derivative,
                {jacobian(0, column), jacobian(1, column), jacobian(2, column)},
                2e-9);
        }
        TERMIN_QOPT_CHECK(articulation.set_state(state) ==
                          Articulation3DDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(result.value.map_force_to_generalized_effort(
                              force_world, {effort.data(), effort.size(), 1}) ==
                          PointKinematics3DDiagnostic::None);
        check_near(dot(state.velocities, effort),
                   result.value.velocity_world.dot(force_world));
    }
    ...
    void test_diagnostics()
    {
        ...
        RigidBody3DContribution invalid_body(
            {0.0, {1.0, 1.0, 1.0}, Pose3::identity()});
        TERMIN_QOPT_CHECK(
            invalid_body.point_kinematics(Vec3::zero()).diagnostic ==
        ...
        const PointKinematics3DResult result =
            body.point_kinematics(Vec3::zero());
        TERMIN_QOPT_CHECK(result.ok());
        std::vector<double> effort(6);
        TERMIN_QOPT_CHECK(
            result.value.map_force_to_generalized_effort(
                {nan, 0.0, 0.0}, {effort.data(), effort.size(), 1}) ==
            PointKinematics3DDiagnostic::NonFiniteForce);
        TERMIN_QOPT_CHECK(
            result.value.map_force_to_generalized_effort(
                Vec3::zero(), {effort.data(), effort.size() - 1, 1}) ==
