...
namespace {
    ...
    void check_near(double actual, double expected, double tolerance = kTolerance) {
        if (std::abs(actual - expected) > tolerance) {
            std::fprintf(
                stderr, "actual=%.17g expected=%.17g error=%.17g\n", actual, expected, std::abs(actual - expected));
        }
    ...
    Contribution* add(DynamicsSystem& system, std::unique_ptr<Contribution> contribution) {
    ...
    Contact3D
    ground_contact(std::uint64_t key, RigidBody3DContribution& body, double gap, std::uint64_t group_key = 0) {
        return {
            .key = key,
            .group_key = group_key,
    ...
    void test_body_static_assembly() {
        RigidBody3DContribution body(unit_inertia(),
                                     {Pose3::translation(0.0, 0.25, 0.0), {{0.3, -0.4, 0.5}, {0.6, -0.7, 0.8}}});
        ContactSet3DContribution contacts("assembly-contact");
        TERMIN_QOPT_CHECK(contacts.set_contacts({ground_contact(17, body, 0.25)}) == Contact3DDiagnostic::None);

        DynamicsTopology topology;
        TERMIN_QOPT_CHECK(body.register_topology(topology) == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(contacts.register_topology(topology) == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(topology.finalize() == AssemblyDiagnostic::None);
        DynamicsUnilateralTopology unilateral_topology;
        TERMIN_QOPT_CHECK(contacts.register_unilateral_constraints(unilateral_topology, 0.1) ==
        ...
        TERMIN_QOPT_CHECK(contacts.assemble(assembly, DynamicsAssemblyPhase::PositionProjection) ==
                          AssemblyDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(assembly.clear() == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(contacts.assemble(assembly, DynamicsAssemblyPhase::VelocityProjection) ==
                          AssemblyDiagnostic::None);
    ...
    void test_two_dynamic_endpoints_accumulate() {
        ...
        TERMIN_QOPT_CHECK(contacts.set_contacts({{
                              .key = 3,
                              .endpoint_a = ContactEndpoint3D::rigid_body(body_a, Vec3::zero()),
                              .endpoint_b = ContactEndpoint3D::rigid_body(body_b, Vec3::zero()),
                              .normal_from_a_to_b_world = Vec3::unit_x(),
                              .signed_gap = 0.0,
                          }}) == Contact3DDiagnostic::None);

        DynamicsTopology topology;
        TERMIN_QOPT_CHECK(body_a.register_topology(topology) == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(body_b.register_topology(topology) == AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(topology.finalize() == AssemblyDiagnostic::None);
        DynamicsUnilateralTopology unilateral_topology;
        TERMIN_QOPT_CHECK(contacts.register_unilateral_constraints(unilateral_topology, 0.01) ==
                          AssemblyDiagnostic::None);
        TERMIN_QOPT_CHECK(unilateral_topology.finalize() == AssemblyDiagnostic::None);
    ...
    void test_split_position_correction() {
        ...
        auto* body = add(system,
                         std::make_unique<RigidBody3DContribution>(
                             unit_inertia(), RigidBody3DState{Pose3::translation(0.0, -0.1, 0.0), Screw3::zero()}));
        auto contacts = std::make_unique<ContactSet3DContribution>("ground");
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(1, *body, -0.1)}) == Contact3DDiagnostic::None);
    ...
    void test_impact_and_removal() {
        ...
        auto* body =
            add(system,
                std::make_unique<RigidBody3DContribution>(
                    unit_inertia(), RigidBody3DState{Pose3::translation(0.0, 0.0, 0.0), {{}, {0.0, -2.0, 0.0}}}));
        auto contacts = std::make_unique<ContactSet3DContribution>("impact");
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(5, *body, 0.0)}) == Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
        ...
        if (!impact.ok()) {
            std::fprintf(stderr,
                         ...
                         qp_diagnostic_name(impact.velocity_projection.diagnostic).data(),
        ...
        TERMIN_QOPT_CHECK(contact_set->set_contacts({}) == Contact3DDiagnostic::None);
    ...
    void test_resting_support_and_separation() {
        ...
        auto* body =
            add(system,
                std::make_unique<RigidBody3DContribution>(unit_inertia(), RigidBody3DState{}, Vec3{0.0, -9.81, 0.0}));
        auto contacts = std::make_unique<ContactSet3DContribution>("support");
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(9, *body, 0.0)}) == Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(body->set_gravity_world(Vec3::zero()) == Multibody3DDiagnostic::None);
        RigidBody3DState separating = body->state();
        separating.velocity_local.lin.y = 1.0;
        TERMIN_QOPT_CHECK(body->set_state(separating) == Multibody3DDiagnostic::None);
        TERMIN_QOPT_CHECK(system.step(step_options()).ok());
    ...
    void test_ground_coulomb_friction() {
        ...
        auto* body =
            add(system,
                std::make_unique<RigidBody3DContribution>(
                    unit_inertia(), RigidBody3DState{Pose3::identity(), {{}, {1.0, 0.0, 0.0}}}, Vec3{0.0, -9.81, 0.0}));
        ...
        TERMIN_QOPT_CHECK(contacts->set_contacts({contact}) == Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);

        const DynamicsSystemStepResult sliding_result = system.step(step_options());
        TERMIN_QOPT_CHECK(sliding_result.ok());
        TERMIN_QOPT_CHECK(sliding_result.friction_projection.status == QpStatus::Optimal);
        ...
        check_near(body->state().velocity_local.lin.x, 1.0 - 0.5 * normal_impulse, 2e-8);
        check_near(contact_set->states()[0].tangent_impulse_world.x, -0.5 * normal_impulse, 2e-8);
        ...
        const DynamicsSystemStepResult sticking_result = system.step(fine_options);
        TERMIN_QOPT_CHECK(sticking_result.ok());
        TERMIN_QOPT_CHECK(sticking_result.friction_projection.status == QpStatus::Optimal);
    ...
    ForcedContactResult run_forced_ground_contact(double friction_coefficient) {
        ...
        auto* body =
            add(system,
                std::make_unique<RigidBody3DContribution>(unit_inertia(), RigidBody3DState{}, Vec3{0.0, -9.81, 0.0}));
        add(system, std::make_unique<ForceOnBody3DContribution>(*body, Screw3{Vec3::zero(), {1.0, 0.0, 0.0}}));
        auto contacts = std::make_unique<ContactSet3DContribution>("forced-friction");
        ContactSet3DContribution* contact_set = contacts.get();
        Contact3D contact = ground_contact(41, *body, 0.0, 81);
        contact.friction_coefficient = friction_coefficient;
        TERMIN_QOPT_CHECK(contacts->set_contacts({contact}, {81}) == Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
    ...
    void test_persistent_contact_cache_and_warm_start() {
        ...
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(9, *body, 0.0, 77)}, {77}) ==
                          Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(contact_set->set_contacts({}, {77}) == Contact3DDiagnostic::None);
        ...
        TERMIN_QOPT_CHECK(second.velocity_projection.iterations <= first.velocity_projection.iterations);
        check_near(contact_set->states()[0].normal_reaction, 9.81, 2e-8);

        // A fresh feature in the same live group replaces, rather than
        // accumulating with, the old feature.
        TERMIN_QOPT_CHECK(contact_set->set_contacts({ground_contact(10, *body, 0.0, 77)}, {77}) ==
                          Contact3DDiagnostic::None);
        TERMIN_QOPT_CHECK(contact_set->contacts().size() == 1);
    ...
    void test_contact_cache_order_capacity_and_rollback() {
        ...
        TERMIN_QOPT_CHECK(ordered.set_contacts({ground_contact(30, body, 0.0),
                                                ...
                                                ground_contact(20, body, 0.0)}) == Contact3DDiagnostic::None);
        ...
        ContactSet3DContribution* contact_set = contacts.get();
        TERMIN_QOPT_CHECK(contacts->set_contacts({ground_contact(1, *dynamic_body, 0.0, 99)}, {99}) ==
                          Contact3DDiagnostic::None);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
        TERMIN_QOPT_CHECK(system.step(step_options()).ok());
        ...
        TERMIN_QOPT_CHECK(contact_set->set_contacts({invalid}, {99}) == Contact3DDiagnostic::InvalidNormal);
        TERMIN_QOPT_CHECK(!system.step(step_options()).ok());
        TERMIN_QOPT_CHECK(contact_set->cached_contact_count() == 0);
    }
    ...
    void test_articulation_endpoint() {
        ...
        TERMIN_QOPT_CHECK(topology.finalize() == AssemblyDiagnostic::None);
        const ContactEndpoint3D endpoint = ContactEndpoint3D::articulation_unit(articulation, 0, Vec3::zero());
        ...
        TERMIN_QOPT_CHECK(contacts.set_contacts({
                              {
                                  .key = 20,
                                  .endpoint_a = ContactEndpoint3D::static_world(Vec3::zero()),
                                  .endpoint_b = ContactEndpoint3D::articulation_unit(articulation, 0, Vec3::zero()),
                                  .normal_from_a_to_b_world = Vec3::unit_y(),
                                  .signed_gap = 0.0,
                              },
                              {
                                  .key = 21,
                                  .endpoint_a = ContactEndpoint3D::rigid_body(body, Vec3::zero()),
                                  .endpoint_b = ContactEndpoint3D::articulation_unit(articulation, 0, Vec3::zero()),
                                  .normal_from_a_to_b_world = Vec3::unit_y(),
                                  .signed_gap = 0.0,
                              },
    ...
    void test_validation() {
        ...
        TERMIN_QOPT_CHECK(contacts.set_contacts({contact}) == Contact3DDiagnostic::InvalidNormal);

        contact = ground_contact(1, body, 0.0);
        contact.signed_gap = std::numeric_limits<double>::quiet_NaN();
        TERMIN_QOPT_CHECK(contacts.set_contacts({contact}) == Contact3DDiagnostic::NonFiniteGap);

        contact = ground_contact(1, body, 0.0);
        TERMIN_QOPT_CHECK(contacts.set_contacts({contact, contact}) == Contact3DDiagnostic::DuplicateKey);

        contact.endpoint_b = ContactEndpoint3D::static_world(Vec3::unit_y());
        TERMIN_QOPT_CHECK(contacts.set_contacts({contact}) == Contact3DDiagnostic::InvalidEndpointPair);

        contact = ground_contact(1, body, 0.0);
        contact.friction_coefficient = -0.1;
        TERMIN_QOPT_CHECK(contacts.set_contacts({contact}) == Contact3DDiagnostic::InvalidFrictionCoefficient);
    }

    void test_invalid_contact_step_is_transactional() {
        DynamicsSystem system;
        auto* body = add(system, std::make_unique<RigidBody3DContribution>(unit_inertia()));
        auto contacts = std::make_unique<ContactSet3DContribution>("invalid-step");
        Contact3D invalid = ground_contact(1, *body, 0.0);
        invalid.normal_from_a_to_b_world = Vec3{0.0, 3.0, 0.0};
        TERMIN_QOPT_CHECK(contacts->set_contacts({invalid}) == Contact3DDiagnostic::InvalidNormal);
        add(system, std::move(contacts));
        TERMIN_QOPT_CHECK(system.finalize() == DynamicsSystemDiagnostic::None);
        ...
        check_near(body->state().velocity_local.lin.y, before.velocity_local.lin.y);
    }
