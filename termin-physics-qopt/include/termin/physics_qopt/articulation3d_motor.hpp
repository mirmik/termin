...
namespace termin::physics_qopt
{
    ...
    articulation_motor_diagnostic_name(
        ArticulationMotorDiagnostic diagnostic) noexcept;
    ...
    class TERMIN_PHYSICS_QOPT_API ArticulationMotorContribution final
        : public DynamicsContribution
    {
    public:
        ArticulationMotorContribution(
        ...
        set_command(std::size_t channel_index, double effort) noexcept;
        [[nodiscard]] ArticulationMotorDiagnostic
        set_effort_limit(std::size_t channel_index,
                         double effort_limit) noexcept;
        [[nodiscard]] double command(std::size_t channel_index) const noexcept;
        [[nodiscard]] double
        applied_effort(std::size_t channel_index) const noexcept;
    ...
    private:
        ...
        ArticulationMotorDiagnostic diagnostic_ =
