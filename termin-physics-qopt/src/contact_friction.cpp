...
namespace termin::physics_qopt
{
    namespace
    {
        ...
        {
            return view.empty() ||
                   (view.data != nullptr && view.row_stride > 0 &&
                    view.column_stride > 0);
        }
                           ...
                           ContactFrictionOptions options) noexcept
    {
        ...
        const std::size_t normal_constraints =
            problem.minimum_normal_velocity.size;
        ...
        if (problem.mass.rows != dofs || problem.mass.columns != dofs ||
            problem.bilateral_jacobian.columns != dofs ||
            problem.normal_jacobian.rows != normal_constraints ||
            ...
            solution.velocity.size != dofs ||
            solution.tangent_impulse.size != tangents ||
            (!solution.normal_impulse.empty() &&
             solution.normal_impulse.size != contacts) ||
            (!solution.friction_work.empty() &&
             solution.friction_work.size != contacts) ||
            (!solution.bilateral_impulse.empty() &&
             solution.bilateral_impulse.size != bilateral))
        {
            return invalid_result(QpDiagnostic::DimensionMismatch);
        }
        if (!valid(problem.mass) || !valid(problem.bilateral_jacobian) ||
            !valid(problem.normal_projected_velocity) ||
            ...
            !valid(solution.tangent_impulse) ||
            !valid(solution.normal_impulse) || !valid(solution.friction_work) ||
        ...
        if (!finite(problem.mass) || !finite(problem.bilateral_jacobian) ||
            !finite(problem.normal_projected_velocity) ||
            !finite(problem.normal_jacobian) ||
            !finite(problem.minimum_normal_velocity) ||
            !finite(problem.contact_normal_jacobian) ||
            !finite(problem.tangent_jacobian) ||
            !finite(problem.normal_impulse) ||
        ...
        for (std::size_t row = 0; row < normal_constraints; ++row)
        {
            ...
            if (normal_velocity + options.qp.active_tolerance <
        ...
        try
        {
            ...
            if (contacts == 0)
            {
                ...
                for (std::size_t row = 0; row < solution.bilateral_impulse.size;
                     ++row)
                {
            ...
            if (!any_capacity)
            {
                ...
                for (std::size_t contact = 0;
                     contact < solution.normal_impulse.size;
                     ++contact)
                {
                    solution.normal_impulse[contact] =
                        base_normal_impulse[contact];
                }
                for (std::size_t contact = 0;
                     contact < solution.friction_work.size;
                     ++contact)
                {
            ...
            for (std::size_t variable = 0; variable < variables; ++variable)
            {
                ...
                for (std::size_t dof = 0; dof < dofs; ++dof)
                {
                    load[dof] =
                        axis == 0
                            ? problem.contact_normal_jacobian(contact, dof)
                            : problem.tangent_jacobian(contact * 2 + axis - 1,
                                                       dof);
                }
                const QpSolveResult response_result =
                    solve_constrained_dynamics(
                        ...
                        },
                        {
                            ...
                            {equality_reaction.data(),
                             equality_reaction.size(),
                             1},
                        },
                ...
                {
                    bilateral_response[row * variables + variable] =
                        equality_reaction[row];
                }
            }
                    ...
                    const double jacobian =
                        axis == 0
                            ...
                            : problem.tangent_jacobian(contact * 2 + axis - 1,
                                                       dof);
                    gradient[row] +=
                        jacobian * problem.normal_projected_velocity[dof];
                }
                for (std::size_t column = 0; column < variables; ++column)
                {
                    for (std::size_t dof = 0; dof < dofs; ++dof)
                    {
                        const double jacobian =
                            axis == 0
                                ? problem.contact_normal_jacobian(contact, dof)
                                : problem.tangent_jacobian(
                                      contact * 2 + axis - 1, dof);
                        hessian[row * variables + column] +=
                            jacobian * response[dof * variables + column];
                    }
                }
            }
            for (std::size_t row = 0; row < variables; ++row)
            {
                for (std::size_t column = row + 1; column < variables; ++column)
                {
                    const double symmetric =
                        0.5 * (hessian[row * variables + column] +
                               hessian[column * variables + row]);
                     ...
                     ++facet)
                {
                    ...
                    inequalities[row * variables + contact * 3] =
                        ...
                        std::cos(std::numbers::pi /
                                 static_cast<double>(options.cone_facets));
                    inequalities[row * variables + contact * 3 + 1] =
                        std::cos(angle);
                    inequalities[row * variables + contact * 3 + 2] =
                        std::sin(angle);
            ...
            for (std::size_t dof = 0; dof < dofs; ++dof)
            {
                ...
                for (std::size_t variable = 0; variable < variables; ++variable)
                {
                    output_velocity[dof] +=
                        response[dof * variables + variable] *
                        variable_impulse[variable];
                }
            }
            for (std::size_t contact = 0; contact < contacts; ++contact)
            {
                output_normal_impulse[contact] = base_normal_impulse[contact] +
                                                 variable_impulse[contact * 3];
                output_impulse[contact * 2] = variable_impulse[contact * 3 + 1];
                output_impulse[contact * 2 + 1] =
                    variable_impulse[contact * 3 + 2];
            }
            ...
            for (std::size_t contact = 0; contact < contacts; ++contact)
            {
                for (std::size_t axis = 0; axis < 2; ++axis)
                {
                    ...
                    for (std::size_t dof = 0; dof < dofs; ++dof)
                    {
                        final_tangent_velocity +=
                            problem.tangent_jacobian(tangent, dof) *
                            output_velocity[dof];
                    }
                    contact_work[contact] += 0.5 *
                                             (gradient[contact * 3 + axis + 1] +
                                              final_tangent_velocity) *
                                             output_impulse[tangent];
                }
            }
            for (std::size_t dof = 0; dof < dofs; ++dof)
            {
        ...
        {
            std::fprintf(stderr,
                         "[termin-qopt] contact friction solve failed: %s\n",
                         error.what());
        }
        catch (...)
        {
