...
{
    std::string_view articulation_motor_diagnostic_name(
        ArticulationMotorDiagnostic value) noexcept
    {
        switch (value)
        {
    ...
    ArticulationMotorContribution::validate() const noexcept
    {
        ...
        for (const ArticulationMotorChannel& channel : channels_)
        {
            if (channel.dof_index >= articulation_->dof_count() ||
                !std::isfinite(channel.effort_limit) ||
                                               ...
                                               double effort) noexcept
    {
        ...
        {
            std::fprintf(stderr,
                         "[termin-qopt] rejected non-finite motor command\n");
    ...
    double ArticulationMotorContribution::command(
    ...
    ArticulationMotorDiagnostic ArticulationMotorContribution::set_effort_limit(
        std::size_t channel_index, double effort_limit) noexcept
    {
        if (channel_index >= channels_.size())
        {
            std::fprintf(stderr,
                         "[termin-qopt] motor limit channel is out of range\n");
            return ArticulationMotorDiagnostic::InvalidChannel;
        }
        if (!std::isfinite(effort_limit) || effort_limit < 0.0)
        {
            std::fprintf(stderr,
                         "[termin-qopt] rejected invalid motor effort limit\n");
            return ArticulationMotorDiagnostic::NonFiniteInput;
        }
    ...
    bool ArticulationMotorContribution::saturated(
        std::size_t channel_index) const noexcept
    {
        return channel_index < commands_.size() &&
    ...
    AssemblyDiagnostic ArticulationMotorContribution::assemble(
        DynamicsAssembly& assembly, DynamicsAssemblyPhase phase) noexcept
    {
        ...
        return assembly.add_load(articulation_->dofs(),
