...
namespace termin::physics_qopt {
    std::string_view articulation_motor_diagnostic_name(ArticulationMotorDiagnostic value) noexcept {
        switch (value) {
    ...
    ArticulationMotorContribution::ArticulationMotorContribution(Articulation3DDynamicsContribution& articulation,
    ...
    ArticulationMotorDiagnostic ArticulationMotorContribution::validate() const noexcept {
        ...
        for (const ArticulationMotorChannel& channel : channels_) {
            if (channel.dof_index >= articulation_->dof_count() || !std::isfinite(channel.effort_limit) ||
                channel.effort_limit < 0.0) {
    ...
    ArticulationMotorDiagnostic ArticulationMotorContribution::diagnostic() const noexcept {
        return diagnostic_;
    }
    ...
    const std::vector<ArticulationMotorChannel>& ArticulationMotorContribution::channels() const noexcept {
        return channels_;
    }

    ArticulationMotorDiagnostic ArticulationMotorContribution::set_command(std::size_t channel_index,
                                                                           double effort) noexcept {
        if (channel_index >= commands_.size()) {
            std::fprintf(stderr, "[termin-qopt] motor command channel is out of range\n");
            return ArticulationMotorDiagnostic::InvalidChannel;
        }
    ...
    double ArticulationMotorContribution::command(std::size_t channel_index) const noexcept {
        return channel_index < commands_.size() ? commands_[channel_index] : std::numeric_limits<double>::quiet_NaN();
    }

    ArticulationMotorDiagnostic ArticulationMotorContribution::set_effort_limit(std::size_t channel_index,
    ...
    double ArticulationMotorContribution::applied_effort(std::size_t channel_index) const noexcept {
        return channel_index < applied_efforts_.size() ? applied_efforts_[channel_index]
                                                       : std::numeric_limits<double>::quiet_NaN();
    }

    bool ArticulationMotorContribution::saturated(std::size_t channel_index) const noexcept {
        return channel_index < commands_.size() &&
               std::abs(commands_[channel_index]) > channels_[channel_index].effort_limit;
    }

    AssemblyDiagnostic ArticulationMotorContribution::register_topology(DynamicsTopology& topology) noexcept {
        (void)topology;
    ...
    AssemblyDiagnostic ArticulationMotorContribution::bind_topology(const DynamicsTopology& topology) noexcept {
        if (!topology.finalized() || articulation_ == nullptr || !articulation_->dofs().valid()) {
            diagnostic_ = ArticulationMotorDiagnostic::TopologyMismatch;
            return AssemblyDiagnostic::InvalidBlock;
        }
        const DenseBlockInfo info = topology.dof_topology().block_info(articulation_->dofs().block);
    ...
    AssemblyDiagnostic ArticulationMotorContribution::assemble(DynamicsAssembly& assembly,
