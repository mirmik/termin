...
namespace termin::physics_qopt
{
    namespace
    {
        ...
        point_diagnostic(robotics::Articulation3DDiagnostic diagnostic) noexcept
        {
            switch (diagnostic)
            {
    ...
    Articulation3DDynamicsContribution::Articulation3DDynamicsContribution(
        ...
        : articulation_(articulation), gravity_world_(gravity_world),
          diagnostic_name_(diagnostic_name.empty()
                               ? std::string(articulation.diagnostic_name())
                               : std::string(diagnostic_name))
    {
        diagnostic_ = articulation_.diagnostic();
        if (diagnostic_ == Articulation3DDiagnostic::None &&
            !gravity_world_.is_finite())
        {
    ...
    const Articulation3D&
    Articulation3DDynamicsContribution::articulation() const noexcept
    {
        return articulation_;
    }

    Articulation3DDiagnostic
    Articulation3DDynamicsContribution::diagnostic() const noexcept
    {
        return diagnostic_;
    }
    ...
    Articulation3DDynamicsContribution::state() const noexcept
    {
    ...
    Articulation3DDynamicsContribution::accelerations() const noexcept
    {
    ...
    termin::Vec3
    Articulation3DDynamicsContribution::gravity_world() const noexcept
    {
        ...
        termin::Vec3 point_local) const noexcept
    {
        ArticulationPointKinematics3DResult source =
            articulation_.floating_base_point_kinematics(point_local);
        if (!source.ok())
        {
        ...
        value.dofs = dofs_;
        value.linear_jacobian_world_storage =
            std::move(source.value.linear_jacobian_world_storage);
    ...
    Articulation3DDiagnostic
    Articulation3DDynamicsContribution::set_gravity_world(
    ...
    AssemblyDiagnostic Articulation3DDynamicsContribution::register_topology(
        DynamicsTopology& topology) noexcept
    {
        ...
        const auto result =
            topology.register_dofs(dof_count(), diagnostic_name_);
        dofs_ = result.handle;
        return result.diagnostic;
    }

    AssemblyDiagnostic
    Articulation3DDynamicsContribution::register_unilateral_constraints(
        ...
        try
        {
                 ...
                 ++index)
            {
                ...
                const double predicted_coordinate =
                    coordinate +
                    time_step * ArticulationAccess::state(articulation_)
                                    .velocities[index];
    ...
    AssemblyDiagnostic Articulation3DDynamicsContribution::assemble(
        DynamicsAssembly& assembly, DynamicsAssemblyPhase phase) noexcept
    {
        try
        {
            ...
            if (!std::isfinite(unilateral_time_step_) ||
                unilateral_time_step_ <= 0.0)
            {
                return AssemblyDiagnostic::NonFiniteContribution;
            }
                ...
                if (unit_limit_rows_[index].minimum.valid())
                {
                    const double coordinate =
                        snapshot_ready_
                            ...
                            : ArticulationAccess::state(articulation_)
                                  .coordinates[index];
                    const double margin =
                        std::max(0.0, coordinate - *limits.minimum);
                    ...
                    const double coordinate =
                        snapshot_ready_
                            ...
                            : ArticulationAccess::state(articulation_)
                                  .coordinates[index];
                    const double margin =
                        std::max(0.0, *limits.maximum - coordinate);
                    const std::array<double, 1> limit{
                        margin / unilateral_time_step_,
                    };
                    AssemblyDiagnostic result =
    ...
    void Articulation3DDynamicsContribution::apply_unilateral_solution(
                                  ...
                                  bool& active)
            {
                ...
                const DenseBlockInfo info =
                    unilateral_topology.constraint_topology().block_info(
                        handle.block);
                if (!info.ok() || info.size != 1 ||
                    info.offset >= reactions.size ||
    ...
    void Articulation3DDynamicsContribution::apply_solution(
    ...
    AssemblyDiagnostic Articulation3DDynamicsContribution::write_velocity(
        const DynamicsTopology& topology,
    ...
    AssemblyDiagnostic Articulation3DDynamicsContribution::set_velocity(
        const DynamicsTopology& topology, ConstDenseVectorView source) noexcept
    {
        const DenseBlockInfo info =
            topology.dof_topology().block_info(dofs_.block);
        const std::size_t unit_offset =
    ...
    AssemblyDiagnostic
    Articulation3DDynamicsContribution::set_trial_configuration(
        const DynamicsTopology& topology,
        ...
        double time_step) noexcept
    {
        ...
        if (ArticulationAccess::floating_base(articulation_).has_value())
        {
            ...
            ArticulationAccess::floating_base(articulation_)->pose_world =
                (floating_base_snapshot_->pose_world *
        ...
        return finite(ArticulationAccess::state(articulation_).coordinates) &&
                       finite(ArticulationAccess::state(articulation_)
                                  .velocities) &&
                       ArticulationAccess::update_kinematics(articulation_)
                   ? AssemblyDiagnostic::None
                   : AssemblyDiagnostic::NonFiniteContribution;
    ...
    AssemblyDiagnostic
    Articulation3DDynamicsContribution::write_corrected_midpoint_velocity(
        const DynamicsTopology& topology,
        ConstDenseVectorView midpoint_velocity,
