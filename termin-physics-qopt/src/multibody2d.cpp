...
namespace termin::physics_qopt {
    namespace {
        ...
        [[nodiscard]] bool finite(RigidBody2DState state) noexcept {
            return finite(state.pose.ang) && finite(state.pose.lin) && finite(state.linear_velocity_world) &&
        ...
        [[nodiscard]] double cross(termin::Vec2 first, termin::Vec2 second) noexcept {
        ...
        [[nodiscard]] ConstDenseMatrixView matrix_view(const std::array<double, 9>& values) noexcept {
            return ConstDenseMatrixView::row_major(values.data(), 3, 3);
        }

        [[nodiscard]] ConstDenseMatrixView matrix_view(const std::array<double, 6>& values) noexcept {
            return ConstDenseMatrixView::row_major(values.data(), 2, 3);
        }

        [[nodiscard]] ConstDenseVectorView vector_view(const std::array<double, 3>& values) noexcept {
            return {values.data(), values.size(), 1};
        }

        [[nodiscard]] ConstDenseVectorView vector_view(const std::array<double, 2>& values) noexcept {
            return {values.data(), values.size(), 1};
        }

        [[nodiscard]] Multibody2DStepResult
        failure(QpStatus status, Multibody2DDiagnostic diagnostic, QpSolveResult dynamics = {}) noexcept {
    ...
    struct Multibody2DSystem::Impl {
        ...
        std::uint64_t id = next_multibody_system_id.fetch_add(1, std::memory_order_relaxed);
        ...
        [[nodiscard]] std::array<double, 9> body_mass(const Body& body) const noexcept {
            const termin::Vec2 center = body.state.pose.rotate_vector(body.inertia.center_of_mass_local);
        ...
        [[nodiscard]] std::array<double, 3> body_load(const Body& body, termin::Vec2 gravity) const noexcept {
            const termin::Vec2 center = body.state.pose.rotate_vector(body.inertia.center_of_mass_local);
            const termin::Vec2 gravity_force = gravity * body.inertia.mass;
            const termin::Vec2 centrifugal =
                center * (body.inertia.mass * body.state.angular_velocity * body.state.angular_velocity);
            const termin::Vec2 force = body.wrench.force_world + gravity_force + centrifugal;
            const double torque = body.wrench.torque_about_origin + cross(center, gravity_force);
            return {force.x, force.y, torque};
        }

        [[nodiscard]] std::array<double, 6> point_jacobian(const Body& body, termin::Vec2 anchor_local) const noexcept {
            const termin::Vec2 radius = body.state.pose.rotate_vector(anchor_local);
            return {
                1.0,
                0.0,
                -radius.y,
        ...
        [[nodiscard]] AssemblyDiagnostic assemble(DynamicsAssembly& assembly, termin::Vec2 gravity) noexcept {
            ...
            for (const Body& body : bodies) {
                ...
                diagnostic = assembly.add_mass(body.dofs, body.dofs, matrix_view(mass_block));
                if (diagnostic != AssemblyDiagnostic::None) {
                    return diagnostic;
                }
                const std::array<double, 3> load_block = body_load(body, gravity);
            ...
            for (const Joint& joint : joints) {
                if (const auto* fixed = std::get_if<FixedPointJoint>(&joint.model)) {
                    ...
                    const std::array<double, 6> jacobian_block = point_jacobian(body, fixed->body_anchor_local);
                    diagnostic =
                        assembly.add_constraint_jacobian(joint.constraint, body.dofs, matrix_view(jacobian_block));
                    if (diagnostic != AssemblyDiagnostic::None) {
                        return diagnostic;
                    }
                    const termin::Vec2 radius = body.state.pose.rotate_vector(fixed->body_anchor_local);
                    const double omega = body.state.angular_velocity;
                    const std::array<double, 2> rhs{
                        omega * omega * radius.x,
                        omega * omega * radius.y,
                    };
                    diagnostic = assembly.add_constraint_rhs(joint.constraint, vector_view(rhs));
                } else {
                    ...
                    const std::array<double, 6> jacobian_a = point_jacobian(body_a, revolute.body_a_anchor_local);
                    std::array<double, 6> jacobian_b = point_jacobian(body_b, revolute.body_b_anchor_local);
                    for (double& value : jacobian_b) {
                        value = -value;
                    }
                    diagnostic =
                        assembly.add_constraint_jacobian(joint.constraint, body_a.dofs, matrix_view(jacobian_a));
                    if (diagnostic != AssemblyDiagnostic::None) {
                        return diagnostic;
                    }
                    diagnostic =
                        assembly.add_constraint_jacobian(joint.constraint, body_b.dofs, matrix_view(jacobian_b));
                    if (diagnostic != AssemblyDiagnostic::None) {
                        return diagnostic;
                    }
                    const termin::Vec2 radius_a = body_a.state.pose.rotate_vector(revolute.body_a_anchor_local);
                    const termin::Vec2 radius_b = body_b.state.pose.rotate_vector(revolute.body_b_anchor_local);
                    const double omega_a = body_a.state.angular_velocity;
                    const double omega_b = body_b.state.angular_velocity;
                    const termin::Vec2 rhs_value = radius_a * (omega_a * omega_a) - radius_b * (omega_b * omega_b);
        ...
        [[nodiscard]] termin::Vec2 position_error(const Joint& joint) const noexcept {
            if (const auto* fixed = std::get_if<FixedPointJoint>(&joint.model)) {
                const Body& body = bodies[fixed->body];
                return body.state.pose.transform_point(fixed->body_anchor_local) - fixed->world_anchor;
            }
            const auto& revolute = std::get<RevoluteJoint>(joint.model);
            return bodies[revolute.body_a].state.pose.transform_point(revolute.body_a_anchor_local) -
                   bodies[revolute.body_b].state.pose.transform_point(revolute.body_b_anchor_local);
        }

        [[nodiscard]] termin::Vec2 velocity_error(const Joint& joint) const noexcept {
            const auto point_velocity = [](const Body& body, termin::Vec2 local) {
                const termin::Vec2 radius = body.state.pose.rotate_vector(local);
                return body.state.linear_velocity_world + perpendicular(radius) * body.state.angular_velocity;
            };
            if (const auto* fixed = std::get_if<FixedPointJoint>(&joint.model)) {
                return point_velocity(bodies[fixed->body], fixed->body_anchor_local);
            }
            const auto& revolute = std::get<RevoluteJoint>(joint.model);
            return point_velocity(bodies[revolute.body_a], revolute.body_a_anchor_local) -
                   point_velocity(bodies[revolute.body_b], revolute.body_b_anchor_local);
        }
        ...
        void apply_generalized_delta(ConstDenseVectorView delta, bool velocity) noexcept {
            for (std::size_t body_index = 0; body_index < bodies.size(); ++body_index) {
                Body& body = bodies[body_index];
                const DenseBlockInfo info = topology.dof_topology().block_info(body.dofs.block);
                if (velocity) {
        ...
        [[nodiscard]] QpSolveResult project_positions(DynamicsAssembly& assembly, QpTolerance tolerance) noexcept {
            std::vector<double> target(constraint_rhs.size());
            for (const Joint& joint : joints) {
                const DenseBlockInfo info = topology.constraint_topology().block_info(joint.constraint.block);
        ...
        [[nodiscard]] QpSolveResult project_velocities(DynamicsAssembly& assembly, QpTolerance tolerance) noexcept {
            ...
            if (constrained.status == QpStatus::Optimal) {
                apply_generalized_delta({projected.data(), projected.size(), 1}, true);
            }
            return constrained;
        }
    };

    std::string_view multibody2d_diagnostic_name(Multibody2DDiagnostic diagnostic) noexcept {
        switch (diagnostic) {
    ...
    Multibody2DSystem::Multibody2DSystem()
        : impl_(std::make_unique<Impl>()) {}

    Multibody2DSystem::~Multibody2DSystem() = default;
    Multibody2DSystem::Multibody2DSystem(Multibody2DSystem&&) noexcept = default;
    Multibody2DSystem& Multibody2DSystem::operator=(Multibody2DSystem&&) noexcept = default;

    Multibody2DRegistrationResult<RigidBody2DHandle> Multibody2DSystem::add_body(
        SpatialInertia2D inertia, RigidBody2DState initial_state, std::string_view diagnostic_name) noexcept {
        if (impl_ == nullptr) {
        ...
        if (!finite(inertia.moment_at_center) || inertia.moment_at_center <= 0.0) {
        ...
        const auto registration = impl_->topology.register_dofs(3, diagnostic_name);
        if (!registration.ok()) {
        ...
        try {
            ...
            std::fprintf(stderr, "[termin-qopt] add 2D body failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr, "[termin-qopt] add 2D body failed with an unknown exception\n");
        }
        return {{}, Multibody2DDiagnostic::InternalFailure};
    }

    Multibody2DRegistrationResult<Joint2DHandle>
    Multibody2DSystem::add_fixed_point_joint(RigidBody2DHandle body,
                                             termin::Vec2 body_anchor_local,
                                             termin::Vec2 world_anchor,
        ...
        const auto registration = impl_->topology.register_constraint(2, diagnostic_name);
        if (!registration.ok()) {
        ...
        try {
            ...
            std::fprintf(stderr, "[termin-qopt] add fixed 2D joint failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    Multibody2DRegistrationResult<Joint2DHandle>
    Multibody2DSystem::add_revolute_joint(RigidBody2DHandle body_a,
                                          termin::Vec2 body_a_anchor_local,
        ...
        if (!impl_->valid(body_a) || !impl_->valid(body_b) || body_a.index == body_b.index) {
            return {{}, Multibody2DDiagnostic::InvalidBody};
        }
        ...
        try {
            ...
            std::fprintf(stderr, "[termin-qopt] add revolute 2D joint failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    Multibody2DDiagnostic Multibody2DSystem::finalize() noexcept {
        ...
        try {
            ...
            std::fprintf(stderr, "[termin-qopt] finalize 2D model failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    Multibody2DDiagnostic Multibody2DSystem::set_body_state(RigidBody2DHandle body, RigidBody2DState state) noexcept {
    ...
    Multibody2DDiagnostic Multibody2DSystem::set_body_wrench(RigidBody2DHandle body,
    ...
    RigidBody2DState Multibody2DSystem::body_state(RigidBody2DHandle body) const noexcept {
        if (impl_ == nullptr || !impl_->valid(body)) {
            return {};
        }
    ...
    RigidBody2DAcceleration Multibody2DSystem::body_acceleration(RigidBody2DHandle body) const noexcept {
        if (impl_ == nullptr || !impl_->valid(body)) {
            return {};
        }
    ...
    termin::Vec2 Multibody2DSystem::joint_reaction(Joint2DHandle joint) const noexcept {
    ...
    Multibody2DStepResult Multibody2DSystem::step(termin::Vec2 gravity_world, Multibody2DStepOptions options) noexcept {
        if (impl_ == nullptr || !impl_->finalized) {
            return failure(QpStatus::InvalidInput, Multibody2DDiagnostic::ModelNotFinalized);
        }
        if (!finite(gravity_world)) {
            return failure(QpStatus::InvalidInput, Multibody2DDiagnostic::NonFiniteInput);
        }
        if (!finite(options.time_step) || options.time_step <= 0.0) {
            return failure(QpStatus::InvalidInput, Multibody2DDiagnostic::InvalidTimeStep);
        }
        if (!finite(options.position_tolerance) || !finite(options.velocity_tolerance) ||
            options.position_tolerance < 0.0 || options.velocity_tolerance < 0.0 ||
            (!impl_->joints.empty() && options.max_position_iterations == 0)) {
            return failure(QpStatus::InvalidInput, Multibody2DDiagnostic::InvalidProjectionOptions);
        }
        ...
        try {
            ...
            if (!assembly.valid() || impl_->assemble(assembly, gravity_world) != AssemblyDiagnostic::None) {
                return failure(QpStatus::InvalidInput, Multibody2DDiagnostic::AssemblyFailure);
            }

            const QpSolveResult dynamics =
            ...
            if (dynamics.status != QpStatus::Optimal) {
                return failure(dynamics.status, Multibody2DDiagnostic::DynamicsFailure, dynamics);
            }

            for (Impl::Body& body : impl_->bodies) {
                const DenseBlockInfo info = impl_->topology.dof_topology().block_info(body.dofs.block);
                body.acceleration.linear_world = {
                    impl_->acceleration[info.offset],
                    impl_->acceleration[info.offset + 1],
                };
                body.acceleration.angular = impl_->acceleration[info.offset + 2];
                body.state.linear_velocity_world += body.acceleration.linear_world * options.time_step;
                body.state.angular_velocity += body.acceleration.angular * options.time_step;
                body.state.pose.lin += body.state.linear_velocity_world * options.time_step;
                body.state.pose.ang += body.state.angular_velocity * options.time_step;
                body.state.pose.normalize_angle();
            }
            for (Impl::Joint& joint : impl_->joints) {
                const DenseBlockInfo info = impl_->topology.constraint_topology().block_info(joint.constraint.block);
                joint.reaction = {
            ...
            for (std::size_t iteration = 0; iteration < options.max_position_iterations; ++iteration) {
                ...
                if (impl_->assemble(assembly, gravity_world) != AssemblyDiagnostic::None) {
                    restore();
                    return failure(QpStatus::InvalidInput, Multibody2DDiagnostic::AssemblyFailure, dynamics);
                }
                const QpSolveResult projection = impl_->project_positions(assembly, options.qp_tolerance);
                ++result.position_iterations;
                if (projection.status != QpStatus::Optimal) {
                    restore();
                    return failure(projection.status, Multibody2DDiagnostic::PositionProjectionFailure, dynamics);
                }
            }
            result.position_constraint_linf = impl_->max_position_error();
            if (result.position_constraint_linf > options.position_tolerance) {
                restore();
                return failure(QpStatus::NumericalFailure, Multibody2DDiagnostic::PositionProjectionFailure, dynamics);
            }
            ...
            const QpSolveResult velocity_projection = impl_->project_velocities(assembly, options.qp_tolerance);
            if (velocity_projection.status != QpStatus::Optimal) {
                restore();
                return failure(velocity_projection.status, Multibody2DDiagnostic::VelocityProjectionFailure, dynamics);
            }
            result.velocity_constraint_linf = impl_->max_velocity_error();
            if (result.velocity_constraint_linf > options.velocity_tolerance) {
                restore();
                return failure(QpStatus::NumericalFailure, Multibody2DDiagnostic::VelocityProjectionFailure, dynamics);
            }
            snapshot_ready = false;
            return result;
        } catch (const std::exception& error) {
            restore();
            std::fprintf(stderr, "[termin-qopt] 2D multibody step failed: %s\n", error.what());
        } catch (...) {
            restore();
            std::fprintf(stderr,
        ...
        return failure(QpStatus::NumericalFailure, Multibody2DDiagnostic::InternalFailure);
    }

    double Multibody2DSystem::max_position_constraint_error() const noexcept {
        return impl_ == nullptr ? std::numeric_limits<double>::infinity() : impl_->max_position_error();
    }

    double Multibody2DSystem::max_velocity_constraint_error() const noexcept {
        return impl_ == nullptr ? std::numeric_limits<double>::infinity() : impl_->max_velocity_error();
    }

    double Multibody2DSystem::total_energy(termin::Vec2 gravity_world) const noexcept {
        ...
        for (const Impl::Body& body : impl_->bodies) {
            ...
            const termin::Vec2 center_velocity =
                body.state.linear_velocity_world + perpendicular(center) * body.state.angular_velocity;
            const termin::Vec2 center_world = body.state.pose.lin + center;
            energy += 0.5 * body.inertia.mass * center_velocity.dot(center_velocity);
            energy += 0.5 * body.inertia.moment_at_center * body.state.angular_velocity * body.state.angular_velocity;
