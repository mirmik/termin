...
namespace termin::physics_qopt
{
    namespace
    {
        ...
        PointJacobian
        point_jacobian_local(termin::Vec3 radius_local,
                             termin::Quat body_orientation) noexcept
        {
            const PointJacobian local = point_jacobian(radius_local);
            const termin::Mat33 rotation =
                termin::Mat33::rotation(body_orientation);
            ...
            for (std::size_t row = 0; row < 3; ++row)
            {
                for (std::size_t column = 0; column < 6; ++column)
                {
                    ...
                    {
                        result[row * 6 + column] +=
                            rotation(static_cast<int>(inner),
                                     static_cast<int>(row)) *
        ...
        AssemblyDiagnostic first(AssemblyDiagnostic a,
                                 AssemblyDiagnostic b) noexcept
        {
        ...
        HingeFrame hinge_frame(termin::Vec3 axis_a,
                               termin::Vec3 axis_b) noexcept
        {
        ...
        {
            return first_body ? basis.cross(a).cross(b)
                              : a.cross(basis.cross(b));
        }
        ...
        void set_orientation_rows(RevoluteJacobian& block,
                                  ...
                                  bool first_body) noexcept
        {
            const std::array<termin::Vec3, 3> basis{termin::Vec3::unit_x(),
                                                    termin::Vec3::unit_y(),
                                                    termin::Vec3::unit_z()};
    ...
    std::string_view
    multibody3d_diagnostic_name(Multibody3DDiagnostic value) noexcept
    {
        switch (value)
        {
    ...
    RigidBody3DContribution::RigidBody3DContribution(
          ...
          diagnostic_name_(name)
    {
        ...
        else if (!finite(state) || state.pose.ang.norm() <= 1e-10 ||
                 !finite(gravity))
        {
        ...
        {
            inertia_.inertia_frame.ang =
                inertia_.inertia_frame.ang.normalized();
    ...
    const termin::SpatialInertia3&
    RigidBody3DContribution::inertia() const noexcept
    {
        return inertia_;
    }
    ...
    PointKinematics3DResult RigidBody3DContribution::point_kinematics(
        termin::Vec3 point_local) const noexcept
    {
        ...
        {
            std::fprintf(
                stderr, "[termin-qopt] rejected non-finite rigid-body point\n");
        ...
        try
        {
            ...
            value.velocity_world =
                state_.velocity_local.velocity_at_offset(point_local)
    ...
    RigidBody3DContribution::set_state(RigidBody3DState value) noexcept
    {
        ...
        {
            std::fprintf(stderr,
                         "[termin-qopt] rejected non-finite 3D body state\n");
    ...
    RigidBody3DContribution::set_gravity_world(termin::Vec3 value) noexcept
    {
        ...
        {
            std::fprintf(stderr,
                         "[termin-qopt] rejected non-finite 3D gravity\n");
            return Multibody3DDiagnostic::NonFiniteInput;
        }
                                      ...
                                      DynamicsAssemblyPhase phase) noexcept
    {
        ...
        if (phase == DynamicsAssemblyPhase::Acceleration)
        {
            const termin::Screw3 momentum =
                inertia_.momentum(state_.velocity_local);
        ...
        {
            load = mass.transform(
                termin::screw3_to_vec6_vw(state_.velocity_local));
        }
    ...
    void
    RigidBody3DContribution::apply_solution(DynamicsAssemblyPhase phase,
                                            const DynamicsTopology& topology,
    ...
    AssemblyDiagnostic RigidBody3DContribution::write_velocity(
        const DynamicsTopology& topology,
        ...
        double time_step) noexcept
    {
        ...
        state_.pose =
            state_snapshot_.pose * termin::se3_exp(velocity * time_step);
        state_.pose.ang = state_.pose.ang.normalized();
    ...
    AssemblyDiagnostic
    RigidBody3DContribution::write_corrected_midpoint_velocity(
        const DynamicsTopology& topology,
        ConstDenseVectorView midpoint_velocity,
                                        ...
                                        DynamicsAssemblyPhase phase) noexcept
    {
        ...
        const termin::Vec6 load = termin::screw3_to_vec6_vw(
            wrench_at_body_origin_world_.inverse_rotated_by(
                body_->state().pose.ang));
    ...
    const termin::Screw3&
    FixedPointJoint3DContribution::reaction_at_joint_anchor_world()
        const noexcept
    {
    ...
    AssemblyDiagnostic FixedPointJoint3DContribution::assemble(
        DynamicsAssembly& assembly, DynamicsAssemblyPhase phase) noexcept
    {
        ...
        else if (phase == DynamicsAssemblyPhase::PositionProjection)
        {
            const termin::Vec3 error =
                body_->state().pose.transform_point(body_anchor_local_) -
                world_anchor_;
            rhs = {-error.x, -error.y, -error.z};
        }
        return first(result,
                     assembly.add_constraint_rhs(constraint_, view(rhs)));
    }
    ...
    PointJoint3DContribution::PointJoint3DContribution(
        RigidBody3DContribution& a,
        termin::Vec3 local_a,
    ...
    AssemblyDiagnostic PointJoint3DContribution::register_topology(
        DynamicsTopology& topology) noexcept
    {
    ...
    PointJoint3DContribution::assemble(DynamicsAssembly& assembly,
                                       DynamicsAssemblyPhase phase) noexcept
    {
        const PointJacobian ja = point_jacobian_local(
            body_a_anchor_local_, body_a_->state().pose.ang);
        PointJacobian jb = point_jacobian_local(body_b_anchor_local_,
                                                body_b_->state().pose.ang);
        for (double& value : jb)
            value = -value;
        AssemblyDiagnostic result = assembly.add_constraint_jacobian(
            constraint_, body_a_->dofs(), view(ja));
        result = first(result,
                       assembly.add_constraint_jacobian(
                           constraint_, body_b_->dofs(), view(jb)));
        std::array<double, 3> rhs{};
        if (phase == DynamicsAssemblyPhase::Acceleration)
        {
    ...
    void PointJoint3DContribution::apply_solution(
        DynamicsAssemblyPhase phase,
        const DynamicsTopology& topology,
        ConstDenseVectorView,
        ConstDenseVectorView reactions) noexcept
    ...
    FixedRevoluteJoint3DContribution::FixedRevoluteJoint3DContribution(
        ...
        : body_(&body), body_anchor_local_(anchor), body_axis_local_(axis),
          world_anchor_(world_anchor), world_axis_(world_axis),
          diagnostic_name_(name)
    {
        ...
        else
        {
            ...
            if (body_->state()
                    .pose.ang.rotate(body_axis_local_)
                    .dot(world_axis_) < 0.0)
    ...
    Multibody3DDiagnostic
    FixedRevoluteJoint3DContribution::diagnostic() const noexcept
    {
        return diagnostic_;
    }

    const termin::Screw3&
    FixedRevoluteJoint3DContribution::reaction_at_joint_anchor_world()
        const noexcept
    {
        return reaction_at_joint_anchor_world_;
    }
    ...
    AssemblyDiagnostic FixedRevoluteJoint3DContribution::assemble(
        DynamicsAssembly& assembly, DynamicsAssemblyPhase phase) noexcept
    {
        const HingeFrame frame = hinge_frame(
            body_->state().pose.ang.rotate(body_axis_local_), world_axis_);
    ...
    double
    FixedRevoluteJoint3DContribution::position_error_linf() const noexcept
    {
        const HingeFrame frame = hinge_frame(
            body_->state().pose.ang.rotate(body_axis_local_), world_axis_);
        const termin::Vec3 point =
    ...
    double
    FixedRevoluteJoint3DContribution::velocity_error_linf() const noexcept
    {
        const HingeFrame frame = hinge_frame(
            body_->state().pose.ang.rotate(body_axis_local_), world_axis_);
    ...
    RevoluteJoint3DContribution::RevoluteJoint3DContribution(
        RigidBody3DContribution& a,
          ...
          body_b_axis_local_(axis_b), diagnostic_name_(name)
    {
        ...
        else
        {
            ...
            if (body_a_->state()
                    ...
                    .dot(body_b_->state().pose.ang.rotate(body_b_axis_local_)) <
                0.0)
    ...
    Multibody3DDiagnostic
    RevoluteJoint3DContribution::diagnostic() const noexcept
    {
        return diagnostic_;
    }
    ...
    AssemblyDiagnostic RevoluteJoint3DContribution::register_topology(
        DynamicsTopology& topology) noexcept
    {
        if (diagnostic_ != Multibody3DDiagnostic::None)
            return AssemblyDiagnostic::NonFiniteContribution;
    ...
    void RevoluteJoint3DContribution::apply_solution(
        DynamicsAssemblyPhase phase,
        const DynamicsTopology& topology,
        ConstDenseVectorView,
        ConstDenseVectorView reactions) noexcept
    ...
    double RevoluteJoint3DContribution::velocity_error_linf() const noexcept
    {
        ...
        const termin::Vec3 angular =
            body_a_->velocity_at_body_origin_world()
                ...
                .cross(frame.axis_b) +
            frame.axis_a.cross(
                body_b_->velocity_at_body_origin_world().ang.cross(
                    frame.axis_b));
