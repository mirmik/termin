...
namespace termin::physics_qopt {
    ...
    std::string_view point_kinematics3d_diagnostic_name(PointKinematics3DDiagnostic diagnostic) noexcept {
        switch (diagnostic) {
    ...
    ConstDenseMatrixView PointKinematics3D::linear_jacobian_world() const noexcept {
        return ConstDenseMatrixView::row_major(linear_jacobian_world_storage.data(), 3, dof_count());
    }

    PointKinematics3DDiagnostic
    PointKinematics3D::map_force_to_generalized_effort(termin::Vec3 force_world,
                                                       DenseVectorView generalized_effort) const noexcept {
        if (!force_world.is_finite()) {
            std::fprintf(stderr, "[termin-qopt] rejected non-finite point force\n");
            return PointKinematics3DDiagnostic::NonFiniteForce;
        }
        const std::size_t columns = dof_count();
        if (linear_jacobian_world_storage.size() != 3 * columns || generalized_effort.size != columns ||
            (columns != 0 && (generalized_effort.data == nullptr || generalized_effort.stride == 0))) {
            std::fprintf(stderr, "[termin-qopt] invalid point-force transpose output\n");
        ...
        for (std::size_t column = 0; column < columns; ++column) {
            ...
            for (std::size_t row = 0; row < 3; ++row) {
                generalized_effort[column] += linear_jacobian_world_storage[row * columns + column] * force[row];
            }
        }
    ...
    PointKinematics3DResult static_point_kinematics(termin::Vec3 position_world) noexcept {
        if (!position_world.is_finite()) {
            std::fprintf(stderr, "[termin-qopt] rejected non-finite static point\n");
