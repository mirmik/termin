...
namespace termin::physics_qopt
{
    ...
    ContactForceVariableBlock3DResult inverse_dynamics_contact_force_block(
    ...
    {
        auto rejected = [](RoboticsControlAdapterDiagnostic3D diagnostic,
        ...
        {
            return rejected(
                RoboticsControlAdapterDiagnostic3D::PointKinematicsFailure,
        ...
        try
        {
            ...
            result.normal_force_direction_world =
                normal_force_direction_world / normal_length;
            ...
            block.generalized_force_basis_storage.assign(
                articulation.dof_count() * 3, 0.0);
            ...
            for (std::size_t dof = 0; dof < articulation.dof_count(); ++dof)
            {
                for (std::size_t variable = 0; variable < 3; ++variable)
                {
                    block.generalized_force_basis_storage[dof * 3 + variable] =
                        directions[variable].x * jacobian(0, dof) +
                        directions[variable].y * jacobian(1, dof) +
                        directions[variable].z * jacobian(2, dof);
                }
            }

            block.inequality_row_count =
                maximum_normal_force.has_value() ? 6 : 5;
            block.inequality_matrix_storage.assign(
                block.inequality_row_count * 3, 0.0);
            block.inequality_target_storage.assign(block.inequality_row_count,
                                                   0.0);
            ...
            {
                block.inequality_matrix_storage[(row + 1) * 3] =
                    -friction_coefficient;
                block.inequality_matrix_storage[(row + 1) * 3 + 1] =
                    signs[row][0];
                block.inequality_matrix_storage[(row + 1) * 3 + 2] =
                    signs[row][1];
            }
            if (maximum_normal_force.has_value())
            {
        ...
        catch (const std::exception& error)
        {
            std::fprintf(
                stderr,
                "[termin-physics-qopt] contact force adapter failed: %s\n",
                error.what());
        }
        catch (...)
        {
