...
namespace termin::physics_qopt {
    ...
    namespace {
        ...
        AssemblyDiagnostic assembly_failure(Contact3DDiagnostic diagnostic) noexcept {
            return diagnostic == Contact3DDiagnostic::InternalFailure ? AssemblyDiagnostic::InternalFailure
                                                                      : AssemblyDiagnostic::NonFiniteContribution;
        }

        double projected(const termin::Vec3& normal, ConstDenseMatrixView jacobian, std::size_t column) noexcept {
            return normal.x * jacobian(0, column) + normal.y * jacobian(1, column) + normal.z * jacobian(2, column);
        }
    ...
    ContactEndpoint3D ContactEndpoint3D::static_world(termin::Vec3 position_world) noexcept {
    ...
    ContactEndpoint3D ContactEndpoint3D::rigid_body(RigidBody3DContribution& body, termin::Vec3 point_local) noexcept {
        ContactEndpoint3D endpoint;
    ...
    ContactEndpoint3D ContactEndpoint3D::articulation_unit(Articulation3DDynamicsContribution& articulation,
    ...
    ContactEndpoint3D ContactEndpoint3D::articulation_base(Articulation3DDynamicsContribution& articulation,
                                                           termin::Vec3 point_local) noexcept {
        ContactEndpoint3D endpoint;
    ...
    bool ContactEndpoint3D::valid() const noexcept {
        return kind_ != Kind::Invalid && point_.is_finite() &&
               (kind_ == Kind::StaticWorld || (kind_ == Kind::RigidBody && body_ != nullptr) ||
    ...
    bool ContactEndpoint3D::belongs_to(const Articulation3DDynamicsContribution& articulation) const noexcept {
        return articulation_ == &articulation && (kind_ == Kind::ArticulationBase || kind_ == Kind::ArticulationUnit);
    }
    ...
    Contact3DDiagnostic ContactSet3DContribution::set_contacts(std::vector<Contact3D> contacts) noexcept {
        return set_contacts(std::move(contacts), {});
    }

    Contact3DDiagnostic ContactSet3DContribution::set_contacts(std::vector<Contact3D> contacts,
        ...
        try {
            ...
            for (const Contact3D& previous : contacts_) {
                const bool key_present = std::binary_search(contacts.begin(), contacts.end(), previous, contact_less);
                if (key_present || previous.group_key == 0 ||
                    !std::binary_search(live_groups.begin(), live_groups.end(), previous.group_key)) {
                    continue;
                }
                const bool group_has_fresh_contacts =
                    std::any_of(contacts.begin(), contacts.end(), [&](const Contact3D& candidate) {
                ...
                const auto cached =
                    std::lower_bound(state_cache_.begin(), state_cache_.end(), previous.key, state_less);
                if (cached == state_cache_.end() || cached->key != previous.key || !cached->active ||
                    cached->normal_impulse <= kWarmImpulseTolerance) {
                    continue;
                }
                const PointKinematics3DResult endpoint_a = previous.endpoint_a.point_kinematics();
                const PointKinematics3DResult endpoint_b = previous.endpoint_b.point_kinematics();
                if (!endpoint_a.ok() || !endpoint_b.ok()) {
                    continue;
                }
                const double current_gap = previous.normal_from_a_to_b_world.dot(endpoint_b.value.position_world -
            ...
            state_cache_.erase(std::remove_if(state_cache_.begin(),
                                              ...
                                              [&](const ContactState3D& state) {
                                                  return !std::binary_search(contacts_.begin(),
                                                                             contacts_.end(),
            ...
            std::fprintf(stderr, "[termin-qopt] setting contact set failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    bool ContactSet3DContribution::set_persistence_distance(double distance) noexcept {
        if (!std::isfinite(distance) || distance < 0.0) {
            std::fprintf(stderr, "[termin-qopt] invalid contact persistence distance\n");
            return false;
        }
    ...
    bool ContactSet3DContribution::set_maximum_cached_contacts(std::size_t maximum) noexcept {
    ...
    Contact3DDiagnostic ContactSet3DContribution::validate_contacts() const noexcept {
        try {
            ...
            for (const Contact3D& contact : contacts_) {
                ...
                if (!contact.normal_from_a_to_b_world.is_finite() || !std::isfinite(normal_norm) ||
                ...
                if (!std::isfinite(contact.friction_coefficient) || contact.friction_coefficient < 0.0) {
    ...
    AssemblyDiagnostic ContactSet3DContribution::register_topology(DynamicsTopology&) noexcept {
        return AssemblyDiagnostic::None;
    }

    AssemblyDiagnostic ContactSet3DContribution::register_unilateral_constraints(DynamicsUnilateralTopology& topology,
                                                                                 double time_step) noexcept {
        diagnostic_ = validate_contacts();
        if (diagnostic_ != Contact3DDiagnostic::None || !std::isfinite(time_step) || time_step <= 0.0) {
            std::fprintf(stderr,
        ...
        try {
            ...
            warm_started_contact_count_ = 0;
            const std::string prefix = diagnostic_name_.empty() ? "contacts" : diagnostic_name_;
            for (std::size_t index = 0; index < contacts_.size(); ++index) {
                const auto cached =
                    std::lower_bound(state_cache_.begin(),
                                     state_cache_.end(),
                                     contacts_[index].key,
                                     [](const ContactState3D& state, std::uint64_t key) { return state.key < key; });
                if (cached != state_cache_.end() && cached->key == contacts_[index].key) {
                    states_[index] = *cached;
                    warm_started_contact_count_ +=
                        cached->active && cached->normal_impulse > kWarmImpulseTolerance ? 1U : 0U;
                }
                PointKinematics3DResult endpoint_a = contacts_[index].endpoint_a.point_kinematics();
                PointKinematics3DResult endpoint_b = contacts_[index].endpoint_b.point_kinematics();
                if (!endpoint_a.ok() || !endpoint_b.ok() ||
                ...
                const auto registration =
                    topology.register_constraint(1, prefix + ".normal." + std::to_string(contacts_[index].key));
                if (!registration.ok()) {
                    return registration.diagnostic;
                }
                step_contacts_[index].row = registration.handle;
                const auto tangents = tangent_basis(contacts_[index].normal_from_a_to_b_world);
                step_contacts_[index].tangent_1_world = tangents[0];
                step_contacts_[index].tangent_2_world = tangents[1];
                step_contacts_[index].reference_separation = contacts_[index].normal_from_a_to_b_world.dot(
                ...
                states_[index].normal_velocity = contacts_[index].normal_from_a_to_b_world.dot(
                    endpoint_b.value.velocity_world - endpoint_a.value.velocity_world);
            }
            state_cache_ = states_;
            return AssemblyDiagnostic::None;
        } catch (const std::exception& error) {
            std::fprintf(stderr, "[termin-qopt] contact row registration failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    AssemblyDiagnostic ContactSet3DContribution::register_friction_contacts(DynamicsFrictionTopology& topology,
                                                                            double time_step) noexcept {
        if (diagnostic_ != Contact3DDiagnostic::None || !std::isfinite(time_step) || time_step <= 0.0 ||
        ...
        try {
            ...
            for (std::size_t index = 0; index < contacts_.size(); ++index) {
                ...
                const auto registration = topology.register_contact(
                    step_contacts_[index].row, prefix + ".friction." + std::to_string(contacts_[index].key));
                if (!registration.ok()) {
                    return registration.diagnostic;
                }
            ...
            std::fprintf(stderr, "[termin-qopt] contact friction registration failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    bool ContactSet3DContribution::update_state(std::size_t index,
        ...
        if (index >= contacts_.size() || index >= step_contacts_.size() || index >= states_.size()) {
            return false;
        }
        ...
        state.signed_gap = contacts_[index].signed_gap +
                           normal.dot(endpoint_b.position_world - endpoint_a.position_world) -
                           step_contacts_[index].reference_separation;
        state.normal_velocity = normal.dot(endpoint_b.velocity_world - endpoint_a.velocity_world);
        return std::isfinite(state.signed_gap) && std::isfinite(state.normal_velocity);
    }

    AssemblyDiagnostic ContactSet3DContribution::assemble(DynamicsAssembly& assembly,
                                                          DynamicsAssemblyPhase phase) noexcept {
        if (phase == DynamicsAssemblyPhase::Acceleration) {
            return AssemblyDiagnostic::None;
        }
        if (diagnostic_ != Contact3DDiagnostic::None || !std::isfinite(time_step_) || time_step_ <= 0.0 ||
            step_contacts_.size() != contacts_.size()) {
            return AssemblyDiagnostic::NonFiniteContribution;
        }
        ...
        try {
            for (std::size_t index = 0; index < contacts_.size(); ++index) {
                ...
                const termin::Vec3& normal = contacts_[index].normal_from_a_to_b_world;
                const auto add_endpoint = [&](const PointKinematics3D& endpoint, double sign) noexcept {
                    if (endpoint.is_static()) {
                        return AssemblyDiagnostic::None;
                    }
                    const ConstDenseMatrixView jacobian = endpoint.linear_jacobian_world();
                    std::vector<double> row(endpoint.dof_count());
                    for (std::size_t column = 0; column < row.size(); ++column) {
                        row[column] = sign * projected(normal, jacobian, column);
                    }
                    return assembly.add_unilateral_jacobian(step_contacts_[index].row,
                                                            endpoint.dofs,
                ...
                result = assembly.add_unilateral_limit(step_contacts_[index].row, {limit.data(), limit.size(), 1});
                if (result != AssemblyDiagnostic::None) {
                    return result;
                }
            }
            ...
            std::fprintf(stderr, "[termin-qopt] contact assembly failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    AssemblyDiagnostic ContactSet3DContribution::assemble_friction(DynamicsFrictionAssembly& assembly) noexcept {
        if (diagnostic_ != Contact3DDiagnostic::None || step_contacts_.size() != contacts_.size() ||
        ...
        try {
            for (std::size_t index = 0; index < contacts_.size(); ++index) {
                ...
                const auto add_endpoint = [&](const PointKinematics3D& endpoint, double sign) noexcept {
                    ...
                    for (std::size_t column = 0; column < endpoint.dof_count(); ++column) {
                        normal_row[column] =
                            sign * projected(contacts_[index].normal_from_a_to_b_world, jacobian, column);
                        rows[column] = sign * projected(step_contacts_[index].tangent_1_world, jacobian, column);
                        rows[endpoint.dof_count() + column] =
                            sign * projected(step_contacts_[index].tangent_2_world, jacobian, column);
                    }
                    AssemblyDiagnostic result = assembly.add_contact_normal_jacobian(
                        handle,
                        endpoint.dofs,
                        ConstDenseMatrixView::row_major(normal_row.data(), 1, endpoint.dof_count()));
                    if (result != AssemblyDiagnostic::None) {
                        return result;
                    }
                    return assembly.add_tangent_jacobian(
                        handle, endpoint.dofs, ConstDenseMatrixView::row_major(rows.data(), 2, endpoint.dof_count()));
                };
                ...
                if (result == AssemblyDiagnostic::None) {
                    result = assembly.add_normal_impulse(handle, states_[index].normal_impulse);
                }
                if (result == AssemblyDiagnostic::None) {
                    result = assembly.add_friction_coefficient(handle, contacts_[index].friction_coefficient);
                }
                if (result != AssemblyDiagnostic::None) {
                    return result;
                }
            ...
            std::fprintf(stderr, "[termin-qopt] contact friction assembly failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    AssemblyDiagnostic ContactSet3DContribution::begin_step() noexcept {
        try {
            ...
            std::fprintf(stderr, "[termin-qopt] contact snapshot failed: %s\n", error.what());
        } catch (...) {
            std::fprintf(stderr,
    ...
    void ContactSet3DContribution::apply_unilateral_solution(const DynamicsTopology&,
        ...
        for (std::size_t index = 0; index < states_.size(); ++index) {
            ...
            if (!update_state(index, endpoint_a, endpoint_b)) {
                diagnostic_ = Contact3DDiagnostic::InvalidState;
                std::fprintf(stderr, "[termin-qopt] cannot update solved contact state\n");
                continue;
            }
            const DenseBlockInfo info =
                unilateral_topology.constraint_topology().block_info(step_contacts_[index].row.block);
    ...
    bool ContactSet3DContribution::write_unilateral_warm_start(const DynamicsUnilateralTopology& topology,
        ...
        for (std::size_t index = 0; index < states_.size(); ++index) {
            if (!states_[index].active || states_[index].normal_impulse <= kWarmImpulseTolerance) {
                continue;
            }
            const DenseBlockInfo info = topology.constraint_topology().block_info(step_contacts_[index].row.block);
    ...
    void ContactSet3DContribution::apply_friction_solution(const DynamicsFrictionTopology& topology,
        ...
        for (std::size_t index = 0; index < states_.size(); ++index) {
            ...
            const DenseBlockInfo contact_info = topology.contact_topology().block_info(handle.contact_block);
            const DenseBlockInfo tangent_info = topology.tangent_topology().block_info(handle.tangent_block);
            if (!contact_info.ok() || contact_info.size != 1 || contact_info.offset >= friction_work.size ||
                !tangent_info.ok() || contact_info.offset >= normal_impulses.size || tangent_info.size != 2 ||
            ...
            states_[index].tangent_impulse_world =
                step_contacts_[index].tangent_1_world * first + step_contacts_[index].tangent_2_world * second;
            ...
            const PointKinematics3DResult endpoint_a = contacts_[index].endpoint_a.point_kinematics();
            const PointKinematics3DResult endpoint_b = contacts_[index].endpoint_b.point_kinematics();
            if (endpoint_a.ok() && endpoint_b.ok()) {
                const termin::Vec3 relative_velocity =
                    endpoint_b.value.velocity_world - endpoint_a.value.velocity_world;
                const termin::Vec3& normal = contacts_[index].normal_from_a_to_b_world;
                states_[index].tangent_velocity_world = relative_velocity - normal * normal.dot(relative_velocity);
            }
            states_[index].friction_work = friction_work[contact_info.offset];
            const double capacity = contacts_[index].friction_coefficient * states_[index].normal_impulse;
            states_[index].sliding =
                capacity > 0.0 && states_[index].tangent_impulse_world.norm() >=
                                      capacity * std::cos(std::numbers::pi / 32.0) * (1.0 - kFrictionBoundaryTolerance);
            state_cache_[index] = states_[index];
        }
    }
    ...
    double ContactSet3DContribution::position_error_linf() const noexcept {
        ...
        for (std::size_t index = 0; index < contacts_.size(); ++index) {
            ...
            const double gap = contacts_[index].signed_gap +
                               normal.dot(endpoint_b.value.position_world - endpoint_a.value.position_world) -
                               step_contacts_[index].reference_separation;
    ...
    double ContactSet3DContribution::velocity_error_linf() const noexcept {
        ...
        for (std::size_t index = 0; index < contacts_.size(); ++index) {
            ...
            const double normal_velocity =
                normal.dot(endpoint_b.value.velocity_world - endpoint_a.value.velocity_world);
