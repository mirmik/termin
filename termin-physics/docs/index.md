...
`compute_mass_properties(collider, entity_scale, mass)` returns the canonical
`termin.geombase.SpatialInertia3` computed from the same uniform-density local
geometry used by
...
authored shape/entity orientation, while `RigidBody.inertia_frame_local` stores
the local center of mass and principal-axis rotation copied from
`SpatialInertia3.inertia_frame`. Use `shape_pose()` and
