...
namespace termin::physics
{
    ...
    size_t PhysicsWorld::add_box(
        double sx, double sy, double sz, double mass, const Pose3& p, bool stat)
    {
    ...
    size_t
    PhysicsWorld::add_sphere(double r, double mass, const Pose3& p, bool stat)
    {
        size_t i = add_body(RigidBody::create_sphere(r, mass, p, stat));
        auto c =
            std::make_shared<SphereCollider>(r, GeneralPose3(p.ang, p.lin));
        owned_colliders_.push_back(c);
        Collider* raw = c.get();
        ensure_collision_world()->add(raw);
        register_collider(i, raw);
        ...
        for (auto& [i, c] : body_to_collider_)
            if (i < bodies_.size())
            {
                if (auto* p = dynamic_cast<colliders::ColliderPrimitive*>(c);
                    p &&
                    dynamic_cast<colliders::AttachedCollider*>(c) == nullptr)
                {
                    const Pose3 pose = bodies_[i].shape_pose();
                    p->transform =
                        GeneralPose3(pose.ang, pose.lin, p->transform.scale);
                }
                if (collision_world_)
