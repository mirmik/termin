...
class EditorViewportFeatures:
    ...
    # ---------- Pipeline debug info (for framegraph debugger) ----------

    def get_passes_info(self, pipeline: RenderPipeline | None) -> list[tuple[str, bool]]:
        """Get list of passes with internal symbol availability."""
    ...
    def get_pass_internal_symbols(self, pipeline: RenderPipeline | None, pass_name: str) -> list[str]:
        """Get internal symbols for a specific pass."""
        if pipeline is None:
            return []
        for p in pipeline.passes:
    ...
    def set_pass_internal_symbol(self, pipeline: RenderPipeline | None, pass_name: str, symbol: str | None) -> None:
