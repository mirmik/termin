...
TC_API bool tc_pool_init(tc_pool* pool, size_t item_size, uint32_t initial_capacity);

// Free pool resources
TC_API void tc_pool_free(tc_pool* pool);

// Clear pool (mark all as free, bump generations)
TC_API void tc_pool_clear(tc_pool* pool);
...
TC_API tc_handle tc_pool_alloc(tc_pool* pool);

// Free a slot by handle (returns true if freed, false if invalid handle)
TC_API bool tc_pool_free_slot(tc_pool* pool, tc_handle h);

// Check if handle is valid (correct generation, occupied)
TC_API bool tc_pool_is_valid(const tc_pool* pool, tc_handle h);

// Get pointer to item by handle (returns NULL if invalid)
TC_API void* tc_pool_get(const tc_pool* pool, tc_handle h);
...
TC_API void tc_pool_foreach(tc_pool* pool, tc_pool_iter_fn callback, void* user_data);
