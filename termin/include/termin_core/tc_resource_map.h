...
TC_API tc_resource_map* tc_resource_map_new(tc_resource_free_fn destructor);

// Destroy map and all resources (calls destructor for each)
TC_API void tc_resource_map_free(tc_resource_map* map);

// Clear all resources (calls destructor for each)
TC_API void tc_resource_map_clear(tc_resource_map* map);
...
TC_API bool tc_resource_map_add(tc_resource_map* map, const char* uuid, void* resource);

// Get resource by UUID, returns NULL if not found
TC_API void* tc_resource_map_get(const tc_resource_map* map, const char* uuid);

// Remove resource by UUID (calls destructor)
// Returns true if removed
TC_API bool tc_resource_map_remove(tc_resource_map* map, const char* uuid);

// Check if UUID exists
TC_API bool tc_resource_map_contains(const tc_resource_map* map, const char* uuid);

// Get number of resources
TC_API size_t tc_resource_map_count(const tc_resource_map* map);
...
TC_API void tc_resource_map_foreach(
    tc_resource_map* map,
