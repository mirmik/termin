...
namespace termin::framegraph_remote_target {
    namespace {
        ...
        bool
        transfer_exact(Socket socket, Bytes bytes, bool write, const std::atomic<bool>& running, Transfer transfer) {
            std::size_t offset = 0;
            const auto deadline = Clock::now() + std::chrono::seconds(2);
            while (offset < bytes.size() && running.load(std::memory_order_acquire)) {
                const auto remaining = std::chrono::duration_cast<std::chrono::milliseconds>(deadline - Clock::now());
                if (remaining <= std::chrono::milliseconds::zero()) {
                    tc_log_error("remote framegraph target: socket transfer timed out");
                    return false;
                }
                const WaitResult wait = wait_socket(socket, write, std::min(remaining, std::chrono::milliseconds(50)));
                ...
                if (interrupted_error(error) || would_block_error(error))
                    continue;
                tc_log_error(
                    "remote framegraph target: socket %s failed (error=%d)", write ? "send" : "receive", error);
                return false;
            }
        ...
        bool send_bytes(Socket socket, std::span<const std::uint8_t> bytes, const std::atomic<bool>& running) {
            return transfer_exact(socket, bytes, true, running, [socket](std::span<const std::uint8_t> remaining) {
                return send(
        ...
        bool receive_bytes(Socket socket, std::span<std::uint8_t> bytes, const std::atomic<bool>& running) {
            return transfer_exact(socket, bytes, false, running, [socket](std::span<std::uint8_t> remaining) {
        ...
        bool topology_structure_equal(const TopologySnapshot& left, const TopologySnapshot& right) {
            return left.selected_target_id == right.selected_target_id && left.targets == right.targets &&
                   left.passes == right.passes && left.schedule == right.schedule &&
                   left.resources == right.resources && left.alias_groups == right.alias_groups;
        }
    ...
    public:
        Impl(FrameGraphDebugger* target_debugger, TargetServiceConfig target_config)
        ...
        bool start() {
            ...
            if (bind(listener, reinterpret_cast<const sockaddr*>(&address), sizeof(address)) != 0 ||
                ...
                tc_log_error("remote framegraph target: loopback bind/listen failed "
                             "(port=%u error=%d)",
            ...
            if (getsockname(listener, reinterpret_cast<sockaddr*>(&actual), &actual_size) != 0) {
                tc_log_error("remote framegraph target: getsockname failed (error=%d)", socket_error());
                close_socket(listener);
                cleanup_socket_runtime();
                return false;
            }
        ...
        void stop() {
            ...
            active_max_payload_bytes.store(WireLimits::max_payload_bytes, std::memory_order_release);
            ...
            if (pending_capture && debugger && std::this_thread::get_id() == owner_thread)
        ...
        void detach_debugger() {
            if (!require_owner("detach_debugger") || !debugger)
                return;
            if (pending_capture)
                finish_operation(StatusCode::resource_unavailable, "render runtime was detached");
            if (stream) {
                emit_status(stream->queued, StatusCode::resource_unavailable, "render runtime was detached");
                stream.reset();
            }
            if (burst) {
                emit_status(burst->queued, StatusCode::resource_unavailable, "render runtime was detached");
                burst.reset();
            }
            clear_ready_preview();
            debugger = nullptr;
    ...
    private:
        bool require_owner(const char* operation) const {
            if (std::this_thread::get_id() == owner_thread)
                return true;
            tc_log_error("remote framegraph target: %s called outside owner thread", operation);
            return false;
        }
        ...
        void validate_config() const {
            if (config.capture_memory_budget_bytes == 0 ||
                ...
                throw std::invalid_argument("remote framegraph target capture budget is invalid");
            }
            if (config.authentication_token.size() > WireLimits::max_token_bytes) {
                throw std::invalid_argument("remote framegraph target token exceeds wire limit");
            }
            for (const auto* field : {&config.platform, &config.abi, &config.build_type, &config.build_id}) {
                if (field->size() > WireLimits::max_identity_bytes) {
                    throw std::invalid_argument("remote framegraph target identity exceeds wire limit");
                }
            }
        }
        ...
        std::uint64_t target_id_for(const RenderExecutionTargetId& native_id,
            ...
            const auto found =
                std::find_if(target_bindings.begin(), target_bindings.end(), [&native_id](const auto& binding) {
                    return binding.native_id == native_id;
                });
            const std::uint64_t id = found == target_bindings.end() ? next_target_id++ : found->id;
            next_bindings.push_back({id, native_id});
            return id;
        }

        bool build_topology(TopologySnapshot& next, std::vector<TargetBinding>& next_bindings, std::string& error) {
            if (!debugger)
                return true;
            ...
            if (targets.size() > WireLimits::max_targets || passes.size() > WireLimits::max_passes ||
                schedule.size() > WireLimits::max_schedule_entries || resources.size() > WireLimits::max_resources ||
            ...
            for (const auto& target : targets) {
                if (target.label.empty() || target.label.size() > WireLimits::max_name_bytes) {
                    error = "framegraph target label exceeds protocol limits";
                    return false;
                }
                next.targets.push_back({target_id_for(target.id, next_bindings), target.label, target.renderable});
            }
            ...
            for (const auto& pass : passes) {
                if (pass.index >= std::numeric_limits<std::uint32_t>::max() || !valid_wire_name(pass.name) ||
                    !valid_wire_name(pass.type) || pass.reads.size() > WireLimits::max_names_per_pass ||
                    pass.writes.size() > WireLimits::max_names_per_pass ||
                    pass.internal_symbols.size() > WireLimits::max_names_per_pass || !valid_wire_names(pass.reads) ||
                    !valid_wire_names(pass.writes) || !valid_wire_names(pass.internal_symbols)) {
                    error = "framegraph pass exceeds protocol limits";
                    return false;
                }
                const std::uint64_t pass_id = static_cast<std::uint64_t>(pass.index) + 1;
                if (!pass_ids.insert(pass_id).second) {
                    error = "framegraph contains duplicate authored pass indices";
                    return false;
                }
            ...
            for (const auto& pass : schedule) {
                ...
                if (!pass_ids.contains(pass_id)) {
                    error = "framegraph schedule references an unpublished pass";
                    return false;
                }
            ...
            for (const auto& [canonical, names] : aliases) {
                if (!valid_wire_name(canonical) || names.size() > WireLimits::max_resources ||
        ...
        bool refresh_topology() {
            ...
            if (!build_topology(next, next_bindings, error)) {
                ...
                tc_log_error("remote framegraph target: topology projection failed: %s", topology_error.c_str());
                return false;
            }
            std::uint64_t next_revision = graph_revision;
            if (topology.graph_revision == 0 || !topology_structure_equal(topology, next)) {
                ++next_revision;
            }
            next.graph_revision = next_revision;
            const auto encoded = encode_message(next, 1, 1);
            if (!encoded) {
                topology_error = "framegraph topology cannot be encoded: " + encoded.detail;
                tc_log_error("remote framegraph target: topology projection failed: %s", topology_error.c_str());
                return false;
            }
            ...
            published_graph_revision.store(graph_revision, std::memory_order_release);
            return true;
        }

        Status make_status(std::uint64_t request_id, StatusCode code, std::string detail) const {
            return {
                request_id,
                graph_revision,
        ...
        void enforce_peer_payload_limit(OutboundPacket& packet, const QueuedCommand& queued) {
            if (!packet.topology)
                return;
            const auto encoded = encode_message(*packet.topology, 1, queued.session_id);
            if (encoded && encoded.value->size() - envelope_size <= queued.max_payload_bytes)
                return;

            packet.topology.reset();
            packet.status = make_status(queued.command.request_id,
                                        StatusCode::limit_exceeded,
                                        encoded ? "topology exceeds the client's negotiated payload limit"
        ...
        void enqueue(OutboundPacket packet) {
            const std::uint64_t capture_bytes = packet.capture ? packet.capture->bytes.size() : 0;
            const std::int64_t capture_frame = packet.capture ? packet.capture->metadata.frame_number : 0;
            if (pending_dropped_outbound != 0) {
                packet.drop = DropEvent{DropKind::receiver, pending_dropped_outbound, pending_dropped_after_frame, 0};
            }
            ...
            if (capture_bytes != 0) {
                queued_capture_bytes.fetch_sub(capture_bytes, std::memory_order_relaxed);
        ...
        void publish_ready_preview(std::shared_ptr<CaptureBlob> blob) {
            ...
            queued_capture_bytes.fetch_add(new_bytes, std::memory_order_relaxed);
            if (auto replaced = preview_slot.publish(std::move(blob))) {
                queued_capture_bytes.fetch_sub((*replaced)->bytes.size(), std::memory_order_relaxed);
                dropped_captures.fetch_add(1, std::memory_order_relaxed);
            }
        }
        ...
        void discard_obsolete_preview() {
            if (auto discarded = preview_slot.discard_ready()) {
                queued_capture_bytes.fetch_sub((*discarded)->bytes.size(), std::memory_order_relaxed);
                dropped_captures.fetch_add(1, std::memory_order_relaxed);
            }
        }

        std::optional<LatestValueSlot<std::shared_ptr<CaptureBlob>>::Delivery> take_ready_preview() {
        ...
        void reject_command(const QueuedCommand& queued, StatusCode code, const std::string& detail) {
            OutboundPacket packet;
            packet.session_id = queued.session_id;
            packet.status = make_status(queued.command.request_id, code, detail);
            enqueue(std::move(packet));
        }

        void emit_status(const QueuedCommand& queued, StatusCode code, std::string detail) {
            OutboundPacket packet;
            packet.session_id = queued.session_id;
            packet.status = make_status(queued.command.request_id, code, std::move(detail));
            enqueue(std::move(packet));
        }

        void emit_lifecycle_update(std::string detail) {
            const std::uint64_t session = active_session.load(std::memory_order_acquire);
            if (session == 0) {
            ...
            const std::uint32_t payload_limit = active_max_payload_bytes.load(std::memory_order_acquire);
            const auto encoded = encode_message(topology, 1, session);
            if (encoded && encoded.value->size() - envelope_size <= payload_limit) {
                packet.topology = topology;
            } else {
                detail += "; topology omitted because it exceeds the peer "
                          "payload limit";
            }
            packet.status = make_status(0, StatusCode::completed, std::move(detail));
            enqueue(std::move(packet));
        ...
        bool configure_capture_selector(const QueuedCommand& queued, std::uint32_t max_long_edge) {
            const Command& command = queued.command;
            if (command.target_id != topology.selected_target_id) {
                reject_command(queued, StatusCode::target_unavailable, "capture target must be selected first");
                return false;
            }
            debugger->set_capture_max_long_edge(max_long_edge);
            if (command.selector_kind == CaptureSelectorKind::resource) {
                if (std::find(topology.resources.begin(), topology.resources.end(), command.resource) ==
                    topology.resources.end()) {
                    reject_command(
                        queued, StatusCode::resource_unavailable, "capture resource is absent from topology");
                    return false;
                }
                debugger->request_resource(command.resource);
                return true;
            }
            const auto pass = std::find_if(topology.passes.begin(),
                                           topology.passes.end(),
                                           [&command](const WirePass& value) { return value.id == command.pass_id; });
            if (pass != topology.passes.end() &&
                std::find(pass->internal_symbols.begin(), pass->internal_symbols.end(), command.symbol) !=
                    pass->internal_symbols.end() &&
                debugger->request_internal(pass->authored_index, command.symbol)) {
                return true;
            }
            reject_command(queued, StatusCode::resource_unavailable, "capture selector is unavailable");
            return false;
        }
        ...
        bool issue_capture(const QueuedCommand& queued,
            ...
            if (pending_capture) {
                if (announce) {
                    reject_command(queued, StatusCode::limit_exceeded, "a framegraph capture is already pending");
                }
                return false;
            }
            const CaptureEncoding required =
                kind == CaptureKind::preview ? CaptureEncoding::rgba8 : CaptureEncoding::native_pixels;
            if (queued.command.encoding != required) {
                reject_command(queued,
                               StatusCode::limit_exceeded,
                               kind == CaptureKind::preview ? "preview capture requires rgba8 encoding"
                                                            : "exact capture requires native_pixels encoding");
                return false;
            }
            const std::uint32_t max_long_edge = kind == CaptureKind::preview ? queued.command.max_preview_long_edge : 0;
            if (!configure_capture_selector(queued, max_long_edge))
                return false;
            pending_capture =
                PendingCapture{queued, debugger->request_generation(), Clock::now(), kind, burst_index, burst_count};
            if (announce) {
                emit_status(queued,
                            StatusCode::accepted,
                            kind == CaptureKind::snapshot  ? "exact capture accepted"
                            : kind == CaptureKind::preview ? "live preview accepted"
        ...
        bool begin_capture(const QueuedCommand& queued) {
            if (stream || burst) {
                reject_command(queued, StatusCode::limit_exceeded, "continuous capture operation is active");
                return false;
            }
        ...
        std::chrono::nanoseconds preview_interval() const {
            ...
            return std::chrono::nanoseconds(1'000'000'000'000ULL / stream->queued.command.max_preview_millifps);
        }
        ...
        void schedule_continuous_capture() {
            ...
            if (stream) {
                ...
                if (!issue_capture(stream->queued, CaptureKind::preview)) {
                    emit_status(
                        stream->queued, StatusCode::resource_unavailable, "live preview selector became unavailable");
                    stream.reset();
                }
                return;
            }
            if (burst) {
                if (!issue_capture(
                        burst->queued, CaptureKind::burst, burst->next_index, burst->queued.command.burst_frames)) {
                    emit_status(burst->queued, StatusCode::resource_unavailable, "burst selector became unavailable");
                    burst.reset();
                }
            }
        }
        ...
        void poll_capture() {
            if (!pending_capture)
                return;
            if (active_session.load(std::memory_order_acquire) != pending_capture->queued.session_id) {
                reset_pending_capture();
                stream.reset();
                burst.reset();
                clear_ready_preview();
            ...
            if (!refresh_topology()) {
                finish_operation(StatusCode::resource_unavailable, "topology refresh failed during capture");
                return;
            }
            if (graph_revision != pending_capture->queued.command.graph_revision) {
                finish_operation(StatusCode::stale_revision, "topology changed while capture was pending");
                return;
            }
            debugger->finish_frame();
            if (debugger->request_generation() != pending_capture->debugger_generation) {
                finish_operation(StatusCode::cancelled, "capture was superseded by local debugger state");
                return;
            }
            if (debugger->capture_status() == FrameGraphCaptureRequestStatus::ResourceUnavailable) {
                finish_operation(StatusCode::resource_unavailable, "capture resource became unavailable");
                return;
            }
            ...
            if (!capture.has_capture() || capture.width() <= 0 || capture.height() <= 0) {
                finish_operation(StatusCode::resource_unavailable, "capture completed without a readable texture");
                return;
            }
            const PendingCapture pending = *pending_capture;
            const std::uint64_t pixels =
                static_cast<std::uint64_t>(capture.width()) * static_cast<std::uint64_t>(capture.height());
            const bool depth = capture.is_depth();
            const bool source_rgba8 = !depth && (capture.format() == tgfx::PixelFormat::RGBA8_UNorm ||
            ...
            const std::uint64_t output_bytes = preview ? pixels * 4ULL
                                                       : pixels * (source_rgba8 ? 4ULL
                                                                   : depth      ? sizeof(float)
                                                                                : 4ULL * sizeof(float));
            const std::uint64_t temporary_bytes = pixels * (depth ? sizeof(float) : 4ULL * sizeof(float));
            const std::uint64_t peer_limit =
                std::min(pending.queued.max_blob_bytes, config.capture_memory_budget_bytes);
            if (preview)
                discard_obsolete_preview();
            const std::uint64_t already_queued = queued_capture_bytes.load(std::memory_order_relaxed);
            if (output_bytes > peer_limit || temporary_bytes > peer_limit - output_bytes ||
                already_queued > peer_limit - output_bytes - temporary_bytes ||
                (pending.kind == CaptureKind::burst && burst && burst->retained_bytes > peer_limit - output_bytes)) {
                ...
                if (preview) {
                    ...
                    reset_pending_capture();
                    if (stream)
                        stream->next_capture_at = Clock::now() + preview_interval();
                    return;
                }
                finish_operation(StatusCode::limit_exceeded, "capture exceeds memory or peer budget");
                return;
            }
            ...
            const bool read = depth ? capture.read_depth_float(values) : capture.read_color_rgba_float(values);
            const std::uint64_t elapsed = static_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() - readback_started).count());
            if (!read) {
                finish_operation(StatusCode::resource_unavailable, "GPU readback failed");
                return;
            }
            ...
            if (preview && depth) {
                for (std::uint64_t pixel = 0; pixel < pixels; ++pixel) {
                    const auto value = static_cast<std::uint8_t>(
                        std::lround(std::clamp(values[static_cast<std::size_t>(pixel)], 0.0F, 1.0F) * 255.0F));
                ...
                std::transform(values.begin(), values.end(), blob->bytes.begin(), [](float value) {
                    const float bounded = std::clamp(value, 0.0F, 1.0F);
                    return static_cast<std::uint8_t>(std::lround(bounded * 255.0F));
                });
            } else {
                std::memcpy(blob->bytes.data(), values.data(), blob->bytes.size());
            }
            const std::uint64_t conversion_elapsed = static_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() - conversion_started).count());
            const std::uint32_t chunk_bytes = std::min(pending.queued.max_chunk_bytes, WireLimits::max_chunk_bytes);
            if (chunk_bytes == 0) {
                finish_operation(StatusCode::limit_exceeded, "capture chunk size is zero");
                return;
            }
            const std::uint64_t chunks64 = (output_bytes + chunk_bytes - 1) / chunk_bytes;
            if (chunks64 == 0 || chunks64 > WireLimits::max_chunks_per_blob) {
                finish_operation(StatusCode::limit_exceeded, "capture chunk count exceeds protocol limits");
                return;
            }
            const std::uint64_t capture_elapsed = static_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() - pending.requested_at).count());
            ...
            blob->metadata = {
                ...
                pending.kind,
                preview ? CaptureEncoding::rgba8 : CaptureEncoding::native_pixels,
            ...
            conversion_time_ns.fetch_add(conversion_elapsed, std::memory_order_relaxed);
            capture_time_ns.fetch_add(capture_elapsed, std::memory_order_relaxed);
            ...
            queued_capture_bytes.fetch_add(output_bytes, std::memory_order_relaxed);
            OutboundPacket packet;
            ...
            if (pending.kind == CaptureKind::burst && burst) {
                ...
                if (burst->next_index == burst->queued.command.burst_frames) {
                    packet.status = make_status(burst->queued.command.request_id,
                                                StatusCode::completed,
                                                "burst completed; frames=" + std::to_string(burst->next_index) +
                                                    "; bytes=" + std::to_string(burst->retained_bytes) +
                                                    "; last_readback_ns=" + std::to_string(elapsed));
                    burst.reset();
                }
            } else {
                packet.status = make_status(pending.queued.command.request_id,
                                            StatusCode::completed,
                                            "exact capture completed; readback_ns=" + std::to_string(elapsed) +
                                                "; bytes=" + std::to_string(output_bytes) +
                                                "; capture_ns=" + std::to_string(capture_elapsed));
            }
            enqueue(std::move(packet));
        }
        ...
        void apply_command(const QueuedCommand& queued) {
            ...
            if (!debugger && command.kind != CommandKind::refresh_topology &&
                command.kind != CommandKind::request_status && command.kind != CommandKind::ping &&
                command.kind != CommandKind::disconnect) {
                reject_command(queued, StatusCode::resource_unavailable, "render runtime is not attached");
                return;
            }
            const auto validation = validate_command(command, graph_revision);
            if (!validation) {
                if (validation.error == SelectorError::stale_revision) {
                    reject_command(queued, StatusCode::stale_revision, validation.detail);
                } else {
                    OutboundPacket packet;
                    packet.session_id = queued.session_id;
                    packet.error = ErrorEvent{422, command.request_id, graph_revision, validation.detail};
                    enqueue(std::move(packet));
                }
                return;
            }
            ...
            if (command.kind == CommandKind::refresh_topology) {
                ...
                if (refresh_topology()) {
                    packet.topology = topology;
                    packet.status = make_status(command.request_id, StatusCode::completed, "topology refreshed");
                } else {
                    packet.status = make_status(command.request_id, StatusCode::limit_exceeded, topology_error);
                }
            ...
            if (command.kind == CommandKind::select_target) {
                const auto binding =
                    std::find_if(target_bindings.begin(), target_bindings.end(), [&command](const auto& item) {
                        return item.id == command.target_id;
                    });
                if (binding == target_bindings.end() || !debugger->select_target(binding->native_id)) {
                    reject_command(queued, StatusCode::target_unavailable, "selected target is unavailable");
                    return;
                }
                OutboundPacket packet;
                packet.session_id = queued.session_id;
                if (refresh_topology()) {
                    packet.topology = topology;
                    packet.status = make_status(command.request_id, StatusCode::completed, "target selected");
                } else {
                    packet.status = make_status(command.request_id, StatusCode::limit_exceeded, topology_error);
                }
                enforce_peer_payload_limit(packet, queued);
            ...
            if (command.kind == CommandKind::request_status || command.kind == CommandKind::ping) {
                OutboundPacket packet;
                packet.session_id = queued.session_id;
                packet.status = make_status(
                    command.request_id, StatusCode::completed, command.kind == CommandKind::ping ? "pong" : "status");
                enqueue(std::move(packet));
                return;
            }
            ...
            if (command.kind == CommandKind::start_stream || command.kind == CommandKind::update_stream) {
                if (burst || (pending_capture && pending_capture->kind != CaptureKind::preview)) {
                    reject_command(queued, StatusCode::limit_exceeded, "another capture operation is active");
                    return;
                }
                if (command.kind == CommandKind::update_stream && !stream) {
                    emit_status(queued, StatusCode::completed, "live preview is already stopped");
                    return;
                }
                if (stream) {
                    ...
                    discard_obsolete_preview();
                    emit_status(previous, StatusCode::completed, "live preview configuration replaced");
                }
                stream = StreamState{queued, Clock::now(), Clock::now(), 0};
                emit_status(queued,
                            StatusCode::accepted,
                            command.kind == CommandKind::start_stream ? "live preview started"
            ...
            if (command.kind == CommandKind::stop_stream) {
                ...
                if (pending_capture && pending_capture->kind == CaptureKind::preview) {
                    reset_pending_capture();
                }
                stream.reset();
                clear_ready_preview();
                const auto elapsed_ns = std::max<std::uint64_t>(
                    ...
                    static_cast<std::uint64_t>(
                        std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() - stopped.started_at)
                            .count()));
                const std::uint64_t effective_millifps = stopped.delivered_frames * 1'000'000'000'000ULL / elapsed_ns;
                effective_preview_millifps.store(effective_millifps, std::memory_order_relaxed);
                emit_status(stopped.queued,
                            StatusCode::completed,
                            "live preview stopped; frames=" + std::to_string(stopped.delivered_frames) +
                                "; effective_millifps=" + std::to_string(effective_millifps) +
                                "; readback_ns=" + std::to_string(readback_time_ns.load(std::memory_order_relaxed)) +
                                "; convert_ns=" + std::to_string(conversion_time_ns.load(std::memory_order_relaxed)) +
                                "; bytes=" + std::to_string(captured_bytes.load(std::memory_order_relaxed)) +
                                "; dropped=" + std::to_string(dropped_captures.load(std::memory_order_relaxed)));
                emit_status(queued, StatusCode::completed, "live preview stop processed");
                return;
            }
            ...
            if (command.kind == CommandKind::capture_burst) {
                ...
                const std::size_t required_slots =
                    outbound_queue.size_approximate() + static_cast<std::size_t>(command.burst_frames) + 1;
                if (required_slots > outbound_queue.capacity()) {
                    reject_command(
                        queued, StatusCode::limit_exceeded, "burst exceeds the target outbound queue capacity");
                    return;
                }
                if (command.encoding != CaptureEncoding::native_pixels) {
                    reject_command(queued, StatusCode::limit_exceeded, "burst capture requires native_pixels");
                    return;
                }
                burst = BurstState{queued, 0, 0};
                emit_status(queued,
                            StatusCode::accepted,
                            "burst capture accepted; frames=" + std::to_string(command.burst_frames));
                return;
            }

            if (command.kind == CommandKind::cancel) {
                if (pending_capture) {
                    finish_operation(StatusCode::cancelled, "capture operation cancelled");
                    OutboundPacket acknowledgement;
                    acknowledgement.session_id = queued.session_id;
                    acknowledgement.status =
                        make_status(command.request_id, StatusCode::cancelled, "capture cancellation processed");
                    enqueue(std::move(acknowledgement));
                    return;
                }
                if (stream) {
                    emit_status(stream->queued, StatusCode::cancelled, "live preview cancelled");
                    stream.reset();
                    clear_ready_preview();
                    emit_status(queued, StatusCode::cancelled, "capture cancellation processed");
                    return;
                } else if (burst) {
                    emit_status(burst->queued, StatusCode::cancelled, "burst capture cancelled");
                    burst.reset();
                    emit_status(queued, StatusCode::cancelled, "capture cancellation processed");
                    return;
                }
                ...
                packet.session_id = queued.session_id;
                packet.status =
                    make_status(command.request_id, StatusCode::cancelled, "no topology operation was pending");
                enqueue(std::move(packet));
                return;
            }
        }

        bool send_wire(Socket socket,
            ...
            if (!encoded) {
                tc_log_error("remote framegraph target: failed to encode message: %s", encoded.detail.c_str());
                return false;
            }
            if (encoded.value->size() - envelope_size > max_payload_bytes) {
                tc_log_error("remote framegraph target: encoded message exceeds the "
        ...
        bool handshake(Socket socket,
            ...
            if (!decoded || !std::holds_alternative<ClientHello>(decoded.value->message)) {
                tc_log_error("remote framegraph target: client handshake is malformed");
                return false;
            }
            ...
            if (!version_ok || !constant_time_equal(hello.authentication_token, config.authentication_token)) {
                ...
                send_wire(socket,
                          ErrorEvent{401, 0, published_graph_revision.load(std::memory_order_acquire), detail},
                          sequence,
                          session_id);
                tc_log_error("remote framegraph target: rejected client: %s", detail.c_str());
                return false;
            }
            TargetHello target;
            target.capabilities = static_cast<std::uint64_t>(Capability::topology) |
            ...
            target.max_payload_bytes = std::min(hello.max_payload_bytes, WireLimits::max_payload_bytes);
            target.max_blob_bytes = std::min(hello.max_blob_bytes, WireLimits::max_blob_bytes);
            target.max_chunk_bytes = std::min({hello.max_chunk_bytes,
                                               WireLimits::max_chunk_bytes,
                                               target.max_payload_bytes > 64 ? target.max_payload_bytes - 64 : 0U});
            ...
            return send_wire(socket, target, sequence, session_id, negotiated_payload_bytes);
        }
        ...
        bool send_packet(Socket socket,
            ...
            if (packet.session_id != session_id) {
                if (packet.capture)
                    queued_capture_bytes.fetch_sub(packet.capture->bytes.size(), std::memory_order_relaxed);
                return true;
            }
            if (packet.drop && !send_wire(socket, *packet.drop, sequence, session_id, max_payload_bytes))
                return false;
            if (packet.error && !send_wire(socket, *packet.error, sequence, session_id, max_payload_bytes))
                return false;
            if (packet.topology && !send_wire(socket, *packet.topology, sequence, session_id, max_payload_bytes))
                return false;
            if (packet.capture) {
                const auto transfer_started = Clock::now();
                const CaptureBlob& capture = *packet.capture;
                bool sent = send_wire(socket, capture.metadata, sequence, session_id, max_payload_bytes);
                for (std::uint32_t index = 0; sent && index < capture.metadata.chunk_count; ++index) {
                    const std::uint64_t offset = static_cast<std::uint64_t>(index) * capture.chunk_bytes;
                    const std::size_t count = static_cast<std::size_t>(
                        std::min<std::uint64_t>(capture.chunk_bytes, capture.bytes.size() - offset));
                    BlobChunk chunk;
                    ...
                    chunk.bytes.assign(capture.bytes.begin() + offset, capture.bytes.begin() + offset + count);
                    sent = send_wire(socket, chunk, sequence, session_id, max_payload_bytes);
                }
                queued_capture_bytes.fetch_sub(capture.bytes.size(), std::memory_order_relaxed);
                capture_transfer_ns = static_cast<std::uint64_t>(
                    std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() - transfer_started).count());
                transfer_encode_time_ns.fetch_add(capture_transfer_ns, std::memory_order_relaxed);
                if (!sent)
                    return false;
            }
            if (packet.status) {
                ...
                if (capture_transfer_ns != 0)
                    status.detail += "; transfer_encode_ns=" + std::to_string(capture_transfer_ns);
                if (!send_wire(socket, status, sequence, session_id, max_payload_bytes))
                    return false;
            }
            return true;
        }
        ...
        void io_main() {
            while (running.load(std::memory_order_acquire)) {
                ...
                if (wait == WaitResult::failed) {
                    if (running.load(std::memory_order_acquire)) {
                        tc_log_error("remote framegraph target: listener wait failed");
                    }
                    break;
                }
                sockaddr_in peer{};
                ...
                const std::uint64_t session_id = next_session_id.fetch_add(1, std::memory_order_relaxed);
                std::uint64_t sequence = 1;
                std::uint32_t max_payload_bytes = WireLimits::max_payload_bytes;
                std::uint64_t max_blob_bytes = WireLimits::max_blob_bytes;
                std::uint32_t max_chunk_bytes = WireLimits::max_chunk_bytes;
                if (!handshake(client, sequence, session_id, max_payload_bytes, max_blob_bytes, max_chunk_bytes)) {
                    rejected_clients.fetch_add(1, std::memory_order_relaxed);
                    release_client(client);
                    continue;
                }
                active_max_payload_bytes.store(max_payload_bytes, std::memory_order_release);
                active_session.store(session_id, std::memory_order_release);
                client_connected.store(true, std::memory_order_release);
                tc_log_info("remote framegraph target: client connected (session=%llu)",
                            static_cast<unsigned long long>(session_id));
                serve_client(client, sequence, session_id, max_payload_bytes, max_blob_bytes, max_chunk_bytes);
                client_connected.store(false, std::memory_order_release);
                active_session.store(0, std::memory_order_release);
                active_max_payload_bytes.store(WireLimits::max_payload_bytes, std::memory_order_release);
                release_client(client);
        ...
        void serve_client(Socket socket,
            ...
            while (running.load(std::memory_order_acquire)) {
                OutboundPacket packet;
                while (outbound_queue.try_pop(packet)) {
                    if (!send_packet(socket, packet, sequence, session_id, max_payload_bytes))
                        return;
                }
                if (auto preview = take_ready_preview()) {
                    ...
                    if (preview->dropped_before != 0) {
                        preview_packet.drop = DropEvent{DropKind::receiver,
                                                        preview->dropped_before,
                                                        preview->value->metadata.frame_number,
                                                        sequence};
                    }
                    preview_packet.capture = std::move(preview->value);
                    if (!send_packet(socket, preview_packet, sequence, session_id, max_payload_bytes))
                        return;
                }
                ...
                if (decoded.value->envelope.session_id != session_id) {
                    const std::uint64_t revision = published_graph_revision.load(std::memory_order_acquire);
                    if (!send_wire(socket,
                                   ErrorEvent{409, 0, revision, "message session_id does not match connection"},
                                   sequence,
                                   session_id,
                                   max_payload_bytes))
                        return;
                ...
                const Command& command = std::get<Command>(decoded.value->message);
                if (command.kind == CommandKind::disconnect) {
                    ...
                    Status status{command.request_id,
                                  ...
                                  now_ns(),
                                  "disconnecting"};
                    send_wire(socket, status, sequence, session_id, max_payload_bytes);
                    return;
                }
                if (!command_queue.try_push(
                        {session_id, max_payload_bytes, max_blob_bytes, max_chunk_bytes, command})) {
                    ...
                    const std::uint64_t revision = published_graph_revision.load(std::memory_order_acquire);
                    if (!send_wire(socket,
                                   ErrorEvent{429, command.request_id, revision, "target command queue is full"},
                                   sequence,
                                   session_id,
                                   max_payload_bytes))
                        return;
        ...
        std::atomic<std::uint32_t> active_max_payload_bytes{WireLimits::max_payload_bytes};
    ...
    RemoteFrameGraphTarget::RemoteFrameGraphTarget(FrameGraphDebugger& debugger, TargetServiceConfig config)
