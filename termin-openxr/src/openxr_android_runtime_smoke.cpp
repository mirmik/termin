    ...
    namespace {
        ...
        struct ScenePrimitiveSmoke {
            ...
            bool init(tgfx::IRenderDevice& device, tgfx::PixelFormat color_format, uint32_t sample_count) {
                ...
                device.upload_buffer(vbo,
                                     std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(mesh->vertices),
                ...
                device.upload_buffer(ebo,
                                     std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(mesh->indices),
                ...
                vertex_layout.attributes.push_back(
                    {1, tgfx::VertexFormat::Float3, static_cast<uint32_t>(normal_attr->offset)});
        ...
        termin::XrOriginComponent* resolve_runtime_xr_origin(termin::TcSceneRef scene,
            ...
            if (xr_config) {
                ...
                termin::XrOriginComponent* xr_origin =
                    xr_origin_component_from_entity_uuid(scene, xr_config->xr_origin_uuid);
                if (!xr_origin) {
            ...
            tc_log(TC_LOG_WARN,
                   "[OpenXR scene] no xr_stereo render target config found; "
        ...
        struct OpenXRRuntimeScene {
            ...
            void install_runtime_pipeline_factory() {
                engine->rendering_manager.set_pipeline_factory([](const std::string& key) -> tc_pipeline_handle {
                    const tc_pipeline_template_handle template_handle = tc_pipeline_template_find(key.c_str());
                    if (tc_pipeline_template_handle_is_invalid(template_handle)) {
                        tc_log_error("[OpenXR scene] compiled runtime pipeline template '%s' was not found",
                                     key.c_str());
                    ...
                    try {
                        ...
                        tc_log_info("[OpenXR scene] runtime pipeline template '%s' model=%d handle=(%u,%u)",
                                    key.c_str(),
                        ...
                        const tc_pipeline_template_handle instance_template = tc_pipeline_get_template(handle);
                        const tc_pipeline_template* instance_definition = tc_pipeline_template_get(instance_template);
                        tc_log_info("[OpenXR scene] instantiated compiled runtime pipeline '%s' passes=%zu "
                                    "model=%d template=(%u,%u)",
                                    key.c_str(),
                                    compiled.pass_count(),
                                    instance_definition ? static_cast<int>(instance_definition->execution_model) : -1,
                                    instance_template.index,
                                    instance_template.generation);
                        return handle;
                    } catch (const std::exception& e) {
                        tc_log_error(
                            "[OpenXR scene] runtime pipeline '%s' instantiation failed: %s", key.c_str(), e.what());
                        return TC_PIPELINE_HANDLE_INVALID;
                    }
                });
            }
            ...
            bool load(const std::string& asset_root, tgfx::GraphicsHost& graphics_host) {
                ...
                for (const char* name : required_components) {
                    if (!tc_component_registry_has(name)) {
                        log_error("OpenXR scene",
                                  (std::string("required component is not registered: ") + name).c_str());
                ...
                if (!std::filesystem::is_regular_file(manifest_path)) {
                    log_error("OpenXR scene",
                              (std::string("runtime manifest not found at ") + manifest_path.string()).c_str());
                ...
                if (!std::filesystem::is_regular_file(ui_font_path)) {
                    ...
                    tc_log_error("[OpenXR scene] packaged native UI font not found at '%s'", ui_font_path.c_str());
                    return false;
                }
                if (setenv("TERMIN_UI_FONT", ui_font_path.c_str(), 1) != 0) {
                    log_error("OpenXR scene", "failed to configure native UI font");
                    tc_log_error("[OpenXR scene] failed to set TERMIN_UI_FONT='%s'", ui_font_path.c_str());
                    return false;
                }
                ...
                xr_render_target =
                    engine->rendering_manager.topology().find_render_target(scene.handle(), xr_target_name);
                if (!tc_render_target_handle_valid(xr_render_target)) {
                    log_error("OpenXR scene", "scene has no restored XR render target");
                    tc_log_error("[OpenXR scene] restored XR render target '%s' was not found", xr_target_name.c_str());
                ...
                if (!pipeline_template || pipeline_template->execution_model != TC_PIPELINE_EXECUTION_XR_MULTIVIEW) {
                    tc_log_error("[OpenXR scene] xr_stereo pipeline model mismatch: model=%d "
                                 "template=(%u,%u)",
                                 pipeline_template ? static_cast<int>(pipeline_template->execution_model) : -1,
                    ...
                    tc_log_error("[OpenXR scene] xr_stereo target rejected non-xr_multiview pipeline");
                    engine->rendering_manager.detach_scene_full(scene.handle());
                    pipeline = {};
                    xr_render_target = TC_RENDER_TARGET_HANDLE_INVALID;
                    package.destroy();
                ...
                engine->rendering_manager.set_render_target_context_provider(
                    ...
                    [this](termin::RenderingManager&,
                        ...
                        return build_active_eye_context(
                            render_target, base_context_name, internal_entities, contexts, default_context_name);
                    });
            }

            bool build_active_eye_context(tc_render_target_handle render_target,
                                          const std::string& base_context_name,
                                          tc_entity_handle internal_entities,
                                          std::unordered_map<std::string, termin::RenderTargetContext>& contexts,
                                          std::string& default_context_name) {
                ...
                termin::Pose3 origin_pose3(origin_transform.global_rotation(), origin_transform.global_position());
                ...
                for (uint32_t eye = 0; eye < 2; ++eye) {
                    ...
                    termin::Vec3 eye_position_in_reference = xr_position_to_scene_position(xr_view.pose.position);
                    ...
                    camera.projection = make_engine_projection_from_xr_fov(
                        xr_view.fov, static_cast<float>(xr_origin->near_clip), static_cast<float>(xr_origin->far_clip));
                ...
                if (!stereo_contract_logged) {
                    ...
                    tc_log_info("[OpenXR scene] stereo views: left_pos=(%.6f,%.6f,%.6f) "
                                "right_pos=(%.6f,%.6f,%.6f) baseline=%.6f",
                                stereo.left.position.x,
                                stereo.left.position.y,
                                stereo.left.position.z,
                                stereo.right.position.x,
                                stereo.right.position.y,
                                stereo.right.position.z,
                                baseline.norm());
                    for (uint32_t eye = 0; eye < 2; ++eye) {
                        const XrView& xr_view = frame.views[eye];
                        const termin::RenderCamera& camera = *cameras[eye];
                        tc_log_info("[OpenXR scene] eye[%u]: xr_pose=(%.6f,%.6f,%.6f) "
                                    "fov=(%.6f,%.6f,%.6f,%.6f) "
                                    ...
                                    camera.projection(2, 1),
                                    camera.projection(1, 1),
                                    camera.projection(1, 2),
                                    camera.projection(3, 2),
                ...
                target.view.primary->position = (stereo.left.position + stereo.right.position) * 0.5;
            ...
            void render_multiview(tgfx::TextureHandle color_texture,
                                  uint32_t width,
                                  uint32_t height,
        ...
        void smoke_thread_main(void* java_vm, void* activity_or_context, std::string asset_root) {
            ...
            xr.get_instance_proc_addr =
                reinterpret_cast<PFN_xrGetInstanceProcAddr>(dlsym(loader, "xrGetInstanceProcAddr"));
            ...
            XrResult result =
                xr.initialize_loader(reinterpret_cast<const XrLoaderInitInfoBaseHeaderKHR*>(&loader_init));
            if (XR_FAILED(result)) {
            ...
            std::strncpy(instance_create_info.applicationInfo.applicationName,
            ...
            load_instance_proc(
                xr, instance, "xrDestroyInstance", reinterpret_cast<PFN_xrVoidFunction*>(&xr.destroy_instance));
            load_instance_proc(xr, instance, "xrGetSystem", reinterpret_cast<PFN_xrVoidFunction*>(&xr.get_system));
            load_instance_proc(
                xr, instance, "xrCreateSession", reinterpret_cast<PFN_xrVoidFunction*>(&xr.create_session));
            load_instance_proc(
                xr, instance, "xrDestroySession", reinterpret_cast<PFN_xrVoidFunction*>(&xr.destroy_session));
            load_instance_proc(xr,
                               instance,
                               "xrCreateReferenceSpace",
                               reinterpret_cast<PFN_xrVoidFunction*>(&xr.create_reference_space));
            load_instance_proc(
                xr, instance, "xrCreateActionSpace", reinterpret_cast<PFN_xrVoidFunction*>(&xr.create_action_space));
            load_instance_proc(
                xr, instance, "xrDestroySpace", reinterpret_cast<PFN_xrVoidFunction*>(&xr.destroy_space));
            ...
            load_instance_proc(
                xr, instance, "xrCreateSwapchain", reinterpret_cast<PFN_xrVoidFunction*>(&xr.create_swapchain));
            load_instance_proc(
                xr, instance, "xrDestroySwapchain", reinterpret_cast<PFN_xrVoidFunction*>(&xr.destroy_swapchain));
            load_instance_proc(xr,
                               instance,
            ...
            load_instance_proc(
                xr, instance, "xrWaitSwapchainImage", reinterpret_cast<PFN_xrVoidFunction*>(&xr.wait_swapchain_image));
            load_instance_proc(xr,
                               instance,
            ...
            load_instance_proc(
                xr, instance, "xrBeginSession", reinterpret_cast<PFN_xrVoidFunction*>(&xr.begin_session));
            ...
            load_instance_proc(
                xr, instance, "xrStringToPath", reinterpret_cast<PFN_xrVoidFunction*>(&xr.string_to_path));
            load_instance_proc(
                xr, instance, "xrCreateActionSet", reinterpret_cast<PFN_xrVoidFunction*>(&xr.create_action_set));
            load_instance_proc(
                xr, instance, "xrDestroyActionSet", reinterpret_cast<PFN_xrVoidFunction*>(&xr.destroy_action_set));
            load_instance_proc(
                xr, instance, "xrCreateAction", reinterpret_cast<PFN_xrVoidFunction*>(&xr.create_action));
            load_instance_proc(
                xr, instance, "xrDestroyAction", reinterpret_cast<PFN_xrVoidFunction*>(&xr.destroy_action));
            load_instance_proc(xr,
                               instance,
            ...
            load_instance_proc(
                xr, instance, "xrGetActionStatePose", reinterpret_cast<PFN_xrVoidFunction*>(&xr.get_action_state_pose));
            load_instance_proc(xr,
                               instance,
            ...
            if (XR_FAILED(result)) {
                __android_log_print(
                    ANDROID_LOG_ERROR, kLogTag, "xrGetVulkanGraphicsRequirementsKHR failed: %d", result);
                xr.destroy_instance(instance);
                g_smoke.running.store(false);
                return;
            }
            ...
            if (!query_openxr_vulkan_extensions(
                    xr.get_vulkan_instance_extensions, instance, system_id, instance_extension_storage) ||
                !query_openxr_vulkan_extensions(
                    xr.get_vulkan_device_extensions, instance, system_id, device_extension_storage)) {
                xr.destroy_instance(instance);
                g_smoke.running.store(false);
                return;
            }
            ...
            try {
                ...
                if (vulkan_requirements.maxApiVersionSupported < requested_vulkan_version) {
                    throw std::runtime_error("OpenXR runtime does not support Vulkan 1.1 required for multiview");
                }
                ...
                device_info.api_version = VK_MAKE_API_VERSION(0,
                ...
                device_info.physical_device_selector = [&](VkInstance vk_instance) -> VkPhysicalDevice {
                    ...
                    XrResult select_result =
                        xr.get_vulkan_graphics_device(instance, system_id, vk_instance, &physical_device);
                    if (XR_FAILED(select_result)) {
                        __android_log_print(
                            ANDROID_LOG_ERROR, kLogTag, "xrGetVulkanGraphicsDeviceKHR failed: %d", select_result);
                ...
                if (!capabilities.supports_texture_arrays || !capabilities.supports_multiview ||
                    capabilities.max_multiview_views < 2) {
                    throw std::runtime_error("Vulkan device does not satisfy the two-view multiview contract");
                }
                render_device = owned_device.get();
                graphics_host = tgfx::GraphicsHost::adopt_application_device(std::move(owned_device));
            } catch (const std::exception& e) {
            ...
            const bool runtime_scene_ready = runtime_scene_requested && runtime_scene.load(asset_root, *graphics_host);
            ...
            xr.enumerate_view_configuration_views(
                instance, system_id, XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO, 0, &view_count, nullptr);
            ...
            xr.enumerate_view_configuration_views(instance,
                                                  system_id,
            ...
            xr.enumerate_environment_blend_modes(
                instance, system_id, XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO, 0, &blend_count, nullptr);
            std::vector<XrEnvironmentBlendMode> blend_modes(blend_count);
            xr.enumerate_environment_blend_modes(instance,
                                                 system_id,
                                                 XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
                                                 blend_count,
                                                 &blend_count,
            ...
            if (tgfx_color_format == tgfx::PixelFormat::Undefined) {
                __android_log_print(ANDROID_LOG_ERROR,
                                    kLogTag,
                                    "unsupported Vulkan swapchain format: 0x%llx",
            ...
            __android_log_print(
                ANDROID_LOG_INFO, kLogTag, "OpenXR smoke color format: 0x%llx", static_cast<long long>(color_format));
                ...
                tgfx::TextureUsage::ColorAttachment | tgfx::TextureUsage::CopySrc | tgfx::TextureUsage::CopyDst;
            {
                ...
                xr.enumerate_swapchain_images(color_swapchain,
                                              image_count,
                                              &image_count,
            ...
            if (runtime_scene_requested && !runtime_scene_ready) {
                __android_log_print(
                    ANDROID_LOG_ERROR, kLogTag, "OpenXR runtime scene is not ready; rendering clear frames only");
            }
            ...
            while (g_smoke.running.load()) {
                ...
                while (xr.poll_event(instance, &event) == XR_SUCCESS) {
                    if (event.type == XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED) {
                        ...
                        __android_log_print(
                            ANDROID_LOG_INFO, kLogTag, "session state: %s", session_state_name(state_event->state));
                ...
                if (frame_state.shouldRender) {
                    ...
                    if (XR_SUCCEEDED(result) && located_view_count == view_count) {
                        ...
                        if (!image_acquired) {
                            __android_log_print(
                                ANDROID_LOG_ERROR, kLogTag, "xrAcquireSwapchainImage failed: %d", result);
                        }
                        ...
                        if (image_acquired) {
                            ...
                            if (XR_FAILED(result)) {
                                __android_log_print(
                                    ANDROID_LOG_ERROR, kLogTag, "xrWaitSwapchainImage failed: %d", result);
                                image_ready = false;
                            }
                        }
                ...
                if (wall_seconds >= 2.0 && fps_window_frames > 0) {
                    ...
                    __android_log_print(ANDROID_LOG_INFO,
                                        ...
                                        fps_window_predicted_delta_min_ms < 0.0 ? 0.0
                                                                                : fps_window_predicted_delta_min_ms,
