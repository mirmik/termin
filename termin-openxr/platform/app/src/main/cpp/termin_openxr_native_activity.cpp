...
namespace {
        ...
        while (index <= path.size()) {
            ...
            if (!part.empty()) {
                ...
                if (mkdir(current.c_str(), 0700) != 0 && errno != EEXIST) {
                    __android_log_print(
                        ANDROID_LOG_ERROR, kLogTag, "mkdir failed '%s': %s", current.c_str(), std::strerror(errno));
                    return false;
                }
            }
    ...
    bool copy_asset_file(AAssetManager* assets, const std::string& asset_path, const std::string& target_path) {
        ...
        if (!out) {
            __android_log_print(
                ANDROID_LOG_ERROR, kLogTag, "open target failed '%s': %s", target_path.c_str(), std::strerror(errno));
            AAsset_close(asset);
            return false;
        }
...
extern "C" void ANativeActivity_onCreate(ANativeActivity* activity, void*, size_t) {
    ...
    if (g_state.asset_root.empty()) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, kLogTag, "asset copy failed into '%s'", g_state.asset_root.c_str());
    } else {
        __android_log_print(ANDROID_LOG_INFO, kLogTag, "assets copied into '%s'", g_state.asset_root.c_str());
    }
