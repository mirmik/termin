...
namespace termin::framegraph_remote {
    ...
    struct SelectorValidation {
        ...
        std::string detail;
        explicit operator bool() const {
            return error == SelectorError::none;
        }
    };
    ...
    TERMIN_FRAMEGRAPH_REMOTE_API bool version_is_compatible(std::uint16_t major, std::uint16_t minor);
    ...
    TERMIN_FRAMEGRAPH_REMOTE_API CodecResult<Envelope> decode_envelope(std::span<const std::uint8_t> bytes);
    TERMIN_FRAMEGRAPH_REMOTE_API CodecResult<DecodedMessage> decode_message(std::span<const std::uint8_t> bytes);
    TERMIN_FRAMEGRAPH_REMOTE_API SelectorValidation validate_command(const Command& command,
