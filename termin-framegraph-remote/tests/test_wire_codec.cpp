...
namespace {
    ...
    template <typename T> void check_round_trip(const T& value, std::uint64_t sequence) {
        ...
        const auto encoded = encode_message(source, sequence, 0x1122334455667788ULL, 0x23);
        REQUIRE(encoded);
...
TEST_CASE("Remote framegraph codec round-trips every v1 message family") {
    ...
    topology.passes = {
        {77, 0, "Shadow", "ShadowPass", true, false, {"DEPTH"}, {"SHADOW"}, {"shadow-map"}},
        {78, 1, "Color", "ColorPass", true, false, {"SHADOW"}, {"OUTPUT"}, {}},
    };
    ...
    check_round_trip(Status{17, 9, SessionState::waiting_capture, StatusCode::accepted, 1, 3, 0, 1234, "queued"}, 5);
...
TEST_CASE("Command validation rejects stale and incomplete selectors") {
    ...
    CHECK(validate_command(command, 9).error == SelectorError::invalid_stream_options);
    ...
    CHECK(validate_command(command, 9).error == SelectorError::invalid_burst_options);
...
TEST_CASE("Blob assembler enforces identity ordering offsets and final size") {
    ...
    CHECK(assembler.bytes() == std::vector<std::uint8_t>({1, 2, 3, 4, 5, 6, 7, 8}));
