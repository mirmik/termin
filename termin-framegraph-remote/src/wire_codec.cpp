...
template <typename T>
CodecResult<T> failure(CodecError error, std::string detail) {
...
public:
    ...
    bool byte_vector(Bytes& value, std::uint32_t limit, const char* field) {
        ...
        if (!count(length, limit, field) || !available(length)) return false;
...
private:
    bool available(std::size_t count) {
        ...
        detail_ = "message payload is truncated at byte " +
...
bool valid_selector_kind(CaptureSelectorKind value) {
    return value == CaptureSelectorKind::resource ||
           value == CaptureSelectorKind::internal_symbol;
}

bool valid_encoding(CaptureEncoding value) {
    return value == CaptureEncoding::native_pixels ||
           value == CaptureEncoding::rgba8 || value == CaptureEncoding::png;
}

bool valid_capture_kind(CaptureKind value) {
    return value == CaptureKind::snapshot || value == CaptureKind::preview ||
           value == CaptureKind::burst;
}
...
bool valid_status_code(StatusCode value) {
    return static_cast<std::uint16_t>(value) <=
           static_cast<std::uint16_t>(StatusCode::limit_exceeded);
}
...
bool valid_hello_limits(std::uint32_t payload, std::uint64_t blob,
                        std::uint32_t chunk) {
    return payload > 0 && payload <= WireLimits::max_payload_bytes &&
           blob > 0 && blob <= WireLimits::max_blob_bytes && chunk > 0 &&
           chunk <= WireLimits::max_chunk_bytes && chunk < payload;
}

template <typename T>
bool read_enum_u8(Reader& reader, T& value) {
    std::uint8_t raw = 0;
...
template <typename T>
bool read_enum_u16(Reader& reader, T& value) {
    std::uint16_t raw = 0;
    if (!reader.u16(raw)) return false;
    value = static_cast<T>(raw);
    return true;
}
...
bool encode_string_list(Writer& out, const std::vector<std::string>& values,
                        std::uint32_t max_count, const char* field,
                        std::string& error) {
...
bool decode_string_list(Reader& in, std::vector<std::string>& values,
                        std::uint32_t max_count, const char* field) {
    std::uint32_t count = 0;
    if (!in.count(count, max_count, field)) return false;
    values.reserve(count);
    for (std::uint32_t i = 0; i < count; ++i) {
        std::string value;
        if (!in.string(value, WireLimits::max_name_bytes, field)) return false;
...
std::optional<CodecResult<Bytes>> encode_payload(const Message& message,
                                                  Writer& out) {
    return std::visit(
        [&](const auto& value) -> std::optional<CodecResult<Bytes>> {
            ...
            if constexpr (std::is_same_v<T, ClientHello>) {
                ...
                if (!valid_hello_limits(value.max_payload_bytes,
                ...
                if (!version_is_compatible(value.negotiated_major,
                                           value.negotiated_minor))
                    return invalid_value("TargetHello selected incompatible version");
                if (!valid_hello_limits(value.max_payload_bytes,
                                        value.max_blob_bytes,
                                        value.max_chunk_bytes))
                    return limit_error("TargetHello advertises invalid limits");
                for (const auto* field : {&value.platform, &value.abi,
                                          &value.build_type, &value.build_id}) {
                ...
                if (!valid_command_kind(value.kind) ||
                    !valid_selector_kind(value.selector_kind) ||
                ...
                const auto validation = validate_command(
                    value, value.graph_revision == 0 ? 1 : value.graph_revision);
                if (!validation)
                ...
                for (const auto& target : value.targets) {
                    if (target.id == 0 || !valid_name(target.label) ||
                ...
                if (value.selected_target_id != 0 &&
                    !target_ids.contains(value.selected_target_id))
                ...
                for (const auto& pass : value.passes) {
                    if (pass.id == 0 || !pass_ids.insert(pass.id).second ||
                        !valid_name(pass.name) || !valid_name(pass.type))
                    ...
                    std::string error;
                    if (!encode_string_list(out, pass.reads,
                                            WireLimits::max_names_per_pass,
                                            "pass reads", error) ||
                        !encode_string_list(out, pass.writes,
                                            WireLimits::max_names_per_pass,
                                            "pass writes", error) ||
                        !encode_string_list(out, pass.internal_symbols,
                                            WireLimits::max_names_per_pass,
                                            "pass symbols", error))
                ...
                std::string error;
                if (!encode_string_list(out, value.resources,
                                        WireLimits::max_resources,
                                        "resources", error))
                    return limit_error(std::move(error));
                ...
                for (const auto& group : value.alias_groups) {
                    ...
                    if (!encode_string_list(out, group.aliases,
                                            WireLimits::max_resources,
                                            "aliases", error))
                        return limit_error(std::move(error));
                }
                ...
                if (value.request_id == 0 || value.graph_revision == 0 ||
                    value.blob_id == 0 || !valid_capture_kind(value.kind) ||
                    !valid_encoding(value.encoding) ||
                    !valid_pixel_format(value.pixel_format) || value.width == 0 ||
                    value.height == 0 || value.byte_count == 0 ||
                    value.byte_count > WireLimits::max_blob_bytes ||
                    value.chunk_count == 0 ||
                ...
                if (value.kind == CaptureKind::burst) {
                    if (value.burst_count < 2 ||
                        value.burst_count > WireLimits::max_burst_frames ||
                ...
                if (value.blob_id == 0 || value.chunk_count == 0 ||
                    ...
                    value.total_bytes == 0 ||
                    value.total_bytes > WireLimits::max_blob_bytes ||
                    value.bytes.empty() ||
                    value.bytes.size() > WireLimits::max_chunk_bytes ||
...
CodecResult<Message> decode_payload(MessageType type,
    ...
    case MessageType::client_hello: {
        ClientHello value;
        ok = in.u16(value.minimum_major) && in.u16(value.minimum_minor) &&
             in.u64(value.capabilities) && in.u32(value.max_payload_bytes) &&
             in.u64(value.max_blob_bytes) && in.u32(value.max_chunk_bytes) &&
             in.string(value.authentication_token, WireLimits::max_token_bytes,
                       "authentication token");
        if (ok && (value.minimum_major == 0 ||
                   value.authentication_token.empty() ||
                   !valid_hello_limits(value.max_payload_bytes,
                                       value.max_blob_bytes,
                                       value.max_chunk_bytes)))
            return failure<Message>(CodecError::invalid_value,
                                    "ClientHello contains invalid fields");
        result = std::move(value);
        break;
    }
    case MessageType::target_hello: {
        TargetHello value;
        ok = in.u16(value.negotiated_major) &&
             in.u16(value.negotiated_minor) && in.u64(value.capabilities) &&
             in.u32(value.process_id) && in.u32(value.max_payload_bytes) &&
             in.u64(value.max_blob_bytes) && in.u32(value.max_chunk_bytes) &&
        ...
        if (ok && !version_is_compatible(value.negotiated_major,
                                         value.negotiated_minor))
            return failure<Message>(CodecError::incompatible_version,
                                    "TargetHello selected incompatible version");
        if (ok && !valid_hello_limits(value.max_payload_bytes,
                                      value.max_blob_bytes,
                                      value.max_chunk_bytes))
            return failure<Message>(CodecError::limit_exceeded,
                                    "TargetHello selected invalid limits");
        result = std::move(value);
        break;
    }
    case MessageType::command: {
        Command value;
        ok = in.u64(value.request_id) && read_enum_u8(in, value.kind) &&
             in.u64(value.target_id) && in.u64(value.graph_revision) &&
             read_enum_u8(in, value.selector_kind) && in.u64(value.pass_id) &&
             in.string(value.resource, WireLimits::max_name_bytes, "resource") &&
             in.string(value.symbol, WireLimits::max_name_bytes, "symbol") &&
             read_enum_u8(in, value.encoding) &&
             in.u32(value.max_preview_millifps) &&
             in.u32(value.max_preview_long_edge) && in.u16(value.burst_frames);
        if (ok && (!valid_command_kind(value.kind) ||
                   !valid_selector_kind(value.selector_kind) ||
                   !valid_encoding(value.encoding)))
            return failure<Message>(CodecError::invalid_value,
                                    "Command contains unknown enum value");
        if (ok) {
            const auto validation = validate_command(
                value, value.graph_revision == 0 ? 1 : value.graph_revision);
            if (!validation)
                return failure<Message>(CodecError::invalid_value,
                                        "Command is invalid: " + validation.detail);
        }
        result = std::move(value);
        break;
    }
    case MessageType::topology_snapshot: {
        ...
        for (std::uint32_t i = 0; i < target_count && ok; ++i) {
            WireTarget target;
            ok = in.u64(target.id) &&
                 in.string(target.label, WireLimits::max_name_bytes, "target label") &&
                 in.boolean(target.renderable);
            if (ok && (target.id == 0 || !valid_name(target.label) ||
                       !target_ids.insert(target.id).second))
                return failure<Message>(CodecError::invalid_value,
                                        "TopologySnapshot target identity is invalid");
            if (ok) value.targets.push_back(std::move(target));
        }
        ...
        for (std::uint32_t i = 0; i < pass_count && ok; ++i) {
            ...
            ok = in.u64(pass.id) && in.u32(pass.authored_index) &&
                 ...
                 in.string(pass.type, WireLimits::max_name_bytes, "pass type") &&
                 in.boolean(pass.enabled) && in.boolean(pass.passthrough) &&
                 decode_string_list(in, pass.reads, WireLimits::max_names_per_pass,
                                    "pass reads") &&
                 decode_string_list(in, pass.writes, WireLimits::max_names_per_pass,
                                    "pass writes") &&
                 decode_string_list(in, pass.internal_symbols,
                                    WireLimits::max_names_per_pass, "pass symbols");
            if (ok && (pass.id == 0 || !valid_name(pass.name) ||
                       !valid_name(pass.type) || !pass_ids.insert(pass.id).second))
                return failure<Message>(CodecError::invalid_value,
                                        "TopologySnapshot pass identity is invalid");
            if (ok) value.passes.push_back(std::move(pass));
        }
        ok = ok && in.count(schedule_count, WireLimits::max_schedule_entries,
                            "schedule");
        ...
        for (std::uint32_t i = 0; i < schedule_count && ok; ++i) {
            ...
            if (ok && !pass_ids.contains(pass_id))
                return failure<Message>(CodecError::invalid_value,
                                        "Schedule references missing pass");
            if (ok) value.schedule.push_back(pass_id);
        }
        ok = ok && decode_string_list(in, value.resources,
                                      WireLimits::max_resources, "resources") &&
        ...
        for (std::uint32_t i = 0; i < alias_count && ok; ++i) {
            ...
            ok = in.string(group.canonical_resource, WireLimits::max_name_bytes,
                           "canonical resource") &&
                 decode_string_list(in, group.aliases, WireLimits::max_resources,
                                    "aliases");
            if (ok && !valid_name(group.canonical_resource))
                return failure<Message>(CodecError::invalid_value,
                                        "Alias canonical resource is invalid");
            if (ok) value.alias_groups.push_back(std::move(group));
        }
        ok = ok && in.string(value.render_stats, WireLimits::max_detail_bytes,
                             "render stats");
        if (ok && (value.graph_revision == 0 ||
                   (value.selected_target_id != 0 &&
                    !target_ids.contains(value.selected_target_id))))
            return failure<Message>(CodecError::invalid_value,
    ...
    case MessageType::status: {
        Status value;
        ok = in.u64(value.request_id) && in.u64(value.graph_revision) &&
             read_enum_u8(in, value.state) && read_enum_u16(in, value.code) &&
             in.u32(value.queue_depth) && in.u64(value.completed_captures) &&
        ...
        if (ok && (!valid_session_state(value.state) ||
                   !valid_status_code(value.code)))
            return failure<Message>(CodecError::invalid_value,
                                    "Status contains unknown enum value");
        result = std::move(value);
        break;
    }
    case MessageType::capture_metadata: {
        ...
        ok = in.u64(value.request_id) && in.u64(value.graph_revision) &&
             in.u64(value.blob_id) && in.i64(value.frame_number) &&
             read_enum_u8(in, value.kind) && read_enum_u8(in, value.encoding) &&
             read_enum_u16(in, value.pixel_format) && in.u32(value.width) &&
             in.u32(value.height) && in.boolean(value.is_depth) &&
             in.boolean(value.exact) && in.u64(value.byte_count) &&
             in.u32(value.chunk_count) && in.u16(value.burst_index) &&
             in.u16(value.burst_count);
        if (ok) {
    ...
    case MessageType::blob_chunk: {
        BlobChunk value;
        ok = in.u64(value.blob_id) && in.u32(value.chunk_index) &&
             in.u32(value.chunk_count) && in.u64(value.offset) &&
             in.u64(value.total_bytes) &&
    ...
    case MessageType::drop: {
        DropEvent value;
        ok = read_enum_u8(in, value.kind) && in.u64(value.dropped_items) &&
             in.i64(value.after_frame_number) && in.u64(value.after_sequence);
        if (ok && (!valid_drop_kind(value.kind) || value.dropped_items == 0))
            return failure<Message>(CodecError::invalid_value,
                                    "DropEvent kind or count is invalid");
        result = value;
        break;
    }
    case MessageType::error: {
        ErrorEvent value;
        ok = in.u32(value.code) && in.u64(value.related_request_id) &&
             in.u64(value.graph_revision) &&
             in.string(value.detail, WireLimits::max_detail_bytes, "error detail");
        if (ok && (value.code == 0 || value.detail.empty()))
            return failure<Message>(CodecError::invalid_value,
                                    "ErrorEvent code or detail is invalid");
        result = std::move(value);
        break;
    }
    }
    ...
    if (!ok) {
        const CodecError error = in.error() == CodecError::none
                                     ? CodecError::truncated
                                     : in.error();
        const std::string detail = in.detail().empty()
                                       ? "message payload is truncated or invalid"
                                       : in.detail();
...
CodecResult<Bytes> encode_message(const Message& message,
                                  std::uint64_t sequence,
                                  std::uint64_t session_id,
                                  std::uint16_t flags) {
    Writer payload;
    if (auto error = encode_payload(message, payload)) return std::move(*error);
...
CodecResult<Envelope> decode_envelope(std::span<const std::uint8_t> bytes) {
    if (bytes.size() < envelope_size)
        return failure<Envelope>(CodecError::truncated,
                                 "framegraph envelope is truncated");
    Reader in(bytes.first(envelope_size));
    std::uint32_t magic = 0;
    std::uint16_t raw_type = 0;
    Envelope envelope;
    if (!in.u32(magic) || !in.u16(envelope.major) ||
        !in.u16(envelope.minor) || !in.u16(raw_type) ||
        !in.u16(envelope.flags) || !in.u32(envelope.payload_length) ||
        !in.u64(envelope.sequence) || !in.u64(envelope.session_id))
        return failure<Envelope>(CodecError::truncated,
                                 "framegraph envelope is truncated");
    if (magic != wire_magic)
        return failure<Envelope>(CodecError::bad_magic,
                                 "framegraph envelope has bad magic");
    if (!version_is_compatible(envelope.major, envelope.minor))
        return failure<Envelope>(CodecError::incompatible_version,
                                 "framegraph protocol major is incompatible");
    if (!known_message_type(raw_type))
        return failure<Envelope>(CodecError::unknown_message_type,
                                 "framegraph message type is unknown");
    if (envelope.payload_length > WireLimits::max_payload_bytes)
        return failure<Envelope>(CodecError::limit_exceeded,
                                 "framegraph payload exceeds hard limit");
...
CodecResult<DecodedMessage> decode_message(std::span<const std::uint8_t> bytes) {
    ...
    if (!envelope_result)
        return {.value = std::nullopt,
                .error = envelope_result.error,
                .detail = std::move(envelope_result.detail)};
    ...
    if (bytes.size() != expected)
        return failure<DecodedMessage>(CodecError::invalid_length,
                                       "framegraph envelope length mismatch");
    auto payload = decode_payload(envelope.type, bytes.subspan(envelope_size));
    if (!payload)
        return {.value = std::nullopt,
                .error = payload.error,
                .detail = std::move(payload.detail)};
    return {.value = DecodedMessage{envelope, std::move(*payload.value)},
            .error = CodecError::none,
            .detail = {}};
}

SelectorValidation validate_command(const Command& command,
                                    std::uint64_t current_graph_revision) {
    if (command.request_id == 0)
        return selector_failure(SelectorError::target_required,
                                "request_id must be non-zero");
    if (!valid_command_kind(command.kind) ||
        !valid_selector_kind(command.selector_kind) ||
        !valid_encoding(command.encoding))
        return selector_failure(SelectorError::target_required,
                                "command contains an unknown enum value");
    ...
    if (topology_bound && command.graph_revision == 0)
        return selector_failure(SelectorError::revision_required,
                                "topology-bound command requires graph_revision");
    if (topology_bound && command.graph_revision != current_graph_revision)
        return selector_failure(SelectorError::stale_revision,
                                "command graph_revision is stale");
    if (topology_bound && command.target_id == 0)
        return selector_failure(SelectorError::target_required,
                                "topology-bound command requires target_id");

    const bool captures = command.kind == CommandKind::capture_snapshot ||
                          command.kind == CommandKind::start_stream ||
                          command.kind == CommandKind::update_stream ||
                          command.kind == CommandKind::capture_burst;
    if (captures && command.selector_kind == CaptureSelectorKind::resource &&
        !valid_name(command.resource))
        return selector_failure(SelectorError::resource_required,
                                "resource capture requires resource name");
    if (captures &&
        command.selector_kind == CaptureSelectorKind::internal_symbol) {
        if (command.pass_id == 0)
            return selector_failure(SelectorError::pass_required,
                                    "internal capture requires pass_id");
        if (!valid_name(command.symbol))
            return selector_failure(SelectorError::symbol_required,
                                    "internal capture requires symbol name");
    }
    const bool stream_options = command.kind == CommandKind::start_stream ||
                                command.kind == CommandKind::update_stream;
    if (stream_options &&
        (command.max_preview_millifps == 0 ||
         command.max_preview_millifps > WireLimits::max_preview_millifps ||
         command.max_preview_long_edge == 0 ||
         command.max_preview_long_edge > WireLimits::max_preview_long_edge))
        return selector_failure(SelectorError::invalid_stream_options,
                                "stream FPS or long-edge limit is invalid");
    if (command.kind == CommandKind::capture_burst &&
        (command.burst_frames < 2 ||
         command.burst_frames > WireLimits::max_burst_frames))
        return selector_failure(SelectorError::invalid_burst_options,
                                "burst frame count must be in 2..16");
    return {};
}
...
BlobAssemblyResult BlobAssembler::append(const BlobChunk& chunk) {
    ...
    if (complete_)
        return reject(BlobAssemblyError::already_complete,
                      "chunk received after blob completion");
    ...
    if (chunk.chunk_count != metadata_.chunk_count)
        return reject(BlobAssemblyError::wrong_chunk_count,
                      "chunk_count differs from capture metadata");
    if (chunk.chunk_index != next_chunk_)
        return reject(BlobAssemblyError::out_of_order_chunk,
                      "chunk index is not the next expected index");
    if (chunk.offset != next_offset_)
        return reject(BlobAssemblyError::wrong_offset,
    ...
    if (chunk.total_bytes != metadata_.byte_count || chunk.bytes.empty() ||
        ...
        return reject(BlobAssemblyError::size_mismatch,
                      "chunk size or total differs from capture metadata");
    ...
    if (next_chunk_ == metadata_.chunk_count) {
        if (next_offset_ != metadata_.byte_count)
            return reject(BlobAssemblyError::size_mismatch,
                          "final chunk does not complete declared blob size");
        complete_ = true;
    } else if (next_offset_ >= metadata_.byte_count) {
        return reject(BlobAssemblyError::size_mismatch,
                      "blob bytes completed before final chunk");
    }
    return {.error = BlobAssemblyError::none,
            .detail = {},
            .complete = complete_};
}
