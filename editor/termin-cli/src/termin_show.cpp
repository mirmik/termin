...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<std::string> command =
            termin_cli::python_backend::python_module_command("termin.model_viewer");
        std::vector<std::string> rest = tail_args(argc, argv);
        command.insert(command.end(), rest.begin(), rest.end());
        return termin_cli::python_backend::run_process(command, "model viewer backend");
    } catch (const std::exception& exc) {
        std::cerr << "termin_show: " << exc.what() << "\n";
        return 2;
    }
}
