    ...
    *,
    ...
    if context.termin_root is not None:
        android_script = android_script or (
            context.termin_root / "scripts" / "build" / _platform_script("android-apk")
        ).resolve()
        quest_script = quest_script or (
            context.termin_root
            / "scripts"
            / "build"
            / _platform_script("quest-openxr-apk")
        ).resolve()
...
def _find_termin_root(search_roots: Sequence[Path]) -> Path | None:
    for start in search_roots:
        for candidate in (start, *start.parents):
            if (candidate / "scripts/build/android-apk.sh").is_file() or (
                candidate / "scripts/build/android-apk.ps1"
            ).is_file():
