...
ANDROID_BUILD_SCRIPT = (
    ...
    if os.name == "nt"
    else "scripts/build/android-apk.sh"
)
QUEST_OPENXR_BUILD_SCRIPT = (
    "scripts/build/quest-openxr-apk.ps1"
    if os.name == "nt"
    else "scripts/build/quest-openxr-apk.sh"
)
...
def _resolve_required_termin_root(
    ...
    if build_script is not None:
        if termin_root is None:
            script_path = Path(build_script).expanduser().resolve()
            if (
                script_path.parent.name == "build"
    ...
    if not android_sdk_root.exists():
        diagnostics.append(
            build_error(
                ...
                "Build and install it first: task build:android -- "
    ...
    if not sdk_lib_dir.is_dir():
        diagnostics.append(
            build_error(
                ...
                "Rebuild and reinstall the Android SDK: task build:android -- "
                f"--abi {abi} --platform {platform}.",
            )
        )
        return
    ...
    if not capabilities.quest_openxr.supports_openxr(abi):
        diagnostics.append(
            build_error(
                ...
                f"task build:android -- --abi {abi} --platform {platform}.",
            )
        )
