...
def test_resolve_termin_root_requires_target_specific_marker(tmp_path: Path) -> None:
    ...
    termin_root.mkdir()
    marker = termin_root / "scripts/build/android-apk.sh"
    marker.parent.mkdir(parents=True)
    marker.write_text("# android marker\n", encoding="utf-8")

    assert resolve_termin_root(
        termin_root,
        marker_script_name="scripts/build/android-apk.sh",
        target_name="Android",
    ) == termin_root.resolve()

    with pytest.raises(FileNotFoundError, match="scripts/build/quest-openxr-apk.sh"):
        resolve_termin_root(
            termin_root,
            marker_script_name="scripts/build/quest-openxr-apk.sh",
...
def test_resolve_termin_root_uses_environment_candidate_first(tmp_path: Path, monkeypatch) -> None:
    termin_root = tmp_path / "termin-root"
    termin_root.mkdir()
    marker = termin_root / "scripts/build/quest-openxr-apk.sh"
    marker.parent.mkdir(parents=True)
    marker.write_text("# quest marker\n", encoding="utf-8")
