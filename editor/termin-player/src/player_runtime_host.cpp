...
namespace termin::player {
    ...
    namespace {
        ...
        required_backend_list(const nos::trent& owner, const char* field, const std::string& context) {
            ...
            for (const nos::trent& item : values->as_list()) {
                if (!item.is_string() || item.as_string().empty()) {
                    throw std::runtime_error(context + "[" + std::to_string(index) + "] must be a non-empty string");
                }
            ...
            for (int i = 1; i < argc; ++i) {
                ...
                if (arg.rfind(exit_after_frames_prefix, 0) == 0) {
                    options.exit_after_frames =
                        parse_int_arg(arg.substr(exit_after_frames_prefix.size()), "--exit-after-frames");
                    continue;
                }
        ...
        int positive_window_int_field(const nos::trent& t, const char* key, int def, const char* context) {
            const nos::trent* value = dict_get(t, key);
            if (!value) {
                return def;
            }
        ...
        bool window_bool_field(const nos::trent& t, const char* key, bool def, const char* context) {
            const nos::trent* value = dict_get(t, key);
            if (!value) {
                return def;
            }
            if (!value->is_bool()) {
                tc_log_warn("termin_player: %s.%s must be a boolean, using %s", context, key, def ? "true" : "false");
                return def;
            }
            return value->as_bool();
        }
        ...
        PlayerWindowSettings player_window_settings_from_manifest(const nos::trent& root) {
            ...
            if (!runtime->is_dict()) {
                tc_log_warn(
                    "termin_player: app manifest runtime field must be an object; using default window settings");
                return settings;
            }
            ...
            if (!window->is_dict()) {
                tc_log_warn("termin_player: app manifest runtime.window field must be an object; using default window "
                            "settings");
                return settings;
            }
        ...
        AppManifest load_app_manifest(const fs::path& app_manifest_path) {
            ...
            manifest.package_manifest_path =
                relative_to_root(manifest.bundle_root, string_field(*package, "manifest", "package/manifest.json"));
            const nos::trent* runtime = dict_get(root, "runtime");
            ...
            if (mcp != nullptr) {
                if (!mcp->is_dict()) {
                    tc_log_warn("termin_player: app manifest runtime.mcp must be an object; "
            ...
            if (modules && modules->is_dict()) {
                ...
                manifest.project_modules_root =
                    relative_to_root(manifest.bundle_root, string_field(*modules, "root", "package/modules"));
                manifest.project_python_root = manifest.project_modules_root / "python";
                manifest.module_manifest_path = relative_to_root(
                    manifest.bundle_root, string_field(*modules, "manifest", "package/modules/modules.json"));
            }
            return manifest;
        }
        ...
        PyMethodDef native_methods[] = {
            ...
            {"window_framebuffer_size",
             native_window_framebuffer_size,
             METH_NOARGS,
             "Return the live window framebuffer size."},
            {"window_should_close", native_window_should_close, METH_NOARGS, "Return the live window close flag."},
            {"set_window_should_close",
             native_set_window_should_close,
             METH_VARARGS,
             "Set the live window close flag."},
        ...
        void ensure_native_module_registered() {
            ...
            if (PyImport_AppendInittab("_termin_player_native", init_native_module) != 0) {
                throw std::runtime_error("failed to register builtin module _termin_player_native");
            }
            registered = true;
        }
        ...
        void bootstrap_python_player_runtime() {
            ...
            if (bootstrap_player == nullptr || !PyCallable_Check(bootstrap_player)) {
                ...
                Py_DECREF(module);
                throw std::runtime_error("termin.bootstrap.bootstrap_player is unavailable");
            }
            ...
            if (result == nullptr) {
                PyErr_Print();
                throw std::runtime_error("termin.bootstrap.bootstrap_player failed");
            }
            Py_DECREF(result);
        }
    ...
    struct PlayerRuntimeHost::Impl {
        ...
        int run(int argc, char** argv) {
            try {
                ...
                if (argc == 2 && std::strcmp(argv[1], "--termin-python-layout-smoke") == 0) {
                ...
                remote_profiler =
                    profiler_remote::start_desktop_target_from_environment("termin_player", native_build_type());
                if (remote_profiler) {
                    remote_profiler_connection = engine->attach_frame_completion_callback(
...
print(json.dumps({
    ...
    "gil_enabled": sys._is_gil_enabled(),
            ...
            for (int cycle = 0; cycle < 2; ++cycle) {
                ...
                if (script_result != 0) {
                    PyErr_Print();
                    tc_log_error("termin_player: Python layout smoke script failed on cycle %d", cycle);
                }
                const int finalize_result = termin::python_host::finalize();
                python_initialized = false;
                if (finalize_result != 0) {
                    tc_log_error("termin_player: Python finalization failed on smoke cycle %d", cycle);
                }
        ...
        void configure_backend_attempts() {
            ...
            if (override_source != nullptr) {
                if (!vector_contains(manifest.runtime_backends, explicit_backend)) {
                    throw std::runtime_error(std::string(override_source) + " selects backend '" + explicit_backend +
                ...
                tc_log_info("termin_player: explicit %s override selects packaged backend '%s' without fallback",
                            override_source,
            ...
            tc_log_info("termin_player: packaged backend initialization order: %s",
        ...
        void initialize_python() {
            ...
            std::string code = "import sys\n"
                               ...
                               "\n"
                               "project_python = " +
                               python_string_literal(project_python) +
                               "\n"
        ...
        void process_automation_pending() {
            ...
            PyGILState_STATE gil = PyGILState_Ensure();
            PyObject* result = PyObject_CallMethod(automation_session, "process_pending", nullptr);
            if (result == nullptr) {
                PyErr_Print();
            ...
            if (!modules_configured) {
                ...
                modules_runtime.set_event_callback([](const termin_modules::ModuleEvent& event) {
                    if (event.kind == termin_modules::ModuleEventKind::Failed) {
                        tc_log_error(
                            "termin_player: module %s failed: %s", event.module_id.c_str(), event.message.c_str());
                        return;
                    }
                    tc_log_info(
                        "termin_player: module %s %s", event.module_id.c_str(), module_event_name(event.kind).c_str());
                });
        ...
        void initialize_window_and_rendering() {
            ...
            const tgfx::PresentationMode presentation_mode =
                manifest.window.vsync ? tgfx::PresentationMode::VSync : tgfx::PresentationMode::Immediate;

            for (const std::string& backend : backend_attempts) {
                if (!tgfx::backend_is_compiled(tgfx::backend_from_name(backend))) {
                    tc_log_error("termin_player: packaged backend '%s' is not compiled into this player",
                                 backend.c_str());
                    continue;
                }
                ...
                try {
                    std::unique_ptr<WindowedGraphicsSession> candidate_session = create_native_windowed_graphics();
                    ...
                    tc_log_error(
                        "termin_player: backend '%s' initialization failed: %s", backend.c_str(), error.what());
                }
            }
            ...
            engine->rendering_manager.render_engine()->set_graphics_host(graphics_session->graphics());
            if (fullscreen) {
            ...
            tc_display_handle handle =
                create_offscreen_display(&graphics_session->graphics().device(), width, height, "Main");
        ...
        void setup_input(RenderingManager& manager) {
            ...
            for (tc_viewport_handle viewport : viewports) {
                ...
                if (mode != "simple" && mode != "basic") {
                    tc_log_warn("termin_player: unknown viewport input mode '%s' for viewport '%s'",
                                mode.c_str(),
                ...
                if (input == nullptr) {
                    tc_log_error("termin_player: failed to create input manager for viewport '%s'",
                                 viewport_name ? viewport_name : "");
                    continue;
                }
        ...
        void clear_input() {
            if (window) {
                if (display && display->is_valid()) {
                    ...
                    tc_display_set_input_platform_services(display->handle(), nullptr);
                }
        ...
        void disable_unrenderable_unused_render_targets(RenderingManager& manager) {
            ...
            for (tc_render_target_handle rt : manager.managed_render_targets()) {
                ...
                bool used_by_viewport = std::any_of(
                    viewport_targets.begin(), viewport_targets.end(), [rt](tc_render_target_handle candidate) {
                ...
                tc_log_warn("termin_player: disabled unused render target '%s' because it has no camera or pipeline",
                            name ? name : "");
            }
        }
    ...
    namespace {
        ...
        PyObject* native_engine_capsule(PyObject*, PyObject*) {
            ...
            return PyCapsule_New(g_active_host->engine.get(), "termin.EngineCore.borrowed", nullptr);
        }
        ...
        PyObject* native_display_handle(PyObject*, PyObject*) {
            std::lock_guard<std::mutex> lock(g_active_host_mutex);
            if (g_active_host == nullptr || !g_active_host->display || !g_active_host->display->is_valid()) {
