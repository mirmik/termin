...
namespace {
    ...
    public:
        ...
        void upload_buffer(tgfx::BufferHandle, std::span<const std::uint8_t>, std::uint64_t = 0) override {}
        void upload_texture(tgfx::TextureHandle, std::span<const std::uint8_t> bytes, std::uint32_t = 0) override {
        ...
        void read_buffer(tgfx::BufferHandle, std::span<std::uint8_t>, std::uint64_t = 0) override {}
...
TEST_CASE("RemoteFrameGraphDebuggerSource reconciles revisions and retains stale topology") {
    ...
    termin::RemoteFrameGraphDebuggerSource source(4, [&commands](const Command& command) {
    ...
    topology.passes.push_back(
        {201, 3, "Remote Pass", "Probe", true, false, {"input"}, {"output"}, {"before", "after"}});
    ...
    CHECK(snapshot->gaps.back().kind == termin::FrameGraphDebuggerGapKind::Disconnect);
...
TEST_CASE("RemoteFrameGraphDebuggerSource assembles exact color HDR and depth blobs") {
    using namespace termin::framegraph_remote;
    std::vector<Command> commands;
    termin::RemoteFrameGraphDebuggerSource source(8, [&commands](const Command& command) {
        commands.push_back(command);
        return true;
    });
    ...
    target_hello.capabilities =
        static_cast<std::uint64_t>(Capability::topology) | static_cast<std::uint64_t>(Capability::exact_snapshot);
    ...
    const auto assemble = [&](PixelFormat format,
        ...
        for (std::uint32_t index = 0; index < 2; ++index) {
            ...
            DecodedMessage chunk_message{{}, Message{std::in_place_type<BlobChunk>, std::move(chunk)}};
    ...
    assemble(PixelFormat::rgba8_unorm, false, 2, 1, {255, 0, 0, 255, 0, 255, 0, 255});
    ...
    CHECK(source.analyze_hdr().find("HDR pixels:</b> 1") != std::string::npos);
    std::vector<float> depth_values = {0.25F, 0.75F};
    std::vector<std::uint8_t> depth_bytes(sizeof(float) * depth_values.size());
    ...
    int depth_width = 0;
    int depth_height = 0;
    const auto normalized = source.read_depth_normalized(device, &depth_width, &depth_height);
    ...
    std::memcpy(depth_bytes.data(), depth_values.data(), depth_bytes.size());
    assemble(PixelFormat::depth32_float, true, 2, 1, depth_bytes);
    const auto repeated = source.read_depth_normalized(device, &depth_width, &depth_height);
...
TEST_CASE("RemoteFrameGraphDebuggerSource tracks live preview and burst gaps") {
    ...
    hello.capabilities =
        static_cast<std::uint64_t>(Capability::topology) | static_cast<std::uint64_t>(Capability::exact_snapshot) |
        static_cast<std::uint64_t>(Capability::live_preview) | static_cast<std::uint64_t>(Capability::burst_capture);
    ...
    REQUIRE(ingest(
        Status{exact_request, 8, SessionState::idle, StatusCode::cancelled, 0, 0, 0, 0, "superseded by live preview"}));
    ...
    REQUIRE(ingest(
        Status{stream_request, 8, SessionState::streaming, StatusCode::accepted, 0, 0, 0, 0, "live preview started"}));
    ...
    const auto frame = [&](std::uint64_t blob_id,
        ...
        metadata.kind = kind;
        metadata.encoding = kind == CaptureKind::preview ? CaptureEncoding::rgba8 : CaptureEncoding::native_pixels;
    ...
    REQUIRE(ingest(
        Status{stream_request, 8, SessionState::idle, StatusCode::completed, 0, 2, 0, 0, "live preview stopped"}));
    REQUIRE(ingest(Status{stop_request, 8, SessionState::idle, StatusCode::completed, 0, 2, 0, 0, "stop processed"}));
    ...
    REQUIRE(ingest(
        Status{burst_request, 8, SessionState::waiting_capture, StatusCode::accepted, 0, 2, 0, 0, "burst accepted"}));
    frame(201, 20, CaptureKind::burst, 0, 3, burst_request);
    frame(203, 22, CaptureKind::burst, 2, 3, burst_request);
    REQUIRE(ingest(Status{burst_request, 8, SessionState::idle, StatusCode::completed, 0, 4, 1, 0, "burst completed"}));
    REQUIRE(source.snapshot()->cpu_capture.has_value());
...
TEST_CASE("FrameGraphDebuggerView switches local remote stale and local in one tree") {
    ...
    termin::framegraph_remote_target::RemoteFrameGraphTarget target_service(*fixture.debugger, target_config);
    REQUIRE(target_service.start());

    DocumentGuard document;
    termin::FrameGraphDebuggerView view(termin::gui_native::TcDocument(document.handle), *fixture.debugger);
    REQUIRE(view.activate());
    const tc_widget_handle root = view.root_handle();
    REQUIRE(view.connect_remote(target_service.status().listening_port, "wrong-session-token"));
    bool auth_failure_visible = false;
    for (int attempt = 0; attempt < 200; ++attempt) {
        view.update();
        const auto remote = view.source_snapshot();
        if (!remote->connected && remote->status_detail.find("authentication token is invalid") != std::string::npos) {
    ...
    REQUIRE(view.connect_remote(target_service.status().listening_port, "view-session-token"));
    ...
    for (int attempt = 0; attempt < 400; ++attempt) {
        ...
        view.update();
        const auto remote = view.source_snapshot();
        if (remote->connected && !remote->stale && !remote->targets.empty() && !remote->passes.empty()) {
    ...
    CHECK(view.source_snapshot()->source_kind == termin::FrameGraphDebuggerSourceKind::Remote);
    ...
    CHECK(view.source_snapshot()->source_kind == termin::FrameGraphDebuggerSourceKind::Local);
    CHECK_EQ(tc_ui_document_root_count(document.handle), 1u);
    CHECK(tc_widget_handle_eq(view.root_handle(), root));
