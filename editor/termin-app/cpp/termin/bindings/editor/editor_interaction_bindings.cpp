...
namespace termin {
    ...
    void bind_editor_interaction(nb::module_& m) {
        ...
        nb::class_<SelectionManager>(m, "SelectionManager")
            ...
            .def(
                "select",
                [](SelectionManager& s, nb::object obj) { s.select(obj.is_none() ? Entity() : nb::cast<Entity>(obj)); },
                nb::arg("entity").none())
            .def(
                "hover",
                [](SelectionManager& s, nb::object obj) { s.hover(obj.is_none() ? Entity() : nb::cast<Entity>(obj)); },
                nb::arg("entity").none())
            ...
            .def_prop_rw(
                ...
                [](SelectionManager& s, std::function<void(Entity)> cb) { s.on_selection_changed = cb; })
            .def_prop_rw(
                "on_hover_changed",
                [](SelectionManager& s) { return s.on_hover_changed; },
                [](SelectionManager& s, std::function<void(Entity)> cb) { s.on_hover_changed = cb; });

        // EditorViewportInputManager (per-viewport)
        nb::class_<EditorViewportInputManager>(m, "EditorViewportInputManager")
            .def(
                "__init__",
                [](EditorViewportInputManager* self,
                   uint32_t vp_index,
                   uint32_t vp_generation,
                   uint32_t display_index,
                   uint32_t display_generation) {
                    tc_viewport_handle vh;
                    vh.index = vp_index;
                    vh.generation = vp_generation;
                    new (self) EditorViewportInputManager(vh, tc_display_handle{display_index, display_generation});
                },
                nb::arg("vp_index"),
                nb::arg("vp_generation"),
                nb::arg("display_index"),
                nb::arg("display_generation"))
            .def("tc_input_manager_ptr",
                 [](EditorViewportInputManager& s) { return reinterpret_cast<uintptr_t>(s.tc_input_manager_ptr()); })
            .def(
                "rebind",
                [](EditorViewportInputManager& self,
                   uint32_t vp_index,
                   ...
                   uint32_t display_index,
                   uint32_t display_generation) {
                    return self.rebind(tc_viewport_handle{vp_index, vp_generation},
        ...
        nb::class_<EditorInteractionSystem>(m, "EditorInteractionSystem")
            .def(nb::init<>())
            .def_static("instance", &EditorInteractionSystem::instance, nb::rv_policy::reference)
            .def_static(
                "set_instance",
                [](nb::object obj) {
                    EditorInteractionSystem::set_instance(obj.is_none() ? nullptr
                                                                        : nb::cast<EditorInteractionSystem*>(obj));
                },
            ...
            .def_prop_rw(
                ...
                [](const EditorInteractionSystem& s) { return s.camera_frustums_visible(); },
                [](EditorInteractionSystem& s, bool visible) { s.set_camera_frustums_visible(visible); })
            .def("set_camera_frustums_visible",
                 &EditorInteractionSystem::set_camera_frustums_visible,
                 nb::arg("visible"))
            .def(
                "set_camera_frustum_render_context",
            ...
            .def(
                "set_gizmo_target",
                [](EditorInteractionSystem& s, nb::object obj) {
            ...
            .def("pick_entity_at",
                 [](EditorInteractionSystem& s,
                    ...
                    uint32_t display_generation) {
                     ...
                     return s.pick_entity_at(Vec2f{x, y}, vp, tc_display_handle{display_index, display_generation});
                 })
            .def("pick_surface_at",
                 [](EditorInteractionSystem& s,
                    float x,
                    ...
                    uint32_t display_generation) {
                     ...
                     vp.index = vp_index;
                     vp.generation = vp_generation;
                     return s.pick_surface_at(Vec2f{x, y}, vp, tc_display_handle{display_index, display_generation});
                 })
            .def_prop_rw(
                "on_request_update",
                [](EditorInteractionSystem& s) { return s.on_request_update; },
                [](EditorInteractionSystem& s, std::function<void()> cb) { s.on_request_update = cb; })
            .def_prop_rw(
                "on_transform_end",
                [](EditorInteractionSystem& s) { return s.on_transform_end; },
                [](EditorInteractionSystem& s, std::function<void(const GeneralPose3&, const GeneralPose3&)> cb) {
                    s.on_transform_end = cb;
                })
            .def_prop_rw(
                "on_key",
                [](EditorInteractionSystem& s) { return s.on_key; },
                [](EditorInteractionSystem& s, std::function<void(const KeyEvent&)> cb) { s.on_key = cb; })
            .def_prop_rw(
                "on_entity_click",
