...
namespace termin {
    ...
    public:
        ...
        TransformGizmo* transform_gizmo() {
            return &_transform_gizmo;
        }
        void set_gizmo_target(Entity entity);
        void set_camera_frustums_visible(bool visible);
        bool camera_frustums_visible() const {
            return _camera_frustums_visible;
        }
        void set_camera_frustum_render_context(tc_scene_handle scene, int width, int height);
        tc_scene_handle camera_frustum_scene() const {
            return _camera_frustum_scene;
        }
        double camera_frustum_aspect_override() const;

        // Picking (reads from ID buffer)
        Entity pick_entity_at(Vec2f screen, tc_viewport_handle viewport, tc_display_handle display);
        SurfacePickResult pick_surface_at(Vec2f screen, tc_viewport_handle viewport, tc_display_handle display);
