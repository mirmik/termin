...
namespace termin {
    ...
    namespace {
        ...
        FrameGraphDebuggerImageSnapshot project_image(const FrameGraphCapture& capture, std::uint64_t generation) {
            FrameGraphDebuggerImageSnapshot result;
            result.available =
                capture.has_capture() && capture.capture_tex() && capture.width() > 0 && capture.height() > 0;
            if (!result.available)
                return result;
        ...
        public:
            ...
            ~LocalFrameGraphDebuggerSource() override {
                close();
            }
            ...
            bool select_target(std::uint64_t target_id) override {
                ...
                if (!index || *index >= debugger_->targets().size()) {
                    tc_log_error("[framegraph-debugger-source] invalid local target id %llu",
            ...
            bool select_pass(std::optional<std::uint64_t> pass_id) override {
                ...
                if (!index) {
                    tc_log_error("[framegraph-debugger-source] invalid local pass id %llu",
                                 static_cast<unsigned long long>(*pass_id));
                    return false;
                }
                const auto found = std::find_if(
                    passes.begin(), passes.end(), [index](const auto& pass) { return pass.index == *index; });
                if (found == passes.end()) {
                    tc_log_error("[framegraph-debugger-source] missing local pass id %llu",
                                 static_cast<unsigned long long>(*pass_id));
                    return false;
                }
            ...
            bool capture_burst(std::uint16_t) override {
                return false;
            }
            ...
            bool render_image(tgfx::RenderContext2& context,
                ...
                if (!debugger_ || !target || width == 0 || height == 0)
                    return false;
                const FrameGraphCapture& capture =
                    kind == FrameGraphDebuggerImageKind::Depth ? debugger_->depth_capture() : debugger_->capture();
            ...
            read_depth_normalized(tgfx::IRenderDevice& device, int* width, int* height) override {
                if (!debugger_)
                    return {};
                const FrameGraphCapture& capture = debugger_->depth_capture();
                if (!capture.has_capture() || !capture.capture_tex())
                    return {};
                return debugger_->presenter().read_depth_normalized(&device, capture.capture_tex(), width, height);
            }
        ...
        private:
            void rebuild_snapshot() {
                ...
                for (const auto& pass : passes) {
                    next->passes.push_back({local_id(pass.index),
                                            pass.index,
                                            pass.name,
                                            pass.type,
                                            pass.enabled,
                ...
                next->main_image = project_image(debugger_->capture(), image_generation_);
                next->depth_image = project_image(debugger_->depth_capture(), image_generation_);
                snapshot_ = std::move(next);
            }
    ...
    std::shared_ptr<IFrameGraphDebuggerSource> make_local_frame_graph_debugger_source(FrameGraphDebugger& debugger) {
