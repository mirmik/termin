...
if [[ "$TERMIN_ENABLE_VULKAN" == "ON" ]]; then
    TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS="vulkan"
fi
if [[ "$TERMIN_ENABLE_OPENGL" == "ON" ]]; then
    TERMIN_BUILD_BUILTIN_SHADER_ARTIFACTS=ON
    if [[ "$OSTYPE" == msys* || "$OSTYPE" == cygwin* ]]; then
        TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS="${TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS:+${TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS};}d3d11;opengl;opengl330"
    else
        TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS="${TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS:+${TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS};}opengl;opengl330"
    fi
elif [[ -n "$TERMIN_BUILTIN_SHADER_ARTIFACT_TARGETS" ]]; then
    TERMIN_BUILD_BUILTIN_SHADER_ARTIFACTS=ON
else
...
cmake -S "$SCRIPT_DIR" -B "$BUILD_DIR" "${cmake_args[@]}" \
    ...
    -DTERMIN_PYTHON_ABI="${TERMIN_PYTHON_ABI:-cp314t}" \
    -DTERMIN_BUILD_PYTHON=ON \
    -DTERMIN_RELOCATABLE_PYTHON_WHEELS="${TERMIN_RELOCATABLE_PYTHON_WHEELS:-OFF}" \
