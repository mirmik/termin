...
PLATFORM_DIR="$SCRIPT_DIR/platform/termin-android/platform"
...
ANDROID_ASSETS_DIR_VALUE="${TERMIN_ANDROID_ASSETS_DIR:-$SCRIPT_DIR/platform/termin-android/assets}"
...
if [[ ! -x "$HOST_PYTHON_VALUE" ]]; then
    ...
    echo "  Run ./scripts/build/sdk.sh first or set TERMIN_HOST_PYTHON." >&2
    exit 1
fi
