...
if (( $# == 1 )) && [[ "$1" == "--help" || "$1" == "-h" ]]; then
    cat <<'EOF'
Usage: ./scripts/run/python.sh [python arguments ...]

Runs bundled SDK Python with the checkout source overlay.

Examples:
  ./scripts/run/python.sh tcplot/examples/demo_sin.py
  ./scripts/run/python.sh -m pytest tcplot/tests/python
  ./scripts/run/python.sh -c "import tcplot; print(tcplot.__file__)"
EOF
    exit 0
fi
