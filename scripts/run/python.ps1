...
if ($args.Count -eq 1 -and ($args[0] -eq "--help" -or $args[0] -eq "-h")) {
    ...
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  task run:python -- tcplot\examples\demo_sin.py"
    Write-Host "  task run:python -- -m pytest tcplot\tests\python"
    Write-Host "  task run:python -- -c `"import tcplot; print(tcplot.__file__)`""
    exit 0
}
...
if (-not (Test-Path $PythonBin -PathType Leaf)) {
    throw "SDK Python launcher is missing: $PythonBin. Run .\scripts\build\sdk.ps1 first."
}
if (-not (Test-Path $OverlayManifest -PathType Leaf)) {
    throw "Checkout Python overlay is missing: $OverlayManifest. Run .\scripts\test\setup-python-env.ps1 first."
}
