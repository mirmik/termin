...
#
# Usage:
#   ./scripts/test/lint-python.sh                 # check the repository Python tree
#   ./scripts/test/lint-python.sh termin-csg ...  # check selected paths
#   ./scripts/test/lint-python.sh --system-python # use PYTHON_BIN / system Python
...
if ! "${PYTHON_COMMAND[@]}" -m ruff --version >/dev/null 2>&1; then
    ...
    echo "Run ./scripts/test/setup-python-env.sh to install test and lint dependencies." >&2
    exit 1
fi
