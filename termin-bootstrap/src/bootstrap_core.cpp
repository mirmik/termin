...
namespace termin::bootstrap {
    ...
    namespace {
        ...
        template <typename H> void register_once(const char* kind_name, bool& registered) {
            if (registered) {
                return;
            }
...
namespace termin::bootstrap {
    ...
    void register_runtime_kinds(const RuntimeKindOptions& options) {
        ...
        if (options.ui_document) {
            register_once<gui_native::TcUiDocumentAsset>("ui_document", g_bootstrap_state.ui_document_registered);
        }
    }
    ...
    void register_builtin_component_types() {
        ...
        register_builtin_physics_component_types();
        register_builtin_physics_fem_component_types();
        register_builtin_physics_fem_ui_component_types();
    ...
    void bootstrap_runtime(RuntimeBootstrapProfile profile) {
        ...
        if (!tcplot::gui_native::register_plot_widget_types()) {
            tc::Log::error("[termin-bootstrap] failed to register tcplot GUI widget types");
        }
#endif
