...
int main() {
    ...
    ok &= require(!tc_scene_ext_registry_initialized(), "registry must not exist before process init");
    ok &=
        require(!tc_scene_ext_register(
                    TC_SCENE_EXT_TYPE_PATHFINDING_WORLD, "lifecycle_probe", "lifecycle_probe", &probe_vtable, nullptr),
                "registration before process init must fail");
    ok &= require(!tc_scene_ext_registry_initialized(), "failed registration must not initialize the registry");
    ok &= require(!tc_scene_ext_attach(TC_SCENE_HANDLE_INVALID, TC_SCENE_EXT_TYPE_PATHFINDING_WORLD),
                  "attach before process init must fail");

    tc_init();
    ok &= require(tc_scene_ext_registry_initialized(), "tc_init must create the registry");
    ok &=
        require(tc_scene_ext_register(
                    TC_SCENE_EXT_TYPE_PATHFINDING_WORLD, "lifecycle_probe", "lifecycle_probe", &probe_vtable, nullptr),
    ...
    ok &= require(tc_scene_ext_attach(scene, TC_SCENE_EXT_TYPE_PATHFINDING_WORLD), "registered extension must attach");
    ok &= require(g_create_count == 1, "extension create callback must run once");

    tc_scene_ext_registry_shutdown();
    ok &= require(tc_scene_ext_registry_initialized(), "registry shutdown must refuse while scenes are alive");
    ...
    tc_shutdown();
    ok &= require(g_destroy_count == 1, "tc_shutdown must destroy scene extensions before their type table");
    ok &= require(!tc_scene_ext_registry_initialized(), "tc_shutdown must destroy the registry");
    ...
    ok &= require(!tc_scene_ext_registry_initialized(), "bootstrap shutdown must own the registry boundary");

    termin::bootstrap::bootstrap_runtime();
    ok &= require(tc_scene_ext_registry_initialized(), "registry must support a second process init cycle");
    termin::bootstrap::shutdown_runtime();
    ok &= require(!tc_scene_ext_registry_initialized(), "registry must support a second process shutdown cycle");
    return ok ? 0 : 1;
}
