...
void tc_texture_init(void) {
    ...
    if (!tc_pool_init(&g_texture_pool, sizeof(tc_texture), 64)) {
        tc_log_error("tc_texture_init: failed to init pool");
        return;
    }

    g_uuid_to_index = tc_resource_map_new(NULL);
    if (!g_uuid_to_index) {
        tc_log_error("tc_texture_init: failed to create uuid map");
...
tc_texture_handle tc_texture_create(const char* uuid) {
    ...
    if (uuid && uuid[0] != '\0') {
        if (tc_texture_contains(uuid)) {
            tc_log_warn("tc_texture_create: uuid '%s' already exists", uuid);
    ...
    if (tc_handle_is_invalid(h)) {
        tc_log_error("tc_texture_create: pool alloc failed");
        return tc_texture_handle_invalid();
    }
    ...
    if (!tc_resource_map_add(g_uuid_to_index, tex->uuid, pack_index(h.index))) {
