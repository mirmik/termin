...
void tc_shader_init(void) {
    ...
    if (!g_uuid_to_index) {
        tc_log_error("tc_shader_init: failed to create uuid map");
    ...
    if (!g_hash_to_index) {
        tc_log_error("tc_shader_init: failed to create hash map");
...
tc_shader_handle tc_shader_create(const char* uuid) {
    ...
    if (uuid && uuid[0] != '\0') {
        if (tc_shader_contains(uuid)) {
            tc_log_warn("tc_shader_create: uuid '%s' already exists", uuid);
    ...
    if (tc_handle_is_invalid(h)) {
        tc_log_error("tc_shader_create: pool alloc failed");
        return tc_shader_handle_invalid();
    }
...
tc_shader_handle tc_shader_get_or_create(const char* uuid) {
    if (!uuid || uuid[0] == '\0') {
        tc_log_warn("tc_shader_get_or_create: empty uuid");
        return tc_shader_handle_invalid();
    }
...
bool tc_shader_release(tc_shader* shader) {
    if (!shader) {
        tc_log_warn("tc_shader_release: null shader");
        return false;
    }
    if (shader->ref_count == 0) {
        tc_log_warn("tc_shader_release: '%s' [%s] already at ref_count=0",
...
void tc_shader_set_variant_info(
    ...
    if (!orig) {
        tc_log_warn("tc_shader_set_variant_info: invalid original handle");
        return;
    }
...
tc_shader_info* tc_shader_get_all_info(size_t* count) {
    ...
    *count = 0;
    ...
    if (!infos) {
        tc_log_error("tc_shader_get_all_info: allocation failed");
        return NULL;
    }
