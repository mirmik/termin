...
namespace termin::robotics
{
    namespace
    {
        ...
        failure(InverseDynamicsControlDiagnostic3D diagnostic,
                std::string_view message) noexcept
        {
            std::fprintf(
                ...
                message.data(),
                static_cast<int>(
                    inverse_dynamics_control_diagnostic_name(diagnostic)
                        .size()),
        ...
        validate_linearization(const TaskLinearization3D& value,
                               std::size_t variable_count) noexcept
        {
            ...
            if (value.variable_count != variable_count || rows == 0 ||
                ...
                (!value.weight_storage.empty() &&
                 value.weight_storage.size() != rows * rows))
            {
                return InverseDynamicsControlDiagnostic3D::DimensionMismatch;
            }
            if (!finite(value.matrix_storage) ||
                !finite(value.target_storage) || !finite(value.weight_storage))
            {
        ...
        default_actuators(const Articulation3D& articulation)
        {
        ...
        InverseDynamicsControlDiagnostic3D validate_actuators(
            const std::vector<InverseDynamicsActuator3D>& actuators,
            std::size_t dof_count) noexcept
        {
            ...
            for (const InverseDynamicsActuator3D& actuator : actuators)
            {
                ...
                if ((actuator.minimum_effort.has_value() &&
                     !std::isfinite(*actuator.minimum_effort)) ||
                    (actuator.maximum_effort.has_value() &&
                     !std::isfinite(*actuator.maximum_effort)) ||
                    (actuator.minimum_effort.has_value() &&
                     actuator.maximum_effort.has_value() &&
        ...
        generalized_velocity(const Articulation3D& articulation)
        {
            ...
            if (articulation.has_floating_base())
            {
                const termin::Screw3& velocity =
                    articulation.floating_base()->velocity_local;
    ...
    std::string_view inverse_dynamics_control_diagnostic_name(
    ...
    {
        return diagnostic == InverseDynamicsControlDiagnostic3D::None &&
               status == qopt::QpStatus::Optimal;
    }

    InverseDynamicsHqpController3D::InverseDynamicsHqpController3D(
        Articulation3D& articulation, termin::Vec3 gravity_world)
    ...
    InverseDynamicsHqpController3D::InverseDynamicsHqpController3D(
        ...
        : articulation_(&articulation), actuators_(std::move(actuators)),
          gravity_world_(gravity_world)
    {
    }

    const std::vector<InverseDynamicsActuator3D>&
    InverseDynamicsHqpController3D::actuators() const noexcept
    {
        return actuators_;
    }
    ...
    InverseDynamicsControlDiagnostic3D
    InverseDynamicsHqpController3D::set_gravity_world(
        termin::Vec3 gravity_world) noexcept
    {
    ...
    InverseDynamicsControlResult3D InverseDynamicsHqpController3D::solve(
        std::span<const ArticulationTask3D* const> tasks,
        InverseDynamicsControlOptions3D options) noexcept
    {
        if (articulation_ == nullptr ||
            articulation_->diagnostic() != Articulation3DDiagnostic::None)
        {
            return failure(InverseDynamicsControlDiagnostic3D::InvalidModel,
        ...
        {
            return failure(InverseDynamicsControlDiagnostic3D::InvalidGravity,
        ...
        try
        {
            ...
            const InverseDynamicsControlDiagnostic3D actuator_diagnostic =
                validate_actuators(actuators_, count);
            ...
            {
                return failure(
                    InverseDynamicsControlDiagnostic3D::DimensionMismatch,
                    "external effort size mismatch");
            }
            if (!finite(options.external_generalized_effort))
            {
                return failure(
                    InverseDynamicsControlDiagnostic3D::NonFiniteInput,
                    "external effort is not finite");
            }
            const std::vector<double> external =
                options.external_generalized_effort.empty()
            ...
            {
                return failure(
                    InverseDynamicsControlDiagnostic3D::DynamicsFailure,
                    "mass matrix construction failed");
            }
            const std::vector<double> velocity =
                generalized_velocity(*articulation_);
            const std::vector<double> zero(count, 0.0);
            std::vector<double> bias;
            if (!articulation_->inverse_dynamics(
                    velocity, zero, gravity_world_, bias))
            {
                return failure(
                    InverseDynamicsControlDiagnostic3D::DynamicsFailure,
                    "bias effort construction failed");
            }
            ...
            for (std::size_t index = 0; index < tasks.size(); ++index)
            {
                ...
                {
                    result =
                        failure(InverseDynamicsControlDiagnostic3D::NullTask,
                                "null task pointer");
                    result.failed_task = index;
                    return result;
                }
                TaskLinearization3DResult linearized =
                    tasks[index]->linearize(context);
                if (!linearized.ok())
                {
                    result = failure(InverseDynamicsControlDiagnostic3D::
                                         TaskLinearizationFailure,
                                     "task could not be linearized");
                    result.task_diagnostic = linearized.diagnostic;
                    result.failed_task = index;
                    result.failed_task_name =
                        std::move(linearized.value.diagnostic_name);
                    return result;
                }
                ...
                const InverseDynamicsControlDiagnostic3D diagnostic =
                    validate_linearization(linearized.value, count);
                if (diagnostic != InverseDynamicsControlDiagnostic3D::None)
                {
                    result = failure(diagnostic,
                                     "invalid active task linearization");
                    result.failed_task = index;
                    result.failed_task_name =
                        std::move(linearized.value.diagnostic_name);
                    return result;
                ...
                levels[linearized.value.priority].push_back(
                    std::move(linearized.value));
            }
            if (levels.empty())
            {
                levels[0] = {};
            ...
            {
                const qopt::HqpLevelRegistrationResult level =
                    solver.add_level(priority);
                if (!level.ok())
                {
                    result = failure(
                        InverseDynamicsControlDiagnostic3D::RegistrationFailure,
            ...
            if (result.status != qopt::QpStatus::Optimal ||
                result.hqp_diagnostic != qopt::HqpDiagnostic::None)
            {
                result.diagnostic =
                    InverseDynamicsControlDiagnostic3D::SolveFailure;
                std::fprintf(stderr,
            ...
            for (std::size_t dof = 0; dof < count; ++dof)
            {
                ...
                if (!actuated[dof])
                {
                    result.unactuated_residual_linf = std::max(
                        ...
                        std::abs(result.required_generalized_effort[dof]));
                }
            }
            for (std::size_t index = 0; index < actuators_.size(); ++index)
            {
                result.actuator_effort[index] =
        ...
        return failure(InverseDynamicsControlDiagnostic3D::InternalFailure,
    ...
    InverseDynamicsHqpController3D::primal_warm_start() const noexcept
    {
