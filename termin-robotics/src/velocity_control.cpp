...
namespace termin::robotics
{
    namespace
    {
        ...
        VelocityControlResult3D failure(VelocityControlDiagnostic3D diagnostic,
                                        std::string_view message) noexcept
        {
            std::fprintf(
                ...
                message.data(),
                static_cast<int>(
                    velocity_control_diagnostic_name(diagnostic).size()),
        ...
        VelocityIntegrationResult3D
        integration_failure(VelocityIntegrationDiagnostic3D diagnostic,
                            ...
                            std::string_view message) noexcept
        {
            std::fprintf(
                ...
                message.data(),
                static_cast<int>(
                    velocity_integration_diagnostic_name(diagnostic).size()),
                velocity_integration_diagnostic_name(diagnostic).data());
            return {diagnostic, articulation_diagnostic};
        }
    } // namespace

    std::string_view velocity_control_diagnostic_name(
        VelocityControlDiagnostic3D diagnostic) noexcept
    {
        switch (diagnostic)
        {
    ...
    {
        return diagnostic == VelocityControlDiagnostic3D::None &&
               status == qopt::QpStatus::Optimal;
    }

    VelocityHqpController3D::VelocityHqpController3D(
        Articulation3D& articulation) noexcept
        : articulation_(&articulation)
    {
    }

    VelocityControlResult3D VelocityHqpController3D::solve(
        ...
        VelocityControlOptions3D options) noexcept
    {
        ...
        {
            return failure(VelocityControlDiagnostic3D::InvalidModel,
                           "invalid articulation");
        }
        if (!std::isfinite(options.time_step) || options.time_step <= 0.0)
        {
            return failure(VelocityControlDiagnostic3D::InvalidTimeStep,
        ...
        try
        {
            ...
            for (std::size_t index = 0; index < tasks.size(); ++index)
            {
                ...
                {
                    result = failure(VelocityControlDiagnostic3D::NullTask,
                                     "null task pointer");
                ...
                {
                    result = failure(
                        VelocityControlDiagnostic3D::TaskLinearizationFailure,
                ...
                const VelocityControlDiagnostic3D diagnostic =
                    validate_linearization(linearized.value, variable_count);
            ...
            for (const auto& [priority, linearizations] : levels)
            {
                ...
                {
                    result = failure(
                        VelocityControlDiagnostic3D::RegistrationFailure,
            ...
            result.hqp_result = solver.solve(
                {{result.generalized_velocity.data(), variable_count},
        ...
        {
            std::fprintf(stderr,
                         "[termin-robotics] velocity HQP failed: %s\n",
                         error.what());
        }
        catch (...)
        {
        ...
        return failure(VelocityControlDiagnostic3D::InternalFailure,
    ...
    VelocityHqpController3D::primal_warm_start() const noexcept
    {
        return primal_warm_start_;
    }

    std::string_view velocity_integration_diagnostic_name(
        VelocityIntegrationDiagnostic3D diagnostic) noexcept
    {
        switch (diagnostic)
        {
    ...
    VelocityIntegrationResult3D integrate_articulation_velocity(
        ...
        double time_step) noexcept
    {
        if (articulation.diagnostic() != Articulation3DDiagnostic::None)
        {
            return integration_failure(
                VelocityIntegrationDiagnostic3D::InvalidModel,
        ...
        {
            return integration_failure(
                VelocityIntegrationDiagnostic3D::InvalidTimeStep,
        ...
        {
            return integration_failure(
                VelocityIntegrationDiagnostic3D::DimensionMismatch,
                Articulation3DDiagnostic::None,
        ...
        {
            return integration_failure(
                VelocityIntegrationDiagnostic3D::NonFiniteVelocity,
                Articulation3DDiagnostic::None,
        ...
        try
        {
            const std::size_t joint_offset =
                articulation.has_floating_base() ? 6 : 0;
            Articulation3DState next_state = articulation.state();
            for (std::size_t joint = 0; joint < next_state.coordinates.size();
                 ++joint)
            {
                const double velocity =
                    generalized_velocity[joint_offset + joint];
            ...
            if (articulation.has_floating_base())
            {
                ...
                const termin::Pose3 next_pose =
                    articulation.floating_base()->pose_world *
                    termin::se3_exp(base_velocity_local * time_step);
                const Articulation3DDiagnostic base_diagnostic =
                    articulation.set_floating_base_state(next_pose,
                                                         base_velocity_local);
                if (base_diagnostic != Articulation3DDiagnostic::None)
                {
                    return integration_failure(
                        VelocityIntegrationDiagnostic3D::StateUpdateFailure,
                        base_diagnostic,
                        "floating-base update was rejected");
                }
            }

            const Articulation3DDiagnostic state_diagnostic =
                articulation.set_state(std::move(next_state));
        ...
        {
            std::fprintf(stderr,
                         "[termin-robotics] velocity integration failed: %s\n",
                         error.what());
        }
        catch (...)
        {
        ...
        return integration_failure(
            VelocityIntegrationDiagnostic3D::InternalFailure,
