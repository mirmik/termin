...
int main()
{
    ...
    for (std::size_t step = 0; step < 500; ++step)
    {
        ...
        PointVelocityTask3D point_task(
            1, Vec3::zero(), (target - point.value.position_world) * 3.0);
        JointVelocityLimitConstraint3D velocity_limit(
            {}, {-2.0, -2.0}, {2.0, 2.0}, {.priority = 0});
        const std::array<const ArticulationTask3D*, 2> tasks{&velocity_limit,
                                                             &point_task};
        const VelocityControlResult3D control =
            controller.solve(tasks, {.time_step = time_step});
        if (!control.ok() || !integrate_articulation_velocity(
                                  arm,
                                  {control.generalized_velocity.data(),
                                   control.generalized_velocity.size()},
                                  time_step)
                                  .ok())
        {
            return 2;
