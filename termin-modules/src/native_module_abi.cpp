...
namespace termin_modules {
    ...
    termin_native_module_host_v1 make_native_module_host_v1(const char* module_id, void* host_context) {
        return {sizeof(termin_native_module_host_v1),
    ...
    NativeModuleValidationResult
    validate_native_module_descriptor_v1(const termin_native_module_descriptor_v1_data* descriptor,
                                         const std::string& expected_module_id) {
        if (!descriptor)
            return incompatible("missing native module descriptor");
        ...
        if (descriptor->module_abi_version != TERMIN_NATIVE_MODULE_ABI_VERSION) {
            return incompatible("native module ABI mismatch: module=" + std::to_string(descriptor->module_abi_version) +
                                " host=" + std::to_string(TERMIN_NATIVE_MODULE_ABI_VERSION));
        }
        ...
        if (descriptor->sdk_version != TC_VERSION) {
            return incompatible("native SDK build mismatch: module=" + std::to_string(descriptor->sdk_version) +
                                " host=" + std::to_string(TC_VERSION));
        }
        if (descriptor->compiler_family != TERMIN_MODULE_COMPILER_FAMILY ||
            descriptor->compiler_version != TERMIN_MODULE_COMPILER_VERSION ||
            descriptor->cxx_abi_flags != TERMIN_MODULE_CXX_ABI_FLAGS || descriptor->pointer_size != sizeof(void*)) {
            std::ostringstream message;
            message << "native compiler/runtime ABI mismatch: module=" << descriptor->compiler_family << ':'
                    << descriptor->compiler_version << ':' << descriptor->cxx_abi_flags << ':'
                    << descriptor->pointer_size << " host=" << TERMIN_MODULE_COMPILER_FAMILY << ':'
                    << TERMIN_MODULE_COMPILER_VERSION << ':' << TERMIN_MODULE_CXX_ABI_FLAGS << ':' << sizeof(void*);
        ...
        if (!expected_module_id.empty() && expected_module_id != descriptor->module_id) {
            return incompatible("native module id mismatch: descriptor='" + std::string(descriptor->module_id) +
                                "' project='" + expected_module_id + "'");
        }
        ...
        if (!descriptor->init)
            return incompatible("native module descriptor has no init function");
        if (!descriptor->shutdown)
            return incompatible("native module descriptor has no shutdown function");
        return {true, {}};
    }
