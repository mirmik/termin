...
namespace termin_modules {
    namespace {
        ...
        bool has_added_path(const std::vector<std::filesystem::path>& added_paths, const std::filesystem::path& path) {
        ...
        bool append_sys_path_once(const std::filesystem::path& path,
        ...
        bool call_module_context_function(const char* function_name, const std::string& module_id, std::string& error) {
            ...
            if (result == nullptr) {
                error = std::string("termin_modules.module_context.") + function_name +
                        " failed: " + fetch_python_error_string();
                return false;
            }
        ...
        bool register_module_packages(const std::string& module_id,
            ...
            PyObject* result =
                PyObject_CallMethod(context_module, "register_module_packages", "sO", module_id.c_str(), package_list);
            Py_DECREF(package_list);
            Py_DECREF(context_module);
            if (result == nullptr) {
                error = "termin_modules.module_context.register_module_packages failed: " + fetch_python_error_string();
                return false;
            }
            Py_DECREF(result);
            return true;
        ...
        bool module_name_matches_package(const std::string& module_name, const std::string& package) {
            ...
            return module_name.compare(0, package.size(), package) == 0 && module_name[package.size()] == '.';
        }
        ...
        bool validate_package_namespace_claims(const std::vector<ModuleRecord>& records, std::string& error) {
            struct Claim {
            ...
            for (const ModuleRecord& record : records) {
                ...
                for (const std::string& package : config->packages) {
                    for (const Claim& existing : claims) {
                        if (existing.module_id != record.spec.id && package_claims_overlap(existing.package, package)) {
                            error = "Python package namespace overlap: module '" + record.spec.id + "' claims '" +
                                    package + "', already claimed as '" + existing.package + "' by module '" +
                                    existing.module_id + "'";
                            return false;
                        }
                    }
        ...
        std::vector<std::pair<std::string, PyObject*>>
        collect_module_subtree(const std::vector<std::string>& packages) {
        ...
        void update_owned_module(PythonImportHandle& handle, const std::string& name, PyObject* module) {
        ...
        void collect_import_transaction_delta(const std::vector<std::string>& packages,
        ...
        bool run_command_capture(const std::string& command, std::string& output, std::string& error) {
            output.clear();
            error.clear();
        ...
        std::string summarize_command_output(const std::string& output, const std::string& fallback) {
        ...
        bool ensure_project_venv(const ModuleEnvironment& environment, std::string& diagnostics, std::string& error) {
            diagnostics.clear();
            error.clear();
            ...
            const std::string command = shell_quote(environment.python_executable) + " -m venv " +
                                        shell_quote(environment.project_venv_path.string()) + " 2>&1";
        ...
        bool append_python_site_paths(const ModuleEnvironment& environment,
                                      std::vector<std::filesystem::path>& added_paths,
                                      std::string& error) {
            error.clear();
            ...
            if (environment.use_project_venv) {
                ...
                std::string output;
                const std::string command = shell_quote(python.string()) + " -c " +
                                            shell_quote("import sysconfig; "
        ...
        bool install_requirements(const ModuleEnvironment& environment,
        ...
        bool ensure_requirements(const PythonModuleConfig& config,
    ...
    bool PythonModuleBackend::prepare_environment(const std::vector<ModuleRecord>& records,
                                                  const ModuleEnvironment& environment,
                                                  std::string& diagnostics,
                                                  std::string& error) {
        diagnostics.clear();
    ...
    bool PythonModuleBackend::teardown_environment(const ModuleEnvironment& environment, std::string& error) {
        (void)environment;
        error.clear();
    ...
    bool PythonModuleBackend::load(ModuleRecord& record, const ModuleEnvironment& environment) {
        const auto config = std::dynamic_pointer_cast<PythonModuleConfig>(record.spec.config);
        if (!config) {
    ...
    bool PythonModuleBackend::unload(ModuleRecord& record, const ModuleEnvironment& environment) {
        (void)environment;
