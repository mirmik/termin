...
namespace termin_modules {
    namespace {
        ...
        void native_module_host_log(void*, int level, const char* message) {
            ...
            switch (level) {
            case TERMIN_NATIVE_MODULE_LOG_DEBUG:
                tc::Log::debug("[NativeModule] %s", text);
                break;
            case TERMIN_NATIVE_MODULE_LOG_INFO:
                tc::Log::info("[NativeModule] %s", text);
                break;
            case TERMIN_NATIVE_MODULE_LOG_WARN:
                tc::Log::warn("[NativeModule] %s", text);
                break;
            default:
                tc::Log::error("[NativeModule] %s", text);
                break;
            }
        }

        std::string native_module_error_message(int32_t status, const char* phase, const char* buffer) {
            std::string result = std::string(phase) + " failed with status " + std::to_string(status);
            if (buffer && buffer[0])
        ...
        bool is_default_cpp_rebuild_input(const std::filesystem::path& path) {
            ...
            if (filename == "CMakeLists.txt" || filename == "CMakePresets.json" || filename == "meson.build" ||
                filename == "meson_options.txt" || filename == "Makefile" || filename == "makefile" ||
                filename == "GNUmakefile" || filename == "BUILD" || filename == "BUILD.bazel" ||
                filename == "WORKSPACE" || filename == "WORKSPACE.bazel") {
                return true;
            }
            ...
            return extension == ".c" || extension == ".cc" || extension == ".cpp" || extension == ".cxx" ||
                   extension == ".m" || extension == ".mm" || extension == ".h" || extension == ".hh" ||
                   extension == ".hpp" || extension == ".hxx" || extension == ".inl" || extension == ".ipp" ||
                   extension == ".ixx" || extension == ".cppm" || extension == ".cu" || extension == ".cuh" ||
                   extension == ".cmake";
        }

        bool path_is_newer_than(const std::filesystem::path& input,
                                const std::filesystem::file_time_type artifact_time) {
            std::error_code ec;
            const bool exists = std::filesystem::exists(input, ec);
            if (ec || !exists) {
                tc::Log::warn("CppModuleBackend: declared rebuild input '%s' is unavailable; assuming rebuild needed",
                              input.string().c_str());
                return true;
            }
            ...
            if (std::filesystem::is_regular_file(input, ec)) {
                if (ec) {
                    tc::Log::warn("CppModuleBackend: cannot inspect rebuild input '%s'; assuming rebuild needed",
                                  input.string().c_str());
                    return true;
                }
        ...
        void prune_abandoned_shadow_sessions(const std::filesystem::path& base_dir) {
            ...
            if (ec) {
                tc::Log::error("CppModuleBackend: failed to enumerate native shadow root '%s': %s",
            ...
            for (const auto& entry : it) {
                ...
                if (ec) {
                    tc::Log::warn("CppModuleBackend: cannot inspect abandoned shadow session '%s': %s",
                ...
                if (ec) {
                    tc::Log::error("CppModuleBackend: failed to remove abandoned shadow session '%s': %s",
                                   entry.path().string().c_str(),
                                   ec.message().c_str());
                    ec.clear();
                } else {
                    tc::Log::info("CppModuleBackend: removed abandoned native shadow session '%s'",
                                  entry.path().string().c_str());
                }
            }
        }
        ...
        std::filesystem::path native_validator_path(const ModuleEnvironment& environment,
            ...
            for (std::filesystem::directory_iterator it(bin_dir, iteration_error), end; !iteration_error && it != end;
                 it.increment(iteration_error)) {
                if (!it->is_directory()) {
                    continue;
                }
                const std::filesystem::path candidate = it->path() / "termin_module_native_validator.exe";
        ...
        bool run_shell_command_capture(const std::string& command,
        ...
        bool validate_native_artifact(ModuleRecord& record,
                                      const ModuleEnvironment& environment,
                                      const std::filesystem::path& artifact_path) {
            const std::filesystem::path validator = native_validator_path(environment, artifact_path);
            if (validator.empty()) {
                return true;
            }
            ...
            std::string output;
            std::string error;
            const std::string command = shell_quote(validator) + " " + shell_quote(artifact_path) + " " +
        ...
        void* load_shared_library(const std::filesystem::path& path,
            ...
            for (const auto& directory : {artifact_dir, sdk_bin_dir}) {
                ...
                if (cookie == nullptr) {
                    ...
                    std::ostringstream ss;
                    ss << "AddDllDirectory failed for " << dependency_path.string() << " with error code "
                       << error_code;
                    error = ss.str();
                    return nullptr;
                }
            ...
            HMODULE handle = LoadLibraryExW(load_path.c_str(),
                                            nullptr,
            ...
            if (handle == nullptr) {
                std::ostringstream ss;
                ss << "LoadLibraryEx failed for " << load_path.string() << " with error code " << load_error_code;
                error = ss.str();
                return nullptr;
            }
    ...
    CppModuleBackend::~CppModuleBackend() noexcept {
        ...
        if (ec) {
            tc::Log::error("CppModuleBackend: failed to remove native shadow session directory '%s': %s",
        ...
        if (ec) {
            tc::Log::error("CppModuleBackend: failed to inspect native shadow session directory '%s': %s",
                           _shadow_session_dir.string().c_str(),
                           ec.message().c_str());
            return;
        }
        if (!removed && still_exists) {
            tc::Log::error("CppModuleBackend: native shadow session directory is not empty at destruction: '%s'",
                           _shadow_session_dir.string().c_str());
            return;
        }
    ...
    bool CppModuleBackend::ensure_shadow_session(const ModuleEnvironment& environment, std::string& error) {
        ...
        for (size_t attempt = 0; attempt < 100; ++attempt) {
            ...
            const std::string name =
                "session-" + std::to_string(process_id()) + "-" + std::to_string(now) + "-" + std::to_string(sequence);
        ...
        error = "Failed to allocate a collision-free native shadow session under '" + _shadow_base_dir.string() + "'";
        return false;
    }

    std::filesystem::path CppModuleBackend::next_shadow_path(const std::filesystem::path& artifact_path,
                                                             const ModuleEnvironment& environment,
                                                             std::string& error) {
    ...
    bool CppModuleBackend::stage_sibling_libraries(ModuleRecord& record,
                                                   const std::filesystem::path& artifact_path,
        ...
        for (const auto& entry : it) {
            ...
            if (entry.path().filename() == artifact_path.filename() || !is_dynamic_library_file(entry.path())) {
                continue;
            }
            ...
            if (ec) {
                record.error_message =
                    "Failed to stage native module dependency '" + entry.path().string() + "': " + ec.message();
                return false;
            }
        }
        return true;
    ...
    bool CppModuleBackend::remove_shadow_artifacts(ModuleRecord& record, const std::filesystem::path& path) {
        if (path.empty()) {
            return true;
        }
        ...
        const std::string cleanup_error =
            "Failed to remove native shadow load directory '" + load_dir.string() + "': " + ec.message();
        ...
        try {
            ...
            for (auto it = std::filesystem::recursive_directory_iterator(module_dir);
                ...
                if (entry.is_directory()) {
                    ...
                    if (dirname == "build" || dirname == "__pycache__" || (!dirname.empty() && dirname[0] == '.')) {
                ...
                if (is_default_cpp_rebuild_input(entry.path()) && entry.last_write_time() > artifact_time) {
                    return true;
                }
            }
        } catch (const std::exception& e) {
            ...
            tc::Log::warn(
                "CppModuleBackend::needs_rebuild: unknown error during filesystem iteration, assuming rebuild needed");
            return true;
        }
    ...
    bool CppModuleBackend::build(ModuleRecord& record, const ModuleEnvironment& environment) {
        const auto config = std::dynamic_pointer_cast<CppModuleConfig>(record.spec.config);
        if (!config) {
    ...
    bool CppModuleBackend::load(ModuleRecord& record, const ModuleEnvironment& environment) {
        ...
        const std::filesystem::path load_path = next_shadow_path(config->artifact_path, environment, shadow_error);
        if (load_path.empty()) {
            record.error_message = shadow_error;
            return false;
        }
        try {
            std::filesystem::copy_file(config->artifact_path, load_path, std::filesystem::copy_options::none);
        } catch (const std::exception& e) {
        ...
        std::string error;
        const std::filesystem::path sdk_bin_dir =
            environment.sdk_prefix.empty() ? std::filesystem::path{} : environment.sdk_prefix / "bin";
        void* native_handle = load_shared_library(load_path, config->artifact_path.parent_path(), sdk_bin_dir, error);
        ...
        const auto* descriptor = reinterpret_cast<const termin_native_module_descriptor_v1_data*>(
        ...
        handle->host_api = make_native_module_host_v1(handle->module_id.c_str(), handle.get());
        ...
        termin_native_module_error init_error{sizeof(termin_native_module_error), error_buffer, sizeof(error_buffer)};
        ...
        if (init_status != 0 || !record.error_message.empty()) {
            if (record.error_message.empty()) {
                record.error_message = native_module_error_message(init_status, "native module init", error_buffer);
            }
            char shutdown_buffer[2048] = {};
            termin_native_module_error shutdown_error{
                sizeof(termin_native_module_error), shutdown_buffer, sizeof(shutdown_buffer)};
            try {
                const int32_t shutdown_status = descriptor->shutdown(&handle->host_api, &shutdown_error);
                if (shutdown_status != 0) {
                    record.diagnostics += "\n" + native_module_error_message(shutdown_status,
            ...
            if (environment.on_cpp_module_load_failure) {
                contributions_clean =
                    environment.on_cpp_module_load_failure(record, record.error_message, contribution_cleanup_error);
            }
            handle->shutdown_called = true;
            if (!contributions_clean) {
                if (contribution_cleanup_error.empty()) {
                    contribution_cleanup_error = "owner contribution cleanup failed after native module init failure";
                }
    ...
    bool CppModuleBackend::unload(ModuleRecord& record, const ModuleEnvironment& environment) {
    ...
    bool CppModuleBackend::begin_unload(ModuleRecord& record, const ModuleEnvironment& environment) {
        (void)environment;
        ...
        char error_buffer[2048] = {};
        termin_native_module_error shutdown_error{
            sizeof(termin_native_module_error), error_buffer, sizeof(error_buffer)};
        int32_t status = -1;
        try {
        ...
        if (status != 0) {
            record.error_message = native_module_error_message(status, "native module shutdown", error_buffer);
            return false;
        }
        handle->shutdown_called = true;
        return true;
    ...
    bool CppModuleBackend::finish_unload(ModuleRecord& record, const ModuleEnvironment& environment) {
        (void)environment;
    ...
    ModuleCleanResult CppModuleBackend::clean(ModuleRecord& record, const ModuleEnvironment& environment) {
        const auto config = std::dynamic_pointer_cast<CppModuleConfig>(record.spec.config);
        if (!config) {
            record.error_message = "Invalid C++ module config";
    ...
    bool CppModuleBackend::run_build_command(const std::string& module_id,
