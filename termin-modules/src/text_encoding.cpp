    ...
    namespace {
        ...
        std::string decode_windows_code_page(const std::string& text, UINT code_page) {
            ...
            const int wide_size = MultiByteToWideChar(
                code_page, MB_ERR_INVALID_CHARS, text.data(), static_cast<int>(text.size()), nullptr, 0);
            if (wide_size <= 0) {
                return {};
            }
            ...
            MultiByteToWideChar(
                code_page, MB_ERR_INVALID_CHARS, text.data(), static_cast<int>(text.size()), wide.data(), wide_size);
        ...
        std::string replace_invalid_utf8(const std::string& text) {
            ...
            for (size_t i = 0; i < text.size();) {
                ...
                if (valid) {
                    valid = (count != 2 || codepoint >= 0x80) && (count != 3 || codepoint >= 0x800) &&
