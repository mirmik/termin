...
namespace termin_modules {
    namespace {
        ...
        get_platform_string(tc::trent_view node, const char* key, const std::string& module_id, std::string& error) {
            error.clear();
        ...
        std::optional<ModuleKind>
        parse_kind(tc::trent_view root, const std::filesystem::path& path, std::string& error) {
    ...
    std::optional<ModuleSpec> ModuleDescriptorParser::parse(const std::filesystem::path& path,
        ...
        try {
            ...
            const bool is_json =
                first_content != std::string::npos && (text[first_content] == '{' || text[first_content] == '[');
        ...
        if (root.contains("components")) {
            error = "Field 'components' is no longer supported in module descriptors; "
                    "component ownership is inferred from runtime registrations in " +
                    path.string();
            return std::nullopt;
        }
        ...
        if (spec.kind == ModuleKind::Cpp) {
            ...
            const std::vector<std::string> rebuild_inputs = get_optional_string_list(build, "inputs", error);
            if (!error.empty()) {
                error += " in " + path.string();
                return std::nullopt;
            }
