...
namespace termin_modules {
    namespace {
        ...
        bool is_same_or_child_path(const std::filesystem::path& path, const std::filesystem::path& parent) {
        ...
        std::vector<std::string> collect_active_dependents(const std::vector<ModuleRecord>& records,
        ...
        const char* cleanup_phase_name(ModuleCleanupPhase phase) {
            switch (phase) {
            case ModuleCleanupPhase::None:
                return "none";
            case ModuleCleanupPhase::Prepare:
                return "prepare";
            case ModuleCleanupPhase::BackendBegin:
                return "backend-begin";
            case ModuleCleanupPhase::RevokeContributions:
                return "revoke-contributions";
            case ModuleCleanupPhase::BackendFinish:
                return "backend-finish";
            case ModuleCleanupPhase::BackendUnload:
    ...
    bool ModuleRuntime::discover(const std::filesystem::path& project_root) {
        ...
        while (it != end) {
            ...
            if (entry.is_directory()) {
                ...
                if (dirname == "build" || dirname == "dist" || dirname == "__pycache__" ||
        ...
        for (const std::filesystem::path& descriptor_path : descriptor_paths) {
            ...
            const auto [existing, inserted] = discovered_descriptors.emplace(spec->id, descriptor_path);
            if (!inserted) {
                _last_error = "Duplicate module id '" + spec->id + "' in descriptors '" + existing->second.string() +
                              "' and '" + descriptor_path.string() + "'";
        ...
        for (const ModuleRecord& record : _records) {
            emit(ModuleEventKind::Discovered, record.spec.id, record.spec.descriptor_path.string());
        }
        return true;
    }
        ...
        for (const std::string& module_id : ordered) {
            ...
            if (!unloaded) {
                ...
                const std::string error =
                    record && !record->error_message.empty() ? record->error_message : _last_error;
        ...
        for (auto& [kind, backend] : _backends) {
            ...
            for (auto& [rollback_kind, rollback_backend] : _backends) {
                ...
                if (!rollback_backend->teardown_environment(_environment, rollback_error)) {
                    tc::Log::error("ModuleRuntime: environment rollback failed: %s", rollback_error.c_str());
                }
            }
    ...
    bool ModuleRuntime::load_module_impl(const std::string& module_id, bool refresh_descriptors) {
        ...
        if (target->state == ModuleState::Unloading || target->state == ModuleState::CleanupFailed ||
            target->cleanup_phase != ModuleCleanupPhase::None) {
            target->error_message = "Module cleanup is incomplete at phase '" +
        ...
        if (target->spec.kind == ModuleKind::Cpp) {
            ...
            load_environment.on_cpp_module_load_failure =
                [this, &cpp_load_failure_handled](
                    const ModuleRecord& record, const std::string& error, std::string& cleanup_error) {
        ...
        if (!backend->load(*target, load_environment)) {
            ...
            if (target->spec.kind == ModuleKind::Cpp) {
                if (!cpp_load_failure_handled && _cpp_callbacks.after_failed_load) {
                    ...
                    if (!_cpp_callbacks.after_failed_load(*target, target->error_message, cleanup_error) &&
                        !cleanup_error.empty()) {
                ...
                if (_python_callbacks.after_failed_load) {
                    std::string cleanup_error;
                    if (!_python_callbacks.after_failed_load(*target, target->error_message, cleanup_error) &&
                        !cleanup_error.empty()) {
                        target->error_message += "; " + cleanup_error;
                    }
                }
            ...
            if (target->handle) {
                ...
                target->cleanup_phase = target->spec.kind == ModuleKind::Cpp && backend->supports_staged_unload()
                ...
                if (unload_module_impl(module_id, false)) {
                    ...
                    target = find_mutable_record(_records, module_id);
                    target->error_message = load_error + "; replacement cleanup failed: " + target->error_message +
                                            "; call unload_module to complete cleanup";
                    _last_error = target->error_message;
                    tc::Log::error("ModuleRuntime: failed load retained module='%s' backend='%s' phase='%s'",
                                   module_id.c_str(),
    ...
    bool ModuleRuntime::unload_module_impl(const std::string& module_id, bool refresh_descriptor) {
        ModuleRecord* target = find_mutable_record(_records, module_id);
        if (target == nullptr) {
            _last_error = "Module not found: " + module_id;
            return false;
        ...
        auto log_failure = [&](ModuleCleanupPhase phase, bool after_boundary) {
            tc::Log::error("ModuleRuntime: unload failure module='%s' backend='%s' phase='%s' "
        ...
        if (!retrying_cleanup) {
            ...
            if (target->spec.kind == ModuleKind::Cpp) {
                if (_cpp_callbacks.before_unload) {
                    ...
                    if (!_cpp_callbacks.before_unload(*target, error)) {
                        return fail_before_boundary(error.empty() ? "C++ module unload preparation failed" : error);
                    }
                }
            } else {
                ...
                if (_python_callbacks.before_module_remove) {
                    ...
                    if (!_python_callbacks.before_module_remove(*target, error)) {
                        return fail_before_boundary(error.empty() ? "Python module unload preparation failed" : error);
                    }
                }
            }
            ...
            target->state = ModuleState::Unloading;
            target->error_message.clear();
            emit(ModuleEventKind::Unloading,
                 module_id,
                 "Retrying cleanup phase: " + std::string(cleanup_phase_name(target->cleanup_phase)));
        }
        ...
        if (target->cleanup_phase == ModuleCleanupPhase::BackendFinish) {
            ...
            return fail_after_boundary("Invalid cleanup phase: " +
                                       std::string(cleanup_phase_name(target->cleanup_phase)));
        }
    ...
    bool ModuleRuntime::execute_reload(const std::vector<std::string>& ordered_module_ids) {
        ...
        for (const std::string& module_id : ordered_module_ids) {
            ...
            if (record->state == ModuleState::CleanupFailed || record->state == ModuleState::Unloading ||
    ...
    bool ModuleRuntime::require_completed_reload_unload(const std::vector<std::string>& module_ids) {
        for (const std::string& module_id : module_ids) {
            ...
            const bool inactive_state = record->state != ModuleState::Loaded && record->state != ModuleState::Loading &&
            ...
            _last_error = "Replacement load blocked because old module generation did not reach "
                          "completed unload: module='" +
                          module_id + "' state='" + module_state_name(record->state) + "' phase='" +
                          cleanup_phase_name(record->cleanup_phase) +
                          "' retained_handle=" + (record->handle ? "yes" : "no");
            tc::Log::error("ModuleRuntime: %s", _last_error.c_str());
            emit(ModuleEventKind::Failed, module_id, _last_error);
            return false;
        }
    ...
    bool ModuleRuntime::clean_module(const std::string& module_id) {
        ...
        if (target->state == ModuleState::Loaded || target->state == ModuleState::Loading ||
            target->state == ModuleState::Unloading || target->state == ModuleState::CleanupFailed ||
            target->cleanup_phase != ModuleCleanupPhase::None) {
    ...
    bool ModuleRuntime::resolve_closure(const std::vector<std::string>& root_module_ids,
    ...
    bool ModuleRuntime::build_reload_order_with_loaded_dependents(const std::string& module_id,
                                                                  std::vector<std::string>& ordered,
                                                                  std::string& error) {
        ordered.clear();
    ...
    bool ModuleRuntime::visit_reload_module(const std::string& module_id,
    ...
    bool ModuleRuntime::visit_module(ModuleRecord& record,
                                     std::unordered_map<std::string, int>& marks,
                                     std::vector<ModuleRecord*>& ordered,
                                     std::string& error) {
    ...
    bool ModuleRuntime::visit_closure_module(const ModuleRecord& record,
                                             std::unordered_map<std::string, int>& marks,
                                             std::vector<const ModuleRecord*>& ordered,
                                             std::vector<std::string>& stack,
        ...
        for (const std::string& dependency : dependencies) {
            ...
            if (!visit_closure_module(*dependency_record, marks, ordered, stack, error))
                return false;
        }
        stack.pop_back();
        marks[record.spec.id] = 2;
    ...
    bool ModuleRuntime::refresh_descriptor_snapshot() {
        ...
        for (size_t index = 0; index < _records.size(); ++index) {
            ...
            if (!parsed.has_value()) {
                return fail(record.spec.id, "Failed to parse descriptor '" + descriptor.string() + "': " + error);
            }
            const auto [existing, inserted] = indices.emplace(parsed->id, index);
            if (!inserted) {
                return fail(parsed->id,
                            "Duplicate module id '" + parsed->id + "' in descriptors '" +
                                snapshot[existing->second].descriptor_path.string() + "' and '" + descriptor.string() +
                                "'");
            }
        ...
        for (size_t index = 0; index < snapshot.size(); ++index) {
            ...
            if (is_loaded_or_holding_handle(record) && parsed.kind != record.spec.kind) {
                return fail(record.spec.id,
                            "Descriptor kind changed while module '" + record.spec.id +
                                "' is loaded: " + parsed.descriptor_path.string());
            }
        }
    ...
    bool ModuleRuntime::restore_reload_state(const std::string& module_id,
                                             const std::shared_ptr<IModuleReloadState>& reload_state) {
