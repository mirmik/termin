...
namespace termin_modules {
    ...
    public:
        ...
        bool resolve_closure(const std::vector<std::string>& root_module_ids,
    ...
    private:
        ...
        bool build_reload_order_with_loaded_dependents(const std::string& module_id,
                                                       std::vector<std::string>& ordered,
                                                       std::string& error);
        bool visit_reload_module(const std::string& module_id,
                                 const std::unordered_map<std::string, bool>& affected,
                                 std::unordered_map<std::string, int>& marks,
                                 std::vector<std::string>& ordered,
                                 std::string& error);
        bool visit_module(ModuleRecord& record,
                          std::unordered_map<std::string, int>& marks,
                          std::vector<ModuleRecord*>& ordered,
                          std::string& error);
        bool visit_closure_module(const ModuleRecord& record,
                                  std::unordered_map<std::string, int>& marks,
                                  std::vector<const ModuleRecord*>& ordered,
        ...
        bool restore_reload_state(const std::string& module_id,
