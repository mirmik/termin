...
namespace termin_modules {
    ...
    public:
        ...
        bool load(ModuleRecord& record, const ModuleEnvironment& environment) override;

        bool build(ModuleRecord& record, const ModuleEnvironment& environment) override;

        bool unload(ModuleRecord& record, const ModuleEnvironment& environment) override;

        bool supports_staged_unload() const override {
            return true;
        }

        bool begin_unload(ModuleRecord& record, const ModuleEnvironment& environment) override;

        bool finish_unload(ModuleRecord& record, const ModuleEnvironment& environment) override;

        ModuleCleanResult clean(ModuleRecord& record, const ModuleEnvironment& environment) override;
        ...
        std::filesystem::path next_shadow_path(const std::filesystem::path& artifact_path,
                                               const ModuleEnvironment& environment,
                                               std::string& error);
        bool stage_sibling_libraries(ModuleRecord& record,
                                     const std::filesystem::path& artifact_path,
                                     const std::filesystem::path& load_dir);
        bool remove_shadow_artifacts(ModuleRecord& record, const std::filesystem::path& path);
        bool run_build_command(const std::string& module_id,
