...
с конкретной регистрацией. При unload backend сначала вызывает owner cleanup,
затем удаляет claims и evict-ит принадлежащие транзакции exact entries из
`sys.modules`.
...
Legacy unowned incremental registrations допустимы только для встроенного кода,
который ещё не переведён на descriptors. Project C++ modules, участвующие в hot
reload, обязаны передавать `host->module_id` в каждый descriptor; ownerless
descriptor отклоняется до публикации, иначе runtime не смог бы гарантированно
снять factory/accessor callbacks перед `dlclose`/`FreeLibrary`.
