...
namespace {
    ...
    public:
        ...
        bool prepare_environment(const std::vector<ModuleRecord>&,
    ...
    std::filesystem::path write_shadow_test_descriptor(const std::filesystem::path& project_root,
        ...
        std::filesystem::copy_file(
            TERMIN_MODULES_TEST_SHADOW_MODULE, artifact, std::filesystem::copy_options::overwrite_existing);
        const std::filesystem::path dependency_source = TERMIN_MODULES_TEST_SHADOW_DEPENDENCY;
        std::filesystem::copy_file(dependency_source,
                                   artifact.parent_path() / dependency_source.filename(),
                                   std::filesystem::copy_options::overwrite_existing);
        write_text_file(project_root / (module_id + ".module"),
                        "name: " + module_id + "\nbuild:\n  output: " + output + "\n");
        return artifact;
    }

    std::filesystem::path write_native_abi_descriptor(const std::filesystem::path& project_root,
                                                      const std::string& module_id,
                                                      const std::filesystem::path& source_artifact) {
        const std::filesystem::path artifact = project_root / "build" / source_artifact.filename();
        std::filesystem::create_directories(artifact.parent_path());
        std::filesystem::copy_file(source_artifact, artifact, std::filesystem::copy_options::overwrite_existing);
        write_text_file(project_root / (module_id + ".module"),
    ...
    std::shared_ptr<FakeCppBackend> make_runtime(ModuleRuntime& runtime, std::vector<ModuleEvent>* events = nullptr) {
        ...
        if (events != nullptr) {
            runtime.set_event_callback([events](const ModuleEvent& event) { events->push_back(event); });
        }
        return backend;
    }
    ...
    void test_rebuild_distinguishes_no_clean_step_from_clean_failure() {
        TempDir tmp;
        write_text_file(tmp.path / "native.module",
                        "name: native\n"
                        "build:\n"
    ...
    void test_cpp_rebuild_ignores_generated_native_outputs() {
        ...
        std::filesystem::last_write_time(generated_extension, baseline + std::chrono::minutes(90));

        ModuleRuntime runtime;
        runtime.set_environment(ModuleEnvironment{});
        runtime.register_backend(std::make_shared<CppModuleBackend>());
        expect(runtime.discover(tmp.path), runtime.last_error());
        expect(!runtime.needs_rebuild("native"),
    ...
    void test_descriptor_parsing_and_discovery() {
        ...
        write_text_file(tmp.path / "alpha.module",
                        "name: alpha\n"
                        "build:\n"
                        "  command:\n"
        ...
        write_text_file(tmp.path / "beta.pymodule",
                        "name: beta\n"
                        "root: Scripts\n"
                        "packages: [Scripts]\n"
                        "requirements: [pytest]\n");
    ...
    void test_discovery_events_observe_committed_descriptor_batch() {
        TempDir tmp;
        write_text_file(tmp.path / "a_dependent.module",
                        "name: dependent\n"
                        "dependencies: [core]\n"
                        "build:\n"
                        "  output: build/libdependent.so\n");
        write_text_file(tmp.path / "z_core.module",
                        "name: core\n"
                        "build:\n"
        ...
        runtime.set_event_callback([&](const ModuleEvent& event) {
            ...
            ++discovered_count;
            complete_graph_visible = complete_graph_visible && runtime.find("dependent") != nullptr &&
                                     runtime.find("core") != nullptr && runtime.list().size() == 2;
    ...
    void test_selected_closure_is_deterministic_and_mixed() {
        ...
        write_text_file(tmp.path / "physics.module", "name: physics\nbuild:\n  output: build/libphysics.so\n");
    ...
    void test_descriptor_rejects_explicit_components() {
        ...
        write_text_file(tmp.path / "legacy.pymodule",
                        "name: legacy\n"
                        "root: .\n"
                        "packages: [legacy]\n"
    ...
    void test_load_order_and_unload_guard() {
        ...
        write_text_file(tmp.path / "core.module",
                        "name: core\n"
                        "build:\n"
                        "  output: build/libcore.so\n");

        write_text_file(tmp.path / "game.module",
                        "name: game\n"
                        "dependencies: [core]\n"
                        "build:\n"
    ...
    void write_dependency_chain(TempDir& tmp) {
        ...
        write_text_file(tmp.path / "ui.module",
                        "name: ui\n"
                        "dependencies: [game]\n"
                        "build:\n"
    ...
    void test_single_reload_crosses_completed_unload_before_replacement_load() {
        TempDir tmp;
        write_text_file(tmp.path / "sample.module", "name: sample\nbuild:\n  output: build/libsample.so\n");
        ...
        expect(backend->operation_order == std::vector<std::string>{"unload:sample", "load:sample"},
    ...
    void test_cascade_unload_failure_prevents_every_replacement_load() {
        ...
        expect(backend->unload_calls == std::vector<std::string>{"ui", "game"},
               "cascade must stop its unload tail at the failing dependent");
        expect(runtime.find("ui")->state == ModuleState::Unloaded, "already cleaned dependent stays unloaded");
    ...
    void test_failed_replacement_handle_is_cleaned_before_reporting_failure() {
        ...
        expect(backend->operation_order == std::vector<std::string>{"unload:sample", "load:sample", "unload:sample"},
    ...
    void test_python_unload_prepare_failure_preserves_loaded_handle() {
        TempDir tmp;
        write_text_file(tmp.path / "sample.pymodule", "name: sample\nroot: Scripts\npackages: [sample]\n");
        write_text_file(tmp.path / "consumer.pymodule",
                        "name: consumer\nroot: Scripts\npackages: [consumer]\ndependencies: [sample]\n");
        ...
        expect(runtime.find("sample")->handle == loaded_handle, "Python backend commit failure must retain the handle");
        ...
        expect(events.back().kind == ModuleEventKind::Unloaded, "successful cleanup retry must publish Unloaded");
    ...
    void test_cpp_unload_prepare_failure_stays_loaded() {
        ...
        retry_callbacks.before_unload = [](const ModuleRecord&, std::string&) {
            return true;
        };
    ...
    void test_live_mutation_accepts_non_creator_thread() {
        ...
        std::thread worker([&runtime, &worker_result]() { worker_result = runtime.load_module("guarded"); });
        worker.join();
    ...
    void test_cycle_detection() {
        ...
        write_text_file(tmp.path / "a.module",
                        "name: a\n"
                        "dependencies: [b]\n"
                        "build:\n"
                        "  output: build/liba.so\n");

        write_text_file(tmp.path / "b.module",
                        "name: b\n"
                        "dependencies: [a]\n"
                        "build:\n"
                        "  output: build/libb.so\n");
    ...
    void test_missing_dependency() {
        ...
        write_text_file(tmp.path / "broken.module",
                        "name: broken\n"
        ...
        expect(runtime.last_error().find("Missing dependency 'missing'") != std::string::npos,
               "missing dependency error expected");
    ...
    void test_cpp_backend_clean_process_validation_reports_unresolved_symbol() {
        ...
        write_text_file(tmp.path / "broken_native.cpp",
        ...
        write_text_file(tmp.path / "broken_native.module",
                        "name: broken_native\n"
                        "build:\n"
                        "  command:\n"
                        "    linux: mkdir -p build && " +
                            compiler +
                            " -shared -fPIC broken_native.cpp -o build/libbroken_native.so\n"
                            "  output:\n"
        ...
        expect(record->error_message.find("Native artifact validation failed") != std::string::npos,
               "validation failure should be reported");
        expect(record->error_message.find("missing_termin_module_symbol") != std::string::npos ||
    ...
    void test_discovery_ignores_dist_directory() {
        ...
        write_text_file(tmp.path / "chess.pymodule",
                        "name: chess\n"
                        "root: .\n"
                        "packages: [Scripts]\n");

        write_text_file(tmp.path / "dist" / "Chess" / "chess.pymodule",
                        "name: chess\n"
                        "root: .\n"
                        "packages: [Scripts]\n");
    ...
    void test_discovery_ignores_configured_roots() {
        ...
        write_text_file(tmp.path / "game.pymodule",
                        "name: game\n"
                        "root: .\n"
                        "packages: [Scripts]\n");

        write_text_file(tmp.path / "tests" / "test_game.pymodule",
                        "name: test_game\n"
                        "root: .\n"
                        "packages: [tests]\n");
    ...
    void test_staged_cpp_finish_retry_does_not_repeat_completed_phases() {
        ...
        expect(runtime.find("native")->state == ModuleState::CleanupFailed, "finish failure must be non-active");
    ...
    void test_discovery_rejects_active_handles_without_losing_records() {
        ...
        write_text_file(second.path / "replacement.module",
    ...
    void test_cpp_backend_cleans_shadow_artifacts_after_repeated_unload() {
        ...
        for (size_t iteration = 0; iteration < 3; ++iteration) {
            ...
            expect(handle->loaded_path.string().starts_with(shadow_root.string()),
                   "shadow must live under configured root");
            expect(std::filesystem::exists(handle->loaded_path), "shadow file should exist while loaded");
#ifdef _WIN32
            expect(count_regular_files(handle->loaded_path.parent_path()) == 1,
    ...
    void test_cpp_reload_removes_old_shadow_before_publishing_replacement() {
        ...
        const auto old_handle = std::dynamic_pointer_cast<CppModuleHandle>(runtime.find("shadow_test")->handle);
        ...
        const auto replacement_handle = std::dynamic_pointer_cast<CppModuleHandle>(runtime.find("shadow_test")->handle);
        expect(replacement_handle != nullptr, "replacement native shadow handle should be available");
        expect(replacement_handle->loaded_path != old_path, "replacement must use a distinct native shadow path");
        expect(!std::filesystem::exists(old_path), "old native shadow must be removed before reload completes");
    ...
    void test_native_abi_rejects_missing_descriptor_before_init_scope() {
        TempDir tmp;
        write_native_abi_descriptor(tmp.path, "abi_missing", TERMIN_MODULES_TEST_ABI_MISSING);
    ...
    void test_native_abi_rejects_version_mismatch_before_init_scope() {
        TempDir tmp;
        write_native_abi_descriptor(tmp.path, "abi_mismatch", TERMIN_MODULES_TEST_ABI_MISMATCH);
    ...
    void test_native_abi_propagates_structured_init_failure_once() {
        TempDir tmp;
        write_native_abi_descriptor(tmp.path, "abi_init_failure", TERMIN_MODULES_TEST_ABI_INIT_FAILURE);
        ...
        expect(runtime.find("abi_init_failure")->handle == nullptr, "failed init must not publish native handle");
    }
    ...
    void test_native_init_failure_retains_library_until_owner_cleanup_completes() {
        ...
        runtime.register_backend(std::make_shared<CppModuleBackend>());
        CppModuleCallbacks callbacks;
        callbacks.after_failed_load = [](const ModuleRecord&, const std::string&, std::string& error) {
            error = "injected failed-load contribution cleanup failure";
            return false;
        };
        callbacks.before_native_close = [](const ModuleRecord&, std::string& error) {
        ...
        expect(std::filesystem::exists(retained_path), "native shadow must stay present while its library is retained");
    ...
    void test_native_abi_shutdown_failure_is_retryable_and_metadata_is_exposed() {
        TempDir tmp;
        write_native_abi_descriptor(tmp.path, "abi_shutdown_retry", TERMIN_MODULES_TEST_ABI_SHUTDOWN_RETRY);
        ...
        auto handle = std::dynamic_pointer_cast<CppModuleHandle>(runtime.find("abi_shutdown_retry")->handle);
        ...
        expect(!runtime.unload_module("abi_shutdown_retry"), "fallible shutdown must keep library loaded on failure");
        expect(runtime.find("abi_shutdown_retry")->handle != nullptr, "failed shutdown must preserve retryable handle");
        expect(runtime.last_error().find("status 23") != std::string::npos,
               "shutdown failure should preserve structured status");
        expect(runtime.unload_module("abi_shutdown_retry"), "idempotent shutdown retry should complete unload");
...
TEST_CASE("module text diagnostics are sanitized to utf8") {
    ...
    expect(is_valid_utf8("\xd1\x80\xd1\x83\xd1\x81\xd1\x81\xd0\xba\xd0\xb8\xd0\xb9 "
