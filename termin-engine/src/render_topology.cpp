...
namespace termin {
    ...
    bool RenderTopology::attach_scene(tc_scene_handle scene) {
        ...
        for (size_t i = 0; i < template_count; ++i) {
            ...
            if (previous != candidate_pipelines.end()) {
                tc_log(TC_LOG_ERROR, "[RenderTopology] Duplicate scene pipeline name '%s'", name.c_str());
                tc_pipeline_destroy(pipeline);
                destroy_pipeline_map(candidate_pipelines);
                return false;
            }
        ...
        if (record.attached && record.attachment_context) {
            tc_scene_render_mount_notify_detach(
                scene, reinterpret_cast<const tc_render_attachment_context*>(record.attachment_context.get()));
    ...
    void RenderTopology::detach_scene(tc_scene_handle scene) {
        ...
        attached_scenes_.erase(
            std::remove_if(attached_scenes_.begin(),
    ...
    tc_scene_handle RenderTopology::attached_scene_at(size_t index) const {
        return index < attached_scenes_.size() ? attached_scenes_[index] : TC_SCENE_HANDLE_INVALID;
    }

    tc_pipeline_handle RenderTopology::get_pipeline(tc_scene_handle scene, const std::string& name) const {
        const SceneRecord* record = find_record(scene);
        if (record == nullptr)
    ...
    size_t RenderTopology::pipeline_target_count(tc_scene_handle scene, tc_pipeline_handle pipeline) const {
        const SceneRecord* record = find_record(scene);
        if (!record)
            return 0;
        for (const auto& [name, candidate] : record->pipelines) {
            if (!tc_pipeline_handle_eq(candidate, pipeline))
                continue;
    ...
    tc_viewport_handle RenderTopology::pipeline_target_viewport_at(tc_scene_handle scene,
    ...
    void RenderTopology::set_pipeline_targets(tc_scene_handle scene,
    ...
    const std::vector<std::string>& RenderTopology::get_pipeline_targets(tc_scene_handle scene,
                                                                         const std::string& pipeline_name) const {
        const SceneRecord* record = find_record(scene);
        if (record == nullptr)
            return empty_pipeline_targets;
    ...
    bool RenderTopology::register_render_target(tc_render_target_handle render_target) {
        ...
        auto existing = std::find_if(managed_render_targets_.begin(),
        ...
        if (existing != managed_render_targets_.end())
            return true;
        ...
        if (target_name != nullptr && target_name[0] != '\0') {
            ...
            if (tc_render_target_handle_valid(named_target)) {
                tc_log(TC_LOG_ERROR, "[RenderTopology] Scene already owns render target named '%s'", target_name);
                return false;
            }
        }
    ...
    bool RenderTopology::unregister_render_target(tc_render_target_handle render_target) {
        ...
        for (auto it = scenes_.begin(); it != scenes_.end(); ++it) {
            ...
            auto target =
                std::find_if(targets.begin(), targets.end(), [render_target](tc_render_target_handle candidate) {
                    return same_render_target(candidate, render_target);
                });
    ...
    tc_render_target_handle RenderTopology::find_render_target(tc_scene_handle scene, const std::string& name) const {
        return find_render_target(scene, name.c_str());
    }

    tc_render_target_handle RenderTopology::find_render_target(tc_scene_handle scene, const char* name) const {
        if (!name)
        ...
        for (tc_render_target_handle target : record->render_targets) {
            ...
            if (target_name != nullptr && std::strcmp(name, target_name) == 0)
                return target;
        }
        return TC_RENDER_TARGET_HANDLE_INVALID;
    }
    ...
    tc_render_target_handle RenderTopology::managed_render_target_at(size_t index) const {
        return index < managed_render_targets_.size() ? managed_render_targets_[index]
                                                      : TC_RENDER_TARGET_HANDLE_INVALID;
    }

    const std::vector<tc_render_target_handle>& RenderTopology::render_targets(tc_scene_handle scene) const {
        const SceneRecord* record = find_record(scene);
        return record == nullptr ? empty_render_targets : record->render_targets;
    }

    bool RenderTopology::register_viewport(tc_scene_handle scene,
                                           tc_viewport_handle viewport,
                                           tc_display_handle display,
                                           bool destroy_on_scene_detach) {
        if (!tc_scene_handle_valid(scene) || !tc_viewport_handle_valid(viewport) || !tc_display_alive(display)) {
        ...
        auto existing = std::find_if(
            ...
            [viewport](const ViewportAttachment& attachment) { return same_viewport(attachment.viewport, viewport); });
        if (existing != viewport_attachments_.end()) {
            if (!same_scene(existing->scene, scene) || !tc_display_handle_eq(existing->display, display) ||
        ...
        viewport_attachments_.push_back(ViewportAttachment{scene, viewport, display, destroy_on_scene_detach});
    ...
    bool RenderTopology::unregister_viewport(tc_viewport_handle viewport) {
        ...
        record->viewports.erase(
            std::remove_if(record->viewports.begin(),
    ...
    tc_viewport_handle RenderTopology::find_viewport(tc_scene_handle scene, const std::string& name) const {
        return find_viewport(scene, name.c_str());
    }

    tc_viewport_handle RenderTopology::find_viewport(tc_scene_handle scene, const char* name) const {
        if (!name)
            return TC_VIEWPORT_HANDLE_INVALID;
        for (tc_viewport_handle viewport : viewports(scene)) {
            ...
            if (viewport_name && std::strcmp(name, viewport_name) == 0)
                return viewport;
        }
        return TC_VIEWPORT_HANDLE_INVALID;
    }

    const RenderTopology::ViewportAttachment* RenderTopology::viewport_attachment_at(size_t index) const {
    ...
    tc_display_handle RenderTopology::display_for_viewport(tc_viewport_handle viewport) const {
        auto attachment = std::find_if(
            viewport_attachments_.begin(),
            viewport_attachments_.end(),
            [viewport](const ViewportAttachment& candidate) { return same_viewport(candidate.viewport, viewport); });
        return attachment == viewport_attachments_.end() ? TC_DISPLAY_HANDLE_INVALID : attachment->display;
    }
    ...
    void RenderTopology::clear_all() {
        for (tc_scene_handle scene : attached_scenes_) {
            if (tc_scene_handle_valid(scene)) {
                SceneRecord* record = find_record(scene);
                if (!record || !record->attachment_context)
                    continue;
                tc_scene_render_mount_notify_detach(
                    scene, reinterpret_cast<const tc_render_attachment_context*>(record->attachment_context.get()));
                record->attachment_context->invalidate();
