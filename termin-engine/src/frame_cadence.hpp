...
namespace termin::engine_detail {
    ...
    inline FrameCadenceObservation observe_frame_start(double start_time_ms,
        ...
        if (target_interval_ms > 0.0) {
            result.missed_intervals = static_cast<int>(std::floor(result.deadline_lateness_ms / target_interval_ms));
        }
        const double resynchronized_start = result.deadline_lateness_ms > 0.0 ? start_time_ms : scheduled_start_time_ms;
