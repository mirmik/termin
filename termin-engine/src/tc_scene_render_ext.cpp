...
namespace termin {
    ...
    TcSceneRef create_scene_with_extensions(const std::string& name,
    ...
    TcSceneRef deserialize_scene_with_render(const nos::trent& data, const std::string& name) {
        ...
        if (!scene.valid()) {
            tc::Log::error("[deserialize_scene_with_render] failed to create render-enabled scene '%s'", name.c_str());
            return scene;
        }
