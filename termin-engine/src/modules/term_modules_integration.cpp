...
namespace termin {
    namespace {
        ...
        std::vector<std::string> module_pass_types_for_owner(const std::string& module_id) {
            ...
            for (size_t i = 0; i < count; ++i) {
                ...
                const char* owner = type_name ? tc_runtime_type_registry_get_owner(type_name) : nullptr;
        ...
        bool prepare_component_unload_for_runtime_type(const char* type_name, void* context, void*) {
            ...
            if (!unload_context->component_batch_attempted) {
                ...
                const std::vector<std::string> type_names = ComponentRegistry::instance().list_owned(
                    unload_context->module_id ? unload_context->module_id : "");
                UnknownComponentDegradationPlan plan;
                std::string error;
                unload_context->component_batch_ok =
                    prepare_registered_components_to_unknown(type_names, plan, &error) && plan.commit(&error);
                if (!unload_context->component_batch_ok) {
                    tc::Log::error("TermModulesIntegration: failed to prepare component batch for module '%s': %s",
            ...
            if (remaining > 0) {
                ...
                tc::Log::error("TermModulesIntegration: component type '%s' still has %zu live instance(s) after "
                               "prepare-unload for module '%s'",
                               type_name,
                               remaining,
                               unload_context->module_id ? unload_context->module_id : "<unknown>");
            }
        ...
        bool prepare_pass_unload_for_runtime_type(const char* type_name, void* context, void*) {
            auto* unload_context = static_cast<RuntimeUnloadContext*>(context);
            if (!type_name)
                return false;
            ...
            if (!unload_context || !unload_context->sync_live_scenes) {
                tc::Log::error("TermModulesIntegration: refusing to unload pass type '%s' with %zu live "
            ...
            if (!unload_context->pass_batch_attempted) {
                ...
                const std::vector<std::string> type_names =
                    module_pass_types_for_owner(unload_context->module_id ? unload_context->module_id : "");
                UnknownPassDegradationPlan plan;
                std::string error;
                unload_context->pass_batch_ok =
                    prepare_passes_to_unknown(type_names, plan, &error) && plan.commit(&error);
                if (!unload_context->pass_batch_ok) {
                    tc::Log::error("TermModulesIntegration: failed to prepare pass batch for module '%s': %s",
                                   unload_context->module_id ? unload_context->module_id : "<unknown>",
                                   error.c_str());
                    return false;
                }
            ...
            if (remaining > 0) {
                ...
                tc::Log::error("TermModulesIntegration: pass type '%s' still has %zu live "
        ...
        void rollback_module_placeholders(const termin_modules::ModuleRecord& record) {
            ...
            if (!component_types.empty()) {
                const UnknownComponentStats stats = upgrade_registered_unknown_components(component_types);
                if (stats.failed > 0) {
                    tc::Log::error("TermModulesIntegration: rollback failed for %zu component placeholder(s) "
            ...
            if (pass_stats.failed > 0) {
                tc::Log::error("TermModulesIntegration: rollback failed for %zu pass placeholder(s) after "
        ...
        bool cleanup_module_registrations(const termin_modules::ModuleRecord& record,
                                          std::string& error,
                                          bool sync_live_scenes) {
            error.clear();

            try {
                RuntimeUnloadContext context{sync_live_scenes, record.spec.id.c_str()};
                const size_t type_count =
                    tc_runtime_type_registry_unregister_owner_with_context(record.spec.id.c_str(), &context);
                const std::vector<std::string> remaining_types = module_runtime_types(record);
                if (!remaining_types.empty()) {
                    if (sync_live_scenes)
                        rollback_module_placeholders(record);
                ...
                if (type_count > 0) {
                    tc::Log::info("TermModulesIntegration: owner='%s' participant='runtime-types' "
        ...
        bool prepare_module_registration_unload(const termin_modules::ModuleRecord& record,
                                                std::string& error,
                                                bool sync_live_scenes) {
            error.clear();
            try {
        ...
        void upgrade_module_components(const termin_modules::ModuleRecord& record) {
            ...
            const UnknownComponentStats stats = upgrade_registered_unknown_components(type_names);
            if (stats.failed > 0) {
                tc::Log::error(
        ...
        bool upgrade_module_passes(const termin_modules::ModuleRecord& record, std::string* error = nullptr) {
            ...
            tc::Log::error("TermModulesIntegration: failed to upgrade %zu unknown pass instance(s) for module '%s'",
                           stats.failed,
                           record.spec.id.c_str());
            return false;
        }
    ...
    void TermModulesIntegration::configure_runtime(termin_modules::ModuleRuntime& runtime) const {
        ...
        tc_component_registry_set_prepare_unload_callback(prepare_component_unload_for_runtime_type, nullptr);
        tc_pass_registry_set_prepare_unload_callback(prepare_pass_unload_for_runtime_type, nullptr);

        auto cpp_before_unload = [sync_live_scenes](const termin_modules::ModuleRecord& record, std::string& error) {
        ...
        cpp_callbacks.after_failed_load =
            [](const termin_modules::ModuleRecord& record, const std::string&, std::string& error) {
                return cleanup_module_registrations(record, error, false);
            };
        cpp_callbacks.before_unload = cpp_before_unload;
        cpp_callbacks.before_native_close = [sync_live_scenes](const termin_modules::ModuleRecord& record,
                                                               std::string& error) {
        ...
        python_callbacks.after_failed_load =
            [](const termin_modules::ModuleRecord& record, const std::string&, std::string& error) {
                return cleanup_module_registrations(record, error, false);
            };
        python_callbacks.before_module_remove = [sync_live_scenes](const termin_modules::ModuleRecord& record,
                                                                   std::string& error) {
            return prepare_module_registration_unload(record, error, sync_live_scenes);
        };
