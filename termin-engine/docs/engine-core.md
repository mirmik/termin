...
обновлениями отдельных сцен. Устаревших режимов `profile_components` и
`detailed_rendering` нет: детализация задаётся явными стабильными секциями в
владельцах подсистем, чтобы профиль не менял структуру от скрытого глобального
флага.
