...
int main() {
    ...
    if (!close_to(first.interval_ms, 0.0) || !close_to(first.deadline_lateness_ms, 0.0) ||
    ...
    if (!close_to(late.interval_ms, 35.0) || !close_to(late.deadline_lateness_ms, 19.0) || late.missed_intervals != 1 ||
    ...
    if (!close_to(unlimited.interval_ms, 0.5) || !close_to(unlimited.target_interval_ms, 0.0) ||
        unlimited.missed_intervals != 0 || !close_to(unlimited.next_scheduled_start_ms, 151.5)) {
