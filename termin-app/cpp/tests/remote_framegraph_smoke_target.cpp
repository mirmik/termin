...
public:
    ...
    void execute(termin::ExecuteContext& context) override {
        ...
        if (!context.ctx2 || output == context.tex2_writes.end() ||
            !output->second) {
...
public:
    explicit SmokeScene(termin::RenderingManager& manager)
        : manager_(manager) {
        scene_ = tc_scene_new();
        const tc_entity_pool_handle scene_pool = tc_entity_pool_registry_find(
            tc_scene_entity_pool(scene_));
        ...
        tc_render_target_set_camera(
            render_target_, camera_->tc_component_ptr());
...
int run_target(std::uint16_t port, const std::string& token) {
    ...
    try {
        graphics_host = tgfx::GraphicsHost::create_application(
    ...
    termin::framegraph_remote_target::RemoteFrameGraphTarget target(
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (saw_client && highest_session >= 2 &&
            !status.client_connected && status.completed_captures >= 5 &&
            status.preview_captures >= 2 && status.burst_captures >= 3) {
            std::cout
                << "COMPLETE captures=" << status.completed_captures
                << " preview=" << status.preview_captures
                << " burst=" << status.burst_captures
                << " bytes=" << status.transmitted_bytes
                << " outbound_drops=" << status.dropped_outbound_messages
                << " capture_drops=" << status.dropped_captures
                << " capture_ms="
                << static_cast<double>(status.capture_time_ns) / 1'000'000.0
                << " readback_ms="
                << static_cast<double>(status.readback_time_ns) / 1'000'000.0
                << " conversion_ms="
                << static_cast<double>(status.conversion_time_ns) / 1'000'000.0
                << " encode_ms="
                << static_cast<double>(status.transfer_encode_time_ns) /
                       1'000'000.0
                << " effective_preview_fps="
                << static_cast<double>(status.effective_preview_millifps) /
                       1000.0
                << '\n';
            target.stop();
            return 0;
    ...
    const auto status = target.status();
    std::cerr << "Remote framegraph target timed out: session="
              << highest_session << " connected=" << status.client_connected
              << " captures=" << status.completed_captures
              << " preview=" << status.preview_captures
              << " burst=" << status.burst_captures
              << " outbound_drops=" << status.dropped_outbound_messages
              << " capture_drops=" << status.dropped_captures << '\n';
