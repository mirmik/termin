...
remote_message(std::uint64_t sequence, std::uint64_t session,
               termin::profiler_remote::Message message) {
  return {{termin::profiler_remote::protocol_major,
           termin::profiler_remote::protocol_minor,
           termin::profiler_remote::message_type(message), 0, 0, sequence,
           session},
          std::move(message)};
}
...
TEST_CASE("Recorded remote protocol replay projects frames and acknowledged "
  ...
  auto source = std::make_unique<termin::RemoteFrameProfilerSource>(
      2, [&sent_controls](const Control &control) {
  ...
  REQUIRE(source_view->ingest(remote_message(
      2, 77, DictionaryAdd{{{1, "Host Frame"}, {2, "Present"}}})));
  ...
  REQUIRE(source_view->ingest(
      remote_message(4, 77, DropEvent{DropKind::producer_queue, 1, 2, 3})));
  FrameBatch latest;
  latest.frames = {{.frame_number = 104,
                    .interval_ms = 40.0,
                    .active_ms = 8.0,
                    .target_interval_ms = 16.0}};
  ...
  CHECK(controller.timeline_model()->samples()[1].gap_before);
  CHECK(controller.timeline_model()->samples()[1].hitch);
  CHECK(controller.status_model()->text().find("Android / arm64-v8a") !=
        std::string::npos);
  CHECK(controller.status_model()->text().find("transport drops: 3") !=
        std::string::npos);
  ...
  REQUIRE(source_view->ingest(
      remote_message(6, 77,
                     Status{sent_controls[0].request_id, true, false, 3, 2, 0,
                            0, "capture started"})));
  REQUIRE(controller.update());
  CHECK(controller.capturing());
  ...
  malformed.frames = {{.frame_number = 105,
                       .sections_profiled = true,
                       .sections = {{999, 1.0, 0.0, 1, -1, -1, -1}}}};
  ...
  CHECK(disconnected->gaps.back().kind ==
        termin::FrameProfilerGapKind::Disconnect);
...
TEST_CASE("Live remote source controls target and inspects an exact frame") {
  ...
  REQUIRE(wait_until([&] {
    ...
    return snapshot->status.capturing &&
           snapshot->status.profiling_sections;
  }));
  ...
  CHECK_EQ(controller.timeline_model()->samples().front().stable_id,
           exact_frame);
  REQUIRE(controller.select_frame(exact_frame));
  CHECK_EQ(controller.section_model()->size(), 2);
  CHECK(controller.detail_model()->text().find(
            "Frame " + std::to_string(exact_frame)) != std::string::npos);
