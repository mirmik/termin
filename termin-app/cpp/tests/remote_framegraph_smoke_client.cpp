...
bool status_is(const ObservedState& state, std::uint64_t request_id,
               StatusCode code) {
    const auto found = state.status_codes.find(request_id);
    return found != state.status_codes.end() && found->second.contains(code);
}

Command topology_command(std::uint64_t request_id, CommandKind kind,
...
void observe_message(ObservedState& observed,
                     const DecodedMessage& decoded) {
    const bool slow_capture =
        std::holds_alternative<CaptureMetadata>(decoded.message) ||
    ...
    if (const auto* hello = std::get_if<TargetHello>(&decoded.message)) {
    ...
    } else if (const auto* topology =
                   std::get_if<TopologySnapshot>(&decoded.message)) {
        ...
        observed.status_codes[status->request_id].insert(status->code);
                   ...
                   std::get_if<CaptureMetadata>(&decoded.message)) {
        ...
        if (found == observed.blobs.end()) {
            ...
            if (found->second.received_bytes ==
                found->second.metadata.byte_count) {
                const CaptureMetadata metadata = found->second.metadata;
                if (metadata.kind == CaptureKind::snapshot) {
                    observed.completed_snapshot_requests.insert(
                        metadata.request_id);
                } else if (metadata.kind == CaptureKind::preview) {
                    observed.completed_preview_requests.insert(
                        metadata.request_id);
                } else if (metadata.kind == CaptureKind::burst) {
                    observed.completed_burst_indices[metadata.request_id].insert(
                        metadata.burst_index);
                }
                observed.blobs.erase(found);
            } else if (found->second.received_bytes >
                       found->second.metadata.byte_count) {
...
bool send_and_wait(RemoteFrameGraphClient& client, ObservedState& observed,
                   const Command& command, StatusCode code,
                   std::chrono::milliseconds timeout = 8s) {
    if (!client.send_command(command)) {
        return false;
    }
    return wait_for(observed, [&] {
        return status_is(observed, command.request_id, code) ||
...
int run_client(std::uint16_t port, const std::string& token) {
    ...
    RemoteFrameGraphClient client(
        config,
        [&](const DecodedMessage& decoded) {
            observe_message(observed, decoded);
        },
        [&](std::string detail) {
            std::lock_guard lock(observed.mutex);
    ...
    if (!client.start() || !wait_for(observed, [&] {
            return observed.hello.has_value() || !observed.error.empty();
        })) {
    ...
    if (!client.send_command(refresh) || !wait_for(observed, [&] {
            return (observed.topology.has_value() &&
                    status_is(observed, 1, StatusCode::completed)) ||
                   !observed.error.empty();
        })) {
    ...
    std::uint64_t first_session = 0;
    {
        ...
        if (!observed.error.empty() || topology.targets.empty() ||
            topology.passes.empty() || topology.resources.empty() ||
            topology.selected_target_id == 0) {
            std::cerr << "Target returned an incomplete topology: "
                      << observed.error << '\n';
            client.stop();
            return 1;
        }
    ...
    if (!send_and_wait(
            client, observed, stale, StatusCode::stale_revision)) {
        std::cerr << "Stale revision check timed out\n";
        client.stop();
        return 1;
    }

    Command snapshot = topology_command(
        3, CommandKind::capture_snapshot, topology);
    snapshot.encoding = CaptureEncoding::native_pixels;
    if (!send_and_wait(client, observed, snapshot, StatusCode::accepted) ||
        !wait_for(observed, [&] {
        ...
        })) {
        {
            ...
            std::cerr << "Exact snapshot timed out: error='" << observed.error
                      ...
                      << " completed="
                      << observed.completed_snapshot_requests.contains(3);
    ...
    if (!send_and_wait(client, observed, stream, StatusCode::accepted) ||
        !wait_for(observed, [&] {
            return observed.completed_preview_requests.contains(4) ||
                   !observed.error.empty();
        })) {
    ...
    if (!send_and_wait(client, observed, burst, StatusCode::accepted) ||
        !wait_for(observed, [&] {
            ...
            return (found != observed.completed_burst_indices.end() &&
                    found->second.size() == 3 &&
    ...
    Command cancelled = topology_command(
        7, CommandKind::capture_burst, topology);
    cancelled.encoding = CaptureEncoding::native_pixels;
    cancelled.burst_frames = 6;
    Command cancel;
    cancel.request_id = 8;
    cancel.kind = CommandKind::cancel;
    if (!client.send_command(cancelled) || !client.send_command(cancel) ||
        !wait_for(observed, [&] {
    ...
    Command slow_stream = topology_command(
        9, CommandKind::start_stream, topology);
    ...
    observed.slow_receiver = false;
    if (!wait_for(observed, [&] {
            return observed.drop_events > 0 || !observed.error.empty();
        }, 12s)) {
    ...
    RemoteFrameGraphClient second(
        std::move(config),
        [&](const DecodedMessage& decoded) {
            observe_message(reconnected, decoded);
        });
    if (!second.start() || !wait_for(reconnected, [&] {
            return reconnected.hello.has_value() || !reconnected.error.empty();
        })) {
    ...
    if (!second.send_command(second_refresh) || !wait_for(reconnected, [&] {
            return (reconnected.topology.has_value() &&
                    status_is(reconnected, 12, StatusCode::completed)) ||
    ...
    std::cout << "target=" << hello.platform << '/' << hello.abi
              << " pid=" << hello.process_id
              << " topology_revision=" << topology.graph_revision
              << " targets=" << topology.targets.size()
              << " passes=" << topology.passes.size()
              << " resources=" << topology.resources.size()
              << " drops=" << drops
              << " reconnect_sessions=" << first_session + 1 << '\n';
    return 0;
}
