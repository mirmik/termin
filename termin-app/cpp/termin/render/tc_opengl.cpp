...
#include "tgfx/opengl/opengl_backend.hpp"  // init_opengl() + gpu_ops_impl::register_gpu_ops() (Stage 8.7 kills this)
...
TC_API bool tc_opengl_init(void) {
    ...
    // Register GPU operations vtable for tc_shader / raw-GL dispatch
    termin::gpu_ops_impl::register_gpu_ops();

    // Register termin_mesh GPU ops (upload/draw/delete) so tc_mesh_upload_gpu
    // dispatches through the live vtable rather than a NULL one.
    {
