...
namespace termin {
    ...
    namespace {
        ...
        constexpr const char* SOLID_PRIMITIVE_ENGINE_SHADER_UUID = "termin-engine-solid-primitive";

        struct SolidPushData {
            float u_mvp[16];
            float u_color[4];
        };
        static_assert(sizeof(SolidPushData) == 80, "SolidPushData must be 80 bytes (mat4 + vec4)");
        ...
        // same tgfx2 device.
        SolidPrimitiveRenderer::MeshRes upload_mesh_tgfx2(tgfx::IRenderDevice& device,
            ...
            device.upload_buffer(
                res.vbo,
                std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(indexed_mesh.vertices.data()), vb_desc.size));
            ...
            device.upload_buffer(
                res.ibo,
                std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(indexed_mesh.indices.data()), ib_desc.size));
    ...
    SolidPrimitiveRenderer::~SolidPrimitiveRenderer() {
        if (_device) {
            auto destroy_mesh = [this](MeshRes& m) {
                if (m.vbo)
                    _device->destroy(m.vbo);
                if (m.ibo)
                    _device->destroy(m.ibo);
                m = {};
            };
            destroy_mesh(_torus);
    ...
    bool SolidPrimitiveRenderer::_ensure_initialized(tgfx::IRenderDevice* device) {
        ...
        if (_initialized && _device == device)
            return true;
        ...
        if (_device && _device != device) {
            _initialized = false;
            if (_torus.vbo)
                _device->destroy(_torus.vbo);
            if (_torus.ibo)
                _device->destroy(_torus.ibo);
            if (_cylinder.vbo)
                _device->destroy(_cylinder.vbo);
            if (_cylinder.ibo)
                _device->destroy(_cylinder.ibo);
            if (_cone.vbo)
                _device->destroy(_cone.vbo);
            if (_cone.ibo)
                _device->destroy(_cone.ibo);
            if (_quad.vbo)
                _device->destroy(_quad.vbo);
            if (_quad.ibo)
                _device->destroy(_quad.ibo);
        ...
        if (tc_shader_handle_is_invalid(_shader_handle)) {
            _shader_handle = tgfx::register_builtin_shader_from_catalog(SOLID_PRIMITIVE_ENGINE_SHADER_UUID);
        }
        tc_shader* raw = tc_shader_get(_shader_handle);
        if (!raw || !tc_shader_ensure_tgfx2(raw, device, &_vs, &_fs)) {
            tc::Log::error("[SolidPrimitiveRenderer] failed to load built-in shader '%s'",
        ...
        _torus =
            upload_mesh_tgfx2(*device, build_unit_torus(TORUS_MAJOR_SEGMENTS, TORUS_MINOR_SEGMENTS, TORUS_MINOR_RATIO));
    ...
    void SolidPrimitiveRenderer::begin(
        tgfx::RenderContext2* ctx2, const Mat44f& view, const Mat44f& proj, bool depth_test, bool blend) {
        if (!ctx2) {
    ...
    void SolidPrimitiveRenderer::draw_arrow(const Vec3f& origin,
        ...
        Mat44f shaft_model = compose_trs(origin, rot, Vec3f{shaft_radius, shaft_radius, shaft_length});
        draw_cylinder(shaft_model, color);

        // Head: cone from shaft_end to tip
        Vec3f shaft_end = origin + dir * shaft_length;
        Mat44f head_model = compose_trs(shaft_end, rot, Vec3f{head_radius, head_radius, head_length});
