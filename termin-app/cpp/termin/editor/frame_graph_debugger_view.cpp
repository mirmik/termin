...
namespace termin {
    ...
    namespace {
        ...
        BoxLayout& labeled_row(DocumentBuilder& builder, std::string stable_id, std::string label, Widget& control) {
        ...
        const char* suspend_reason_name(FrameGraphDebuggerSuspendReason reason) {
            switch (reason) {
            case FrameGraphDebuggerSuspendReason::None:
                return "none";
            case FrameGraphDebuggerSuspendReason::TargetRemoved:
                return "target removed";
            case FrameGraphDebuggerSuspendReason::PipelineUnavailable:
                return "pipeline unavailable";
            case FrameGraphDebuggerSuspendReason::TargetNotRenderable:
                return "target not renderable";
            }
            return "unknown";
        }

        tgfx::TextureHandle create_preview_target(tgfx::IRenderDevice& device, uint32_t width, uint32_t height) {
    ...
    FrameGraphDebuggerView::FrameGraphDebuggerView(gui_native::TcDocument document,
                                                   FrameGraphDebugger& debugger,
                                                   std::function<void()> request_render)
        : FrameGraphDebuggerView(
              document, make_local_frame_graph_debugger_source(debugger), std::move(request_render)) {}
    ...
    void FrameGraphDebuggerView::build() {
        ...
        root_ = &builder.make_root<BoxLayout>(Orientation::Vertical, "native-framegraph-debugger");
        ...
        auto& settings = builder.make<BoxLayout>(Orientation::Vertical, "framegraph-settings");
        settings.set_stable_id("editor.framegraph.settings");
        settings.set_spacing(4.0f);

        auto& remote_controls = builder.make<BoxLayout>(Orientation::Horizontal, "framegraph-remote-controls");
        ...
        disconnect_remote_button.set_stable_id("editor.framegraph.remote-disconnect");
        ...
        auto& capture_controls = builder.make<BoxLayout>(Orientation::Horizontal, "framegraph-remote-capture-controls");
        capture_controls.set_stable_id("editor.framegraph.remote-capture-controls");
        ...
        settings.add_fixed_child(labeled_row(builder, "editor.framegraph.target-row", "Target:", *target_combo_),
                                 30.0f);
        ...
        settings.add_fixed_child(labeled_row(builder, "editor.framegraph.mode-row", "Mode:", *mode_combo_), 30.0f);

        inside_panel_ = &builder.make<BoxLayout>(Orientation::Vertical, "framegraph-inside-panel");
        ...
        inside_panel_->add_fixed_child(labeled_row(builder, "editor.framegraph.pass-row", "Pass:", *pass_combo_),
                                       30.0f);
        symbol_combo_ = &builder.make<ComboBox>();
        symbol_combo_->set_stable_id("editor.framegraph.symbol");
        inside_panel_->add_fixed_child(labeled_row(builder, "editor.framegraph.symbol-row", "Symbol:", *symbol_combo_),
                                       30.0f);
        ...
        between_panel_ = &builder.make<BoxLayout>(Orientation::Vertical, "framegraph-between-panel");
        ...
        between_panel_->add_fixed_child(
            labeled_row(builder, "editor.framegraph.resource-row", "Resource:", *resource_combo_), 30.0f);
        ...
        auto& hdr_row = builder.make<BoxLayout>(Orientation::Horizontal, "framegraph-hdr-row");
        ...
        auto& controls = builder.make<BoxLayout>(Orientation::Horizontal, "framegraph-controls");
        ...
        auto& previews = builder.make<BoxLayout>(Orientation::Horizontal, "framegraph-previews");
        ...
        auto make_preview = [&](const char* prefix, bool depth) -> Preview {
            auto& panel = builder.make<BoxLayout>(Orientation::Vertical, (std::string(prefix) + "-panel").c_str());
            ...
            auto& view_controls =
                builder.make<BoxLayout>(Orientation::Horizontal, (std::string(prefix) + "-view-controls").c_str());
            view_controls.set_stable_id(std::string("editor.framegraph.") + prefix + "-controls");
            ...
            sampling.set_stable_id(std::string("editor.framegraph.") + prefix + "-sampling");
            ...
            auto& status = builder.make<StatusBar>("Source: — | Zoom: Fit (100%) | Pixel: —");
            ...
            actual.clicked().connect([this, destination](Button&) {
                ...
                destination->canvas->set_zoom(1.0f,
                                              tc_ui_point{
            ...
            sampling.changed().connect([this, destination](ComboBox&, int index, const std::string&) {
                if (index < 0)
                    return;
                destination->canvas->set_texture_sampling(index == 1 ? TC_UI_TEXTURE_SAMPLING_NEAREST
            ...
            canvas.pointer_input().connect([this, destination](Canvas&, tc_ui_point point, const tc_ui_pointer_event&) {
                if (point.x >= 0.0f && point.y >= 0.0f && point.x < static_cast<float>(destination->width) &&
        ...
        target_combo_->changed().connect([this](ComboBox&, int index, const std::string&) {
            ...
            if (static_cast<size_t>(index) >= snapshot->targets.size() ||
                ...
                tc_log_error("[framegraph-debugger-view] failed to select target at index %d", index);
                return;
            }
        ...
        mode_combo_->changed().connect([this](ComboBox&, int index, const std::string&) {
            if (!updating_) {
                source_->set_mode(index == 0 ? FrameGraphDebuggerMode::InsidePass
        ...
        refresh_stats.clicked().connect([this](Button&) { refresh_info(); });
        connect_remote_button.clicked().connect([this](Button&) {
            ...
            if (parsed.ec != std::errc{} || parsed.ptr != port_text.data() + port_text.size() || port == 0 ||
                ...
                tc_log_error("[framegraph-debugger-view] invalid remote port '%s'", port_text.c_str());
                request_render();
                return;
            }
            connect_remote(static_cast<std::uint16_t>(port), remote_token_input_->text());
        });
        disconnect_remote_button.clicked().connect([this](Button&) { disconnect_remote(); });
        ...
        start_preview.clicked().connect([this](Button&) {
            ...
            const auto fps_result = std::from_chars(fps_text.data(), fps_text.data() + fps_text.size(), fps);
            const auto edge_result = std::from_chars(edge_text.data(), edge_text.data() + edge_text.size(), edge);
            if (fps_result.ec != std::errc{} || fps_result.ptr != fps_text.data() + fps_text.size() ||
                edge_result.ec != std::errc{} || edge_result.ptr != edge_text.data() + edge_text.size() || fps == 0 ||
                fps > 60 || edge == 0 || edge > 4096 || !source_->start_live_preview(fps * 1000, edge)) {
                state_status_->set_text("Live preview requires remote capability, FPS 1..60 and edge 1..4096");
                request_render();
                return;
            }
            refresh_info();
        ...
        capture_burst.clicked().connect([this](Button&) {
            ...
            const auto result = std::from_chars(text.data(), text.data() + text.size(), frames);
            if (result.ec != std::errc{} || result.ptr != text.data() + text.size() || frames < 2 || frames > 16 ||
                !source_->capture_burst(static_cast<std::uint16_t>(frames))) {
                state_status_->set_text("Burst requires remote capability and 2..16 frames");
                request_render();
                return;
            }
            refresh_info();
    ...
    bool FrameGraphDebuggerView::show_resource(const std::string& resource) {
        ...
        if (std::find(resources.begin(), resources.end(), resource) == resources.end()) {
            tc_log_error("[framegraph-debugger-view] cannot select missing resource '%s'", resource.c_str());
            return false;
        }
    ...
    bool FrameGraphDebuggerView::connect_remote(std::uint16_t port, std::string authentication_token) {
        require_open();
        if (port == 0 || authentication_token.empty()) {
            tc_log_error("[framegraph-debugger-view] remote port and token are required");
            return false;
        }
        ...
        if (!remote->connect(std::move(config))) {
            tc_log_error("[framegraph-debugger-view] failed to start remote source");
            return false;
        }
    ...
    bool FrameGraphDebuggerView::use_local() {
        require_open();
        if (!local_source_ || source_ == local_source_)
            return false;
        if (remote_source_)
            remote_source_->disconnect();
    ...
    bool FrameGraphDebuggerView::start_live_preview(std::uint32_t max_millifps, std::uint32_t max_long_edge) {
        require_open();
    ...
    std::shared_ptr<const FrameGraphDebuggerSnapshot> FrameGraphDebuggerView::source_snapshot() const {
        return source_->snapshot();
    }

    void FrameGraphDebuggerView::switch_source(std::shared_ptr<IFrameGraphDebuggerSource> source, bool connect_source) {
        if (!source || source == source_)
            return;
        source_->disconnect();
        release_previews();
        source_ = std::move(source);
        if (active_ && connect_source)
            source_->connect();
        source_->refresh();
        select_initial_values();
        refresh_lists();
    ...
    void FrameGraphDebuggerView::select_initial_values() {
        ...
        if (!snapshot->selected_target_id && !snapshot->targets.empty() &&
            ...
            tc_log_error("[framegraph-debugger-view] failed to select initial target '%s'",
    ...
    void FrameGraphDebuggerView::refresh_selection() {
        const auto snapshot = source_->snapshot();
        updating_ = true;
        mode_combo_->set_selected_index(snapshot->mode == FrameGraphDebuggerMode::InsidePass ? 0 : 1);
    ...
    void FrameGraphDebuggerView::refresh_info() {
        ...
        if (snapshot->source_kind == FrameGraphDebuggerSourceKind::Remote && snapshot->stale) {
            state += " [STALE]";
        }
    ...
    bool FrameGraphDebuggerView::render_preview(tgfx::RenderContext2& context, Preview& preview) {
        const auto snapshot = source_->snapshot();
        const auto& image = preview.force_depth ? snapshot->depth_image : snapshot->main_image;
        ...
        if (!preview.target || preview.width != width || preview.height != height) {
            ...
            if (!preview.target) {
                tc_log_error("[framegraph-debugger-view] failed to create %ux%u preview target", width, height);
                return false;
            }
            preview.width = width;
            preview.height = height;
            preview.canvas->set_texture(preview.target.id,
                                        tc_ui_size{static_cast<float>(width), static_cast<float>(height)});
        }

        const bool rendered = source_->render_image(context,
                                                    preview.target,
                                                    preview.force_depth ? FrameGraphDebuggerImageKind::Depth
    ...
    std::string FrameGraphDebuggerView::refresh_depth(tgfx::IRenderDevice& device) {
        ...
        if (!snapshot->depth_image.available) {
            ...
            int width = 0;
            int height = 0;
            const auto pixels = source_->read_depth_normalized(device, &width, &height);
            text = pixels.empty() ? "No depth data"
                                  : "Depth: " + std::to_string(width) + "x" + std::to_string(height) + " read OK";
        }
    ...
    void FrameGraphDebuggerView::release_preview(Preview& preview) {
        ...
        preview.ready = false;
        preview.has_cursor = false;
        if (preview.canvas && preview.status)
            refresh_preview_status(preview);
    }
    ...
    void FrameGraphDebuggerView::close() {
        ...
        deactivate();
        source_->close();
        if (remote_source_ && remote_source_ != source_)
            remote_source_->close();
        if (local_source_ && local_source_ != source_)
            local_source_->close();
        closed_ = true;
        if (document_.valid() && root_ && tc_ui_document_is_alive(document_.handle(), root_->handle())) {
    ...
    void FrameGraphDebuggerView::require_open() const {
        if (closed_)
            throw std::logic_error("FrameGraphDebuggerView is closed");
        if (!document_.valid()) {
