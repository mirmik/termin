...
namespace termin {
    ...
    void GizmoManager::render(ImmediateRenderer* renderer,
        ...
        if (profile)
            tc_profiler_begin_section("GizmoManager::render");
    ...
    bool GizmoManager::on_mouse_down(const Vec3f& ray_origin, const Vec3f& ray_dir) {
        ...
        _active_gizmo->on_click(_active_collider->id, _has_last_drag_position ? &_last_drag_position : nullptr);
    ...
    void GizmoManager::_update_drag(const Vec3f& ray_origin, const Vec3f& ray_dir) {
        ...
        _active_gizmo->on_drag(_active_collider->id, new_position.value(), delta);
    }

    std::optional<Vec3f> GizmoManager::_project_ray_to_constraint(const Vec3f& ray_origin,
                                                                  const Vec3f& ray_dir,
