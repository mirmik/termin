...
FrameProfilerCapabilities capabilities_for(std::uint64_t remote) {
  ...
  if ((remote & static_cast<std::uint64_t>(Capability::cadence_capture)) != 0)
    result |= static_cast<FrameProfilerCapabilities>(
        FrameProfilerCapability::Capture);
  if ((remote &
       static_cast<std::uint64_t>(Capability::hierarchical_sections)) != 0)
    result |= static_cast<FrameProfilerCapabilities>(
        FrameProfilerCapability::SectionProfiling);
  if ((remote & static_cast<std::uint64_t>(Capability::clear_capture)) != 0)
    result |=
        static_cast<FrameProfilerCapabilities>(FrameProfilerCapability::Clear);
  return result;
}
...
public:
  Impl(std::size_t target_capacity, CommandSender target_sender)
      : capacity(target_capacity), sender(std::move(target_sender)) {
    if (capacity == 0)
  ...
  bool ingest(const profiler_remote::DecodedMessage &decoded) {
    ...
    const bool starts_session =
        std::holds_alternative<profiler_remote::TargetHello>(decoded.message);
    ...
    bool changed = std::visit(
        [&](const auto &message) { return apply(*next, decoded, message); },
        decoded.message);
    if (!changed)
      return false;
  ...
  void disconnected(std::string detail) {
    ...
    next->capabilities = 0;
    if (was_connected)
      next->gaps.push_back({FrameProfilerGapKind::Disconnect, 0, 0, 0,
  ...
  bool connect(profiler_remote::ClientConfig config) {
    ...
    auto next_client = std::make_unique<profiler_remote::RemoteProfilerClient>(
        std::move(config),
        [this](const profiler_remote::DecodedMessage &message) {
          ingest(message);
        },
  ...
  template <typename T>
  bool apply(FrameProfilerSnapshot &, const profiler_remote::DecodedMessage &,
             const T &) {
  ...
  bool apply(FrameProfilerSnapshot &next,
    ...
    next.frames.clear();
    next.gaps.clear();
    next.identity.source_id =
        "remote:" + hello.platform + ":" + std::to_string(hello.process_id);
    next.identity.session_id = std::to_string(decoded.envelope.session_id);
    next.identity.display_name =
        hello.platform + " / " + hello.abi + " / " + hello.build_type;
  ...
  bool apply(FrameProfilerSnapshot &next,
    ...
    for (const auto &entry : dictionary.entries) {
      if (candidate.contains(entry.id)) {
        tc_log_error("remote frame profiler: duplicate dictionary id %u",
                     entry.id);
        return false;
      }
      if (candidate.size() >=
          profiler_remote::WireLimits::max_dictionary_entries) {
  ...
  bool apply(FrameProfilerSnapshot &next,
    ...
    for (const auto &wire : batch.frames) {
      ...
      for (const auto &section : wire.sections) {
        ...
        if (found == names.end()) {
          tc_log_error(
              "remote frame profiler: frame references unknown name %u",
              section.name_id);
          return false;
        }
        frame.sections.push_back({found->second, section.cpu_ms,
                                  section.children_ms, section.call_count,
                                  section.parent_index, section.first_child,
                                  section.next_sibling});
      }
      if (pending_gap_before ||
          (!next.frames.empty() &&
           frame.frame_number != next.frames.back().frame_number + 1) ||
          (!converted.empty() &&
           frame.frame_number != converted.back().frame_number + 1))
    ...
    if (next.frames.size() > capacity) {
      ...
      next.frames.erase(next.frames.begin(),
                        next.frames.begin() +
                            static_cast<std::ptrdiff_t>(overflow));
      next.frames.front().gap_before = true;
      next.status.dropped_frames += overflow;
      next.gaps.push_back({FrameProfilerGapKind::TransportDrop, first, last,
                           overflow, "bounded receiver history overwritten"});
    }
  ...
  bool apply(FrameProfilerSnapshot &next,
    ...
    const auto count = static_cast<std::uint64_t>(gap.last_missing_frame -
                                                  gap.first_missing_frame + 1);
    next.gaps.push_back({gap_kind(gap.kind), gap.first_missing_frame,
                         gap.last_missing_frame, count, "target source gap"});
    return true;
  }
  ...
  bool apply(FrameProfilerSnapshot &next,
    ...
    next.gaps.push_back({FrameProfilerGapKind::TransportDrop, 0, 0,
                         drop.dropped_frames, "target producer queue drop"});
    return true;
  }
  ...
  bool apply(FrameProfilerSnapshot &next,
    ...
    next.status.dropped_frames =
        std::max(next.status.dropped_frames, status.dropped_frames);
  ...
  bool apply(FrameProfilerSnapshot &next,
    ...
    next.status.detail =
        "target error " + std::to_string(error.code) + ": " + error.detail;
    tc_log_error("remote frame profiler: target error %u: %s", error.code,
                 error.detail.c_str());
    return true;
  }
  ...
  std::unordered_map<std::uint64_t, profiler_remote::ControlKind>
      pending_commands;
...
RemoteFrameProfilerSource::RemoteFrameProfilerSource(std::size_t capacity,
                                                     CommandSender sender)
...
std::shared_ptr<const FrameProfilerSnapshot>
RemoteFrameProfilerSource::snapshot() {
  std::lock_guard lock(impl_->mutex);
  return impl_->state;
}
...
bool RemoteFrameProfilerSource::set_include_ui(bool) { return false; }
void RemoteFrameProfilerSource::close() {
  impl_->disconnect_live();
  impl_->disconnected("remote source closed");
}
bool RemoteFrameProfilerSource::ingest(
    const profiler_remote::DecodedMessage &message) {
