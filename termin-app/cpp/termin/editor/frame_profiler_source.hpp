...
constexpr FrameProfilerCapabilities operator|(FrameProfilerCapability left,
                                              FrameProfilerCapability right) {
  return static_cast<FrameProfilerCapabilities>(left) |
         static_cast<FrameProfilerCapabilities>(right);
}

constexpr FrameProfilerCapabilities operator|(FrameProfilerCapabilities left,
                                              FrameProfilerCapability right) {
  return left | static_cast<FrameProfilerCapabilities>(right);
}

constexpr bool has_capability(FrameProfilerCapabilities capabilities,
                              FrameProfilerCapability capability) {
  return (capabilities & static_cast<FrameProfilerCapabilities>(capability)) !=
         0;
}
