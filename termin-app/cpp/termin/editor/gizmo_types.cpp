...
namespace termin {
    ...
    std::optional<float> TorusGeometry::ray_intersect(const Vec3f& o, const Vec3f& d) const {
        ...
        Vec3f rel = o - center, lo{rel.dot(t), rel.dot(b), rel.dot(axis)}, ld{d.dot(t), d.dot(b), d.dot(axis)};
    ...
    std::optional<float> QuadGeometry::ray_intersect(const Vec3f& o, const Vec3f& d) const {
        ...
        return side(p0, p1) && side(p1, p2) && side(p2, p3) && side(p3, p0) ? std::optional<float>(t) : std::nullopt;
    }
    ...
    Vec3f closest_point_on_axis(const Vec3f& o, const Vec3f& d, const Vec3f& p, const Vec3f& a) {
        Vec3f w = p - o;
        float aa = a.dot(a), bb = a.dot(d), cc = d.dot(d), dd = a.dot(w), ee = d.dot(w), den = aa * cc - bb * bb;
        if (std::abs(den) < 1e-10f)
            return p;
        return p + a * ((bb * ee - cc * dd) / den);
    }
    std::optional<Vec3f> ray_plane_intersect(const Vec3f& o, const Vec3f& d, const Vec3f& p, const Vec3f& n) {
        float den = d.dot(n);
        if (std::abs(den) < 1e-6f)
            return std::nullopt;
