...
const FrameProfilerFrame *
FrameProfilerSnapshot::find(std::int64_t frame_number) const {
  const auto found = std::find_if(frames.begin(), frames.end(),
                                  [frame_number](const auto &frame) {
...
public:
  ...
  ~LocalFrameProfilerSource() override { close(); }

  std::uint64_t revision() const override {
    return capture_ ? tc_profiler_capture_revision(capture_)
                    : snapshot_->revision;
  }

  std::shared_ptr<const FrameProfilerSnapshot> snapshot() override {
    if (capture_ &&
        snapshot_->revision != tc_profiler_capture_revision(capture_)) {
      rebuild_snapshot();
    } else if (capture_) {
      refresh_status();
    }
...
private:
  void refresh_status() {
    ...
    current->status.profiling_sections =
        tc_profiler_capture_profiling(capture_);
    current->status.external_profiling =
        tc_profiler_enabled() && !current->status.profiling_sections;
  ...
  void rebuild_snapshot() {
    ...
    next->capabilities = FrameProfilerCapability::Capture |
                         FrameProfilerCapability::SectionProfiling |
                         FrameProfilerCapability::Clear |
                         FrameProfilerCapability::IncludeUi;
    ...
    for (int index = 0; index < count; ++index) {
      ...
      if (!source) {
        tc_log_error("local frame profiler: capture returned null frame at %d",
                     index);
        continue;
      }
      ...
      frame.missed_intervals =
          source->missed_intervals > 0
              ? static_cast<std::uint32_t>(source->missed_intervals)
              : 0;
      ...
      if (!next->frames.empty() &&
        ...
        next->gaps.push_back({
            ...
            static_cast<std::uint64_t>(std::max<std::int64_t>(
                0, frame.frame_number - next->frames.back().frame_number - 1)),
    ...
    next->status.overwritten_frames =
        overwritten > 0 ? static_cast<std::uint64_t>(overwritten) : 0;
...
std::unique_ptr<IFrameProfilerSource>
