...
static bool scan_pick_id_lookup(tc_entity_pool *pool, tc_entity_id id,
                                void *user_data) {
...
static void log_pick_lookup_miss_details(tc_entity_pool *pool, uint32_t pick_id,
                                         const char *context) {
  ...
  tc_log(TC_LOG_WARN,
        ...
        "pick_range=%u..%u entity='%s' entity_id=(%u,%u)",
         context ? context : "pick lookup", pick_id, scan.found ? 1 : 0,
         tc_entity_pool_count(pool), scan.scanned_count, scan.min_pick_id,
         scan.max_pick_id, scan.found_name ? scan.found_name : "<none>",
         scan.found_id.index, scan.found_id.generation);
}

static bool mesh_triangle_indices(const tc_mesh *mesh, uint32_t tri,
                                  uint32_t out[3]) {
...
void EditorInteractionSystem::set_camera_frustum_render_context(
    tc_scene_handle scene, int width, int height) {
...
double EditorInteractionSystem::camera_frustum_aspect_override() const {
  ...
  return static_cast<double>(_camera_frustum_view_width) /
         static_cast<double>(_camera_frustum_view_height);
}
...
void EditorInteractionSystem::_rebuild_component_visual_gizmos(Entity entity) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        entity, component, context, new_gizmos);
    }
...
bool EditorInteractionSystem::_snap_transform_gizmo_target(
  ...
  if (!EditorSnapRegistry::instance().snap(request, result) ||
      !result.success) {
...
bool EditorInteractionSystem::_dispatch_entity_click(
    Vec2f screen, const SurfacePickResult &pick) {
    if (!on_entity_click) {
        return false;
    }
...
bool EditorInteractionSystem::_dispatch_viewport_pointer(
...
bool EditorInteractionSystem::_start_async_surface_pick(Vec2f screen,
  ...
  if (!dev)
    ...
    uint64_t color_request = dev->request_pixel_rgba8(id_tex, fbo.x, fbo.y);
  uint64_t depth_request =
      dev->request_pixel_depth_float(depth_tex, fbo.x, fbo.y);
    if (color_request == 0 || depth_request == 0) {
        if (debug_pick) {
            ...
            tc_log(TC_LOG_WARN,
             ...
             fbo.x, fbo.y, static_cast<unsigned long long>(color_request),
             static_cast<unsigned long long>(depth_request), id_tex.id,
             color_desc.width, color_desc.height,
             static_cast<int>(color_desc.format), depth_tex.id,
...
void EditorInteractionSystem::_poll_async_release_pick() {
  ...
  tc_render_target_handle rt =
    ...
    _async_release_pick.color_ready = dev->poll_pixel_rgba8(
        _async_release_pick.color_request, _async_release_pick.color);
    }
    if (!_async_release_pick.depth_ready) {
    _async_release_pick.depth_ready = dev->poll_pixel_depth_float(
        _async_release_pick.depth_request, &_async_release_pick.depth);
    }
    ...
    if (debug_pick) {
        ...
        tc_log(TC_LOG_INFO,
           ...
           _async_release_pick.fbo.x, _async_release_pick.fbo.y, r, g, b,
           pick_id, _async_release_pick.depth, viewport_scene.index,
           viewport_scene.generation, rt_scene.index, rt_scene.generation);
    }
    SurfacePickResult pick = _surface_from_pick_color_depth(
      _async_release_pick.color, _async_release_pick.depth,
      _async_release_pick.fbo, event.vp);
    _async_release_pick.valid = false;
    ...
    if (debug_pick) {
        tc_log(TC_LOG_INFO,
           ...
           pick.has_world_point ? 1 : 0, pick.has_mesh_hit ? 1 : 0);
    }

    bool click_handled = _dispatch_entity_click(event.screen, pick);
    if (debug_pick) {
    tc_log(TC_LOG_INFO, "[PickingDebug] async release callback: handled=%d",
               click_handled ? 1 : 0);
    }
    if (click_handled) {
        _request_update();
...
void EditorInteractionSystem::_handle_double_click(Vec2f screen,
    ...
    tc_log(TC_LOG_WARN, "[EditorInteractionSystem] cannot focus double-click "
  ...
  OrbitCameraController *orbit =
      camera->entity().get_component<OrbitCameraController>();
    if (!orbit) {
        tc_log(TC_LOG_WARN,
...
Entity
EditorInteractionSystem::_entity_from_pick_color(const float color[4],
                                                 tc_viewport_handle viewport) {
...
SurfacePickResult EditorInteractionSystem::pick_surface_at(
    ...
    if (debug_pick) {
        ...
        tc_log(TC_LOG_INFO,
           ...
           viewport_scene.index, viewport_scene.generation, rt_scene.index,
           rt_scene.generation);
    }
  if (pick_id == 0)
    return result;
        ...
        if (debug_pick) {
            tc_log(TC_LOG_WARN,
             ...
             "failed in viewport scene",
                   pick_id);
      log_pick_lookup_miss_details(pool, static_cast<uint32_t>(pick_id),
...
SurfacePickResult EditorInteractionSystem::_surface_from_pick_color_depth(
  ...
  if (!cxx)
    ...
    const Vec2 reproj_error{reproj_x - static_cast<double>(fbo.x),
    ...
    Vec3 local_direction =
        inverse_mesh_offset.transform_direction(entity_local_direction)
            .normalized();
...
bool EditorInteractionSystem::_screen_to_ray(Vec2f screen,
  ...
  auto [orig, dir] =
      camera->screen_point_to_ray(screen.x, screen.y, vp_x, vp_y, vp_w, vp_h);
