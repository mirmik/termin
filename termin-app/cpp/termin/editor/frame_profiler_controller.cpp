...
namespace termin {
    ...
    namespace {
        ...
        gui_native::CommandData
        command(std::string id, std::string label, bool checkable = false, bool checked = false) {
        ...
        bool has_gap_before(const FrameProfilerSnapshot& snapshot, const FrameProfilerFrame& frame) {
            if (frame.gap_before)
                return true;
            return std::any_of(snapshot.gaps.begin(), snapshot.gaps.end(), [&frame](const FrameProfilerGap& gap) {
                return gap.missing_count > 0 && gap.last_missing_frame == frame.frame_number - 1;
            });
        }
        ...
        FrameStatistics statistics(const FrameProfilerSnapshot& snapshot, double hitch_ratio) {
            ...
            for (const auto& frame : snapshot.frames) {
                ...
                result.max_interval_ms = std::max(result.max_interval_ms, frame.interval_ms);
    ...
    FrameProfilerController::FrameProfilerController(std::unique_ptr<IFrameProfilerSource> source, double hitch_ratio)
        ...
        include_ui_command_ =
            commands_->append(command("include-ui", "Include UI", true, snapshot_->status.include_ui));
    ...
    std::shared_ptr<gui_native::CommandModel> FrameProfilerController::command_model() const {
        return commands_;
    }
    std::shared_ptr<gui_native::FrameTimelineModel> FrameProfilerController::timeline_model() const {
        return timeline_;
    }
    std::shared_ptr<gui_native::TreeTableModel> FrameProfilerController::section_model() const {
        return sections_;
    }
    std::shared_ptr<gui_native::RichTextModel> FrameProfilerController::summary_model() const {
        return summary_;
    }
    std::shared_ptr<gui_native::RichTextModel> FrameProfilerController::detail_model() const {
        return detail_;
    }
    std::shared_ptr<gui_native::RichTextModel> FrameProfilerController::status_model() const {
        return status_;
    }
    ...
    void FrameProfilerController::clear() {
        ...
        const bool session_changed = snapshot_->identity.source_id != observed_source_id_ ||
    ...
    bool FrameProfilerController::connect_remote(std::uint16_t port, std::string authentication_token) {
    ...
    bool FrameProfilerController::update() {
        ...
        observed_revision_ = snapshot_->revision;
        if (follow_latest_) {
            selected_frame_number_ = snapshot_->frames.empty() ? -1 : snapshot_->frames.back().frame_number;
        } else if (selected_frame_number_ >= 0 && !snapshot_->find(selected_frame_number_)) {
            selected_frame_number_ = -1;
        }
        refresh_models();
    ...
    bool FrameProfilerController::select_adjacent_hitch(int direction) {
        if (direction > 0) {
            for (const auto& frame : snapshot_->frames) {
                if (frame_is_hitch(frame, hitch_ratio_) && frame.frame_number > selected_frame_number_) {
                    return select_frame(frame.frame_number);
                }
            }
        } else {
            const auto selected = selected_frame_number_ >= 0 ? selected_frame_number_ : INT64_MAX;
            for (auto iterator = snapshot_->frames.rbegin(); iterator != snapshot_->frames.rend(); ++iterator) {
                if (frame_is_hitch(*iterator, hitch_ratio_) && iterator->frame_number < selected) {
    ...
    void FrameProfilerController::refresh_summary() {
        ...
        const char* profiling_state =
            snapshot_->status.profiling_sections ? "on" : (snapshot_->status.external_profiling ? "external" : "off");
        std::ostringstream text;
        text << (snapshot_->status.capturing ? "Capturing" : "Paused") << " | profiling " << profiling_state << " | "
             << stats.frame_count << " frames | p50 " << milliseconds(stats.interval_p50_ms, 2) << " ms | p95 "
             << milliseconds(stats.interval_p95_ms, 2) << " ms | p99 " << milliseconds(stats.interval_p99_ms, 2)
             << " ms | max " << milliseconds(stats.max_interval_ms, 2) << " ms | hitches " << stats.hitch_count;
        summary_->set_text(text.str());
        status_->set_text(snapshot_->identity.display_name + " / " + snapshot_->identity.session_id +
                          (snapshot_->status.connected ? " | connected" : " | disconnected") +
                          " | History overwritten: " + std::to_string(snapshot_->status.overwritten_frames) +
                          " | transport drops: " + std::to_string(snapshot_->status.dropped_frames) + " | " +
    ...
    void FrameProfilerController::refresh_selected_frame() {
        const FrameProfilerFrame* frame =
            selected_frame_number_ >= 0 ? snapshot_->find(selected_frame_number_) : nullptr;
        if (!frame) {
            sections_->clear();
        ...
        const auto found =
            std::find_if(snapshot_->frames.begin(), snapshot_->frames.end(), [frame](const auto& candidate) {
        ...
        const std::string gap = has_pacing_gap ? milliseconds(pacing_gap_ms) + " ms" : "—";
        detail_->set_html("<b>Frame " + std::to_string(frame->frame_number) + "</b><br>" + "Interval: " +
                          milliseconds(frame->interval_ms) + " ms<br>" + "Active: " + milliseconds(frame->active_ms) +
                          " ms<br>" + "Sections: " + (frame->sections_profiled ? "profiled" : "not profiled") + "<br>" +
                          "Pacing gap: " + gap + "<br>" + "Target: " + milliseconds(frame->target_interval_ms) +
                          " ms<br>" + "Deadline lateness: " + milliseconds(frame->deadline_lateness_ms) + " ms<br>" +
    ...
    void FrameProfilerController::show_section_details(gui_native::TreeTableNodeId node) {
        ...
        if (cells.size() < 6)
            return;
        detail_->set_html("<b>" + cells[0] + "</b><br>Inclusive: " + cells[1] + " ms<br>Self: " + cells[2] +
                          " ms<br>Frame share: " + cells[3] + "<br>Child coverage: " +
                          (cells[4].empty() ? "—" : cells[4]) + "<br>Calls: " + (cells[5].empty() ? "1" : cells[5]));
    }
    ...
    void FrameProfilerController::update_capability_commands() {
        ...
        commands_->set_enabled(capture_command_, has_capability(capabilities, FrameProfilerCapability::Capture));
        commands_->set_enabled(profiling_command_,
                               has_capability(capabilities, FrameProfilerCapability::SectionProfiling));
        commands_->set_enabled(clear_command_, has_capability(capabilities, FrameProfilerCapability::Clear));
        commands_->set_enabled(include_ui_command_, has_capability(capabilities, FrameProfilerCapability::IncludeUi));
        commands_->set_checked(include_ui_command_, snapshot_ && snapshot_->status.include_ui);
    }
