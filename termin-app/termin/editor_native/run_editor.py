...
def _compose_native_editor(
    ...
    platform_stage = session.begin_stage("native platform")
    ...
    window.maximize()
    host = platform_stage.own("native UI host", NativeUiHost(window))
    window_manager = platform_stage.own(
        "native window manager",
        NativeUiWindowManager(host),
...
def init_editor_native(
    ...
    *,
    ...
    return EditorSession.build(compose, failure_injector=_failure_injector)
