"""Toolkit-neutral foliage painting extension and presentation state."""
...
_VALID_MODES = frozenset(("idle", "paint", "erase"))


@dataclass(frozen=True)
class FoliageBrushSnapshot:
    mode: str
    radius: float
    stamp_count: int
    asset_name: str | None
...
class FoliageLayerEditorExtension:
    """Own foliage brush behavior independently from its frontend widgets."""
    ...
    def __init__(self) -> None:
        ...
        self._changed_handler: Callable[[FoliageBrushSnapshot], None] | None = None

    @property
    def snapshot(self) -> FoliageBrushSnapshot:
        handle = self._foliage_handle()
        return FoliageBrushSnapshot(
            mode=self._mode,
            radius=self._radius,
            stamp_count=self._stamp_count,
            asset_name=None if handle is None else handle.name,
            instance_count=0 if handle is None else handle.instance_count,
        )

    def set_changed_handler(
        self,
        handler: Callable[[FoliageBrushSnapshot], None] | None,
    ) -> None:
    ...
    def set_mode(self, mode: str) -> None:
        if mode not in _VALID_MODES:
            log.error(f"[FoliageLayerEditor] invalid brush mode '{mode}'")
            raise ValueError(f"invalid foliage brush mode: {mode}")
        self._mode = mode
    ...
    def change_count(self, delta: int) -> None:
    ...
    def _local_point_from_world(self, point: Vec3) -> Vec3 | None:
        entity = self._entity
    ...
    def _world_point_from_local(self, point: Vec3) -> Vec3 | None:
        entity = self._entity
        if entity is None or not entity.valid():
            return None
    ...
    def _draw_overlay(self) -> None:
        ...
        renderer.line(
            hit.world_point,
            hit.world_point + hit.world_normal * 0.5,
...
def _basis_from_normal(normal: Vec3) -> tuple[Vec3, Vec3]:
    ...
    tangent_a = _normalized(helper.cross(normal))
    tangent_b = _normalized(normal.cross(tangent_a))
